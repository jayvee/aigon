...
// REGRESSION: repair must be exported through the misc wrapper and listed in CLI help.
const commands = createAllCommands();
assert.strictEqual(typeof commands.repair, 'function', 'repair command must be registered');

const miscCommands = require('../../lib/commands/misc').createMiscCommands();
assert.strictEqual(typeof miscCommands.repair, 'function', 'repair must be exported by misc command wrapper');
...
const helpSrc = fs.readFileSync(path.join(__dirname, '../../templates/help.txt'), 'utf8');
assert.ok(/repair <feature\|research> <ID> \[--dry-run\]/.test(helpSrc), 'CLI help must expose repair');
