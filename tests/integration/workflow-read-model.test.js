...
const reviewComplete = { reviewerId: 'cx', reviewCompletedAt: '2026-04-01T10:05:00Z', requestRevision: true, revisionAgentId: 'cc' };
const readyAgents = [{ id: 'cc', status: 'ready' }];
for (const [desc, lifecycle, snapshot, auto, agents, expectedStatuses] of [
    ['running implement', 'implementing', {}, {}, [{ id: 'cc', status: 'implementing' }], ['running', 'waiting', 'waiting', 'waiting']],
    ['running review', 'code_review_in_progress', { mode: 'solo_worktree', agents: { cc: { status: 'ready' } }, codeReview: { reviewerId: 'cx', reviewStartedAt: '2026-04-01T10:03:00Z' } }, { reviewTriggered: true }, [...readyAgents, { id: 'cx', status: 'code_review_in_progress', tmuxRunning: true }], ['complete', 'waiting', 'waiting', 'waiting']],
    ['requested revision', 'implementing', { mode: 'solo_worktree', agents: { cc: { status: 'ready' } }, codeReview: reviewComplete }, { reviewTriggered: true }, readyAgents, ['complete', 'complete', 'waiting', 'waiting']],
    ['running revision', 'implementing', { mode: 'solo_worktree', agents: { cc: { status: 'implementing' } }, codeReview: reviewComplete }, { reviewTriggered: true, feedbackInjected: true }, [{ id: 'cc', status: 'implementing' }], ['complete', 'complete', 'running', 'waiting']],
    ['running eval', 'evaluating', {}, { mode: 'fleet', agents: ['cc', 'cx'], evalAgent: 'gg', evalTriggered: true, workflowSlug: null }, [...readyAgents, { id: 'cx', status: 'ready' }], ['complete', 'running', 'waiting']],
    ['eval complete', 'evaluating', { mode: 'fleet', agents: { cc: { status: 'ready' }, cx: { status: 'ready' } }, winnerAgentId: 'cc' }, { mode: 'fleet', agents: ['cc', 'cx'], evalAgent: 'gg', evalTriggered: true, workflowSlug: null }, [...readyAgents, { id: 'cx', status: 'ready' }], ['complete', 'complete', 'waiting']],
    ['close running', 'closing', { mode: 'solo_worktree', agents: { cc: { status: 'ready' } }, codeReview: { ...reviewComplete, requestRevision: false } }, { closeTriggered: true }, readyAgents, ['complete', 'complete', 'complete', 'running']],
    ['close complete', 'closing', { mode: 'solo_worktree', agents: { cc: { status: 'ready' } }, codeReview: { ...reviewComplete, requestRevision: false } }, { status: 'completed', running: false, closeTriggered: true, endedAt: '2026-04-01T10:30:00Z' }, [{ id: 'cc', status: 'implementation-complete' }], ['complete', 'complete', 'complete', 'complete']],
]) {
    test(`autonomous plan progress index: ${desc}`, () => withTempDir('aigon-rm-', (repo) => {
        seed(repo);
        const id = String(20 + expectedStatuses.join('').length + desc.length);
        writeSpec(repo, 'features', '03-in-progress', `feature-${id}-progress.md`);
        writeSnap(repo, 'features', id, lifecycle, snapshot);
        writeFeatureAuto(repo, id, {
            featureId: id, status: 'running', running: true, mode: 'solo_worktree', stopAfter: 'close',
            agents: ['cc'], reviewAgent: 'cx', workflowSlug: 'solo-cc-reviewed-cx',
            startedAt: '2026-04-01T10:00:00Z', updatedAt: '2026-04-01T10:05:00Z', ...auto,
        });
        ...
        assert.deepStrictEqual(state.autonomousPlan.stages.map(stage => stage.status), expectedStatuses, desc);
        assert.deepStrictEqual(
            state.autonomousPlan.stages.map(stage => stage.type),
            expectedStatuses.length === 3 ? ['implement', 'eval', 'close'] : ['implement', 'review', 'revision', 'close'],
        );
    }));
}
