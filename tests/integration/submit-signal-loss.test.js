...
testAsync('submit signals survive concurrent CLI submits', () => withTempDirAsync('aigon-submit-race-', async (repo) => {
    ...
    const [ccResearch, ggResearch] = await Promise.all([cli(['agent-status', 'implementation-complete', '34', 'cc'], repo), cli(['agent-status', 'implementation-complete', '34', 'gg'], repo)]);
...
testAsync('explicit feature agent-status implementation-complete works from main (no branch evidence gate)', () => withTempDirAsync('aigon-explicit-feat-main-', async (repo) => {
...
testAsync('submit command fails before writing stale cache when engine write fails', () => withTempDirAsync('aigon-submit-fail-', async (repo) => {
    ...
    const result = await cli(['agent-status', 'implementation-complete', '34', 'cc'], repo, {}, preload);
    assert.notStrictEqual(result.status, 0);
    assert.match(result.stderr || result.stdout, /Failed to record '(?:implementation-complete|research-complete)' for research 34 \(cc\): forced signal failure/);
