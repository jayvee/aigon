...
testAsync('storage convert dry-run reports planned config and import keys without writing', async () => {
  // REGRESSION: dry-run must preview config/refs without mutating disk (feature 597 AC).
  await withTempDirAsync('storage-convert-dry-', async (base) => {
    ...
    assert.strictEqual(result.planned.backend, 'git-ref');
...
testAsync('storage convert refuses missing remote unless dry-run', async () => {
  // REGRESSION: convert must fail loudly when remote is absent (feature 597 AC).
...
testAsync('storage convert is idempotent when git-ref config already exists', async () => {
  // REGRESSION: rerunning convert must report already configured (feature 597 AC).
...
testAsync('storage convert accepts URL remote paths', async () => {
  // REGRESSION: remote may be a URL rather than a named remote (feature 597 AC).
...
testAsync('storage convert imports local projections on first enable', async () => {
  // REGRESSION: first convert must import numeric local workflow events (feature 597 AC).
...
testAsync('storage convert rejects non-aigon directories', async () => {
  // REGRESSION: convert must refuse non-Aigon repos (feature 597 AC).
