...
import { renderContractCardBody, renderSetContractCardBody } from './contract-cards/card.js';
    ...
    const AGENT_STATUS_META = {
      ...
      'implementation-complete': { icon: '✓', label: 'Implemented',   cls: 'status-ready' },
      'ready':                { icon: '✓', label: 'Implemented',       cls: 'status-ready' },
      'submitted':            { icon: '✓', label: 'Implemented',       cls: 'status-ready' },
      'feedback-addressed':   { icon: '✓', label: 'Revised',           cls: 'status-ready' },
      'research-complete':    { icon: '✓', label: 'Research ready',    cls: 'status-ready'  },
    ...
    function buildIdleLadderChip(agent) {
      ...
      const suffix = idle.nudged ? ' · nudged' : '';
    ...
    function buildAgentSectionHtml(agent, agentValidActions, feature, repoPath, pipelineType, repoStorage) {
      ...
      const markCompleteItem = (agent.pendingCompletionSignal && !agent.isWorking)
        ...
        : '';
      if (overflowActions.length > 0 || secondaryInfraOverflow || sessionOverflow || markCompleteItem) {
  ...
  /** Fields consumed by buildKanbanCard — per-card fingerprint for surgical updates. */
    ...
    function replaceKeyedCard(parent, key, feature, repo, pType, stats, renderOptions = {}) {
      const map = getKeyedMap(parent);
      const fp = cardFingerprint(feature, repo, pType);
      let node = map.get(key);
      const fresh = buildKanbanCard(feature, repo.path, pType, repo, renderOptions);
    ...
    function reconcileKeyedCards(parent, orderedFeatures, repo, pType, stats, renderOptions = {}) {
      const map = getKeyedMap(parent);
      const seen = new Set();
      ...
      for (let i = orderedFeatures.length - 1; i >= 0; i--) {
        ...
        seen.add(key);
        const node = replaceKeyedCard(parent, key, feature, repo, pType, stats, renderOptions);
