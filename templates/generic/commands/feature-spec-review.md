...
- Do not run `aigon feature-spec-revise {{ARG1_SYNTAX}}` — that is a different command for a later stage, run by the feature author after reviewers have completed their review. It is not your job here.
...
`{{CMD_PREFIX}}feature-spec-revise {{ARG1_SYNTAX}}`
