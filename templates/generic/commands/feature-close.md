...
Example: `aigon feature-close {{ARG1_SYNTAX}} cc` to merge Claude's implementation

### Fleet mode with adoption
Merge the winner and print diffs from losing agents for selective adoption:
```bash
aigon feature-close {{ARG1_SYNTAX}} <winning-agent> --adopt <agents...|all>
```
...
- **GitHub PR-aware close**: If `origin` is GitHub and `gh` is available, close checks for a PR on the feature branch. No PR means normal local close. An open PR blocks close. A merged PR causes Aigon to sync `{{DEFAULT_BRANCH_NAME}}` from `origin/{{DEFAULT_BRANCH_NAME}}`, write the final done-state spec/log commit, push that close-state commit, and clean up the branch/worktree.
...
End your response with the suggested next command on its own line. This helps agent UIs surface the next suggested Aigon command. Use the actual ID/name from the pipeline check above.
