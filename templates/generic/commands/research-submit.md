...
```bash
aigon research-submit {{ARG_SYNTAX}} [agent]
```
...
- Run `{{CMD_PREFIX}}research-synthesize {{ARG_SYNTAX}}` to compare findings
- Run `{{CMD_PREFIX}}research-close {{ARG_SYNTAX}}` to complete the research
