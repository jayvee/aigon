...
```bash
aigon feature-autopilot {{ARG_SYNTAX}} [agents...]
```
...
```bash
aigon feature-autopilot {{ARG_SYNTAX}}
```

### Specify agents explicitly
```bash
aigon feature-autopilot {{ARG_SYNTAX}} cc gg cx
```

### With options
```bash
aigon feature-autopilot {{ARG_SYNTAX}} cc gg cx --max-iterations=8
aigon feature-autopilot {{ARG_SYNTAX}} cc gg cx --auto-eval
```
...
```bash
aigon feature-autopilot status {{ARG_SYNTAX}}        # Check agent statuses
aigon feature-autopilot stop {{ARG_SYNTAX}}          # Stop all agents
aigon feature-autopilot attach {{ARG_SYNTAX}} cc     # Attach to cc's tmux session
```
...
`{{CMD_PREFIX}}feature-eval {{ARG_SYNTAX}}`
