...
- When suggesting what to work on next, reference items by their letter labels (e.g., "You could prioritize item c) create-plugin") and use the agent's native Aigon command format for suggestions (e.g., `{{CMD_PREFIX}}feature-prioritise c`) rather than raw `aigon ...` CLI syntax.
