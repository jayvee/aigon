...
```bash
aigon feature-eval $1 --allow-same-model-judge
```

This will:
- Move the spec to `04-in-evaluation/` (if not already there)
- Create an evaluation template at `./docs/specs/features/evaluations/feature-$1-eval.md`
...
- File: `./docs/specs/features/evaluations/feature-$1-eval.md`
...
2. Review the code changes: `git diff main..feature-$1-*`
...
1. For each agent worktree listed:
   - Read implementation log from the worktree (e.g., `../feature-$1-cc-*/docs/specs/features/logs/feature-$1-cc-*-log.md`)
   - **Examine the actual code changes** in each worktree
   - Run `git diff main..HEAD` in each worktree to see all changes
   - Check spec compliance

2. **Worktree locations:** `../feature-$1-<agent>-*`
...
For example: `$aigon-feature-close $1 cc` if Claude's implementation wins.
