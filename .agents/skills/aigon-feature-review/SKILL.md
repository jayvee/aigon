...
- **IMPORTANT: All commits (fixes + log updates) must go to the WORKTREE, not main.**
  Find the worktree path: `WORKTREE=$(git worktree list | grep "feature-$1" | awk '{print $1}')`
...
```bash
SPEC_PATH=$(aigon feature-spec $1)
cat "$SPEC_PATH"
```
...
- Location: `./docs/specs/features/logs/feature-$1-*-log.md`
...
- In the implementer's session, run `feature-review-check $1` (slash command for cc/gg/cu/mv, skill `$aigon-feature-review-check` for codex) so the implementer reads the review and decides accept/challenge/modify
- Run `$aigon-feature-close $1` when ready to merge
