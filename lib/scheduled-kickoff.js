...
const LABEL_RE = /^[a-z0-9][a-z0-9-]{0,59}$/;
const AGENT_PROMPT_KIND = 'agent_prompt';
const SET_AUTONOMOUS_KIND = 'set_autonomous';
...
function withStoreLockSync(repoPath, work) {
    ...
    // Break stale lock immediately before entering the retry loop — prevents
    // the 8-second busy-wait spin when a crash left a lock file behind.
    ...
    while (Date.now() < deadline) {
        try {
            ...
            // Break stale lock on every retry as well (another process may have
            // just crashed mid-write and left the file behind).
...
function validateAgentPromptPayload(payload, registry) {
    const agentId = String((payload && payload.agentId) || '').trim();
    if (!agentId) return { ok: false, error: 'agentId is required' };
    const knownAgents = new Set(registry.getAllAgentIds());
    ...
    let label = String((payload && payload.label) || '').trim();
    if (label) {
        if (!LABEL_RE.test(label)) {
            return { ok: false, error: 'label must match [a-z0-9-]+ (start with alphanumeric, max 60 chars)' };
        }
    } else {
        label = `prompt-${_shortUuid()}`;
    }

    let cron = null;
    const cronRaw = payload && payload.cron != null ? String(payload.cron).trim() : '';
    if (cronRaw) {
        try {
            cronParse.parseCron(cronRaw);
        } catch (e) {
    ...
    return { ok: true, normalized: { agentId, prompt, label, cron } };
}

function buildAgentPromptArgv(repoPath, normalized) {
    const args = [
        'agent-launch',
...
function validateKnownAgents(agentIds, registry) {
    const available = new Set(registry.getAllAgentIds());
    const unknown = agentIds.filter(a => !available.has(a));
    if (unknown.length > 0) return { ok: false, error: `Unknown agent(s): ${unknown.join(', ')}` };
    return { ok: true };
}

function validateSetAutonomousPayload(payload, registry) {
    const agents = Array.isArray(payload.agents) ? payload.agents.map(v => String(v || '').trim()).filter(Boolean) : [];
    if (agents.length === 0) return { ok: false, error: 'At least one agent is required' };
    const agentCheck = validateKnownAgents(agents, registry);
    if (!agentCheck.ok) return agentCheck;

    const reviewAgent = payload.reviewAgent != null ? String(payload.reviewAgent).trim() : '';
    if (reviewAgent) {
        const reviewCheck = validateKnownAgents([reviewAgent], registry);
        if (!reviewCheck.ok) return reviewCheck;
    }

    const mode = payload.mode != null ? String(payload.mode).trim() : 'sequential';
    if (mode !== 'sequential') return { ok: false, error: `Unsupported mode: ${mode}. Only --mode=sequential is available.` };

    const stopAfter = payload.stopAfter != null ? String(payload.stopAfter).trim() : 'close';
    if (stopAfter !== 'close') return { ok: false, error: `Unsupported --stop-after=${stopAfter}. Only --stop-after=close is available.` };

    const models = payload.models != null ? String(payload.models).trim() : '';
    const efforts = payload.efforts != null ? String(payload.efforts).trim() : '';
    return {
        ok: true,
        normalized: {
            agents,
...
function buildSetAutonomousArgv(setSlug, normalized) {
    const args = ['set-autonomous-start', setSlug, ...normalized.agents];
    if (normalized.reviewAgent) args.push(`--review-agent=${normalized.reviewAgent}`);
    if (normalized.models) args.push(`--models=${normalized.models}`);
    if (normalized.efforts) args.push(`--efforts=${normalized.efforts}`);
    args.push(`--stop-after=${normalized.stopAfter || 'close'}`);
    return args;
}

function assertEntitySchedulable(repoPath, kind, entityId) {
    const id = String(entityId).trim();
    if (kind === AGENT_PROMPT_KIND) {
        // No backing entity — label is a free-form slug.
        if (!id || !LABEL_RE.test(id)) {
            return { ok: false, error: 'label must match [a-z0-9-]+ (start with alphanumeric, max 60 chars)' };
        }
        return { ok: true };
    }
    ...
    if (kind === SET_AUTONOMOUS_KIND) {
        if (!featureSets.isValidSetSlug(id)) {
            return { ok: false, error: `Invalid set slug: "${id}"` };
        }
        const paths = featureSets.featurePathsForRepo(repoPath);
        const members = featureSets.scanFeatureSets(paths).get(id) || [];
        if (members.length === 0) {
            return { ok: false, error: `No features tagged with set: ${id}` };
        }
        const missingIds = members.filter(m => !m.paddedId);
        if (missingIds.length > 0) {
            const labels = missingIds.map(m => m.slug).join(', ');
            return { ok: false, error: `Set members without numeric IDs must be prioritised first: ${labels}` };
        }
        return { ok: true };
    }
...
function addJob(repoPath, jobInput) {
    ...
    let entityId = String(jobInput.entityId || '').trim();
    ...
    if (kind === 'feature_autonomous') {
        ...
        if (!v.ok) return v;
        payload = v.normalized;
    } else if (kind === SET_AUTONOMOUS_KIND) {
        if (!entityId) return { ok: false, error: 'set slug is required' };
        const entityCheck = assertEntitySchedulable(repo, kind, entityId);
        if (!entityCheck.ok) return entityCheck;
        const v = validateSetAutonomousPayload(jobInput.payload || {}, agentRegistry);
        if (!v.ok) return v;
        payload = v.normalized;
    } else if (kind === AGENT_PROMPT_KIND) {
        const v = validateAgentPromptPayload(jobInput.payload || {}, agentRegistry);
        if (!v.ok) return v;
        payload = v.normalized;
        // entityId is the label slug — overwrite with normalised value (handles
        // auto-generated label when caller omitted one).
        entityId = payload.label;
        const entityCheck = assertEntitySchedulable(repo, kind, entityId);
        if (!entityCheck.ok) return entityCheck;
    } else {
        return { ok: false, error: 'kind must be feature_autonomous, set_autonomous, research_start, or agent_prompt' };
    }
...
function buildSpawnArgvForJob(job) {
    ...
    if (job.kind === SET_AUTONOMOUS_KIND) {
        return buildSetAutonomousArgv(job.entityId, job.payload);
    }
    if (job.kind === AGENT_PROMPT_KIND) {
        return buildAgentPromptArgv(job.repoPath, job.payload);
    }
...
function processRepoDueJobs(repoPath, deps) {
    ...
    for (const job of claimed) {
        ...
        // Cron re-enqueue: only on successful agent_prompt fires with a cron set.
        // Failure during re-enqueue leaves the original status untouched and is
        // logged — the poller must not crash if a stored cron is malformed.
        if (run.ok && job.kind === AGENT_PROMPT_KIND && job.payload && job.payload.cron) {
            try {
                const fired = parseRunAt(job.runAt);
                const fromMs = fired.ok ? fired.ms : nowMs;
                const parsed = cronParse.parseCron(job.payload.cron);
                const nextMs = cronParse.nextTime(parsed, fromMs);
                const nextIso = new Date(nextMs).toISOString();
                const enq = addJob(job.repoPath, {
                    kind: AGENT_PROMPT_KIND,
                    entityId: job.entityId,
                    runAt: nextIso,
                    payload: { ...job.payload },
                });
                if (!enq.ok) {
                    log(`[scheduled-kickoff] cron re-enqueue failed for ${job.jobId}: ${enq.error}`);
                }
            } catch (e) {
                log(`[scheduled-kickoff] cron re-enqueue error for ${job.jobId}: ${e && e.message}`);
            }
        }
    }
...
function buildPendingScheduleIndex(repoPath) {
    ...
    for (const j of jobs) {
        ...
        else if (j.kind === SET_AUTONOMOUS_KIND) upsert(setBest, j.entityId, j.runAt, j.kind);
    }
    ...
    return {
        ...
        lookupSet: (slug) => lookup(setBest, slug),
    };
}
...
module.exports = {
    ...
    validateSetAutonomousPayload,
    buildSetAutonomousArgv,
