    ...
    for (;;) {
        ...
        if (state === 'failed') {
            ...
            if (autoReason === 'feature-close-failed'
                ...
                return { status: 'failed', reason: 'criteria-attestation-failed' };
            }
...
function buildPauseNotificationMessage(setSlug, featureId, order, reason) {
    ...
    if (reason === 'criteria-attestation-failed') {
        return `Set \`${setSlug}\` paused at ${featureLabel} — criteria attestation incomplete. Update the implementation log, then \`aigon feature-close ${featureId}\` or \`aigon set-autonomous-resume ${setSlug}\`.`;
    }
