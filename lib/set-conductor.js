...
const { normalizeFeatureId } = require('./auto-session-state');
...
/** True if a *-f{id}-auto tmux session exists (AutoConductor inner loop is live). */
function findAutoConductorTmuxSessionForFeature(featureId) {
    try {
        assertTmuxAvailable();
        ...
        return sessions.find((name) => {
            const parsed = matchTmuxSessionByEntityId(name, featureId);
...
function buildSetRunLoopCommandArgs({ setSlug, mode, stopAfter, sessionName, reviewAgent, explicitAgents, pollSeconds = DEFAULT_POLL_SECONDS, models, efforts }) {
    const args = [
    ...
    if (efforts) args.push(`--efforts=${efforts}`);
    return args;
}
...
function isFeatureSpecInDoneFolder(mainRepoPath, normalizedId) {
    const doneDir = path.join(mainRepoPath, 'docs', 'specs', 'features', STAGE_FOLDERS.DONE);
    if (!fs.existsSync(doneDir)) return false;
    let files;
    try {
        files = fs.readdirSync(doneDir);
    } catch {
        return false;
    }
    const prefix = `feature-${normalizedId}-`;
    return files.some(f => f.startsWith(prefix) && f.endsWith('.md'));
}

/**
 * SetConductor advance guard: true when the feature is fully closed.
 *
 * Engine-first per F397: snapshot.lifecycle === 'done' is authoritative when an
 * engine snapshot exists. Folder position is consulted only as an explicit
 * fallback for pre-engine legacy features (no engine dir on disk). A spec that
 * has drifted out of `05-done/` while the engine still says `done` does NOT
 * stall the set — that was the bug this guard previously caused.
 */
function isFeatureDone(mainRepoPath, featureId) {
    const id = normalizeFeatureId(featureId);
    // 1. Engine snapshot first (uncached so a just-closed feature is observed).
    const snapshot = workflowSnapshotAdapter.readWorkflowSnapshotSyncUncached(mainRepoPath, 'feature', id);
    if (snapshot) {
        const stage = String(snapshot.currentSpecState || snapshot.lifecycle || '').toLowerCase();
        return stage === 'done';
    }
    // 2. No snapshot: only trust 05-done/ for genuine pre-engine features
    //    (no engine dir at all). An engine dir without a snapshot is drift,
    //    not legacy — refuse to advance until it's resolved.
    if (engineDirExists(mainRepoPath, 'feature', id)) return false;
    return isFeatureSpecInDoneFolder(mainRepoPath, id);
}
...
function startFeatureAutonomous(mainRepoPath, featureId, options) {
    ...
    if (featureAuto && featureAuto.running) {
        const idNorm = normalizeFeatureId(featureId);
        if (findAutoConductorTmuxSessionForFeature(idNorm)) {
            return { attached: true };
        }
        // Stale .aigon/state feature-*-auto.json said running but tmux is gone (e.g. bad CLI path) — start fresh.
    }
    ...
    if (options.efforts) args.push(`--efforts=${options.efforts}`);
...
function isStaleFeatureAutoFailure(featureAuto, snapshot) {
    if (!featureAuto || !snapshot || String(featureAuto.status || '') !== 'failed') return false;
    const reason = String(featureAuto.reason || '').trim();
    const staleReasons = new Set(['review-timeout', 'review-exited-without-signal']);
    if (!staleReasons.has(reason)) return false;
    const autoTs = featureAuto.updatedAt || featureAuto.endedAt;
    const reviewCompletedAt = snapshot.codeReview && snapshot.codeReview.reviewCompletedAt;
    if (!autoTs || !reviewCompletedAt) return false;
    if (new Date(autoTs).getTime() >= new Date(reviewCompletedAt).getTime()) return false;
    const lifecycle = snapshot.currentSpecState || snapshot.lifecycle || '';
    return lifecycle === 'ready' || lifecycle === 'code_revision_in_progress' || lifecycle === 'closing' || lifecycle === 'done';
}

const FEATURE_AUTO_QUOTA_PAUSE_REASONS = new Set(['review-quota-paused', 'quota-cap']);

function isFeatureAutoQuotaPause(featureAuto) {
    if (!featureAuto) return false;
    const state = String(featureAuto.status || '').toLowerCase();
    const reason = String(featureAuto.reason || '').trim();
    if (state === 'quota-paused') return true;
    return state === 'stopped' && FEATURE_AUTO_QUOTA_PAUSE_REASONS.has(reason);
}

const SET_PAUSED_STATUSES = new Set(['paused-on-failure', 'paused-on-quota']);

function isSetPausedStatus(status) {
    return SET_PAUSED_STATUSES.has(String(status || ''));
}
...
function waitForFeatureAutonomousResult(mainRepoPath, featureId, pollSeconds) {
    for (;;) {
        ...
        if (state === 'completed' || state === 'done') {
            return { status: 'done', reason: (featureAuto && featureAuto.reason) || state };
        }
        if (state === 'failed') {
            // A feature closed externally (manually or via a separate session) will cause the
            // inner AutoConductor to write status=failed (e.g. feature-close-failed or
            // feature-close-state-timeout) even though the feature actually reached done.
            // Always do a final lifecycle check before propagating failure upward.
            if (isFeatureDone(mainRepoPath, featureId)) {
                return { status: 'done', reason: 'feature-done-externally' };
            }
            const snapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepoPath, 'feature', featureId);
            if (isStaleFeatureAutoFailure(featureAuto, snapshot)) {
                return { status: 'done', reason: 'workflow-advanced-past-failure' };
            }
            const autoReason = (featureAuto && featureAuto.reason) || 'feature-auto-failed';
            const integrityPolicy = resolveCloseIntegrityPolicy(loadProjectConfig(mainRepoPath));
            if (autoReason === 'feature-close-failed'
                && snapshot
                && snapshot.lastCloseFailure
                && snapshot.lastCloseFailure.kind === 'post-merge-gate'
                && isCloseFindingBlocking(integrityPolicy, 'post-merge-gate')) {
                return { status: 'failed', reason: 'post-merge-gate-failed' };
            }
            if (autoReason === 'feature-close-failed'
                && snapshot
                && snapshot.lastCloseFailure
                && snapshot.lastCloseFailure.kind === 'preauth-validation'
                && isCloseFindingBlocking(integrityPolicy, 'preauth-validation')) {
                return { status: 'failed', reason: 'preauth-validation-failed' };
            }
            if ((autoReason === 'escalation-pending' || (snapshot && hasOpenReviewEscalations(snapshot)))
                && isCloseFindingBlocking(integrityPolicy, 'review-escalation')) {
                return { status: 'failed', reason: 'escalation-pending' };
            }
            return { status: 'failed', reason: autoReason };
        }
        if (isFeatureAutoQuotaPause(featureAuto)) {
            return {
                status: 'paused-on-quota',
                reason: (featureAuto && featureAuto.reason) || 'review-quota-paused',
            };
        }
        if (isFeatureDone(mainRepoPath, featureId)) {
...
function buildPauseNotificationMessage(setSlug, featureId, order, reason) {
    const numericId = parseInt(featureId, 10);
    const featureLabel = `feature #${Number.isNaN(numericId) ? featureId : numericId}`;
    if (reason === 'post-merge-gate-failed') {
        return `Set \`${setSlug}\` paused at ${featureLabel} — post-merge gate failed on main. Fix main, then \`aigon feature-close ${featureId}\` or \`aigon set-autonomous-resume ${setSlug}\`.`;
    }
    if (reason === 'preauth-validation-failed') {
        return `Set \`${setSlug}\` paused at ${featureLabel} — pre-auth validation failed. Fix spec/commits, then \`aigon feature-close ${featureId}\` or \`aigon set-autonomous-resume ${setSlug}\`.`;
    }
    if (reason === 'escalation-pending') {
        return `Set \`${setSlug}\` paused at ${featureLabel} — open review escalation(s) need operator disposition (\`aigon feature-escalation …\`).`;
    }
    return `Set \`${setSlug}\` paused at ${featureLabel} — review failed. Run \`aigon set-autonomous-resume ${setSlug}\` to continue after fix.`;
}

function buildQuotaPauseNotificationMessage(setSlug, featureId, reason) {
    const numericId = parseInt(featureId, 10);
    const featureLabel = `feature #${Number.isNaN(numericId) ? featureId : numericId}`;
    const detail = reason === 'review-quota-paused'
        ? 'reviewer quota exhausted'
        : 'agent quota exhausted';
    return `Set \`${setSlug}\` paused at ${featureLabel} — ${detail}. Re-run review or resume with new agents, then \`aigon set-autonomous-resume ${setSlug}\`.`;
}

// Keep the tmux session alive while paused so the user can attach and inspect.
// Exits when the state is cleared (reset) or changed away from a paused status.
function waitWhilePaused(mainRepoPath, setSlug) {
    const initial = readSetAutoState(mainRepoPath, setSlug);
    const isQuota = initial && initial.status === 'paused-on-quota';
    console.log('\n⏸  SetConductor paused — session kept alive for inspection.');
    if (isQuota) {
        console.log('   Reviewer/implementer quota pause — fix review or choose new agents, then resume.');
    }
    console.log(`   Run: aigon set-autonomous-resume ${setSlug}   (same agents, after fix)`);
    console.log(`   Or:  aigon set-autonomous-start ${setSlug} <agents...> --review-agent=<agent>   (new pairing)`);
    ...
    for (;;) {
        ...
        if (!state || !isSetPausedStatus(state.status)) break;
    }
}

function notifySetPausedOnFailure(setSlug, featureId, order, reason) {
    try {
        if (!isSupervisorNotificationsEnabled()) return;
        const message = buildPauseNotificationMessage(setSlug, featureId, order, reason);
...
function notifySetPausedOnQuota(setSlug, featureId, reason) {
    try {
        if (!isSupervisorNotificationsEnabled()) return;
        const message = buildQuotaPauseNotificationMessage(setSlug, featureId, reason);
        sendNotification(message, 'Aigon Set Paused (Quota)');
    } catch (_) {
        // non-fatal
    }
}
...
async function runLoop(rawArgs, mainRepoPath) {
    ...
    const models = String(getOptionValue(options, 'models') || '').trim() || null;
    const efforts = String(getOptionValue(options, 'efforts') || '').trim() || null;
    ...
    try {
        ...
        for (const featureId of order) {
            ...
            try {
                startFeatureAutonomous(mainRepoPath, featureId, { agentIds: explicitAgents, reviewAgent, stopAfter, models, efforts });
            } catch (error) {
                ...
                notifySetPausedOnFailure(setSlug, featureId, order, error.message);
                waitWhilePaused(mainRepoPath, setSlug);
                return;
            }

            const result = waitForFeatureAutonomousResult(mainRepoPath, featureId, pollSeconds);
            if (result.status === 'paused-on-quota') {
                await persistSetState(mainRepoPath, setSlug, {
                    currentFeature: featureId,
                    completed: [...completed],
                    failed,
                    pausedFeature: featureId,
                    status: 'paused-on-quota',
                    running: false,
                    endedAt: new Date().toISOString(),
                    reason: result.reason,
                });
                notifySetPausedOnQuota(setSlug, featureId, result.reason);
                waitWhilePaused(mainRepoPath, setSlug);
                return;
            }
            if (result.status === 'failed') {
                ...
                notifySetPausedOnFailure(setSlug, featureId, order, result.reason);
                waitWhilePaused(mainRepoPath, setSlug);
                return;
            }

            completed.add(featureId);
            failed = failed.filter((id) => id !== String(featureId));
            await persistSetState(mainRepoPath, setSlug, {
                currentFeature: null,
                completed: [...completed],
                failed,
        ...
        if (currentFeature) notifySetPausedOnFailure(setSlug, currentFeature, order, error.message);
        waitWhilePaused(mainRepoPath, setSlug);
    }
}
...
async function start(rawArgs, mainRepoPath, options = {}) {
    ...
    let explicitAgents = normalizeAgentIds(cliOptions._.slice(1));
    const mode = String(getOptionValue(cliOptions, 'mode') || 'sequential').trim();
    let reviewAgent = String(getOptionValue(cliOptions, 'review-agent') || '').trim() || null;
    let models = String(getOptionValue(cliOptions, 'models') || '').trim() || null;
    let efforts = String(getOptionValue(cliOptions, 'efforts') || '').trim() || null;
    const stopAfter = String(getOptionValue(cliOptions, 'stop-after') || 'close').trim();
    if (!setSlug) {
        console.error('Usage: aigon set-autonomous-start <slug> [agents...] [--mode=sequential] [--review-agent=<agent>] [--models=<csv>] [--efforts=<csv>] [--stop-after=close]');
        process.exitCode = 1;
        return;
    }
    if (!featureSets.isValidSetSlug(setSlug)) {
    ...
    const existing = readSetAutoState(mainRepoPath, setSlug);
    // Resume only passes the set slug; re-apply agents + review from persisted state.
    if (options.resumeOnly && existing) {
        if (explicitAgents.length === 0 && Array.isArray(existing.agents) && existing.agents.length) {
            explicitAgents = normalizeAgentIds(existing.agents);
        }
        if (!reviewAgent && existing.reviewAgent) {
            reviewAgent = String(existing.reviewAgent).trim() || null;
        }
        if (!models && existing.models) models = String(existing.models).trim() || null;
        if (!efforts && existing.efforts) efforts = String(existing.efforts).trim() || null;
    }
    let resumeFailed = Array.isArray(existing && existing.failed) ? existing.failed.map(String) : [];
    if (options.resumeOnly && existing && existing.currentFeature) {
        resumeFailed = resumeFailed.filter((id) => id !== String(existing.currentFeature));
    }
    if (options.resumeOnly && !existing) {
    ...
    // On resume: kill the paused session (which is alive in a wait loop)
    // before spawning a fresh run-loop session.
    if (existingSession && existing && isSetPausedStatus(existing.status)) {
    ...
    // F545: prefix the same HOME/env hygiene the agent path uses, so a poisoned
    // parent HOME (e.g. a dashboard daemon with a stale e2e HOME) cannot leak
    // into the Set conductor orchestrator session.
    const hygieneLines = typeof buildAgentWrapperEnvironmentLines === 'function'
        ? buildAgentWrapperEnvironmentLines()
        : [];
    const baseLoopCmd = cmdParts.map(v => shellQuote(String(v))).join(' ');
    const loopCmd = [...hygieneLines, baseLoopCmd].join('; ');
    ...
    await persistSetState(mainRepoPath, setSlug, {
        ...
        failed: options.resumeOnly ? resumeFailed : (Array.isArray(existing && existing.failed) ? existing.failed : []),
        status: 'starting',
        running: false,
        mode,
        reviewAgent,
...
module.exports = {
    ...
    buildQuotaPauseNotificationMessage,
    isFeatureAutoQuotaPause,
    isSetPausedStatus,
    startFeatureAutonomous,
    isFeatureDone,
    isStaleFeatureAutoFailure,
    findAutoConductorTmuxSessionForFeature,
};
