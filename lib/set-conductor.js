...
function startFeatureAutonomous(mainRepoPath, featureId, options) {
    ...
    const agents = resolveFeatureAgents(mainRepoPath, featureId);
    if (agents.length === 0) {
        throw new Error(`Cannot resolve agents for feature ${featureId}. Start this feature once or set an author agent.`);
    }
...
async function runLoop(rawArgs, mainRepoPath) {
    ...
    try {
        ...
        console.log(`   mode: ${mode} | stop-after: ${stopAfter} | poll: ${pollSeconds}s`);
        for (const featureId of order) {
            ...
            try {
                startFeatureAutonomous(mainRepoPath, featureId, { reviewAgent, stopAfter });
            } catch (error) {
...
async function start(rawArgs, mainRepoPath, options = {}) {
    ...
    if (!setSlug) {
        console.error('Usage: aigon set-autonomous-start <slug> [--mode=sequential] [--review-agent=<agent>] [--stop-after=close]');
        process.exitCode = 1;
        return;
    }
    if (!featureSets.isValidSetSlug(setSlug)) {
    ...
    if (existing && existing.status === 'done' && !options.resumeOnly) {
    ...
    if (reviewAgent) cmdParts.push(`--review-agent=${reviewAgent}`);
