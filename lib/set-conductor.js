...
/**
 * SetConductor advance guard: true when the feature is fully closed (engine done
 * and/or spec in 05-done). Uses uncached snapshot read + canonical id path.
 */
function isFeatureDone(mainRepoPath, featureId) {
    const id = normalizeFeatureId(featureId);
    if (isFeatureSpecInDoneFolder(mainRepoPath, id)) return true;
    const snapshot = workflowSnapshotAdapter.readWorkflowSnapshotSyncUncached(mainRepoPath, 'feature', id);
    if (!snapshot) return isFeatureSpecInDoneFolder(mainRepoPath, id);
