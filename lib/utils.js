...
function runDashboardServer(port, instanceName, serverId) {
    ...
    function pollStatus() {
        ...
        const repoLabel = (repo) => `Aigon · ${repo.name || path.basename(repo.path)}`;
        (latestStatus.repos || []).forEach(repo => {
            (repo.features || []).forEach(feature => {
                (feature.agents || []).forEach(agent => {
                    ...
                    if (prev && prev !== 'waiting' && agent.status === 'waiting') {
                        emitNotification('agent-waiting', `${agent.id} is waiting on #${feature.id} ${feature.name}`, { title: repoLabel(repo), openUrl: dashboardUrl, repoPath: repo.path, featureId: feature.id, agentId: agent.id });
                    }
                ...
                if (allSubmitted && feature.stage !== 'in-evaluation' && !allSubmittedNotified.has(featureKey)) {
                    ...
                    emitNotification('all-submitted', `All agents submitted #${feature.id} ${feature.name} — ready for eval`, { title: repoLabel(repo), openUrl: dashboardUrl, repoPath: repo.path, featureId: feature.id });
                }
            });
            ...
            (repo.research || []).forEach(item => {
                (item.agents || []).forEach(agent => {
                    ...
                    const prev = lastStatusByAgent[key];
                    if (prev && prev !== 'waiting' && agent.status === 'waiting') {
                        emitNotification('agent-waiting', `${agent.id} is waiting on R#${item.id} ${item.name}`, { title: repoLabel(repo), openUrl: dashboardUrl, repoPath: repo.path, researchId: item.id, agentId: agent.id });
                    }
                    lastStatusByAgent[key] = agent.status;
                });
                ...
                if (allSubmitted && !allSubmittedNotified.has(researchKey)) {
                    ...
                    emitNotification('all-research-submitted', `All agents submitted R#${item.id} ${item.name} — ready for synthesis`, { title: repoLabel(repo), openUrl: dashboardUrl, repoPath: repo.path, researchId: item.id });
                }
            });
        });
