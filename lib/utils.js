...
function inferDashboardNextCommand(featureId, agents, stage) {
    ...
    if (allSubmitted) {
        return { command: `/afs`, reason: 'Implementation submitted; submit feature' };
    }
    return null;
}
                ...
                try {
                    ...
                    if (fs.existsSync(worktreeBase)) {
                        const entries = fs.readdirSync(worktreeBase).filter(d => {
                            const m = d.match(/^feature-(\d+)-([a-z]{2})-.+$/);
                    ...
                    if (activeSession) {
                        ...
                        createDetachedTmuxSession(sessionName, worktreePath, null);
                        openTerminalAppWithCommand(worktreePath, `tmux attach -t ${shellQuote(sessionName)}`, sessionName);
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, message: `Opened worktree for F${featureId} ${agentId}`, sessionName }));
                    }
                } catch (e) {
                    res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
