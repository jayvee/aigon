...
function collectAnalyticsData(globalConfig) {
    ...
    repos.forEach(repoPath => {
        ...
        doneFiles.forEach(specFile => {
            ...
            const billableTokens = (inputTokens !== null || outputTokens !== null)
            ...
            const hasAadeData = [costUsd, tokensPerLineChanged, autonomyLabel].some(v => v !== null) || hasReworkSignals;
    ...
    const featWithTpl30d = f30d.filter(f => f.tokensPerLineChanged !== null);
    ...
    const avgCost30d = featWithCost30d.length > 0
        ...
        : null;
    const avgTokensPerLine30d = featWithTpl30d.length > 0
        ? featWithTpl30d.reduce((sum, f) => sum + f.tokensPerLineChanged, 0) / featWithTpl30d.length
        : null;
...
function checkAigonCliOrigin() {
    try {
        ...
        try {
            ...
            return { behind: 0, error: null }; // No remote — skip silently
        }
        ...
        try {
            ...
            return { behind: 0, error: `Could not reach origin: ${e.message}` };
        }
        ...
        return { behind: parseInt(count, 10) || 0, error: null };
    } catch (e) {
        return { behind: 0, error: e.message };
    }
}
