...
function getModelProvenance(agentId, taskType) {
    const getConfigModelValue = (config) => {
        if (!config || !config.agents || !config.agents[agentId]) return undefined;
        const agentConfig = config.agents[agentId];
        // Prefer canonical shape: agents.<agent>.<task>.model
    ...
    // 2. Project config
    const projectConfig = loadProjectConfig();
    const projectModelValue = getConfigModelValue(projectConfig);
    ...
    // 3. Global config
    const globalConfig = loadGlobalConfig();
    const globalModelValue = getConfigModelValue(globalConfig);
    if (globalModelValue) {
