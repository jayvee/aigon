...
function isProxyAvailable() {
    try {
        ...
        // Check that Caddy is actually running (not just installed)
        const result = execSync('brew services info caddy --json', { stdio: 'pipe' }).toString();
        const info = JSON.parse(result);
        return Array.isArray(info) && info.length > 0 && info[0].running === true;
    } catch (e) {
        return false;
    }
}
