    ...
    repos.forEach(repoPath => {
        ...
        doneFiles.forEach(specFile => {
            ...
            if ((!startedAt || !completedAt) && selectedLogPath) {
                try {
                    const fmFields = parseLogFrontmatterFull(fs.readFileSync(selectedLogPath, 'utf8')).fields;
            ...
            if (fmEvents.length === 0 && selectedLogPath) {
                try {
                    fmEvents = parseLogFrontmatterFull(fs.readFileSync(selectedLogPath, 'utf8')).events;
                } catch (e) { /* ignore */ }
            }
            ...
            // Read log frontmatter for cycleTimeExclude and AADE fields
            let cycleTimeExclude = false;
            let costUsd = null;
            let tokensPerLineChanged = null;
            let totalTokens = null;
            let linesChanged = null;
            let autonomyLabel = null;
            let reworkThrashing = false;
            let reworkFixCascade = false;
            let reworkScopeCreep = false;
            if (selectedLogPath) {
                try {
                    const logFm = parseLogFrontmatterFull(fs.readFileSync(selectedLogPath, 'utf8')).fields;
                    cycleTimeExclude = logFm.cycleTimeExclude === 'true' || logFm.cycleTimeExclude === true;
                    // AADE telemetry fields
                    const toNum = v => { const n = Number(v); return Number.isFinite(n) ? n : null; };
                    if (logFm.cost_usd !== undefined) costUsd = toNum(logFm.cost_usd);
                    if (logFm.tokens_per_line_changed !== undefined) tokensPerLineChanged = toNum(logFm.tokens_per_line_changed);
                    if (logFm.total_tokens !== undefined) totalTokens = toNum(logFm.total_tokens);
                    if (logFm.lines_changed !== undefined) linesChanged = toNum(logFm.lines_changed);
                    if (logFm.autonomy_label) {
                        // Strip surrounding quotes from YAML string values
                        const raw = String(logFm.autonomy_label);
                        autonomyLabel = (raw.startsWith('"') && raw.endsWith('"')) || (raw.startsWith("'") && raw.endsWith("'"))
                            ? raw.slice(1, -1) : raw;
                    }
                    const isTruthy = v => v === true || v === 'true';
                    reworkThrashing = isTruthy(logFm.rework_thrashing);
                    reworkFixCascade = isTruthy(logFm.rework_fix_cascade);
                    reworkScopeCreep = isTruthy(logFm.rework_scope_creep);
                } catch (e) { /* ignore */ }
            }
            const hasRework = reworkThrashing || reworkFixCascade || reworkScopeCreep;
            const hasAadeData = costUsd !== null || tokensPerLineChanged !== null || autonomyLabel !== null
                || reworkThrashing || reworkFixCascade || reworkScopeCreep;
            // firstPassNoRework: feature completed without any rework flags (only meaningful when signals exist)
            const firstPassNoRework = hasAadeData ? !hasRework : null;
