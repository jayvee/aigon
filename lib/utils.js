...
function collectAnalyticsData(globalConfig) {
    ...
    repos.forEach(repoPath => {
        ...
        doneFiles.forEach(specFile => {
            ...
            // Cache log frontmatter (avoid reading the file multiple times)
            let logFrontmatter = { fields: {}, events: [] };
            if (selectedLogPath) {
                try {
                    logFrontmatter = parseLogFrontmatterFull(fs.readFileSync(selectedLogPath, 'utf8'));
                } catch (e) { /* ignore */ }
            }
            ...
            if ((!startedAt || !completedAt) && selectedLogPath) {
                try {
                    const fmFields = logFrontmatter.fields || {};
            ...
            // Check autonomous flag in log content (uses cached frontmatter source)
            let autonomousMode = false;
            if (selectedLogPath) {
                try {
                    const logContent = fs.readFileSync(selectedLogPath, 'utf8');
                    autonomousMode = /--autonomous/.test(logContent);
                } catch (e) { /* ignore */ }
            }
            ...
            if (fmEvents.length === 0 && selectedLogPath) {
                try {
                    fmEvents = logFrontmatter.events || [];
                } catch (e) { /* ignore */ }
            }
            ...
            // Read cycleTimeExclude and AADE fields from cached frontmatter
            let cycleTimeExclude = false;
            const logFm = logFrontmatter.fields || {};
            cycleTimeExclude = logFm.cycleTimeExclude === 'true' || logFm.cycleTimeExclude === true;

            // AADE telemetry fields — use parseYamlScalar for robust scalar parsing
            const toNullNum = v => {
                if (v === null || v === undefined || v === '') return null;
                const parsed = typeof v === 'string' ? parseYamlScalar(v) : v;
            ...
            const costUsd = toNullNum(logFm.cost_usd);
            const tokensPerLineChanged = toNullNum(logFm.tokens_per_line_changed);
            const totalTokens = toNullNum(logFm.total_tokens);
            const inputTokens = toNullNum(logFm.input_tokens);
            const outputTokens = toNullNum(logFm.output_tokens);
            const sessions = toNullNum(logFm.sessions);
            const linesChanged = toNullNum(logFm.lines_changed);
            const telemetryModel = logFm.model ? String(parseYamlScalar(logFm.model)) : null;
            const autonomyLabel = logFm.autonomy_label !== undefined && logFm.autonomy_label !== null && String(logFm.autonomy_label).trim()
                ? String(parseYamlScalar(logFm.autonomy_label))
                : null;
            const reworkThrashing = toNullBool(logFm.rework_thrashing);
            const reworkFixCascade = toNullBool(logFm.rework_fix_cascade);
            const reworkScopeCreep = toNullBool(logFm.rework_scope_creep);
            const hasRework = reworkThrashing === true || reworkFixCascade === true || reworkScopeCreep === true;
            const hasAadeData = [costUsd, tokensPerLineChanged, totalTokens, autonomyLabel, reworkThrashing, reworkFixCascade, reworkScopeCreep]
                .some(v => v !== null && v !== undefined);
            // firstPassNoRework: feature completed without any rework flags (only meaningful when signals exist)
            const firstPassNoRework = (reworkThrashing !== null || reworkFixCascade !== null || reworkScopeCreep !== null)
                ? !hasRework : null;
    ...
    // Amplification aggregates (AADE metrics)
    const ampFeatures = allFeatures.filter(f => f.hasAadeData);
    const ampWithCost = ampFeatures.filter(f => f.costUsd !== null);
    const ampWithTpl = ampFeatures.filter(f => f.tokensPerLineChanged !== null);
    const ampWithReworkSignals = allFeatures.filter(f => f.hasAadeData);
    const ampFirstPassRate = ampWithReworkSignals.length > 0
        ? ampWithReworkSignals.filter(f => !f.hasRework).length / ampWithReworkSignals.length : null;
    const ampAutonomyCounts = {};
    ampFeatures.forEach(f => {
        if (f.autonomyLabel) ampAutonomyCounts[f.autonomyLabel] = (ampAutonomyCounts[f.autonomyLabel] || 0) + 1;
    });

    // Build daily cost trend for sparklines
    function buildDailyTrend(feats, key, daysBack) {
        const since = nowTs - daysBack * 86400000;
        const buckets = {};
        feats.forEach(f => {
            if (!f.completedTime || f.completedTime < since) return;
            if (f[key] === null || f[key] === undefined) return;
            const day = new Date(f.completedTime).toISOString().slice(0, 10);
            if (!buckets[day]) buckets[day] = { sum: 0, count: 0 };
            buckets[day].sum += f[key];
            buckets[day].count += 1;
        });
        return Object.keys(buckets).sort().map(day => ({
            day,
            avg: buckets[day].count > 0 ? buckets[day].sum / buckets[day].count : 0
        }));
    }

    // Free-tier totals: aggregate token and cost data across all features (not just ampFeatures)
    const allWithCost = allFeatures.filter(f => f.costUsd !== null);
    const allWithTokens = allFeatures.filter(f => f.totalTokens !== null);
    const reworkFlagCount = allFeatures.filter(f => f.hasRework).length;

    const amplification = {
        featuresWithData: ampFeatures.length,
        avgCostUsd: ampWithCost.length > 0 ? ampWithCost.reduce((s, f) => s + f.costUsd, 0) / ampWithCost.length : null,
        totalCostUsd: allWithCost.length > 0 ? Math.round(allWithCost.reduce((s, f) => s + f.costUsd, 0) * 100) / 100 : null,
        totalTokens: allWithTokens.length > 0 ? allWithTokens.reduce((s, f) => s + f.totalTokens, 0) : null,
        avgTokensPerLine: ampWithTpl.length > 0 ? ampWithTpl.reduce((s, f) => s + f.tokensPerLineChanged, 0) / ampWithTpl.length : null,
        firstPassRate: ampFirstPassRate,
        reworkFlagCount,
        autonomyCounts: ampAutonomyCounts,
        trends: {
            cost7d: buildDailyTrend(allFeatures, 'costUsd', 7),
            cost30d: buildDailyTrend(allFeatures, 'costUsd', 30),
            tpl7d: buildDailyTrend(allFeatures, 'tokensPerLineChanged', 7),
            tpl30d: buildDailyTrend(allFeatures, 'tokensPerLineChanged', 30)
        }
    };
    ...
    return {
        ...
        features: allFeatures.map(f => ({
            ...
            sessions: f.sessions,
            telemetryModel: f.telemetryModel,
            ...
            hasRework: f.hasRework,
            hasReworkFlags: f.hasRework,
