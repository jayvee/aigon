...
const { tryOrDefault, classifyError } = require('./errors');
...
function getConfigValueWithProvenance(key) {
    ...
    const rawGlobalConfig = tryOrDefault(
        () => fs.existsSync(GLOBAL_CONFIG_PATH) ? JSON.parse(fs.readFileSync(GLOBAL_CONFIG_PATH, 'utf8')) : {},
        {}, { warn: true, context: 'global config' });
...
function readBasePort() {
    ...
    for (const file of envFiles) {
        ...
        try {
            ...
            if (classifyError(e) !== 'missing') process.stderr.write(`[readBasePort] Warning: ${e.message}\n`);
        }
    }
    return null;
}
...
// Port data is stored in servers.json under the _portRegistry key (merged in v2.50+)
// PORT_REGISTRY_PATH kept for backwards-compatible migration reads only
...
function getAppId() {
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            if (classifyError(e) === 'parse') process.stderr.write(`[getAppId] Warning: package.json parse failed: ${e.message}\n`);
        }
    }
...
function proxyDiagnostics() {
    ...
    try { execSync('caddy version', { stdio: 'pipe' }); caddyInstalled = true; } catch (e) { /* not installed */ } // probe
    ...
    try {
        ...
        caddyAdminApi = true;
    } catch (e) { /* not responding */ } // probe

    // dnsmasq installed?
    let dnsmasqInstalled = false;
    try { execSync('brew list dnsmasq', { stdio: 'pipe' }); dnsmasqInstalled = true; } catch (e) { /* not installed */ } // probe
...
function loadProxyRegistry() {
    ...
    return tryOrDefault(
        () => JSON.parse(fs.readFileSync(DEV_PROXY_REGISTRY, 'utf8')),
        {}, { warn: true, context: 'servers.json' });
}
 ...
 * Port data is stored in the _portRegistry key of servers.json (merged in v2.50+).
 * Falls back to the legacy ports.json if servers.json has no _portRegistry yet.
 * @returns {Object} Registry object { name: { basePort, path } }
 */
function loadPortRegistry() {
    // Primary: read from _portRegistry in servers.json
    const proxy = loadProxyRegistry();
    if (proxy._portRegistry) return proxy._portRegistry;

    // Legacy fallback: migrate from ports.json on first access
    if (fs.existsSync(PORT_REGISTRY_PATH)) {
        const legacy = tryOrDefault(
            () => JSON.parse(fs.readFileSync(PORT_REGISTRY_PATH, 'utf8')),
            null, { warn: true, context: 'ports.json migration' });
        if (legacy && typeof legacy === 'object') {
            // Migrate: write into servers.json and remove ports.json
            savePortRegistry(legacy);
            try { fs.unlinkSync(PORT_REGISTRY_PATH); } catch (_e) {} // optional - cleanup legacy file
            return legacy;
        }
    }
...
/**
 * Save the global port registry into servers.json under _portRegistry.
 * @param {Object} registry - Registry object
 */
function savePortRegistry(registry) {
    const proxy = loadProxyRegistry();
    proxy._portRegistry = registry;
    saveProxyRegistry(proxy);
}
    ...
    for (const entry of entries) {
        ...
        if (fs.existsSync(aigonConfigPath)) {
            try {
                ...
                if (classifyError(e) !== 'missing') process.stderr.write(`[scanPorts] Warning: ${aigonConfigPath}: ${e.message}\n`);
            }
        }
        ...
        for (const envFile of ['.env.local', '.env']) {
            ...
            try {
                ...
                if (classifyError(e) !== 'missing') process.stderr.write(`[scanPorts] Warning: ${envPath}: ${e.message}\n`);
            }
        }
    }
...
function addCaddyRoute(hostname, port, routeId) {
    if (isCaddyAdminAvailable()) {
        ...
        try {
            ...
            try { fs.unlinkSync(tmpFile); } catch (e2) { /* ignore */ } // optional - temp file cleanup
        }
    }
...
/**
 * Validate the server registry on dashboard startup.
 * Checks that each registered PID is alive. Removes stale entries (dead process).
 * Uses lsof to verify port is still in use when PID is not recorded.
 * @returns {{ live: number, staleRemoved: number }} Validation summary
 */
function validateRegistry() {
    ...
    for (const [appId, servers] of Object.entries(registry)) {
        if (appId === '_portRegistry') continue; // skip embedded port registry
        for (const [serverId, info] of Object.entries(servers)) {
            // Handle legacy nested service/dashboard PIDs
            ...
            if (pid > 0) {
                ...
                // No PID recorded — check if something is listening on the port
...
/**
 * Check if a port is in use (synchronous, uses lsof on macOS/Linux).
 * @param {number} port
 * @returns {boolean} True if something is listening on the port
 */
function isPortInUseSync(port) {
    try {
        execSync(`lsof -ti tcp:${port}`, { stdio: 'pipe' });
        return true; // lsof found a process — port in use
    } catch (_e) {
        return false; // lsof returned nothing — port free // probe
    }
}
...
function detectDevServerContext() {
    ...
    if (wtMatch) {
        ...
        } catch (e) { /* not in a git repo */ } // probe
    }
    ...
    repos.forEach(repoPath => {
            ...
            if (fs.existsSync(dir)) {
                try {
                    ...
                    fs.readdirSync(dir)
                        ...
                        .sort((a, b) => {
                            const mtimeA = (() => { try { return fs.statSync(path.join(dir, a)).mtimeMs; } catch (e) { return 0; } })() // optional;
                            const mtimeB = (() => { try { return fs.statSync(path.join(dir, b)).mtimeMs; } catch (e) { return 0; } })() // optional;
            ...
            try {
                ...
                const doneWithStats = allDone
                    .map(f => {
                        ...
                        try { const st = fs.statSync(path.join(doneDir, f)); mtime = st.mtime.getTime(); birthtime = st.birthtime.toISOString(); } catch (e) {} // optional
            ...
            try {
                fs.readdirSync(logDir)
                    ...
                    .forEach(logFile => {
                        ...
                        let content = '';
                        try { content = fs.readFileSync(logPath, 'utf8'); } catch (e) { return; } // optional
        ...
        if (fs.existsSync(researchDoneDir)) {
            try {
                ...
                allDone
                    .map(f => ({ f, mtime: (() => { try { return fs.statSync(path.join(researchDoneDir, f)).mtime.getTime(); } catch (e) { return 0; } })() })) // optional
                    .sort((a, b) => b.mtime - a.mtime)
                    .slice(0, 10)
        ...
        if (fs.existsSync(feedbackDoneDir)) {
            try {
                ...
                allDone
                    .map(f => ({ f, mtime: (() => { try { return fs.statSync(path.join(feedbackDoneDir, f)).mtime.getTime(); } catch (e) { return 0; } })() })) // optional
                    .sort((a, b) => b.mtime - a.mtime)
                    .slice(0, 10)
...
async function captureDashboardScreenshot(url, outputPath, width, height) {
    ...
    try {
        ...
        try { puppeteer = require('puppeteer-core'); } catch (_) { /* ignore */ } // probe
    }
...
function runDashboardServer(port, instanceName, serverId) {
        ...
        if (reqPath === '/api/sessions/cleanup' && req.method === 'POST') {
            try {
                ...
                for (const s of orphans) {
                    ...
                    } catch (e) { /* ignore individual failures */ } // optional
                }
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        ...
        if (reqPath === '/api/settings/notifications' && req.method === 'POST') {
            ...
            req.on('end', () => {
                try {
                    ...
                    let rawConfig = {};
                    try { rawConfig = JSON.parse(fs.readFileSync(GLOBAL_CONFIG_PATH, 'utf8')); } catch (_) {} // optional
    ...
    // Validate registry before starting — remove dead entries and log summary
    const validation = validateRegistry();
    log(`Registry: ${validation.live} live, ${validation.staleRemoved} stale removed`);
    if (validation.staleRemoved > 0) {
        console.log(`   Registry: ${validation.live} live, ${validation.staleRemoved} stale removed`);
    }
    ...
    server.listen(port, host, () => {
        ...
        resetIdleTimer();
        try { openInBrowser(dashboardUrl); } catch (e) { /* non-fatal */ } // optional
    });
}
...
function detectProjectProfile() {
    ...
    if (fs.existsSync(pkgPath)) {
        ...
        } catch (e) { if (classifyError(e) === 'parse') process.stderr.write(`[aigon] Warning: package.json parse failed: ${e.message}\n`); } // optional
    }
...
function openTerminalAppWithCommand(cwd, command, title) {
    ...
    if (tmuxApp === 'iterm2') {
        ...
        const resolvedCommand = command.replace(/^(\S+)/, (bin) => {
            ...
            try { return execSync(`which ${bin}`, { encoding: 'utf8' }).trim(); } catch { return bin; } // probe
        });
...
function collectAnalyticsData(globalConfig) {
    ...
    if (!timezone) {
        try { timezone = Intl.DateTimeFormat().resolvedOptions().timeZone; } catch (e) { timezone = 'UTC'; } // optional
    }
