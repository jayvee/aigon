...
const DASHBOARD_INSTANCES_DIR = path.join(GLOBAL_CONFIG_DIR, 'instances');
// Legacy aliases (kept for any remaining references in dev-server / proxy code)
const RADAR_DEFAULT_PORT = DASHBOARD_DEFAULT_PORT;
const RADAR_DYNAMIC_PORT_START = DASHBOARD_DYNAMIC_PORT_START;
 ...
 * @returns {{ isWorktree: boolean, instanceName: string, worktreePath: string|null }}
 */
function detectDashboardContext() {
    const dirname = path.basename(process.cwd());
    const wtMatch = dirname.match(/^feature-\d+-[a-z]{2}-.+$/);
    if (wtMatch) {
        return { isWorktree: true, instanceName: dirname, worktreePath: process.cwd() };
    }
    ...
    try {
        ...
        if (branch && branch !== 'main' && branch !== 'master') {
            const branchMatch = branch.match(/^feature-\d+-[a-z]{2}-.+$/);
            if (branchMatch) {
                return { isWorktree: true, instanceName: branch, worktreePath: process.cwd() };
            }
        }
    } catch (e) { /* not in a git repo */ }

    return { isWorktree: false, instanceName: 'main', worktreePath: null };
}
...
// ── Dashboard instance discovery files ────────────────────────────────────────

function writeDashboardInstance(name, data) {
    fs.mkdirSync(DASHBOARD_INSTANCES_DIR, { recursive: true });
    fs.writeFileSync(path.join(DASHBOARD_INSTANCES_DIR, name + '.json'), JSON.stringify(data, null, 2) + '\n');
}

function removeDashboardInstance(name) {
    const f = path.join(DASHBOARD_INSTANCES_DIR, name + '.json');
    if (fs.existsSync(f)) fs.unlinkSync(f);
}

function listDashboardInstances() {
    if (!fs.existsSync(DASHBOARD_INSTANCES_DIR)) return [];
    return fs.readdirSync(DASHBOARD_INSTANCES_DIR)
        .filter(f => f.endsWith('.json'))
        .flatMap(f => {
            try { return [JSON.parse(fs.readFileSync(path.join(DASHBOARD_INSTANCES_DIR, f), 'utf8'))]; }
            catch (e) { return []; }
        });
}
...
function runDashboardServer(port, instanceName) {
    const http = require('http');
    ...
    // ── Idle timer: auto-exit after 5 min with no requests ────────────────────
    const IDLE_TIMEOUT_MS = 5 * 60 * 1000;
    let idleTimer;
    function resetIdleTimer() {
        clearTimeout(idleTimer);
        idleTimer = setTimeout(() => { log('Idle timeout — shutting down'); shutdown(); }, IDLE_TIMEOUT_MS);
    }
    ...
    const shutdown = () => {
        ...
        removeDashboardInstance(instanceName);
    ...
    server.listen(port, host, () => {
        writeDashboardInstance(instanceName, { name: instanceName, port, pid: process.pid, worktreePath: process.cwd(), startedAt: new Date().toISOString() });
...
const COMMAND_REGISTRY = {
    ...
    'radar': { argHints: '[subcommand]', disableModelInvocation: true },
 ...
 * Ignores .env* files (e.g. .env.local, .env.test) which are never committed.
...
function getWorktreeStatus(worktreePath) {
    try {
        const raw = execSync(`git -C "${worktreePath}" status --porcelain`, { encoding: 'utf8' }).trim();
        if (!raw) return '';
