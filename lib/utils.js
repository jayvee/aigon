...
function collectAnalyticsData(globalConfig) {
    ...
    const tokensByActivityTimeSeries = (() => {
        ...
        const seriesModels = {}; // { seriesKey: { model: count } }
        ...
        allTelemetrySessions.forEach(s => {
            ...
            const seriesKey = activity ? `${agent}:${activity}` : agent;

            if (!buckets[day]) buckets[day] = {};
            if (!buckets[day][seriesKey]) buckets[day][seriesKey] = 0;
            buckets[day][seriesKey] += Number(s.tokenUsage && s.tokenUsage.billable || 0);

            if (activity && s.featureId) {
                featuresWithActivity.add(`${s.repoPath || ''}:${s.featureId}`);
            }

            if (s.model) {
                if (!seriesModels[seriesKey]) seriesModels[seriesKey] = {};
                seriesModels[seriesKey][s.model] = (seriesModels[seriesKey][s.model] || 0) + 1;
            }
        });
        ...
        const points = sortedDays.map(day => ({
            day,
            series: Object.keys(buckets[day])
                .sort()
                .reduce((acc, key) => {
                    acc[key] = Math.round(buckets[day][key]);
                    return acc;
                }, {})
        }));

        const modelBySeries = Object.entries(seriesModels).reduce((acc, [seriesKey, counts]) => {
            const topModel = Object.entries(counts)
                .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0];
            acc[seriesKey] = topModel ? topModel[0] : null;
            return acc;
        }, {});

        return {
            points,
            series: [...new Set(points.flatMap(point => Object.keys(point.series)))].sort(),
            seriesModels: modelBySeries,
            featuresWithActivity: featuresWithActivity.size
        };
    })();
    amplification.tokensByActivityTimeSeries = tokensByActivityTimeSeries;
