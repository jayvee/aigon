                ...
                try {
                    fs.readdirSync(researchLogsDir)
                        ...
                        .forEach(f => {
                            ...
                            const idPadded = String(rid).padStart(2, '0');
                            entry.agents.push({
                                id: agent,
                                status,
                                updatedAt,
        ...
        (latestStatus.repos || []).forEach(repo => {
                    ...
                    if (autoEvalEnabled) {
                        if (tmuxSessionExists(evalSessionName)) {
                            ...
                            try {
                                ...
                                sendMacNotification(`Auto-eval started for #${feature.id} ${feature.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`);
                            } catch (e) {
                                log(`Auto-eval failed for #${feature.id} (${repo.path}): ${e.message}`);
                                sendMacNotification(`Auto-eval failed for #${feature.id} ${feature.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`);
                            }
                        }
                    } else {
            ...
            (repo.research || []).forEach(item => {
                (item.agents || []).forEach(agent => {
                    ...
                    const prev = lastStatusByAgent[key];
                    if (prev && prev !== 'waiting' && agent.status === 'waiting') {
                        sendMacNotification(`${agent.id} is waiting on R#${item.id} ${item.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`);
                    }
                    lastStatusByAgent[key] = agent.status;
                });
                ...
                if (allSubmitted && !allSubmittedNotified.has(researchKey)) {
                    ...
                    sendMacNotification(`All agents submitted R#${item.id} ${item.name} — ready for synthesis`, `Aigon · ${repo.name || path.basename(repo.path)}`);
