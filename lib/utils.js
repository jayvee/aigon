...
function collectAnalyticsData(globalConfig) {
    ...
    repos.forEach(repoPath => {
        ...
        doneFiles.forEach(specFile => {
            ...
            // Check cycleTimeExclude and AADE fields from legacy frontmatter (old features) or manifest
            ...
            let reworkThrashing = null;
            let reworkFixCascade = null;
            let reworkScopeCreep = null;
            if (selectedLogPath) {
                try {
                    ...
                    if (logFm.autonomy_label) autonomyLabel = String(logFm.autonomy_label);
                    // AADE rework flags — parse string 'true'/'false' and boolean
                    const toBool = v => v === true || v === 'true' ? true : (v === false || v === 'false' ? false : null);
                    reworkThrashing = toBool(logFm.rework_thrashing);
                    reworkFixCascade = toBool(logFm.rework_fix_cascade);
                    reworkScopeCreep = toBool(logFm.rework_scope_creep);
                } catch (e) { /* ignore */ }
            }
            const hasReworkFlags = reworkThrashing === true || reworkFixCascade === true || reworkScopeCreep === true;
            const hasAadeData = costUsd !== null || tokensPerLineChanged !== null || autonomyLabel !== null
                || reworkThrashing !== null || reworkFixCascade !== null || reworkScopeCreep !== null;
            // firstPassNoRework: true if no rework flags AND no wait events
            const firstPassNoRework = hasAadeData
                ? (!hasReworkFlags && (firstPassSuccess === null || firstPassSuccess === true))
                : null;
    ...
    // Amplification (AADE aggregates)
    const ampFeatures = allFeatures.filter(f => f.hasAadeData);
    const ampWithCost = allFeatures.filter(f => f.costUsd !== null);
    const ampWithTpl = allFeatures.filter(f => f.tokensPerLineChanged !== null);
    const ampWithRework = allFeatures.filter(f => f.reworkThrashing !== null || f.reworkFixCascade !== null || f.reworkScopeCreep !== null);
    const ampAvgCost = ampWithCost.length > 0
        ? ampWithCost.reduce((s, f) => s + f.costUsd, 0) / ampWithCost.length : null;
    const ampAvgTpl = ampWithTpl.length > 0
        ? ampWithTpl.reduce((s, f) => s + f.tokensPerLineChanged, 0) / ampWithTpl.length : null;
    const ampReworkRate = ampWithRework.length > 0
        ? ampWithRework.filter(f => f.hasReworkFlags).length / ampWithRework.length : null;

    // Build simple daily trends for amplification sparklines
    function buildAmpTrend(features, key, days) {
    ...
    const amplification = {
        totalWithAadeData: ampFeatures.length,
        avgCost: ampAvgCost !== null ? Math.round(ampAvgCost * 10000) / 10000 : null,
        avgTokensPerLine: ampAvgTpl !== null ? Math.round(ampAvgTpl * 100) / 100 : null,
        reworkRate: ampReworkRate !== null ? Math.round(ampReworkRate * 100) / 100 : null,
        trends: {
            cost7d: buildAmpTrend(allFeatures, 'costUsd', 7),
            cost30d: buildAmpTrend(allFeatures, 'costUsd', 30),
            tpl7d: buildAmpTrend(allFeatures, 'tokensPerLineChanged', 7),
            tpl30d: buildAmpTrend(allFeatures, 'tokensPerLineChanged', 30),
        }
    };
