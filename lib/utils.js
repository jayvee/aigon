 ...
 * @returns {boolean} True if Caddy is installed, running, and the Caddyfile is in place
 */
function isProxyAvailable() {
    try {
        ...
        if (!fs.existsSync(DEV_PROXY_CADDYFILE)) return false;
        // Check that Caddy is actually running (pgrep works for both user and root/sudo services)
        execSync('pgrep -x caddy', { stdio: 'pipe' });
        return true;
    } catch (e) {
        return false;
    }
...
function renderRadarMenubarFromStatus(payload, port) {
    ...
    console.log('---');
    const localhostUrl = `http://127.0.0.1:${port || RADAR_DEFAULT_PORT}`;
    console.log(`Open Dashboard | href=${localhostUrl}`);
    if (isProxyAvailable()) {
        console.log(`Open Dashboard (aigon.test) | alternate=true href=http://aigon.test`);
    }
...
// This set supplements RADAR_INTERACTIVE_ACTIONS to accept all state-machine-defined
...
function parseRadarActionRequest(payload, options = {}) {
    ...
    if (!RADAR_INTERACTIVE_ACTIONS.has(action) && !SM_INVOCABLE_ACTIONS.has(action)) {
...
function runRadarServiceDaemon(port) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if ((reqPath === '/api/feature-open' || reqPath === '/api/worktree-open') && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                let payload = {};
...
function collectAnalyticsData(globalConfig) {
    ...
    // Quality
    const featWithDuration = f30d.filter(f => f.durationMs !== null && f.durationMs > 0);
    ...
    const quality = {
        durationHours: {
            ...
            median: durHours.length > 0 ? round1(durHours[Math.floor(durHours.length / 2)]) : null,
    ...
    const agents = Object.entries(agentMap).map(([agent, feats]) => {
        ...
        const withDur = feats.filter(f => f.durationMs !== null && f.durationMs > 0);
        const agentCycle = withDur.length > 0
            ? withDur.reduce((s, f) => s + f.durationMs, 0) / withDur.length / (1000 * 3600) : null;
        return {
            agent,
 ...
 * @returns {string} - Porcelain output (truthy if dirty), or empty string if clean
 */
function getWorktreeStatus(worktreePath) {
    try {
        return execSync(`git -C "${worktreePath}" status --porcelain`, { encoding: 'utf8' }).trim();
    } catch (e) {
        return '';
    }
}
