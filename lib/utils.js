...
function isProxyAvailable() {
    ...
    if (!fs.existsSync(DEV_PROXY_CADDYFILE)) return false; // proxy-setup not run
    try {
        execSync('pgrep -x caddy', { stdio: 'pipe' });
        return true;
    } catch (e) {
        return false; // Caddy installed but not running
    }
}

function proxyUnavailableReason() {
    try { execSync('caddy version', { stdio: 'pipe' }); } catch (e) {
        return 'Caddy is not installed (run: brew install caddy)';
    }
    if (!fs.existsSync(DEV_PROXY_CADDYFILE)) {
        return 'Proxy not configured (run: aigon proxy-setup)';
    }
    try { execSync('pgrep -x caddy', { stdio: 'pipe' }); } catch (e) {
        return 'Caddy is not running (run: sudo brew services start caddy)';
    }
    return null;
}
        ...
        (latestStatus.repos || []).forEach(repo => {
            (repo.features || []).forEach(feature => {
                ...
                if (allSubmitted && feature.stage !== 'in-evaluation' && !allSubmittedNotified.has(featureKey)) {
                    ...
                    log(`All agents submitted for #${feature.id} (${repo.path}) — ready for eval`);
                }
            });
