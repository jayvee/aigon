...
const RADAR_DEFAULT_PORT = 4100;
const RADAR_DASHBOARD_PORT = 4200;
const RADAR_DYNAMIC_PORT_START = 4201;
const RADAR_APP_ID = 'aigon';
...
function generateCaddyfile(registry) {
    ...
    for (const [appId, servers] of Object.entries(registry)) {
        ...
        for (const [serverId, info] of Object.entries(servers)) {
            ...
            // Radar entries have nested dashboard.port; regular entries have port directly
...
function gcDevServers() {
    ...
    for (const [appId, servers] of Object.entries(registry)) {
        for (const [serverId, info] of Object.entries(servers)) {
            // Radar entries have nested service/dashboard PIDs
...
/**
 * Detect the Radar context — is this the main repo or a worktree?
 * Returns serverId ('' for main, 'cc-119' for worktrees) and worktree metadata.
 * @returns {{ isWorktree: boolean, agentId: string|null, featureId: string|null, serverId: string, worktreePath: string|null }}
 */
function detectRadarContext() {
    const dirname = path.basename(process.cwd());
    const wtMatch = dirname.match(/^feature-(\d+)-([a-z]{2})-/);
    if (wtMatch) {
        return {
    ...
    try {
        ...
        if (branchMatch) {
            return {
                isWorktree: true,
                featureId: branchMatch[1],
                agentId: branchMatch[2],
                serverId: `${branchMatch[2]}-${branchMatch[1]}`,
                worktreePath: process.cwd()
            };
        }
    } catch (e) { /* not in a git repo */ }

    return { isWorktree: false, agentId: null, featureId: null, serverId: '', worktreePath: null };
}

/**
 * Register a Radar instance (service + dashboard pair) with the dev proxy.
 * @param {string} serverId - '' for main, 'cc-119' for worktrees
 * @param {Object} entry - { service: { port, pid }, dashboard: { port, pid }, worktree?, started }
 */
function registerRadarServer(serverId, entry) {
    const registry = loadProxyRegistry();
    if (!registry[RADAR_APP_ID]) registry[RADAR_APP_ID] = {};
    registry[RADAR_APP_ID][serverId] = entry;
    saveProxyRegistry(registry);
    reloadCaddy(registry);
}

/**
 * Deregister a Radar instance from the dev proxy.
 * @param {string} serverId - '' for main, 'cc-119' for worktrees
 */
function deregisterRadarServer(serverId) {
    deregisterDevServer(RADAR_APP_ID, serverId);
}

/**
 * Resolve the Radar dashboard URL for a given context.
 * @param {string} serverId - '' for main, 'cc-119' for worktrees
 * @param {number} dashboardPort - Dashboard port number
 * @returns {string} URL
 */
function resolveRadarUrl(serverId, dashboardPort) {
    if (isProxyAvailable()) {
        return getDevProxyUrl(RADAR_APP_ID, serverId);
    }
...
function inferDashboardNextCommand(featureId, agents, stage) {
    ...
    const isFleet = agents.filter(agent => agent.id !== 'solo').length > 1;

    if (hasWaiting) {
        return { command: `aigon terminal-focus ${idPadded}`, reason: 'Focus waiting agent terminal' };
    }
    if (allSubmitted && isFleet) {
        return { command: `/afe ${idPadded}`, reason: 'All agents submitted; evaluate winners' };
    }
    if (allSubmitted && !isFleet) {
        return { command: `aigon feature-close ${idPadded}`, reason: 'Solo implementation submitted; close feature' };
    }
    return null;
}
...
function inferDashboardNextActions(featureId, agents, stage) {
    ...
    if (hasWaiting) {
        ...
        const agentSuffix = waitingAgent && waitingAgent.id !== 'solo' ? ` ${waitingAgent.id}` : '';
        actions.push({ command: `aigon terminal-focus ${id}${agentSuffix}`, label: 'Focus terminal', reason: 'Agent is waiting for input', mode: 'terminal' });
        actions.push({ command: `aigon feature-stop ${id}`, label: 'Stop agent', reason: 'Kill the waiting agent session', mode: 'fire-and-forget' });
    }

    if (hasImplementing && !hasWaiting) {
        actions.push({ command: `aigon terminal-attach ${id}`, label: 'Attach', reason: 'Open terminal to view progress', mode: 'terminal' });
        actions.push({ command: `aigon feature-stop ${id}`, label: 'Stop agent', reason: 'Kill the implementing agent session', mode: 'fire-and-forget' });
    }
    ...
    repos.forEach(repoPath => {
        ...
        if (fs.existsSync(doneDir)) {
            try {
                ...
                const doneFiles = allDone
                    .map(f => ({ f, mtime: (() => { try { return fs.statSync(path.join(doneDir, f)).mtime.getTime(); } catch (e) { return 0; } })() }))
                    .sort((a, b) => b.mtime - a.mtime)
                    .slice(0, 10)
                    .map(({ f }) => f);
                doneFiles.forEach(f => specFiles.push({ file: f, stage: 'done', dir: doneDir }));
            } catch (e) { /* ignore */ }
        }
        ...
        researchSpecFiles.forEach(({ file, stage, dir: specDir }) => {
            ...
            research.push({ id, name, stage, specPath: path.join(specDir, file), agents });
        });
        ...
        feedbackSpecFiles.forEach(({ file, stage, dir: specDir }) => {
            ...
            feedback.push({ id: hasId ? fm[1] : null, name: hasId ? fm[2] : fm[1], stage, specPath: path.join(specDir, file), agents: [] });
        });
...
function sendMacNotification(message, title = 'Aigon Radar', { openUrl } = {}) {
    try {
...
function renderRadarMenubarFromStatus(payload, port) {
    ...
    repos.forEach(repo => {
        ...
        (repo.features || [])
            .filter(feature => feature.stage === 'in-progress' || feature.stage === 'in-evaluation')
            .filter(feature => (feature.agents || []).length > 0)
            .sort((a, b) => Number(a.id) - Number(b.id))
            .forEach(feature => {
                const paddedId = String(feature.id).padStart(2, '0');
                const headerCmd = `/afd ${paddedId}`;
                lines.push(`#${feature.id} ${feature.name} | size=13 bash=/bin/bash param1=-c param2="echo '${headerCmd}' | pbcopy" terminal=false`);
                ...
                if (feature.stage === 'in-evaluation') {
                    ...
                    attentionItems.push({ repoPath, fid: feature.id, name: feature.name, reason: evalReason, action: `/afe ${paddedId}` });
                } else if (allSubmitted) {
                    attentionItems.push({ repoPath, fid: feature.id, name: feature.name, reason: 'All agents submitted', action: `/afe ${paddedId}` });
                } else if (hasWaiting) {
    ...
    console.log(`Tile Windows | bash="${nodeExec}" param1="${aigonScript}" param2=radar param3=tile terminal=false`);
...
function isRadarAutoEvalEnabled(globalConfig) {
    const cfg = globalConfig || {};
    const conductorConfig = cfg.conductor || {};
    return cfg.autoEval !== false && conductorConfig.autoEval !== false;
}

function buildRadarFeatureEvalSessionName(repoPath, featureId) {
    const repo = path.basename(repoPath || process.cwd());
    return buildTmuxSessionName(featureId, 'ev', { repo, desc: 'eval' });
}

function shouldRadarAutoEvalFeature(feature) {
    if (!feature) return false;
    if (feature.stage === 'in-evaluation') return false;
    const agents = Array.isArray(feature.agents) ? feature.agents : [];
    if (agents.length === 0) return false;
    const hasFleetAgents = agents.some(agent => agent.id && agent.id !== 'solo');
    if (!hasFleetAgents) return false;
    return agents.every(agent => agent.status === 'submitted');
}

const RADAR_INTERACTIVE_ACTIONS = new Set([
    'feature-create',
    'feature-prioritise',
    'feature-setup',
    'feature-do',
...
function resolveRadarActionRepoPath(requestedRepoPath, registeredRepos, defaultRepoPath = process.cwd()) {
    ...
    if (requested) {
        if (repos.length > 0 && !repos.includes(requested)) {
            return { ok: false, status: 403, error: 'repoPath is not registered with Radar' };
        }
...
function parseRadarActionRequest(payload, options = {}) {
    const data = payload && typeof payload === 'object' ? payload : {};
    const action = String(data.action || '').trim();
    if (!action) {
        return { ok: false, status: 400, error: 'action is required' };
    }
    if (!RADAR_INTERACTIVE_ACTIONS.has(action)) {
    ...
    const repoResolution = resolveRadarActionRepoPath(
        data.repoPath,
...
function buildRadarActionCommandArgs(action, args) {
    const actionName = String(action || '').trim();
    const actionArgs = Array.isArray(args) ? args.map(value => String(value)) : [];
    return [CLI_ENTRY_PATH, actionName, ...actionArgs];
}

function runRadarInteractiveAction(request) {
    const parsed = parseRadarActionRequest(request, {
    ...
    const cliArgs = buildRadarActionCommandArgs(parsed.action, parsed.args);
...
function runRadarServiceDaemon(port) {
    ...
    const globalConfig = loadGlobalConfig();
    const autoEvalEnabled = isRadarAutoEvalEnabled(globalConfig);
        ...
        (latestStatus.repos || []).forEach(repo => {
            (repo.features || []).forEach(feature => {
                (feature.agents || []).forEach(agent => {
                    ...
                    if (prev && prev !== 'waiting' && agent.status === 'waiting') {
                        sendMacNotification(`${agent.id} is waiting on #${feature.id} ${feature.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                    }
                    lastStatusByAgent[key] = agent.status;
                });

                const featureKey = `${repo.path}:${feature.id}`;
                if (shouldRadarAutoEvalFeature(feature) && !allSubmittedNotified.has(featureKey)) {
                    allSubmittedNotified.add(featureKey);
                    const evalSessionName = buildRadarFeatureEvalSessionName(repo.path, feature.id);
                    if (autoEvalEnabled) {
                        if (tmuxSessionExists(evalSessionName)) {
                            log(`Auto-eval skipped for #${feature.id} (${repo.path}): session exists (${evalSessionName})`);
                        } else {
                            try {
                                createDetachedTmuxSession(evalSessionName, repo.path, `aigon feature-eval ${feature.id}`);
                                log(`Auto-eval started for #${feature.id} (${repo.path}) via tmux session ${evalSessionName}`);
                                sendMacNotification(`Auto-eval started for #${feature.id} ${feature.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                            } catch (e) {
                                log(`Auto-eval failed for #${feature.id} (${repo.path}): ${e.message}`);
                                sendMacNotification(`Auto-eval failed for #${feature.id} ${feature.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                            }
                        }
                    } else {
                        sendMacNotification(`All agents submitted #${feature.id} ${feature.name} — ready for eval`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                        log(`Auto-eval disabled; notified ready-for-eval for #${feature.id} (${repo.path})`);
                    }
                }
            });
            ...
            (repo.research || []).forEach(item => {
                (item.agents || []).forEach(agent => {
                    ...
                    const prev = lastStatusByAgent[key];
                    if (prev && prev !== 'waiting' && agent.status === 'waiting') {
                        sendMacNotification(`${agent.id} is waiting on R#${item.id} ${item.name}`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                    }
                    lastStatusByAgent[key] = agent.status;
                });
                ...
                if (allSubmitted && !allSubmittedNotified.has(researchKey)) {
                    ...
                    sendMacNotification(`All agents submitted R#${item.id} ${item.name} — ready for synthesis`, `Aigon · ${repo.name || path.basename(repo.path)}`, { openUrl: dashboardUrl });
                    log(`All agents submitted for R#${item.id} (${repo.path})`);
                }
            });
        });
    ...
    const server = http.createServer((req, res) => {
        ...
        if (reqPath === '/api/worktree-open' && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                let payload = {};
                ...
                try {
                    ...
                    const sessionName = buildTmuxSessionName(featureId, agentId, { repo: path.basename(absRepo) });
                    const tmuxInfo = safeTmuxSessionExists(featureId, agentId);
                    const activeSession = tmuxInfo && tmuxInfo.running ? tmuxInfo.sessionName : null;
                    ...
                    if (activeSession) {
                        ...
                        // Build the agent startup command based on pipeline type
                        const agentCmd = isResearch
                            ? buildResearchAgentCommand(agentId, featureId)
                            : buildAgentCommand({ agent: agentId, featureId });
        ...
        if (reqPath === '/api/action' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                log(`Action: ${payload.action} ${(payload.args || []).join(' ')} | ok=${result.ok} exitCode=${result.exitCode !== undefined ? result.exitCode : 'n/a'}${result.stderr ? ' stderr=' + String(result.stderr).trim().slice(0, 120) : ''}`);
        ...
        if (reqPath === '/api/session/run' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    log(`Session run: ${command} | exitCode=${exitCode}${result.stderr ? ' stderr=' + String(result.stderr).trim().slice(0, 120) : ''}`);
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        ...
        if (reqPath === '/api/session/start' && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                let payload = {};
                ...
                const command = String(payload.command || '').trim();
                const cwd = String(payload.cwd || '').trim() || process.cwd();
                const label = String(payload.label || 'cmd').replace(/[^a-z0-9]/gi, '-').toLowerCase().slice(0, 20) || 'cmd';
                if (!command) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: 'command is required' })); return;
                }
                try {
                    const effectiveCwd = fs.existsSync(cwd) ? cwd : process.cwd();
                    const sessionName = `aigon-dash-${label}-${Date.now()}`;
                    createDetachedTmuxSession(sessionName, effectiveCwd, command);
                    log(`Session started: ${sessionName} | ${command}`);
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
    ...
    // Active relay state per session: sessionName → { outputFile, timer, socket }
    const terminalRelays = new Map();
    ...
    function wsParseFrames(buffer) {
        ...
        while (offset < buffer.length) {
            ...
            if (payloadLen === 126) {
                if (buffer.length - dataOffset < 2) break;
                payloadLen = buffer.readUInt16BE(dataOffset); dataOffset += 2;
            } else if (payloadLen === 127) {
                if (buffer.length - dataOffset < 8) break;
                payloadLen = Number(buffer.readBigUInt64BE(dataOffset)); dataOffset += 8;
            }
            const maskStart = masked ? dataOffset : -1;
            if (masked) dataOffset += 4;
            if (buffer.length - dataOffset < payloadLen) break;
            let data = buffer.slice(dataOffset, dataOffset + payloadLen);
            if (masked && maskStart >= 0) {
        ...
        return { frames, remaining: offset < buffer.length ? buffer.slice(offset) : Buffer.alloc(0) };
    }

    server.on('upgrade', (req, socket, head) => {
        const reqPath = (req.url || '/').split('?')[0];
        if (reqPath !== '/ws/terminal') {
        ...
        const sessionParam = (qs.match(/(?:^|&)session=([^&]*)/) || [])[1] || '';
        const sessionName = decodeURIComponent(sessionParam);

        if (!sessionName || !tmuxSessionExists(sessionName)) {
            socket.write('HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n');
            socket.destroy();
            log(`WS terminal: session not found: ${sessionName}`);
            return;
        }

        if (!wsHandshake(socket, req)) return;
        log(`WS terminal: connected to session ${sessionName}`);

        // Stop any existing relay for this session
        const existing = terminalRelays.get(sessionName);
        if (existing) {
            try { clearInterval(existing.timer); } catch (e) {}
            try { existing.socket.destroy(); } catch (e) {}
            try { runTmux(['pipe-pane', '-t', sessionName], { stdio: 'ignore' }); } catch (e) {}
            terminalRelays.delete(sessionName);
        }

        // Set up output capture file
        const safeSession = sessionName.replace(/[^a-zA-Z0-9_-]/g, '-');
        const outputFile = path.join(os.tmpdir(), 'aigon-term-' + safeSession + '.txt');
        try { fs.writeFileSync(outputFile, ''); } catch (e) {}

        // Snapshot is deferred until after first resize message (so pane dimensions match xterm)
        let snapshotSent = false;
        function sendSnapshot() {
            if (snapshotSent) return;
            snapshotSent = true;
            try {
                // No -e: plain text only, no absolute-position escape sequences that misformat at different sizes
                const snapResult = runTmux(['capture-pane', '-p', '-t', sessionName], { encoding: 'utf8', stdio: 'pipe' });
                if (!snapResult.error && snapResult.status === 0 && snapResult.stdout) {
                    // Convert \n → \r\n so xterm doesn't staircase the text
                    const clean = snapResult.stdout.replace(/\r?\n/g, '\r\n');
                    wsSend(socket, clean);
                }
            } catch (e) { /* ignore */ }
        }
        ...
        try {
            runTmux(['pipe-pane', '-O', '-t', sessionName, 'cat >> ' + shellQuote(outputFile)], { stdio: 'ignore' });
        } catch (e) {
            log(`WS terminal: pipe-pane failed for ${sessionName}: ${e.message}`);
        }
        ...
        const pollTimer = setInterval(checkNewOutput, 50);
        terminalRelays.set(sessionName, { outputFile, timer: pollTimer, socket });

        // Handle incoming WebSocket frames (keyboard input → tmux send-keys)
        let frameBuffer = Buffer.alloc(0);
        if (head && head.length > 0) frameBuffer = head;

        socket.on('data', (chunk) => {
            frameBuffer = Buffer.concat([frameBuffer, chunk]);
            const { frames, remaining } = wsParseFrames(frameBuffer);
            ...
            frames.forEach(frame => {
                ...
                if (frame.opcode === 0x09) { // ping → pong
                    const pongHeader = Buffer.from([0x8a, frame.data.length]);
                    try { socket.write(Buffer.concat([pongHeader, frame.data])); } catch (e) {}
                    return;
                }
                if ((frame.opcode === 0x01 || frame.opcode === 0x02) && frame.data.length > 0) {
                    const text = frame.data.toString('utf8');
                    // Check for JSON control messages (resize) vs raw terminal input
                    let handled = false;
                    if (text.startsWith('{')) {
                        try {
                            const msg = JSON.parse(text);
                            if (msg.type === 'resize' && msg.cols && msg.rows) {
                                runTmux(['resize-pane', '-t', sessionName, '-x', String(msg.cols), '-y', String(msg.rows)], { stdio: 'ignore' });
                                // Send snapshot after first resize so dimensions match xterm
                                setTimeout(sendSnapshot, 100);
                                handled = true;
                            } else if (msg.type === 'input') {
                                runTmux(['send-keys', '-t', sessionName, '-l', msg.data], { stdio: 'ignore' });
                                handled = true;
                            }
                        } catch (e) { /* not JSON, fall through */ }
                    }
                    if (!handled) {
                        try { runTmux(['send-keys', '-t', sessionName, '-l', text], { stdio: 'ignore' }); } catch (e) {}
                    }
                }
            });
        });
        ...
        function cleanup() {
            ...
            log(`WS terminal: disconnected from session ${sessionName}`);
        }

        socket.on('close', cleanup);
        socket.on('error', (e) => { log(`WS terminal error (${sessionName}): ${e.message}`); cleanup(); });
    });
...
module.exports = {
    ...
    GLOBAL_CONFIG_PATH,
    RADAR_DEFAULT_PORT,
    RADAR_DASHBOARD_PORT,
    RADAR_DYNAMIC_PORT_START,
    RADAR_APP_ID,
    RADAR_PID_FILE,
    RADAR_LOG_FILE,
    RADAR_META_FILE,
