...
// lib/utils.js — backward-compatible re-export hub (charter: < 200 LOC, no domain logic)
//
// Charter: this file is a re-export hub for modules that sit in a circular
// require chain (utils → dashboard-server → dashboard-status-collector →
// feedback → utils). It MUST use Object.assign(module.exports, {...}) and
// NEVER module.exports = {...}, otherwise modules that required this file
// during the cycle receive the original empty-object reference.
//
// What belongs here: feedback status constants, dev-server URL helper,
// safeWrite, setTerminalTitle — cross-cutting I/O primitives used by 3+
// command-level modules that do not fit any single domain module.
//
// What does NOT belong here: domain logic (analytics, hooks, spec CRUD,
// version, CLI parsing, deploy) — those live in their own lib/*.js modules.
...
const { safeWrite, safeWriteWithStatus } = require('./safe-write');
