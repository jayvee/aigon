...
function hasInstallManifest(repoPath) {
    try {
        ...
        // Corrupt manifest — treat as "no manifest" so we don't try to
        // regenerate on top of broken state. `doctor --fix` is the path.
        return false;
    }
}
...
function regenerateAgentInstructions(repoPath) {
    ...
    const stagePaths = ['.claude', '.cursor', '.gemini', '.agents', '.codex', '.aigon'];
