...
function isAllowedOrigin(origin, allowedHosts = []) {
    if (!origin) return false;
    if (isValidOrigin(origin)) return true;
    try {
        const u = new URL(origin);
        const okProto = u.protocol === 'http:' || u.protocol === 'https:';
        return okProto && isAllowedHost(u.hostname, allowedHosts);
    } catch (_) {
        return false;
    }
}
...
function parseCookies(cookieHeader) {
    ...
    String(cookieHeader || '').split(';').forEach(part => {
        ...
        try {
            ...
            // Malformed cookie input is untrusted; ignore the bad cookie instead
            // of throwing out of the request guard.
        }
    });
    return out;
}
...
function evaluateHttpSecurity(req, { token = null, allowedHosts = [] } = {}) {
    ...
    if (isStateChanging || isTokenMint) {
        ...
        if (origin) {
            if (!isAllowedOrigin(origin, allowedHosts)) return { ok: false, status: 403, error: 'Forbidden: cross-origin request' };
        } else if (referer) {
            if (!isAllowedOrigin(referer, allowedHosts)) return { ok: false, status: 403, error: 'Forbidden: cross-origin referer' };
        }
