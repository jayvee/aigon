...
async function handleLaunchCloseResolve(ctx) {
    ...
    if (lcf && lcf.kind === 'merge-conflict' && Array.isArray(lcf.conflictFiles) && lcf.conflictFiles.length > 0) {
        ...
        task = `A prior close attempt recorded incomplete criteria attestation (no longer blocking). Run \`aigon feature-close ${featureId}\` to retry close — attestation in the implementation log is optional.`;
    } else {
