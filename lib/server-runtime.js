...
function buildDashboardServerOptions(options = {}) {
    const { isPreview, repoRoot, appId, _isRestart, instanceId } = options;
    ...
    return {
        ...
        ...(repoRoot ? { defaultRepoPath: repoRoot } : {}),
        ...(appId ? { appId } : {}),
...
async function launchDashboardServer(params, deps) {
    ...
    runDashboardServer(port, instanceName, serverId, buildDashboardServerOptions({
        isPreview, repoRoot, appId, _isRestart: params._isRestart, instanceId: params.instanceId,
    }));
}
