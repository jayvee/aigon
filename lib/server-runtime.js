...
/**
 * Read dashboard-runtime.json for the effective AIGON_HOME (or os homedir).
 * Does not verify PID liveness — use getDashboardRuntimeEntry for that.
 * @param {{ instanceId?: string }} [options]
 * @returns {{ pid: number, port: number, startedAt?: string, version?: string }|null}
 */
function readDashboardRuntimeEntry(options = {}) {
    const fs = require('fs');
    const { getDashboardRuntimePath } = require('./global-config-migration');
    const runtimePath = getDashboardRuntimePath(options.instanceId);
    try {
        if (!fs.existsSync(runtimePath)) return null;
        const rt = JSON.parse(fs.readFileSync(runtimePath, 'utf8'));
        if (!rt || !rt.pid) return null;
        const { getConfiguredServerPort } = require('./config');
        return {
            pid: rt.pid,
            port: rt.port || getConfiguredServerPort(),
...
/**
 * Dashboard server registry entry scoped to the current AIGON_HOME and instance id.
 * Never infers foreign processes via lsof on the default port.
 * @param {{ isProcessAlive?: (pid: number) => boolean, instanceId?: string }} [deps]
 * @returns {{ pid: number, port: number, startedAt?: string, version?: string }|null}
 */
function getDashboardRuntimeEntry(deps = {}) {
    const isProcessAlive = deps.isProcessAlive || defaultIsProcessAlive;
    const entry = readDashboardRuntimeEntry({ instanceId: deps.instanceId });
    if (!entry || !isProcessAlive(entry.pid)) return null;
    return entry;
}

function buildDashboardServerOptions(options = {}) {
    const { isPreview, repoRoot, templateRoot, defaultRepoPath, appId, _isRestart, instanceId } = options;
    const { startSupervisorLoop, getSupervisorStatus } = require('./supervisor');
    const effectiveTemplateRoot = templateRoot || (isPreview ? repoRoot : null);
    const effectiveDefaultRepo = defaultRepoPath || repoRoot;
    return {
        ...(effectiveTemplateRoot ? { templateRoot: effectiveTemplateRoot } : {}),
        // Always pin the main repo path so the server never inherits a worktree cwd.
        ...(effectiveDefaultRepo ? { defaultRepoPath: effectiveDefaultRepo } : {}),
        ...(appId ? { appId } : {}),
...
async function resolveDashboardPort(dashCtx, instanceName, deps) {
    ...
    const preferred = hashBranchToPort(instanceName || identity.instanceId);
 ...
 */
function ensureDashboardCaddyRoute(port, deps, serverId = null) {
    ...
    try {
        ...
        addCaddyRoute(hostname, port, serverId ? `Dashboard: ${serverId}` : 'Dashboard');
    } catch (_) { /* non-fatal */ }
}
...
async function launchDashboardServer(params, deps) {
    ...
    if (proxyAvailable) {
        ensureDashboardCaddyRoute(port, deps, serverId || null);
    }
    runDashboardServer(port, instanceName, serverId, buildDashboardServerOptions({
        isPreview,
        repoRoot,
        templateRoot: params.templateRoot,
        defaultRepoPath: params.defaultRepoPath,
        appId,
        _isRestart: params._isRestart,
 ...
 * Used by `aigon apply` to verify that a launchd/systemd or manual respawn
