 ...
 */
function readDashboardRuntimeEntry() {
    const fs = require('fs');
...
/**
 * Dashboard server registry entry scoped to the current AIGON_HOME.
 * Never infers foreign processes via lsof on the default port.
 * @param {{ isProcessAlive?: (pid: number) => boolean }} [deps]
 * @returns {{ pid: number, port: number, startedAt?: string, version?: string }|null}
 */
function getDashboardRuntimeEntry(deps = {}) {
    const isProcessAlive = deps.isProcessAlive || defaultIsProcessAlive;
    const entry = readDashboardRuntimeEntry();
