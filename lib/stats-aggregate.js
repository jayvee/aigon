...
function buildAggregate(records) {
    ...
    for (const rec of records) {
        ...
        if (byAgent && typeof byAgent === 'object') {
            ...
            for (const [agentId, row] of agentEntries) {
                ...
                // Per-triplet rollup (feature 291): keyed on
                // `${agentId}|${model}|${effort}`. Uses modelOverride /
                // effortOverride when available; falls back to telemetry model.
                const tripletModel = row.modelOverride || row.model || '';
                const tripletEffort = row.effortOverride || row.effort || '';
                const tripletKey = `${agentId}|${tripletModel}|${tripletEffort}`;
                const tBucket = aggregate.perTriplet[tripletKey] || (aggregate.perTriplet[tripletKey] = {
                    agent: agentId, model: tripletModel || null, effort: tripletEffort || null,
                    features: 0, research: 0, cost: 0, sessions: 0,
                    inputTokens: 0, outputTokens: 0, thinkingTokens: 0,
                });
                if (entityType === 'feature') tBucket.features += 1;
                else tBucket.research += 1;
                tBucket.cost += Number(row.costUsd || 0);
                tBucket.sessions += Number(row.sessions || 0);
                tBucket.inputTokens += Number(row.inputTokens || 0);
                tBucket.outputTokens += Number(row.outputTokens || 0);
                tBucket.thinkingTokens += Number(row.thinkingTokens || 0);
            }
    ...
    for (const t of Object.values(aggregate.perTriplet)) t.cost = round(t.cost, 4);
...
/**
 * Read the cached aggregate without triggering a rebuild.
 * Returns the parsed cache object or null if no cache exists.
 * Use this in read-only paths (e.g. start-modal recommendation) that must
 * never block on a full stats rebuild.
 */
function readCachedAggregate(repoPath) {
    return safeReadJson(cachePath(repoPath));
}
