...
function writeNormalizedTelemetryRecord(record, options = {}) {
    ...
    const activity = record.activity || null; // 'implement' | 'evaluate' | 'review' | null
    const normalized = {
        schemaVersion: 1,
...
function captureSessionTelemetry(transcriptPath, options = {}) {
    ...
    if (process.env.AIGON_ACTIVITY) {
        ...
        let branch;
        try { branch = getCurrentBranch(); } catch (_) { /* use default */ }
        if (branch) {
            if (/\beval\b/.test(branch)) activity = 'evaluate';
