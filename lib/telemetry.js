...
/**
 * Read a telemetry JSON file transparently, supporting both plain .json and
 * gzipped .json.gz (produced by vault retention compression).
 */
...
/**
 * Normalise a filesystem path for comparison: resolve, strip trailing slash,
 * collapse double slashes. Both sides of a `.project_root` comparison must be
 * normalised — Gemini sometimes stores paths with trailing slashes while
 * `path.resolve` does not, so a raw equality check fails.
 */
 ...
 *
 * F453: SHA256 strategy was removed — Gemini does not hash with SHA256, so
 * Strategy 1 never matched. Strategy 2 (`.project_root` scan) is the viable
 * read path. Strategy 3 added as a timing-gap fallback for fresh bench
 * worktrees where `.project_root` has not yet been written.
 */
function resolveGeminiChatsDir(projectPath, options = {}) {
    const absPath = _normalisePath(projectPath);
    ...
    for (const d of allDirs) {
        ...
        try {
            ...
            const storedPath = _normalisePath(fs.readFileSync(projectRootFile, 'utf8'));
...
// ── Antigravity CLI conversation parsing ─────────────────────────────────────

const ANTIGRAVITY_CLI_ROOT = path.join(os.homedir(), '.gemini', 'antigravity-cli');

/**
 * Resolve the active Antigravity conversation id for a workspace path.
 * Antigravity maps cwd → conversation id in cache/last_conversations.json.
 */
...
function _antigravityQuery(dbPath, sql) {
...
/**
 * Parse token/step counts from an Antigravity conversation SQLite DB.
 * Step payloads are opaque blobs — we count rows as a turn proxy.
 */
function parseAntigravityConversationDb(dbPath) {
    ...
    const rows = _antigravityQuery(dbPath, 'SELECT COUNT(*) AS c FROM steps');
...
/**
 * Run a SQL query against the OpenCode SQLite DB via the `sqlite3` CLI in
 * JSON mode. Returns parsed rows (array of objects). Returns null when the
 * DB is missing or the binary is unavailable. We deliberately avoid taking
 * a `better-sqlite3` dependency just for this read path — the CLI binary is
 * available on macOS by default and on most CI Linux images.
 */
function _opencodeQuery(sql) {
    if (!fs.existsSync(OPENCODE_DB_PATH)) return null;
    try {
        const result = spawnSync('sqlite3', ['-readonly', '-json', OPENCODE_DB_PATH, sql], {
            encoding: 'utf8',
...
/**
 * Parse OpenCode session token usage from the SQLite database for a given
 * worktree. Aggregates assistant messages whose `data` JSON contains a
 * top-level `tokens` field. Writes one normalized telemetry record per
 * session (keyed by OpenCode session ID).
 *
 * @param {string} worktreePath
 * @param {Object} options
 * @param {number} [options.afterMs] - only include sessions created after this ms timestamp
 * @param {string|number} [options.featureId]
 * @param {string} [options.entityType]
 * @param {string} [options.repoPath]
 * @param {number} [options.linesChanged]
...
function parseOpenCodeDb(worktreePath, options = {}) {
    ...
    const absWorktree = _normalisePath(worktreePath);
    ...
    const projects = _opencodeQuery(
        `SELECT id FROM project WHERE worktree = '${_sqlEscape(absWorktree)}'`
    );
    if (projects && projects.length > 0) {
        const projectIds = projects.map(p => `'${_sqlEscape(p.id)}'`).join(',');
        sessions = _opencodeQuery(
    ...
    if (!sessions || sessions.length === 0) {
        sessions = _opencodeQuery(
            `SELECT id, time_created FROM session ` +
            `WHERE directory = '${_sqlEscape(absWorktree)}' AND time_created >= ${afterMs}`
        );
    }

    // Second fallback: git root = parent two levels up from worktree path (.aigon/worktrees/<seed>/<branch>)
    if (!sessions || sessions.length === 0) {
        const gitRoot = _normalisePath(path.resolve(absWorktree, '..', '..', '..', '..'));
        const rootProjects = _opencodeQuery(
            `SELECT id FROM project WHERE worktree = '${_sqlEscape(gitRoot)}'`
        );
        if (rootProjects && rootProjects.length > 0) {
            const rootProjectIds = rootProjects.map(p => `'${_sqlEscape(p.id)}'`).join(',');
            sessions = _opencodeQuery(
                `SELECT id, time_created FROM session ` +
                `WHERE project_id IN (${rootProjectIds}) ` +
                `AND directory = '${_sqlEscape(absWorktree)}' ` +
    ...
    for (const sess of sessions) {
        const messages = _opencodeQuery(
            `SELECT data, time_created FROM message ` +
            `WHERE session_id = '${_sqlEscape(sess.id)}' ` +
...
function captureAgentTelemetry(featureId, featureDesc, agentId, options = {}) {
    ...
    if (hasTranscript) {
        const aggregated = aggregateNormalizedTelemetryRecords(featureId, agentId, {
            ...
            linesChanged,
            afterMs: options.afterMs,  // scope to this session window; prevents cross-run accumulation
        });
        if (aggregated) {
    ...
    if (tStrat === 'gemini-transcript' && hasTranscript) {
        ...
        if (worktree) {
            ...
            if (options.afterMs != null) ggOpts.afterMs = options.afterMs;
            const transcriptData = parseGeminiTranscripts(worktree, ggOpts);
            if (transcriptData) Object.assign(result, transcriptData);
        }
    } else if (tStrat === 'antigravity-transcript' && hasTranscript) {
        const worktree = options.worktreePath || options.repoPath;
        if (worktree) {
            const agOpts = {
                featureId, entityType: 'feature',
            ...
            if (options.afterMs != null) agOpts.afterMs = options.afterMs;
            const transcriptData = parseAntigravityTranscripts(worktree, agOpts);
            if (transcriptData) Object.assign(result, transcriptData);
        }
    } else if (tStrat === 'opencode-db' && hasTranscript) {
        // OpenCode: read sessions/messages from ~/.local/share/opencode/opencode.db
        const worktree = options.worktreePath || options.repoPath;
        if (worktree) {
            const opOpts = {
                featureId, entityType: 'feature',
            ...
            if (options.afterMs != null) opOpts.afterMs = options.afterMs;
            const transcriptData = parseOpenCodeDb(worktree, opOpts);
            if (transcriptData) Object.assign(result, transcriptData);
        }
