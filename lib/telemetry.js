...
function findTranscriptFiles(featureId, featureDesc, options = {}) {
    ...
    const hasWorktree = !!(options.worktreePath || (options.repoPath && options.agentId));
...
function captureAgentTelemetry(featureId, featureDesc, agentId, options = {}) {
    ...
    // 2. Transcript-based telemetry (cc agents only)
    const isCcAgent = agentId === 'cc' || agentId === 'solo';
    if (isCcAgent) {
        const transcriptData = captureFeatureTelemetry(featureId, featureDesc, {
            agentId: isCcAgent ? agentId : undefined,
