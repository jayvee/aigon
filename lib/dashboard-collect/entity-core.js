...
const { getDefaultHost } = require('../agent-sessions');
...
function buildFeatureAgentRow(options) {
    ...
    return {
        ...
        // Only expose session names when the tmux pane is alive on this machine.
        // Sidecar names can sync across git-branch clones; peek/attach must not
        // surface for remote holders (see sanitizeForeignLeaseSessionAffordances).
        tmuxSession: tmux && tmux.running ? tmux.sessionName : null,
        tmuxRunning: tmux ? tmux.running : false,
        attachCommand: tmux && tmux.running ? `tmux attach -t ${tmux.sessionName}` : null,
...
function enrichReviewSessionsWithLiveness(absRepoPath, featureId, sessions, options = {}) {
    if (!Array.isArray(sessions)) return sessions;
    const sessionHost = options.sessionHost || getDefaultHost();
    return sessions.map(session => {
        let sessionRunning = Boolean(session.sessionRunning || session.running);
        if (session.session) {
            try {
                sessionRunning = sessionHost.isSessionAlive(session.session);
            } catch (_) {
                // Preserve the workflow fact when the local session host cannot be queried.
            }
        }
        const enriched = { ...session, sessionRunning };
        if (!sessionRunning || !session.agent || !session.session) return enriched;
        ...
        return { ...enriched, idleLadder, idleAtPrompt, idleAtPromptDetectedAt };
    });
}
