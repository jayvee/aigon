...
function collectDoneSpecs(doneDir, pattern, limit = 10, options = {}) {
    ...
    // Recent order: spec-file mtime descending — the spec is moved into
    // `05-done/` at close, so mtime ≈ close date (a just-closed older-id
    // feature sorts above recently-created ones). Numeric id descending is
    // the tiebreaker for equal mtimes. A statSync per file is far cheaper
    // than the JSON reads this path deliberately avoids.
    ...
    const sorted = files
        .map(file => {
            ...
            return {
                file,
                numId: m ? Number(m[1]) : -1,
                mtimeMs: safeStatMtimeMs(path.join(doneDir, file)),
            };
        })
        .sort((a, b) => b.mtimeMs - a.mtimeMs || b.numId - a.numId)
        .map(({ file }) => ({ file }));
