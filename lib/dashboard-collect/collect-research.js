...
function collectResearch(repoContext, response) {
    ...
    dedupedResearchSpecFiles.forEach(({ file, stage, dir: specDir }) => {
        ...
        research.push({
            ...
            // F678: lifecycle is a server fact the contract resolves state from.
            // Pre-start inbox/backlog rows have no snapshot but folder position
            // is authoritative; drift rows (engine dir, no snapshot) stay null.
            currentSpecState: snapshot
                ? (snapshot.currentSpecState || snapshot.lifecycle || null)
                : (!researchState.engineDirExists && ['inbox', 'backlog'].includes(effectiveStage)
                    ? effectiveStage
                    : null),
            stateRenderMeta: resolveStateRenderMeta(snapshot && (snapshot.currentSpecState || snapshot.lifecycle), snapshot),
    ...
    research.forEach((row) => {
        // F678: presentation needs a headline, the contract does not — inbox and
        // backlog rows have actions but no headline.
        if (row.cardHeadline) {
            row.cardPresentation = buildCardPresentation(row, { entityType: 'research' });
        }
        // A contract that cannot be built is a collector defect, not a reason to
        // hand the browser a row it has to guess about.
        try {
            row.uiContract = buildResearchUiContract(row, {});
        } catch (error) {
            throw new Error(`Research ${row.id} UI contract build failed: ${error.message}`);
        }
    });
