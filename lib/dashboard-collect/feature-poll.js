...
function collectFeatures(repoContext, response) {
    ...
    features.forEach((feature) => {
        if (!feature.cardHeadline) return;
        feature.cardPresentation = buildCardPresentation(feature, { entityType: 'feature' });
        // F678: a contract that cannot be built is a collector defect. Fail with
        // the entity id attached rather than shipping a row the browser must guess about.
        try {
