...
function collectFeatures(repoContext, response) {
    ...
    specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
        ...
        features.push({
            ...
            // F678: pre-start inbox/backlog rows have no snapshot but do have a
            // folder-derived lifecycle the contract must project. Drift rows
            // (engine dir, no snapshot) stay null and inert.
            currentSpecState: featureState.workflowSnapshot
                ? (featureState.workflowSnapshot.currentSpecState || featureState.workflowSnapshot.lifecycle || null)
                : (!featureState.engineDirExists && ['inbox', 'backlog'].includes(featureState.stage)
                    ? featureState.stage
                    : null),
