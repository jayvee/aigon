...
const { buildSetValidActions, resolveSetLifecycle } = require('../feature-set-workflow-rules');
const {
...
function buildSetDashboardCard(absRepoPath, summary, paths, specIndex) {
    ...
    const inboxMemberCount = Number(summary.counts && summary.counts.inbox) || 0;
    ...
    const activeNodes = graphNodes.filter(node => node.featureId && (node.state === 'in-progress' || node.state === 'in-review'));
    const activeMemberCount = activeNodes.length;
    const autonomousCurrentId = autonomous && autonomous.currentFeature
        ? String(autonomous.currentFeature)
        : null;
    const currentFeatureId = autonomousCurrentId || (activeNodes[0] && String(activeNodes[0].featureId)) || null;
    graphNodes.forEach((node) => {
        node.isCurrent = Boolean(currentFeatureId && String(node.featureId) === currentFeatureId);
    });
    const currentMember = currentFeatureId ? memberById.get(currentFeatureId) || null : null;
    const engineStatus = autonomous && autonomous.status
        ? String(autonomous.status)
        : (summary.isComplete ? 'done' : 'idle');
    const status = resolveSetLifecycle({
        status: engineStatus,
