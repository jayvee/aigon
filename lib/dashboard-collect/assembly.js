...
const { readDashboardPollRepos, getProjectConfigPath } = require('../config');
...
const { buildMonitorOperationalProjection } = require('../monitor-operational-projection');
...
function assembleRepoStatusAfterFeatures(absRepoPath, response, options, assemblyCtx) {
    ...
    try {
        const buildPendingScheduleIndex = scheduledKickoff.buildPendingScheduleIndex;
...
function collectDashboardStatusData(options = {}) {
    ...
    response.monitorOperational = buildMonitorOperationalProjection(response.repos);
    if (perfEnabled) {
