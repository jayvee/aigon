...
function getPreviewUrl(worktreePath) {
    ...
    return `http://localhost:${identity.port}`;
}
...
async function startPreviewForWorktree(worktreePath, options = {}) {
    ...
    const previewUrl = getPreviewUrl(worktreePath);

    if (existing && existing.pid && isProcessAlive(existing.pid)) {
        return { alreadyRunning: true, url: previewUrl, instanceId, port: existing.port, pid: existing.pid };
    }
    ...
    const healthy = await waitForServerHealthy(port, options.timeoutMs || 15000);
    const entry = getDashboardRuntimeEntry({ instanceId, isProcessAlive });
    ...
    return {
        ...
        url: previewUrl,
        instanceId,
        port: entry && entry.port ? entry.port : port,
