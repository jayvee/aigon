...
function buildSetValidActions(setState, options = {}) {
    ...
    if (!isComplete && remainingWork && inboxMemberCount === 0 && (status === 'idle' || status === 'stopped' || status === 'paused-on-quota')) {
        actions.push({
            action: 'set-autonomous-start',
            label: (status === 'stopped' || status === 'paused-on-quota') ? 'Resume (choose agents…)' : 'Start autonomous',
            priority: 'high',
            category: 'lifecycle',
            requiresInput: 'agentPicker',
        });
    ...
    if (status === 'running' && persistedAutonomous) {
        actions.push({
