...
function buildSetValidActions(setState, options = {}) {
    ...
    if (!isComplete && remainingWork && (status === 'idle' || status === 'stopped')) {
        actions.push({
            action: 'set-autonomous-start',
            label: status === 'stopped' ? 'Resume (choose agents…)' : 'Start set autonomously',
            priority: 'high',
            category: 'lifecycle',
            requiresInput: 'agentPicker',
        });
    ...
    if (!isComplete && remainingWork && persistedAutonomous && (status === 'paused-on-failure' || status === 'stopped')) {
        actions.push({
