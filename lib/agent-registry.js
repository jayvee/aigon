 ...
 */
function getModelOptions(agentId) {
    ...
    return getAllAgents()
        ...
        .map(agent => ({
            ...
            modelOptions: Array.isArray(agent?.cli?.modelOptions)
                ? agent.cli.modelOptions.map(o => ({ ...o }))
                : [],
            effortOptions: Array.isArray(agent?.cli?.effortOptions)
                ? agent.cli.effortOptions.map(o => ({ ...o }))
                : [],
            supportsModelPicker: Array.isArray(agent?.cli?.modelOptions) && agent.cli.modelOptions.length > 0,
