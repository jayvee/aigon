...
function getModelOptions(agentId, { includeQuarantined = false } = {}) {
    ...
    const shipped = agent?.cli?.modelOptions;
    if (!Array.isArray(shipped)) return [];

    const { loadProjectConfig, loadGlobalConfig } = require('./config');
    const projectCustom = _getCustomModelOptions(loadProjectConfig(), agentId);
    const globalCustom = _getCustomModelOptions(loadGlobalConfig(), agentId);

    const seen = new Set();
    const merged = [];
    for (const opt of [...projectCustom, ...globalCustom, ...shipped]) {
        if (!includeQuarantined && isModelOptionQuarantined(opt)) continue;
...
function _getCustomModelOptions(config, agentId) {
    ...
    return Array.isArray(arr) ? arr.filter(o => o && typeof o.value === 'string' && typeof o.label === 'string') : [];
}
...
function isKnownModelValue(agentId, value, { includeQuarantined = false } = {}) {
    ...
    const concreteOptions = getModelOptions(agentId, { includeQuarantined })
        ...
        .filter(Boolean);
    return concreteOptions.length === 0 || concreteOptions.includes(modelValue);
}
...
function getDashboardAgents(opts = {}) {
    ...
    if (globalConfig || projectConfig) {
        ...
        const builtIn = cfg.DEFAULT_GLOBAL_CONFIG;
    ...
    return getAllAgents()
        ...
        .map(agent => ({
            ...
            modelOptions: getModelOptions(agent.id)
                    .map(o => {
            ...
            effortOptions: Array.isArray(agent?.cli?.effortOptions)
                ...
                : [],
            supportsModelPicker: getModelOptions(agent.id).length > 0,
