...
function _stripJsoncComments(content) {
...
function _parseJsoncObject(content) {
    const withoutComments = _stripJsoncComments(content);
    ...
    for (let i = 0; i < chars.length; i++) {
        ...
        if (ch !== ',') continue;
        let nextIndex = i + 1;
        while (nextIndex < chars.length && /\s/.test(chars[nextIndex])) nextIndex++;
        if (chars[nextIndex] === '}' || chars[nextIndex] === ']') chars[i] = ' ';
    }
    return JSON.parse(chars.join(''));
}

function _findJsonArrayEnd(content, openIndex) {
    let depth = 0;
    let inString = false;
    let escaped = false;
    let lineComment = false;
    ...
    for (let i = openIndex; i < content.length; i++) {
        ...
        else if (ch === '/' && next === '*') { blockComment = true; i++; }
        else if (ch === '[') depth++;
...
function _formatJsonArrayAtIndent(values, indent) {
    return JSON.stringify(values, null, 2).replace(/\n/g, `\n${indent}`);
}

function _updateJsoncArrayProperty(content, key, values, parsed) {
    const source = String(content || '');
    const masked = _stripJsoncComments(source);
    const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const propertyPattern = new RegExp(`"${escapedKey}"\\s*:\\s*\\[`);
    const match = propertyPattern.exec(masked);
    if (match) {
        const propertyIndex = match.index;
        const openIndex = masked.indexOf('[', propertyIndex + match[0].indexOf('['));
        const closeIndex = _findJsonArrayEnd(source, openIndex);
        if (closeIndex < 0) throw new Error(`Invalid JSONC array for ${key}`);
        const lineStart = source.lastIndexOf('\n', propertyIndex) + 1;
        const indent = (source.slice(lineStart, propertyIndex).match(/^\s*/) || [''])[0];
        const replacement = _formatJsonArrayAtIndent(values, indent);
        return source.slice(0, openIndex) + replacement + source.slice(closeIndex + 1);
    }

    const openIndex = masked.indexOf('{');
    if (openIndex < 0) throw new Error('Invalid JSONC object');
    const newline = source.includes('\r\n') ? '\r\n' : '\n';
    const hasProperties = parsed && Object.keys(parsed).length > 0;
    const indent = '  ';
    const property = `${newline}${indent}"${key}": ${_formatJsonArrayAtIndent(values, indent)}${hasProperties ? ',' : ''}`;
    const needsClosingNewline = !hasProperties && source[openIndex + 1] !== '\n' && source[openIndex + 1] !== '\r';
    return source.slice(0, openIndex + 1) + property + (needsClosingNewline ? newline : '') + source.slice(openIndex + 1);
}
    ...
    switch (trust.type) {
        ...
        case 'json-array': {
            ...
            try {
                ...
                let originalContent = '';
                if (fs.existsSync(trustPath)) {
                    originalContent = fs.readFileSync(trustPath, 'utf8');
                    settings = _parseJsoncObject(originalContent);
                }
                ...
                if (changed) {
                    const nextContent = originalContent
                        ? _updateJsoncArrayProperty(originalContent, key, settings[key], settings)
                        : `${JSON.stringify(settings, null, 2)}\n`;
                    _safeWrite(trustPath, nextContent);
                    console.log(`\uD83D\uDD13 Pre-seeded ${agent.displayName || agent.name} workspace trust for worktree(s)`);
                }
            } catch (e) {
                console.warn(`\u26A0\uFE0F  Could not pre-seed ${agent.displayName || agent.name} trust: ${e.message}`);
