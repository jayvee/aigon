...
function ensureAgentTrust(agentId, paths) {
    ...
    switch (trust.type) {
        ...
        case 'vscode-settings-array': {
            // Adds the parent worktree directory to a JSON array setting (e.g. Cursor's trusted folders)
            try {
                let settings = {};
                if (fs.existsSync(trustPath)) {
                    settings = JSON.parse(fs.readFileSync(trustPath, 'utf8'));
                }
                if (!Array.isArray(settings[trust.key])) settings[trust.key] = [];
                // Trust the parent of wtBase (i.e. ~/.aigon/worktrees) so all repos are covered
                const parentDir = paths.length > 0 ? path.dirname(path.resolve(paths[0])) : null;
                if (parentDir && !settings[trust.key].includes(parentDir)) {
                    settings[trust.key].push(parentDir);
                    _safeWrite(trustPath, JSON.stringify(settings, null, 2));
                    console.log(`\uD83D\uDD13 Pre-seeded ${agent.displayName || agent.name} workspace trust for worktrees`);
                }
            } catch (e) {
                console.warn(`\u26A0\uFE0F  Could not pre-seed ${agent.displayName || agent.name} trust: ${e.message}`);
            }
