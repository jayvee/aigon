...
function validateModelOptions(agentConfig) {
    ...
    for (const opt of opts) {
        ...
        // §5 — quarantine record must carry its audit fields.
        if (opt.quarantined) {
            for (const field of ['since', 'reason', 'evidence']) {
                if (!opt.quarantined[field]) {
                    errors.push(`${where}: quarantined block is missing "${field}" (§5/§7)`);
                }
            }
        }
...
function getDefaultFleetAgents() {
    const agents = getLaunchableAgents().filter(a => a.defaultFleetAgent);
    if (agents.length === 0) return getLaunchableAgentIds().slice(0, 2); // fallback
