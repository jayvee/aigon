...
function collectFeatures(repoContext, response) {
    ...
    specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
        ...
        if (!parsed || !parsed.id || workflowFeatureIds.has(parsed.id)) return;
        const specPath = path.join(specDir, specFile);
        const { updatedAt: fallbackUpdatedAt, createdAt } = safeStatIsoTimes(specPath);
        const legacyAgents = [];
        if (stage === 'in-progress' || stage === 'in-evaluation') {
            const manifest = manifestsByFeatureId[parsed.id] || null;
            const statusAgents = safeReadDir(stateDir, file => new RegExp(`^feature-${parsed.id}-[a-z0-9]+\\.json$`).test(file))
                .map(file => {
                    const match = file.match(/^feature-\d+-([a-z0-9]+)\.json$/);
                    return match ? match[1] : null;
                })
                .filter(Boolean);
            const logAgents = safeReadDir(mainLogsDir, file => new RegExp(`^feature-${parsed.id}-[a-z0-9]+-.+-log\\.md$`).test(file))
                .map(file => {
                    const match = file.match(/^feature-\d+-([a-z0-9]+)-/);
                    return match ? match[1] : null;
                })
                .filter(Boolean);
            const candidates = [...new Set([
                ...((manifest && Array.isArray(manifest.agents)) ? manifest.agents : []),
                ...statusAgents,
                ...logAgents,
            ])].sort((a, b) => a.localeCompare(b));

            candidates.forEach(agent => {
                const statusFile = path.join(stateDir, `feature-${parsed.id}-${agent}.json`);
                const logFile = safeReadDir(mainLogsDir, file => file.startsWith(`feature-${parsed.id}-${agent}-`) && file.endsWith('-log.md'))
                    .map(file => path.join(mainLogsDir, file))[0];
                ...
                if (fs.existsSync(statusFile)) {
                    try {
                        ...
                        status = normalizeDashboardStatus(parsedStatus.status);
                        updatedAt = parsedStatus.updatedAt || updatedAt;
                        flags = parseStatusFlags(parsedStatus.flags);
                    } catch (_) { /* ignore */ }
                } else if (logFile && fs.existsSync(logFile)) {
                    try {
                        const fm = parseSimpleFrontMatter(fs.readFileSync(logFile, 'utf8'));
                        status = normalizeDashboardStatus(fm.status || status);
                ...
                legacyAgents.push({
                    ...
                    updatedAt,
                    flags,
                    slashCommand: status === 'waiting' ? `aigon terminal-focus ${String(parsed.id).padStart(2, '0')} ${agent}` : null,
                    tmuxSession: null,
                    tmuxRunning: false,
                    attachCommand: null,
                    worktreePath: null,
        ...
        const featureState = workflowReadModel.getFeatureDashboardState(absRepoPath, parsed.id, stage, legacyAgents);
        features.push({
            id: parsed.id,
            name: parsed.name,
            stage,
