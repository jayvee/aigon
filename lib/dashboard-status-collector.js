...
function computePendingCompletionSignal(dashboardStatus, fileStatus, taskType, snapshotAgentStatus, entityType) {
    ...
    if (signal === 'implementation-complete' || signal === 'research-complete') {
        ...
        if (['submitted', 'implementation-complete', 'research-complete',
            'addressing-review', 'feedback-addressed', 'revision-complete'].includes(eff)) return null;
    } else if (signal === 'revision-complete') {
        if (snapshotAgentStatus === 'ready') return null;
        if (['revision-complete', 'feedback-addressed'].includes(eff)) return null;
...
function buildFeatureAgentsFromSnapshot(options) {
    ...
    return Object.keys(snapshotStatuses)
        ...
        .map(agent => {
            ...
            // F405: escape hatch — show "Mark X complete" when agent has a session but
            // hasn't emitted the expected completion signal.
            ...
            row.pendingCompletionSignal = hasStatusFile
                ...
                : null;
            // isWorking: agent is actively running — suppress escape hatch.
            // Requires both liveness 'alive' AND a real heartbeat (heartbeatAgeMs non-null).
            // A session that just started (tmux alive, no heartbeat yet) has
            // heartbeatAgeMs=null and should still show the escape hatch so the
            // Playwright fixture setup works without needing to kill the session.
            // In practice the escape hatch is shown for dead/ended sessions (tmuxRunning=false).
            row.isWorking = row.liveness === 'alive' && row.heartbeatAgeMs !== null;
            return row;
        });
}
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        try {
            ...
            agents.forEach(agent => {
                ...
                const isDerivedReviewState = agent.status === 'addressing-review' || agent.status === 'feedback-addressed' || agent.status === 'revision-complete';
        ...
        if (id && isActiveStage) {
            ...
            agentList.slice().sort((a, b) => a.localeCompare(b)).forEach(agent => {
                ...
                agents.push({
                    ...
                    isWorking: researchLiveness ? (researchLiveness.liveness === 'alive' && researchLiveness.heartbeatAgeMs !== null) : false,
                });
                response.summary.total++;
...
function collectRepoStatus(absRepoPath, response) {
    ...
    try {
        const { buildPendingScheduleIndex } = require('./scheduled-kickoff');
