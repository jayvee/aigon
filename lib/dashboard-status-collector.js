...
function getSpecReviewEntries(repoPath, items) {
    ...
    (Array.isArray(items) ? items : []).forEach(item => {
        ...
        const snapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(repoPath, item.entityType, item.id);
        const specReview = snapshot && snapshot.specReview
            ? snapshot.specReview
            : { pendingCount: 0, pendingAgents: [], pendingLabel: '', pendingReviews: [] };
        entries.set(item.specPath, specReview);
    });
    return entries;
}
...
function applySpecReviewStatus(repoPath, features, research) {
    ...
    items.forEach(({ item, entityType }) => {
        ...
        const metadata = {
            ...
            pendingLabel: isDone ? '' : (record ? record.pendingLabel : ''),
        };
