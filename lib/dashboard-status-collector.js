...
function collectFeatures(repoContext, response) {
    ...
    if ((safeStat(backlogDir)?.mtimeMs || 0) !== tierCache.warm.backlogMtime) {
        tierCache.warm.backlogMtime = safeStat(backlogDir)?.mtimeMs || 0;
        tierCache.warm.backlog = listStageSpecFiles([
            { dir: backlogDir, stage: 'backlog', pattern: /^feature-\d+-.+\.md$/ },
        ]);
    }
    if ((safeStat(inboxDir)?.mtimeMs || 0) !== tierCache.warm.inboxMtime) {
        tierCache.warm.inboxMtime = safeStat(inboxDir)?.mtimeMs || 0;
        tierCache.warm.inbox = listStageSpecFiles([
            { dir: inboxDir, stage: 'inbox', pattern: /^feature-.+\.md$/ },
        ]);
    }
    if ((safeStat(pausedDir)?.mtimeMs || 0) !== tierCache.warm.pausedMtime) {
        tierCache.warm.pausedMtime = safeStat(pausedDir)?.mtimeMs || 0;
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        ...
        const autonomousSession = stage !== 'done' ? safeFeatureAutoSessionExists(featureId) : null;
    ...
    specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
        ...
        const autonomousSession = stage !== 'done' && /^\d+$/.test(String(featureId)) ? safeFeatureAutoSessionExists(featureId) : null;
        features.push({
            id: featureId,
            name: parsed.name,
            stage,
...
function collectFeedback(absRepoPath) {
    ...
    const doneDirMtime = safeStat(feedbackDoneDir)?.mtimeMs || 0;
    if (doneDirMtime !== tierCache.cold.feedbackDirMtime) {
        tierCache.cold.feedbackDirMtime = doneDirMtime;
        tierCache.cold.feedback = collectDoneSpecs(feedbackDoneDir, /^feedback-.+\.md$/);
    }
    const doneSpecs = tierCache.cold.feedback;
    doneSpecs.recent.forEach(({ file }) => {
        feedbackSpecFiles.push({ file, stage: 'done', dir: feedbackDoneDir });
    });
