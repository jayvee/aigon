...
function computeRebaseNeeded(worktreePath, defaultBranch) {
    ...
    try {
        const count = parseInt(execSync('git rev-list --count HEAD..' + defaultBranch, {
            cwd: worktreePath,
            encoding: 'utf8',
...
function collectFeatures(repoContext, response) {
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        ...
        if (stage === 'in-progress') {
            const worktreeAgent = agents.find(a => a.worktreePath);
            if (worktreeAgent) {
                rebaseNeeded = computeRebaseNeeded(worktreeAgent.worktreePath, defaultBranch);
            }
        }
