...
function mergeSpecReviewActions(entityType, item, metadata) {
    ...
    // Dedup helper — when the same (action, agentId) already exists, prefer the newer
    // spec-review entry (fresh label from synthetic context) over any stale copy.
    function mergeByKey(list, additions) {
        const byKey = new Map();
        (list || []).forEach(a => byKey.set(`${a.action}:${a.agentId || ''}`, a));
        additions.forEach(a => byKey.set(`${a.action}:${a.agentId || ''}`, a));
        return [...byKey.values()];
    }

    item.validActions = mergeByKey(item.validActions, extras);
    item.nextActions = mergeByKey(item.nextActions, extras);
...
function getSpecReviewEntries(repoPath, items) {
    // One-shot backfill from git history on first read. Safe no-op after marker is written.
    try { specReviewState.migrateFromGitHistory(repoPath); } catch (_) { /* non-fatal */ }

    const entries = new Map();
    (Array.isArray(items) ? items : []).forEach(item => {
        if (!item || !item.specPath || !item.id) return;
        const state = specReviewState.readState(repoPath, item.entityType, item.id);
        const { pendingCount, pendingAgents } = specReviewState.derivePending(state);
        entries.set(item.specPath, {
...
function applySpecReviewStatus(repoPath, features, research) {
    ...
    items.forEach(({ item, entityType }) => {
        ...
        const metadata = {
            pendingCount: isDone ? 0 : (record ? record.pendingCount : 0),
            pendingAgents: isDone ? [] : (record ? record.pendingAgents.slice() : []),
            pendingLabel: !isDone && record && record.pendingCount > 0
