...
function collectFeatures(repoContext, response) {
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        ...
        const featureState = featureDashboardModel.getFeatureDashboardModel(absRepoPath, featureId, stage, agents);
    ...
    specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
        ...
        const featureState = featureDashboardModel.getFeatureDashboardModel(absRepoPath, parsed.id, stage, []);
        features.push({
            id: parsed.id,
            name: parsed.name,
            stage,
