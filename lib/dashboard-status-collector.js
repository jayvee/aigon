...
function collectFeatures(repoContext, response) {
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        try {
            ...
            // F590: done features never get full enrichment on the poll path.
            // If inside the recent-N window, push the lean shape; otherwise omit
            // entirely (still reachable via /api/repos/all-features). This skips
            // the agent-row + detailFingerprint + cardHeadline work below for the
            // overwhelming majority of historical features (F459/F469 invariant).
            if (stage === 'done') {
                if (!recentDoneIds.has(String(featureId))) return;
                const doneIdentity = resolveFeatureIdentity(absRepoPath, featureId, manifestsByFeatureId, snapshot, specIndex);
                const doneSpecPath = doneIdentity.specPath || snapshot.specPath;
            ...
            agents.forEach(agent => {
                // Preserve runtime task statuses derived from the agent status
                // file in buildFeatureAgentRow. snapshotStatuses only carries
                // engine liveness (idle/running/ready/...), so without this
                // guard the override would clobber 'addressing-code-review',
                // 'reviewing', 'revision-complete' etc. back to 'ready' while
            ...
            features.push({
                ...
                // F492: the dashboard's stage track renderer reads
                // feature.autonomousPlan.stages directly. The summary above
                // only carries headline-level state, not per-stage data, so
                // emit the full plan (small — ~5 stages × ~80 bytes) when one
                // exists. Stage objects are already shaped for the renderer
                // by buildAutonomousStagePlan in workflow-read-model.js.
    ...
    // F590: every done feature outside the recent-N window, in the same lean
    // shape — used to build the full `allFeatures` list for the lazy All Items
    // view endpoint (no longer shipped on the poll path).
    ...
    // Full uncapped lean list for the All Items / Logs view (F67 contract).
    // F590: kept off the default /api/status payload — served on demand via
    // GET /api/repos/all-features. Active rows are projected to the lean shape;
    // done rows already are lean.
    const allFeatures = [
...
function collectRepoStatus(absRepoPath, response, options = {}) {
    ...
    try {
        ...
        // When Pro is installed, decorate features/research/sets with their next
    ...
    // F590: `allFeatures` is the full uncapped lean list (F67). It is no longer
    // shipped on the hot poll path — the All Items view fetches it on demand via
    // GET /api/repos/all-features (collectAllFeaturesLean). Strip it here so the
    // default /api/status payload stays lean.
    const { allFeatures: _omitAllFeatures, ...featureStatusForPoll } = featureStatus;
    const result = {
        path: absRepoPath,
        displayPath: absRepoPath.replace(os.homedir(), '~'),
        name: path.basename(absRepoPath),
        githubRemote: tierCache.cold.githubRemote,
        ...featureStatusForPoll,
        ...researchStatus,
        ...feedbackStatus,
