...
function collectFeatures(repoContext, response) {
    ...
    [...workflowFeatureIds].sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(featureId => {
        ...
        const autonomousSession = safeFeatureAutoSessionExists(featureId);
    ...
    specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
        ...
        const autonomousSession = /^\d+$/.test(String(featureId)) ? safeFeatureAutoSessionExists(featureId) : null;
        features.push({
            id: featureId,
            name: parsed.name,
            stage,
