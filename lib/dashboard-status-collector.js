...
function buildSetMemberState(member, snapshot, autoState, doneIds, blockedIds) {
    ...
    if (lifecycle === 'reviewing' || lifecycle === 'ready_for_review' || member.stage === 'in-evaluation') return 'in-review';
    if (lifecycle === 'implementing' || lifecycle === 'evaluating' || lifecycle === 'closing' || member.stage === 'in-progress') return 'in-progress';
...
function applySpecReviewFromSnapshots(repoPath, items) {
    ...
    (Array.isArray(items) ? items : []).forEach(entry => {
        ...
        const pendingReviewerSet = new Set((record.pendingReviews || []).map(r => r && r.reviewerId).filter(Boolean));
        const activeReviewers = isDone ? [] : (record.activeReviewers || [])
            .filter(r => r && r.agentId && !pendingReviewerSet.has(r.agentId))
            .map(r => r.agentId);
        const activeCheckers = isDone ? [] : (record.activeCheckers || [])
            .filter(r => r && r.agentId)
            .map(r => r.agentId);
        item.specReview = {
