...
function parseSpecReviewSubject(subject) {
    ...
    return {
        entityType: match[1],
        id: match[2],
        isAck: /^spec-review-check:/.test(String(subject || '')),
        isReview: /^spec-review:/.test(String(subject || '')),
    };
}
...
function getSpecReviewEntries(repoPath, items) {
    ...
    trackedItems.forEach(item => {
        ...
        entityIndex.set(entityKey(item.entityType, item.id), record);
        addAlias(record, item.relPath);
    });

    const ackLogOutput = safeGitRead(
        repoPath,
        "git log --all --format=%x1e%H%x1f%s --extended-regexp --grep='^spec-review-check:'"
    );
    ackLogOutput.split('\x1e').forEach(block => {
        const trimmed = block.trim();
        if (!trimmed) return;
        const [sha, subject] = trimmed.split('\x1f');
        if (!sha || !subject) return;
        const subjectInfo = parseSpecReviewSubject(subject);
        if (!subjectInfo || !subjectInfo.isAck) return;
        const record = entityIndex.get(entityKey(subjectInfo.entityType, subjectInfo.id));
    ...
    logOutput.split('\x1e').forEach(block => {
        ...
        if (subjectInfo) {
            const subjectRecord = entityIndex.get(entityKey(subjectInfo.entityType, subjectInfo.id));
            if (subjectRecord) touchedKeys.add(subjectRecord.key);
        }
        ...
        touchedKeys.forEach(key => {
            ...
            if (subjectInfo && subjectInfo.isAck) {
                record.closed = true;
                record.pending = [];
                record.pendingAgents = [];
                record.pendingCount = 0;
                [...record.aliases].forEach(alias => removeAlias(record, alias));
                return;
            }

            if (subjectInfo && subjectInfo.isReview) {
