...
function readBenchmarkTelemetryUsage({ repoPath, featureId, agentId }) {
    ...
    for (const file of files) {
        try {
            ...
            // Skip "no-data" records (cursor's 'no-telemetry-cursor' strategy
            // writes one of these per interactive session — all token fields
            // and costUsd are null). Letting them through would bump the
            // sessions count while contributing zero to every sum, masking
            // the "we don't know" state with a confident "$0.0000".
            const hasAnyTokenData = usage.input != null || usage.output != null
                || usage.cacheReadInput != null || usage.cacheCreationInput != null
                || usage.thinking != null || usage.total != null || usage.billable != null;
            if (!hasAnyTokenData && data.costUsd == null) continue;
...
/**
 * Guarantee the bench-mode gold-image fixture matches the current aigon version.
 *
 * Without this check, an out-of-date snapshot makes bench-mode seed-reset extract
 * a stale repo, and downstream commands (feature-reset, feature-start) either
 * fail outright (the 2026-05-20 cu sweep incident: 3/3 pairs failed at
 * feature-reset 07 because the gold image was built against aigon v2.65 while
 * the CLI had moved to v2.66) or — worse — succeed but produce plausible-looking
 * numbers from a fixture that no longer matches what real users have installed.
 *
 * Idempotent: returns immediately when the image is fresh, so calling it once
 * per pair in a sweep costs ~ms after the first rebuild.
 */
function ensureFreshGoldImage(seedName, deps = {}) {
    const seedResetMod = deps.seedReset || require('./commands/setup/seed-reset');
    const getVersion = deps.getInstalledAigonVersion
        || require('./commands/bench').getInstalledAigonVersion;
    const runShellFn = deps.runShellFn || spawnSync;
    const out = deps.out || process.stdout;

    const currentVersion = getVersion();
    const exists = seedResetMod.goldImageExists(seedName);
    const meta = exists ? seedResetMod.readGoldMeta(seedName) : null;
    const versionMatch = exists && meta && meta.aigonVersion === currentVersion;

    if (exists && versionMatch) return { action: 'noop', currentVersion };

    const reason = !exists
        ? `missing at ${seedResetMod.goldImagePath(seedName)}`
        : `stale (built with aigon v${(meta && meta.aigonVersion) || 'unknown'}, current v${currentVersion})`;
    out.write(`\n📦 Bench fixture ${seedName}: ${reason}\n`);
    out.write(`   Rebuilding via 'aigon bench-snapshot ${seedName}' — one-time per aigon version (~5-10 min)...\n\n`);

    const result = runShellFn('aigon', ['bench-snapshot', seedName], { stdio: 'inherit' });
    if (result.status !== 0) {
        throw new Error(
            `bench-snapshot ${seedName} failed (exit ${result.status}). `
            + `Cannot proceed with benchmark — fixture rebuild required. `
            + `Try 'aigon bench-snapshot ${seedName}' manually to see full output.`
        );
    }
    return { action: 'rebuilt', reason, currentVersion };
}

/**
 * Read Cursor's per-invocation usage capture from the bench-mode stream-json
 * dump. Returns null if the capture file is missing or unreadable.
 *
 * Cursor's `agent -p --output-format stream-json` writes one JSON object per
 * line; the final `{"type":"result","usage":{...}}` event aggregates token
 * counts across every internal model turn (thinking + tool_calls + final
 * message). Earlier turns don't carry usage, so we scan from the end.
 */
function readCursorBenchCapture(worktreePath) {
    if (!worktreePath) return null;
    const captureFile = path.join(worktreePath, '.aigon-cu-bench.jsonl');
    if (!fs.existsSync(captureFile)) return null;
    let lines;
    try { lines = fs.readFileSync(captureFile, 'utf8').split('\n'); } catch (_) { return null; }
    for (let i = lines.length - 1; i >= 0; i--) {
        const line = (lines[i] || '').trim();
        if (!line.startsWith('{')) continue;
        let ev;
        try { ev = JSON.parse(line); } catch (_) { continue; }
        if (ev.type !== 'result' || !ev.usage) continue;
        const u = ev.usage;
        return {
...
/**
 * Poll the cursor bench capture file until a `result` event with usage appears
 * (cursor writes it as the very last line before exit). Required because the
 * bench's poll for `implementation-complete` races ahead of cursor's exit —
 * the agent calls agent-status implementation-complete via bash mid-run, but
 * the model's final `usage` block isn't streamed to stdout until the whole
 * invocation finishes (often 20-40s later for the brewboard footer task).
 * Without this wait the override fires against an in-progress capture file
 * with no result event yet, and tokenUsage falls back to the all-zero stub.
 */
async function waitForCursorBenchCapture(worktreePath, { timeoutMs = 90000, pollMs = 1000 } = {}) {
    if (!worktreePath) return null;
    const deadline = nowMs() + timeoutMs;
    while (nowMs() < deadline) {
        const usage = readCursorBenchCapture(worktreePath);
        if (usage) return usage;
        await new Promise(r => setTimeout(r, pollMs));
    }
    return null;
...
/**
 * Compute USD cost from a cursor usage block + pricing in templates/agents/cu.json.
 * Returns null when the model lacks pricing (e.g. composer-2 has pricing: null).
 *
 * Cursor's pricing is per-million tokens (input/output). cu.json does not carry
 * a separate cached-input price, so we count cacheReadTokens at the same rate
 * as fresh input — slightly overestimates real spend but is honest about the
 * uncertainty (an explicit cached-input price would let us discount ~10x).
 */
function computeCursorCost(usage, model) {
    if (!usage || !model) return null;
    const agentRegistry = require('./agent-registry');
    const agent = agentRegistry.getAgent('cu');
    const opt = (agent?.cli?.modelOptions || []).find(o => o.value === model);
    const pricing = opt && opt.pricing;
    if (!pricing || pricing.input == null || pricing.output == null) return null;
    const inputMTok = (usage.inputTokens + (usage.cacheReadTokens || 0)) / 1_000_000;
    const outputMTok = usage.outputTokens / 1_000_000;
    return Math.round((inputMTok * pricing.input + outputMTok * pricing.output) * 10000) / 10000;
}
...
async function runBenchmark(opts = {}) {
    ...
    // Bench-mode cursor capture: cu's standard telemetry strategy is
    // 'no-telemetry-cursor' (writes all-null tokens because the interactive
    // TUI exposes nothing). In bench mode we redirect cursor through
    // `agent -p --output-format stream-json` which DOES carry per-invocation
    // usage; harvest it from the per-worktree capture file and override the
    // all-null record above. See buildRawAgentCommand's AIGON_BENCH_MODE
    // branch in lib/worktree.js for the launch side.
    if (agent === 'cu') {
        const wtPath = implementationArtifact && implementationArtifact.worktreePath;
        process.stdout.write(`💱 Waiting for cursor bench capture (up to 90s)...\n`);
        const cursorUsage = await waitForCursorBenchCapture(wtPath);
        if (cursorUsage) {
            tokenUsage = {
                inputTokens: cursorUsage.inputTokens,
                cachedInputTokens: cursorUsage.cacheReadTokens,
                freshInputTokens: Math.max(0, cursorUsage.inputTokens - cursorUsage.cacheReadTokens),
                outputTokens: cursorUsage.outputTokens,
                thinkingTokens: 0,
                totalTokens: cursorUsage.inputTokens + cursorUsage.outputTokens,
                billableTokens: cursorUsage.inputTokens + cursorUsage.outputTokens,
                sessions: 1,
                costUsd: computeCursorCost(cursorUsage, model),
                model,
            };
            process.stdout.write(`   captured: in=${cursorUsage.inputTokens} out=${cursorUsage.outputTokens} cache=${cursorUsage.cacheReadTokens}  cost=${tokenUsage.costUsd != null ? '$' + tokenUsage.costUsd : 'n/a (no pricing)'}\n`);
        } else {
            process.stdout.write(`⚠️  Cursor capture timed out — bench result will carry stub tokenUsage (no real cost data)\n`);
        }
    }
...
module.exports = {
    ...
    collectAllPairs,
    runBaseline,
    ensureFreshGoldImage,
    readCursorBenchCapture,
    waitForCursorBenchCapture,
    computeCursorCost,
