...
const COMPLETED_LEGACY_STATUSES = new Set([
    'submitted', 'ready', 'implementation-complete', 'revision-complete', 'research-complete',
]);

function collectIncompleteFeatureEvalAgents({ featureNum, worktrees = [], engineSnapshot = null }) {
    const incompleteAgents = [];
    worktrees.forEach(w => {
        // Engine snapshot is authoritative — agent marked 'ready' has submitted
        ...
        if (!COMPLETED_LEGACY_STATUSES.has(status)) {
...
async function run(args, deps) {
    ...
    for (const [agId, agState] of Object.entries(snapshot.agents)) {
        ...
        if (legacyStatus && COMPLETED_LEGACY_STATUSES.has(legacyStatus.status)) {
    ...
    const specFrom = findFile(PATHS.features, name, [STAGE_FOLDERS.IN_PROGRESS]);
    const specTo = specFrom
        ? path.join(PATHS.features.root, STAGE_FOLDERS.IN_EVALUATION, specFrom.file)
        : null;
    ...
    let found = findFile(PATHS.features, name, [STAGE_FOLDERS.IN_EVALUATION]);
    if (!found) {
        ...
        if (found) {
            if (specLifecycle.shouldMoveSpecFiles(repoPath)) {
                ...
                moveFile(found, STAGE_FOLDERS.IN_EVALUATION, null, { actor: 'cli/feature-eval' });
            } else {
                specLifecycle.refreshLifecycleView(repoPath, { warn: console.warn });
            }
            found = findFile(PATHS.features, name, [STAGE_FOLDERS.IN_EVALUATION]);
        }
...
## Spec
See: \`./docs/specs/features/${STAGE_FOLDERS.IN_EVALUATION}/${found.file}\`
