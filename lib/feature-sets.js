...
const { CANONICAL_STAGE_DIRS, STAGE_FOLDERS } = require('./workflow-core/paths');
...
function scanFeatureSets(paths = _defaultPaths(), specIndex = null) {
    ...
    if (resolvedIndex && Array.isArray(resolvedIndex.entries)) {
        resolvedIndex.entries.forEach(entry => {
            ...
            const folder = Object.keys(STAGE_BY_FOLDER).find(key => STAGE_BY_FOLDER[key] === entry.stage) || STAGE_FOLDERS.INBOX;
            const setEntry = {
                paddedId: entry.id,
                slug: entry.slug,
                file: entry.file,
...
function computeSetInboxPrioritisationOrder(setSlug, paths = _defaultPaths(), specIndex = null) {
    ...
    const inboxPeers = members.filter(m => m.stage === 'inbox');
    if (inboxPeers.length === 0) {
        throw new Error(`No inbox features in set "${setSlug}" to prioritise (set is empty or has no inbox members).`);
    }

    const peerKeys = new Set(inboxPeers.map(m => m.paddedId || m.slug));
    const peerSlugByKey = new Map(inboxPeers.map(m => [m.paddedId || m.slug, m.slug]));
    ...
    for (const m of inboxPeers) {
        const selfKey = m.paddedId || m.slug;
        let content;
        try {
        ...
        for (const ref of rawDeps) {
            ...
            const peerMatch = [...peerKeys].find((key) => key === norm || peerSlugByKey.get(key) === norm);
            if (!peerMatch) {
                const label = String(ref).trim();
                if (/^\d+$/.test(label)) {
                    throw new Error(`depends_on "${label}" in ${m.file} does not match any existing prioritised feature.`);
                }
                throw new Error(`depends_on "${label}" in ${m.file} is not another inbox member of set "${setSlug}" and does not match a backlog feature slug or id.`);
            }

            adj.get(peerMatch).push(selfKey);
            indegree.set(selfKey, (indegree.get(selfKey) || 0) + 1);
        }
    }

    const ready = [...peerKeys].filter(k => indegree.get(k) === 0).sort((a, b) => a.localeCompare(b));
    const order = [];
