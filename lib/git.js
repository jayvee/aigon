...
const AI_AGENT_IDS = ['cc', 'gg', 'cx', 'cu'];
const AI_AGENT_EMAIL_RE = /^(cc|gg|cx|cu)(?:\+[-\w.]+)?@aigon\.dev$/i;
const AIGON_NOTES_REF = 'aigon-attribution';

const COMMIT_CACHE_RELATIVE_PATH = path.join('.aigon', 'cache', 'commits.json');
const KNOWN_AGENT_IDS = new Set(['cc', 'gg', 'cx', 'cu', 'mv', 'solo']);
...
function getMainRepoPath(cwd) {
    ...
    // In a worktree, commonDir is an absolute path like /path/to/repo/.git
    if (commonDir && path.isAbsolute(commonDir)) {
        return path.dirname(commonDir);
    }
    // Not in a worktree — main repo is cwd
...
function _parseAgentIdFromEmail(email) {
    ...
    return AI_AGENT_IDS.includes(base) ? base : null;
}

function _isAgentEmail(email) {
    return !!_parseAgentIdFromEmail(email);
}
...
function _parseMessageAttribution(body) {
    ...
    while ((match = trailerRegex.exec(body)) !== null) {
        ...
        if (AI_AGENT_IDS.includes(id)) out.agentIds.add(id);
...
function _parseNoteAttribution(note) {
    ...
    lines.forEach(line => {
        ...
        raw.forEach(candidate => {
            const parsed = AI_AGENT_IDS.includes(candidate) ? candidate : _parseAgentIdFromEmail(candidate);
...
function _classifyCommitAttribution(commit, cwd, notesRef = AIGON_NOTES_REF) {
    ...
    const hasHumanSignal = !!authorEmail && !_isAgentEmail(authorEmail);
    const mixed = hasAISignal && (hasHumanSignal || messageSignals.hasNonAgentCoAuthor);
 ...
 * AI signals come from agent author emails, Aigon trailers, or Aigon git notes.
...
function _extractAttribution(commit) {
    const refs = [];
    const pushRef = (value) => {
        if (!value) return;
        String(value)
    ...
    if (!agent && commit.body) {
        ...
        while ((m = coAuthorRegex.exec(commit.body)) !== null) {
            ...
            const tokenMatch = line.match(/\b(cc|gg|cx|cu|mv)\b/);
            if (tokenMatch) {
                agent = tokenMatch[1];
                break;
            }
...
function _collectCommitHistory(cwd) {
    ...
    const pushCurrent = () => {
        if (!current) return;
        const attribution = _extractAttribution(current);
        commits.push({
            hash: current.hash,
