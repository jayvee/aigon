...
const { isEphemeralWorkingTreePath } = require('./ephemeral-paths');
...
function run(command, options = {}) {
    ...
    try {
        ...
        if (command.includes('git push')) {
            ...
            execSync(command, { stdio: 'inherit', ...spawnOptions });
        }
    } catch (e) {
        // REGRESSION: callers that tolerate failure (e.g. feature-close push to origin)
        // pass silent:true so the ❌ marker doesn't pollute stderr and trip the
        // dashboard's error-line extractor into reporting a successful close as failed.
        if (!silent) console.error('❌ Git command failed.');
        throw e;
    }
}
...
function getStatus(cwd) {
    try {
        ...
        return raw.split('\n').filter((line) => {
            ...
            if (line.match(/\.env(\.\w+)?$/)) return false;
            if (line.match(/test-results\//)) return false;
            const match = line.trimEnd().match(/^[ MADRCU?!]{1,2}\s+(.*)$/);
            const filePath = match
                ? (match[1].includes(' -> ') ? match[1].split(' -> ')[1] : match[1])
                : line;
            return !isEphemeralWorkingTreePath(filePath);
        }).join('\n').trim();
    } catch (e) {
        return '';
    }
...
function _extractAttribution(commit, cwd) {
    ...
    if (!agent && commit.body) {
        ...
        const agentTokens = require('./agent-registry').getAttributionAgentIds().join('|');
        const tokenRegex = new RegExp(`\\b(${agentTokens})\\b`);
        let m;
