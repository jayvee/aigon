...
function resolveDashboardActionRepoPath(requestedRepoPath, registeredRepos, defaultRepoPath = process.cwd()) {
    ...
    if (requested) {
        if (repos.length > 0) {
            if (!repos.includes(requested)) {
                return { ok: false, status: 403, error: 'repoPath is not registered with dashboard' };
            }
            return { ok: true, repoPath: requested };
        }
        // F672: fail closed when no repos are registered — a caller-supplied
        // repoPath is only honored if it matches the server's own default/cwd,
        // never an arbitrary attacker-chosen directory.
        if (defaultRepo && requested === defaultRepo) {
            return { ok: true, repoPath: requested };
        }
        return { ok: false, status: 403, error: 'repoPath is not registered with dashboard' };
    }
