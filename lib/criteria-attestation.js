...
function validateCriteriaAttestation(specPath, repoPath, featureId, options = {}) {
    ...
    if (!log) {
        if (options.snapshot) {
            const snapResult = validateCriteriaFromSnapshot(specPath, options.snapshot);
            if (snapResult.ok && !snapResult.skipped) {
                const snapAttestations = new Map();
                for (const entry of (options.snapshot.lastCriteriaAttestation.criteria || [])) {
                    if (entry && entry.index != null) {
                        snapAttestations.set(Number(entry.index), entry);
                    }
                }
                return {
                    ok: true,
                    skipped: false,
                    criteria,
                    attestations: snapAttestations,
                    unattested: [],
                    invalid: [],
                    counts: snapResult.counts,
                    log: null,
                    source: 'snapshot',
                };
            }
...
function formatCriteriaAttestationFailureMessage(criteria, unattested, invalid) {
    ...
    lines.push('Attestation is optional — close proceeds without it. Add lines above for an audit trail if you want one.');
    return lines.join('\n');
}

function formatCriteriaAttestationWarning(unattested) {
    return `Unattested acceptance criteria (${unattested.join(', ')}) — add ## Criteria Attestation lines in the implementation log (optional audit trail; does not block close).`;
}
...
/**
 * True when snapshot.lastCriteriaAttestation covers every spec criterion.
 */
function validateCriteriaFromSnapshot(specPath, snapshot) {
    const specContent = readSpecContent(specPath);
    const criteria = enumerateAcceptanceCriteria(specContent);
    if (criteria.length === 0) {
        return { ok: true, skipped: true, criteria, counts: { total: 0, met: 0, deferred: 0, dropped: 0 } };
    }
    const last = snapshot && snapshot.lastCriteriaAttestation;
    if (!last || !Array.isArray(last.criteria) || last.criteria.length === 0) {
        return { ok: false, criteria, unattested: criteria.map((c) => c.index), invalid: [] };
    }
    const attMap = new Map();
    for (const entry of last.criteria) {
        if (entry && entry.index != null) attMap.set(Number(entry.index), entry);
    }
    const unattested = [];
    const invalid = [];
    const counts = { total: criteria.length, met: 0, deferred: 0, dropped: 0 };
    for (const criterion of criteria) {
        ...
        if (!['met', 'deferred', 'dropped'].includes(status)) {
            invalid.push(criterion.index);
            continue;
        }
        counts[status] += 1;
...
/**
 * Pre-close readiness check (AutoConductor + CLI guards).
 */
function criteriaAttestationReady(specPath, repoPath, featureId, options = {}) {
    const result = validateCriteriaAttestation(specPath, repoPath, featureId, options);
    // Advisory only — never blocks feature-close or AutoConductor (F647 gate removed per operator policy).
