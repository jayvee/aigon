...
function computeStatusFingerprint(data) {
    ...
    (data.repos || []).forEach(repo => {
        ...
        features.forEach(f => {
            ...
            const escCount = Array.isArray(f.openEscalations) ? f.openEscalations.length : 0;
            parts.push('F' + f.id + ':' + (f.stage || '') + ':' + (f.currentSpecState || '') + ':' + (f.startupPhase || '') + ':' + agents + closeFail + ':' + escCount);
        });
        research.forEach(r => {
        ...
        (repo.sets || []).forEach(set => {
            ...
            if (!sr) return;
            parts.push('S:' + set.slug + ':' + (sr.running ? '1' : '0') + ':' + (sr.agent || '') + ':' + (sr.sessionName || ''));
        });
    });
    return parts.join('\n');
}
