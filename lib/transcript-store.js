 ...
 *
 * Copies native transcript body at feature-close so transcripts survive worktree
 * deletion and native log rotation.
 */
...
function _findTelemetryRecord(repoPath, entityType, entityId, agentId, sessionId) {
    ...
    for (const file of files) {
        ...
        let record;
        try { record = JSON.parse(readTelemetryFile(path.join(telemetryDir, file))); } catch (_) { continue; }
 ...
 * @param {string} [finalisedBy]  context label
