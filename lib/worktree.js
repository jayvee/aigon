...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    const injectionLines = wt._injectPromptFile ? [
        ...
        '    fi',
        '  done',
        '  echo "[inject] marker found, sleeping before paste" >> "$_aigon_inject_log" 2>/dev/null',
        '  sleep 1.5',
 ...
 */
const VALID_TMUX_ROLES = ['do', 'eval', 'review', 'revise', 'spec-review', 'spec-check', 'close', 'auto'];
...
function parseTmuxSessionName(name) {
    ...
    const roleMatch = name.match(/^(.+)-(f|r)(\d+)-(do|eval|review|spec-review|spec-check|close)-([a-z]{2})(?:-|$)/);
