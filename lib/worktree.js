...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    // Injection-via-tmux mode (op TUI, km term): launch the bare CLI; the prompt
    ...
    if (cliConfig.injectPromptViaTmux) {
        ...
        if (flags) {
            return `${cdPrefix}${launchEnvExports}${prefix}${cliConfig.command} ${flags}`;
        }
...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    // initial-prompt argument (op TUI, km term), buildRawAgentCommand stashes
    // the slash-command prompt file on wt._injectPromptFile. Sleep ~2.5s so
    // the TUI has time to render, then paste the prompt via tmux paste-buffer
    // (bracketed paste, `-p`) and press Enter. Runs in background so the
    // foreground rawCmd (the TUI) is unaffected.
    const injectionLines = wt._injectPromptFile ? [
        '(',
        '  sleep 2.5',
        '  _aigon_inject_session=$(tmux display-message -p "#{session_name}" 2>/dev/null)',
        '  if [ -n "$_aigon_inject_session" ]; then',
