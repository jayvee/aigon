...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    //
    // Kimi (`km`): `kimi --print` is one-shot (like `opencode run`); the wrapper
    // shell exits as soon as the model finishes and tmux tears the pane down,
    // taking the review output with it. Same `exec bash -l` post-hook keeps the
    // pane around for inspection.
        ...
        : canonicalAgentId === 'cu'
            ...
            : canonicalAgentId === 'km'
                ? [
                    ...
                    'trap - EXIT 2>/dev/null || true',
                    'echo ""',
                    'echo "Kimi finished (exit 0). Session kept open — scroll up to review output, run follow-up kimi commands, or type exit when done."',
                    'exec bash -l',
                ]
                : [rawCmd];
