...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    const featurePromptTemplates = {
        do: cliConfig.implementPrompt,
        eval: cliConfig.evalPrompt || cliConfig.implementPrompt,
        review: cliConfig.reviewPrompt || cliConfig.implementPrompt
    };
    ...
    if (isResearch) {
        ...
        const promptTemplate = featurePromptTemplates[normalizedTaskType];
        if (!promptTemplate) {
        ...
        if (normalizedTaskType === 'eval' || normalizedTaskType === 'review') prompt += ' --no-launch';
 ...
 * Handles new-style ({repo}-f{id}-{role}-{agent}(-desc)) and legacy ({repo}-f{id}-{agent}(-desc)) names.
 */
function parseTmuxSessionName(name) {
    ...
    // 3. Legacy fallback (no role prefix): {repo}-{type}{id}-{agent}(-desc) → role 'do'
