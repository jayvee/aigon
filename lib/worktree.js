...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    // to ~12s) for the TUI's ready marker, then paste the prompt via
    // bracketed-paste tmux paste-buffer and press Enter. Polling beats a
    // fixed sleep because cold-start times vary (opencode ~3-5s, kimi ~1s).
    const injectionLines = wt._injectPromptFile ? [
        ...
        '        | grep -qE "Ask anything|Welcome to Kimi|/help for help"; then',
        '      break',
        '    fi',
        '  done',
        '  sleep 0.5',
