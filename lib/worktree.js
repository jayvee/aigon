...
function _wfEngine() { return require('./workflow-core/engine'); }
...
function classifyOrphanReason(parsed, stageResult) {
    ...
    // For feature/research, check the engine snapshot — terminal lifecycle means orphaned
    if (entityType === 'feature' || entityType === 'research') {
        const TERMINAL = new Set(['done', 'closing']);
        if (TERMINAL.has(stageResult.stage)) return { reason: stageResult.stage };
        return null;
    }
