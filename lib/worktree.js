...
// F554: session naming/parsing now lives in lib/agent-sessions/names.js.
// worktree.js re-exports these for backwards compatibility; tmux session hosting
// moved behind the AgentSessionService + TmuxSessionHost.
...
function _isTmuxTranscriptCaptureEnabled() {
    return require('./config').isTmuxTranscriptCaptureEnabled();
}
function _getTmuxTranscriptOptions() {
    return require('./config').getTmuxTranscriptOptions();
}
function _resolveTranscriptEntityDir(repoPath, entityType, entityId) {
    return require('./transcript-store').resolveTranscriptEntityDir(repoPath, entityType, entityId);
}
...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    if (cliConfig.injectPromptViaTmux) {
        if (cliConfig.injectViaTmuxSkillCommand) {
            const agentCfg = _loadAgentConfig(wt.agent);
...
// toUnpaddedId, resolveTmuxRepoName, VALID_TMUX_ROLES, buildTmuxSessionName,
// buildResearchTmuxSessionName, and parseTmuxSessionName now live in
// lib/agent-sessions/names.js (imported + re-exported above) — F554.
...
// Agents with native transcript capture (session-strategy in agent-registry).
// Populated lazily to avoid loading agent-registry at module init.
let _capturableAgentsCache = null;
function _getCapturableAgents() {
...
/**
 * Write (once) the rotating pipe-pane wrapper script to ~/.aigon/scripts/.
 * Returns the absolute path to the script.
 *
 * The script reads stdin line-by-line, appends to LOG_FILE, and rotates when
 * the file exceeds CAP_BYTES. At most MAX_FILES rotated copies are kept.
 */
function _ensureTmuxRotateScript() {
...
/**
 * Attach a tmux pipe-pane to an existing session that writes all pane output
 * to a rotating log file.
 *
 * @param {string} sessionName
 * @param {string} logPath     absolute path to the .tmux.log file
 * @param {number} maxBytes    rotation threshold in bytes
 * @param {number} maxFiles    maximum rotated files to retain
 */
function _attachTmuxPipePane(sessionName, logPath, maxBytes, maxFiles) {
    try {
        fs.mkdirSync(path.dirname(logPath), { recursive: true });
    } catch (_) { /* ignore */ }

    const scriptPath = _ensureTmuxRotateScript();
    // pipe-pane -O: output only (not input). The sub-command runs with /bin/sh.
...
function _shouldAttachTmuxPipePane(meta, options = {}) {
    ...
    const tmuxCaptureEnabled = options.tmuxCaptureEnabled !== undefined
        ...
        : _isTmuxTranscriptCaptureEnabled();
    const capturableAgents = options.capturableAgents || _getCapturableAgents();
...
// matchTmuxSessionByEntityId now lives in lib/agent-sessions/names.js (F554).
...
function resolveTmuxBinary() {
    ...
    // Broken Homebrew symlink: standard bin paths missing (ENOENT) but Cellar still has a version.
...
// F637: every runTmux call is a short-lived query (has-session, list-*,
// capture-pane, display-message) or a fire-and-forget control (kill-session,
// send-keys, new-session -d) — none attach or block. A synchronous
// `tmux capture-pane` against a wedged/abandoned pane can hang indefinitely,
// which (via the supervisor sweep's liveness checks and the poll↔sweep
// coordination) freezes the dashboard's event loop hard. Bound every tmux spawn
// with a default timeout so a hung command is killed and surfaced as an error —
// callers already treat error/non-zero status as "session not alive", which is
// the correct degradation. Callers may still pass their own `timeout` to override.
...
/**
 * Post-launch transcript-capture side effects for a freshly created session.
 *
 * Extracted from createDetachedTmuxSession (F554) so the TmuxSessionHost can
 * invoke the same capture wiring without duplicating it. Binds the provider
 * transcript file to the sidecar and (opt-in) attaches a tmux pipe-pane for
 * agents without native transcript files.
 *
 * @param {string} sessionName
 * @param {object} meta - { repoPath, worktreePath, agent, entityType, entityId, role, createdAt }
 */
function attachSessionCapture(sessionName, meta) {
    ...
    // F430: opt-in tmux pipe-pane capture for agents without native transcript files.
    // AutoConductor sessions are Aigon controller output, not provider transcripts,
    // and must remain inspectable after the controller tmux session exits.
    if (_shouldAttachTmuxPipePane(meta)) {
        try {
            const { maxBytes, maxFiles } = _getTmuxTranscriptOptions();
            ...
            const agentDir = path.join(
                _resolveTranscriptEntityDir(path.resolve(meta.repoPath), entityTypeLong, meta.entityId),
                meta.agent
            );
            const logPath = path.join(agentDir, `${role}-${sessionUuid}.tmux.log`);
            _attachTmuxPipePane(sessionName, logPath, maxBytes, maxFiles);
            // Persist the log path in the sidecar so the read model can surface it.
            _getSessionSidecar().updateSessionSidecar(sessionName, path.resolve(meta.repoPath), { tmuxLogPath: logPath });
        } catch (_) { /* pipe-pane is best-effort */ }
    }
}

/**
 * F554 compatibility wrapper. Tmux session hosting now lives behind
 * AgentSessionService + TmuxSessionHost; this keeps the historical signature so
 * existing call sites (agent-resume, set-conductor, feature-autonomous,
 * dashboard-actions, …) migrate one at a time.
 *
 * The host owns tmux mechanics + capture wiring; the sidecar is written here via
 * the legacy writeSessionSidecarRecord so the on-disk shape (and all F351/F357
 * fields) is byte-for-byte preserved.
 *
 * @param {string} sessionName
 ...
 * @returns {{ tmuxId: string|null, shellPid: number|null }}
 */
function createDetachedTmuxSession(sessionName, cwd, command, meta) {
    const host = require('./agent-sessions/hosts').getDefaultHost();
    ...
    // Sidecar persistence kept on the legacy path so the shape is identical to
    // pre-F554 (the store's sidecar shape is validated separately; this wrapper
    // must not regress fields consumed by F351/F357 readers).
    if (meta && meta.repoPath) {
        ...
        // Capture wiring runs after the sidecar write so updateSessionSidecar
        // (pipe-pane path) finds the record. Only for entity sessions with an agent.
        if (meta.agent) {
