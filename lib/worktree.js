...
function buildAgentCommand(wt, taskType = 'implement') {
    ...
    const promptTemplate = taskType === 'evaluate' ? (cliConfig.evalPrompt || cliConfig.implementPrompt) : cliConfig.implementPrompt;
    let prompt = promptTemplate.replaceAll('{featureId}', wt.featureId);
    // When launching eval from dashboard, skip the CLI's launch mode (we're already in an agent session)
    if (taskType === 'evaluate') prompt += ' --no-launch';
