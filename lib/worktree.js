...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    const researchCommands = {
        ...
        'spec-check': 'research-spec-review-check',
    ...
    if (flags) {
        return `${cdPrefix}${launchEnvExports}${prefix}${cliConfig.command} ${flags} ${quotedPrompt}${stdinRedirect}`;
    }
    return `${cdPrefix}${launchEnvExports}${prefix}${cliConfig.command} ${quotedPrompt}${stdinRedirect}`;
}
...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    // OpenCode (`op`): `opencode run` exits when the batch turn completes, which
    // tears down the tmux pane immediately (wrapper shell exits). Record the
    // lifecycle signal on the agent CLI's exit code, then drop into an
    // interactive shell so operators can run follow-up `opencode` invocations
    // in the same pane. `sessions-close` / feature-reset still kill the tmux
    // session like any other agent.
    //
    // Cursor (`cu`): `agent` often stays in Composer after a review/implement
    // turn (waiting for follow-ups), so the EXIT trap never fires and
    // `agent-status` is never run. Same pattern as op: record lifecycle on the
    // agent process exit, then `exec bash -l`. `buildRawAgentCommand` appends
    // `< /dev/null` so `--print` runs tend to exit when the model is done.
    ...
    const agentInvocationLines = canonicalAgentId === 'op'
        ? [
            rawCmd,
            '_aigon_agent_rc=$?',
            ...
            'echo ""',
            'echo "OpenCode run finished (exit 0). Session kept open — run follow-up commands (e.g. opencode) or type exit when done."',
            'exec bash -l',
        ]
        : canonicalAgentId === 'cu'
            ? [
    ...
    const lines = [
        ...
        // Heartbeat sidecar
        ...heartbeatLines,
        // Agent CLI (OpenCode gets a post-run interactive shell — see above)
 ...
 */
const VALID_TMUX_ROLES = ['do', 'eval', 'review', 'spec-review', 'spec-check', 'auto'];
...
/**
 * Delete `.aigon/sessions/{name}.json` when no matching tmux session exists.
 * @param {string[]} repos - conductor repo paths
 * @param {Set<string>} liveSessionNames
 */
function pruneStaleSessionSidecars(repos, liveSessionNames) {
    for (const repo of repos) {
        ...
        for (const f of entries) {
            ...
            if (liveSessionNames.has(stem)) continue;
            try {
...
/**
 * Load sidecar records for live sessions (filename stem must match tmux session name).
 * @param {string[]} repos
 * @param {Set<string>} liveSessionNames
 * @returns {Map<string, object>}
 */
function loadSessionSidecarIndex(repos, liveSessionNames) {
    const map = new Map();
    for (const repo of repos) {
        const dir = path.join(path.resolve(repo), '.aigon', 'sessions');
        if (!fs.existsSync(dir)) continue;
        ...
        for (const f of entries) {
            ...
            try {
                ...
                if (raw.entityType !== 'f' && raw.entityType !== 'r') continue;
...
function parseEnrichedTmuxSessionRow(line) {
    ...
    if (text.includes(TMUX_SESSION_ROW_SEPARATOR)) {
        const [name, createdEpoch, attached] = text.split(TMUX_SESSION_ROW_SEPARATOR);
        return { name, createdEpoch, attached };
    }
    if (text.includes('\t')) {
        const [name, createdEpoch, attached] = text.split('\t');
        return { name, createdEpoch, attached };
    }
...
function parseEnrichedTmuxSessionsOutput(output, repos) {
    ...
    const preliminary = lines.map(line => {
        const { name, createdEpoch, attached } = parseEnrichedTmuxSessionRow(line);
    ...
    pruneStaleSessionSidecars(repos, liveNames);
    const sidecarBySession = loadSessionSidecarIndex(repos, liveNames);
    ...
    return preliminary.map(row => {
        ...
        const orphan = parsed ? classifyOrphanReason(parsed.type, parsed.id, stageResult) : null;
...
function getEnrichedSessions() {
    assertTmuxAvailable();
    const fmt = `#{session_name}${TMUX_SESSION_ROW_SEPARATOR}#{session_created}${TMUX_SESSION_ROW_SEPARATOR}#{session_attached}`;
...
/**
 * Persist tmux→entity mapping under `.aigon/sessions/{sessionName}.json`.
 * @param {object} meta - requires repoPath, entityType (f|r), entityId, role; optional agent, worktreePath, createdAt
...
function writeSessionSidecarRecord(meta) {
    ...
    const record = {
        sessionName,
        entityType: et,
        entityId: toUnpaddedId(String(meta.entityId)),
        agent: meta.agent != null ? String(meta.agent) : null,
        role: String(meta.role || 'do'),
    ...
    if (meta && meta.repoPath) {
        try {
            writeSessionSidecarRecord(Object.assign({}, meta, {
                sessionName,
                worktreePath: meta.worktreePath != null ? meta.worktreePath : cwd,
            }));
...
function openSingleWorktree(wt, agentCommand, terminalAppOverride = null) {
    try {
        ...
        const terminalAppName = process.platform === 'linux'
            ? 'tmux'
            : ({ warp: 'Warp', iterm2: 'iTerm2', 'apple-terminal': 'Terminal.app' }[terminalApp] || 'Terminal.app');
