...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    if (isResearch) {
        ...
        const promptText = normalizedTaskType === 'spec-review' || normalizedTaskType === 'spec-check'
            ...
            : resolveAgentPromptBody({
                ...
                extraArgs: (verb === 'eval' || verb === 'review') ? '--no-launch' : '',
                cliConfig,
            });
        if (wt.agent === 'cx') {
    ...
    if (flags) {
        return `${cdPrefix}${prefix}${cliConfig.command} ${flags} ${quotedPrompt}`;
    }
    return `${cdPrefix}${prefix}${cliConfig.command} ${quotedPrompt}`;
}
