...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    // Bench-mode cursor capture: Cursor's interactive TUI doesn't expose token
    // usage, but `agent -p --output-format stream-json` does (final result event
    // carries an aggregated usage block across all internal model turns).
    // Cursor only resolves slash commands in interactive mode — in -p mode they
    // pass through as literal text — so we inline the markdown body of the
    // slash command (same path cx/op use for headless prompts). Stdout is
    // redirected to a per-worktree capture file that perf-bench parses after
    // the implementation-complete signal to write a real telemetry record.
    // Only fires when AIGON_BENCH_MODE=1; interactive cursor (feature-open,
    // foreground feature-start) stays untouched.
    if (process.env.AIGON_BENCH_MODE === '1' && wt.agent === 'cu' && absPath && !isResearch) {
        const { resolveCxCommandBody } = require('./agent-prompt-resolver');
        const verbToCommand = { do: 'feature-do', eval: 'feature-eval', review: 'feature-code-review' };
        const benchCommand = verbToCommand[normalizedTaskType] || 'feature-do';
        const inlinedPromptText = resolveCxCommandBody(benchCommand, String(wt.featureId), wt.agent);
        const promptFile = _writeInlinePromptFile(wt, benchCommand, inlinedPromptText);
        const benchPrompt = `"$(< ${shellQuote(promptFile)})"`;
        const captureFile = path.join(absPath, '.aigon-cu-bench.jsonl');
        const benchFlags = `-p --output-format stream-json ${flags || ''}`.trim();
        return `${cdPrefix}${launchEnvExports}${prefix}${cliConfig.command} ${benchFlags} ${benchPrompt} > ${shellQuote(captureFile)} 2>&1`;
    }
...
function buildAgentCommand(wt, taskType = 'do') {
    ...
    const injectionLines = (wt._injectPromptFile || wt._injectSkillCommand) ? [
        ...
        '    fi',
        '  done',
        '  echo "[inject] marker found, sleeping before inject" >> "$_aigon_inject_log" 2>/dev/null',
        '  sleep 1.5',
        ...
        ...(wt._injectSkillCommand ? [
            ...
            '    echo "[inject] pasting buffer (no-bracket mode)" >> "$_aigon_inject_log" 2>/dev/null',
            '    tmux paste-buffer -b "$_aigon_inject_buf" -t "$_aigon_inject_session" -d 2>/dev/null',
            '    sleep 0.8',
        ]),
    ...
    const lines = [
        ...
        wt.path
            ? `aigon trust-worktree ${shellQuote(path.resolve(wt.path))} 2>/dev/null || true`
        ...
        `    _aigon_run_timed aigon agent-status ${successStatus} 2>/dev/null || true`,
        '  else',
        '    _aigon_run_timed env AIGON_EXIT_CODE="$_aigon_exit_code" AIGON_PANE_TAIL_B64="$_aigon_pane_tail" aigon agent-status error 2>/dev/null || true',
        '  fi',
        '}',
        // Install trap
...
function parseTmuxSessionName(name) {
    ...
    const roleMatch = name.match(/^(.+)-(f|r)(\d+)-(do|eval|review|spec-review|spec-revise|spec-check|close)-([a-z]{2})(?:-|$)/);
...
function createDetachedTmuxSession(sessionName, cwd, command, meta) {
    ...
    if (meta && meta.repoPath) {
        ...
        if (meta.agent && meta.entityType && meta.entityId
                && _isTmuxTranscriptCaptureEnabled()
                && !_getCapturableAgents().has(meta.agent)) {
            try {
