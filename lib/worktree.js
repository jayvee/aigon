...
function buildAgentCommand(wt, taskType = 'implement') {
    ...
    let prompt = promptTemplate.replaceAll('{featureId}', wt.featureId);
    ...
    // AIGON_TEST_MODEL env var overrides configured model (used by e2e test suite)
    const model = process.env.AIGON_TEST_MODEL || cliConfig.models?.[taskType];
 ...
 * @param {string} [taskType='do'] - 'do' or 'synthesize'
...
function buildResearchAgentCommand(agentId, researchId, taskType = 'do') {
    ...
    const prompt = `${cmdPrefix}${command} ${researchId}`;

    // Unset CLAUDECODE to prevent "nested session" error when launched from a Claude Code terminal
    const prefix = cliConfig.command === 'claude' ? 'unset CLAUDECODE && ' : '';

    // AIGON_TEST_MODEL env var overrides configured model (used by e2e test suite)
    const model = process.env.AIGON_TEST_MODEL || cliConfig.models?.['research'];
    const modelFlag = model ? `--model ${shellQuote(model)}` : '';
...
function parseTmuxSessionName(name) {
    // Match eval/review sessions: {repo}-f{id}-eval, {repo}-f{id}-review-{agent}
    const roleMatch = name.match(/^(.+)-(f|r)(\d+)-(eval|review-[a-z]{2})$/);
...
function matchTmuxSessionByEntityId(sessionName, entityId) {
    const unpadded = toUnpaddedId(entityId);
    // Match eval/review sessions: {anything}-f{id}-eval or {anything}-f{id}-review-{agent}
...
function setupWorktreeEnvironment(worktreePath, options) {
    ...
    // Always write PORT to .env.local — agents must never fall back to port 3000
    {
        ...
        console.log(`   \uD83D\uDCCB .env.local created${port ? ` with PORT=${port}` : ''}, banner vars${devUrl ? ', dev URL' : ''}`);
    }
