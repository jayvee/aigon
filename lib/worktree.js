...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    // pick up; strip the prompt AND any model flag from the launch string.
    // These agents manage model selection via their own TUI config — passing
    // --model with a snapshot-stored override breaks auth (e.g. kimi-k2.6 is
    // not a valid kimi CLI model identifier and causes "LLM not set").
    if (cliConfig.injectPromptViaTmux) {
        const m = String(prompt || '').match(/^\$\(<\s+(.+?)\s*\)$/);
        if (m) {
            wt._injectPromptFile = m[1].replace(/^'(.+)'$/, '$1').replace(/^"(.+)"$/, '$1');
        }
        // Strip --model <value> if present; TUI manages its own model.
        const injectFlags = flags ? flags.replace(/--model\s+\S+\s*/g, '').trim() : '';
        if (injectFlags) {
