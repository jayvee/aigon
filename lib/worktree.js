...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    if (isResearch) {
        ...
        if (wt.agent === 'cx') {
            ...
            const promptFile = _writeCxPromptFile(wt, command, promptText);
        ...
        const promptText = normalizedTaskType === 'spec-review' || normalizedTaskType === 'spec-check'
            ? resolveAgentCommandPrompt({
                agentId: wt.agent,
                commandName: normalizedTaskType === 'spec-review' ? 'feature-spec-review' : 'feature-spec-review-check',
    ...
    return agents.map(agent => {
        ...
        if (tmuxSessionExists(sessionName)) {
            if (restartExisting) {
                try { runTmux(['kill-session', '-t', sessionName], { stdio: 'ignore' }); } catch (_) { /* ignore */ }
                try {
                    createDetachedTmuxSession(sessionName, cwd, command);
