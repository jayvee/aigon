...
test('buildAgentCommand includes heartbeat sidecar', () => {
  ...
  assert.ok(cmd.includes('mkdir -p '), 'should create the heartbeat state dir');
});
...
test('buildAgentCommand writes worktree heartbeats into the main repo state dir', () => {
  const mainRepo = fs.mkdtempSync(path.join(os.tmpdir(), 'aigon-main-'));
  const worktree = fs.mkdtempSync(path.join(os.tmpdir(), 'aigon-worktree-'));
  fs.mkdirSync(path.join(worktree, '.aigon'), { recursive: true });
  fs.writeFileSync(
    path.join(worktree, '.aigon', 'worktree.json'),
    JSON.stringify({ mainRepo }, null, 2)
  );

  const cmd = buildAgentCommand({ agent: 'cc', featureId: '169', desc: 'test', path: worktree });
  const expectedStateDir = path.join(mainRepo, '.aigon', 'state');
  const expectedHeartbeat = path.join(expectedStateDir, 'heartbeat-169-cc');

  assert.ok(cmd.includes(expectedStateDir), 'heartbeat should use the main repo state dir');
  assert.ok(cmd.includes(expectedHeartbeat), 'heartbeat file should live in the main repo state dir');
});
