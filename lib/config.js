...
function detectActiveAgentSession() {
    ...
    const agentProcesses = _agentRegistry().getProcessDetectionMap();
...
function getDefaultGlobalConfig() {
    if (!_defaultGlobalConfigCache) {
        _defaultGlobalConfigCache = {
            ...
            agents: _agentRegistry().buildDefaultAgentConfigs(),
        };
    }
...
function isSameProviderFamily(agentA, agentB) {
    const families = _agentRegistry().getProviderFamilies();
...
function normalizeConfiguredAgentId(rawAgentId, { launchableOnly = false } = {}) {
    ...
    if (!_agentRegistry().getAllAgentIds().includes(normalized)) return null;
    if (launchableOnly && !_agentRegistry().isAgentLaunchable(normalized)) return null;
    return normalized;
}
...
function getDefaultAgent(repoPath = process.cwd()) {
    ...
    const builtInAgent = normalizeConfiguredAgentId(DEFAULT_GLOBAL_CONFIG.defaultAgent, { launchableOnly: true });
    if (builtInAgent) return builtInAgent;

    const registeredAgents = _agentRegistry().getLaunchableAgentIds();
...
function isAgentDisabled(agentId, repoPath = process.cwd()) {
    return require('./agent-availability').isAgentDisabled(agentId, repoPath);
}
...
function getAgentCliConfig(agentId, repoPath = process.cwd()) {
    ...
    for (const taskType of ['research', 'implement', 'evaluate', 'review']) {
        ...
        if (configured && !_agentRegistry().isKnownModelValue(agentId, configured)) {
            const fallback = DEFAULT_GLOBAL_CONFIG.agents?.[agentId]?.models?.[taskType] || null;
            if (fallback && _agentRegistry().isKnownModelValue(agentId, fallback)) {
...
module.exports = {
    ...
    get PROVIDER_FAMILIES() { return _agentRegistry().getProviderFamilies(); },
    // Config constants
    SPECS_ROOT,
    ROOT_DIR,
    CLI_ENTRY_PATH,
