...
function getAgentLaunchFlagTokens(command, flagValue, options = {}) {
    ...
    // Cursor CLI (`agent`): `--trust` only applies with headless `--print`.
    // Tmux-spawned sessions (feature-open, dashboard review, Fleet) have no TTY
    // to answer the trust prompt — inject both for that launch site only.
    if (command === 'agent' && cursorTmuxLaunch) {
        if (!tokens.some(t => t === '--print' || t === '-p')) {
            tokens = ['--print', ...tokens];
        }
        if (!tokens.includes('--trust')) {
            tokens.push('--trust');
        }
    }
