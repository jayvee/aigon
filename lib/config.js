...
/**
 * Core process-ancestry detection — injectable for unit testing.
 *
 * Two-pass strategy:
 *   Pass 1: exact commBase match wins at any ancestry depth.
 *   Pass 2: tokenized argv/path-basename fuzzy match (rejects partial-token
 *           hits, e.g. 'agent' inside 'agent-context' or 'AIGON_AGENT_ID').
 *
 * @param {Object} opts
 * @param {function(number): {commBase:string,argsRaw:string,ppid:number}|null} opts.getProcessInfo
 * @param {number}  opts.startPid      first PID to inspect (typically process.ppid)
 * @param {Object}  opts.agentProcesses  map of CLI command → { agentId, agentName }
 * @returns {{ detected: true, agentId: string, agentName: string }|null}
 */
function _detectActiveAgentSessionCore({ getProcessInfo, startPid, agentProcesses }) {
    const processHints = Object.entries(agentProcesses).map(([key, info]) => ({ key, info }));

    // Collect parent process chain: nearest (shallowest) first.
    const ancestry = [];
    try {
        let pid = startPid;
        ...
        // Silently ignore process detection failures
    }

    // Pass 1: exact commBase match — wins over any fuzzy match at any depth.
    for (const { commBase } of ancestry) {
        if (agentProcesses[commBase]) {
            return { detected: true, ...agentProcesses[commBase] };
        }
    }

    // Pass 2: tokenized argv/path-basename fuzzy match (nearest ancestor wins).
    // Split args on whitespace; include path.basename of each word so node-launched
    // agents (e.g. `node /usr/local/bin/claude`) still resolve. Whole-token equality
    // prevents 'agent' matching inside 'agent-context', 'AIGON_AGENT_ID', etc.
    for (const { argsRaw } of ancestry) {
        const tokens = new Set(
            argsRaw.split(/\s+/).flatMap(w => [w, path.basename(w)])
        );
        for (const hint of processHints) {
            if (tokens.has(hint.key)) {
...
function detectActiveAgentSession() {
    ...
    const agentProcesses = configAgentLayer.getProcessDetectionMap();
    ...
    function getProcessInfo(pid) {
        ...
        return { commBase: path.basename(commRaw), argsRaw, ppid: parseInt(ppidRaw, 10) };
    }

    const result = _detectActiveAgentSessionCore({
        getProcessInfo,
    ...
    if (!process.stdin.isTTY) {
        ...
        if (process.env.ANTIGRAVITY_TOKEN || process.env.AGY_TOKEN) {
            return { detected: true, agentId: 'ag', agentName: 'Antigravity CLI' };
        }
    }
...
const configAgentLayer = require('./config-agent-layer');
const agentAvailability = require('./agent-availability');

function _profilePlaceholders() {
    return require('./profile-placeholders');
}

function getDefaultGlobalConfig() {
    return configAgentLayer.buildDefaultGlobalConfig();
}
...
function isSameProviderFamily(agentA, agentB) {
    return configAgentLayer.isSameProviderFamily(agentA, agentB);
}
...
function getConfiguredServerPort() {
    try {
        ...
        if (identity.isPrimary) return DASHBOARD_DEFAULT_PORT;
...
const DEFAULT_GLOBAL_CONFIG = new Proxy({}, {
    get(_target, prop) {
        return getDefaultGlobalConfig()[prop];
    },
    ownKeys() {
        return Reflect.ownKeys(getDefaultGlobalConfig());
    },
    getOwnPropertyDescriptor(_target, prop) {
        return Object.getOwnPropertyDescriptor(getDefaultGlobalConfig(), prop);
    },
});
...
function loadGlobalConfig() {
    ...
    Object.keys(userConfig).forEach(key => {
        ...
        if (key === 'terminal') {
            // Legacy string form is consumed by migrateLegacyTerminalSettings; only the
            // new object form (e.g. { focusOnLaunch }) should reach the merged config.
...
// Profile presets live in lib/profile-placeholders.js — loaded lazily via _profilePlaceholders().
...
function normalizeConfiguredAgentId(rawAgentId, options) {
    return configAgentLayer.normalizeConfiguredAgentId(rawAgentId, options);
}

function getDefaultAgent(repoPath = process.cwd()) {
    return configAgentLayer.getDefaultAgent(repoPath);
}
...
function getConfigValueWithProvenance(key, repoPath = process.cwd()) {
    ...
    let globalValue = getNestedValue(rawGlobalConfig, key);
    if (globalValue === undefined && key === 'terminalApp') {
        globalValue = resolveCompatTerminalApp(rawGlobalConfig);
    }
...
// F521 — Settings scope model.
// USER_SCOPE_KEYS lists every dot-notation key that is "about the user, not the
// repo". A stale value in `.aigon/config.json` for one of these keys is *ignored*
// at resolution time (a server-start warning lists affected paths). The schema
// in lib/dashboard-server.js carries scope tags per entry; the schema-scope test
// asserts these two lists stay in sync.
const USER_SCOPE_KEYS = [
    'backgroundAgents',
    'terminalApp',
    'terminal.focusOnLaunch',
    'autoNudge.enabled',
    'autoNudge.idleVisibleSec',
    'autoNudge.idleAutoNudgeSec',
    'autoNudge.idleEscalateSec',
];

// agents.<id>.cli and agents.<id>.implementFlag are user-scope: "what's
// installed on my machine" is never a per-repo decision. These aren't in the
// dashboard schema (their UI lives elsewhere) but they share the resolver rule.
const USER_SCOPE_AGENT_SUBKEYS = new Set(['cli', 'implementFlag']);

function isUserScopeKey(keyPath) {
    if (!keyPath) return false;
    const resolved = resolveConfigKeyAlias(keyPath);
    if (USER_SCOPE_KEYS.includes(resolved)) return true;
    const m = resolved.match(/^agents\.[^.]+\.(cli|implementFlag)$/);
    return !!m;
}

// REPO_SCOPE_KEYS — meaningful only per-repo, no global default.
const REPO_SCOPE_KEYS = ['profile', 'devServer.enabled'];

function isRepoScopeKey(keyPath) {
    if (!keyPath) return false;
    return REPO_SCOPE_KEYS.includes(resolveConfigKeyAlias(keyPath));
}

/**
 * Returns the scope ('user' | 'shared' | 'repo') for a setting key.
 * Unknown keys default to 'shared' so legacy values keep working.
 */
function getSettingScope(keyPath) {
    if (isUserScopeKey(keyPath)) return 'user';
    if (isRepoScopeKey(keyPath)) return 'repo';
    return 'shared';
}

/**
 * Returns dot-notation paths in projectConfig that are user-scope and therefore
 * being ignored by the resolver. Used to surface a server-start warning so
 * operators can decide whether to clean up the stale entries.
 */
function listStaleUserScopeProjectOverrides(projectConfig) {
    const stale = [];
    if (!projectConfig || typeof projectConfig !== 'object') return stale;
    USER_SCOPE_KEYS.forEach(key => {
        const v = getNestedValue(projectConfig, key);
        if (v !== undefined) stale.push(key);
    });
    if (projectConfig.agents && typeof projectConfig.agents === 'object') {
        Object.entries(projectConfig.agents).forEach(([agentId, cfg]) => {
            if (!cfg || typeof cfg !== 'object') return;
            USER_SCOPE_AGENT_SUBKEYS.forEach(sub => {
                if (cfg[sub] !== undefined) stale.push(`agents.${agentId}.${sub}`);
            });
        });
    }
    return stale;
...
/**
 * After the project merge, reset every user-scope key in `merged` back to the
 * value it would have had with only global+default applied. We pre-compute that
 * value from `globalConfig` (which is `globalConfig.terminalApp` etc — *not* the
 * merged structure) and `DEFAULT_GLOBAL_CONFIG`.
 */
function applyUserScopeShortCircuit(merged, globalConfig) {
    USER_SCOPE_KEYS.forEach(key => {
        const gv = getNestedValue(globalConfig, key);
        const dv = getNestedValue(DEFAULT_GLOBAL_CONFIG, key);
        const resolved = gv !== undefined ? gv : dv;
        if (resolved === undefined) {
            // Project may have introduced an unknown user-scope key value; nuke it.
            ...
            setNestedValue(merged, key, resolved);
        }
    });
    if (merged.agents && typeof merged.agents === 'object') {
        Object.keys(merged.agents).forEach(agentId => {
            USER_SCOPE_AGENT_SUBKEYS.forEach(sub => {
                const gv = globalConfig.agents?.[agentId]?.[sub];
                const dv = DEFAULT_GLOBAL_CONFIG.agents?.[agentId]?.[sub];
                const resolved = gv !== undefined ? gv : dv;
                if (resolved === undefined) {
                    if (merged.agents[agentId] && typeof merged.agents[agentId] === 'object') {
                        delete merged.agents[agentId][sub];
                    }
                } else {
                    if (!merged.agents[agentId] || typeof merged.agents[agentId] !== 'object') {
                        merged.agents[agentId] = {};
                    }
                    merged.agents[agentId][sub] = resolved;
                }
            });
        });
    }
...
function getEffectiveConfig(repoPath = process.cwd()) {
    ...
    Object.keys(globalConfig).forEach(key => {
        if (key === 'tmuxApp' || key === 'terminalApp' || key === 'agents' || key === 'security') return;
        if (key === 'terminal' && (typeof globalConfig.terminal !== 'object' || globalConfig.terminal === null || Array.isArray(globalConfig.terminal))) {
            return;
        }
    ...
    Object.keys(projectConfig).forEach(key => {
        ...
        } else if (key === 'terminal' && projectConfig.terminal && typeof projectConfig.terminal === 'object' && !Array.isArray(projectConfig.terminal)) {
            merged.terminal = { ...(merged.terminal || {}), ...projectConfig.terminal };
        } else {
            merged[key] = projectConfig[key];
        }
    });

    // F521: user-scope keys ignore the project layer. We let the merge run
    // (cheaper than gating every branch) and then reset user-scope keys back
    // to the global-or-default value.
    applyUserScopeShortCircuit(merged, globalConfig);

    return merged;
}

function normalizeFailoverChain(raw) {
...
function getAgentFailoverConfig(repoPath = process.cwd(), snapshot = null) {
    const effective = getEffectiveConfig(repoPath);
    const configured = effective.agentFailover && typeof effective.agentFailover === 'object'
        ? effective.agentFailover
        : {};
    const snapshotFailover = snapshot && snapshot.agentFailover && typeof snapshot.agentFailover === 'object'
        ? snapshot.agentFailover
        : {};
    const limits = {
        perSessionBillableTokens: null,
        ...(configured.tokenLimits && typeof configured.tokenLimits === 'object' ? configured.tokenLimits : {}),
        ...(snapshotFailover.tokenLimits && typeof snapshotFailover.tokenLimits === 'object' ? snapshotFailover.tokenLimits : {}),
    };
    return {
        policy: String(snapshotFailover.policy || configured.policy || DEFAULT_GLOBAL_CONFIG.agentFailover.policy || 'notify').trim(),
        chain: normalizeFailoverChain(snapshotFailover.chain != null ? snapshotFailover.chain : configured.chain),
        tokenLimits: {
            perSessionBillableTokens: limits.perSessionBillableTokens != null
                ? Number(limits.perSessionBillableTokens)
                : null,
        },
    };
}
...
function showPortSummary() {
    const profile = _profilePlaceholders().getActiveProfile();
...
/** Dashboard-only: repos the operator hid in the UI (synced from browser localStorage). */
let _dashboardHiddenRepos = new Set();

function setDashboardHiddenRepos(repoPaths) {
    _dashboardHiddenRepos = new Set(
        (Array.isArray(repoPaths) ? repoPaths : [])
...
/**
 * Registered conductor repos minus dashboard-hidden ones. Background status polls
 * use this so a 16-repo registry does not scan 11 hidden repos on every cycle.
 */
function readDashboardPollRepos() {
    const all = readConductorReposFromGlobalConfig();
    if (_dashboardHiddenRepos.size === 0) return all;
    return all.filter((repoPath) => !_dashboardHiddenRepos.has(path.resolve(repoPath)));
}
...
/**
 * Return true when the operator has opted in to tmux pipe-pane transcript capture
 * for agents that have no native session file (cu, op, km).
 * Reads `transcripts.tmux` from the global config; project config cannot override
 * this flag (it is a machine-level privacy/security decision).
 */
function isTmuxTranscriptCaptureEnabled() {
    try {
        const cfg = loadGlobalConfig();
        const t = cfg && cfg.transcripts;
        return t && t.tmux === true;
    } catch (_) {
        return false;
    }
}
...
/**
 * Return the size cap (bytes) and max retained files for tmux pipe-pane logs.
 * @returns {{ maxBytes: number, maxFiles: number }}
 */
function getTmuxTranscriptOptions() {
    try {
        const cfg = loadGlobalConfig();
        const t = (cfg && cfg.transcripts) || {};
        return {
            maxBytes: (typeof t.tmuxMaxBytes === 'number' && t.tmuxMaxBytes > 0)
                ? t.tmuxMaxBytes
                : DEFAULT_GLOBAL_CONFIG.transcripts.tmuxMaxBytes,
            maxFiles: (typeof t.tmuxMaxFiles === 'number' && t.tmuxMaxFiles > 0)
                ? t.tmuxMaxFiles
                : DEFAULT_GLOBAL_CONFIG.transcripts.tmuxMaxFiles,
        };
    } catch (_) {
        return {
            maxBytes: DEFAULT_GLOBAL_CONFIG.transcripts.tmuxMaxBytes,
            maxFiles: DEFAULT_GLOBAL_CONFIG.transcripts.tmuxMaxFiles,
        };
    }
}
...
function isAgentDisabled(agentId, repoPath = process.cwd()) {
    return agentAvailability.isAgentDisabled(agentId, repoPath);
}
...
function getAgentCliConfig(agentId, repoPath = process.cwd()) {
    const agentConfig = configAgentLayer.getAgent(agentId);
    const globalConfig = loadGlobalConfig();
    const projectConfig = loadProjectConfig(repoPath);
    ...
    const cli = {
        ...
        planFlag: agentConfig?.cli?.planFlag ?? null,
        ...
        injectPromptViaTmux: agentConfig?.cli?.injectPromptViaTmux === true,
        injectViaTmuxSkillCommand: agentConfig?.cli?.injectViaTmuxSkillCommand === true,
    ...
    if (globalConfig.agents?.[agentId]) {
        ...
        if (globalConfig.agents[agentId].planFlag !== undefined) {
            cli.planFlag = globalConfig.agents[agentId].planFlag;
        }
    ...
    if (projectConfig.agents?.[agentId]) {
        ...
        if (projectConfig.agents[agentId].planFlag !== undefined) {
            cli.planFlag = projectConfig.agents[agentId].planFlag;
        }
    ...
    for (const taskType of ['research', 'implement', 'evaluate', 'review']) {
        const configured = cli.models[taskType];
        if (configured && !configAgentLayer.isKnownModelValue(agentId, configured)) {
            const fallback = DEFAULT_GLOBAL_CONFIG.agents?.[agentId]?.models?.[taskType] || null;
            if (fallback && configAgentLayer.isKnownModelValue(agentId, fallback)) {
                cli.models[taskType] = fallback;
            } else {
                delete cli.models[taskType];
            }
        }
    }
    ...
    const resolvedCli = agentRegistry.resolveAgentCliBinary(agentId, cli.command);
    if (resolvedCli) cli.command = resolvedCli;
...
function getAgentLaunchFlagTokens(command, flagValue, options = {}) {
    const { autonomous = false, cursorTmuxLaunch = false } = options;
    let tokens = parseCliFlagTokens(flagValue);

    // Cursor CLI (`agent`): run interactively so the tmux pane shows the native
    // Composer UI — thinking, tool calls, and responses exactly as they appear
    // when running `agent` directly on the command line. The agent calls
    // `aigon agent-status implementation-complete` from inside the slash command,
    // so the EXIT trap is not needed for lifecycle signalling. --print mode is
    // deliberately not used here: it produces either a blank screen (text format)
    // or machine JSON (stream-json format), neither of which is acceptable for
    // live tmux observation.

    // Codex has no persistent "yolo" config (unlike Gemini's approvalMode:yolo or
    // Claude Code's settings.json allowedTools). The bypass flag must be passed at
    // launch time. Aigon worktrees are externally sandboxed via git worktree
    // isolation + trusted project entries in ~/.codex/config.toml, so skipping
    // in-process approval prompts is safe for all aigon-managed sessions.
    if (command === 'codex') {
...
module.exports = {
    ...
    _detectActiveAgentSessionCore,
    printAgentContextWarning,
    normalizeMode,
    isSameProviderFamily,
    get PROVIDER_FAMILIES() { return configAgentLayer.getProviderFamilies(); },
    // Config constants
    SPECS_ROOT,
    ROOT_DIR,
    CLI_ENTRY_PATH,
    ...
    get PROFILE_PRESET_STRING_FILES() { return _profilePlaceholders().PROFILE_PRESET_STRING_FILES; },
    get PROFILE_PRESETS() { return _profilePlaceholders().PROFILE_PRESETS; },
    // Config functions
    loadGlobalConfig,
    loadProfilePresetStrings: (...args) => _profilePlaceholders().loadProfilePresetStrings(...args),
    loadProjectConfig,
    saveProjectConfig,
    saveGlobalConfig,
    safeBackupGlobalConfig,
    ...
    isRepoScopeKey,
    getSettingScope,
    listStaleUserScopeProjectOverrides,
    getAgentFailoverConfig,
    readBasePort,
    showPortSummary,
    ...
    writeRepoRegistry,
    // Profile detection
    detectProjectProfile: (...args) => _profilePlaceholders().detectProjectProfile(...args),
    getActiveProfile: (...args) => _profilePlaceholders().getActiveProfile(...args),
    getProfilePlaceholders: (...args) => _profilePlaceholders().getProfilePlaceholders(...args),
    resolveTestingPlaceholders: (...args) => _profilePlaceholders().resolveTestingPlaceholders(...args),
    resolveLoggingPlaceholders: (...args) => _profilePlaceholders().resolveLoggingPlaceholders(...args),
    resolveDevServerPlaceholders: (...args) => _profilePlaceholders().resolveDevServerPlaceholders(...args),
    resolvePlanModePlaceholders: (...args) => _profilePlaceholders().resolvePlanModePlaceholders(...args),
    resolveDocumentationPlaceholders: (...args) => _profilePlaceholders().resolveDocumentationPlaceholders(...args),
    resolveInstructionDirectives: (...args) => _profilePlaceholders().resolveInstructionDirectives(...args),
    computeInstructionsConfigHash: (...args) => _profilePlaceholders().computeInstructionsConfigHash(...args),
    // Applied digest — content-based drift detection (F497)
    get APPLIED_DIGEST_FILE() { return _profilePlaceholders().APPLIED_DIGEST_FILE; },
    get DIGEST_CATEGORY_LABELS() { return _profilePlaceholders().DIGEST_CATEGORY_LABELS; },
    computeAppliedDigest: (...args) => _profilePlaceholders().computeAppliedDigest(...args),
    computeAppliedDigestDetailed: (...args) => _profilePlaceholders().computeAppliedDigestDetailed(...args),
    readAppliedDigest: (...args) => _profilePlaceholders().readAppliedDigest(...args),
    writeAppliedDigest: (...args) => _profilePlaceholders().writeAppliedDigest(...args),
    buildDriftSummary: (...args) => _profilePlaceholders().buildDriftSummary(...args),
    getRecoveryConfig,
    ...
    getModelProvenance,
    isTmuxTranscriptCaptureEnabled,
    getTmuxTranscriptOptions,
};
