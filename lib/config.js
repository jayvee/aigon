...
function getAgentLaunchFlagTokens(command, flagValue, options = {}) {
    ...
    // Cursor tmux launches stay interactive so operators can watch and
    // interact with the agent in real time. Print mode is reserved for
    // explicit headless automation, not the live tmux session path.
    //
    // We still allow the caller-supplied Cursor flags through untouched so
    // repo config can decide whether `--trust` belongs on a given launch.
