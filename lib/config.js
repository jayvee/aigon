...
const DEFAULT_GLOBAL_CONFIG = {
    terminal: 'terminal',  // Terminal emulator: 'warp', 'terminal', 'tmux'
            ...
            if (!raw || raw === '{}' || raw === 'null' || raw === 'undefined') {
                ...
                if (fs.existsSync(GLOBAL_CONFIG_BACKUP_LATEST_PATH)) {
                    try {
                        ...
                        if (parsed && typeof parsed === 'object' && (parsed.repos || parsed.agents || parsed.terminal)) {
...
function saveGlobalConfig(config) {
    ...
    if (!config.repos && !config.agents && !config.terminal) {
        throw new Error('Refusing to save global config — missing all expected keys (repos, agents, terminal). This looks like a bug. Backup at: ~/.aigon/backups/config.latest.json');
    }
