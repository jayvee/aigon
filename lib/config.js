...
function normalizeConfiguredAgentId(rawAgentId) {
    const normalized = String(rawAgentId || '').trim().toLowerCase();
    if (!normalized) return null;
    return agentRegistry.getAllAgentIds().includes(normalized) ? normalized : null;
}

function getDefaultAgent(repoPath = process.cwd()) {
    const projectConfig = loadProjectConfig(repoPath);
    const projectAgent = normalizeConfiguredAgentId(projectConfig.defaultAgent);
    if (projectAgent) return projectAgent;

    const globalConfig = loadGlobalConfig();
    const globalAgent = normalizeConfiguredAgentId(globalConfig.defaultAgent);
    if (globalAgent) return globalAgent;

    const builtInAgent = normalizeConfiguredAgentId(DEFAULT_GLOBAL_CONFIG.defaultAgent);
    if (builtInAgent) return builtInAgent;

    const registeredAgents = agentRegistry.getAllAgentIds();
