...
const DEFAULT_GLOBAL_CONFIG = {
    ...
    terminal: {
        // 'background' (default) launches new terminal tabs/windows without stealing focus
        // from the dashboard or whatever the user is currently looking at. 'foreground'
        // restores the legacy behaviour of activating the terminal app on every launch.
        focusOnLaunch: 'background',
    },
    ...
    // F446 mid-run quota handling — default is pause-and-wait; other modes reserved for follow-ups
    quotaPolicy: {
        mode: 'pause-and-wait',
    },
    autoNudge: {
    ...
    // F430: opt-in tmux pipe-pane capture for agents without native transcript files (cu, op, km).
    // Off by default — raw ANSI noise, secrets-in-pane risk, and 5-10× size vs native logs.
    transcripts: {
        tmux: false,
        tmuxMaxBytes: 104857600,  // 100 MB per file before rotation
        tmuxMaxFiles: 3,          // retain at most 3 rotated files
    },
};
...
function getAgentCliConfig(agentId, repoPath = process.cwd()) {
    ...
    for (const taskType of ['research', 'implement', 'evaluate', 'review']) {
        ...
        if (configured && !agentRegistry.isKnownModelValue(agentId, configured)) {
            const fallback = DEFAULT_GLOBAL_CONFIG.agents?.[agentId]?.models?.[taskType] || null;
            if (fallback && agentRegistry.isKnownModelValue(agentId, fallback)) {
