...
function readFeedbackDocument(fileObj) {
    ...
    const fallbackStatus = utils.FEEDBACK_FOLDER_TO_STATUS[fileObj.folder] || 'inbox';
...
function collectFeedbackItems(folders = utils.PATHS.feedback.folders) {
    const items = [];

    folders.forEach(folder => {
        const folderPath = path.join(utils.PATHS.feedback.root, folder);
        if (!fs.existsSync(folderPath)) return;

        const files = fs.readdirSync(folderPath)
            .filter(file => file.startsWith(`${utils.PATHS.feedback.prefix}-`) && file.endsWith('.md'))
            .sort();
