...
const { isEntityDone, readSnapshotSync } = require('./workflow-core/entity-lifecycle');
...
function checkUnmetDependencies(specPath, featurePaths) {
    ...
    try {
        ...
        for (const dep of data.depends_on) {
            ...
            if (isEntityDone(repoPath, 'feature', resolvedId, folderFallback)) continue;
            // Unmet — derive the stage label engine-first for accurate reporting.
            const snapshot = readSnapshotSync(repoPath, 'feature', resolvedId);
            const stage = snapshot
                ? String(snapshot.currentSpecState || snapshot.lifecycle || 'unknown').toLowerCase()
                : (entry ? entry.folder.replace(/^\d+-/, '') : 'unknown');
