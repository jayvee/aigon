...
const { readSnapshotSync, isEntityDone } = require('./workflow-core/entity-lifecycle');
 ...
 */
function buildFeatureIndex(paths, specIndex = null) {
    if (specIndex && Array.isArray(specIndex.entries)) {
        ...
        specIndex.entries.forEach(entry => {
            ...
            index.byId[entry.id] = node;
            index.byId[String(parseInt(entry.id, 10))] = node;
            index.bySlug[entry.slug] = node;
        });
        return index;
    }
    const index = { byId: {}, bySlug: {} };
 ...
 */
function buildDependencyGraph(paths, utils = {}, featureIndex = buildFeatureIndex(paths), specIndex = null) {
    const parse = parseFrontMatter;
    const graph = new Map();

    if (specIndex && Array.isArray(specIndex.entries)) {
        specIndex.entries.forEach(entry => {
            if (!entry || !entry.id || !entry.frontmatterRaw) return;
            const deps = Array.isArray(entry.frontmatterRaw.depends_on)
                ? entry.frontmatterRaw.depends_on
...
/**
 * F454: async, chunked variant of buildDependencyGraph.
 *
 * Yields to the event loop every `chunkSize` files via `setImmediate`. The
 * sync version reads ~700 spec files in a single tick; on the dashboard host
 * that holds the loop long enough that `/api/spec`, `/api/status`, etc. stall
 * behind it. This variant drains between chunks so HTTP requests stay
 * responsive while a stale dep-graph cache rebuilds in the background.
 *
 * Same return shape and semantics as `buildDependencyGraph`.
 */
async function buildDependencyGraphAsync(paths, utils = {}, featureIndex = buildFeatureIndex(paths), opts = {}) {
    const chunkSize = Number.isFinite(opts.chunkSize) && opts.chunkSize > 0 ? opts.chunkSize : 25;
    const parse = parseFrontMatter;
    const graph = new Map();
    let processed = 0;
    for (const folder of paths.folders) {
...
function stageFromFolder(folder) {
    if (folder === STAGE_FOLDERS.DONE) return 'done';
    if (folder === STAGE_FOLDERS.IN_EVALUATION) return 'in-evaluation';
    if (folder === STAGE_FOLDERS.IN_PROGRESS) return 'in-progress';
    if (folder === STAGE_FOLDERS.BACKLOG) return 'backlog';
    if (folder === STAGE_FOLDERS.INBOX) return 'inbox';
    if (folder === STAGE_FOLDERS.PAUSED) return 'paused';
    return 'unknown';
}
...
function resolveStageEngineFirst(featureId, entry, repoPath) {
    // F397 fix #8: prefer engine snapshot lifecycle for graph node colour.
    // Folder is consulted only when no snapshot is on disk (pre-engine).
    if (repoPath) {
        const snapshot = readSnapshotSync(repoPath, 'feature', featureId);
        if (snapshot) {
            ...
            if (lifecycle === 'evaluating' || lifecycle === 'closing' || lifecycle === 'ready_for_review') return 'in-evaluation';
            if (lifecycle === 'inbox') return 'inbox';
            if (lifecycle === 'backlog') return 'backlog';
            if (lifecycle) return 'in-progress';
        }
    }
    return entry ? stageFromFolder(entry.folder) : 'unknown';
}

function getNodeMeta(featureId, featureIndex, repoPath) {
    const entry = featureIndex.byId[featureId];
    if (!entry) {
        return {
            id: featureId,
            name: `feature ${featureId}`,
            stage: resolveStageEngineFirst(featureId, null, repoPath),
        };
    }
    return {
        id: entry.paddedId,
        name: entry.slug.replace(/-/g, ' '),
        stage: resolveStageEngineFirst(entry.paddedId, entry, repoPath),
    };
}

function buildFeatureDependencySvg(featureId, featureIndex, graph, options = {}) {
    ...
    for (const id of [...included].sort(idSort)) {
        ...
        if (!pos) continue;
        const meta = getNodeMeta(id, featureIndex, repoPath);
...
function completedSetMemberIds(paths, specIndex) {
    const { scanFeatureSets } = require('./feature-sets');
    const skip = new Set();
    const sets = scanFeatureSets(paths, specIndex);
    for (const members of sets.values()) {
        if (members.length > 0 && members.every((member) => member.stage === 'done')) {
            members.forEach((member) => {
                if (member.paddedId) skip.add(member.paddedId);
            });
        }
    }
    return skip;
...
function shouldRefreshDependencyGraph(entry, repoPath, completedSetMembers) {
    if (!entry || !entry.paddedId || !entry.fullPath) return false;
    if (entry.folder === STAGE_FOLDERS.DONE) return false;
    if (completedSetMembers.has(entry.paddedId)) return false;
    return !isEntityDone(repoPath, 'feature', entry.paddedId, entry.folder || null);
}

function refreshFeatureDependencyGraphs(paths, utils, options = {}) {
    const specIndex = options.specIndex || null;
    const featureIndex = buildFeatureIndex(paths, specIndex);
    const graph = buildDependencyGraph(paths, utils, featureIndex, specIndex);
    const repoPath = paths && paths.repoPath
        ? paths.repoPath
        : (paths && paths.root ? path.resolve(paths.root, '..', '..', '..') : process.cwd());
    const completedSetMembers = options.includeDone
        ? new Set()
        : completedSetMemberIds(paths, specIndex);
    ...
    [...uniqueEntries.values()]
        ...
        .filter(entry => shouldRefreshDependencyGraph(entry, repoPath, completedSetMembers))
        .forEach(entry => {
            const svg = buildFeatureDependencySvg(entry.paddedId, featureIndex, graph, { repoPath });
            if (upsertDependencyGraphSection(entry.fullPath, svg)) {
                changedSpecs += 1;
                updatedIds.push(entry.paddedId);
                updatedPaths.push(entry.fullPath);
            }
        });
...
/**
 * Check if a feature spec has unmet dependencies.
 *
 * Engine-first per F397: a dependency is "met" when the engine snapshot says
 * `lifecycle === 'done'`. Folder position is consulted only as an explicit
 * fallback for pre-engine legacy features (no engine dir). This unblocks
 * dependents whose dependency is engine-done but whose spec drifted out of
 * `05-done/`, and refuses to mark a dependency met when its spec was hand-moved
 * to `05-done/` while the engine still says in-progress.
 *
...
function checkUnmetDependencies(specPath, featurePaths) {
    ...
    try {
        ...
        const repoPath = featurePaths && featurePaths.repoPath
            ? featurePaths.repoPath
            : (featurePaths && featurePaths.root ? path.resolve(featurePaths.root, '..', '..', '..') : process.cwd());
        // F517: project each dependency's done/stage state from the canonical
        // entity view instead of re-deriving snapshot + isEntityDone here. The
        // view composes the same F397 engine-first `isEntityDone` rule.
        // `computeBlocked: false` breaks the buildEntityView → checkUnmetDependencies
        // recursion — we only need each dependency's own closed/stage, not its deps.
        ...
        for (const dep of data.depends_on) {
            ...
            const folderFallback = entry ? entry.folder : null;
            const view = buildEntityView(repoPath, 'feature', resolvedId, {
                folderFallback,
                computeBlocked: false,
                includeSessions: false,
            });
            if (view.closed) continue;
            unmet.push({ id: resolvedId, slug: entry ? entry.slug : String(dep), stage: view.stage || 'unknown' });
        }
        return unmet;
    } catch (e) {
        return [];
