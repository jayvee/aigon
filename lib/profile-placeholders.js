...
That's it. The iterate gate is scoped to your changes, runs in <30s, and auto-includes Playwright smoke coverage if you touched dashboard files. **Trust it.** The pre-push gate (\`npm run test:deploy\`) is the safety net that catches everything else — it runs at \`feature-close\` / before push.
