...
**Before writing any tests**, check if the test budget is already at or over the ceiling:

\`\`\`bash
bash scripts/check-test-budget.sh 2>/dev/null || true
\`\`\`

If the script exits non-zero (budget exceeded), **skip this step entirely — do not write new tests.** Proceed to Step 4. Writing tests you cannot commit wastes time and blocks the push gate.
...
- **Follow existing test patterns** — look at nearby \`*.test.js\` files for conventions
- **Verify with the iterate gate** (\`npm run test:iterate\`) — scoped, fast. Do NOT run the full suite here; the pre-push gate will catch everything.

> **Where the rules live**: \`docs/testing.md\` (digest), \`AGENTS.md\` § Testing Discipline (T2/T3 rules), \`CLAUDE.md\` hot rule #6 (gate commands).`;

    const MINIMAL_WRITE_SECTION = `## Step 3.8: Verify existing tests

If a test suite exists, run \`npm run test:iterate\` to verify you haven't broken anything. Do not write new tests.`;

    const FULL_RUN_SECTION = `## Step 4.8: Run the iterate gate before submitting

\`\`\`bash
npm run test:iterate
\`\`\`

That's it. The iterate gate is scoped to your changes, runs in <30s, and auto-includes Playwright if you touched dashboard files. **Trust it.** The pre-push gate (\`npm test && MOCK_DELAY=fast npm run test:ui && bash scripts/check-test-budget.sh\`) is the safety net that catches everything else — it runs at \`feature-close\` / before push.

**Do NOT** manually run \`npm test\`, \`npm run test:ui\`, or the full pre-push gate here. Re-running a green suite "to be sure" is the single biggest source of dev-cycle slowdown. Per \`docs/testing.md\`: ship within 60s of green tests.

If the iterate gate fails: fix the failure, then re-run. Do not commit code that breaks tests.`;
...
Fill in the 7-section skeleton — **target 200–400 words**. Evaluators compare agents side-by-side using \`## Key Decisions\` and \`## New API Surface\`; make those sections dense with signal.
...
One line per acceptance criterion (by index): \`1. met — integration test foo.test.js\` / \`2. deferred — reason\` / \`3. dropped — spec revised, see <sha>\`. Append as criteria land — do not rewrite prior lines.
...
Fill in the 7-section skeleton — **target 400–700 words (~600–1,000 tokens)**. The next agent reads this before coding; dense signal here prevents re-inventing or contradicting your decisions.
