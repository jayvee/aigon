...
If a test suite exists, run \`npm test\` to verify you haven't broken anything. Do not write new tests.`;
    ...
    const FULL_RUN_SECTION = `## Step 4.8: Run targeted tests before submitting

Run the tests that cover the files you changed — not the full suite unless you're unsure.

**How to pick which tests to run:**
1. List the files you changed: \`git diff --name-only main..HEAD\`
2. For each changed \`lib/foo.js\`, check if \`tests/integration/foo.test.js\` exists and run it directly: \`node tests/integration/foo.test.js\`
3. Use your judgment: if the change touches core lifecycle, shared state, or a module many others depend on, run \`npm test\` (full suite) instead
4. When in doubt, run \`npm test\` — a missed regression costs more than a full suite run

Fix any failures before proceeding — do not commit code that breaks existing tests.`;
