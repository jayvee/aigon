...
Write and commit the log **before** calling \`aigon agent-status submitted\`. Run \`aigon feature-do {{ARG1_SYNTAX}}\` once from the checkout where you implement — the CLI prints your mode.
...
**Commit the log file in one commit, then call \`aigon agent-status submitted\`.**`;
...
If you already have \`./docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\`, add **one line** (what you shipped / where to look). If no file exists, skip creating one. If you add or change the file, commit it before calling \`aigon agent-status submitted\`.`;
