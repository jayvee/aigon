...
const MINIMAL_LOGGING = `## Step 4.5: Update and commit the log (required one-liner)

Create or update \`./docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\` with **at least one line** (what you shipped / where to look). Commit the log file before calling \`aigon agent-status implementation-complete\`.`;
