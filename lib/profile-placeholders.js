...
function buildTestingSteps(profile, directives, playwrightEnabled, playwrightContent) {
    if (directives.testing === 'skip' && directives.logging === 'skip') {
        return directives.devServer ? readSection('dev-server-light.md') : '';
    }
    return [readSection('testing-steps.md'), profile.testInstructions || '',
...
function getProfilePlaceholders() {
    ...
    return {
        ...
        STOP_DEV_SERVER_STEP: profile.devServer.enabled ? readSection('stop-dev-server.md') : '',
        ...
        ...resolveDevServerPlaceholders(directives.devServer),
