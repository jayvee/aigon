...
// Lazy-loaded references to config-core to avoid circular deps with config.js
let _configCore;
function config() {
    if (!_configCore) _configCore = require('./config-core');
    return _configCore;
}
...
function buildAgentPortMap(basePort) {
    ...
    const ports = {};
    for (const file of fs.readdirSync(agentsDir).filter(name => name.endsWith('.json'))) {
        try {
            const agent = JSON.parse(fs.readFileSync(path.join(agentsDir, file), 'utf8'));
            const id = agent.id || path.basename(file, '.json');
            if (agent.portOffset != null) ports[id] = Number(basePort) + Number(agent.portOffset);
        } catch (_) { /* skip malformed agent template */ }
    }
    return ports;
}
 ...
 * @param {string} [repoPath] - Path to the repository root (defaults to process.cwd())
 * @returns {Object} Resolved profile with devServer, testInstructions, setupEnvLine, and metadata
 */
function getActiveProfile(repoPath) {
    ...
    // If we're in a worktree, repo-scope settings (devServer.enabled, profile) must
    // come from the canonical repo config, not the snapshot copy in the worktree.
    const worktreeJsonPath = path.join(cwd, '.aigon', 'worktree.json');
    if (fs.existsSync(worktreeJsonPath)) {
        try {
            const wt = JSON.parse(fs.readFileSync(worktreeJsonPath, 'utf8'));
            if (wt.mainRepo) {
                const mainConfigPath = path.join(wt.mainRepo, '.aigon', 'config.json');
                if (fs.existsSync(mainConfigPath)) {
                    const mainConfig = JSON.parse(fs.readFileSync(mainConfigPath, 'utf8'));
                    if (mainConfig.profile) projectConfig.profile = mainConfig.profile;
                    if (mainConfig.devServer && typeof mainConfig.devServer.enabled === 'boolean') {
                        projectConfig.devServer = { ...projectConfig.devServer, enabled: mainConfig.devServer.enabled };
                    }
                }
            }
        } catch (_) { /* ignore */ }
...
**Before writing any tests**, check whether the project defines a test budget or ceiling and respect it.

If the budget is exceeded (or the project forbids new tests), **skip this step entirely — do not write new tests.** Proceed to Step 4.

If the budget is healthy, write tests normally:

- **Write unit/integration tests** for new modules, functions, and non-trivial logic
- **Add test cases** to existing test files when extending existing modules
- **Every new test** includes a one-line regression comment naming the specific bug or behaviour it pins. If you can't write that comment, the test isn't worth keeping.
- **Follow existing test patterns** — look at nearby test files for conventions
- **Verify with the project's quick / scoped validation** — do NOT run the full suite here; the pre-push gate will catch everything.

> **Where the rules live**: the project's test docs and root instructions file (e.g. \`AGENTS.md\`, \`README.md\`).`;

    const MINIMAL_WRITE_SECTION = `## Step 3.8: Verify existing tests

If a test suite exists, run the project's quick validation command to verify you haven't broken anything. Do not write new tests.`;

    const FULL_RUN_SECTION = `## Step 4.8: Run quick validation before submitting

Run whatever quick / scoped validation command the project documents (e.g. in \`AGENTS.md\` or \`README.md\`).

That's it. The scoped gate is meant to be fast. **Trust it.** The pre-push or deploy validation is the safety net that catches everything else — it runs at \`feature-close\` / before push.

**Do NOT** manually run the full validation suite here. Re-running a green scoped gate "to be sure" is the single biggest source of dev-cycle slowdown.

If a validation command fails: fix the failure, then re-run the exact failed test file or smallest failing command first. Do not restart a full suite after a single file fails unless the operator explicitly asks for a full gate.`;
...
const LOGGING_NO_FILE_LINE = `**No implementation log is required in this mode.** The spec and git commits are the authoritative record — do **not** create or edit \`docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\`. Proceed directly to **Step 5** (\`aigon agent-status implementation-complete\`).`;

/** Slash-command templates are installed once — cwd is not a feature worktree; spell out all modes. */
const COMMAND_INSTALL_LOGGING_GUIDE = `## Step 4.5: Implementation log (depends on your mode)
...
const FLEET_LOGGING = `## Step 4.5: Update and commit the log (Fleet evaluator read)

Find your implementation log at \`./docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\`.

Fill in the log skeleton — **target 200–400 words**. Evaluators compare agents side-by-side using \`## Key Decisions\` and \`## New API Surface\`; make those sections dense with signal.

Sections (write each in the order shown — primacy/recency bias means most critical sections go first and last):

\`\`\`
## Status
One sentence: what is shipped, what is blocked, what remains ("all N acceptance criteria met").

## New API Surface
Bullet per new public export or module — include only what callers must know.

## Key Decisions
3–5 bullets: what was chosen, why, and what was rejected and why not. Do not restate the spec.

## Gotchas / Known Issues
Symptom → cause → resolution or current state. Omit if none.

## Explicitly Deferred
What was intentionally NOT done. Close the door so the next agent doesn't reopen it.

## For the Next Feature in This Set
What the next set member must call, respect, or avoid. Pointer-level: file + function name.
Omit if this is a standalone feature (no set: tag).

## Test Coverage
What tests exist and pass. What is still missing or skipped.
\`\`\`

**Do NOT include:** conversation transcript, spec restatement, planning notes, code dumps.

**Commit the log file in one commit, then call \`aigon agent-status implementation-complete\`.**`;

const FULL_LOGGING = `## Step 4.5: Update and commit the log

Find your implementation log at \`./docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\`.

Fill in the log skeleton — **target 400–700 words (~600–1,000 tokens)**. The next agent reads this before coding; dense signal here prevents re-inventing or contradicting your decisions.

Sections (write each in the order shown — primacy/recency bias means most critical sections go first and last):

\`\`\`
## Status
One sentence: what is shipped, what is blocked, what remains. Include numbers ("all 4 acceptance criteria met").
...
**Do NOT include:** conversation transcript, spec restatement, planning notes, code dumps or file lists.

**Commit the log file in one commit, then call \`aigon agent-status implementation-complete\`.**`;

const MINIMAL_LOGGING = `## Step 4.5: Update the log (optional one-line)

If you already have \`./docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md\`, add **one line** (what you shipped / where to look). If no file exists, skip creating one. If you add or change the file, commit it before calling \`aigon agent-status implementation-complete\`.`;

/**
 * Whether to create the starter implementation log file on disk (worktree bootstrap / init_log).
 * @param {{ mode: ImplementationLogMode, loggingLevel?: string|null, instructions?: Object }} opts
 * @returns {boolean}
 */
function shouldWriteImplementationLogStarter(opts) {
    if (opts.instructions) {
        const directives = resolveInstructionDirectives(opts.instructions);
        if (directives.logging === 'skip') return false;
    }
...
function resolveLoggingPlaceholders(directivesLogging, ctx = {}) {
    if (directivesLogging === 'skip') {
        const noLog = 'Do not create `docs/specs/features/logs/feature-{{ARG1_SYNTAX}}-*-log.md`.';
        const skipText = ctx.devServerEnabled
            ? `**No implementation log (instructions rigor: light).** ${noLog}`
            : `**No implementation log (instructions rigor: light).** Proceed directly to **Step 5** (\`aigon agent-status implementation-complete\`). ${noLog}`;
        return { LOGGING_SECTION: skipText };
    }
...
function resolveDevServerPlaceholders(devServerEnabled) {
    ...
    return {
        DEV_SERVER_SECTION: `## Step 6.5: Start the dev server (mandatory)
...
// --- Applied digest (content-based drift detection) ---

const APPLIED_DIGEST_FILE = path.join('.aigon', 'applied-digest');

const DIGEST_CATEGORY_LABELS = {
    docs:     { one: 'vendored doc',      many: 'vendored docs' },
    specs:    { one: 'spec template',     many: 'spec templates' },
    commands: { one: 'slash command',     many: 'slash commands' },
    agents:   { one: 'agent config',      many: 'agent configs' },
    sections: { one: 'command section',   many: 'command sections' },
    profiles: { one: 'profile template',  many: 'profile templates' },
    config:   { one: 'project config',    many: 'project config' },
};

/**
 * Recursively collect files from a directory into (relKey, content) pairs.
 * @param {string} dirPath - Absolute directory path
 * @param {string} relPrefix - Prefix for the relative key (using forward slashes)
 * @param {Array} results - Output array of [relKey, content] tuples
 */
function collectTemplateFiles(dirPath, relPrefix, results) {
    if (!fs.existsSync(dirPath)) return;
    const entries = fs.readdirSync(dirPath, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = path.join(dirPath, entry.name);
        const relKey = `${relPrefix}/${entry.name}`;
        if (entry.isDirectory()) {
            collectTemplateFiles(fullPath, relKey, results);
        } else if (entry.isFile()) {
            try {
                results.push([relKey, fs.readFileSync(fullPath, 'utf8')]);
...
/**
 * Compute a content-based applied digest covering all CLI-emitted artifacts.
 * Returns a detailed object with overall digest, per-category digests, and per-file digests.
 * @param {string} [repoPath] - Repo root for reading project config (defaults to cwd)
 * @returns {{ digest: string, cats: Object<string,string>, files: Object<string,string> }}
 */
function computeAppliedDigestDetailed(repoPath) {
    const cwd = repoPath ? path.resolve(repoPath) : process.cwd();
    const cfg = config().loadProjectConfig(cwd);

    // Project config fields that affect what apply writes
    ...
    // Category → array of [relKey, content] pairs
    const categoryEntries = {
        ...
        agents: [],
        profiles: [],
        config: [['config/__config__', configData]],
    };

    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'docs'), 'docs', categoryEntries.docs);
    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'specs'), 'specs', categoryEntries.specs);
    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'generic', 'commands'), 'commands', categoryEntries.commands);
    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'sections'), 'sections', categoryEntries.sections);
    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'agents'), 'agents', categoryEntries.agents);
    collectTemplateFiles(path.join(TEMPLATES_ROOT, 'profiles'), 'profiles', categoryEntries.profiles);
    if (fs.existsSync(PROFILES_JSON_PATH)) {
        categoryEntries.profiles.push(['profiles/_profiles.json', fs.readFileSync(PROFILES_JSON_PATH, 'utf8')]);
    }

    // Per-category digests and per-file hash map
    const cats = {};
    const files = {};

    for (const [cat, entries] of Object.entries(categoryEntries)) {
        entries.sort((a, b) => a[0].localeCompare(b[0]));
        const catHash = crypto.createHash('sha256');
        for (const [relKey, content] of entries) {
            catHash.update(relKey).update('\0').update(content).update('\0');
            const fileHash = crypto.createHash('sha256').update(content).digest('hex');
            files[relKey] = fileHash;
        }
        cats[cat] = catHash.digest('hex');
    }

    // Overall digest: hash all entries sorted by key
    ...
    for (const [relKey, content] of allEntries) {
        overall.update(relKey).update('\0').update(content).update('\0');
    }

    return { digest: overall.digest('hex'), cats, files };
}

/**
 * Compute the applied digest hex string (the primary drift check value).
 * @param {string} [repoPath]
 * @returns {string} Hex SHA-256 digest
 */
function computeAppliedDigest(repoPath) {
    return computeAppliedDigestDetailed(repoPath).digest;
}

/**
 * Read the stored applied digest from a repo.
 * Returns null if not found. Returns a parsed object or legacy-string compat object.
 * @param {string} [repoPath]
 * @returns {{ digest: string, cats: Object|null, files: Object|null }|null}
 */
function readAppliedDigest(repoPath) {
    const digestPath = path.join(repoPath || process.cwd(), APPLIED_DIGEST_FILE);
    if (!fs.existsSync(digestPath)) return null;
    try {
        const raw = fs.readFileSync(digestPath, 'utf8').trim();
        if (raw.startsWith('{')) {
            const parsed = JSON.parse(raw);
            return { digest: parsed.digest || '', cats: parsed.cats || null, files: parsed.files || null };
        }
        // Legacy plain hex string (pre-F497)
        return { digest: raw, cats: null, files: null };
    } catch (_) {
        return null;
    }
}
...
/**
 * Write the applied digest to .aigon/applied-digest atomically.
 * Skips write in worktrees (same convention as setInstalledVersion).
 * @param {string} repoPath
 * @param {{ digest: string, cats: Object, files: Object }} detailed
 */
function writeAppliedDigest(repoPath, detailed) {
    const worktreeMarker = path.join(repoPath, '.aigon', 'worktree.json');
    if (fs.existsSync(worktreeMarker)) return;
    const digestPath = path.join(repoPath, APPLIED_DIGEST_FILE);
    const dir = path.dirname(digestPath);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const payload = JSON.stringify({ v: 1, digest: detailed.digest, cats: detailed.cats, files: detailed.files });
    fs.writeFileSync(digestPath, payload);
}

/**
 * Build a human-readable drift summary by comparing stored vs current per-file hashes.
 * @param {{ cats: Object|null, files: Object|null }|null} stored
 * @param {{ cats: Object, files: Object }} current
 * @returns {string} e.g. "3 slash commands changed, 1 agent config changed"
 */
function buildDriftSummary(stored, current) {
    const labelFor = (cat, count) => {
        const entry = DIGEST_CATEGORY_LABELS[cat];
        if (!entry) return cat;
        return count === 1 ? entry.one : entry.many;
    };

    if (!stored || !stored.files || !current.files) {
        // Fallback: report category-level changes only
        if (!stored || !stored.cats || !current.cats) return '';
        const changedCats = Object.keys(current.cats).filter(c => current.cats[c] !== (stored.cats || {})[c]);
        return changedCats.map(c => labelFor(c, 2)).join(', ') + ' changed';
    }

    // Count changed files per category key prefix
    const catCounts = {};
    const allKeys = new Set([...Object.keys(stored.files), ...Object.keys(current.files)]);
    for (const key of allKeys) {
        if (stored.files[key] !== current.files[key]) {
    ...
    return Object.entries(catCounts)
        .map(([cat, count]) => `${count} ${labelFor(cat, count)} changed`)
        .join(', ');
}
...
/**
 * Merge agent placeholders with profile-derived values for agent doc rendering.
 * Blanks AGENT_DEV_SERVER_NOTE when dev-server support is disabled for the repo.
 */
function resolveAgentDocPlaceholders(agentConfig, repoPath) {
    const cwd = repoPath ? path.resolve(repoPath) : process.cwd();
    const profilePlaceholders = getProfilePlaceholders({ repoPath: cwd });
    const profile = getActiveProfile(cwd);
    const projectConfig = config().loadProjectConfig(cwd);
    const directives = resolveInstructionDirectives(projectConfig?.instructions);
    const devServerEnabled = Boolean(profile.devServer.enabled && directives.devServer);
    const merged = { ...(agentConfig.placeholders || {}), ...profilePlaceholders };
    if (!devServerEnabled) {
        merged.AGENT_DEV_SERVER_NOTE = '';
    }
    return merged;
}
