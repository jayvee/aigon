...
const DEFAULT_AUTH_FAILURE_BACKOFF_SECONDS = 6 * 60 * 60;
const OUTPUT_LIMIT = 500;
const VERDICTS = new Set(['available', 'depleted', 'unknown', 'error', 'unauthenticated', 'low']);
...
function quotaConfig(repoPath = process.cwd()) {
    const project = configCore.loadProjectConfig(repoPath);
    const global = configCore.loadGlobalConfig();
    const cfg = { ...(global.quota || {}), ...(project.quota || {}) };
    return {
        ...
        authFailureBackoffSeconds: Number(cfg.authFailureBackoffSeconds) > 0
            ? Number(cfg.authFailureBackoffSeconds)
            : DEFAULT_AUTH_FAILURE_BACKOFF_SECONDS,
    };
}

function statePath(repoPath = process.cwd()) {
    return agentQuotaRead.getLegacyQuotaPath(repoPath);
}

function emptyState() {
    return { schemaVersion: SCHEMA_VERSION, agents: {}, providers: {} };
}

function normalizeQuotaState(parsed) {
...
function readQuotaState(repoPath = process.cwd()) {
    return agentQuotaRead.projectQuotaApi(agentQuotaRead.readAgentQuotaState(repoPath), repoPath);
}

function migrateQuotaStateOnDisk(repoPath = process.cwd()) {
    return agentQuotaRead.mergeLegacyFiles(repoPath);
}

function writeQuotaState(state, repoPath = process.cwd()) {
    const unified = agentQuotaRead.readAgentQuotaState(repoPath);
    unified.agents = state.agents || {};
    unified.providers = state.providers || {};
    agentQuotaRead.writeAgentQuotaState(unified, repoPath);
}
...
function isPairDepleted(repoPath, agentId, modelValue) {
    // Read raw cache — not projectQuotaApi/readQuotaState, which filters via
    // isAgentQuotaPanelVisible → getAgentAvailability and would recurse (F638).
...
function shouldProbeProvider(entry, pollIntervalSeconds = DEFAULT_POLL_INTERVAL_SECONDS) {
    if (!entry || !entry.lastPolledAt) return true;
    const last = new Date(entry.lastPolledAt).getTime();
    if (!Number.isFinite(last)) return true;
    const interval = Number(pollIntervalSeconds) > 0 ? Number(pollIntervalSeconds) : DEFAULT_POLL_INTERVAL_SECONDS;
    return Date.now() - last >= interval * 1000;
}

function getProviderEntry(repoPath, providerId) {
    const state = readQuotaState(repoPath);
    return state.providers && state.providers[providerId] ? state.providers[providerId] : null;
}

function isProviderDepleted(repoPath, providerId) {
    const entry = getProviderEntry(repoPath, providerId);
    return entry && entry.verdict === 'depleted' ? entry : null;
}

function mergeProviderEntry(repoPath, providerId, entry, prev) {
    const changed = JSON.stringify(prev || null) !== JSON.stringify(entry);
    if (changed) agentQuotaRead.updateProviderEntry(repoPath, providerId, entry);
    return { entry, changed, skipped: false };
}
...
function shouldProbe(entry, cfg = quotaConfig()) {
    ...
    if (!Number.isFinite(last)) return true;
    const now = Date.now();
    if (entry.verdict === 'unauthenticated') {
        const backoff = cfg.authFailureBackoffSeconds || DEFAULT_AUTH_FAILURE_BACKOFF_SECONDS;
        return now - last >= backoff * 1000;
    }
...
function probePair({ repoPath = process.cwd(), agentId, modelValue = null, modelLabel = null, force = true } = {}) {
    ...
    const unified = agentQuotaRead.readAgentQuotaState(repoPath);
    if (!unified.agents[agentId]) unified.agents[agentId] = { models: {} };
    const modelKey = modelValue || '__default__';
    const prev = unified.agents[agentId].models && unified.agents[agentId].models[modelKey];
    ...
    const changed = JSON.stringify(prev || null) !== JSON.stringify(entry);
    agentQuotaRead.updateProbeEntry(repoPath, agentId, modelKey, entry, agentEnabled);
    return { entry, changed, result };
}

async function probePairAsync({ repoPath = process.cwd(), agentId, modelValue = null, modelLabel = null, force = true } = {}) {
    const agentConfig = agentRegistry.getAgent(agentId);
    if (!agentConfig) throw new Error(`Unknown agent: ${agentId}`);
    const unified = agentQuotaRead.readAgentQuotaState(repoPath);
    if (!unified.agents[agentId]) unified.agents[agentId] = { models: {} };
    ...
    const result = await probeAgent.runProbeAsync(agentConfig, modelValue, modelLabel || modelValue);
    const entry = toQuotaEntry(agentConfig, modelValue, modelLabel, result);
    if (entry.verdict === 'depleted' && !entry.resetAt) {
        const cfg = quotaConfig(repoPath);
        entry.backoffSeconds = Math.min(Math.max(cfg.pollIntervalSeconds, Number(prev?.backoffSeconds || cfg.pollIntervalSeconds) * 2), cfg.maxBackoffSeconds);
    ...
    const latest = agentQuotaRead.readAgentQuotaState(repoPath);
    const previousWritten = latest.agents[agentId] && latest.agents[agentId].models
        ? latest.agents[agentId].models[modelKey]
        : null;
    const changed = JSON.stringify(previousWritten || null) !== JSON.stringify(entry);
    agentQuotaRead.updateProbeEntry(repoPath, agentId, modelKey, entry, agentEnabled);
    return { entry, changed, result };
}
...
function mergeMidRunDepletion(repoPath, agentId, modelValue, modelLabel, paneText, classified) {
    ...
    if (changed) agentQuotaRead.updateProbeEntry(repoPath, agentId, modelKey, entry, agentEnabled);
