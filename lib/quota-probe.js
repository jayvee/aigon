...
function readQuotaState(repoPath = process.cwd()) {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object') return emptyState();
        if (parsed.schemaVersion === 1 && parsed.agents) return normalizeQuotaState(parsed);
        if (parsed.schemaVersion === SCHEMA_VERSION && parsed.agents) return normalizeQuotaState(parsed);
        return emptyState();
    } catch (_) {
        return emptyState();
    }
...
function migrateQuotaStateOnDisk(repoPath = process.cwd()) {
    ...
    try {
        const parsed = JSON.parse(fs.readFileSync(file, 'utf8'));
        if (!parsed || parsed.schemaVersion === SCHEMA_VERSION) return false;
        writeQuotaState(normalizeQuotaState(parsed), repoPath);
        return true;
    } catch (_) {
        return false;
    }
...
async function probePairAsync({ repoPath = process.cwd(), agentId, modelValue = null, modelLabel = null, force = true } = {}) {
    const agentConfig = agentRegistry.getAgent(agentId);
    if (!agentConfig) throw new Error(`Unknown agent: ${agentId}`);
    let state = readQuotaState(repoPath);
    if (!state.agents[agentId]) state.agents[agentId] = { models: {} };
    const modelKey = modelValue || '__default__';
    const prev = state.agents[agentId].models[modelKey];
    if (!force && !shouldProbe(prev, quotaConfig(repoPath))) return { entry: prev, changed: false, skipped: true };
    ...
    state = readQuotaState(repoPath);
    if (!state.agents[agentId]) state.agents[agentId] = { models: {} };
    const previousWritten = state.agents[agentId].models[modelKey];
    state.agents[agentId].models[modelKey] = entry;
    state.agents[agentId].lastRefreshedAt = entry.lastProbedAt;
    state.agents[agentId].agentEnabled = agentEnabled(state.agents[agentId]);
