...
function buildCardPresentation(entity, options) {
    ...
    agents.forEach((a) => {
        ...
        } else if (a.status === 'ready' || a.status === 'review-complete' || a.status === 'spec-review-complete') {
            const label = a.status === 'ready' ? 'Review approved' : 'Reviewed';
            pushTimelineUnique(timeline, {
                status: 'complete',
                label,
                detail: 'by ' + agentUpper(a.id),
    ...
    const suppress = {
        ...
        readyToClose: Boolean(lcf) || isFailureState || headlineVerb === 'Recovering close',
