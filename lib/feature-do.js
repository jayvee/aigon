...
function buildPlanningContextSection(planningContextPaths) {
    ...
    const lines = [];
    for (const p of planningContextPaths) {
        const resolved = p.startsWith('~') ? path.join(os.homedir(), p.slice(1)) : p;
        if (fs.existsSync(resolved)) {
            lines.push(`- \`${p}\` — read before coding; focus on: design decisions, file paths, before/after diffs, and constraints`);
        } else {
            lines.push(`- \`${p}\` — file not found on disk; see \`## Planning Context\` in the implementation log for a durable copy`);
        }
    }
...
function printPlanningContextInstructions(planningContextPaths) {
    ...
    const present = planningContextPaths.filter(p => {
        const resolved = p.startsWith('~') ? path.join(os.homedir(), p.slice(1)) : p;
        return fs.existsSync(resolved);
    });
    const missing = planningContextPaths.filter(p => {
        const resolved = p.startsWith('~') ? path.join(os.homedir(), p.slice(1)) : p;
        return !fs.existsSync(resolved);
    });
    ...
    if (missing.length > 0) {
        console.log(`   (${missing.length} file(s) not found on disk — see ## Planning Context in the implementation log for a durable copy)`);
    }
