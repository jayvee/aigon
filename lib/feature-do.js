...
const RESEARCH_STAGE_FOLDERS = [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.DONE, STAGE_FOLDERS.PAUSED];
...
/**
 * Build the {{PLANNING_CONTEXT_SECTION}} placeholder value for non-slash agents.
 * If the plan file exists, points the agent at it. If deleted after feature-start,
 * falls back to the durable ## Planning Context section in the implementation log.
 */
function buildPlanningContextSection(planningContextPaths) {
    if (!planningContextPaths || planningContextPaths.length === 0) return '';

    const lines = planningContextPaths.map(p => {
        const resolved = p.startsWith('~') ? path.join(os.homedir(), p.slice(1)) : p;
        return fs.existsSync(resolved)
            ? `- \`${p}\` — read before coding; focus on: design decisions, file paths, before/after diffs, and constraints`
            : `- \`${p}\` — file not found on disk; see \`## Planning Context\` in the implementation log for a durable copy`;
    });

    return `## Step 2.7: Planning context

Before coding, read the planning session notes:

${lines.join('\n')}

Focus on: design decisions, file paths agreed upon, before/after diffs, constraints, and anything ruled out. Do not restate what you read — use it to sharpen your approach.`;
}

/**
 * Print planning context directly to stdout in instruction mode (cc reads this inline).
 */
function printPlanningContextInstructions(planningContextPaths) {
    if (!planningContextPaths || planningContextPaths.length === 0) return;

    const checked = planningContextPaths.map(p => {
        const resolved = p.startsWith('~') ? path.join(os.homedir(), p.slice(1)) : p;
        return { p, exists: fs.existsSync(resolved) };
    });
    const missingCount = checked.filter(c => !c.exists).length;

    console.log(`\n📋 Planning context`);
    console.log(`   Read these planning notes before writing any code:`);
    for (const { p, exists } of checked) {
        if (exists) console.log(`   ${p}`);
    }
    if (missingCount > 0) {
        console.log(`   (${missingCount} file(s) not found on disk — see ## Planning Context in the implementation log for a durable copy)`);
    }
    console.log(`   Focus on: design decisions, file paths, constraints, what was ruled out.`);
}
...
function run(args, deps) {
    return runAsync(args, deps).catch((error) => {
        if (error && error.message) console.error(`❌ ${error.message}`);
        process.exitCode = 1;
    });
}

async function runAsync(args, deps) {
    const { ctx } = deps;
    const u = ctx.utils;
    ...
    try {
        await coordinateMutatingCommand(mainRepoPath, {
            entityType: 'feature',
            entityId: num,
            role: 'impl',
    ...
    const localContinuity = entityContext.readEntityContext(cwd, 'feature', num) || {};
    const mainContinuity = entityContext.readEntityContext(mainRepoPath, 'feature', num) || {};
    const continuityState = {
        ...mainContinuity,
        ...localContinuity,
        originSession: localContinuity.originSession || mainContinuity.originSession || null,
        authorHandoff: localContinuity.authorHandoff || mainContinuity.authorHandoff || null,
    };
    const continuityDecision = resolveContinuityPolicy({
        phase: 'implementation',
        selectedAgent: resolvedAgent,
        authorAgentId: activeSnapshot.authorAgentId || null,
        originSession: continuityState.originSession || null,
        authorHandoff: continuityState.authorHandoff || null,
    });
    continuityDecision.currentSessionId = `implementation-feature-${String(num).padStart(2, '0')}-${Date.now()}`;
    continuityDecision.role = 'do';
    entityContext.recordContinuityDecision(cwd, 'feature', num, continuityDecision);
    ...
    // Detect set slug, research IDs, and planning_context from spec frontmatter.
    let setSlug = null;
    let researchIds = null;
    let planningContextPaths = null;
    try {
        ...
        planningContextPaths = (parsed && parsed.data && parsed.data.planning_context) || null;
    } catch (_) {}
    ...
    if (!sessionInfo.detected) {
        ...
        // Build PLANNING_CONTEXT_SECTION — injects plan file path(s) from planning_context: frontmatter.
        const planningContextSection = buildPlanningContextSection(planningContextPaths);

        let prompt = resolveAgentPromptBody({
            ...
            extraPlaceholders: {
                ...
                PLANNING_CONTEXT_SECTION: planningContextSection,
            },
        });
        if (continuityState.authorHandoff && continuityState.authorHandoff.status === 'valid') {
            prompt += `\n\n## Author handoff (durable context; current files remain authoritative)\n\n${JSON.stringify(continuityState.authorHandoff, null, 2)}`;
        }
        if (continuityDecision.strategy === 'resume-origin') {
            prompt += `\n\n## Continuation checkpoint\n\nYou are resuming as the implementation agent in ${cwd}. Treat the current checkout, spec, and git state as authoritative. After reconciling context, run exactly one:\n\n`;
            prompt += `aigon agent-status continuation-ready ${paddedNum} ${resolvedAgent} --session=${continuityDecision.currentSessionId}\n`;
            prompt += `aigon agent-status continuation-fallback ${paddedNum} ${resolvedAgent} --session=${continuityDecision.currentSessionId} --reason=<context-missing|context-conflict|task-delivery-failed>`;
        }
        ...
        const modelTokens = buildModelArgTokens(resolvedAgent, model);
        ...
        let spawnArgs = [...flagTokens, ...modelTokens, ...(promptFlag ? [promptFlag] : []), prompt];
        if (continuityDecision.strategy === 'resume-origin') {
            const resumeSpec = resolveResumeArgs(resolvedAgent, continuityState.originSession.providerSessionId);
            if (resumeSpec && resumeSpec.isSubcommand) {
                spawnArgs = [...resumeSpec.prependArgs, prompt];
            } else if (resumeSpec) {
                spawnArgs = [...flagTokens, ...modelTokens, ...resumeSpec.appendArgs, ...(promptFlag ? [promptFlag] : []), prompt];
            }
        }
    ...
    // Print planning context if present.
    if (planningContextPaths) {
        printPlanningContextInstructions(planningContextPaths);
    }
    ...
    if (logVariant === 'skip') {
        ...
        const reason = cfg.logging_level === 'never'
            ? 'opt-out: logging_level is "never"'
            : 'not required in this mode';
        console.log(`📝 Log: not required (${reason}).`);
    } else {
    ...
    console.log(`\n📝 Next Steps:`);
    console.log(`   1. Run 'aigon agent-status implementing' before you start`);
    console.log(`   2. Implement the feature according to the spec above`);
    console.log(`   3. Commit your code with conventional commits (feat:, fix:, chore:)`);
    if (logVariant !== 'skip') {
        console.log(`   4. Update the implementation log`);
        console.log(`   5. Commit the log file`);
    }

    console.log(`\n✅ When done — MANDATORY completion signal:`);
    console.log(`   aigon agent-status implementation-complete`);
    console.log(`\n   Hard rules — implementation is NOT complete until this exits 0:`);
    console.log(`   - Do NOT say "done", "complete", or "ready for review" before exit 0`);
    console.log(`   - The status command IS the completion signal; prose is not a substitute`);
    console.log(`   - Do NOT run 'aigon feature-close' or 'aigon feature-eval' yourself — those are user decisions`);

    if (mode === 'fleet') {
        console.log(`\n⚠️  IMPORTANT:`);
        console.log(`   - Do NOT run 'aigon feature-close' from a worktree`);
        console.log(`   - Return to main repo when done`);
        console.log(`   - The user runs 'aigon feature-eval ${num}' to compare implementations`);
    } else if (mode === 'drive-wt') {
        console.log(`\n⚠️  IMPORTANT:`);
        console.log(`   - Do NOT run 'aigon feature-close' from a worktree`);
        console.log(`   - Return to main repo when done`);
        console.log(`   - The user runs 'aigon feature-close ${num}' from the main repo`);
    } else {
        console.log(`\n   The user will run 'aigon feature-close ${num}' after reviewing.`);
    }
}
...
module.exports = {
    ...
    printPlanningContextInstructions,
};
