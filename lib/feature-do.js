...
const { readSetTag, getSetMembersSorted } = require('./feature-sets');

const LOGS_DIR = 'docs/specs/features/logs';

/**
 * Find the resolved log path for a completed set member, or null if not found.
 * @param {string} paddedId - e.g. '07'
 * @param {string} repoRoot
 * @returns {string|null} relative path like 'docs/specs/features/logs/feature-07-...-log.md'
 */
function resolveSetMemberLogPath(paddedId, repoRoot) {
    const logsDir = path.join(repoRoot, LOGS_DIR);
    try {
        const files = fs.readdirSync(logsDir);
        const prefix = `feature-${paddedId}-`;
        const match = files.find(f => f.startsWith(prefix) && f.endsWith('-log.md'));
        return match ? path.join(LOGS_DIR, match) : null;
    } catch (_) {
        return null;
    }
}
 ...
 */
function buildSetContextSection(setSlug, repoRoot) {
    const members = getSetMembersSorted(setSlug);
    const done = members.filter(m => m.stage === 'done' && m.paddedId);
    if (done.length === 0) return '';

    const logLines = done
        .map(m => {
            const logPath = resolveSetMemberLogPath(m.paddedId, repoRoot);
            return logPath
                ? `- Feature ${m.paddedId} log: \`./${logPath}\``
                : `- Feature ${m.paddedId}: no log found`;
        })
        .join('\n');
 ...
 */
function printSetContextInstructions(setSlug, repoRoot) {
    const members = getSetMembersSorted(setSlug);
    const done = members.filter(m => m.stage === 'done' && m.paddedId);
    if (done.length === 0) return;
    ...
    for (const m of done) {
        const logPath = resolveSetMemberLogPath(m.paddedId, repoRoot);
        if (logPath) {
            console.log(`   Feature ${m.paddedId}: ./${logPath}`);
        } else {
            console.log(`   Feature ${m.paddedId}: no log found`);
        }
    }
