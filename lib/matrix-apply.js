...
/**
 * matrix-apply — apply a reviewed patch entry from a pricing-refresh run
 * to the corresponding agent JSON in templates/agents/.
 *
 * The refresh agent (weekly-agent-matrix-pricing-refresh) never mutates
 * agent files directly; it writes .aigon/matrix-refresh/<date>/proposed.json
 * and creates feedback items. This module performs the actual write once
 * the operator approves a feedback item.
 */
...
/**
 * Find the patch entry for a given feedbackId across all patch files.
 * Returns { entry, patchFile } or null.
 */
function findPatchEntry(repoPath, feedbackId) {
    ...
    for (const patchFile of findPatchFiles(repoPath)) {
        ...
        const entry = doc.changes.find(c => String(c.feedbackId) === normalised);
        if (entry) return { entry, patchFile };
    }
    return null;
}
...
function applyEntryToConfig(config, entry) {
    ...
    if (changeKind === 'new-model') {
        ...
        newOpt.score = { draft: null, spec_review: null, implement: null, review: null };
...
/**
 * Apply a reviewed feedback entry to the agent registry.
 *
 * @param {string} repoPath - Repo root directory.
 * @param {string|number} feedbackId - Feedback item ID to apply.
 * @param {{ dryRun?: boolean }} [opts]
 * @returns {{ agentId: string, modelValue: string, changeKind: string, patchFile: string }}
 */
function applyFeedback(repoPath, feedbackId, opts = {}) {
    const found = findPatchEntry(repoPath, feedbackId);
    if (!found) {
        throw new Error(
            `No patch entry found for feedback #${feedbackId}. ` +
            `Check .aigon/matrix-refresh/*/proposed.json files.`
        );
    }

    const { entry, patchFile } = found;
    const { agentId, modelValue, changeKind } = entry;
    ...
    return { agentId, modelValue, changeKind, patchFile, filePath, dryRun: Boolean(opts.dryRun) };
}
