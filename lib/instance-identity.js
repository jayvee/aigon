...
function resolveRegisteredMainCheckout() {
    const { readConductorReposFromGlobalConfig } = require('./config-core');
...
function deriveInstanceId(codeRoot, profileHome, isPrimary) {
    ...
    const { deriveServerIdFromBranch, sanitizeForDns } = require('./proxy-dns');
...
function resolveInstanceIdentity(options = {}) {
    ...
    const { hashBranchToPort, buildCaddyHostname } = require('./proxy-dns');
...
function formatMixedInvocationHint(subcommand) {
    return [
        ...
        '   For an isolated preview in this worktree, use: aigon preview <id>',
    ].join('\n');
}
