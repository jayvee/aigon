...
function buildDefaultGlobalConfigBase() {
    return {
        ...
        quota: {
            ...
            // When false (default), dashboard ↻ / agent-quota refresh probes only each
            // agent's default model. Set true to probe every non-quarantined model
            // (expensive for OpenCode/OpenRouter — see templates/agents/op.json).
            probeAllModelsOnRefresh: false,
        },
...
function saveGlobalConfig(config) {
    ...
    const content = JSON.stringify(config, null, 2) + '\n';
    const existingMode = fs.existsSync(GLOBAL_CONFIG_PATH) ? (fs.statSync(GLOBAL_CONFIG_PATH).mode & 0o777) : 0o600;
    const privateMode = existingMode & 0o600;
    fs.mkdirSync(path.dirname(GLOBAL_CONFIG_PATH), { recursive: true, mode: 0o700 });
    safeBackupGlobalConfig();
    const tempPath = `${GLOBAL_CONFIG_PATH}.${process.pid}.${Date.now()}.tmp`;
    fs.writeFileSync(tempPath, content, { mode: 0o600 });
    fs.renameSync(tempPath, GLOBAL_CONFIG_PATH);
    fs.chmodSync(GLOBAL_CONFIG_PATH, privateMode);
}
