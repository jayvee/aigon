...
async function run(args, deps) {
    ...
    // F544: export AIGON_SKIP_FIRST_RUN to the orchestrator process AND every
    // aigon subcommand it shells out to, so a mis-resolved HOME can never surface
    // the interactive setup wizard inside this unattended tmux pane. (This is
    // defense-in-depth; the root fix is tmux-server isolation in the e2e harness.
    // It does NOT fix agent CLI auth, which genuinely needs the correct HOME.)
    const loopCmd = 'AIGON_SKIP_FIRST_RUN=1 ' + loopCmdParts.map(part => shellQuote(String(part))).join(' ');
