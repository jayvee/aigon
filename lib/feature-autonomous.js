...
/** After review feedback injection, detect re-submit (counter-review) vs stale pre-review status. */
            ...
            while (true) {
                ...
                // Solo: after allReady, optionally spawn review, then close
                    ...
                    if (feedbackInjected && !feedbackAddressed && !closeTriggered) {
                        ...
                        console.log(`  [feedback ${feedbackPolls}/${MAX_FEEDBACK_POLLS}] implSession running=${implSessionRunning} status=${implStatus && implStatus.status} signaled=${signaled} progressed=${progressed} readyAfterFeedback=${readyAfterFeedback}`);
                        ...
                        if (signaled) {
                            ...
                            // REGRESSION: agents often re-run `agent-status submitted` after counter-review instead of
