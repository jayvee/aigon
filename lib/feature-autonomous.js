 ...
 * close_recovery_in_progress (post-merge gate, preauth, criteria) — AutoConductor must
    ...
    if (subcommand === '__run-loop') {
        ...
        try {
            ...
            while (true) {
                ...
                if (!isFleet && !allReady && !closeTriggered && !reviewTriggered && !feedbackInjected) {
                    ...
                    if (stageIsImplementing && !implSessionRunning) {
                        ...
                        if (implRespawnAttempts < MAX_IMPL_RESPAWN_ATTEMPTS) {
                            implRespawnAttempts++;
                            console.log(`⚠️  Implementation session not running (${implSessionName}). Respawn ${implRespawnAttempts}/${MAX_IMPL_RESPAWN_ATTEMPTS}...`);
                            try {
                                ...
                                if (!wtPath) {
                                    ...
                                    if (tmuxSessionExists(implSessionName)) {
                                        console.log(`✅ Respawned implementation session: ${implSessionName}`);
                                        implDeadPolls = 0;
                                    } else {
                ...
                // Solo: after allReady, optionally spawn review, then close
                    ...
                    if (hasOpenReviewEscalations(snapForClose)) {
                    ...
                    let criteriaAttestationComplete = true;
                    const closeSpecPath = snapForClose && snapForClose.specPath;
                    if (closeSpecPath && soloReadyForFeatureClose && effectiveStopAfter === 'close') {
                        const implWts = filterByFeatureId(findWorktrees(), featureNum);
                        const criteriaCheck = criteriaAttestationReady(closeSpecPath, mainRepo, featureNum, {
                            snapshot: snapForClose,
                            worktreePath: implWts[0] ? implWts[0].path : null,
                        });
                        criteriaAttestationComplete = criteriaCheck.ready;
                        if (!criteriaAttestationComplete && postTriggerPolls > 0 && postTriggerPolls % 4 === 0) {
                            console.warn(`⏳ Waiting for ## Criteria Attestation in the implementation log before feature-close ${featureNum}…`);
                            const missing = criteriaCheck.result && criteriaCheck.result.unattested;
                            if (missing && missing.length) {
                                console.warn(`   Missing criterion index(es): ${missing.join(', ')}`);
                            }
                        }
                    }
