...
async function resume(args, deps) {
    ...
    if (!featureId || !/^\d+$/.test(featureId)) {
        console.error('Usage: aigon feature-autonomous-resume <feature-id>');
        process.exitCode = 1;
        return;
    }
    ...
    const agentIds = Array.isArray(existing.agents) ? existing.agents.map(a => String(a).trim()).filter(Boolean) : [];
    if (agentIds.length === 0) {
    ...
    const reviewAgent = existing.reviewAgent || null;
    const workflowSlug = existing.workflowSlug || null;
    const modelsForChild = stringifyAgentOverrideMap(existing.modelOverrides || {});
    const effortsForChild = stringifyAgentOverrideMap(existing.effortOverrides || {});
    const isFleet = agentIds.length > 1;
