...
function implAgentReadyForAutonomousClose(snap, agent, featureId, mainRepo) {
    ...
    if (st === 'ready' || st === 'submitted' || st === 'feedback-addressed') return true;
    ...
    if (agent && agent !== 'solo' && snap.agents && snap.agents.solo) {
        const s = snap.agents.solo.status;
        if (s === 'ready' || s === 'submitted' || s === 'feedback-addressed') return true;
    }
    const a = readAgentStatus(featureId, agent, 'feature', { mainRepoPath: mainRepo });
    const fileSt = a && a.status != null ? String(a.status) : '';
    if (fileSt === 'submitted' || fileSt === 'ready' || fileSt === 'feedback-addressed') return true;
    if (agent && agent !== 'solo') {
        ...
        if (sfs === 'submitted' || sfs === 'ready' || sfs === 'feedback-addressed') return true;
    }
    return false;
}
            ...
            while (true) {
                ...
                // Solo: after allReady, optionally spawn review, then close
                    ...
                    if (reviewStarted && effectiveStopAfter === 'close' && !closeTriggered && !feedbackInjected) {
                        ...
                        const legacyReviewCompleted = !!((reviewState.history || []).some(entry => entry.agent === reviewAgent));
                        const engineRevisionReady = isCodeRevisionInProgress(reviewSnapshot);
                        const reviewCompleted = engineRevisionReady || legacyReviewCompleted;
                        const reviewCurrentStatus = reviewState.current && reviewState.current.agent === reviewAgent
                            ? reviewState.current.status
                            : 'idle';
                        console.log(`  [review-close ${reviewClosePolls}/${MAX_REVIEW_CLOSE_POLLS}] reviewSession running=${reviewSessionRunning} engineRevisionReady=${engineRevisionReady} legacyReviewCompleted=${legacyReviewCompleted} reviewState=${reviewCurrentStatus}`);
                    ...
                    const legacyReviewCompleted = !!(reviewAgent && reviewState && (reviewState.history || []).some(entry => entry.agent === reviewAgent));
                    const engineRevisionComplete = reviewAgent ? isCodeRevisionComplete(snapForClose, mainRepo, featureNum, agentIds[0]) : false;
                    const reviewCompleted = !reviewAgent || engineRevisionComplete || legacyReviewCompleted;
