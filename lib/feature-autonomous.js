            ...
            while (true) {
                ...
                // Solo: after allReady, optionally spawn review, then close
                    ...
                    if (feedbackInjected && !feedbackAddressed && !closeTriggered) {
                        ...
                        const engineRevisionCompleteRaw = isCodeRevisionComplete(snapFb, mainRepo, featureNum, implAgent);
                        // F530: when the reviewer approved-with-output, `isCodeRevisionComplete` would short-
                        // circuit on `requestRevision === false` and close before the implementor signals.
                        // Require explicit revision completion evidence (revisionCompletedAt or events) in
                        // that case rather than trusting the approve flag.
                        const hasExplicitRevisionEvidence = Boolean(
                            snapFb && (
                                snapFb.currentSpecState === 'code_revision_complete'
                                || snapFb.lifecycle === 'code_revision_complete'
                                || (snapFb.codeReview && snapFb.codeReview.revisionCompletedAt)
                                || hasCodeRevisionCompletedEvent(mainRepo, featureNum, implAgent)
                            )
                        );
                        const engineRevisionComplete = dispositionRequiredForClose
                            ? hasExplicitRevisionEvidence
                            : engineRevisionCompleteRaw;
                    ...
                    const engineRevisionCompleteRawForClose = reviewAgent ? isCodeRevisionComplete(snapForClose, mainRepo, featureNum, agentIds[0]) : false;
                    // F530: when reviewer approved but left reviewer output, `isCodeRevisionComplete` would
                    // be true via the `requestRevision === false` clause before the implementor signals.
                    // Wait for explicit revision evidence in that case.
                    const engineRevisionComplete = reviewAgent && dispositionRequiredForClose
                        ? Boolean(
                            snapForClose && (
                                snapForClose.currentSpecState === 'code_revision_complete'
                                || snapForClose.lifecycle === 'code_revision_complete'
                                || (snapForClose.codeReview && snapForClose.codeReview.revisionCompletedAt)
                                || hasCodeRevisionCompletedEvent(mainRepo, featureNum, agentIds[0])
                            )
                        )
                        : engineRevisionCompleteRawForClose;
...
module.exports = { run, implAgentReadyForAutonomousClose, requiresImplementorDisposition };
