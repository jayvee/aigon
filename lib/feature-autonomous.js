            ...
            while (true) {
                ...
                // Solo: after allReady, optionally spawn review, then close
                    ...
                    if (reviewStarted && effectiveStopAfter === 'close' && !closeTriggered && !feedbackInjected) {
                        ...
                        if (reviewCompleted) {
                            if (reviewApprovedNoRevision) {
                                // Reviewer approved with no changes needed — skip feedback injection entirely
                                console.log('✅ Review approved (no revision required). Skipping feedback injection, proceeding to close...');
