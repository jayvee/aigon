...
const FEATURE_TRANSITIONS = [
    createTransition('inbox', 'backlog', 'feature-prioritise', 'Prioritise', { uiTrigger: 'drag-drop, Prioritise button' }),
    createTransition('backlog', 'in-progress', 'feature-start', 'Start', { requiresInput: 'agentPicker', uiTrigger: 'Start button' }),
    createTransition('in-progress', 'in-evaluation', 'feature-eval', 'Evaluate', {
        guard: (ctx) => isFleet(ctx) && allAgentsSubmitted(ctx),
        uiTrigger: 'Evaluate button'
    }),
    createTransition('in-evaluation', 'done', 'feature-close', 'Accept & Close', { uiTrigger: 'Close button' }),
    createTransition('inbox', 'paused', 'feature-pause', 'Pause', { uiTrigger: 'Drag to Paused' }),
    createTransition('backlog', 'paused', 'feature-pause', 'Pause', { uiTrigger: 'Drag to Paused' }),
    createTransition('in-progress', 'paused', 'feature-pause', 'Pause', { uiTrigger: 'Drag to Paused' }),
    createTransition('paused', 'backlog', 'feature-resume', 'Resume', { uiTrigger: 'Drag to Backlog' }),
    createTransition('paused', 'inbox', 'feature-resume', 'Resume to Inbox', { uiTrigger: 'Drag to Inbox' }),
];
...
const FEATURE_ACTIONS = [
    createAction('in-progress', 'feature-open', {
        guard: (ctx, agentId) => { const s = (ctx.agentStatuses || {})[agentId]; return s === 'idle' || s === 'error' || s === undefined; },
        label: (ctx, agentId) => (ctx.agentStatuses || {})[agentId] === 'error' ? `Restart ${agentId}` : `Start ${agentId}`,
        perAgent: true,
        mode: 'terminal'
    }),
    createAction('in-progress', 'feature-attach', {
    ...
    createAction('in-progress', 'feature-focus', {
    ...
    createAction('in-progress', 'feature-stop', {
        guard: (ctx, agentId) => { const s = (ctx.agentStatuses || {})[agentId]; return s === 'implementing' || s === 'waiting'; },
        label: (ctx, agentId) => `Stop ${agentId}`,
        perAgent: true
    }),
    createAction('in-progress', 'feature-close', {
        guard: (ctx) => !isFleet(ctx) && allAgentsSubmitted(ctx),
        label: () => 'Accept & Close',
        priority: 'high'
    }),
    createAction('in-progress', 'feature-review', {
        guard: (ctx) => !isFleet(ctx) && allAgentsSubmitted(ctx),
        label: () => 'Run Review',
        mode: 'agent'
    }),
    createAction('in-progress', 'feature-eval', {
        guard: (ctx) => isFleet(ctx) && allAgentsSubmitted(ctx),
        label: () => 'Run Evaluation',
        mode: 'agent',
        priority: 'high'
    }),
    createAction('in-evaluation', 'feature-eval', {
        guard: (ctx) => isFleet(ctx),
        label: () => 'Continue Evaluation',
        mode: 'agent',
        priority: 'high'
    }),
    createAction('in-evaluation', 'feature-review', {
        guard: (ctx) => !isFleet(ctx),
        label: () => 'Run Review',
        mode: 'agent'
    }),
    createAction('backlog', 'feature-start', {
        label: () => 'Start feature',
        mode: 'terminal',
        requiresInput: 'agentPicker'
    }),
    createAction('backlog', 'feature-autopilot', {
        label: () => 'Run Autopilot',
        mode: 'terminal',
        requiresInput: 'agentPicker'
    }),
    createAction('in-progress', 'feature-autopilot', {
        guard: (ctx) => { const states = ctx.tmuxSessionStates || {}; return !Object.values(states).some(s => s === 'running'); },
        label: () => 'Run Autopilot',
        mode: 'terminal',
        requiresInput: 'agentPicker'
    }),
