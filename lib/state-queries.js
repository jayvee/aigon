...
const AGENT_STATUSES = ['idle', 'implementing', 'waiting', 'ready', 'error'];
...
function allAgentsReady(ctx) {
    const statuses = Object.values(ctx.agentStatuses || {});
    if (statuses.length === 0) return false;
    return statuses.every(s => s === 'ready');
}

/** Agent has finished its implementation/revision pass — eligible for review/close CTAs. */
const AGENT_SUBMITTED_STATUSES = new Set([
    'ready',
    'submitted',
...
function allAgentsSubmitted(ctx) {
    const statuses = Object.values(ctx.agentStatuses || {});
    if (statuses.length === 0) return false;
    return statuses.every((s) => AGENT_SUBMITTED_STATUSES.has(s));
}
...
function resolveFeatureRuleGuard(name) {
    const guards = {
        ...
        agentImplementingOrReadyWithTmux: (ctx, agentId) => {
            const status = (ctx.agentStatuses || {})[agentId];
            return (status === 'implementing' || status === 'ready') && (ctx.tmuxSessionStates || {})[agentId] === 'running';
        },
        // Open Terminal should be available whenever the agent's tmux session
        // is running — regardless of lifecycle state. Users still want to peek
        // at the session after the agent has signalled completion (e.g. to see
        // what a revision actually changed). The previous narrow guard
        // (agentImplementingOrSubmittedWithTmux) hid the affordance once the
        // agent reached 'addressing-code-review' / 'revision-complete' /
        // 'review-complete' / etc., even though the session was still alive.
        agentHasRunningTmux: (ctx, agentId) =>
...
function resolveFeatureLabel(actionDef) {
    ...
    if (actionDef.labelType === 'agent-view') return () => 'Open Terminal';
...
const RESEARCH_TRANSITIONS = [
    ...
    createTransition('backlog', 'inbox', 'research-unprioritise', 'Move back to inbox'),
...
const RESEARCH_ACTIONS = [
    ...
    createAction('in-progress', 'research-attach', {
        // Open Terminal should be available whenever the research agent's
        // tmux session is running — including post-completion states so users
        // can peek at the agent's output after it signals done. Same widening
        // as the feature-attach guard agentHasRunningTmux.
        guard: (ctx, agentId) => (ctx.tmuxSessionStates || {})[agentId] === 'running',
        label: () => 'Open Terminal',
        perAgent: true,
        mode: 'terminal'
    }),
...
const ENTITY_DEFINITIONS = {
    // Feature and research action derivation now lives in the workflow-core engine.
    // Constants (FEATURE_TRANSITIONS, RESEARCH_TRANSITIONS, etc.) are retained for
    // diagram generation and rules reporting only.
...
module.exports = {
    ...
    getSessionAction,
    allAgentsReady,
    allAgentsSubmitted, // legacy alias
    isFleet,
};
