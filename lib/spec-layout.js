...
const ACTIVE_STAGE_DIRS = Object.freeze(['03-in-progress', '04-in-evaluation']);
...
function buildMigrationPlan(repoPath, options = {}) {
  ...
  for (const entityType of ENTITY_TYPES) {
    ...
    for (const spec of legacy) {
      ...
      if (number == null) {
        ...
        // reservation contract (feature 667).
        if (!allocateId) {
          blockers.push(
            `unnumbered spec needs an ID but no allocator available: ${path.relative(repoPath, spec.path)}`,
          );
          continue;
        }
  ...
  return { moves, blockers, warnings, needsAck };
}
...
function applyMigrationPlan(repoPath, plan, options = {}) {
  ...
  if (options.commit) {
    ...
    for (const move of applied) {
      paths.push(path.relative(repoPath, move.from));
      paths.push(path.relative(repoPath, move.to));
    }
    paths.push(path.relative(repoPath, configPath));
    tryGit(repoPath, ['add', '-A', '--', ...paths]);
    const message = options.message
      || `chore(spec-layout): migrate ${applied.length} spec(s) to 00-specs and set layout=stable`;
    // Only commit if something is staged (idempotent re-run stages nothing new).
    const staged = tryGit(repoPath, ['diff', '--cached', '--quiet']);
    // diff --cached --quiet returns null (throws → non-zero) when there ARE staged changes.
    if (staged == null) {
      runGit(repoPath, ['commit', '-m', message, '--', ...paths]);
