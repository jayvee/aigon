...
testAsync('collectDashboardStatusData preserves legacy fallback for non-engine features', async () => {
    const repo = makeTempRepo();
    try {
        fs.writeFileSync(
        ...
        assert.ok(feature, 'expected feature 02 in dashboard status');
        assert.strictEqual(feature.workflowEngine, 'legacy');
        assert.ok(feature.validActions.some(action => action.action === 'feature-focus'));
        assert.strictEqual(feature.agents.find(agent => agent.id === 'cc').status, 'waiting');
    } finally {
        fs.rmSync(repo, { recursive: true, force: true });
    }
});
