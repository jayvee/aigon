...
function mergeJsonlByTimestamp(localFile, importedFile) {
    const readLines = (fp) => (fs.existsSync(fp)
        ? fs.readFileSync(fp, 'utf8').split('\n').filter(Boolean)
        : []);
    const localLines = readLines(localFile);
    const importedLines = readLines(importedFile);
    ...
    [...localLines, ...importedLines].forEach((line) => {
        ...
        seen.add(normalized);
        let t = null;
        try { t = JSON.parse(normalized).t || null; } catch (_) { /* keep null */ }
        parsed.push({ line: normalized, t });
    });
    ...
    ensureDirForFile(localFile);
    const out = parsed.map((p) => p.line);
    fs.writeFileSync(localFile, `${out.join('\n')}${out.length > 0 ? '\n' : ''}`, 'utf8');
}
...
function mergeTelemetry(localRoot, importedRoot, summary) {
    ...
    importedFiles.forEach((rel) => {
        ...
        // jsonl files (signal-health and any other *.jsonl) — merge by timestamp dedup.
        if (rel.endsWith('.jsonl')) {
            mergeJsonlByTimestamp(dst, src);
