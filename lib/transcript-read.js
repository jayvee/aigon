...
const { resolveTelemetryDir, readTelemetryFile } = require('./telemetry');
...
function normalizeEntityId(entityId) {
    return String(entityId == null ? '' : entityId).trim().replace(/^0+(?=\d)/, '');
}
...
function findTelemetryForSession(repoPath, entityType, entityId, agentId, sessionId) {
    ...
    const entityIdStr = normalizeEntityId(entityId);
    ...
    for (const file of files) {
        ...
        if (normalizeEntityId(record.featureId) !== entityIdStr) continue;
...
function collectTranscriptRecords(repoPath, entityType, entityId, agentId) {
    ...
    for (const f of entries) {
        ...
        if (normalizeEntityId(raw.entityId) !== entityIdStr) continue;
...
function findControllerLogRecord(repoPath, featureId) {
    const records = collectTranscriptRecords(repoPath, 'feature', featureId, 'auto');
    const captured = records
        .filter(r => r.captured === true && r.agentSessionPath)
        .filter(r => {
            try { return fs.existsSync(r.agentSessionPath); } catch (_) { return false; }
        })
        .sort((a, b) => {
    ...
    if (captured.length > 0) return { record: captured[0], records };
    return { record: null, records };
}

function getControllerLogStatus(repoPath, featureId) {
    const { record, records } = findControllerLogRecord(repoPath, featureId);
    if (record) {
        return {
            available: true,
    ...
    const reason = records.length > 0 && records[0].reason
        ? records[0].reason
        : 'Controller log is not available for this autonomous run.';
    return {
        available: false,
        reason,
        sessionName: null,
...
function readControllerLog(repoPath, featureId, options = {}) {
    const { record } = findControllerLogRecord(repoPath, featureId);
    if (!record) {
        return { ok: false, status: 200, available: false, reason: getControllerLogStatus(repoPath, featureId).reason };
    }
    const maxBytes = Number.isFinite(options.maxBytes) ? options.maxBytes : 256 * 1024;
    let content = '';
    try {
        const stat = fs.statSync(record.agentSessionPath);
        const start = Math.max(0, stat.size - maxBytes);
        const fd = fs.openSync(record.agentSessionPath, 'r');
        try {
        ...
        return { ok: false, status: 200, available: false, reason: `Controller log could not be read: ${e.message}` };
    }
}
...
function openTranscriptPath(filePath) {
    ...
    if (editor) {
        ...
        cmdArgs = [filePath];
        try {
            spawn(cmd, cmdArgs, { detached: true, stdio: 'ignore', shell: true }).unref();
