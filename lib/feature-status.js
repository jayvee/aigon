...
function collectCost(repoPath, featureId, agentId, worktreePath, entityType = 'feature') {
    const defaults = {
        inputTokens: 0,
        cachedInputTokens: 0,
        freshInputTokens: 0,
    ...
    if (fs.existsSync(telemetryDir)) {
        try {
            const files = fs.readdirSync(telemetryDir)
                .filter(f => f.startsWith(`${prefix}-${featureId}-`) && f.endsWith('.json'));
            for (const file of files) {
                try {
                    ...
                    if (input > 0 || cachedInput > 0 || output > 0 || thinking > 0 || total > 0 || billable > 0 || cost > 0) hasRealData = true;
                    inputTokens += input;
                    ...
                    sessions += 1;
                    // Per-agent aggregation (keyed by agent+activity for breakdown)
                    const agentKey = data.agent || 'unknown';
                    const recordActivity = data.activity || null;
                    const byAgentKey = recordActivity ? `${agentKey}:${recordActivity}` : agentKey;
                    if (!byAgent[byAgentKey]) {
                        byAgent[byAgentKey] = {
                            agent: agentKey,
                    ...
                    byAgent[byAgentKey].inputTokens += input;
                    byAgent[byAgentKey].cachedInputTokens += cachedInput;
                    byAgent[byAgentKey].outputTokens += output;
                    byAgent[byAgentKey].thinkingTokens += thinking;
                    byAgent[byAgentKey].totalTokens += total;
                    byAgent[byAgentKey].billableTokens += billable || (input + output + thinking);
                    byAgent[byAgentKey].costUsd += cost;
                    if (!byAgent[byAgentKey].model && data.model) byAgent[byAgentKey].model = data.model;
        ...
        try {
            ...
            if (transcriptData && transcriptData.cost_usd > 0) {
                ...
                return {
                    ...
                    totalTokens: transcriptData.total_tokens || (transcriptInput + transcriptOutput + transcriptCached + transcriptThinking),
                    billableTokens: transcriptData.billable_tokens || (transcriptInput + transcriptOutput + transcriptThinking),
                    estimatedUsd: Math.round((transcriptData.cost_usd || 0) * 10000) / 10000,
    ...
    if (!hasRealData && sessions === 0) return defaults;

    // Derive combined model label
    const models = [...new Set(Object.values(byAgent).map(a => a.model).filter(Boolean))];
    const modelLabel = models.length === 1 ? models[0] : (models.length > 1 ? models.join(', ') : model);
    Object.values(byAgent).forEach(agentData => {
    ...
    return {
        ...
        model: modelLabel,
        sessions,
        byAgent: Object.keys(byAgent).length > 1 ? byAgent : undefined,
    };
}
...
function collectFeatureDeepStatus(repoPath, featureId, options = {}) {
    ...
    if (stats && stats.cost && stats.cost.totalTokens != null) {
        ...
        cost = collectCost(absRepo, featureId, primaryAgent, worktreePath, entityType);
    }
