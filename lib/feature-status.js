    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                if (!byAgent[agentId]) byAgent[agentId] = { model: data.model, inputTokens: 0, outputTokens: 0, costUsd: 0 };
                byAgent[agentId].inputTokens += input;
                byAgent[agentId].outputTokens += output;
                byAgent[agentId].costUsd += cost;
                if (!byAgent[agentId].model && data.model) byAgent[agentId].model = data.model;
