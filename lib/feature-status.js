...
const { resolveSpecAuthor, resolveAuthorAgentId } = require('./spec-author-provenance');
...
function collectFeatureDeepStatus(repoPath, featureId, options = {}) {
    ...
    const specAuthor = resolveSpecAuthor(snapshot);
    const authorAgentId = resolveAuthorAgentId(snapshot, specAuthor, agents);
    const continuity = (entityType === 'feature' || entityType === 'research')
        ? entityContext.readPublicEntityContext(absRepo, entityType, featureId)
    ...
    return {
        ...
        specAuthor,
        authorAgentId,
        originSession: continuity.originSession || null,
        authorHandoff: continuity.authorHandoff || null,
        latestContinuityDecision: continuity.latestContinuityDecision || null,
        session,
        agentSessions,
        progress,
        cost,
