        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    if (input > 0 || output > 0 || billable > 0 || cost > 0) hasRealData = true;
                    inputTokens += input;
                    outputTokens += output;
                    costUsd += cost;
                    ...
                    if (!byAgent[byAgentKey]) byAgent[byAgentKey] = { agent: agentKey, activity: recordActivity, model: data.model, inputTokens: 0, outputTokens: 0, costUsd: 0 };
    ...
    if (!hasRealData && agentId && (agentRegistry.supportsTranscriptTelemetry(agentId) || agentId === 'solo') && entityType !== 'research') {
        try {
            ...
            if (transcriptData && transcriptData.cost_usd > 0) {
                return {
                    inputTokens: transcriptData.input_tokens || 0,
                    outputTokens: transcriptData.output_tokens || 0,
...
function collectFeatureDeepStatus(repoPath, featureId, options = {}) {
    ...
    let cost;
    if (stats && stats.cost && stats.cost.byAgent) {
        cost = stats.cost;
    } else {
