...
function readState(cwd) {
    ...
    if (CONFLICT_MARKER_RE.test(raw)) {
        throw new Error(`${STATE_FILE} contains unresolved conflict markers; resolve it before running security-scan`);
    }
    try {
        return JSON.parse(raw);
    } catch (err) {
        throw new Error(`${STATE_FILE} is not valid JSON: ${err.message}`);
    }
}
