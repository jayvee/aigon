...
const { getEventsPathForEntity, STAGE_FOLDERS } = require('./workflow-core/paths');
...
function collectAnalyticsData(globalConfig) {
    ...
    repos.forEach(repoPath => {
        ...
        const doneDir = path.join(absRepo, 'docs', 'specs', 'features', STAGE_FOLDERS.DONE);
        ...
        [...completed.values()].forEach(entry => {
            ...
            if (fmEvents.length >= 2) {
                ...
                // F501 backward compat: old log frontmatter may still list the deprecated 'submitted' status.
