...
function sessionPresentationLabel(session, sessionStatus, entityType) {
    const role = String(session.role || '').toLowerCase();
    const completed = sessionStatus === 'completed';
    if (role === 'code-review' || (role === 'review' && entityType === 'feature')) {
        if (!completed) return 'Reviewing code';
        return session.requestRevision ? 'Changes requested' : 'Implementation approved';
    }
    if (role === 'findings-review' || (role === 'review' && entityType === 'research')) {
        return completed ? 'Findings reviewed' : 'Reviewing findings';
    }
...
function normalizeSessions(sessions, options = {}) {
    ...
    return (Array.isArray(sessions) ? sessions : []).map((source) => {
        ...
        const presentationLabel = sessionPresentationLabel(session, sessionStatus, options.entityType);
...
function actionIdentity(action) {
    // A review can raise more than one escalation. They deliberately share the
    // same action kind and feature scope, but each is a separate operator
    // decision; use its durable escalation id to keep those contract actions
    // distinct without changing dispatch semantics.
    const escalationId = action.metadata && action.metadata.escalationId || '';
    return [action.actionId, action.scope || '', action.entityId || '', action.agentId || '', action.sessionId || '', escalationId].join(':');
}
