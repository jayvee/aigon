...
function attachPtyWebSocketServer(wss, { tmuxSessionExists }) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        function spawnPty(cols, rows) {
            ...
            inputQueue.length = 0;
            // Force a resize shortly after attach so full-screen TUIs (Claude Code,
            // kimi, opencode) repaint — they may not redraw when a new client attaches
            // even if dimensions match, depending on timing with the existing client.
            setTimeout(() => {
                try { if (pty) pty.resize(pty.cols || 80, pty.rows || 24); } catch (_) {}
            }, 300);
        }
