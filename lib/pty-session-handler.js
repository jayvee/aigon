...
function attachPtyWebSocketServer(wss, { tmuxSessionExists }) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        // Force a resize shortly after attach so full-screen TUIs (kimi, opencode)
        // repaint themselves — they don't automatically redraw when a new client
        // connects and the terminal dimensions change.
        setTimeout(() => {
            try { pty.resize(pty.cols || 80, pty.rows || 24); } catch (_) {}
        }, 300);
