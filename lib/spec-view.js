...
const { isStableLayout, listCanonicalSpecs } = require('./spec-layout');
...
function reconcileView(repoPath, options = {}) {
  ...
  const managed = Object.entries(desired).map(([linkPath, info]) => ({
    path: toRel(repoPath, linkPath),
    target: info.target,
    entityType: info.entityType,
    id: info.id,
  ...
  return { created, removed, replaced, kept, blocked, managed, dryRun, desiredCount: managed.length };
}
