 ...
 * 2. Registry `deprecated` — outranks user `active` for default/recommended selection;
 *    explicit launch allowed with warning unless `hardBlock` is set.
 * 3. Global user `disabled` — outranks project `active` (v1: project cannot override).
 * 4. Project `disabled` / legacy `agents.<id>.disabled`.
 * 5. `unconfigured` — inferred from missing CLI binary (doctor / getAgentBinMap).
 * 6. `quota_depleted` — runtime annotation only; never overrides user `disabled`.
 */
