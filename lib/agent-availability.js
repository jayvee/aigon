...
const { loadGlobalConfig, loadProjectConfig, saveGlobalConfig, saveProjectConfig } = require('./config');
...
function getAgentAvailability(agentId, repoPath = process.cwd(), options = {}) {
    ...
    // TEMPORARY KILL-SWITCH — see feature `dashboard-agent-availability-hang`.
    // The probes below (readQuotaAnnotation → quota-probe, isAgentCliInstalled)
    // spin/hang and wedge getDashboardAgents, freezing the dashboard `/` page and
    // `aigon server status` (a single request can hard-wedge the server's event
    // loop). Until that root cause is fixed, short-circuit to a permissive
    // "active/usable" default for EVERY agent so the dashboard can never hang here.
    // This intentionally bypasses quota / CLI-install / user-preference gating —
    // the picker may show agents that are not actually installed/authed. Revert
    // this whole block (restore the code below it) once the hang is fixed.
    if (process.env.AIGON_ENABLE_AGENT_AVAILABILITY !== '1') {
        return {
            agentId: id,
            state: 'active',
            usable: true,
            ...
            recommended: true,
            reason: null,
            registryPolicy: { state: 'active', deprecated: false, hardBlock: false, defaultHidden: false },
            userPreference: null,
            quota: null,
            availabilityProbeDisabled: true,
        };
    }
...
function isAgentQuotaPanelVisible(agentId, repoPath = process.cwd()) {
    const avail = getAgentAvailability(agentId, repoPath);
    return avail.state !== 'disabled' && avail.state !== 'retired';
}
