...
/**
 * Supervisor module — observes agent liveness and emits token-exhaustion failover events.
 *
 ...
 *   4. Emits token-exhaustion workflow events when a positive detector fires.
 *   5. Sends desktop notifications for genuinely dead agents.
 *
 * The supervisor NEVER:
 *   - Kills tmux sessions
 *   - Moves spec files
 *   - Writes state files directly
 */
...
const { buildTokenExhaustionSignal, chooseNextAgent, getLastReachableCommit, buildFailoverPrompt, resolveFailoverConfig, getAgentRuntimeId } = require('./agent-failover');
