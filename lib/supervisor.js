...
async function sweep() {
  ...
  for (const repoPath of repos) {
    ...
    for (const { featureId, snapshot } of snapshots) {
      ...
      // --- Heartbeat expiry observation (LOG ONLY) ---
      try {
        // Just log, don't emit signals
        const stateDir = path.join(absRepo, '.aigon', 'state');
        if (fs.existsSync(stateDir)) {
          const heartbeatFiles = fs.readdirSync(stateDir).filter(f => f.startsWith(`heartbeat-${featureId}-`));
          for (const hbFile of heartbeatFiles) {
            const hbPath = path.join(stateDir, hbFile);
            const stat = fs.statSync(hbPath);
            const age = Date.now() - stat.mtimeMs;
            if (age > DEFAULT_HEARTBEAT_TIMEOUT_MS) {
              const agentId = hbFile.replace(`heartbeat-${featureId}-`, '');
              log(`[observe] F${featureId} ${agentId}: heartbeat expired (${Math.round(age/1000)}s ago)`);
            }
          }
        }
      } catch (err) {
