...
// Workflow engine — the ONLY mutation path (append-only events)
const wf = require('./workflow-core');
const { getSnapshotPath } = require('./workflow-core/paths');
const { getHeartbeatConfig, sweepExpiredHeartbeats } = require('./workflow-heartbeat');

// Config — read-only
const { loadGlobalConfig, readConductorReposFromGlobalConfig, loadProjectConfig } = require('./config');
  ...
  for (const repoPath of repos) {
    ...
    for (const { featureId, snapshot } of snapshots) {
      ...
      for (const [agentId, agent] of Object.entries(snapshot.agents)) {
        // Only check agents the engine thinks are running
        if (agent.status !== 'running' && agent.status !== 'idle') continue;

        // --- Dead session detection ---
        // Only emit session_lost if BOTH the tmux session is dead AND the
        // heartbeat is stale. A fresh heartbeat means the agent is alive
        // even if we can't find the tmux session (name mismatch, etc.)
        ...
        if (!tmuxSessionAlive(sessionName) && !heartbeatFresh) {
          try {
            await wf.emitSignal(absRepo, featureId, 'session-lost', agentId);
            log(`signal.session_lost: F${featureId} ${agentId} (session ${sessionName} dead, heartbeat stale)`);
            sendNotification(`Session lost: F${featureId} ${agentId}`, `Aigon - ${path.basename(absRepo)}`);
          } catch (err) {
            log(`signal.session_lost failed for F${featureId} ${agentId}: ${err.message}`);
          }
        }
      }

      // --- Heartbeat expiry detection (reuse existing sweep logic) ---
      try {
        const expired = await sweepExpiredHeartbeats(absRepo, featureId, { projectConfig: projConfig });
        for (const agentId of expired) {
          log(`signal.heartbeat_expired: F${featureId} ${agentId}`);
          sendNotification(`Heartbeat expired: F${featureId} ${agentId}`, `Aigon - ${path.basename(absRepo)}`);
        }
      } catch (err) {
        log(`heartbeat sweep error for F${featureId}: ${err.message}`);
      }
    }
  }
