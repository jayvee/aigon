...
const VALID_TMUX_ROLES = ['spec-draft', 'do', 'eval', 'review', 'spec-review', 'spec-revise', 'spec-check'];
...
/**
 * Token-exhaustion failover handler (F421). Until F693 this was a runtime
 * registry that Aigon Pro filled at startup; the engine now lives in-repo,
 * so the supervisor calls it directly. Required lazily to keep the module
 * graph acyclic — lib/agent-failover.js reaches back into worktree/status
 * helpers that the supervisor also owns.
 */
function runExhaustionHandler(ctx) {
  return require('./agent-failover').handleExhaustion(ctx);
}
  ...
  for (const [agentId, agent] of Object.entries(snapshot.agents)) {
    ...
    if (entityType === 'feature' && exhaustionSignal && !(statusData.flags && statusData.flags.tokenExhausted)) {
      ...
      if (failoverConfig.policy === 'pause') {
        ...
        // Run the handler after token_exhausted is written so failover_switched
        // always appends after it (prevents out-of-order snapshot on lock contention).
        tokenExhaustedPromise
          .then(() => runExhaustionHandler({ repoPath, entityId, agentId, signal: exhaustionSignal, snapshot, failoverConfig }))
          .catch((error) => {
