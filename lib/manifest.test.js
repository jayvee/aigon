...
test('exports STATE_DIR string', () => {
    assert.strictEqual(typeof manifest.STATE_DIR, 'string');
});

test('exports LOCKS_DIR string', () => {
    assert.strictEqual(typeof manifest.LOCKS_DIR, 'string');
});
...
test('readManifest bootstraps and returns a manifest with correct schema', () => {
...
test('readManifest creates file on disk after bootstrap', () => {
    assert.ok(fs.existsSync(manifest.coordinatorPath(TEST_ID)));
});

test('readManifest returns same data on second call (no double-bootstrap)', () => {
