...
test('readManifest returns null when coordinator file is missing', () => {
...
test('readManifest consistently returns null when missing', () => {
...
test('writeManifest creates and persists manifest file when missing', () => {
    const m = manifest.writeManifest(TEST_ID, {
        id: TEST_ID,
        type: 'feature',
        name: 'manifest-test',
        stage: 'backlog',
    ...
    assert.ok(fs.existsSync(manifest.coordinatorPath(TEST_ID)),
        'writeManifest should persist the file');
    assert.strictEqual(typeof m.id, 'string');
    assert.strictEqual(m.type, 'feature');
});

test('readManifest returns persisted data after writeManifest', () => {
    const m = manifest.readManifest(TEST_ID);
    assert.ok(fs.existsSync(manifest.coordinatorPath(TEST_ID)));
    assert.strictEqual(m.type, 'feature');
});
...
test('writeManifest updates fields and persists', () => {
    // writeManifest was called above, so file exists
...
// ---------------------------------------------------------------------------
// Worktree isolation (redirection via mainRepoPath)
// ---------------------------------------------------------------------------

console.log('# manifest.js — worktree isolation (mainRepoPath)');

const mainRepoTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aigon-main-repo-test-'));
const mainRepoStateDir = path.join(mainRepoTmp, '.aigon', 'state');
const mainRepoLocksDir = path.join(mainRepoTmp, '.aigon', 'locks');

fs.mkdirSync(mainRepoStateDir, { recursive: true });
fs.mkdirSync(mainRepoLocksDir, { recursive: true });

test('getStateDir(options) redirects to mainRepoPath', () => {
    const p = manifest.getStateDir({ mainRepoPath: mainRepoTmp });
    assert.strictEqual(p, mainRepoStateDir);
});

test('getLocksDir(options) redirects to mainRepoPath', () => {
    const p = manifest.getLocksDir({ mainRepoPath: mainRepoTmp });
    assert.strictEqual(p, mainRepoLocksDir);
});

test('writeManifest(options) redirects to mainRepoPath', () => {
    const options = { mainRepoPath: mainRepoTmp };
    manifest.writeManifest(TEST_ID, { type: 'feature', name: 'redirect-test' }, null, 'feature', options);
    
    const expectedPath = path.join(mainRepoStateDir, `feature-${manifest.canonicalId(TEST_ID)}.json`);
    assert.ok(fs.existsSync(expectedPath), `Manifest should exist at ${expectedPath}`);
    
    const m = manifest.readManifest(TEST_ID, 'feature', options);
    assert.strictEqual(m.name, 'redirect-test');
});

test('writeAgentStatus(options) redirects to mainRepoPath', () => {
    const options = { mainRepoPath: mainRepoTmp };
    manifest.writeAgentStatus(TEST_ID, 'cc', { status: 'testing-redirect' }, 'feature', options);
    
    const expectedPath = path.join(mainRepoStateDir, `feature-${manifest.canonicalId(TEST_ID)}-cc.json`);
    assert.ok(fs.existsSync(expectedPath), `Agent status should exist at ${expectedPath}`);
    
    const s = manifest.readAgentStatus(TEST_ID, 'cc', 'feature', options);
    assert.strictEqual(s.status, 'testing-redirect');
});

test('acquireLock(options) redirects to mainRepoPath', () => {
    const options = { mainRepoPath: mainRepoTmp };
    const result = manifest.acquireLock(LOCK_TEST_ID, 'feature', options);
    assert.strictEqual(result, true);
    
    const expectedPath = path.join(mainRepoLocksDir, `feature-${manifest.canonicalId(LOCK_TEST_ID)}.lock`);
    assert.ok(fs.existsSync(expectedPath), `Lock file should exist at ${expectedPath}`);
    
    manifest.releaseLock(LOCK_TEST_ID, 'feature', options);
    assert.ok(!fs.existsSync(expectedPath), 'Lock file should be removed from main repo');
});
