...
function resolveImplementationContinuity({ featureId, agentId, snapshot, continuity, currentSessionId }) {
    const decision = resolveContinuityPolicy({
        phase: 'implementation',
        selectedAgent: agentId,
        authorAgentId: snapshot && snapshot.authorAgentId,
        originSession: continuity && continuity.originSession,
        authorHandoff: continuity && continuity.authorHandoff,
    });
    const instructions = [];
    if (continuity && continuity.authorHandoff && continuity.authorHandoff.status === 'valid') {
        instructions.push(`## Author handoff (current files are authoritative)\n${JSON.stringify(continuity.authorHandoff, null, 2)}`);
    }
    if (decision.strategy === 'resume-origin') {
        instructions.push(
            `## Continuation checkpoint\nAfter reconciling the current checkout, run exactly one:\n`
            + `aigon agent-status continuation-ready ${featureId} ${agentId} --session=${currentSessionId}\n`
            + `aigon agent-status continuation-fallback ${featureId} ${agentId} --session=${currentSessionId} --reason=<context-missing|context-conflict|task-delivery-failed>`
        );
    }
    return {
        decision,
        continuityInstructions: instructions.join('\n\n'),
        resumeProviderSessionId: decision.strategy === 'resume-origin' && continuity && continuity.originSession
            ? continuity.originSession.providerSessionId
            : null,
    };
}

async function abortWorktreeStartup(repoPath, featureId, message) {
    process.exitCode = 1;
    try {
        await wf.pauseFeatureForReason(repoPath, featureId, 'startup_failed');
    } catch (e) {
        console.warn(`⚠️  Could not pause feature after startup failure: ${e.message}`);
    }
    console.error(`❌ ${message}`);
}

function createStartupPhaseTimer(featureId) {
    const start = process.hrtime.bigint();
    let previous = start;
    return (phase, details = '') => {
        const now = process.hrtime.bigint();
        const deltaMs = Number(now - previous) / 1e6;
        const totalMs = Number(now - start) / 1e6;
        previous = now;
        const suffix = details ? ` ${details}` : '';
        console.log(`[aigon:start-phase] feature=${featureId} phase=${phase} delta_ms=${deltaMs.toFixed(1)} total_ms=${totalMs.toFixed(1)}${suffix}`);
    };
}

function openFeatureDetached(args) {
    const entrypoint = process.argv[1];
    if (!entrypoint) return false;
    try {
        // Bounded async work: one opener process handles GUI attachment after
        // detached tmux sessions already exist, keeping git/workflow state serial.
        const argv = [entrypoint, 'feature-open', ...args.map(a => String(a))];
...
async function run(args, deps) {
    ...
    agentIds = agentIds.map(raw => aliasMap[String(raw).toLowerCase()] || String(raw).toLowerCase());
    for (const agentId of agentIds) {
        try {
            agentAvailability.assertAgentUsable(agentId, process.cwd(), {
                explicit: true,
                suggestStart: true,
                featureId: name,
            });
    ...
    let found = findFile(PATHS.features, name, [STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS]);
    ...
    const markStartupPhase = createStartupPhaseTimer(featureId);

    // Guard legacy/mis-operated sets as well as new ones: a member cannot be
    // started while a sibling is still in inbox. The set must be prioritised
    // as one unit first, otherwise the delivery unit has only partially begun.
    if (found.folder === STAGE_FOLDERS.BACKLOG) {
        const setSlug = featureSets.readSetTag(fs.readFileSync(found.fullPath, 'utf8'));
        if (setSlug) {
            const inboxMembers = (featureSets.scanFeatureSets(PATHS.features).get(setSlug) || [])
                .filter((member) => member.stage === 'inbox');
            if (inboxMembers.length > 0) {
                process.exitCode = 1;
                return console.error(
                    `❌ Feature ${featureId} belongs to set "${setSlug}", which still has ${inboxMembers.length} inbox member(s).\n`
                    + `   Run: aigon set-prioritise ${setSlug}`,
                );
            }
        }
    }
    ...
    if (found.folder === STAGE_FOLDERS.IN_PROGRESS) {
        ...
        if (
            ...
            try {
                const { isFeatureSuspended } = require('./sync-state');
    ...
    for (const id of agentIds) {
        if (!agentRegistry.getAgent(id)) {
            return console.error(`❌ Unknown agent '${id}'. Launchable agents: ${agentRegistry.getLaunchableAgentIds().join(', ')}`);
        }
        const deactivatedMsg = agentRegistry.formatDeactivatedAgentMessage(id);
        if (deactivatedMsg) {
            return console.error(`❌ Cannot start: ${deactivatedMsg}`);
        }
    }
    ...
    if (!skipQuotaCheck && agentIds.length > 0) {
        ...
        // Cache-only check: only block on a known-depleted verdict from the
        // poller-maintained cache. Cache miss (never probed, available, or
        // unknown) permits start; lib/agent-quota-poller.js keeps the cache fresh
        // in the background, and we kick a best-effort refresh below so the
        // next call is warm. Probing synchronously here used to add ~1-2s
        // per agent on cache miss, blocking the dashboard's Start button.
        for (const agentId of agentIds) {
            ...
            const quotaEntry = quotaProbe.isPairDepleted(process.cwd(), agentId, modelValue);
            if (quotaEntry) {
                process.exitCode = 1;
            ...
            for (const providerId of providerIds) {
                const providerEntry = quotaProbe.isProviderDepleted(process.cwd(), providerId);
                if (providerEntry) {
                    process.exitCode = 1;
                    return console.error(providerQuotaPoller.formatProviderStartGateMessage({
                        providerId,
                        entry: providerEntry,
                        agentId,
                        featureId,
            ...
            setImmediate(() => {
                try {
                    ...
                    for (const providerId of providerIds) {
                        providerQuotaPoller.pollProvider(providerId, { repoPath: process.cwd(), force: false }).catch(() => {});
                    }
...

    {
        ...
        const repoPath = process.cwd();
        const specFromPath = found.folder === STAGE_FOLDERS.BACKLOG
            ? path.join(PATHS.features.root, STAGE_FOLDERS.BACKLOG, found.file)
            : null;
        const specToPath = specFromPath
            ? path.join(PATHS.features.root, STAGE_FOLDERS.IN_PROGRESS, found.file)
            : null;
        ...
        if (specFromPath && agentIds.length === 0 && shouldWriteImplementationLogStarter({
            ...
            instructions: projCfgStart.instructions,
        })) {
        ...
        try {
            ...
            if (snapshot && snapshot.lifecycle !== 'backlog') {
                ...
                const startEffects = [];
                if (specLifecycle.shouldMoveSpecFiles(repoPath) && specFromPath && specToPath && specFromPath !== specToPath) {
        ...
        found = findFile(PATHS.features, name, [STAGE_FOLDERS.IN_PROGRESS]);
        if (!found) { process.exitCode = 1; return console.error(`❌ Could not find feature "${name}" in in-progress after engine start.`); }
        const movedFromBacklog = !!specFromPath && specLifecycle.shouldMoveSpecFiles(process.cwd());
        ...
        if (movedFromBacklog) {
            try {
                ...
                markStartupPhase('spec_move', 'status=committed');
            } catch (e) {
                if (mode !== 'drive') {
                    ...
                    await abortWorktreeStartup(repoPath, featureId, 'Feature start aborted: spec move commit failed.');
                    return;
                }
                console.warn(`⚠️  Could not commit spec move: ${e.message}`);
                markStartupPhase('spec_move', 'status=commit_failed_nonfatal');
            }
        } else {
            if (specFromPath) specLifecycle.refreshLifecycleView(process.cwd(), { warn: console.warn });
            markStartupPhase('spec_move', 'status=skipped');
        }
    }
    ...
    if (mode === 'drive') {
        ...
        let worktreeStartupFailed = false;
        ...
        if (useTmux || backgroundMode) {
            try {
                ...
                await abortWorktreeStartup(process.cwd(), featureId, e.message);
        ...
        agentIds.forEach(agentId => {
            ...
            if (isValidWorktree) {
                ...
                preparedWorktrees.push({ agentId, worktreePath });
                markStartupPhase('worktree_add', `agent=${agentId} status=reused`);
                markStartupPhase('worktree_setup', `agent=${agentId} status=skipped_existing`);
            } else {
                try {
                    runGit(`git worktree add ${worktreePath} -b ${branchName}`);
                    console.log(`📂 Worktree: ${worktreePath}`);
                    markStartupPhase('worktree_add', `agent=${agentId} status=created`);
                    ...
                    const wtSpecDir = path.join(worktreePath, 'docs', 'specs', 'features', STAGE_FOLDERS.IN_PROGRESS);
                    ...
                    if (!specExistsInWt && specLifecycle.isStableSpecLayout(process.cwd())) {
                        specLifecycle.syncWorktreeLifecycleLinks(process.cwd(), worktreePath, 'feature', num);
                        specExistsInWt = fs.existsSync(wtSpecPath)
                            || Boolean(specLifecycle.findCanonicalSpecPath(process.cwd(), 'feature', num));
                    } else if (!specExistsInWt) {
                        const mainSpecPath = path.join(PATHS.features.root, STAGE_FOLDERS.IN_PROGRESS, specFileName);
                        if (fs.existsSync(mainSpecPath)) {
                            ...
                            try {
                                ...
                                runGit(`git -C ${JSON.stringify(worktreePath)} commit -m ${JSON.stringify(`chore: sync feature ${num} spec to worktree`)} --trailer "Aigon-Internal: true" -- ${JSON.stringify(relSpecPath)}`);
                    ...
                    if (!specExistsInWt) {
                        ...
                        worktreeStartupFailed = true;
                        markStartupPhase('worktree_setup', `agent=${agentId} status=missing_spec`);
                        return;
                    }
                    ...
                    preparedWorktrees.push({ agentId, worktreePath });
                    markStartupPhase('worktree_setup', `agent=${agentId}`);
                    ...
                    worktreeStartupFailed = true;
                    markStartupPhase('worktree_add', `agent=${agentId} status=failed`);
                    markStartupPhase('worktree_setup', `agent=${agentId} status=skipped_failed_add`);
                }
            }
            // Engine tracks its own effects — no legacy completePendingOp needed
        });

        if (worktreeStartupFailed || preparedWorktrees.length < agentIds.length) {
            await abortWorktreeStartup(
                process.cwd(),
                featureId,
                'Feature start aborted: worktree setup did not complete for all agents.',
            );
            return;
        }

        // Add read permissions for all worktrees to Claude settings
        const allWorktreePaths = preparedWorktrees.map(wt => wt.worktreePath);
        ...
        markStartupPhase('trust_writes', `worktrees=${allWorktreePaths.length} agents=${agentIds.length}`);
        ...
        if (preparedWorktrees.length > 0) {
            ...
            const worktreeByAgent = new Map(preparedWorktrees.map(wt => [wt.agentId, wt.worktreePath]));
            const currentSnapshot = await wf.showFeatureOrNull(process.cwd(), featureId);
            const continuity = entityContext.readEntityContext(process.cwd(), 'feature', featureId) || {};
            const continuityByAgent = new Map(agentIds.map((agentId) => {
                const currentSessionId = `implementation-feature-${String(featureId).padStart(2, '0')}-${agentId}-${Date.now()}`;
                ...
                launch.decision.currentSessionId = currentSessionId;
                launch.decision.role = 'do';
                entityContext.recordContinuityDecision(process.cwd(), 'feature', featureId, launch.decision);
                console.log(`🧭 Continuity (${agentId}): ${launch.decision.strategy} — ${launch.decision.reasons.join(', ')}`);
                return [agentId, launch];
            }));
            const sessionResults = ensureAgentSessions(num, preparedWorktrees.map(wt => wt.agentId), {
                sessionNameBuilder: (featureId, agent) => buildTmuxSessionName(featureId, agent, {
                    desc,
                    role: 'do',
                    worktreePath: worktreeByAgent.get(agent),
                }),
                cwdBuilder: (_, agent) => worktreeByAgent.get(agent),
                commandBuilder: (featureId, agent) => {
                    const continuation = continuityByAgent.get(agent) || {};
                    const wt = {
                        featureId,
                        agent,
                        ...
                        desc,
                        repoPath: process.cwd(),
                        continuityInstructions: continuation.continuityInstructions || '',
                        resumeProviderSessionId: continuation.resumeProviderSessionId || null,
                    };
            ...
            sessionResults.forEach(result => {
                if (result.error) {
                    ...
                    if (result.created) clearSessionEndedFlag(num, result.agent);
                    console.log(`   ✓ ${result.sessionName}${result.created ? ' → started' : ' (already exists)'}`);
                }
            });
            const createdCount = sessionResults.filter(result => !result.error && result.created).length;
            const existingCount = sessionResults.filter(result => !result.error && !result.created).length;
            const errorCount = sessionResults.filter(result => result.error).length;
            markStartupPhase('tmux_session_creation', `created=${createdCount} existing=${existingCount} errors=${errorCount}`);
            const repoName = path.basename(process.cwd());
            console.log(`\n   Attach: tmux attach -t ${repoName}-f${parseInt(num, 10)}-<agent>-${desc}`);
            console.log(`   List:   tmux ls`);
        } else {
            markStartupPhase('tmux_session_creation', 'created=0 existing=0 errors=0');
        }
        ...
        if (agentIds.length === 1) {
            ...
            if (backgroundMode) {
                ...
                markStartupPhase('terminal_opening', 'mode=background status=skipped');
            } else {
                console.log(`\n🚀 Starting agent terminal...`);
                cmds['feature-open']([num, agentIds[0]]);
                markStartupPhase('terminal_opening', 'mode=foreground status=completed');
            }
            console.log(`   When done: aigon feature-close ${num}`);
        } else {
            ...
            if (backgroundMode) {
                ...
                console.log(`\n🚀 Starting agent terminals in the background...`);
                const opened = openFeatureDetached([num, '--all']);
                if (opened) {
                    console.log(`   Detached opener started. View all: aigon feature-open ${num} --all`);
                    markStartupPhase('terminal_opening', 'mode=detached status=scheduled');
                } else {
                    console.warn(`   ⚠️  Could not start detached opener. View all: aigon feature-open ${num} --all`);
                    markStartupPhase('terminal_opening', 'mode=detached status=failed_to_schedule');
                }
            }
...
module.exports = { run, resolveImplementationContinuity };
