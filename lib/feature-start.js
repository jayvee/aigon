    ...
    if (found.folder === '03-in-progress') {
        ...
        if (existingSnapshot) {
            ...
            if (
                ...
                // F359: detect suspended state — snapshot says worktree mode
                // but no live local worktree (e.g. after `aigon sync pull` from
                // another machine). Don't block the user from restarting.
                ...
                try {
                    ...
                    isSuspended = isFeatureSuspended(repoPath, runningId, { hasLocalWorktree: wts.length > 0 });
                } catch (_) { /* fall through */ }
                if (isSuspended) {
                    console.log(`⚠️  Feature ${runningId} is suspended (worktree missing on this machine).`);
                    const snapshotAgents = Object.keys(existingSnapshot.agents || {});
                    const agentHint = snapshotAgents.length > 0
                        ? snapshotAgents.map(a => `--agent=${a}`).join(' ')
                        : '--agent=<id>';
                    console.log(`   Re-create the worktree by re-running with explicit agents:`);
                    console.log(`   aigon feature-start ${runningId} ${agentHint}`);
                    return;
                }
