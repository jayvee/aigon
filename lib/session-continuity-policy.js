...
function resolveContinuityPolicy(facts = {}) {
    ...
    // A provider ID is usable when it was either captured by Aigon's normal
    // launcher or read from the agent adapter's declared runtime variable.
    // `runtime-env` is how a direct Codex conversation is attributable; it is
    // intentionally not downgraded merely because Aigon did not launch it.
    const nativeAvailable = Boolean(origin && origin.providerSessionId
        && ['attributed', 'runtime-env'].includes(origin.nativeProvenance));
    ...
    if (authorMatches && handoffComplete && nativeAvailable && capability.resumeById
        && capability.taskDelivery === 'initial-argument' && healthy) {
        return {
            strategy: 'resume-origin',
            confidence: 'medium',
            reasons: [phase === 'spec-revise' ? 'phase-prefers-author' : 'implementation-origin-healthy', 'native-origin-attributed', 'adapter-resume-and-task-delivery-verified'],
            selectedAgent,
