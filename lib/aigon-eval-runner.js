...
/** Lists paths changed versus baseRef (working tree + index vs snapshot at baseRef — includes unstaged edits). */
...
/**
 * Copies canned Markdown + bootstrap file(s), persists engine snapshots, commits once so scope
 * diffs omit template copies and mirror only the agent phase.
 *
 * REGRESSION: Scoped file checks compared to bare `git diff --name-only HEAD` after commits were
 * empty (everything staged as commits); baseline must include a post-bootstrap revision.
 *
 * Returns the HEAD SHA after bootstrap (pass to readChangedFiles).
 */
function bootstrapWorkloadInWorktree(worktreePath, workload, fixture = {}) {
    const id = String(fixture.id || '').trim();
    if (!id) throw new Error('aigon-eval: workload fixture missing id');

    if (workload === 'feature') {
        fs.mkdirSync(path.join(worktreePath, 'docs', 'specs', 'features', '03-in-progress'), { recursive: true });
        const specMd = fs.readFileSync(path.join(WORKLOADS_DIR, 'feature', 'spec.md'), 'utf8');
        const specDest = path.join(worktreePath, 'docs', 'specs', 'features', '03-in-progress', `feature-${id}-aigon-eval-fixture.md`);
        fs.writeFileSync(
            specDest,
            ['---', 'complexity: low', '---', '', specMd].join('\n'),
            'utf8'
        );
        fs.writeFileSync(path.join(worktreePath, 'eval-fixture.txt'), 'before\n', 'utf8');
        engine.ensureEntityBootstrappedSync(worktreePath, 'feature', id, 'implementing', specDest);
    } else if (workload === 'research') {
        fs.mkdirSync(path.join(worktreePath, 'docs', 'specs', 'research-topics', '03-in-progress'), { recursive: true });
        fs.mkdirSync(path.join(worktreePath, 'docs', 'specs', 'research-topics', 'logs'), { recursive: true });
        const specMd = fs.readFileSync(path.join(WORKLOADS_DIR, 'research', 'spec.md'), 'utf8');
        const specDest = path.join(worktreePath, 'docs', 'specs', 'research-topics', '03-in-progress', `research-${id}-aigon-eval-fixture.md`);
        fs.writeFileSync(
            specDest,
            ['---', 'complexity: low', '---', '', specMd].join('\n'),
            'utf8'
        );
        engine.ensureEntityBootstrappedSync(worktreePath, 'research', id, 'implementing', specDest);
    } else {
        throw new Error(`aigon-eval: unknown workload ${workload}`);
    }

    runGit(worktreePath, ['add', '-A']);
    runGit(worktreePath, ['commit', '-qm', 'aigon-eval: bootstrap canned workload']);
...
function readTextFileIfExists(filePath, maxBytes = 512000) {
...
function transcriptLinesWithAigonCommands(text) {
    return String(text || '')
        .split(/\r?\n/)
        .map(line => line.trim())
        .filter(line => /\baigon\s+(feature-close|research-close|feature-eval)\b/.test(line));
}

function collectCommandEventsFromSessionSidecars(repoPath, workload, fixture = {}, agent = null) {
    const sessionsDir = path.join(repoPath, '.aigon', 'sessions');
    if (!fs.existsSync(sessionsDir)) return [];
    const expectedEntityType = workload === 'research' ? 'r' : 'f';
    const expectedId = String(fixture.id || '');
    const events = [];
    let entries = [];
    ...
    entries.forEach(file => {
        ...
        let sidecar;
        try { sidecar = JSON.parse(fs.readFileSync(path.join(sessionsDir, file), 'utf8')); } catch (_) { return; }
        if (!sidecar || typeof sidecar !== 'object') return;
        if (sidecar.entityType !== expectedEntityType) return;
        if (expectedId && String(sidecar.entityId) !== expectedId) return;
        if (agent && sidecar.agent && String(sidecar.agent) !== String(agent)) return;
        const text = readTextFileIfExists(sidecar.agentSessionPath || sidecar.tmuxLogPath);
        transcriptLinesWithAigonCommands(text).forEach(commandLine => {
            events.push({
                command: commandLine,
                source: sidecar.agentSessionPath ? 'agent-session-transcript' : 'tmux-pane-log',
                sessionName: sidecar.sessionName || path.basename(file, '.json'),
            });
        });
    });
...
async function runSingleEval(options = {}) {
    ...
    try {
        ...
        const scopeBaselineSha = bootstrapWorkloadInWorktree(worktreePath, workload, fixture);
        ...
        const finalEngineSnapshot = readJsonIfExists(path.join(worktreePath, '.aigon', 'workflows', workload === 'research' ? 'research' : 'features', String(fixture.id), 'snapshot.json')) || {};
        const gitDiff = { changedFiles: readChangedFiles(worktreePath, scopeBaselineSha) };
        const commandEvents = [
            { command: `aigon ${runArgs.join(' ')}`, result: commandResult.status, source: 'runner' },
            ...collectCommandEventsFromSessionSidecars(worktreePath, workload, fixture, agent),
        ];
        const absoluteFinalSpec = fixture.finalSpecPath
            ? path.join(worktreePath, fixture.finalSpecPath)
            : null;
        const matrix = checks.runCheckMatrix({
            fixture,
            telemetryEvents,
            ...
            gitDiff,
            commandEvents,
            finalSpecPath: absoluteFinalSpec,
            slaSeconds: options.slaSeconds || fixture.slaSeconds || DEFAULT_SLA_SECONDS,
        });
        const result = {
            agent,
...
async function runEvaluationMatrix(options = {}) {
    ...
    const results = [];
    const injectedList = Array.isArray(options.injectedRuns) ? options.injectedRuns : null;
    let injectedIdx = 0;
    for (const pair of pairs) {
        for (const workload of workloads) {
            for (let i = 0; i < runs; i++) {
                const injectedRun = injectedList != null ? injectedList[injectedIdx++] : null;
...
module.exports = {
    ...
    aggregateMatrix,
    writeRunResult,
    collectCommandEventsFromSessionSidecars,
    transcriptLinesWithAigonCommands,
};
