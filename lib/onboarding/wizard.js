'use strict';

const fs = require('fs');
const os = require('os');
const path = require('path');
const { execFileSync, spawnSync } = require('child_process');
const http = require('http');
const clack = require('@clack/prompts');
const { STAGE_FOLDERS } = require('../workflow-core/paths');
const {
    readOnboardingState,
    writeStepState,
    markComplete,
    isOnboardingComplete,
    getFirstResumableStep,
    validateStepIds,
    shouldRunStep: shouldRunPersistedStep,
    STEP_IDS,
} = require('./state');

const REPO_SCAN_EXCLUDE = new Set([
    '.aigon',
    '.cache',
    '.config',
    '.git',
    '.npm',
    '.Trash',
    'Library',
    'node_modules',
]);

function expandHome(input) {
    const value = String(input || '').trim();
    if (!value) return value;
    return value.startsWith('~') ? path.join(os.homedir(), value.slice(1)) : value;
}

function getDefaultRepoScanDir() {
    const candidates = [
        path.join(os.homedir(), 'src'),
        path.join(os.homedir(), 'SRC'),
        path.join(os.homedir(), 'projects'),
        path.join(os.homedir(), 'Projects'),
        os.homedir(),
    ];
    return candidates.find(candidate => fs.existsSync(candidate)) || os.homedir();
}

function isDirectGitRepo(candidatePath) {
    try {
        return fs.existsSync(path.join(candidatePath, '.git'));
    } catch (_) {
        return false;
    }
}

function discoverGitRepos(parentDir) {
    const root = path.resolve(expandHome(parentDir));
    let entries;
    try {
        entries = fs.readdirSync(root, { withFileTypes: true });
    } catch (_) {
        return [];
    }

    return entries
        .filter(entry => entry.isDirectory())
        .filter(entry => !REPO_SCAN_EXCLUDE.has(entry.name))
        .map(entry => path.join(root, entry.name))
        .filter(isDirectGitRepo)
        .map(repoPath => path.resolve(repoPath))
        .sort((a, b) => path.basename(a).localeCompare(path.basename(b)));
}

function isAigonInitialized(repoPath) {
    return fs.existsSync(path.join(repoPath, '.aigon', 'config.json'))
        || fs.existsSync(path.join(repoPath, 'docs', 'specs'));
}

function isProPackageInstalled() {
    try {
        require.resolve('@senlabsai/aigon-pro');
        return true;
    } catch (_) {
        try {
            const globalRoot = spawnSync('npm', ['root', '-g'], { encoding: 'utf8', stdio: 'pipe' });
            if (globalRoot.status !== 0) return false;
            const candidate = path.join(String(globalRoot.stdout || '').trim(), '@senlabsai', 'aigon-pro');
            require.resolve(candidate);
            return true;
        } catch (_) {
            return false;
        }
    }
}

function getGlobalConfigPath() {
    return path.join(os.homedir(), '.aigon', 'config.json');
}

function readGlobalConfig() {
    try {
        return JSON.parse(fs.readFileSync(getGlobalConfigPath(), 'utf8'));
    } catch (_) {
        return {};
    }
}

function writeGlobalConfig(config) {
    require('../config').saveGlobalConfig({
        ...config,
        repos: config.repos || [],
        machineId: config.machineId || require('../config').getDefaultMachineId(),
    });
}

function parseSelectedSteps(args) {
    const selected = [];
    for (let index = 0; index < args.length; index++) {
        const arg = args[index];
        if (arg === '--step') selected.push(args[++index]);
        else if (arg.startsWith('--step=')) selected.push(arg.slice('--step='.length));
    }
    const invalid = selected.some(id => !id) ? ['(missing)'] : validateStepIds(selected);
    if (invalid.length) throw new Error(`Usage: aigon setup [--yes] [--resume] [--step <${STEP_IDS.join('|')}>]\nInvalid setup step: ${invalid.join(', ')}`);
    return [...new Set(selected)];
}

function waitForDashboardHealth(port = 4100, attempts = 12) {
    return new Promise(resolve => {
        let remaining = attempts;
        const check = () => {
            const request = http.get(`http://127.0.0.1:${port}/api/health`, response => {
                response.resume();
                if (response.statusCode === 200) return resolve(true);
                retry();
            });
            request.setTimeout(1000, () => request.destroy());
            request.on('error', retry);
            function retry() {
                if (--remaining <= 0) return resolve(false);
                setTimeout(check, 500);
            }
        };
        check();
    });
}

function installProPackage() {
    const npmArgs = ['install', '-g', '@senlabsai/aigon-pro@beta'];
    const needsSudo = process.platform !== 'darwin' && process.getuid && process.getuid() !== 0;
    const bin = needsSudo ? 'sudo' : 'npm';
    const args = needsSudo ? ['npm', ...npmArgs] : npmArgs;
    return spawnSync(bin, args, { stdio: 'inherit' });
}

async function runWizard(args = []) {
    const yesFlag = args.includes('--yes');
    const resumeFlag = args.includes('--resume');
    const selectedSteps = parseSelectedSteps(args);

    process.on('SIGINT', () => {
        clack.cancel('Setup cancelled.');
        process.exit(0);
    });

    // Non-interactive guard
    const isCI = !!(process.env.CI);
    const isTTY = process.stdin.isTTY && process.stdout.isTTY;
    if ((!isTTY || isCI) && !yesFlag) {
        console.log('aigon setup: non-interactive environment detected. Run with --yes to apply defaults.');
        return;
    }

    const state = readOnboardingState();

    // Already complete — print summary and exit
    if (isOnboardingComplete(state) && !resumeFlag && selectedSteps.length === 0) {
        clack.intro('Aigon Setup');
        clack.note(
            STEP_IDS.map(id => `${state.steps[id] === 'done' ? '✅' : '⏭️ '} ${id}`).join('\n'),
            'Setup already complete'
        );
        clack.outro('Run with --resume to re-run skipped steps, or aigon global-setup --force to reconfigure.');
        return;
    }

    // Determine start step
    const startStep = resumeFlag ? getFirstResumableStep(state) : STEP_IDS[0];
    if (resumeFlag && !startStep && selectedSteps.length === 0) {
        clack.intro('Aigon Setup');
        clack.note('All steps are already complete.', 'Nothing to resume');
        clack.outro('Done!');
        return;
    }

    clack.intro('🚀 Aigon Setup Wizard');

    // ── Step 1: prereqs ─────────────────────────────────────────────────────
    if (shouldRunStep('prereqs', startStep, state, { resumeFlag, selectedSteps })) {
        const { getDetectors } = require('./detectors');
        const detectors = getDetectors();
        let hardFail = false;

        for (const detector of detectors) {
            const spin = clack.spinner();
            spin.start(`Checking ${detector.label}…`);
            const result = await detector.check();
            if (result.found) {
                spin.stop(`✅ ${detector.label}${result.version ? ` (${result.version})` : ''}`);
            } else if (detector.required) {
                const installResult = await detector.install();
                const remediation = installResult.output || 'Manual install required.';
                spin.stop(`❌ ${detector.label} — required but not found`);
                clack.note(remediation, `Install ${detector.label}`);
                hardFail = true;
            } else {
                spin.stop(`⚠️  ${detector.label} — not found (optional)`);
                if (!yesFlag) {
                    const shouldInstall = await clack.confirm({
                        message: `Install ${detector.label} now?`,
                        initialValue: true,
                    });
                    if (clack.isCancel(shouldInstall)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    if (shouldInstall) {
                        const installSpin = clack.spinner();
                        installSpin.start(`Installing ${detector.label}…`);
                        const installResult = await detector.install();
                        installSpin.stop(installResult.ok ? `✅ Installed ${detector.label}` : `❌ Install failed: ${installResult.output}`);
                    }
                }
            }
        }

        if (hardFail) {
            writeStepState('prereqs', 'failed');
            clack.cancel('Setup aborted: required dependencies are missing. See remediation steps above.');
            process.exit(1);
        }

        writeStepState('prereqs', 'done');

        // ── Git identity ─────────────────────────────────────────────────────
        const gitName = (() => { try { return execFileSync('git', ['config', '--global', 'user.name'], { encoding: 'utf8', stdio: 'pipe' }).trim(); } catch { return ''; } })();
        const gitEmail = (() => { try { return execFileSync('git', ['config', '--global', 'user.email'], { encoding: 'utf8', stdio: 'pipe' }).trim(); } catch { return ''; } })();
        if (!gitName || !gitEmail) {
            clack.log.warn('Git identity not set — required for committing spec moves.');
            if (!yesFlag) {
                const name = !gitName ? await clack.text({ message: 'Your name (for git commits):', placeholder: 'Jane Smith' }) : gitName;
                if (clack.isCancel(name)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                const email = !gitEmail ? await clack.text({ message: 'Your email (for git commits):', placeholder: 'jane@example.com' }) : gitEmail;
                if (clack.isCancel(email)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                if (name) execFileSync('git', ['config', '--global', 'user.name', String(name)], { stdio: 'ignore' });
                if (email) execFileSync('git', ['config', '--global', 'user.email', String(email)], { stdio: 'ignore' });
                clack.log.success('Git identity saved.');
            }
        }

        // ── gh auth ──────────────────────────────────────────────────────────
        const ghAuthed = (() => { try { execFileSync('gh', ['auth', 'status'], { encoding: 'utf8', stdio: 'pipe' }); return true; } catch { return false; } })();
        const ghInstalled = (() => { try { execFileSync('gh', ['--version'], { encoding: 'utf8', stdio: 'pipe' }); return true; } catch { return false; } })();
        if (ghInstalled && !ghAuthed && !yesFlag) {
            const doAuth = await clack.confirm({ message: 'Authenticate with GitHub now? (optional — only for PR-based workflows and remote checks; feature-close works without it)', initialValue: false });
            if (clack.isCancel(doAuth)) { clack.cancel('Setup cancelled.'); process.exit(0); }
            if (doAuth) {
                clack.log.info('Launching gh auth login — follow the prompts.');
                spawnSync('gh', ['auth', 'login'], { stdio: 'inherit' });
            }
        }
    }

    // ── Step 2: terminal ─────────────────────────────────────────────────────
    if (shouldRunStep('terminal', startStep, state, { resumeFlag, selectedSteps })) {
        const { selectTerminal, saveTerminalPreference } = require('./terminal');

        const terminalApp = await selectTerminal(yesFlag);
        if (terminalApp) {
            saveTerminalPreference(terminalApp);
            clack.note(`Set terminal: ${terminalApp}`, 'Terminal preference saved');
        }
        // Linux: skip terminal step silently

        writeStepState('terminal', 'done');
    }

    // ── Step 3: agents ───────────────────────────────────────────────────────
    if (shouldRunStep('agents', startStep, state, { resumeFlag, selectedSteps })) {
        const { getAgentDetectors } = require('./detectors');
        const agentDetectors = getAgentDetectors();

        if (agentDetectors.length === 0) {
            clack.note('No agents registered.', 'Agents');
            writeStepState('agents', 'skipped');
        } else {
            // Pre-verify every detector so we can flag already-installed agents
            // in the picker and skip the install + auth steps for them.
            const checkSpin = clack.spinner();
            checkSpin.start('Checking which agent CLIs are already installed…');
            const verifiedById = {};
            for (const d of agentDetectors) {
                try {
                    verifiedById[d.id] = await d.verify();
                } catch (_) {
                    verifiedById[d.id] = { found: false };
                }
            }
            const alreadyInstalledCount = Object.values(verifiedById).filter(v => v && v.found).length;
            checkSpin.stop(alreadyInstalledCount > 0
                ? `Detected ${alreadyInstalledCount} agent CLI${alreadyInstalledCount === 1 ? '' : 's'} already installed`
                : 'No agent CLIs detected on PATH');

            let selectedAgents = [];
            if (yesFlag) {
                selectedAgents = []; // opt-in only — don't auto-install
            } else {
                // Put the installed indicator in the LABEL itself (not just
                // the hint, which clack only shows on focus). So every option
                // visibly reads as "Claude Code ✓ installed v2.1.138" or just
                // "Cursor" — the difference is unmissable scanning the list.
                const installedLabels = [];
                const options = agentDetectors.map(d => {
                    const v = verifiedById[d.id] || {};
                    if (v.found) {
                        const ver = v.version ? ` v${String(v.version).replace(/^v/, '')}` : '';
                        installedLabels.push(d.label);
                        return {
                            value: d.id,
                            label: `${d.label}  ✓ installed${ver}`,
                            hint: 'select to reinstall',
                        };
                    }
                    return { value: d.id, label: d.label };
                });
                const installedSummary = installedLabels.length
                    ? `Already installed: ${installedLabels.join(', ')}. Tick to reinstall, or proceed to install the others.`
                    : 'No agents detected — pick the ones you want.';

                const choices = await clack.multiselect({
                    message: `Which AI agents do you want to install?  (${installedSummary})`,
                    options,
                    initialValues: [],
                    required: false,
                });
                if (clack.isCancel(choices)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                selectedAgents = choices || [];
            }

            // Auth instructions per agent binary — shown after install
            for (const agentId of selectedAgents) {
                const detector = agentDetectors.find(d => d.id === agentId);
                if (!detector) continue;

                const preVerified = verifiedById[agentId];
                if (preVerified && preVerified.found) {
                    // User explicitly picked an already-installed agent → still
                    // run install (treat as upgrade), but skip the auth flow
                    // since they presumably already have credentials.
                    const spin = clack.spinner();
                    spin.start(`Re-installing ${detector.label} (already present)…`);
                    const result = await detector.install();
                    spin.stop(result.ok
                        ? `✅ ${detector.label} re-installed`
                        : `❌ ${detector.label}: ${result.output}`);
                    continue;
                }

                const spin = clack.spinner();
                spin.start(`Installing ${detector.label}…`);
                const result = await detector.install();
                if (result.ok) {
                    const verified = await detector.verify();
                    spin.stop(verified.found ? `✅ ${detector.label} installed` : `⚠️  ${detector.label} install succeeded but verify failed`);
                } else {
                    spin.stop(`❌ ${detector.label}: ${result.output}`);
                    continue;
                }

                // Offer auth for agents that need it
                const registryAgent = require('../agent-registry').getAgent(agentId.replace('agent:', ''));
                const binary = registryAgent && registryAgent.cli && registryAgent.cli.command;
                const authInfo = registryAgent && registryAgent.auth;
                if (binary && authInfo && !yesFlag) {
                    const doAuth = await clack.confirm({
                        message: `Authenticate ${detector.label} now?`,
                        initialValue: true,
                    });
                    if (clack.isCancel(doAuth)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    if (doAuth) {
                        clack.log.info(`Launching ${detector.label}. ${authInfo.hint || 'Complete authentication, then return here.'}`);
                        spawnSync(binary, [], { stdio: 'inherit' });
                        clack.log.success(`${detector.label} authentication step complete.`);
                    }
                } else if (!yesFlag && !authInfo) clack.log.info(`${detector.label} authentication is manual; consult its CLI documentation.`);
            }

            writeStepState('agents', 'done');
        }
    }

    // ── Step 4: pro ──────────────────────────────────────────────────────────
    if (shouldRunStep('pro', startStep, state, { resumeFlag, selectedSteps })) {
        const config = readGlobalConfig();
        const installed = isProPackageInstalled();
        const hasKey = Boolean(config.proKey);

        if (yesFlag) {
            const envKey = String(process.env.AIGON_PRO_KEY || '').trim();
            let proInstalled = installed;
            if (!proInstalled && envKey) {
                const result = installProPackage();
                proInstalled = result.status === 0 && isProPackageInstalled();
            }
            if (proInstalled && envKey) {
                const nextConfig = readGlobalConfig();
                nextConfig.proKey = envKey;
                writeGlobalConfig(nextConfig);
                writeStepState('pro', 'done');
            } else {
                writeStepState('pro', proInstalled && hasKey ? 'done' : 'skipped');
            }
        } else {
            const installPro = await clack.confirm({
                message: installed ? 'Aigon Pro is installed. Configure the Pro beta key now?' : 'Install and activate Aigon Pro beta?',
                initialValue: true,
            });
            if (clack.isCancel(installPro)) { clack.cancel('Setup cancelled.'); process.exit(0); }

            if (!installPro) {
                writeStepState('pro', 'skipped');
            } else {
                if (!installed) {
                    const spin = clack.spinner();
                    spin.start('Installing @senlabsai/aigon-pro@beta…');
                    const result = installProPackage();
                    spin.stop(result.status === 0 ? '✅ Aigon Pro installed' : '❌ Aigon Pro install failed');
                    if (result.status !== 0) {
                        clack.note('You can retry later with: sudo npm install -g @senlabsai/aigon-pro@beta', 'Pro setup');
                        writeStepState('pro', 'skipped');
                    }
                }

                if (isProPackageInstalled()) {
                    const key = await clack.password({
                        message: 'Aigon Pro beta key',
                        placeholder: hasKey ? 'Already saved — press Enter to keep it' : 'paste your Aigon Pro key',
                    });
                    if (clack.isCancel(key)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    const nextKey = String(key || '').trim();
                    if (nextKey) {
                        const nextConfig = readGlobalConfig();
                        nextConfig.proKey = nextKey;
                        writeGlobalConfig(nextConfig);
                        clack.log.success('Aigon Pro key saved.');
                    } else if (hasKey) {
                        clack.log.info('Keeping existing Aigon Pro key.');
                    } else {
                        clack.log.warn('No Pro key saved. Pro features remain disabled until you run: aigon pro activate <key>');
                    }
                    writeStepState('pro', (nextKey || hasKey) ? 'done' : 'skipped');
                }
            }
        }
    }

    // ── Step 5: seed-repo ────────────────────────────────────────────────────
    if (shouldRunStep('seed-repo', startStep, state, { resumeFlag, selectedSteps })) {
        let doClone = false;
        if (!yesFlag) {
            const confirm = await clack.confirm({
                message: 'Clone the brewboard seed repo for a ready-to-use Aigon project?',
                initialValue: true,
            });
            if (clack.isCancel(confirm)) { clack.cancel('Setup cancelled.'); process.exit(0); }
            doClone = confirm;
        }

        if (doClone) {
            const defaultDir = path.join(os.homedir(), 'src', 'brewboard');
            let targetDir = defaultDir;
            if (!yesFlag) {
                const dirAnswer = await clack.text({
                    message: 'Clone to which directory?',
                    placeholder: defaultDir,
                    defaultValue: defaultDir,
                });
                if (clack.isCancel(dirAnswer)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                targetDir = path.resolve((dirAnswer || '').trim() || defaultDir);
            }

            const targetExists = fs.existsSync(targetDir);
            const targetIsGitRepo = isDirectGitRepo(targetDir);
            let seedReady = false;
            if (targetIsGitRepo) {
                clack.log.info(`Using existing Git repo at ${targetDir}`);
                seedReady = true;
            } else if (targetExists) {
                clack.log.error(`${targetDir} already exists and is not a Git repo — marking step as skipped`);
                writeStepState('seed-repo', 'skipped');
            } else {
                fs.mkdirSync(path.dirname(targetDir), { recursive: true });
                console.log(`Cloning to ${targetDir}…`);
                const cloneResult = spawnSync('git', ['clone', '--depth', '1', 'https://github.com/jayvee/brewboard-seed.git', targetDir], { encoding: 'utf8', stdio: 'inherit' });
                if (cloneResult.status !== 0) {
                    clack.log.error(`Clone failed — marking step as failed`);
                    writeStepState('seed-repo', 'failed');
                } else {
                    clack.log.success(`Cloned to ${targetDir}`);
                    seedReady = true;
                }
            }

            if (seedReady) {
                const aigonCli = process.argv[1];
                const subEnv = { ...process.env, AIGON_SKIP_FIRST_RUN: '1' };

                console.log('📦 Initializing project...');
                const initResult = spawnSync(process.execPath, [aigonCli, 'apply'], {
                    cwd: targetDir,
                    stdio: 'inherit',
                    env: subEnv,
                });
                if (initResult.status === 0) {
                    clack.log.success('Project initialized');
                } else {
                    clack.log.warn('aigon apply completed with warnings');
                }

                const config = readGlobalConfig();
                writeGlobalConfig({ ...config, seedRepoPath: targetDir });

                // Register with the dashboard server (safe to call before server starts —
                // aigon server add just writes to the global config, server picks it up on start).
                spawnSync(process.execPath, [aigonCli, 'server', 'add', targetDir], {
                    stdio: 'ignore',
                    env: subEnv,
                });
                clack.log.success(`Registered with dashboard: ${targetDir}`);

                clack.note(`cd ${targetDir}`, 'Next step');
                writeStepState('seed-repo', 'done');
            }
        } else {
            writeStepState('seed-repo', 'skipped');
        }
    }

    // ── Step 6: repos ────────────────────────────────────────────────────────
    if (shouldRunStep('repos', startStep, state, { resumeFlag, selectedSteps })) {
        let selectedRepos = [];
        const envScanDir = process.env.AIGON_ONBOARDING_REPO_SCAN_DIR;
        const envSelectAll = process.env.AIGON_ONBOARDING_REPO_SELECT_ALL === '1';

        if (yesFlag && !envScanDir) {
            writeStepState('repos', 'skipped');
        } else {
            let doScan = !!envScanDir;
            if (!yesFlag && !envScanDir) {
                const confirm = await clack.confirm({
                    message: 'Add existing Git repos to the Aigon dashboard?',
                    initialValue: true,
                });
                if (clack.isCancel(confirm)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                doScan = confirm;
            }

            if (!doScan) {
                writeStepState('repos', 'skipped');
            } else {
                const defaultScanDir = envScanDir ? path.resolve(expandHome(envScanDir)) : getDefaultRepoScanDir();
                let scanDir = defaultScanDir;
                if (!yesFlag) {
                    const dirAnswer = await clack.text({
                        message: 'Scan which folder for Git repos?',
                        placeholder: defaultScanDir,
                        defaultValue: defaultScanDir,
                    });
                    if (clack.isCancel(dirAnswer)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    scanDir = path.resolve(expandHome(dirAnswer || defaultScanDir));
                }

                const repos = discoverGitRepos(scanDir);
                if (repos.length === 0) {
                    clack.log.info(`No direct Git repos found under ${scanDir}`);
                    writeStepState('repos', 'skipped');
                } else if (yesFlag || envSelectAll) {
                    selectedRepos = repos;
                } else {
                    const choices = await clack.multiselect({
                        message: `Found ${repos.length} Git repo${repos.length === 1 ? '' : 's'} under ${scanDir}. Register with the dashboard?`,
                        options: repos.map(repoPath => ({
                            value: repoPath,
                            label: path.basename(repoPath),
                            hint: repoPath,
                        })),
                        initialValues: repos,
                        required: false,
                    });
                    if (clack.isCancel(choices)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    selectedRepos = choices || [];
                }

                if (selectedRepos.length === 0) {
                    writeStepState('repos', 'skipped');
                } else {
                    const aigonCli = process.argv[1];
                    const subEnv = { ...process.env, AIGON_SKIP_FIRST_RUN: '1' };
                    let registered = 0;
                    for (const repoPath of selectedRepos) {
                        const result = spawnSync(process.execPath, [aigonCli, 'server', 'add', repoPath], {
                            stdio: 'ignore',
                            env: subEnv,
                        });
                        if (result.status === 0) registered++;
                    }

                    const uninitialized = selectedRepos.filter(repoPath => !isAigonInitialized(repoPath));
                    let doInit = false;
                    if (uninitialized.length > 0 && !yesFlag) {
                        const initConfirm = await clack.confirm({
                            message: `${uninitialized.length} selected repo${uninitialized.length === 1 ? ' is' : 's are'} not initialized for Aigon. Initialize now?`,
                            initialValue: true,
                        });
                        if (clack.isCancel(initConfirm)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                        doInit = initConfirm;
                    }
                    if (doInit) {
                        for (const repoPath of uninitialized) {
                            clack.log.info(`Applying Aigon into ${path.basename(repoPath)}…`);
                            spawnSync(process.execPath, [aigonCli, 'apply'], {
                                cwd: repoPath,
                                stdio: 'inherit',
                                env: subEnv,
                            });
                        }
                    }

                    clack.log.success(`Registered ${registered} repo${registered === 1 ? '' : 's'} with the dashboard.`);
                    writeStepState('repos', registered > 0 ? 'done' : 'skipped');
                }
            }
        }
    }

    // ── Step 7: server ───────────────────────────────────────────────────────
    if (shouldRunStep('server', startStep, state, { resumeFlag, selectedSteps })) {
        let doStart = false;
        if (!yesFlag) {
            const confirm = await clack.confirm({
                message: 'Start the Aigon dashboard server now?',
                initialValue: true,
            });
            if (clack.isCancel(confirm)) { clack.cancel('Setup cancelled.'); process.exit(0); }
            doStart = confirm;
        }

        if (doStart) {
            const aigonCli = process.argv[1];
            const result = spawnSync(process.execPath, [aigonCli, 'server', 'start', '--persistent'], { stdio: 'inherit' });
            const healthy = result.status === 0 && await waitForDashboardHealth();
            if (healthy) {
                clack.log.success('Persistent dashboard started — open http://localhost:4100 in your browser.');
                writeStepState('server', 'done');
            } else {
                clack.log.error('Dashboard health check failed. Run `aigon server status` and inspect ~/.aigon/dashboard.log.');
                writeStepState('server', 'failed');
            }
        } else {
            writeStepState('server', 'skipped');
        }
    }

    // ── Step 8: demo ─────────────────────────────────────────────────────────
    if (shouldRunStep('demo', startStep, state, { resumeFlag, selectedSteps })) {
        const brewboardPath = readGlobalConfig().seedRepoPath || path.join(os.homedir(), 'src', 'brewboard');
        const backlogDir = path.join(brewboardPath, 'docs', 'specs', 'features', STAGE_FOLDERS.BACKLOG);
        const persistedSteps = readOnboardingState().steps || {};

        if (yesFlag) {
            writeStepState('demo', 'skipped');
        } else if (persistedSteps['seed-repo'] === 'skipped') {
            clack.log.info('Skipping demo — Brewboard not available');
            writeStepState('demo', 'skipped');
        } else if (!fs.existsSync(brewboardPath) || !fs.existsSync(backlogDir)) {
            clack.log.info('Skipping demo — Brewboard not available');
            writeStepState('demo', 'skipped');
        } else {
            const launchableAgent = require('../agent-registry').getLaunchableAgents().find(agent => agent.cli && agent.cli.command && spawnSync(agent.cli.command, ['--version'], { stdio: 'pipe' }).status === 0);

            if (!launchableAgent) {
                clack.log.info('Skipping demo — no agent CLI installed');
                writeStepState('demo', 'skipped');
            } else {
                const confirm = await clack.confirm({
                    message: 'Run a demo feature on Brewboard to see Aigon in action? (~2 min)',
                    initialValue: false,
                });
                if (clack.isCancel(confirm)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                if (!confirm) {
                    writeStepState('demo', 'skipped');
                } else {
                    let files;
                    try {
                        files = fs.readdirSync(backlogDir).filter(f => f.endsWith('.md'));
                    } catch {
                        files = [];
                    }
                    if (files.length === 0) {
                        clack.log.info('Skipping demo — no backlog features in Brewboard');
                        writeStepState('demo', 'skipped');
                    } else {
                        const firstSpec = files[0];
                        const match = firstSpec.match(/feature-(\d+)/);
                        if (!match) {
                            clack.log.warn('Demo start failed — continuing setup.');
                            writeStepState('demo', 'skipped');
                        } else {
                            const featureId = match[1];
                            const aigonCli = process.argv[1];
                            clack.log.info('Starting demo feature… watch the terminal open.');
                            const result = spawnSync(process.execPath, [aigonCli, 'feature-start', featureId, launchableAgent.id], {
                                cwd: brewboardPath,
                                stdio: 'inherit',
                                env: process.env,
                            });
                            if (result.status === 0) {
                                clack.log.success('Demo feature running!');
                                clack.note(`A supervised feature was started. Watch it: cd ${brewboardPath} && aigon feature-open ${featureId}`, 'Demo');
                                writeStepState('demo', 'done');
                            } else {
                                clack.log.warn('Demo start failed — continuing setup.');
                                writeStepState('demo', 'failed');
                            }
                        }
                    }
                }
            }
        }
    }

    // ── Step 9: vault ────────────────────────────────────────────────────────
    // Vault setup drives the in-repo backup engine (F693 merged it from Pro).
    // The wizard's remaining Pro step is F695's to remove.
    if (shouldRunStep('vault', startStep, state, { resumeFlag, selectedSteps })) {
        const backup = require('../backup');
        {
        const existingRemote = backup.getRemote();
        if (existingRemote) {
            clack.note(`Already configured: ${existingRemote}`, 'Backup');
            writeStepState('vault', 'done');
        } else {
            let doVault = false;
            if (!yesFlag) {
                const ans = await clack.confirm({
                    message: 'Set up aigon-vault to back up your data? (Recommended)',
                    initialValue: true,
                });
                if (clack.isCancel(ans)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                doVault = ans;
            }
            if (!doVault) {
                clack.note('You can set this up later with: aigon backup configure', 'Backup skipped');
                writeStepState('vault', 'skipped');
            } else {
                const { execSync } = require('child_process');
                const ghAvailable = (() => { try { execSync('gh --version', { stdio: 'pipe' }); return true; } catch { return false; } })();
                const ghAuthed = ghAvailable && (() => { try { execSync('gh auth status', { stdio: 'pipe' }); return true; } catch { return false; } })();
                let remote = null;
                if (ghAuthed) {
                    const useGh = await clack.confirm({
                        message: 'Create a private aigon-vault repo on GitHub now?',
                        initialValue: true,
                    });
                    if (clack.isCancel(useGh)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    if (useGh) {
                        const nameAns = await clack.text({
                            message: 'Repo name',
                            placeholder: 'aigon-vault',
                            defaultValue: 'aigon-vault',
                        });
                        if (clack.isCancel(nameAns)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                        const name = String(nameAns || 'aigon-vault').trim() || 'aigon-vault';
                        const spin = clack.spinner();
                        spin.start(`Creating ${name} on GitHub…`);
                        try {
                            remote = backup.createVaultOnGitHub(name);
                            spin.stop(`✅ Created ${remote}`);
                        } catch (e) {
                            spin.stop(`❌ ${e.message}`);
                        }
                    }
                }
                if (!remote) {
                    const url = await clack.text({
                        message: 'Git URL for the vault repo (leave empty to skip)',
                        placeholder: 'git@github.com:you/aigon-vault.git',
                    });
                    if (clack.isCancel(url)) { clack.cancel('Setup cancelled.'); process.exit(0); }
                    remote = url ? String(url).trim() : null;
                }
                if (!remote) {
                    clack.note('You can set this up later with: aigon backup configure', 'Backup skipped');
                    writeStepState('vault', 'skipped');
                } else {
                    try {
                        backup.configure(remote);
                        const spin = clack.spinner();
                        spin.start('Running initial backup push…');
                        try {
                            const result = backup.push();
                            spin.stop(`✅ Initial push: ${result.fileCount || 0} files`);
                        } catch (e) {
                            spin.stop(`⚠️  Initial push failed: ${e.message}`);
                        }
                        writeStepState('vault', 'done');
                    } catch (e) {
                        clack.log.warn(`Vault configure failed: ${e.message}`);
                        writeStepState('vault', 'skipped');
                    }
                }
            }
        }
        }
    }

    // ── Step 8: done ─────────────────────────────────────────────────────────
    if (selectedSteps.length === 0 && isOnboardingComplete(readOnboardingState())) markComplete();

    const finalState = readOnboardingState();
    const wizardComplete = isOnboardingComplete(finalState);

    // Set onboarded: true only after every step is done or intentionally skipped.
    if (wizardComplete) {
        const cfg = require('../config');
        const { existsSync, readFileSync } = require('fs');
        let rawConfig = {};
        if (existsSync(cfg.GLOBAL_CONFIG_PATH)) {
            try { rawConfig = JSON.parse(readFileSync(cfg.GLOBAL_CONFIG_PATH, 'utf8')); } catch (_) {}
        }
        cfg.saveGlobalConfig(Object.assign({}, rawConfig, {
            machineId: rawConfig.machineId || cfg.getDefaultMachineId(),
            onboarded: true,
            repos: rawConfig.repos || [],
        }));
    }

    const summary = STEP_IDS.map(id => {
        const status = finalState.steps && finalState.steps[id];
        const icon = status === 'done' ? '✅' : status === 'failed' ? '❌' : '⏭️ ';
        const suffix = status === 'skipped' ? ' (skipped)' : status === 'failed' ? ' (failed)' : '';
        return `${icon} ${id}${suffix}`;
    }).join('\n');
    clack.note(summary, 'Setup complete');
    clack.outro('✅ All done! Run `aigon board` to see your project, or open http://localhost:4100 in your browser.');
}

function shouldRunStep(stepId, startStep, state, options) {
    if (options.selectedSteps.length > 0) return shouldRunPersistedStep(stepId, { state, selectedSteps: options.selectedSteps });
    if (options.resumeFlag && STEP_IDS.indexOf(stepId) < STEP_IDS.indexOf(startStep)) return false;
    return shouldRunPersistedStep(stepId, { state, resume: options.resumeFlag });
}

module.exports = { runWizard, parseSelectedSteps, waitForDashboardHealth };
