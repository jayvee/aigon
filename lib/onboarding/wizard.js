...
function shouldRunStep(stepId, startStep, state, options) {
    ...
    // A startStep that is no longer a known step id (e.g. a pre-F695 state file
    // resumed mid-'pro') yields indexOf === -1, which would make every step
    // compare as "at or after start" and silently re-run the whole wizard.
    // Fall through to the persisted-status check instead.
    const startIndex = STEP_IDS.indexOf(startStep);
    if (options.resumeFlag && startIndex >= 0 && STEP_IDS.indexOf(stepId) < startIndex) return false;
    return shouldRunPersistedStep(stepId, { state, resume: options.resumeFlag });
}

module.exports = { runWizard, parseSelectedSteps, waitForDashboardHealth, shouldRunStep };
