...
async function runWizard(args = []) {
    ...
    if (shouldRunStep('agents', startStep, state)) {
        ...
        if (agentDetectors.length === 0) {
            ...
            if (yesFlag) {
                ...
                // Pre-select only the missing ones so the default action is "install
                // what isn't installed yet". User can deselect or add reinstalls.
                const initialValues = agentDetectors
                    .filter(d => !(verifiedById[d.id] && verifiedById[d.id].found))
                    .map(d => d.id);
    ...
    // ── Step 4: pro ──────────────────────────────────────────────────────────
    if (shouldRunStep('pro', startStep, state)) {
