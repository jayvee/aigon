...
async function runWizard(args = []) {
    ...
    if (shouldRunStep('server', startStep, state)) {
        ...
        if (doStart) {
            ...
            // Register the brewboard repo if the seed-repo step completed
            const finalStateNow = readOnboardingState();
            if (finalStateNow.steps && finalStateNow.steps['seed-repo'] === 'done') {
                const brewboardPath = path.join(os.homedir(), 'src', 'brewboard');
                if (require('fs').existsSync(brewboardPath)) {
                    spawnSync(process.execPath, [aigonCli, 'server', 'add', brewboardPath], {
                        stdio: 'ignore',
                        env: { ...process.env, AIGON_SKIP_FIRST_RUN: '1' },
                    });
                    clack.log.success(`Registered project: ${brewboardPath}`);
                }
            }
