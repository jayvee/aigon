...
// F695 removed the Pro step. Existing ~/.aigon/onboarding-state.json files may
// still carry a `steps.pro` entry; every reader below iterates STEP_IDS, so an
// unknown key in the persisted state is ignored rather than blocking completion.
const STEP_IDS = ['prereqs', 'terminal', 'agents', 'seed-repo', 'repos', 'server', 'demo', 'vault'];
