...
function checkoutSyncBranch(helper, { hadRemote }) {
    // Resolve to a clean working state on the sync branch.
    ...
    if (hadRemote && remoteRef.ok) {
        ...
        // Wipe any inherited working tree (none if helper is fresh, but safe).
...
function push() {
    ...
    if (!pushResult.ok) {
        // Possible non-fast-forward: surface as a clear conflict for the user.
...
function pull() {
    ...
    // Fast-forward checkout to remote tip.
    checkoutSyncBranch(helper, { hadRemote: true });
...
function isFeatureSuspended(repoPath, featureId, opts = {}) {
    ...
    const snapshotPath = path.join(repoPath, '.aigon', 'workflows', 'features', numericId, 'snapshot.json');
