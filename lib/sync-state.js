...
function helperRepoPath() {
    return path.join(repoRoot(), HELPER_REPO_REL);
}

function metaPath() {
    return path.join(repoRoot(), META_REL);
}
...
function ensureSyncIgnore() {
    const p = path.join(repoRoot(), SYNC_IGNORE_REL);
    if (fs.existsSync(p)) return false;
    fs.mkdirSync(path.dirname(p), { recursive: true });
    fs.writeFileSync(p, DEFAULT_SYNCIGNORE, 'utf8');
    return true;
}
...
function patternToRegex(pattern) {
    // Anchored, posix-only; supports trailing `/` for directories and `*` glob.
...
function makeSyncIgnoreMatcher(patterns) {
    ...
    return function isIgnored(relPath) {
        ...
        for (const re of regexes) {
            ...
            // Match any segment of the path against the pattern (gitignore-style)
...
function ensureHelperRepo(remoteUrl) {
    ...
    if (!fs.existsSync(path.join(helper, '.git'))) {
        git(helper, ['init', '--quiet']);
        git(helper, ['config', 'user.email', 'aigon-sync@local']);
        git(helper, ['config', 'user.name', 'aigon-sync']);
    }
    if (remoteUrl) {
        const current = git(helper, ['remote', 'get-url', 'origin'], { allowFail: true });
        if (!current.ok) {
            git(helper, ['remote', 'add', 'origin', remoteUrl]);
        } else if (current.stdout !== remoteUrl) {
            git(helper, ['remote', 'set-url', 'origin', remoteUrl]);
        }
    }
    return helper;
}

function fetchSyncBranch(helper) {
    const r = git(helper, ['fetch', 'origin', SYNC_BRANCH], { allowFail: true });
    return r.ok;
}
 ...
 * @param {boolean} [opts.clearWorkingTree=true] - true for push (we'll repopulate
 *   from the live .aigon/), false for pull (we want the synced files in place).
 */
function checkoutSyncBranch(helper, { hadRemote, clearWorkingTree = true }) {
    const localBranchExists = git(helper, ['rev-parse', '--verify', SYNC_BRANCH], { allowFail: true }).ok;
    const remoteRef = git(helper, ['rev-parse', '--verify', `origin/${SYNC_BRANCH}`], { allowFail: true });

    if (hadRemote && remoteRef.ok) {
        if (localBranchExists) {
            git(helper, ['checkout', '--quiet', SYNC_BRANCH]);
            git(helper, ['reset', '--hard', `origin/${SYNC_BRANCH}`]);
        } else {
            git(helper, ['checkout', '--quiet', '-B', SYNC_BRANCH, `origin/${SYNC_BRANCH}`]);
        }
    } else if (localBranchExists) {
        git(helper, ['checkout', '--quiet', SYNC_BRANCH]);
    } else {
        git(helper, ['checkout', '--quiet', '--orphan', SYNC_BRANCH]);
        const r = git(helper, ['rm', '-rf', '--cached', '.'], { allowFail: true });
        void r;
    }
    if (clearWorkingTree) {
        for (const ent of fs.readdirSync(helper)) {
            if (ent === '.git') continue;
            fs.rmSync(path.join(helper, ent), { recursive: true, force: true });
        }
    }
}
...
function push() {
    ...
    if (hadRemote) {
        ...
        if (remoteHead.ok && remoteHead.stdout && meta.lastSyncedRemoteSha
            && meta.lastSyncedRemoteSha !== remoteHead.stdout) {
            const e = new Error(
                `Sync remote has new commits (was ${meta.lastSyncedRemoteSha.slice(0, 8)}, now ${remoteHead.stdout.slice(0, 8)}).\n` +
