 ...
 * --models cx=y`). Legacy `agentId:value` pairs are accepted for old
 * autonomous payloads. An entry whose value is "none" or empty is treated as
 * an explicit null (clears any workflow-stage default). Unknown/garbled pairs
...
function parseAgentOverrideMap(raw) {
    ...
    for (const entry of values) {
        ...
        for (const pair of entry.split(',')) {
            const delimiter = pair.includes('=') ? '=' : (pair.includes(':') ? ':' : null);
            if (!delimiter) continue;
            const [rawKey, ...rest] = pair.split(delimiter);
...
function parseFrontMatter(content) {
    ...
    if (typeof data.depends_on === 'string' && data.depends_on.trim()) {
        data.depends_on = [data.depends_on.trim()];
    }

    if (data.research !== undefined && data.research !== null && data.research !== '') {
        const raw = Array.isArray(data.research) ? data.research : [data.research];
        const ids = raw
        ...
        if (ids.length === 0) {
            ...
            data.research = [...new Set(ids)];
        }
    } else {
        delete data.research;
    }

    if (data.planning_context !== undefined && data.planning_context !== null && data.planning_context !== '') {
        const raw = Array.isArray(data.planning_context) ? data.planning_context : [data.planning_context];
        const paths = raw.map(v => String(v).trim()).filter(v => v.length > 0);
        if (paths.length === 0) {
            delete data.planning_context;
        } else {
            data.planning_context = paths;
        }
    } else {
        delete data.planning_context;
    }

    if (data.feedback_refs !== undefined && data.feedback_refs !== null && data.feedback_refs !== '') {
        const raw = Array.isArray(data.feedback_refs) ? data.feedback_refs : [data.feedback_refs];
        const refs = raw.map(v => String(v).trim()).filter(v => v.length > 0);
        if (refs.length === 0) {
            delete data.feedback_refs;
        } else {
            data.feedback_refs = refs;
        }
    } else {
        delete data.feedback_refs;
    }
