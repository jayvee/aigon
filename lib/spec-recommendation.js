...
function rankAgentsForOperation(op, complexity, opts = {}) {
    ...
    let perTriplet = {};
    try {
        const cached = statsAggregate.readCachedAggregate(repoPath || process.cwd());
        perTriplet = (cached && cached.perTriplet) || {};
    } catch (_) {}

    const candidates = [];

    const repoRoot = repoPath || process.cwd();
    const getAvail = require('./agent-availability').getAgentAvailability;
    for (const agent of agentRegistry.getAllAgents()) {
        const avail = getAvail(agent.id, repoRoot, { forRecommendation: true });
        if (!avail.recommended) continue;
