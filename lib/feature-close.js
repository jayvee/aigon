...
async function recordCriteriaAttestationFailure(repoPath, featureId, payload) {
    const at = new Date().toISOString();
    const gateFailedEvent = {
        type: 'feature.close_gate_failed',
        featureId: String(featureId),
        gateKind: 'criteria-attestation',
        unattested: Array.isArray(payload.unattested) ? payload.unattested : [],
        invalid: Array.isArray(payload.invalid) ? payload.invalid : [],
        noVerifyCriteria: Boolean(payload.noVerifyCriteria),
...
async function runPostMergeGatePhase(target, { loadProjectConfig, resolveDeployCommand }) {
    ...
    if (gateSpec.mode === 'disabled') {
        if (gateSpec.explicit) {
            ...
            console.warn('⚠️  Post-merge gate skipped: featureClose.postMergeGate is not configured.');
            console.warn('   Set featureClose.postMergeGate in .aigon/config.json to verify merged main before close.');
        }
    ...
    await recordPostMergeGateFailure(target.repoPath, target.name, {
        ...
        mergedCommitSha: getMergedHeadSha(target.repoPath),
...
async function recordCriteriaAttestationBypass(repoPath, featureId) {
    await wf.persistEntityEvents(repoPath, 'feature', String(featureId), [{
...
async function recordCriteriaAttested(repoPath, featureId, payload) {
    await wf.persistEntityEvents(repoPath, 'feature', String(featureId), [{
...
async function runCriteriaAttestationPhase(target, deps) {
    const {
        findFile,
        PATHS,
        noVerifyCriteria = false,
    } = deps;
    ...
    const logLookupOptions = {};
    if (target.worktreePath && fs.existsSync(target.worktreePath)) {
        logLookupOptions.worktreePath = target.worktreePath;
    }
    const snapshot = await wf.showFeatureOrNull(target.repoPath, target.name);
    const validation = validateCriteriaAttestation(specPath, target.repoPath, target.name, {
        ...logLookupOptions,
        snapshot,
    });
    ...
    if (noVerifyCriteria) {
        console.warn('');
        console.warn('⚠️  CRITERIA ATTESTATION BYPASSED (--no-verify-criteria)');
        console.warn('   Acceptance criteria were not checked against the implementation log.');
        console.warn('   This bypass is recorded in the workflow event log.');
        console.warn('');
        await recordCriteriaAttestationBypass(target.repoPath, target.name);
        return { ok: true, bypassed: true };
    }

    if (!validation.ok) {
        const returnSpecState = isCriteriaAttestationRetry(snapshot)
            ? ((snapshot.closeRecovery && snapshot.closeRecovery.returnSpecState) || 'ready')
            : (snapshot && snapshot.currentSpecState) || 'ready';
        const message = validation.error === 'missing-log'
            ? [
                'Criteria attestation requires an implementation log but none was found.',
                'Add docs/specs/features/logs/feature-<ID>-*-log.md with a ## Criteria Attestation section,',
                'or use solo-branch mode only when the spec has no acceptance criteria.',
                'Emergency bypass: aigon feature-close <ID> --no-verify-criteria',
            ].join('\n')
            : formatCriteriaAttestationFailureMessage(
                validation.criteria,
        ...
        await recordCriteriaAttestationFailure(target.repoPath, target.name, {
            unattested: validation.unattested,
            invalid: validation.invalid,
            noVerifyCriteria: false,
            outputTail: message,
            returnSpecState: returnSpecState === 'close_recovery_in_progress' ? 'ready' : returnSpecState,
            alreadyInRecovery: isCriteriaAttestationRetry(snapshot) || isInCloseRecovery(snapshot),
        });

        console.error(`❌ ${message}`);
        return { ok: false, error: 'Criteria attestation incomplete' };
    }

    await syncCriteriaDeferredEscalations(target.repoPath, target.name, validation, wf);
    const attestedPayload = buildCriteriaAttestedPayload(validation);
    await recordCriteriaAttested(target.repoPath, target.name, attestedPayload);

    if (isInCloseRecovery(snapshot) && snapshot.lastCloseFailure?.kind === 'criteria-attestation') {
        await wf.recordCloseRecoveryEnded(target.repoPath, target.name, {
            source: 'criteria-attestation-passed',
        });
    }

    const { met, deferred, dropped, total } = validation.counts;
    console.log(`✅ Criteria attestation recorded (${met} met, ${deferred} deferred, ${dropped} dropped / ${total})`);
    if (deferred > 0) {
        console.warn(`⚠️  ${deferred} deferred criterion(s) raised spec-shortfall escalations — disposition before close completes.`);
    }

    return { ok: true, validation, attestedPayload };
}
...
function warnUnrelatedWorkingTreeChanges(dirtyStatus, defaultBranch) {
    const paths = parseGitStatusPaths(dirtyStatus);
...
function commitSpecMove(target, engineResult, { PATHS, findFile, runGit }) {
    ...
    if (preCloseSpecPath) stagedPaths.push(preCloseSpecPath);
    ...
    const stuckSpec = findFile(PATHS.features, num, [STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.PAUSED]);
    if (stuckSpec) {
...
async function run(args, deps) {
    ...
    // Phase 4.84: Criteria attestation (before escalation guard — deferred raises escalations).
    if (!skipMerge) {
        const criteriaResult = await close.runCriteriaAttestationPhase(target, {
            findFile,
            PATHS,
            noVerifyCriteria: args.includes('--no-verify-criteria'),
        });
    ...
    // Phase 4.85: Open review escalations (before merge — operator may reopen).
    if (!skipMerge) {
        const { runEscalationCloseGuard } = require('./feature-escalation');
        const escalationResult = await runEscalationCloseGuard(target.repoPath, target.name);
