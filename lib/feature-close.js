...
// Pop the auto-stash created during feature-close. On conflict, git leaves
// the conflict markers in the working tree and exits non-zero — but the stash
// itself stays on the stack as a safety net. If we swallow that error (as this
// used to do), the repo is left with `<<<<<<<` markers in source files, which
// will break the aigon CLI on its next invocation (lib/ files stop parsing).
// So: detect the conflict, report the exact files, give accurate recovery
// commands, and flag the result so the caller can re-surface the warning at
// end of run. Keep the close itself successful — the merge already landed.
function restoreAutoStash(phaseLabel) {
    try {
        ...
        const oursList = conflicted.map(f => `"${f}"`).join(' ');

        console.warn('');
        console.warn('⚠️  ─────────────────────────────────────────────────────────────');
        console.warn(`⚠️  Stash pop had CONFLICTS after ${phaseLabel}.`);
        console.warn('⚠️  Your working tree contains `<<<<<<<` markers.');
        console.warn('⚠️  The aigon CLI will FAIL to load until these are resolved.');
        console.warn('⚠️');
        console.warn('⚠️  Conflicted files:');
        console.warn(fileList);
        console.warn('⚠️');
        console.warn('⚠️  To recover — pick one per file:');
        console.warn(`⚠️     git checkout --ours   ${oursList}   # keep main-branch version`);
        console.warn(`⚠️     git checkout --theirs ${oursList}   # keep stashed (your WIP) version`);
        console.warn('⚠️     # or open files and resolve manually');
        console.warn('⚠️  Then drop the stash once you\'re satisfied:');
        console.warn('⚠️     git stash drop');
        console.warn('⚠️  ─────────────────────────────────────────────────────────────');
        console.warn('');
