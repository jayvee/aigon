...
function snapshotFinalStats(target, { getDefaultBranch, preMergeBaseRef }) {
    ...
    try {
        ...
        try {
            const countStr = execSync( // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
        ...
        if (commitCount > 0) {
            try {
                lastCommitAt = execSync( // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
                    `git log ${branchName} -1 --format=%aI`,
                    { encoding: 'utf8', cwd, stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000 }
                ).trim() || null;
            } catch (_) {}
            try {
                lastCommitMessage = execSync( // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
                    `git log ${branchName} -1 --format=%s`,
                    { encoding: 'utf8', cwd, stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000 }
                ).trim() || null;
            } catch (_) {}
            try {
                const diffStat = execSync( // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
                    `git diff --numstat ${baseRef}..${branchName}`,
