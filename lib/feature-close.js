...
function mergeFeatureBranch(target, { getDefaultBranch, runGit }) {
    ...
    try {
        ...
        if (overwrittenMatch) {
            ...
            // Auto-resolve conflicts in non-code files (logs, settings, configs)
            // Log files: take theirs (worktree/implementation version has the real content)
            // Settings files: take ours (main's version, already reset before stash)
            try {
                const unmerged = execSync('git diff --name-only --diff-filter=U', { encoding: 'utf8' }).trim();
                if (unmerged) {
                    const conflictFiles = unmerged.split('\n').filter(Boolean);
                    const autoResolvable = conflictFiles.every(f =>
                        f.endsWith('-log.md') || f.endsWith('settings.json') ||
                        f.startsWith('.claude/') || f.startsWith('.gemini/') ||
                        f.startsWith('.aigon/')
                    );
                    if (autoResolvable) {
                        for (const f of conflictFiles) {
                            const strategy = f.endsWith('-log.md') ? '--theirs' : '--ours';
                            try {
                                execSync(`git checkout ${strategy} "${f}"`, { stdio: 'pipe' });
                        ...
                        console.log(`✅ Merged branch: ${branchName} (auto-resolved ${conflictFiles.length} conflict(s))`);
                    } else {
                        if (didStash) try { execSync('git stash pop', { stdio: 'pipe' }); } catch (_) {}
                        return { ok: false, error: `❌ Merge failed with code conflicts. Resolve manually:\n   ${conflictFiles.join('\n   ')}` };
                    }
                } else {
                    if (didStash) try { execSync('git stash pop', { stdio: 'pipe' }); } catch (_) {}
                    return { ok: false, error: `❌ Merge failed. You may need to resolve conflicts manually.` };
