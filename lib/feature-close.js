...
async function runCriteriaAttestationPhase(target, deps) {
    ...
    if (!validation.ok) {
        const message = validation.error === 'missing-log'
            ? 'Criteria attestation: no implementation log found (optional — close proceeds).'
            : formatCriteriaAttestationFailureMessage(
                validation.criteria,
        ...
        console.warn('');
        console.warn('⚠️  Criteria attestation incomplete — close is not blocked (advisory only).');
        for (const line of message.split('\n')) {
            console.warn(`   ${line}`);
        }
        console.warn('');
        if (isCriteriaAttestationRetry(snapshot)) {
            await wf.recordCloseRecoveryEnded(target.repoPath, target.name, {
                source: 'criteria-attestation-advisory',
            });
        }
        return { ok: true, advisory: true, incomplete: true, validation };
    }
