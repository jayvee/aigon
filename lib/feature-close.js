...
async function runPostMergeGatePhase(target, { loadProjectConfig, resolveDeployCommand }) {
    ...
    await recordPostMergeGateFailure(target.repoPath, target.name, {
        ...
        alreadyInRecovery: isPostMergeGateRetry(snapshot),
    });
...
async function runPreauthValidationPhase(target, deps) {
    ...
    if (!validation.ok) {
        ...
        await recordPreauthValidationFailure(target.repoPath, target.name, {
            ...
            outputTail: message,
            returnSpecState: returnSpecState === 'close_recovery_in_progress' ? 'ready' : returnSpecState,
            alreadyInRecovery: isPreauthValidationRetry(snapshot),
        });
...
async function runCriteriaAttestationPhase(target, deps) {
    ...
    if (!validation.ok) {
        ...
        await recordCriteriaAttestationFailure(target.repoPath, target.name, {
            ...
            outputTail: message,
            returnSpecState: returnSpecState === 'close_recovery_in_progress' ? 'ready' : returnSpecState,
            alreadyInRecovery: isCriteriaAttestationRetry(snapshot),
        });
