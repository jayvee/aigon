...
// Agents with discoverable session storage
const CAPTURABLE_AGENTS = new Set(['cc', 'gg', 'cx']);
...
function spawnCaptureProcess(sessionName, repoPath, worktreePath, agentId, createdAt) {
    if (!CAPTURABLE_AGENTS.has(agentId)) return;
    const args = [
...
function resolveResumeArgs(agentId, agentSessionId) {
    ...
    if (agentId === 'cc') {
        return { prependArgs: [], appendArgs: ['--resume', agentSessionId], isSubcommand: false };
    }
    if (agentId === 'cx') {
        return { prependArgs: ['resume', agentSessionId], appendArgs: [], isSubcommand: true };
    }
    if (agentId === 'gg') {
        return { prependArgs: [], appendArgs: ['--resume', agentSessionId], isSubcommand: false };
    }
