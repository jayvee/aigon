...
const { resolveClaudeProjectDir, resolveGeminiChatsDir, findCodexSessionFiles, resolveAntigravityConversationId } = require('./telemetry');
...
const SESSION_FINDERS = {
    ...
    'antigravity-conversations'(worktreePath, threshold) {
        ...
        try {
            ...
            if (mtime < threshold) return null;
        } catch (_) { return null; }
        return { sessionId: conversationId, sessionPath: dbPath };
    },
};
...
function updateSessionSidecar(sessionName, repoPath, patch) {
    ...
    const current = readJsonSafe(sidecarPath, {});
    const updated = Object.assign({}, current, patch);
    writeJsonAtomic(sidecarPath, updated);
}
...
function readLatestSidecarWithSession(repoPath, entityType, entityId, agentId) {
    ...
    for (const f of entries) {
        ...
        try {
            const raw = readJsonSafe(path.join(dir, f));
