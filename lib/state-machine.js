...
const FEATURE_TRANSITIONS = [
    ...
    },
    {
        ...
        action: 'feature-resume',
        guard: () => true,
        label: 'Resume to Inbox',
        uiTrigger: 'Drag to Inbox'
    }
];
...
const FEATURE_ACTIONS = [
    ...
    // implementing or submitted + session running: Attach
    {
        type: 'action',
        stage: 'in-progress',
        ...
        guard: (ctx, agentId) => {
            const status = (ctx.agentStatuses || {})[agentId];
            return (status === 'implementing' || status === 'submitted') &&
                (ctx.tmuxSessionStates || {})[agentId] === 'running';
        },
        label: (ctx, agentId) => `View ${agentId}`,
        perAgent: true,
        mode: 'terminal'
    },
    // implementing + no session (e.g. warp/vscode): Start
    {
        type: 'action',
        stage: 'in-progress',
        action: 'feature-open',
    ...
    // idle/undefined agent (set up but never started, or session ended): Start
    {
        type: 'action',
        stage: 'in-progress',
        action: 'feature-open',
        guard: (ctx, agentId) => {
            ...
            return s === 'idle' || s === 'error' || s === undefined;
        },
        label: (ctx, agentId) => `Start ${agentId}`,
        perAgent: true,
        mode: 'terminal'
    ...
    // in-progress: autopilot (only when no agents are running)
    {
        type: 'action',
        stage: 'in-progress',
        action: 'feature-autopilot',
...
const RESEARCH_ACTIONS = [
    ...
    // implementing or submitted + running session: attach
    {
        type: 'action',
        stage: 'in-progress',
    ...
    // implementing + no session (ended/crashed): restart
    {
        type: 'action',
        stage: 'in-progress',
        action: 'research-open',
...
/**
 * Returns true if a notification of the given type should fire for this entity.
 * Uses getAvailableActions() as the single source of truth — no inline guard logic.
 *
 * @param {'feature'|'research'} entityType
 * @param {string} stage
 * @param {StateContext} context
 * @param {'all-submitted'} notificationType
 * @returns {boolean}
 */
function shouldNotify(entityType, stage, context, notificationType) {
    if (notificationType === 'all-submitted') {
        if (entityType === 'feature') {
            // Use getValidTransitions: feature-eval transition (in-progress → in-evaluation)
            // is guarded by isFleet && allAgentsSubmitted. Only valid from in-progress,
            // so this naturally excludes re-notification once already in in-evaluation.
            return getValidTransitions(entityType, stage, context).some(t => t.action === 'feature-eval');
        }
        if (entityType === 'research') {
            // research-eval transition (in-progress → in-evaluation) guarded by allAgentsSubmitted
...
// ---------------------------------------------------------------------------
// Outbox transition definitions
// ---------------------------------------------------------------------------

/**
 * Maps each action to its valid source stages, target stage, and side effects builder.
 * Used by requestTransition() to validate and record transitions atomically.
 */
const TRANSITION_DEFS = {
    'feature-prioritise': {
        from: ['inbox'],
        to: 'backlog',
        sideEffects: () => ['move-spec'],
    },
    'feature-start': {
        from: ['backlog'],
        to: 'in-progress',
        sideEffects: (opts) => {
            ...
            if (agents.length === 0) return ['move-spec', 'init-log'];
            const effects = ['move-spec'];
            for (const a of agents) {
                effects.push(`create-worktree-${a}`);
                effects.push(`init-log-${a}`);
            }
            return effects;
        },
    },
    ...
    'feature-close': {
        from: ['in-evaluation', 'in-progress'],
        to: 'done',
        sideEffects: () => ['move-spec'],
    },
    'feature-pause': {
        from: ['inbox', 'backlog', 'in-progress'],
        to: 'paused',
        sideEffects: () => ['move-spec'],
    },
    'feature-resume': {
    ...
    'research-start': {
        ...
        sideEffects: (opts) => {
            const agents = opts.agents || [];
            if (agents.length === 0) return ['move-spec'];
            const effects = ['move-spec'];
            for (const a of agents) {
                effects.push(`create-worktree-${a}`);
            }
...
/**
 * Derive the manifest prefix from an action name.
 * 'feature-start' → 'feature', 'research-close' → 'research'
 */
function prefixFromAction(action) {
    if (action.startsWith('research-')) return 'research';
    return 'feature';
}

/**
 * Validate and record a stage transition in the manifest, writing pending side effects
 * atomically before any filesystem operations begin.
 *
 * Callers should check for pending ops first (replay path) and only call this on
 * the normal (non-replay) path when the entity is in the expected source stage.
 *
 * @param {string|number} entityId - Feature or research ID
 * @param {string} action - e.g. 'feature-start', 'research-close'
 * @param {Object} [opts] - { agents: string[], actor: string, prefix: string }
 * @returns {string[]} Pending side effects to execute
 * @throws {Error} If action is invalid from current stage or lock cannot be acquired
 */
function requestTransition(entityId, action, opts = {}) {
    const manifest = require('./manifest');
    const def = TRANSITION_DEFS[action];
    if (!def) throw new Error(`Unknown action: ${action}`);

    const prefix = opts.prefix || prefixFromAction(action);
    const entityLabel = prefix === 'research' ? 'Research' : 'Feature';

    if (!manifest.acquireLock(entityId, prefix)) {
        throw new Error(`${entityLabel} ${entityId} is locked by another process`);
    }
    try {
        const m = manifest.readManifest(entityId, prefix);
        if (!m) {
            const priCmd = prefix === 'research' ? 'research-prioritise' : 'feature-prioritise';
            throw new Error(
                `Manifest not found for ${prefix} ${entityId}. ` +
                `Run 'aigon ${priCmd} <name>' before '${action}'.`
            );
        }
        const currentStage = m.stage;

        if (!def.from.includes(currentStage)) {
            throw new Error(
                `Invalid transition: '${action}' from stage '${currentStage}'. ` +
                `Expected: ${def.from.join(' or ')}`
            );
        }

        const agents = (opts.agents && opts.agents.length > 0) ? opts.agents : (m.agents || []);
        const pending = def.sideEffects(Object.assign({}, opts, { agents }));

        manifest.writeManifest(entityId, {
            stage: def.to,
            agents,
            pending,
        }, { type: `transition:${action}`, actor: opts.actor || 'cli' }, prefix);

        return pending;
    } finally {
        manifest.releaseLock(entityId, prefix);
    }
}

/**
 * Remove the first occurrence of a completed side effect from the manifest's pending array.
 * Safe to call even if the op is no longer in the array (already removed).
 *
 * @param {string|number} entityId - Feature or research ID
 * @param {string} op - The completed op string to remove (e.g. 'move-spec', 'create-worktree-cc')
 * @param {string} [prefix='feature'] - Entity prefix ('feature' or 'research')
 */
function completePendingOp(entityId, op, prefix = 'feature') {
    const manifest = require('./manifest');
    const m = manifest.readManifest(entityId, prefix);
    if (!m) return;
    ...
    manifest.writeManifest(entityId, { pending: newPending }, undefined, prefix);
}
