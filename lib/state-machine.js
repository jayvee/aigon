...
const RESEARCH_ACTIONS = [
    ...
    },
    {
        ...
        guard: (ctx) => allAgentsSubmitted(ctx) && !ctx.synthesizeSession,
        label: () => 'Synthesize',
        perAgent: false,
        mode: 'agent',
        priority: 'high'
...
function shouldNotify(entityType, stage, context, notificationType) {
    if (notificationType === 'all-submitted') {
        ...
        if (entityType === 'research') {
            // research-synthesize is an in-state action guarded by allAgentsSubmitted
            return getAvailableActions(entityType, stage, context).some(a => a.action === 'research-synthesize');
        }
    }
    return false;
}
 ...
 * the normal (non-replay) path when the feature is in the expected source stage.
 *
 * @param {string|number} featureId
 * @param {string} action - e.g. 'feature-start', 'feature-close'
 * @param {Object} [opts] - { agents: string[], actor: string }
 ...
 */
function requestTransition(featureId, action, opts = {}) {
    const manifest = require('./manifest');
    const def = TRANSITION_DEFS[action];
    if (!def) throw new Error(`Unknown action: ${action}`);

    if (!manifest.acquireLock(featureId)) {
        throw new Error(`Feature ${featureId} is locked by another process`);
    }
    try {
        let m = manifest.readManifest(featureId);
        if (!m) {
            // Pre-manifest feature (created before manifest system) — bootstrap from folder
            m = manifest.ensureManifest(featureId);
            if (!m || !m.stage || m.stage === 'unknown') {
                throw new Error(
                    `Manifest not found for feature ${featureId}. ` +
                    `Run 'aigon feature-prioritise <name>' before '${action}'.`
                );
            }
        }
        ...
        manifest.writeManifest(featureId, {
            stage: def.to,
            agents,
            pending,
        }, { type: `transition:${action}`, actor: opts.actor || 'cli' });

        return pending;
    } finally {
        manifest.releaseLock(featureId);
    }
}
 ...
 */
function completePendingOp(featureId, op) {
    const manifest = require('./manifest');
    const m = manifest.readManifest(featureId);
    if (!m) return;
    ...
    manifest.writeManifest(featureId, { pending: newPending });
}
