...
const { filterQuotaStateByAvailability } = require('../agent-availability');
...
module.exports = [
    ...
    {
        method: 'GET',
        // Deprecated shim — projects unified state; removal tracked in CHANGELOG.
        path: '/api/budget',
        handler(req, res, ctx) {
            try {
                const state = agentQuotaRead.readAgentQuotaState(process.cwd());
                const data = agentQuotaRead.projectBudgetApi(state, process.cwd());
                ...
                ctx.sendJson(200, { ...(data || { cc: null, cx: null, gg: null, km: null, ag: null }), lastTokenKickoffAt });
            } catch (e) {
                ctx.sendJson(200, { cc: null, cx: null, gg: null, km: null, ag: null, lastTokenKickoffAt: null });
            }
        }
    },
    {
        method: 'POST',
        // Deprecated shim — delegates to unified refresh.
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            try {
                const state = agentQuotaRead.readAgentQuotaState(process.cwd());
                const projected = agentQuotaRead.projectQuotaApi(state, process.cwd());
                benchHydrate.mergeBenchVerdictsIntoQuota(projected, process.cwd());
                ctx.sendJson(200, filterQuotaStateByAvailability(projected, process.cwd()));
            } catch (e) {
                ctx.sendJson(200, { schemaVersion: 2, agents: {}, providers: {} });
            }
        }
    },
    {
