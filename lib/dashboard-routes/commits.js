...
const PLUMBING_PATTERNS = [
    /^chore: worktree setup for /i,
    /^chore: sync feature \d+ spec to worktree/i,
];

function isPlumbingCommit(message) {
    return PLUMBING_PATTERNS.some(re => re.test(message));
}
...
async function collectFromWorktree(worktreePath) {
    ...
    const raw = await gitLog(worktreePath, `${defaultBranch}..HEAD`);
    const commits = raw.filter(c => !isPlumbingCommit(c.message));
    return { source: 'worktree', commits: await attachFiles(worktreePath, commits) };
}
...
async function collectFromMerged(repoPath, featureId) {
    ...
    const filtered = commits.filter(c => !isPlumbingCommit(c.message));
    return {
        source: 'merged',
