...
// Primary: trailer added since F484. Fallback: message pattern for commits predating the trailer.
function isInternalPlumbingCommit(cwd, fullHash, message) {
    if (/Aigon-Internal:\s*true/i.test(commitMessageBody(cwd, fullHash))) return true;
    return /^chore: worktree setup for /i.test(message) ||
           /^chore: sync feature \d+ spec to worktree/i.test(message);
}
...
function collectFromWorktree(worktreePath) {
    ...
    const commits = gitLog(worktreePath, range).filter(c => !isInternalPlumbingCommit(worktreePath, c.fullHash, c.message));
...
function collectFromMerged(repoPath, featureId) {
    ...
    const filtered = commits.filter(c => !isInternalPlumbingCommit(repoPath, c.fullHash, c.message));
    return {
        source: 'merged',
