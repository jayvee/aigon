    ...
    },
    {
        ...
        handler(req, res, ctx, match) {
            ctx.readJsonBody().then(async payload => {
                ...
                try {
                    const result = await sendNudge(path.resolve(repoPath), researchId, message, {
                        agentId,
                        role,
                        entityType: 'research',
                    });
                ...
                try {
                    if (action === 'mark-submitted') {
                        ...
                        if (!hasEvidence) {
                            ...
                            ctx.sendJson(409, { error: `Cannot mark ${agent} complete for ${entityType} ${id} without ${evidenceLabel}.` });
                            return;
                        }
                        workflowEngine.emitSignal(repoPath, id, 'agent-ready', agent, { entityType })
                            .then(() => {
                                ctx.setLatestStatus(ctx.routes.collectDashboardStatusData());
                                const successMsg = `Marked ${agent} as complete`;
                                ctx.sendJson(200, { ok: true, message: successMsg });
                            })
                    ...
                    if (action === 'switch-agent') {
                        ctx.sendJson(403, { error: 'switch-agent requires aigon-pro', proRequired: true });
                        return;
                    }
