...
// F554: session-name parsing/matching + live-session listing come from the
// agent-sessions facade so HTTP routes no longer parse tmux names directly.
const { matchTmuxSessionByEntityId, toUnpaddedId } = require('../agent-sessions/names');
const { createAgentSessionService } = require('../agent-sessions');
const { findFirstTmuxSessionByPrefix, findSpecReviewTmuxSession } = require('../dashboard-status-helpers');
...
module.exports = [
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                try {
                    const result = ctx.routes.handleDashboardAgentControl({
                        ...payload,
                        control: 'ask-session',
                        tmuxSessionExists,
                    });
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            try {
                const enriched = createAgentSessionService().listLiveSessions();
        ...
        handler(req, res, ctx, match) {
            ...
            if (!sessionName) {
                for (const entityType of ['feature', 'research']) {
                    for (const role of ['spec-review', 'spec-revise', 'spec-check']) {
                        const candidate = findSpecReviewTmuxSession(repoPath, entityType, fid, role, agentId);
                        if (candidate && tmuxSessionExists(candidate)) {
            ...
            try {
                // F554: console snapshot via the service rather than a raw capture-pane.
                const snapshot = createAgentSessionService().getConsoleSnapshot(sessionName, { lines: 200 });
                const cleaned = String(snapshot.text || '').replace(/\n{3,}/g, '\n\n').replace(/\s+$/, '\n');
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                ...
                try {
                    if (sanitized) {
                        tmuxInjectLiteral(sessionName, sanitized, { submitKey: enter ? 'Enter' : null });
                    } else if (enter) {
                        runTmux(['send-keys', '-t', sessionName, 'Enter'], { stdio: 'ignore', timeout: 2000 });
                    }
                    ctx.sendJson(200, { ok: true });
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
