...
function logStatusRequest(ctx, serializeMs, bytes, { notModified } = {}) {
    _statusReqCount += 1;
    const logFn = ctx.helpers && typeof ctx.helpers.log === 'function' ? ctx.helpers.log : null;
    if (notModified) {
        if (logFn) logFn(`[perf] /api/status 304 etag=${ctx.getStatusVersion()}`);
        return;
    }
    ...
    // feature 236. Pro registers them via lib/pro-bridge.js.
    {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then((payload) => {
                ...
                const hiddenRepos = Array.isArray(data.hiddenRepos)
                    ? data.hiddenRepos.map((value) => String(value))
                    : [];
                const { setDashboardHiddenRepos } = require('../config');
                setDashboardHiddenRepos(hiddenRepos);
                ctx.sendJson(200, { ok: true, hiddenRepos });
            }).catch(() => ctx.sendJson(400, { error: 'Invalid JSON body' }));
        }
    },
    {
        ...
        async handler(req, res, ctx) {
            ...
            try {
                let payload = {};
                try {
                    payload = await ctx.readJsonBody();
                } catch (_) {
                    payload = {};
                }
                const repoPath = payload && payload.repoPath ? String(payload.repoPath).trim() : '';
                if (repoPath) {
                    await ctx.helpers.pollRepoStatus(repoPath);
                } else {
                    await ctx.helpers.pollStatus({ force: true });
                }
                const { ifNoneMatchSatisfied } = require('../dashboard-status-version');
                const etag = String(ctx.getStatusVersion());
                ...
                ctx.sendJson(200, ctx.getLatestStatus(), { etag: `"${etag}"` });
            } catch (e) {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(async (payload) => {
                ...
                try {
                    ...
                    const consumed = close.consumeRestartMarkerFromCandidates([
                        result.repoPath,
                        dedupeRepoPath,
                        process.cwd(),
                        ...registered.map((repo) => (typeof repo === 'string' ? repo : repo && repo.path)),
                    ]);
                    restartMarker = consumed && consumed.marker ? consumed.marker : null;
                } catch (_) { /* best-effort */ }
                ...
                if (result.stderr && /^❌/.test(String(result.stderr).trim())) {
                    ...
                    if (restartMarker) {
                        ctx.helpers.log(`🔄 Lib files changed (${(restartMarker.files || []).length}) — scheduling dashboard self-restart despite stderr warning`);
                        scheduleDashboardSelfRestart({
                            broadcast: ctx.helpers.broadcastServerRestarting,
                            log: ctx.helpers.log,
                            cliEntryPath: ctx.routes.CLI_ENTRY_PATH,
                        });
                    }
                    return;
                }
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            ...
            if (ifNoneMatchSatisfied(ifNoneMatch, etag)) {
                ...
                res.end();
                logStatusRequest(ctx, 0, 0, { notModified: true });
                return;
            }
            // F590/F620: pre-serialized body (gzip negotiated in sendJsonSerialized).
            const body = ctx.getSerializedStatusBody();
            const bytes = ctx.sendJsonSerialized(200, body, { etag: `"${etag}"` });
            logStatusRequest(ctx, 0, bytes);
        }
    },
    {
