...
const { regenerateAgentInstructions } = require('../agent-instructions-regen');
...
module.exports = [
    {
        ...
        async handler(req, res, ctx, match) {
            ...
            try {
                ...
                const actionResult = await ctx.routes.runDashboardInteractiveAction({
                    action: 'dev-server',
                    args: ['start'],
                    repoPath,
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                try {
                    ...
                    if (typeof ctx.helpers.registerRepoFsWatch === 'function') {
                        ctx.helpers.registerRepoFsWatch(absPath);
                    }
                    ctx.setLatestStatus(ctx.routes.collectDashboardStatusData());
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                try {
                    ...
                    if (typeof ctx.helpers.unregisterRepoFsWatch === 'function') {
                        ctx.helpers.unregisterRepoFsWatch(repoPath);
                    }
                    ctx.setLatestStatus(ctx.routes.collectDashboardStatusData());
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                ...
                // F521: enforce scope. The schema's `scope` field is authoritative;
                // 'user' rejects project writes, 'repo' rejects global writes.
                const allowedScope = settingDef.scope || 'shared';
                const requestedScope = scope; // 'global' | 'project'
                if (allowedScope === 'user' && requestedScope !== 'global') {
                    ctx.sendJson(400, {
                        error: 'scope_violation',
                        key,
                        requestedScope,
                        allowedScope,
                        message: `Setting "${key}" is user-scope and can only be set globally; per-repo writes are rejected.`,
                    });
                    return;
                }
                if (allowedScope === 'repo' && requestedScope !== 'project') {
                    ctx.sendJson(400, {
                        error: 'scope_violation',
                        key,
                        requestedScope,
                        allowedScope,
                        message: `Setting "${key}" is repo-scope and can only be set per-project; global writes are rejected.`,
                    });
                    return;
                }
                ...
                try {
                    ...
                    // F523: if this key is baked into installed agent command
                    // files (profile, devServer.enabled, …), re-run install-agent
                    // --all in the target repo and commit. Skips silently when
                    // there's no install manifest.
                    let regen = null;
                    if (settingDef.affectsInstalledCommands) {
                        regen = regenerateAgentInstructions(repoPath);
                    }
                    ctx.sendJson(200, {
                        ok: true,
                        regenerated: !!(regen && regen.regenerated),
                        regenerateError: regen && regen.error ? regen.error : null,
                        ...ctx.routes.buildDashboardSettingsPayload(repoPath),
                    });
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
                }
    ...
    },
    {
        ...
        handler(req, res, ctx) {
            ctx.readJsonBody().then(payload => {
                ...
                const repoPathRaw = String(payload.repoPath || '').trim();
                const repoPath = scope === 'project'
                    ? ctx.helpers.resolveRequestedRepoPathOrRespond(res, repoPathRaw)
                    : process.cwd();
                ...
                try {
                    if (action === 'disable') {
                        ...
                        agentAvailability.enableAgent(agentId, {
                            scope,
                            repoPath: repoPath || process.cwd(),
                        });
                    } else {
                        ctx.sendJson(400, { error: 'action must be "disable" or "enable"' });
                        return;
                    }
                    const targetRepo = repoPath || process.cwd();
                    ctx.sendJson(200, {
                        ok: true,
                        ...ctx.routes.buildDashboardSettingsPayload(targetRepo, { globalOnly: scope === 'global' }),
                    });
                } catch (e) {
                    ctx.sendJson(400, { error: e.message });
                }
