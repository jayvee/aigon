...
function buildTokenExhaustionSignal({ slotAgentId, agentState, statusRecord, featureId, repoPath, failoverConfig, livePaneTail }) {
    ...
    // Live pane match: the supervisor passes captured tmux pane text for alive
    // agents. Most CLIs print quota messages and stay at the REPL prompt
    // instead of exiting, so the post-mortem stderrPatterns+exitCode path
    // never fires for them. Live-pane detection has no exit-code requirement
    // because the agent has not exited.
    const liveText = String(livePaneTail || '');
    const lowerLive = liveText.toLowerCase();
    const livePaneMatched = liveText.length > 0 && detectors.stderrPatterns.some(pattern => lowerLive.includes(String(pattern).toLowerCase()));
    ...
    return {
        ...
        source,
        paneTail: livePaneMatched ? liveText : paneTail,
