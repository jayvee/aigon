...
const FEATURE_ENGINE_RULES = FEATURE_ENGINE_STATES;
...
function renderStateQueryEntity(entityType) {
    const def = stateQueries.ENTITY_DEFINITIONS[entityType] || {
        stages: stateQueries[`${entityType.toUpperCase()}_STAGES`],
        transitions: stateQueries[`${entityType.toUpperCase()}_TRANSITIONS`],
        actions: stateQueries[`${entityType.toUpperCase()}_ACTIONS`],
    };
    const lines = [];
...
function buildWorkflowRulesJson() {
    return {
        ...
        stateQueries: {
            feature: { stages: stateQueries.FEATURE_STAGES, transitions: stateQueries.FEATURE_TRANSITIONS, actions: stateQueries.FEATURE_ACTIONS },
            research: { stages: stateQueries.RESEARCH_STAGES, transitions: stateQueries.RESEARCH_TRANSITIONS, actions: stateQueries.RESEARCH_ACTIONS },
