...
function deriveFeatureDashboardStatus(rawStatus, options = {}) {
    ...
    // `revision-complete` — workflow snapshots only know engine states (idle/running/...),
    // so a snapshot-derived status will never carry it. Prefer it when present so the signal survives normalization.
    ...
    if (normalizedStatus === 'feedback-addressed' || normalizedStatus === 'revision-complete') return normalizedStatus;
