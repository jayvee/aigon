...
function safeSetAutoSessionExists(setSlug, repoPath) {
    ...
    let persisted = repoPath ? reconcileStaleSetAutoPauseState(repoPath, setSlug) : null;
    try {
        const { sessions } = _getCachedTmuxList();
        const candidates = sessions.filter(s => s === expected);
        if (candidates.length > 0) {
            candidates.sort((a, b) => b.length - a.length || a.localeCompare(b));
            if (!persisted && repoPath) persisted = readSetAutoState(repoPath, setSlug);
    ...
    if (!repoPath) return null;
    try {
        if (!persisted) persisted = reconcileStaleSetAutoPauseState(repoPath, setSlug);
        if (!persisted) return null;
