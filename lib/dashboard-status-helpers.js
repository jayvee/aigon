...
const {
    ...
    reconcileStaleSetAutoSessionState,
    SET_PAUSED_STATUSES,
...
function safeFeatureAutoSessionExists(featureId, repoPath) {
    ...
    if (repoPath) {
        reconcileStaleFeatureAutoState(repoPath, featureId, autoSessionRunning);
    }
    if (!repoPath) return null;
    try {
...
function _tmuxNameMatchesSetSpecCycle(sessionName, setSlug, repoPrefix, role) {
...
function safeSetSpecCycleSessionExists(setSlug, repoPath, {
    role,
    metadataKey,
    label,
}) {
    ...
    try {
        ...
        for (const sessionName of sessions) {
            ...
            const sidecarSetSlug = sidecar && sidecar.metadata
                && sidecar.metadata[metadataKey]
                && sidecar.metadata[metadataKey].setSlug;
            ...
            if (sidecar && sidecar.role === role) {
                continue;
            }

            if (!sidecar && _tmuxNameMatchesSetSpecCycle(sessionName, setSlug, repoPrefix, role)) {
                const parsed = parseTmuxSessionName(sessionName);
                candidates.push({
                    sessionName,
...
function safeSetSpecReviewSessionExists(setSlug, repoPath) {
    return safeSetSpecCycleSessionExists(setSlug, repoPath, {
        role: 'spec-review',
        metadataKey: 'setSpecReview',
        label: 'Spec review: running',
    });
}

function safeSetSpecRevisionSessionExists(setSlug, repoPath) {
    return safeSetSpecCycleSessionExists(setSlug, repoPath, {
        role: 'spec-revise',
        metadataKey: 'setSpecRevise',
        label: 'Spec revision: running',
    });
}

function buildSetAutoSessionDto(persisted, { sessionName = null, sessionAlive = false } = {}) {
    if (!persisted && !sessionName) return null;
    const persistedStatus = persisted && persisted.status ? String(persisted.status) : '';
    const isPaused = SET_PAUSED_STATUSES.has(persistedStatus);
    const activeSession = Boolean(sessionAlive) && !isPaused;

    return {
        sessionName: sessionName || (persisted && persisted.sessionName) || null,
        sessionAlive: Boolean(sessionAlive),
        running: activeSession || (persisted ? Boolean(persisted.running) : false),
        status: isPaused
            ? persistedStatus
            : (activeSession ? 'running' : (persistedStatus || 'stopped')),
        currentFeature: persisted && persisted.currentFeature ? String(persisted.currentFeature) : null,
        failedFeature: persisted && persisted.failedFeature ? String(persisted.failedFeature) : null,
        pausedFeature: persisted && persisted.pausedFeature ? String(persisted.pausedFeature) : null,
        completed: persisted && Array.isArray(persisted.completed) ? persisted.completed.slice() : [],
        failed: persisted && Array.isArray(persisted.failed) ? persisted.failed.slice() : [],
        members: persisted && Array.isArray(persisted.members) ? persisted.members.slice() : [],
        updatedAt: persisted && persisted.updatedAt || null,
        startedAt: persisted && persisted.startedAt || null,
        endedAt: persisted && persisted.endedAt || null,
        reason: persisted && persisted.reason || null,
    };
}
...
function safeSetAutoSessionExists(setSlug, repoPath) {
    ...
    let persisted = repoPath ? readSetAutoState(repoPath, setSlug) : null;
    try {
        const { sessions } = _getCachedTmuxList();
        const candidates = sessions.filter(s => s === expected);
        if (candidates.length > 0) {
            candidates.sort((a, b) => b.length - a.length || a.localeCompare(b));
            if (repoPath) persisted = reconcileStaleSetAutoPauseState(repoPath, setSlug) || persisted;
            return buildSetAutoSessionDto(persisted, {
    ...
    if (!repoPath) return null;
    try {
        persisted = reconcileStaleSetAutoPauseState(repoPath, setSlug) || persisted;
        persisted = reconcileStaleSetAutoSessionState(repoPath, setSlug, false) || persisted;
        if (!persisted) return null;
        return buildSetAutoSessionDto(persisted, { sessionAlive: false });
    } catch (_) {
        return null;
    }
}
...
function listSpecReviewTmuxSessions(repoPath, entityType, entityId, role) {
    const prefix = buildSpecReviewSessionPrefix(repoPath, entityType, entityId, role);
    return findTmuxSessionsByPrefix(prefix, session => {
        const parsed = parseTmuxSessionName(session);
        const agent = parsed && parsed.role === role
            ? parsed.agent
            : session.slice(prefix.length).split('-')[0];
        return agent ? { session, agent, running: true, sessionRunning: true } : null;
    });
}
