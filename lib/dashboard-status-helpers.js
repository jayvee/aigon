...
function deriveFeatureDashboardStatus(rawStatus, options = {}) {
    // The agent status file is the source of truth for `feedback-addressed` —
    // workflow snapshots only know engine states (idle/running/...), so a snapshot-derived
    // status will never carry it. Prefer it when present so the signal survives normalization.
    const fileStatus = String(options.fileStatus || '').trim().toLowerCase();
    if (fileStatus === 'feedback-addressed') return 'feedback-addressed';
    if (fileStatus === 'addressing-review') return 'addressing-review';
    const normalizedStatus = normalizeDashboardStatus(rawStatus);
    if (normalizedStatus === 'feedback-addressed') return normalizedStatus;
    const reviewStatus = String(options.reviewStatus || '').trim().toLowerCase();
    if (
        normalizedStatus === 'implementing'
        && reviewStatus === 'done'
        && options.tmuxRunning
    ) {
...
function computeRebaseNeeded(worktreePath, defaultBranch) {
    ...
    try {
        ...
        const count = parseInt(execSync(`git -C ${quoted} rev-list --count HEAD..${defaultBranch}`, {
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'ignore']
        }).trim(), 10);
