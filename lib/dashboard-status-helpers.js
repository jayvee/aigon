...
function deriveFeatureDashboardStatus(rawStatus, options = {}) {
    ...
    if (fileStatus === 'quota-paused') return 'quota-paused';
...
/**
 * True when `defaultBranch` has commits not reachable from HEAD (integrate via rebase/merge).
 * REGRESSION: must stay a pure git read with false on error — dashboard collector and tests rely on it.
 */
function computeRebaseNeeded(worktreePath, defaultBranch) {
    if (!worktreePath || !defaultBranch) return false;
    try {
        const out = execFileSync('git', ['-C', worktreePath, 'rev-list', '--count', `HEAD..${defaultBranch}`], {
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'ignore'],
        }).trim();
