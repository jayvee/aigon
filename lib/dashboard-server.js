...
function handleLaunchSpecReview(ctx, options = {}) {
    ...
    const recordStart = role === 'spec-check'
        ? workflowEngine.recordSpecReviewCheckStarted(absRepo, entityTypeFull, featureId, { checkerId: agentId })
...
function handleLaunchCloseResolve(ctx) {
    ...
    try {
        ...
        if (returnSpecState && returnSpecState !== 'close_recovery_in_progress') {
            workflowEngine.recordCloseRecoveryStarted(absRepo, featureId, {
                agentId,
                sessionName,
                source: 'dashboard',
                returnSpecState,
            }).catch((err) => {
                console.warn(`[close-resolve] failed to record close_recovery.started for F${featureId}: ${err.message}. Run aigon doctor --fix.`);
            });
        }
    } catch (err) {
        console.warn(`[close-resolve] failed to record close_recovery.started for F${featureId}: ${err.message}. Run aigon doctor --fix.`);
