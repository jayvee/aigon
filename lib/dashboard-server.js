    ...
    repos.forEach(repoPath => {
        ...
        specFiles.forEach(({ file: specFile, stage, dir: specDir }) => {
            ...
            features.push({
                ...
                nextAction: workflowReadModel.getDashboardNextCommand('feature', parsed.id, stage, agents),
                nextActions: workflowReadModel.getDashboardNextActions('feature', parsed.id, stage, agents),
                validActions: featureReadModel.validActions
            });
        });
    ...
    const server = http.createServer((req, res) => {
        ...
        if (repoAgentDevServerPokeMatch && req.method === 'POST') {
            ...
            try {
                ...
                const sessionName = tmuxInfo ? tmuxInfo.sessionName : buildTmuxSessionName(featureId, agentId, { repo: path.basename(repoPath) });
                const worktreePath = agent.worktreePath || resolveFeatureWorktreePath(repoPath + '-worktrees', featureId, agentId) || repoPath;
                const command = 'aigon dev-server start';

                if (tmuxInfo && tmuxInfo.running) {
                    runTmux(['send-keys', '-t', sessionName, command, 'Enter'], { stdio: 'ignore' });
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                        ok: true,
                        mode: 'send-keys',
                        sessionName,
                        message: `Sent dev-server start to ${sessionName}`
                    }));
                    return;
                }
                ...
                res.end(JSON.stringify({
                    ...
                    mode: 'spawned',
                    sessionName,
                    message: `Started ${sessionName} and launched dev-server command`
                }));
            } catch (e) {
                res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
