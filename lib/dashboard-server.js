...
const { collectDashboardStatusData, collectAgentLogs } = require('./dashboard-status-collector');
const {
    ...
    if (type === 'feature') {
        ...
        if (featureName) {
            Object.entries(agentFiles).forEach(([agentId, file]) => {
                const baseDir = (file && file.worktreePath && fs.existsSync(file.worktreePath))
                    ? path.join(file.worktreePath, 'docs', 'specs', 'features', 'logs')
                    : repoLogDir;
                expectedLogs[agentId] = path.join(baseDir, `feature-${id}-${agentId}-${featureName}-log.md`);
            });
        }

        try {
            agentLogs = collectAgentLogs(logsDirs, id, expectedLogs);
        } catch (_) {
            agentLogs = {};
        }
    }
...
function handleLaunchEval(ctx) {
    ...
    ensureTmuxSession(sessionName, taskCwd, () =>
        isResearch ? buildResearchAgentCommand(agentId, featureId, 'eval', absRepo)
                   : buildAgentCommand({ agent: agentId, featureId, path: taskCwd, desc, repoPath: absRepo }, 'evaluate'));
    return { ok: true, message: `Opened eval for ${label}${featureId}`, sessionName };
}

function handleLaunchImplementation(ctx) {
    const { worktreePath, absRepo, featureId, agentId, desc, isResearch, repoName, latestStatus } = ctx;
    ...
    const agentCmd = isResearch
        ? buildResearchAgentCommand(agentId, featureId, 'do', absRepo)
        : buildAgentCommand({ agent: agentId, featureId, path: worktreePath, desc, repoPath: absRepo });
...
function workflowSourceLabel(source) {
    ...
    return String(source || 'Workflow');
}

function serializeWorkflowDefinition(definition, repoPath) {
    const runtime = workflowDefinitions.applyWorkflowDefinition(definition);
    return {
        slug: definition.slug,
        label: definition.label,
        description: definition.description || '',
        source: definition.source,
        sourceLabel: workflowSourceLabel(definition.source),
        path: workflowDefinitions.getWorkflowDefinitionPath(definition, repoPath),
        readOnly: !!definition.readOnly,
        version: definition.version === 2 ? 2 : 1,
        stages: Array.isArray(definition.stages)
            ...
            : null,
        agents: Array.isArray(runtime.agents) ? runtime.agents : [],
        evalAgent: runtime.evalAgent || null,
        reviewAgent: runtime.reviewAgent || null,
        stopAfter: runtime.stopAfter || 'close',
        summary: workflowDefinitions.formatWorkflowSummary(definition),
    };
}
...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    const ctx = { absRepo, worktreePath, featureId, agentId, desc, isResearch, worktreePrefix, repoName, latestStatus };

                    const handler = mode === 'review' ? handleLaunchReview
                        : mode === 'eval' ? handleLaunchEval : handleLaunchImplementation;
                    const body = JSON.stringify(handler(ctx));
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(body);
                } catch (e) {
                    res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        ...
        if ((reqPath === '/api/workflows' || reqPath === '/api/playbooks') && req.method === 'GET') {
            const url = new URL(req.url || '/api/workflows', 'http://127.0.0.1');
            const repoPath = resolveRequestedRepoPathOrRespond(res, String(url.searchParams.get('repoPath') || '').trim());
            if (!repoPath) return;
            try {
                const workflows = workflowDefinitions.listAvailableWorkflows(repoPath)
                    .map(definition => serializeWorkflowDefinition(definition, repoPath));
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        ...
        if (reqPath === '/api/workflows' && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                let payload = {};
                ...
                const repoPath = resolveRequestedRepoPathOrRespond(res, payload.repoPath);
                if (!repoPath) return;

                const name = String(payload.name || payload.label || payload.slug || '').trim();
                const agents = workflowDefinitions.normalizeAgentList(payload.agents);
                const evalAgent = String(payload.evalAgent || '').trim().toLowerCase();
                const reviewAgent = String(payload.reviewAgent || '').trim().toLowerCase();
                const stopAfter = String(payload.stopAfter || 'close').trim().toLowerCase();
                ...
                if (agents.length === 0) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: 'At least one implementation agent is required' }));
                    return;
                }

                try {
                    const saved = workflowDefinitions.saveWorkflowDefinition('project', repoPath, {
                        slug: name,
                        label: name,
                        ...
                        availableAgents: agentRegistry.getAllAgentIds(),
                    });
                    res.writeHead(201, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                        ok: true,
                        message: `Saved project workflow: ${saved.slug}`,
                        workflow: serializeWorkflowDefinition(saved, repoPath),
                    }));
                } catch (error) {
        ...
        const autonomousRunMatch = reqPath.match(/^\/api\/features\/([^/]+)\/run$/);
        if (autonomousRunMatch && req.method === 'POST') {
            let featureId = '';
            try {
                featureId = decodeURIComponent(autonomousRunMatch[1] || '').trim();
            } catch (_) {
                res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: 'Invalid feature id in path' }));
                return;
            }
            ...
            req.on('end', () => {
                ...
                if (!featureId || !/^\d+$/.test(featureId)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: 'Feature id must be numeric' }));
                    return;
                }
                if (agents.length === 0) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                ...
                if (!['implement', 'eval', 'review', 'close'].includes(stopAfter)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: 'stopAfter must be one of: implement, eval, review, close' }));
                    return;
                }
                const agentPattern = /^[a-z]{2,4}$/;
                const invalidAgents = agents.filter(a => !agentPattern.test(a));
                if (invalidAgents.length > 0) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: `Invalid agent identifier(s): ${invalidAgents.join(', ')}` }));
                    return;
                }
                if (evalAgent && !agentPattern.test(evalAgent)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: `Invalid eval agent identifier: ${evalAgent}` }));
                    return;
                }
                if (reviewAgent && !agentPattern.test(reviewAgent)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: `Invalid review agent identifier: ${reviewAgent}` }));
                    return;
                }
                ...
                if (unknownAgents.length > 0) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                        error: `Unknown agent(s): ${unknownAgents.join(', ')}`,
                ...
                if (evalAgent && !availableAgents.has(evalAgent)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                        error: `Unknown eval agent: ${evalAgent}`,
                        availableAgents: [...availableAgents].sort((a, b) => a.localeCompare(b)),
                    }));
                    return;
                }
                if (reviewAgent && !availableAgents.has(reviewAgent)) {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                        error: `Unknown review agent: ${reviewAgent}`,
                        availableAgents: [...availableAgents].sort((a, b) => a.localeCompare(b)),
                    }));
                    return;
                }
                ...
                if (result.error) {
                    res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: `Failed to start autonomous run: ${result.error.message}` }));
                    return;
                }
                ...
                if (exitCode !== 0) {
                    ...
                    // Recognize both ❌ (generic errors) and 🔒 (Pro capability
                    // gate) so users see the real reason the action failed
                    // instead of "Autonomous start failed with exit code 1".
                ...
                // Parse the session name from stdout (feature-autonomous-start prints:
                // "✅ AutoConductor started: <sessionName>")
                // Fall back to a direct tmux query if not found in stdout.
                let autoSessionName = null;
                const sessionMatch = stdout.match(/AutoConductor started:\s*(\S+)/);
                if (sessionMatch) {
                    autoSessionName = sessionMatch[1];
                } else {
                    // Direct query bypasses the stale tmux list cache
                    const repo = path.basename(repoPath);
                    const prefix = `${repo}-f${featureId}-auto`;
                    try {
                        const r = runTmux(['list-sessions', '-F', '#S'], { encoding: 'utf8', stdio: 'pipe' });
                        if (!r.error && r.status === 0) {
                            autoSessionName = r.stdout.split('\n').map(s => s.trim()).filter(Boolean)
        ...
        if (repoMainDevServerStartMatch && req.method === 'POST') {
            ...
            try {
                ...
                // Check if main dev server is already running by looking for its Caddy route
                ...
                // After starting, check if the route now has an active backend
        ...
        const repoFeaturePrStatusMatch = reqPath.match(/^\/api\/repos\/(.+)\/features\/([^/]+)\/pr-status$/);
        if (repoFeaturePrStatusMatch && req.method === 'GET') {
            const repoResolution = resolveRepoFromPathParam(repoFeaturePrStatusMatch[1]);
            if (!repoResolution.ok) {
                res.writeHead(repoResolution.status || 400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: repoResolution.error || 'Invalid repo path' }));
                return;
            ...
            let featureId;
            try {
                featureId = decodeURIComponent(repoFeaturePrStatusMatch[2] || '').trim();
            } catch (_) {
                res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: 'Invalid featureId path parameter' }));
                return;
            }

            const payload = getFeaturePrStatusPayload(repoResolution.repoPath, featureId);
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
            res.end(JSON.stringify(payload));
            return;
        }
        ...
        if (reqPath === '/api/action' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                // feature 234: in-flight dedupe — reject parallel dispatches of the
                // same action+args+repoPath with HTTP 409 so double-clicks can't spawn
                // two close subprocesses in parallel.
                const dedupeRepoPath = payload.repoPath || process.cwd();
                const dedupeKey = inflightKey(dedupeRepoPath, payload.action, payload.args || []);
                if (inflightActions.has(dedupeKey)) {
                    const existing = inflightActions.get(dedupeKey);
                    res.writeHead(409, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({
                ...
                inflightActions.set(dedupeKey, { startedAt: new Date().toISOString(), action: payload.action });

                const actionStartTime = Date.now();
                let result;
                try {
                    result = runDashboardInteractiveAction({
                        ...payload,
                        registeredRepos: readConductorReposFromGlobalConfig(),
                        defaultRepoPath: process.cwd()
                    });
                } finally {
                    inflightActions.delete(dedupeKey);
                }
                ...
                // feature 234: check for a "restart needed" marker dropped by the close
                // child (via lib/feature-close.js writeRestartMarkerFile). If present,
                // include `serverRestarting: true` in the response so the frontend can
                // show the "Reloading backend…" banner, THEN spawn a fully detached
                // `aigon server restart` and exit this process. Detached + stdio:ignore
                // means the restarter does not inherit our (dying) stdio, avoiding the
                // EPIPE death spiral that killed feature 233's close from the dashboard.
                ...
                if (restartMarker) {
                    log(`🔄 Lib files changed (${(restartMarker.files || []).length}) — scheduling dashboard self-restart`);
                    // 100ms gives the HTTP response time to flush to the proxy client.
                    setTimeout(() => {
                        try {
                            const child = spawn(
                                process.execPath,
                                [CLI_ENTRY_PATH, 'server', 'restart'],
                                { detached: true, stdio: 'ignore', cwd: process.cwd() }
                            );
                            child.unref();
                        } catch (e) {
                            log(`Failed to spawn detached restart: ${e.message}`);
                        }
                        // 50ms gives unref() time to take effect before exit.
        ...
        if (reqPath === '/api/stats-aggregate') {
            // Feature 230: structured aggregate read from per-feature stats.json
            // files with a versioned cache at .aigon/cache/stats-aggregate.json.
            try {
                ...
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ version: statsAggregate.CACHE_VERSION, repos: byRepo }));
            } catch (e) {
        ...
        // Deep status — computed on demand for a single feature/research entity
        if (reqPath.startsWith('/api/feature-status/') && req.method === 'GET') {
            const m = reqPath.match(/^\/api\/feature-status\/(\d+)$/);
            if (!m) {
                res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: 'Invalid feature-status route. Expected /api/feature-status/:id' }));
                return;
            }
            const id = m[1];
            ...
            if (!resolvedRepo) {
                res.writeHead(404, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: 'Could not resolve repository' }));
                return;
            }
            try {
                const deepStatus = collectFeatureDeepStatus(resolvedRepo, id, { entityType });
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify(deepStatus));
            } catch (e) {
                res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: e.message }));
            }
        ...
        // /api/spec PUT removed — specs are edited through the editor/CLI, not the dashboard.
        ...
        if ((reqPath === '/api/open-folder' || reqPath === '/api/open-path') && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                try {
        ...
        if (reqPath === '/api/session/run' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    const resolved = resolveDashboardSessionCommand(command);
                    const result = spawnSync(resolved.bin, resolved.args, { // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
                        cwd: effectiveCwd,
                        encoding: 'utf8',
                        timeout: 120000,
                        maxBuffer: 1024 * 1024,
        ...
        // ── Peek by feature/agent — resolves session name server-side ────────
        if (reqPath.startsWith('/api/peek/') && req.method === 'GET') {
            const m = reqPath.match(/^\/api\/peek\/(\d+)\/([a-zA-Z0-9_-]+)$/);
            if (!m) {
                res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: 'Expected /api/peek/:featureId/:agentId' })); return;
            }
            const fid = toUnpaddedId(m[1]);
            const agentId = m[2];
            const url = new URL(req.url, `http://${req.headers.host}`);
            const repoPath = resolveRequestedRepoPathOrRespond(res, String(url.searchParams.get('repoPath') || '').trim());
            if (!repoPath) return;
            ...
            // Try to find the tmux session — try feature then research prefix
            const repo = path.basename(repoPath);
            ...
            // Feature eval: prefix match (e.g. repo-f204-eval, repo-f204-eval-desc)
            if (!sessionName) {
                const evalPrefixF = `${repo}-f${fid}-eval`;
                sessionName = findFirstTmuxSessionByPrefix(evalPrefixF, s => s) || null;
            }
            // Research eval: repo-r{id}-eval-{agent}
            if (!sessionName) {
            ...
            if (!sessionName) {
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ lines: [], sessionName: '', uptime: '', lastActivity: '', alive: false })); return;
            }
            ...
            try {
                ...
                const { uptime, lastActivity } = getTmuxSessionPeekMeta(sessionName);
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ lines, sessionName, uptime, lastActivity, alive: true }));
            } catch (e) {
                res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: e.message }));
            }
        ...
        if (reqPath === '/api/session-peek' && req.method === 'GET') {
            ...
            try {
                ...
                const { uptime, lastActivity } = getTmuxSessionPeekMeta(sessionName);
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ output, offset: output.length, alive: true, uptime, lastActivity }));
            } catch (e) {
                res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: e.message }));
            }
        ...
        if (reqPath === '/api/logs' && req.method === 'GET') {
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
            res.end(JSON.stringify({ events: logsBuffer.slice() }));
            return;
        }
        ...
        if (reqPath === '/api/settings' && req.method === 'GET') {
            ...
            const resolvedRepoPath = resolveRequestedRepoPathOrRespond(res, repoPath);
            if (!resolvedRepoPath) return;
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
            res.end(JSON.stringify(buildDashboardSettingsPayload(resolvedRepoPath)));
            return;
        }
        ...
        if (reqPath === '/api/settings' && req.method === 'PUT') {
            ...
            req.on('end', () => {
                ...
                const repoPath = resolveRequestedRepoPathOrRespond(res, repoPathRaw);
                if (!repoPath) return;
                ...
                try {
                    ...
                    const settingsBody = JSON.stringify({ ok: true, ...buildDashboardSettingsPayload(repoPath) });
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(settingsBody);
                } catch (e) {
                    res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: e.message }));
                }
            ...
            req.on('end', () => {
                ...
                try {
                    if (action === 'mark-submitted') {
                        ...
                        if (!hasEvidence) {
                            res.writeHead(409, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                            res.end(JSON.stringify({ error: `Cannot mark ${agent} submitted for ${entityType} ${id} without implementation evidence.` }));
                            return;
                        }
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    const shutdown = (sig) => {
        log(`Dashboard shutting down (PID ${process.pid})${sig ? ` — ${sig}` : ''}`);
