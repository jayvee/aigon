...
function inferDashboardNextActions(featureId, agents, stage) {
    ...
    recommended.forEach(a => {
        ...
        switch (a.action) {
            ...
            case 'research-eval':  command = `/are ${id}`; break;
...
function collectDashboardStatusData() {
    ...
    repos.forEach(repoPath => {
        ...
        researchSpecFiles.forEach(({ file, stage, dir: specDir }) => {
            ...
            const agents = [];
            if (id && (stage === 'in-progress' || stage === 'in-evaluation') && researchLogsByAgent[id]) {
                researchLogsByAgent[id].forEach(({ agent, status, updatedAt }) => {
                    ...
                    // Auto-detect submission: if no status file but findings file has real content
                    // (not TBD) and agent session has ended, mark as submitted automatically.
                    // This catches agents that commit findings but forget to run agent-status/research-submit.
                    if (!hasStatusFile && normalizedStatus !== 'submitted') {
                        try {
                            const findingsPath = path.join(researchLogsDir, `research-${idPadded}-${agent}-findings.md`);
                            if (fs.existsSync(findingsPath)) {
                                const content = fs.readFileSync(findingsPath, 'utf8');
                                const hasTBD = /^TBD\s*$/m.test(content.replace(/^---[\s\S]*?---/, '').trim());
                                const hasRealContent = content.length > 200 && !hasTBD;
                                if (hasRealContent) {
                                    // Auto-submit: write the status file so this only runs once
                                    const { writeAgentStatus } = require('./manifest');
                                    writeAgentStatus(id, agent, { status: 'submitted', flags: {}, autoDetected: true });
            ...
            if (stage === 'in-evaluation') {
                const repoBaseName = path.basename(absRepoPath);
                const evalPrefix = `${repoBaseName}-r${id}-eval-`;
                try {
                    assertTmuxAvailable();
                    const listResult = runTmux(['list-sessions', '-F', '#S'], { encoding: 'utf8', stdio: 'pipe' });
                    if (!listResult.error && listResult.status === 0) {
            ...
            research.push({ id, name, stage, specPath: path.join(specDir, file), agents, evalSession, validActions: stateMachine.getAvailableActions('research', stage, researchSmContext) });
        });
        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    // Agent-mode actions (eval, review, etc.): run from main repo with a dedicated session
                    const AGENT_MODE_TASKS = {
                        'eval': { sessionSuffix: isResearch ? `eval-${agentId}` : 'eval', taskType: 'evaluate', label: isResearch ? 'R' : 'F' },
                        'review': { sessionSuffix: `review-${agentId}`, taskType: 'review', label: 'F' },
                    };
                    if (AGENT_MODE_TASKS[mode]) {
                        const task = AGENT_MODE_TASKS[mode];
                        const unpadded = toUnpaddedId(featureId);
                        const sessionName = `${path.basename(absRepo)}-${task.label.toLowerCase()}${unpadded}-${task.sessionSuffix}`;
                        ...
                        if (tmuxSessionExists(sessionName)) {
                            ...
                            const cmd = (isResearch && mode === 'eval')
                                ? buildResearchAgentCommand(agentId, featureId, 'eval')
