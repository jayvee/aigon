...
/**
 * Resolve a file under @senlabsai/aigon-pro/dashboard (e.g. backup-sync.js).
 * Tries `getPro().dashboardDir` first, then `dirname(require.resolve('@senlabsai/aigon-pro/package.json'))/dashboard`.
...
window.AigonProBenchmarkMatrix = { mount: function (section) {
  var w = document.createElement('div');
  w.className = 'matrix-section';
  w.innerHTML = '<div class="settings-empty">Install <code>@senlabsai/aigon-pro</code> and restart the dashboard to load latest <code>aigon perf-bench</code> results here.</div>';
  section.appendChild(w);
}};`);
                return;
            }
  ...
  c.innerHTML = '<p class="settings-empty" style="margin-top:4px;font-size:12px;color:var(--text-tertiary)">' +
    ...
    server.listen(port, host, () => {
        ...
        try {
            const { DASHBOARD_RUNTIME_FILE } = require('./version-status');
            const runtimeDir = require('path').dirname(DASHBOARD_RUNTIME_FILE);
            if (!require('fs').existsSync(runtimeDir)) require('fs').mkdirSync(runtimeDir, { recursive: true });
            require('fs').writeFileSync(DASHBOARD_RUNTIME_FILE, JSON.stringify({
