...
function handleLaunchReview(ctx) {
    ...
    ensureTmuxSession(sessionName, taskCwd, () => buildAgentCommand({ agent: agentId, featureId, path: taskCwd, desc }, commandType));
...
function buildDashboardSettingsPayload(repoPath, options = {}) {
    ...
    const settings = DASHBOARD_SETTINGS_SCHEMA.map(def => {
        const modelKeyMatch = def.key.match(/^agents\.(\w+)\.(research|implement|evaluate)\.model$/);
