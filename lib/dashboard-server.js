...
const { createRestartBackstop } = require('./dashboard-restart-backstop');
...
function handleSpecReconcileApiRequest(req, res, options = {}) {
    ...
    req.on('data', chunk => {
        ...
        // F672: cap request body size (defense-in-depth).
        if (bodyBytes > dashboardSecurity.MAX_BODY_BYTES) {
            bodyAborted = true;
...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    // F672: bind to loopback by default; a non-loopback bind requires a
    // configured shared secret or the server refuses to start (throws
    // AIGON_INSECURE_BIND). `security` also carries the token + Host allow-list
    // consumed by the request/upgrade guards below.
    let security;
    try {
        security = dashboardSecurity.resolveServerSecurity({ globalConfig: loadGlobalConfig(), env: process.env });
    } catch (err) {
        if (err && err.code === 'AIGON_INSECURE_BIND') {
            console.error(`\n❌ ${err.message}\n`);
        }
        throw err;
    }
    const host = security.host;
    instanceName = instanceName || 'main';
    options = options || {};
    ...
    // Keyed by actionId (client-generated). Each entry includes log lines plus
    // action metadata used by status collection while the command is running.
    const activeActionLogs = new Map();
    function buildStatusCollectOptions(extra = {}) {
        const opts = { ...(extra || {}) };
        const closingFeatureIdsByRepo = new Map();
        for (const entry of activeActionLogs.values()) {
            if (!entry || entry.done || entry.action !== 'feature-close' || !entry.repoPath || !entry.featureId) continue;
            const repoKey = path.resolve(entry.repoPath);
            if (!closingFeatureIdsByRepo.has(repoKey)) closingFeatureIdsByRepo.set(repoKey, new Set());
            closingFeatureIdsByRepo.get(repoKey).add(String(entry.featureId));
        }
        if (closingFeatureIdsByRepo.size > 0) {
            opts.closingFeatureIdsByRepo = closingFeatureIdsByRepo;
        }
        return opts;
    }
    ...
    function refreshLatestStatus() {
        try {
            return replaceLatestStatus(
                collectDashboardStatusData(buildStatusCollectOptions(process.env.AIGON_DASH_TIMING === '1' ? { collectPerf: true } : undefined)),
                'refresh'
            );
        } catch (err) {
    ...
    const restartBackstop = createRestartBackstop({
        getRegisteredRepos: readConductorReposFromGlobalConfig,
        getServerCwd: () => process.cwd(),
        hasInflightActions: () => inflightActions.size > 0,
    ...
    async function pollStatus(options = {}) {
        ...
        const run = (async () => {
            ...
            try {
                ...
                const collected = await collectDashboardStatusDataAsync(buildStatusCollectOptions({ collectPerf: true }));
    ...
    async function pollRepoStatus(repoPath) {
        ...
        const run = (async () => {
            ...
            try {
                const envTimingOn = process.env.AIGON_DASH_TIMING === '1';
                const refreshed = refreshRepoInDashboardStatus(latestStatus, repoPath, buildStatusCollectOptions({ collectPerf: true }));
                const pollTotalMs = Date.now() - pollStart;
    ...
    async function afterPollSideEffects(meta = {}) {
        ...
        try {
            restartBackstop.tick();
        } catch (e) {
            log.warn(`Restart backstop tick failed: ${e && e.message ? e.message : e}`);
        }
    }
    ...
    const dashboardRoutes = createDashboardRouteDispatcher({
        ...
        routes: {
            ...
            parseFeatureSpecFileName,
            safeTmuxSessionExists,
            collectDashboardStatusData: (options) => collectDashboardStatusData(buildStatusCollectOptions(options)),
            collectDashboardStatusDataAsync: (options) => collectDashboardStatusDataAsync(buildStatusCollectOptions(options)),
            collectAllFeaturesLean,
            collectDashboardHealth,
    ...
    const server = http.createServer((req, res) => {
        ...
        // F672: central security guard — Host/Origin validation (always) plus a
        // shared-secret token gate when a secret is configured (non-loopback
        // bind). Runs before any route dispatch or static handling.
        const secDecision = dashboardSecurity.evaluateHttpSecurity(req, {
            token: security.token,
            allowedHosts: security.allowedHosts,
        });
        if (secDecision.setCookie) res.setHeader('Set-Cookie', secDecision.setCookie);
        if (!secDecision.ok) {
            res.writeHead(secDecision.status || 403, { 'content-type': 'text/plain; charset=utf-8', 'cache-control': 'no-store' });
            res.end(secDecision.error || 'Forbidden');
            return;
        }
        ...
        if (reqPath.startsWith('/assets/')) {
            // F672: contain within ROOT_DIR/assets — reject path traversal.
            const assetFile = dashboardSecurity.resolveWithinBase(path.join(ROOT_DIR, 'assets'), reqPath.slice('/assets'.length));
        ...
        if (reqPath.startsWith('/js/') || reqPath === '/styles.css') {
            ...
            // F672: contain within templates/dashboard — reject path traversal.
            const dashFile = dashboardSecurity.resolveWithinBase(path.join(templateRoot, 'templates', 'dashboard'), reqPath);
            if (dashFile && fs.existsSync(dashFile) && fs.statSync(dashFile).isFile()) {
    ...
    server.on('upgrade', (req, socket, head) => {
        ...
        if (pathname.match(/^\/api\/session\/pty\//)) {
            // F672: run the shared Host/token guard before delegating to the PTY
            // handler (which still enforces its Origin allow-list + single-use
            // PTY token). Closes the "bare LAN client mints a token" hole.
            const upgradeDecision = dashboardSecurity.evaluateUpgradeSecurity(req, {
                token: security.token,
                allowedHosts: security.allowedHosts,
            });
    ...
    server.listen(port, host, () => {
        ...
        // F693: recurring-feature spawning, scheduled-kickoff polling and the
        // scheduled vault push used to start inside Aigon Pro's register(api)
        // hook. Pro is merged into OSS, so they start unconditionally here —
        // same intervals, same .unref() so they never hold the process open.
        const recurring = require('./recurring');
        const scheduledKickoff = require('./scheduled-kickoff');
        const backup = require('./backup');

        // Recurring features: check on startup, then every 24 hours.
        const runRecurringCheck = () => {
            const repos = readConductorReposFromGlobalConfig().map(r => path.resolve(String(r)));
            if (repos.length === 0) repos.push(options.defaultRepoPath || process.cwd());
            for (const repoPath of repos) {
                try { recurring.checkRecurringFeatures(repoPath); }
                catch (e) { log(`[recurring] Check failed for ${repoPath}: ${e && e.message}`); }
            }
        };
        try { runRecurringCheck(); } catch (e) { log(`[recurring] initial check failed: ${e && e.message}`); }
        setInterval(runRecurringCheck, 24 * 60 * 60 * 1000).unref();

        // Scheduled kickoff poller (F703, F704).
        try {
            scheduledKickoff.startScheduledKickoffPoller({
                log,
                getRepoRoots: () => readConductorReposFromGlobalConfig().map(r => path.resolve(String(r))),
                cliEntryPath: CLI_ENTRY_PATH,
            });
        } catch (e) {
            log(`Scheduled kickoff poller failed to start: ${e && e.message}`);
        }

        // Scheduled vault backup (F706). Hourly tick; the engine decides when due.
        const runBackupCheck = () => {
            const r = backup.runScheduledPushIfDue();
            if (!r.ran) return;
            if (r.ok) {
                log(`[backup] scheduled push ok (${(r.result && r.result.fileCount) || 0} files)`);
            } else {
                log(`[backup] scheduled push failed: ${r.error}`);
                try { emitNotification('backup-failed', `Backup push failed: ${r.error}`, { code: r.code }); }
                catch (_) { /* notification subsystem not yet ready */ }
            }
        };
        try { runBackupCheck(); } catch (e) { log(`[backup] initial check failed: ${e && e.message}`); }
        setInterval(runBackupCheck, 60 * 60 * 1000).unref();
        try {
