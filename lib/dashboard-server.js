...
const { writeAgentStatusAt } = require('./manifest');
...
const {
    ...
    buildAgentCommand, buildResearchAgentCommand, tileITerm2Windows,
} = require('./worktree');
...
function inferDashboardNextActions(featureId, agents, stage) {
    ...
    recommended.forEach(a => {
        ...
        switch (a.action) {
            ...
            case 'feature-start':  command = `aigon feature-start ${id}`; break;
...
['cc', 'gg', 'cx', 'cu'].forEach(agentId => {
    ['research', 'implement', 'evaluate'].forEach(task => {
        DASHBOARD_SETTINGS_SCHEMA.push({
            key: `agents.${agentId}.${task}.model`,
            label: `${agentId.toUpperCase()} ${task} model`,
            type: 'string',
            description: `Overrides the default model used by ${agentId.toUpperCase()} for ${task} work. Leave unset to use the built-in default.`
        });
    });
});
...
function buildDashboardSettingsPayload(repoPath, options = {}) {
    ...
    const settings = DASHBOARD_SETTINGS_SCHEMA.map(def => {
        const modelKeyMatch = def.key.match(/^agents\.(cc|gg|cx|cu)\.(research|implement|evaluate)\.model$/);
...
function maybeFlagEndedSession(repoPath, options = {}) {
    ...
    return { status: 'implementing', flags: updatedFlags, hasStatusFile: true };
}
...
function collectDashboardStatusData() {
    ...
    repos.forEach(repoPath => {
        ...
        const profile = getActiveProfile(absRepoPath);
                    ...
                    if (!listResult.error && listResult.status === 0) {
                        ...
                        for (const s of sessions) {
                            if (s.startsWith(reviewPrefix)) {
                                const agentCode = s.slice(reviewPrefix.length);
        ...
        researchSpecFiles.forEach(({ file, stage, dir: specDir }) => {
            ...
            if (id && (stage === 'in-progress') && researchLogsByAgent[id]) {
                researchLogsByAgent[id].forEach(({ agent, status, updatedAt }) => {
                    ...
                    const statusFile = path.join(stateDir, `feature-${id}-${agent}.json`);
                    let normalizedStatus = normalizeDashboardStatus(status);
            ...
            // Detect synthesis session for this research
            let synthesizeSession = null;
            if (stage === 'in-progress') {
                const repoBaseName = path.basename(absRepoPath);
                const synthPrefix = `${repoBaseName}-r${id}-synthesize-`;
                try {
                    assertTmuxAvailable();
                    const listResult = runTmux(['list-sessions', '-F', '#S'], { encoding: 'utf8', stdio: 'pipe' });
                    if (!listResult.error && listResult.status === 0) {
                        const sessions = listResult.stdout.split('\n').map(s => s.trim()).filter(Boolean);
                        for (const s of sessions) {
                            if (s.startsWith(synthPrefix)) {
                                const agentCode = s.slice(synthPrefix.length);
                                synthesizeSession = { session: s, agent: agentCode, running: tmuxSessionExists(s) };
                                break;
                            }
                        }
                    }
            ...
            // Pass synthesis state into context so state machine can hide Synthesize when done
            researchSmContext.synthesizeSession = synthesizeSession;
            // Also check if synthesis output exists in the research doc (survived tmux exit)
            if (!synthesizeSession && stage === 'in-progress') {
                try {
                    const specContent = fs.readFileSync(path.join(specDir, file), 'utf8');
                    if (/^## (?:Output|Recommendation)\b/m.test(specContent) && /Selected Features/i.test(specContent)) {
                        researchSmContext.synthesizeSession = { session: null, agent: null, running: false, completed: true };
                        synthesizeSession = researchSmContext.synthesizeSession;
                    }
                } catch (e) { /* ignore */ }
            }

            research.push({ id, name, stage, specPath: path.join(specDir, file), agents, synthesizeSession, validActions: stateMachine.getAvailableActions('research', stage, researchSmContext) });
        });
...
function runDashboardServer(port, instanceName, serverId) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                const mode = String(payload.mode || 'implement').trim(); // 'implement' or 'eval'
                ...
                try {
                    ...
                    const sessionName = isResearch
                        ? buildResearchTmuxSessionName(featureId, agentId, { repo: path.basename(absRepo) })
                        : buildTmuxSessionName(featureId, agentId, { repo: path.basename(absRepo), desc });
                    ...
                    // Agent-mode actions (eval, review, synthesize, etc.): run from main repo with a dedicated session
                    const AGENT_MODE_TASKS = {
                        'eval': { sessionSuffix: 'eval', taskType: 'evaluate', label: 'F' },
                        'review': { sessionSuffix: `review-${agentId}`, taskType: 'review', label: 'F' },
                        'synthesize': { sessionSuffix: `synthesize-${agentId}`, taskType: 'synthesize', label: 'R' },
                    };
                    if (AGENT_MODE_TASKS[mode]) {
                        const task = AGENT_MODE_TASKS[mode];
                        const sessionName = `${path.basename(absRepo)}-${task.label.toLowerCase()}${featureId}-${task.sessionSuffix}`;

                        // If session already running, just attach — don't kill and restart
                        if (tmuxSessionExists(sessionName)) {
                            openTerminalAppWithCommand(absRepo, `tmux attach -t ${shellQuote(sessionName)}`, sessionName);
                        } else {
                            const cmd = (isResearch && mode === 'synthesize')
                                ? buildResearchAgentCommand(agentId, featureId, 'synthesize')
                                : buildAgentCommand({ agent: agentId, featureId, path: absRepo, desc }, task.taskType);
                            createDetachedTmuxSession(sessionName, absRepo, cmd);
                            openTerminalAppWithCommand(absRepo, `tmux attach -t ${shellQuote(sessionName)}`, sessionName);
                        }
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, message: `Opened ${mode} for ${task.label}${featureId}`, sessionName }));
                        return;
                    }
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    process.on('uncaughtException', (err) => {
        log(`CRASH uncaughtException: ${err.stack || err.message || err}`);
        console.error('💥 Dashboard crashed (uncaughtException):', err.message);
        console.error('   Log:', DASHBOARD_LOG_FILE);
        process.exit(1);
    });
    process.on('unhandledRejection', (reason) => {
        const msg = reason instanceof Error ? reason.stack || reason.message : String(reason);
        log(`CRASH unhandledRejection: ${msg}`);
        console.error('💥 Dashboard unhandled rejection:', String(reason).slice(0, 200));
        console.error('   Log:', DASHBOARD_LOG_FILE);
    });
