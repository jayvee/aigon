...
function handleLaunchSpecReview(ctx, options = {}) {
    ...
    ensureTmuxSession(sessionName, taskCwd, () => rawCommand);
...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    const server = http.createServer((req, res) => {
        ...
        const specReviewApiMatch = reqPath.match(/^\/api\/(feature|research)-spec-review(?:-(check))?$/);
        if (specReviewApiMatch && req.method === 'POST') {
            let body = '';
            req.on('data', chunk => { body += chunk.toString('utf8'); });
            req.on('end', () => {
                let payload = {};
                ...
                const isResearch = specReviewApiMatch[1] === 'research';
                const isCheck = specReviewApiMatch[2] === 'check';
                const entityId = String(payload.entityId || payload.featureId || payload.researchId || '').trim();
                const agentId = String(payload.agentId || '').trim();
                const repoPath = String(payload.repoPath || '').trim();
                if (!entityId || !agentId || agentId === 'solo') {
                    res.writeHead(400, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: 'entityId and non-solo agentId are required' }));
                    return;
                }
                ...
                try {
                    ...
                    if (!resolvedSpec || !resolvedSpec.path) {
                        ...
                        res.end(JSON.stringify({ error: `Could not resolve ${isResearch ? 'research' : 'feature'} spec ${entityId}` }));
                        return;
                    }
                    const repoName = path.basename(absRepo);
                    const parsed = isResearch
                        ? (path.basename(resolvedSpec.path).match(/^research-(?:(\d+)-)?(.+)\.md$/) || [])
                        : parseFeatureSpecFileName(path.basename(resolvedSpec.path));
                    const desc = isResearch
                        ? (parsed[2] || parsed[1] || undefined)
                        : (parsed && parsed.name) || undefined;
                    const ctx = { absRepo, featureId: entityId, agentId, desc, isResearch, repoName };
                    const result = handleLaunchSpecReview(ctx, {
                        commandName: isResearch
                            ? (isCheck ? 'research-spec-review-check' : 'research-spec-review')
                            : (isCheck ? 'feature-spec-review-check' : 'feature-spec-review'),
                    ...
                    res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(JSON.stringify({ error: `Failed to open spec review session: ${e.message}` }));
                }
            });
            return;
        }

        if (reqPath === '/api/spec-reconcile' && req.method === 'POST') {
        ...
        if (reqPath === '/api/workflows' && req.method === 'GET') {
            try {
        ...
        if (reqPath.startsWith('/api/workflows/') && req.method === 'DELETE') {
            try {
