        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    // Agent-mode actions (eval, review): run in the worktree so agents edit the right branch
                    ...
                    if (AGENT_MODE_TASKS[mode]) {
                        ...
                        // Use the worktree path if available, fall back to main repo for eval-only
                        const taskCwd = (worktreePath && fs.existsSync(worktreePath)) ? worktreePath : absRepo;
                        const sessionName = buildTmuxSessionName(unpadded, task.sessionSuffix, { repo: path.basename(absRepo), desc, entityType: task.label.toLowerCase() });

                        // Record manifest event for review/eval launches
                        try {
                            const manifestMod = require('./manifest');
                            const typePrefix = isResearch ? 'research' : 'feature';
                            manifestMod.writeCoordinatorManifest(absRepo, featureId, {}, {
                                type: `${typePrefix}-${mode}`,
                                actor: `dashboard/${agentId}`
                            }, typePrefix);
                        } catch (_) { /* best-effort event recording */ }

                        // If session already running, just attach — don't kill and restart
                        if (tmuxSessionExists(sessionName)) {
                            openTerminalAppWithCommand(taskCwd, `tmux attach -t ${shellQuote(sessionName)}`, sessionName);
                        } else {
                            const cmd = (isResearch && mode === 'eval')
                                ? buildResearchAgentCommand(agentId, featureId, 'eval')
                                : buildAgentCommand({ agent: agentId, featureId, path: taskCwd, desc }, task.taskType);
                            createDetachedTmuxSession(sessionName, taskCwd, cmd);
                            openTerminalAppWithCommand(taskCwd, `tmux attach -t ${shellQuote(sessionName)}`, sessionName);
                        }
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
