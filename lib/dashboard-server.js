...
const {
    ...
    getNestedValue, setNestedValue, DEFAULT_GLOBAL_CONFIG,
} = require('./config');
...
const DASHBOARD_SETTINGS_SCHEMA = [
    { key: 'backgroundAgents', label: 'Background agents', type: 'boolean' },
    { key: 'terminal', label: 'Terminal', type: 'enum', options: ['warp', 'tmux', 'code', 'cursor', 'terminal', 'iterm2'] },
    { key: 'profile', label: 'Profile', type: 'enum', options: ['web', 'api', 'ios', 'android', 'library', 'generic'] },
    { key: 'security.enabled', label: 'Security enabled', type: 'boolean' },
    { key: 'security.mode', label: 'Security mode', type: 'enum', options: ['enforce', 'warn', 'off'] },
    { key: 'devServer.enabled', label: 'Dev server enabled', type: 'boolean' },
];
...
function buildDashboardSettingsPayload(repoPath) {
    ...
    const effectiveConfig = JSON.parse(JSON.stringify(DEFAULT_GLOBAL_CONFIG || {}));
    Object.keys(globalConfigRaw).forEach(key => {
        if (key === 'agents' && globalConfigRaw.agents) {
            ...
            effectiveConfig.security = { ...(effectiveConfig.security || {}), ...globalConfigRaw.security };
        } else {
            effectiveConfig[key] = globalConfigRaw[key];
        }
    });
    ...
    const settings = DASHBOARD_SETTINGS_SCHEMA.map(def => {
        const globalValue = getNestedValue(globalConfigRaw, def.key);
        const projectValue = getNestedValue(projectConfig, def.key);
        const effectiveValue = getNestedValue(effectiveConfig, def.key);
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    if (AGENT_MODE_TASKS[mode]) {
                        ...
                        // Kill existing session if any, then create fresh
                        try { runTmux(['kill-session', '-t', sessionName], { stdio: 'ignore' }); } catch (_) {}
