...
function runDashboardServer(port, instanceName, serverId, options) {
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    server.listen(port, host, () => {
        ...
        // Recurring features: check on startup then every 24 hours, independent of the poll loop
        const _recurringRepoPath = options.defaultRepoPath || process.cwd();
        const _runRecurringCheck = () => {
            try {
                checkRecurringFeatures(_recurringRepoPath);
            } catch (e) {
                log(`[recurring] Check failed: ${e.message}`);
            }
        };
        _runRecurringCheck();
        setInterval(_runRecurringCheck, 24 * 60 * 60 * 1000).unref();
        try {
        ...
        // F388: scheduled vault backup. Checked hourly; the engine itself
        // decides whether the configured cadence (daily/hourly/weekly) is
        // due, so this is cheap when nothing needs doing.
        const _backup = require('./backup');
        const _runBackupCheck = () => {
            const r = _backup.runScheduledPushIfDue();
            if (!r.ran) return;
            if (r.ok) {
                ...
                try {
                    emitNotification('backup-failed', `Backup push failed: ${r.error}`, { code: r.code });
                } catch (_) { /* notification subsystem not yet ready */ }
            }
        };
        _runBackupCheck();
        setInterval(_runBackupCheck, 60 * 60 * 1000).unref();
        resetIdleTimer();
