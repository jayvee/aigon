...
function runDashboardInteractiveAction(request) {
    ...
    // Post-dispatch verification for feature-start: confirm agents were registered
    if (parsed.action === 'feature-start' && parsed.args.length >= 2) {
        ...
        try {
            ...
            if (fs.existsSync(manifestPath)) {
                ...
                if (missing.length > 0) {
                    payload.ok = false;
                    payload.agentWarning = `Agents not registered in manifest: ${missing.join(', ')}`;
                }
            }
        } catch (e) {
...
function runDashboardServer(port, instanceName, serverId) {
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    '<div style="color:var(--text-tertiary);font-size:12px">Install <code>@aigon/pro</code> to unlock this view.</div>' +
    '</div>';
}`);
                return;
            }
