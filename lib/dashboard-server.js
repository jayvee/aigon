        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    // Eval/review mode: run from main repo with a dedicated session
                    if (mode === 'eval' || mode === 'review') {
                        const sessionSuffix = mode === 'review' ? `review-${agentId}` : 'eval';
                        const taskType = mode === 'review' ? 'review' : 'evaluate';
                        const sessionName = `${path.basename(absRepo)}-f${featureId}-${sessionSuffix}`;
                        const cmd = buildAgentCommand({ agent: agentId, featureId, path: absRepo, desc }, taskType);
                        ...
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, message: `Opened ${mode} for F${featureId}`, sessionName }));
                        return;
                    }
