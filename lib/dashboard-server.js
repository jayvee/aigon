...
const {
    ...
    sanitizeForDns, loadProxyRegistry, isProcessAlive,
} = require('./proxy');
...
function detectRepoProfileName(repoPath) {
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            if (pkg && pkg.scripts && pkg.scripts.dev) return 'web';
        } catch (_) { /* ignore */ }
    }

    const apiMarkers = ['manage.py', 'app.py', 'main.go', 'server.js', 'server.ts'];
    if (apiMarkers.some(marker => fs.existsSync(path.join(repoPath, marker)))) return 'api';

    return 'generic';
}

function isRepoDevServerEnabled(repoPath) {
    const profileName = detectRepoProfileName(repoPath);
    return profileName === 'web' || profileName === 'api';
}

function getRepoAppId(repoPath) {
    const configPath = path.join(repoPath, '.aigon', 'config.json');
    if (fs.existsSync(configPath)) {
        try {
            const cfg = JSON.parse(fs.readFileSync(configPath, 'utf8'));
            if (typeof cfg.appId === 'string' && cfg.appId.trim()) return sanitizeForDns(cfg.appId.trim());
        } catch (_) { /* ignore */ }
    }
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
            if (pkg && typeof pkg.name === 'string' && pkg.name.trim()) return sanitizeForDns(pkg.name.trim());
        } catch (_) { /* ignore */ }
    }

    return sanitizeForDns(path.basename(repoPath));
}

function resolveFeatureWorktreePath(worktreeBaseDir, featureId, agentId) {
    if (!featureId || !agentId || agentId === 'solo') return null;
...
function collectDashboardStatusData() {
    ...
    repos.forEach(repoPath => {
        ...
        const devServerEnabled = isRepoDevServerEnabled(absRepoPath);
        const repoAppId = getRepoAppId(absRepoPath);
                ...
                if (manifestAgents) {
                    ...
                    if (hasFleetAgents) {
                        [...agentSet].sort((a, b) => a.localeCompare(b)).forEach(agent => {
                            ...
                            const tmux = safeTmuxSessionExists(parsed.id, agent);
                            const worktreePath = resolveFeatureWorktreePath(worktreeBaseDir, parsed.id, agent);
                            const devServerUrl = (devServerEnabled && worktreePath)
                                ? getDevProxyUrl(repoAppId, `${agent}-${parsed.id}`)
                                : null;
                            agents.push({
                                id: agent,
        ...
        if (reqPath === '/api/feature-open' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                try {
                    ...
                    // Eval mode: run from main repo with a dedicated eval session
                    if (mode === 'eval') {
                        const evalSessionName = `${path.basename(absRepo)}-f${featureId}-eval`;
                        const evalCmd = buildAgentCommand({ agent: agentId, featureId, path: absRepo, desc }, 'evaluate');

                        // Kill existing eval session if any, then create fresh
                        try { runTmux(['kill-session', '-t', evalSessionName], { stdio: 'ignore' }); } catch (_) {}
                        createDetachedTmuxSession(evalSessionName, absRepo, evalCmd);
                        openTerminalAppWithCommand(absRepo, `tmux attach -t ${shellQuote(evalSessionName)}`, evalSessionName);
                        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                        res.end(JSON.stringify({ ok: true, message: `Opened evaluation for F${featureId}`, sessionName: evalSessionName }));
                        return;
                    }
