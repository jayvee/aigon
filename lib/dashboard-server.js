...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (reqPath.startsWith('/api/peek/') && req.method === 'GET') {
            ...
            const agentId = m[2];
            const url = new URL(req.url, `http://${req.headers.host}`);
            const linesParam = parseInt(url.searchParams.get('lines') || '20', 10) || 20;
            ...
            for (const typeChar of ['f', 'r']) {
                const candidate = `${repo}-${typeChar}${featureId}-${agentId}`;
                if (tmuxSessionExists(candidate)) { sessionName = candidate; break; }
            }
            // Also try eval/review variants
            if (!sessionName) {
                const evalCandidate = `${repo}-f${featureId}-eval-${agentId}`;
                if (tmuxSessionExists(evalCandidate)) sessionName = evalCandidate;
            }
            if (!sessionName) {
                const reviewCandidate = `${repo}-f${featureId}-review-${agentId}`;
                if (tmuxSessionExists(reviewCandidate)) sessionName = reviewCandidate;
            }

            if (!sessionName) {
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ lines: [], sessionName: '', uptime: '', alive: false })); return;
            }
            ...
            try {
                ...
                res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ lines, sessionName, uptime: '', alive: true }));
            } catch (e) {
                res.writeHead(500, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
                res.end(JSON.stringify({ error: e.message }));
            }
