...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    async function pollStatus() {
        ...
        try {
            ...
            latestStatus = await collectDashboardStatusDataAsync({ collectPerf: true });
            const pollTotalMs = Date.now() - pollStart;
            const summary = latestStatus && latestStatus._perf;
            ...
            if (!envTimingOn && latestStatus && latestStatus._perf) delete latestStatus._perf;
        } catch (e) {
    ...
    async function pollRepoStatus(repoPath) {
        ...
        const envTimingOn = process.env.AIGON_DASH_TIMING === '1';
        try {
            latestStatus = refreshRepoInDashboardStatus(latestStatus, repoPath, { collectPerf: true });
            const pollTotalMs = Date.now() - pollStart;
            const absPath = path.resolve(repoPath);
            const repoPerf = (latestStatus && latestStatus.repos || [])
