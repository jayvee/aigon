...
function buildDetailPayload(repoPath, type, id, specPathHint) {
    ...
    const workflowEvents = type === 'feature'
        ...
        : [];
    const detailEvents = workflowEvents.length > 0
        ? workflowEvents
...
function recordManifestEvent(ctx, eventMode) {
    try {
        const manifestMod = require('./manifest');
        const typePrefix = ctx.isResearch ? 'research' : 'feature';
        manifestMod.writeCoordinatorManifest(ctx.absRepo, ctx.featureId, {}, {
            type: `${typePrefix}-${eventMode}`, actor: `dashboard/${ctx.agentId}`
        }, typePrefix);
    } catch (_) { /* best-effort */ }
}
...
function handleLaunchReview(ctx) {
    ...
    recordManifestEvent(ctx, 'review');
