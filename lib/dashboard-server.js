...
function buildDetailPayload(repoPath, type, id, specPathHint) {
    ...
    const typePrefix = type === 'research' ? 'research' : 'feature';
    ...
    for (const name of manifestCandidates) {
        ...
        try {
            rawManifest = fs.readFileSync(p, 'utf8');
    ...
    if (Array.isArray(manifest.agents)) manifest.agents.forEach(a => discoveredAgents.add(String(a)));
    ...
    return {
        manifest,
        rawManifest: rawManifest || JSON.stringify(manifest || {}, null, 2),
        agentFiles,
        rawAgentFiles,
        logExcerpts,
