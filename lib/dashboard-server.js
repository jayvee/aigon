...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (reqPath === '/api/insights' && req.method === 'GET') {
            ...
            const repoPath = resolveRequestedRepoPathOrRespond(res, String(parsedUrl.searchParams.get('repoPath') || '').trim());
            if (!repoPath) return;
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    server.on('error', (err) => {
        ...
        if (err.code === 'EADDRINUSE') {
            // Port is already held by another process (often a stale `aigon dashboard`
    ...
    server.listen(port, host, () => {
        ...
        resetIdleTimer();
        // Only open browser on first start, not on restarts
        if (!options._isRestart) {
