...
function collectDashboardStatusData() {
    ...
    repos.forEach(repoPath => {
        ...
        // Build map of feature ID → log file paths from logs/selected/ and logs/alternatives/
        const logPathsByFeatureId = {};
        const selectedLogsDir = path.join(mainLogsDir, 'selected');
        const alternativesLogsDir = path.join(mainLogsDir, 'alternatives');
        [selectedLogsDir, alternativesLogsDir].forEach(dir => {
            try {
                if (!fs.existsSync(dir)) return;
                fs.readdirSync(dir)
                    .filter(f => /^feature-\d+-.+-log\.md$/.test(f))
                    .forEach(f => {
                        ...
                        logPathsByFeatureId[fid].push(path.join(dir, f));
                    });
            } catch (_) { /* ignore */ }
        });
    ...
    function pollStatus() {
        ...
        (latestStatus.repos || []).forEach(repo => {
            ...
            (repo.features || []).forEach(feature => {
                ...
                const namedAgents = agents.filter(a => a.id && a.id !== 'solo');
                const allSubmitted = namedAgents.length >= 2 && agents.every(a => a.status === 'submitted');
