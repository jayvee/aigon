...
const { buildFeatureIndex, buildDependencyGraph, buildDependencyGraphAsync, buildFeatureDependencySvg } = require('./feature-dependencies');
...
// `recurring` engine moved to @aigon/pro (feature 236). Pro starts its own
// recurring-features poller inside register().
const budgetPoller = require('./budget-poller');
const quotaPoller = require('./quota-poller');
// `scheduled-kickoff` engine moved to @aigon/pro (feature 236). Pro starts
// its own poller inside register(); OSS no longer needs the import.

/**
 * Resolve a file under @aigon/pro/dashboard (e.g. backup-sync.js).
 * Tries `getPro().dashboardDir` first, then `dirname(require.resolve('@aigon/pro/package.json'))/dashboard`.
 * REGRESSION: without the fallback, linked or hoisted installs could report Pro available while
 * `path.join(getPro().dashboardDir, …)` pointed at a path where new dashboard files were absent.
 */
function resolveProDashboardAsset(fileName) {
    ...
    try {
        const pkgRoot = path.dirname(require.resolve('@aigon/pro/package.json'));
...
function handleLaunchSpecReview(ctx, options = {}) {
    ...
    const recordStart = role === 'spec-revise'
        ? workflowEngine.recordSpecRevisionStarted(absRepo, entityTypeFull, featureId, { checkerId: agentId })
...
async function handleLaunchCloseResolve(ctx) {
    ...
    // Engine-first: append the recovery event before spawning the tmux session so the
    // dashboard never observes a "close session running but no recovery state" half-state.
    const wfAdapter = require('./workflow-snapshot-adapter');
    const snapForReturn = wfAdapter.readWorkflowSnapshotSync(absRepo, 'feature', featureId);
...
function buildDashboardHtml(initialData, instanceName, templateRootOverride, configs = {}) {
    ...
    // F454: thread globalConfig + projectConfig so the bootstrap AIGON_AGENTS
    // payload includes resolved default-model strings — the autonomous modal
    // reads these synchronously instead of awaiting /api/settings.
    ...
    const serializedDefaultAgent = escapeForHtmlScript(getDefaultAgent());
    let htmlTemplate;
    ...
    return htmlTemplate
        ...
        .replace('${AIGON_DEFAULT_AGENT}', () => serializedDefaultAgent);
}
...
async function runDashboardInteractiveAction(request) {
    ...
    if (actionId && activeActionLogs) {
        ...
        activeActionLogs.set(actionId, { logPath, lines: [], done: false });
...
function runDashboardServer(port, instanceName, serverId, options) {
    ...
    function refreshLatestStatus() {
        try {
            ...
            if (process.env.AIGON_DASH_TIMING === '1' && latestStatus && latestStatus._perf) {
                const summary = latestStatus._perf;
                const top = (summary.repos || [])
                    ...
                    .join(', ');
                log(`[perf] poll summary total=${summary.totalMs}ms repos=${(summary.repos || []).length}${top ? ` top=[${top}]` : ''}`);
            }
        } catch (err) {
            ...
            if (!latestStatus) latestStatus = emptyDegradedStatus(err);
        }
        return latestStatus;
    }
    refreshLatestStatus();
    ...
    const NOTIFICATION_TYPES = ['agent-waiting', 'agent-submitted', 'agent-needs-attention', 'all-submitted', 'all-research-submitted', 'error'];
    ...
    // F460: 10s → 20s. Aligns with `templates/dashboard/js/state.js` POLL_MS so
    // the browser doesn't request /api/status twice per server poll cycle.
    const POLL_INTERVAL_ACTIVE_MS = 20_000;
    const POLL_INTERVAL_IDLE_MS   = 60_000;

    function hasActiveEntities() {
        return (latestStatus.repos || []).some(repo =>
            (repo.features || []).some(f => f.stage === 'in-progress' || f.stage === 'in-evaluation') ||
            (repo.research || []).some(r => r.stage === 'in-progress')
        );
    }
        ...
        (latestStatus.repos || []).forEach(repo => {
            ...
            (repo.features || []).forEach(feature => {
                ...
                if (featureAllSubmitted && !allSubmittedNotified.has(featureKey)) {
                    ...
                    emitNotification('all-submitted', `All complete #${feature.id} ${feature.name} — ready for eval · ${repoShort}`, notifMeta({ featureId: feature.id }));
                }
            });
            ...
            (repo.research || []).forEach(item => {
                ...
                if (researchAllSubmitted && !allSubmittedNotified.has(researchKey)) {
                    ...
                    emitNotification('all-research-submitted', `All complete R#${item.id} ${item.name} — ready for synthesis · ${repoShort}`, notifMeta({ researchId: item.id }));
                }
            });
        });
    ...
    const server = http.createServer((req, res) => {
        ...
        if (!isProAvailable() && reqPath.startsWith('/api/benchmarks')) {
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
            res.end(JSON.stringify({ proRequired: true, error: 'Performance benchmarks require @aigon/pro' }));
            return;
        }
        ...
        if (reqPath.startsWith('/js/') || reqPath === '/styles.css') {
            ...
            if (reqPath === '/js/pro-reports.js') {
                const proFile = resolveProDashboardAsset('pro-reports.js');
                if (proFile) {
            ...
            if (reqPath === '/js/insights-dashboard.js' || reqPath === '/js/amplification.js') {
                const proFile = resolveProDashboardAsset('insights-dashboard.js');
                if (proFile) {
                    res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(fs.readFileSync(proFile, 'utf8'));
                    return;
                }
                // Pro not available — serve stub that shows upgrade message (/js/amplification.js kept as legacy URL)
                res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                res.end(`function renderInsightsDashboard() {
  ...
  if (c) c.innerHTML = '<div class="stats-empty-msg" style="text-align:center;padding:40px 20px">' +
    ...
    '</div>';
            ...
            if (reqPath === '/js/benchmark-matrix.js') {
                const proFile = resolveProDashboardAsset('benchmark-matrix.js');
                if (proFile) {
                    res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(fs.readFileSync(proFile, 'utf8'));
                    return;
                ...
                res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                const proMissingBenchUi = isProAvailable();
                res.end(proMissingBenchUi ? `'use strict';
window.AigonProBenchmarkMatrix = { mount: function (section) {
  ...
  w.innerHTML = '<div class="amp-empty" style="padding:20px 0;text-align:center">' +
    ...
    '<div style="color:var(--text-secondary);font-size:12px">Pro is active but <code>dashboard/benchmark-matrix.js</code> is missing from your <code>@aigon/pro</code> install. Update or relink the package.</div>' +
    '</div>';
...
window.AigonProBenchmarkMatrix = { mount: function (section) {
  var w = document.createElement('div');
  w.className = 'matrix-section';
  w.innerHTML = '<div class="settings-empty">Install <code>@aigon/pro</code> and restart the dashboard to load latest <code>aigon perf-bench</code> results here.</div>';
  section.appendChild(w);
}};`);
                return;
            }
            if (reqPath === '/js/backup-sync.js') {
                const proFile = resolveProDashboardAsset('backup-sync.js');
                if (proFile) {
                    res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(fs.readFileSync(proFile, 'utf8'));
                    return;
                ...
                res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                const proMissingSyncUi = isProAvailable();
                res.end(proMissingSyncUi ? `function renderBackupSync() {
  ...
  c.innerHTML = '<div class="amp-empty" style="padding:28px 0;text-align:center">' +
    ...
    '<div style="color:var(--text-secondary);font-size:12px">Pro is active but the Aigon Sync dashboard script is missing from your <code>@aigon/pro</code> install. ' +
    'Run <code>npm link @aigon/pro</code> from your checkout or reinstall so <code>dashboard/backup-sync.js</code> is present. ' +
  ...
  c.innerHTML = '<div class="amp-empty" style="padding:28px 0;text-align:center">' +
    '<div style="font-size:15px;font-weight:600;margin-bottom:8px">Aigon Sync <span style="font-size:10px;opacity:0.7">(Pro)</span></div>' +
    '<div style="color:var(--text-secondary);font-size:12px">Install <code>@aigon/pro</code> for remote vault sync. Manual backup does not require Pro.</div>' +
    '</div>';
}`);
                return;
            }
            if (reqPath === '/js/scheduled-features.js') {
                const proFile = resolveProDashboardAsset('scheduled-features.js');
                if (proFile) {
                    res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(fs.readFileSync(proFile, 'utf8'));
                    return;
                ...
                res.end(proMissingSchedUi ? `function renderScheduledFeatures() {
  ...
  c.innerHTML = '<p class="settings-empty" style="margin-top:4px;font-size:12px;color:var(--text-tertiary)">' +
    'Pro is active but <code>dashboard/scheduled-features.js</code> is missing from <code>@aigon/pro</code>. Update or npm-link your Pro checkout.</p>';
}` : `function renderScheduledFeatures() {
  var c = document.getElementById('scheduled-features-view');
  if (!c) return;
  c.innerHTML = '<p class="settings-empty" style="margin-top:4px;font-size:12px;color:var(--text-tertiary)">' +
    'Install <code>@aigon/pro</code> for recurring batch scheduling details here.</p>';
}`);
                return;
            }
            if (reqPath === '/js/failover-dashboard.js') {
                const proFile = resolveProDashboardAsset('failover-dashboard.js');
                if (proFile) {
                    res.writeHead(200, { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' });
                    res.end(fs.readFileSync(proFile, 'utf8'));
                    return;
        ...
        const html = buildDashboardHtml(latestStatus, instanceName, isPreview ? templateRoot : null, { globalConfig });
    ...
    server.listen(port, host, () => {
        ...
        function scheduleNextPoll() {
            const active = hasActiveEntities();
            const delay = active ? POLL_INTERVAL_ACTIVE_MS : POLL_INTERVAL_IDLE_MS;
            log(`Next poll in ${delay / 1000}s (${active ? 'active' : 'idle'})`);
        ...
        try {
            budgetPoller.startBudgetPoller({ repoPath: process.cwd(), log });
        } catch (e) {
            log(`Budget poller failed to start: ${e && e.message}`);
        }
        try {
            quotaPoller.startQuotaPoller({
                repoPath: process.cwd(),
                log,
                onRefresh: emitServerEvent,
            });
        } catch (e) {
            log(`Quota poller failed to start: ${e && e.message}`);
        }
        resetIdleTimer();
