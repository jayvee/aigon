...
const VERB_TO_TEMPLATE = {
    ...
    'review-check': 'feature-code-review-check',
};
...
function resolveAgentPromptBody({ agentId, verb, featureId, extraArgs = '', cliConfig = null }) {
    ...
    if (!template.includes('{featureId}')) {
        return resolveCxPromptBody(verb, featureId, extraArgs, agentId);
    }
 ...
 */
function resolveCxPromptBody(verb, featureId, extraArgs = '', agentId = 'cx') {
    ...
    return resolveCxCommandBody(templateName, argsString, agentId);
}

function resolveCxCommandBody(commandName, argsString = '', agentId = 'cx') {
 ...
 *   code-review-check ${featureId}` sourced from the agent's `reviewCheckPrompt`.
...
function buildReviewCheckFeedbackPrompt(agentId, featureId, { loadAgentConfig: loader } = {}) {
    ...
    const skillPath = buildAgentSkillPath(agentConfig, 'feature-code-review-check');
