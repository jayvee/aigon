...
/**
 * Build the mid-session directive the AutoConductor injects when the reviewer
 * approved the feature *and* left reviewer-authored output on the branch (or
 * `ESCALATE:` entries in the log). Frames the next step as the implementor's
 * disposition of reviewer output — accept / revert / modify — rather than as
 * "the reviewer requested revision". Completion signal is unchanged
 * (`aigon agent-status revision-complete` or `feedback-addressed`).
 */
function buildPostReviewDispositionPrompt(agentId, featureId, { loadAgentConfig: loader } = {}) {
    const loadCfg = loader || loadAgentConfig;
    const agentConfig = loadCfg(agentId) || {};
    const slashCommandInvocable = typeof agentConfig?.capabilities?.resolvesSlashCommands === 'boolean'
        ? agentConfig.capabilities.resolvesSlashCommands
    ...
    if (slashCommandInvocable) {
        ...
        return `The reviewer is done and approved this feature, but they committed changes or notes on the branch. As the implementing agent, you decide how to dispose of the reviewer's output — accept, revert, or modify. Please run ${invocation} to inspect what the reviewer did and pick a disposition, then signal completion with: aigon agent-status revision-complete`;
    }
    const skillPath = buildAgentSkillPath(agentConfig, 'feature-code-revise');
    return `The reviewer is done and approved this feature, but they committed changes or notes on the branch. As the implementing agent, you decide: accept, revert, or modify. Read \`${skillPath}\` and follow it for feature ${featureId}. When done: aigon agent-status revision-complete`;
}
