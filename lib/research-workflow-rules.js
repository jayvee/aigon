...
const RESEARCH_STATE_META = Object.freeze({
    ...
    implementing: { lane: 'in-progress', phase: 'research', label: 'Researching', severity: 'active' },
    ready: { lane: 'in-progress', phase: 'decision', label: 'Findings ready', severity: 'normal' },
    code_review_in_progress: { lane: 'in-progress', phase: 'findings-review', label: 'Reviewing findings', severity: 'active' },
    code_review_complete: { lane: 'in-progress', phase: 'findings-review', label: 'Findings review complete', severity: 'normal' },
    code_revision_in_progress: { lane: 'in-progress', phase: 'findings-revision', label: 'Revising findings', severity: 'active' },
    code_revision_complete: { lane: 'in-progress', phase: 'findings-revision', label: 'Findings revision complete', severity: 'normal' },
    evaluating: { lane: 'in-evaluation', phase: 'evaluation', label: 'Evaluating findings', severity: 'active' },
    closing: { lane: 'in-evaluation', phase: 'closing', label: 'Closing research', severity: 'active' },
...
const RESEARCH_ENGINE_STATES = Object.freeze({
    ...
    inbox: [
        { event: 'research.spec_review.started', to: 'spec_review_in_progress', effect: 'captureSpecReviewReturnLifecycle' },
        { event: 'research.spec_revision.started', to: 'spec_revision_in_progress', effect: 'captureSpecReviewReturnLifecycle' },
    ...
    spec_review_in_progress: [
        ...
        { event: 'research.spec_review.cancelled', to: 'inbox', guard: 'specReviewReturnToInbox' },
        { event: 'research.spec_review.cancelled', to: 'backlog' },
        { event: 'research.spec_revision.started', to: 'spec_revision_in_progress', effect: 'captureSpecReviewReturnLifecycle' },
    ],
    spec_review_complete: [],
    spec_revision_in_progress: [
        { event: 'research.spec_revision.completed', to: 'spec_revision_complete' },
        { event: 'research.spec_revision.cancelled', to: 'inbox', guard: 'specReviewReturnToInbox' },
        { event: 'research.spec_revision.cancelled', to: 'backlog' },
    ],
    spec_revision_complete: [],
    implementing: [
        { event: 'research.pause', to: 'paused' },
    ...
    code_review_in_progress: [
        ...
        { event: 'research.code_review.cancelled', to: 'ready' },
        { event: 'research.close', to: 'closing' },
        { event: 'research.pause', to: 'paused' },
    ],
    code_review_complete: [],
...
const RESEARCH_ACTION_CANDIDATES = Object.freeze([
    ...
    {
        kind: ManualActionKind.RESEARCH_UNPRIORITISE,
        ...
        metadata: {
            confirmationMessage: 'Return this research topic to the inbox (slug filename, no numeric id) until you prioritise again?',
        },
    },
    {
        ...
        bypassMachine: true,
        category: 'session',
        // Same widening as feature-side: allow Open Terminal whenever the
        // research agent exists, including post-completion states
        // (research-complete) where the tmux session is still alive.
    ...
    {
        kind: ManualActionKind.RESEARCH_SCHEDULE,
        label: 'Schedule',
        eventType: null,
        recommendedOrder: 9,
        bypassMachine: true,
    ...
    },
    {
        ...
        bypassMachine: true,
        category: ActionCategory.LIFECYCLE,
        guard: ({ context }) => context.currentSpecState === 'paused',
    },
    {
        kind: ManualActionKind.RESEARCH_NUDGE,
        label: 'Nudge agent...',
        eventType: null,
        recommendedOrder: 36,
        bypassMachine: true,
        ...
        guard: ({ context }) => {
            ...
            return context.currentSpecState !== 'done'
                && agents.some(agent => ['running', 'idle', 'waiting', 'ready', 'researching'].includes(agent.status))
    ...
    {
        kind: ManualActionKind.RESEARCH_CANCEL_SPEC_REVIEW,
        label: 'Cancel review',
        eventType: 'research.spec_review.cancelled',
        recommendedOrder: 48,
        category: ActionCategory.SPEC_REVIEW,
        guard: ({ context }) => context.currentSpecState === 'spec_review_in_progress',
    },
    {
        kind: ManualActionKind.RESEARCH_CANCEL_SPEC_REVISION,
        label: 'Cancel revision',
        eventType: 'research.spec_revision.cancelled',
        recommendedOrder: 48,
        category: ActionCategory.SPEC_REVIEW,
        guard: ({ context }) => context.currentSpecState === 'spec_revision_in_progress',
    },
    {
        kind: ManualActionKind.RESEARCH_CANCEL_CODE_REVIEW,
        label: 'Cancel code review',
        eventType: 'research.code_review.cancelled',
        recommendedOrder: 48,
        category: ActionCategory.LIFECYCLE,
    ...
    },
    {
        ...
        mode: 'agent',
        modeFilter: 'solo',
        guard: ({ context }) => ['implementing', 'ready'].includes(context.currentSpecState),
    },
    {
    ...
    },
    {
        ...
        bypassMachine: true,
        category: ActionCategory.LIFECYCLE,
        guard: ({ context }) => ['implementing', 'ready', 'code_review_in_progress', 'code_revision_in_progress', 'evaluating', 'paused', 'closing'].includes(context.currentSpecState),
        metadata: {
    ...
    },
    {
        ...
        bypassMachine: true,
        category: ActionCategory.LIFECYCLE,
        guard: ({ context }) => ['inbox', 'backlog', 'paused'].includes(context.currentSpecState),
        metadata: {
            destructive: true,
...
const RESEARCH_INTERACTION_DEFINITION = Object.freeze({
    definitionVersion: 2,
    states: RESEARCH_ENGINE_STATES,
    transientStates: RESEARCH_TRANSIENT_STATES,
    stateMeta: RESEARCH_STATE_META,
    actions: Object.freeze([...RESEARCH_ACTION_CANDIDATES, ...RESEARCH_INFRA_CANDIDATES]),
});

module.exports = {
    RESEARCH_INTERACTION_DEFINITION,
    RESEARCH_STATE_META,
