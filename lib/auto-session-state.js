...
// A freshly spawned tmux session may not yet be visible to a caller reading
// from a cached `tmux list-sessions` snapshot (dashboard-status-helpers.js
// caches for 3s). Without this grace window, a status poll landing in that
// gap treats "not in the stale cache yet" as "session died", stomping a
// bogus failed/auto-session-lost onto state a set conductor is about to
// read — pausing the whole set even though the session is actually alive.
const RECONCILE_GRACE_MS = 10000;
...
function reconcileStaleFeatureAutoState(repoPath, featureId, autoSessionRunning) {
    ...
    const startedAt = persisted.startedAt ? Date.parse(persisted.startedAt) : NaN;
    if (Number.isFinite(startedAt) && (Date.now() - startedAt) < RECONCILE_GRACE_MS) {
        return persisted;
    }
    const patch = {};
...
/**
 * A set conductor is a detached tmux process. Unlike a feature controller,
 * the old read path only healed paused runs; it left a missing `running`
 * conductor looking live forever and hid the recovery controls.
 */
function reconcileStaleSetAutoSessionState(repoPath, setSlug, sessionAlive) {
    if (sessionAlive) return null;
    const persisted = readSetAutoState(repoPath, setSlug);
    if (!persisted || !persisted.running || String(persisted.status || '') !== 'running') return persisted;
    const startedAt = persisted.startedAt ? Date.parse(persisted.startedAt) : NaN;
    if (Number.isFinite(startedAt) && (Date.now() - startedAt) < RECONCILE_GRACE_MS) return persisted;
    return writeSetAutoStateSync(repoPath, setSlug, {
        status: 'interrupted',
        running: false,
        reason: 'set-auto-session-lost',
