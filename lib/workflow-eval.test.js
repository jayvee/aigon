...
testAsync('recovers missing eval effects after eval was already requested', async () => {
  const tmp = makeTmpDir();
  try {
    await wf.startFeature(tmp, '68', 'fleet', ['cc', 'gg']);
    await wf.signalAgentReady(tmp, '68', 'cc');
    await wf.signalAgentReady(tmp, '68', 'gg');
    await wf.requestFeatureEval(tmp, '68');

    const inProgressDir = path.join(tmp, 'docs', 'specs', 'features', '03-in-progress');
    const inEvalDir = path.join(tmp, 'docs', 'specs', 'features', '04-in-evaluation');
    fs.mkdirSync(inProgressDir, { recursive: true });
    const specFromPath = path.join(inProgressDir, 'feature-68-demo.md');
    const specToPath = path.join(inEvalDir, 'feature-68-demo.md');
    fs.writeFileSync(specFromPath, '# Feature 68\n');

    const result = await runWorkflowEval(tmp, '68', {
      specFromPath,
      specToPath,
    });
    ...
    const evalStubPath = path.join(tmp, '.aigon', 'workflows', 'features', '68', 'eval-started.md');
    assert.strictEqual(fs.existsSync(evalStubPath), true);
  } finally {
    cleanTmpDir(tmp);
  }
