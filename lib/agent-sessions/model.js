...
function normalizeAgent(rawAgent, fallbackAgent, source) {
    const agent = isPlainObject(rawAgent) ? rawAgent : { id: fallbackAgent };
    if (!agent.id || typeof agent.id !== 'string') {
        throw invalidRecord('Missing agent id', source, ['agent.id']);
    }
    return compactObject({
        id: agent.id,
        slotAgentId: agent.slotAgentId,
...
function normalizeAgentSessionRecord(raw, source = 'record') {
    ...
    const agent = normalizeAgent(raw.agent, raw.agentId, source);
...
function validateAgentSessionStartRequest(request) {
    try {
        ...
        const raw = {
            ...request,
            category,
            state: request && request.state ? request.state : SESSION_STATES.REQUESTED,
        };
