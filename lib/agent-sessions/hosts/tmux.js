...
function createTmuxSessionHost(deps = {}) {
    ...
    function submitMessage(target, text, transport) {
        ...
        for (let attempt = 0; attempt < attempts; attempt += 1) {
            ...
            const composer = extractComposerText(paneTail) || extractLastPromptLine(paneTail);
            if (composer && !paneContainsMessage(composer, text)) return { ok: true, paneTail };
        }
        return { ok: false, paneTail };
    }
    ...
    function deliverOperatorMessage(sessionRef, message, options = {}) {
        ...
        loadBuffer(text);
        pasteBuffer(target);
        let confirmation = { ok: false, paneTail: '' };
        try {
            confirmation = confirmDelivery(target, text);
        } catch (error) {
            confirmation = {
                ok: false,
                paneTail: error && error.paneTail ? String(error.paneTail) : '',
            };
        }
        let submit;
        try {
            submit = submitMessage(target, text, transport);
        } catch (error) {
            error.paneTail = error.paneTail || confirmation.paneTail || '';
            throw error;
        }
        if (!submit.ok) {
