...
function createTmuxSessionHost(deps = {}) {
    ...
    const attachSessionCapture = deps.attachSessionCapture
        || ((name, meta) => lazyWorktree().attachSessionCapture(name, meta));
    ...
    function startSession(request) {
        ...
        // Transcript-capture side effects (provider session binding + opt-in
        // pipe-pane). Owned by worktree/config/transcript-store; out of scope to
        // redesign here. Only meaningful for entity sessions with an agent.
        const repoPath = paths.repoPath || req.repoPath;
        const agentId = req.agent && req.agent.id;
        if (repoPath && agentId) {
            try {
                attachSessionCapture(sessionName, {
                    repoPath,
                    worktreePath: paths.worktreePath || cwd,
                    agent: agentId,
                    entityType: req.entityType || (req.entity && req.entity.type),
                    entityId: req.entityId || (req.entity && req.entity.id),
                    role: req.role || 'do',
