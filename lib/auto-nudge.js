...
function getIdleSec(input, nowMs) {
    ...
    // Reviewer sessions: use time-since-review-started when idleAtPrompt is unavailable
    // (agents like Cursor have no confirmed idle pattern so captureAndDetectIdle returns null).
    if (input.role === 'review' && !input.idleAtPrompt) {
        const startMs = new Date(input.updatedAt || 0).getTime();
...
function maybeDispatchNudge(repoPath, input, cfg, state, deps = {}) {
    if (state.nudged || state.nudgeInFlight || !cfg.enabled || !cfg.agentEnabled) return;
    state.nudged = true;
