...
function reconcileEntitySpec(repoPath, entityType, entityId, options = {}) {
    ...
    try {
        ...
        if (!/unknown-lifecycle/.test(String(error && error.message || ''))) throw error;
        const fallbackVisible = featureSpecResolver.listVisibleEntitySpecs(repoPath, normalizedEntityType, normalizedId)[0] || null;
        return {
            entityType: normalizedEntityType,
            entityId: normalizedId,
            snapshot,
            lifecycle,
            visibleStage: fallbackVisible ? fallbackVisible.stage : null,
            currentPath: fallbackVisible ? fallbackVisible.path : null,
            expectedPath: path.join(repoPath, '.aigon', 'workflows', `${normalizedEntityType}s`, String(normalizedId), path.basename((fallbackVisible && fallbackVisible.path) || 'spec.md')),
            driftDetected: false,
            moved: false,
