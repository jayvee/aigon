...
const FEEDBACK_STATUS_TO_FOLDER = Object.freeze({
    inbox: feedbackFolder(1, 'inbox'),
    triaged: feedbackFolder(2, 'triaged'),
    actionable: feedbackFolder(3, 'actionable'),
    done: feedbackFolder(4, 'done'),
    'wont-fix': feedbackFolder(5, 'wont-fix'),
    duplicate: feedbackFolder(6, 'duplicate'),
});
...
function reconcileEntitySpec(repoPath, entityType, entityId, options = {}) {
    ...
    if (specLifecycle.isStableSpecLayout(repoPath)) {
        const snapshot = options.snapshot || readWorkflowSnapshot(repoPath, normalizedEntityType, normalizedId);
        ...
        if (options.dryRun !== false) {
            return {
                ...
                lifecycle,
                visibleStage: lifecycle,
                currentPath: snapshot ? snapshot.specPath : null,
                expectedPath: specLifecycle.findCanonicalSpecPath(repoPath, normalizedEntityType, normalizedId),
                driftDetected: false,
                moved: false,
                skipped: 'stable-layout-view',
            };
        }
    ...
    let expectedPath;
    try {
        visibleSpec = featureSpecResolver.resolveEntitySpec(repoPath, normalizedEntityType, normalizedId, { snapshot });
        expectedPath = getSpecPathForEntity(repoPath, normalizedEntityType, normalizedId, lifecycle, { snapshot });
    } catch (err) {
        // REGRESSION: F276 made getSpecStateDirForEntity throw on unknown lifecycles,
        // which broke every caller of reconcileEntitySpec that received a snapshot
        // with a lifecycle the engine didn't recognize (e.g. legacy states, hand-edited
        // snapshots). Swallow that one class of error so read paths don't 500.
        if (err && typeof err.message === 'string' && err.message.includes('unknown-lifecycle')) {
            return {
                entityType: normalizedEntityType,
                entityId: normalizedId,
                snapshot,
