...
function resolveFeedbackVisibleSpec(repoPath, entityId) {
    ...
    const folders = Object.values(utils.FEEDBACK_STATUS_TO_FOLDER);
    ...
    for (const folder of folders) {
        ...
        for (const file of files) {
            ...
            return {
                ...
                file,
                folder,
                stage: utils.FEEDBACK_FOLDER_TO_STATUS[folder] || null,
            };
        }
    }
...
function resolveFeedbackLifecycle(currentPath) {
    ...
    try {
        const parsed = utils.parseFrontMatter(fs.readFileSync(currentPath, 'utf8'));
        return feedbackLib.normalizeFeedbackStatus(parsed.data.status) || 'inbox';
    } catch (_) {
        return 'inbox';
    }
}
...
function getFeedbackExpectedPath(repoPath, currentPath, lifecycle) {
    const folder = utils.FEEDBACK_STATUS_TO_FOLDER[feedbackLib.normalizeFeedbackStatus(lifecycle) || 'inbox'];
