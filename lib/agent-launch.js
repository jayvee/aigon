...
function isKnownEffortValue(agentId, value) {
    const effortValue = value == null ? '' : String(value).trim();
    if (!effortValue) return false;
    const options = agentRegistry.getEffortOptions(agentId);
    if (!options.length) return false;
    return options.some((opt) => opt.value != null && String(opt.value) === effortValue);
}
...
function resolveLaunchTriplet({ agentId, slotAgentId, snapshot, stageDefaultModel, launcherModel, launcherEffort }) {
    ...
    if (launcherE) {
        effort = launcherE;
        effortSource = 'launcher';
    } else if (effortOverride && isKnownEffortValue(agentId, effortOverride)) {
        effort = effortOverride;
        effortSource = 'event';
    }

    if (agentId === 'op' && model) {
        const { resolveOpModel, refreshIfStale } = require('./op-models');
        refreshIfStale().catch(() => {});
        model = resolveOpModel(model);
    }
 ...
 */
function buildAgentLaunchInvocation({ agentId, slotAgentId, snapshot, stageDefaultModel, launcherModel, launcherEffort }) {
...
function buildModelArgTokens(agentId, model) {
    let resolvedModel = model;
    if (resolvedModel && agentId === 'op') {
        const { resolveOpModel } = require('./op-models');
        resolvedModel = resolveOpModel(resolvedModel);
    }
    if (!resolvedModel || !agentRegistry.supportsModelFlag(agentId)) return [];
    const modelFlag = agentRegistry.getModelFlag(agentId) || '--model';
    return [modelFlag, resolvedModel];
}
