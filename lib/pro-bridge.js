...
const { MAX_BODY_BYTES } = require('./dashboard-security');
...
function buildBridgeApi(helpers) {
    return {
        ...
        helpers: {
            ...helpers,
            // Failover extension points (F421):
            // registerExhaustionHandler — Pro registers a fn(ctx) to perform
            //   the actual switch when a token-exhaustion signal is detected.
            registerExhaustionHandler: supervisor.registerExhaustionHandler,
            // registerFailoverActionAppender — Pro registers a fn(repoPath, entityType,
            //   entityId, snapshot, agents, validActions) => validActions that injects
            //   switch-agent menu items into the dashboard validActions list.
            registerFailoverActionAppender: workflowReadModel.registerFailoverActionAppender,
        },
    };
}
 ...
 *   (e.g. `loadProjectConfig`, `resolveRequestedRepoPath`,
 *   `readConductorReposFromGlobalConfig`, `sendJson`).
