...
function getBoardAction(typePrefix, folder, item, worktreeMap, currentBranch, repoPath = process.cwd()) {
    ...
    // Read actions from workflow-core engine snapshot (single source of truth)
    const snapshot = item.id
        ? workflowSnapshotAdapter.readWorkflowSnapshotSync(repoPath, entityType, String(item.id))
        : null;
    if (snapshot) {
