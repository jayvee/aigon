 ...
 */
function inferBoardAgents(repoPath, typePrefix, stage, item, worktreeMap, currentBranch) {
    if (!item.id || (stage !== '03-in-progress' && stage !== '04-in-evaluation')) return [];

    const stateDir = path.join(repoPath, '.aigon', 'state');
    const prefix = typePrefix;
    const entityId = String(item.id);
    const paddedId = entityId.padStart(2, '0');
    const statusByAgent = {};
    const agents = [];
    ...
    try {
        if (fs.existsSync(stateDir)) {
            fs.readdirSync(stateDir)
                .filter(f => f.startsWith(`${prefix}-${entityId}-`) && f.endsWith('.json') && !f.startsWith(`${prefix}-${entityId}.json`))
                .forEach(f => {
                    const match = f.match(new RegExp(`^${prefix}-${entityId}-(.+)\\.json$`));
                    if (!match) return;
                    try {
                        const parsed = JSON.parse(fs.readFileSync(path.join(stateDir, f), 'utf8'));
                        statusByAgent[match[1]] = parsed.status || 'implementing';
                    } catch (_) { /* ignore */ }
                });
        }
    } catch (_) { /* ignore */ }

    const worktrees = (worktreeMap[entityId] || []).filter(w => w.type === (typePrefix === 'feature' ? 'feature' : 'research'));
    worktrees.forEach(w => {
        agents.push({
            id: w.agent,
            status: statusByAgent[w.agent] || 'implementing',
            tmuxRunning: false,
            tmuxSession: null
        });
    });
    ...
    const branchName = `${typePrefix}-${paddedId}-${item.name}`;
    const legacyBranchName = `${typePrefix}-${entityId}-${item.name}`;
    let branchExists = currentBranch === branchName || currentBranch === legacyBranchName;
    if (!branchExists) {
        try {
            ...
            try {
                execSync(`git rev-parse --verify ${legacyBranchName}`, { stdio: 'pipe' });
                branchExists = true;
            } catch (_) { /* ignore */ }
        }
    }

    if (branchExists || statusByAgent.solo) {
        agents.push({
            id: 'solo',
            status: statusByAgent.solo || 'implementing',
            tmuxRunning: false,
            tmuxSession: null
        });
    }
...
function getBoardAction(typePrefix, folder, item, worktreeMap, currentBranch, repoPath = process.cwd()) {
    ...
    const entityType = typePrefix === 'feature' ? 'feature' : 'research';
    const stage = stageByFolder[folder];
    if (!stage) return null;

    // Prefer workflow-core snapshot for actions when available
    if (entityType === 'feature' && item.id) {
        const snapshot = workflowSnapshotAdapter.readFeatureSnapshotSync(repoPath, String(item.id));
        if (snapshot) {
            const cmd = workflowSnapshotAdapter.snapshotToBoardCommand(entityType, item.id, snapshot);
            if (cmd) return cmd;
        }
    }

    // Fallback: legacy state machine
    const agents = inferBoardAgents(repoPath, typePrefix, folder, item, worktreeMap, currentBranch);
    const model = workflowReadModel.getWorkflowReadModel(entityType, item.id || item.name, stage, agents);
    const action = model.recommendedActions[0] || model.validActions[0] || null;
    if (!action) return null;

    return workflowReadModel.formatBoardCommand(entityType, item.id || item.name, action);
}
...
function displayListSection(title, typeConfig, options) {
    ...
    typeConfig.folders.forEach(folder => {
        ...
        files.forEach(file => {
            ...
            if (showActions) {
                const action = getBoardAction(typeConfig.prefix, folder, { id: itemId, name: itemName }, worktreeMap, currentBranch);
                if (action) {
