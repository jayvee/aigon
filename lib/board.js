...
function collectBoardItems(typeConfig, folderFilter, repoPath = process.cwd()) {
    ...
    [...deduped.values()].forEach(item => {
        ...
        let blockedBy = [];
        if (typeConfig.prefix === 'feature' && resolvedFolder === '02-backlog' && item.id) {
...
function displayKanbanSection(title, typeConfig, options, mapping = {}, startLetterIndex = 0) {
    ...
    for (let i = 0; i < maxRows; i++) {
        const row = displayFolders.map(folder => {
            ...
            if (folder === '02-backlog' && item.blockedBy && item.blockedBy.length > 0) {
            ...
            if (folder === '03-in-progress' && item.id && typeConfig.prefix === 'feature') {
    ...
    const counts = displayFolders.map(f => {
        ...
        if (f === '02-backlog' && typeConfig.prefix === 'feature') {
...
function displayListSection(title, typeConfig, options) {
    ...
    typeConfig.folders.forEach(folder => {
        ...
        if (folder === '02-backlog' && typeConfig.prefix === 'feature') {
            const blockedCount = folderItems.filter(item => item.blockedBy && item.blockedBy.length > 0).length;
