...
// Sync state moved to @aigon/pro with feature 236. Free tier has no sync,
// so feature suspension is always false; Pro overrides via getPro().
function isFeatureSuspended(...args) {
    try {
        const { getPro } = require('./pro');
        const pro = getPro();
        const fn = pro && (
            (pro.sync && pro.sync.isFeatureSuspended) ||
            (pro.profile && pro.profile.isFeatureSuspended)
        );
...
const STAGE_BY_FOLDER = Object.freeze({
    ...
    [STAGE_FOLDERS.IN_PROGRESS]: 'in-progress',
    [STAGE_FOLDERS.IN_EVALUATION]: 'in-evaluation',
...
function getItemDriftBadge(item) {
    return item && item.specDrift ? ' ⚠ drift' : '';
}
...
function collectBoardItems(typeConfig, folderFilter, repoPath = process.cwd()) {
    ...
    [...deduped.values()].forEach(item => {
        ...
        let blockedBy = [];
        if (typeConfig.prefix === 'feature' && resolvedFolder === STAGE_FOLDERS.BACKLOG && item.id) {
            const specPath = path.join(typeConfig.root, resolvedFolder, item.file);
            blockedBy = checkUnmetDependencies(specPath, typeConfig);
        }
        items[resolvedFolder].push({
            ...item,
            stage: state.stage || item.visibleStage,
            specDrift: state.specDrift,
 ...
 * @param {string} folder - Stage folder from STAGE_FOLDERS
...
function displayBoardKanbanView(options) {
    ...
    const redMain = readRedMainCondition(process.cwd());
    if (redMain) {
        console.log(`⚠️  Main is red: ${redMain.gateCommand || 'post-merge gate'} failed at ${redMain.latestSeenAt || redMain.firstSeenAt || 'unknown time'}`);
        if (redMain.gateLogPath) console.log(`   Gate log: ${redMain.gateLogPath}`);
        console.log('');
    }
...
function displayKanbanSection(title, typeConfig, options, mapping = {}, startLetterIndex = 0) {
    ...
    if (showAll) {
        ...
        if (showInbox) folderFilter.add(STAGE_FOLDERS.INBOX);
        if (showBacklog) folderFilter.add(STAGE_FOLDERS.BACKLOG);
        if (showActive) {
            folderFilter.add(STAGE_FOLDERS.IN_PROGRESS);
            if (typeConfig.prefix === 'feature') folderFilter.add(STAGE_FOLDERS.IN_EVALUATION);
            if (typeConfig.prefix === 'research') folderFilter.add(STAGE_FOLDERS.IN_EVALUATION);
        }
        if (showDone) {
            if (typeConfig.prefix === 'feature') folderFilter.add(STAGE_FOLDERS.DONE);
            if (typeConfig.prefix === 'research') folderFilter.add(STAGE_FOLDERS.DONE);
        }
    } else {
        // Default: everything except done
        typeConfig.folders.forEach(f => {
            if (typeConfig.prefix === 'feature' && f !== STAGE_FOLDERS.DONE) folderFilter.add(f);
            if (typeConfig.prefix === 'research' && f !== STAGE_FOLDERS.DONE) folderFilter.add(f);
        });
    }
    ...
    const columnMap = {
        'feature': {
            ...
            [STAGE_FOLDERS.BACKLOG]: 'Backlog',
            [STAGE_FOLDERS.IN_PROGRESS]: 'In Progress',
            [STAGE_FOLDERS.IN_EVALUATION]: 'Evaluation',
            [STAGE_FOLDERS.DONE]: 'Closed',
            [STAGE_FOLDERS.PAUSED]: 'Paused'
        },
        'research': {
            [STAGE_FOLDERS.INBOX]: 'Inbox',
    ...
    for (let i = 0; i < maxRows; i++) {
        const row = displayFolders.map(folder => {
            ...
            display += getItemDriftBadge(item);

            // Add letter label for unprioritized inbox items
            if (folder === STAGE_FOLDERS.INBOX && !item.id) {
            ...
            // Show blocked indicator for backlog features with unmet deps
            if (folder === STAGE_FOLDERS.BACKLOG && item.blockedBy && item.blockedBy.length > 0) {
                const ids = item.blockedBy.map(d => `#${parseInt(d.id, 10)}`).join(',');
                display += ` [BLOCKED:${ids}]`;
            }

            // F359: suspended badge when the snapshot says a worktree exists
            // but it's missing locally (e.g. after `aigon sync pull` from another machine).
            if (folder === STAGE_FOLDERS.IN_PROGRESS && item.id && typeConfig.prefix === 'feature') {
                const wts = worktreeMap[item.id] || [];
                if (isFeatureSuspended(process.cwd(), item.id, { hasLocalWorktree: wts.length > 0 })) {
                    display += ' [suspended]';
                }
            }

            // Add worktree/mode indicator for in-progress items
            if (folder === STAGE_FOLDERS.IN_PROGRESS && item.id) {
                const wts = worktreeMap[item.id] || [];
                if (wts.length > 1) {
    ...
    // Display counts (with blocked count for backlog)
    const counts = displayFolders.map(f => {
        const folderItems = items[f] || [];
        const count = folderItems.length;
        if (f === STAGE_FOLDERS.BACKLOG && typeConfig.prefix === 'feature') {
            const blockedCount = folderItems.filter(item => item.blockedBy && item.blockedBy.length > 0).length;
            const label = blockedCount > 0 ? `(${count}, ${blockedCount} blocked)` : `(${count})`;
            return label.padEnd(colWidth).substring(0, colWidth);
        }
...
function displayListSection(title, typeConfig, options) {
    ...
    typeConfig.folders.forEach(folder => {
        ...
        if (folder === STAGE_FOLDERS.BACKLOG && typeConfig.prefix === 'feature') {
            const blockedCount = folderItems.filter(item => item.blockedBy && item.blockedBy.length > 0).length;
            const countLabel = blockedCount > 0 ? `${folderItems.length}, ${blockedCount} blocked` : folderItems.length;
            console.log(`\n${label} (${countLabel})`);
        } else {
        ...
        folderItems.forEach(item => {
            ...
            if (folder === STAGE_FOLDERS.BACKLOG && item.blockedBy && item.blockedBy.length > 0) {
                const ids = item.blockedBy.map(d => `#${parseInt(d.id, 10)}`).join(', ');
                detail = `  Blocked by: ${ids}`;
            }

            if (folder === STAGE_FOLDERS.IN_PROGRESS && itemId) {
            ...
            const itemLine = `  ${prefix}  ${itemName}${getItemDriftBadge(item)}${detail}`;
