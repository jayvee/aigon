...
function runRalphCommand(args) {
    const options = utils.parseCliOptions(args);
    ...
    const found = utils.findFile(utils.PATHS.features, id, ['03-in-progress']);
    if (!found) {
    ...
    const defaultMaxIterations = Number.isInteger(configuredDefaultMax) && configuredDefaultMax > 0
        ...
        : 5;
    const maxIterationsRaw = utils.getOptionValue(options, 'max-iterations');
    ...
    const dryRun = Boolean(utils.getOptionValue(options, 'dry-run'));
    ...
    const autoSubmitFlagExplicit = utils.getOptionValue(options, 'auto-submit');
    const noAutoSubmitFlagExplicit = utils.getOptionValue(options, 'no-auto-submit');
        ...
        if (autoSubmit && !dryRun) {
            ...
            if (logFile) {
                ...
                try {
                    ...
                    console.warn('   ⚠️  Auto-submit could not signal agent status via `aigon agent-status submitted`.');
                    console.warn('      Re-run manually in this worktree if needed.');
                }
...
function runFeatureValidateCommand(args) {
    ...
    const dryRun = Boolean(utils.getOptionValue(options, 'dry-run'));
    const noUpdate = Boolean(utils.getOptionValue(options, 'no-update'));
