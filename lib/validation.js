...
function runRalphAgentIteration(agentId, prompt, dryRun = false) {
    ...
    const args = [...flagTokens, ...(promptFlag ? [promptFlag] : []), prompt];
...
function runRalphCommand(args) {
    ...
    const found = findFile(utils.PATHS.features, id, [STAGE_FOLDERS.IN_PROGRESS]);
    if (!found) {
    ...
    const selectedAgentRaw = String(getOptionValue(options, 'agent') || getDefaultAgent(process.cwd())).toLowerCase();
    ...
    if (loopSucceeded) {
        ...
        if (autoSubmit && !dryRun) {
            ...
            try {
                execSync('aigon agent-status implementation-complete', { stdio: 'pipe' });
            } catch (e) {
                submissionSignaled = false;
                const stderr = e && e.stderr ? String(e.stderr).trim() : '';
                const stdout = e && e.stdout ? String(e.stdout).trim() : '';
                const detail = stderr || stdout || e.message;
                console.error('   ❌ Auto-submit failed: could not signal `aigon agent-status implementation-complete`.');
                if (detail) console.error(`      ${detail}`);
                console.error('      Run `aigon agent-status implementation-complete` manually after resolving the issue.');
                process.exitCode = 1;
            }
            ...
            if (submissionSignaled) {
                ...
                console.log('   The feature may remain in implementing until `aigon agent-status implementation-complete` succeeds.');
            }
        } else {
            if (autoSubmit && dryRun) {
            ...
            if (!autoSubmit) {
                ...
                console.log('   2) run: aigon agent-status implementation-complete');
...
function getProfileValidationCommands(profileName, projectConfig = {}) {
    ...
    switch (profileName) {
        ...
        default: {
            ...
            // Node.js iterate-loop default: scoped runner (lint + matched tests
            // + Playwright only when dashboard touched). Full suite stays at
            // pre-push gate. See scripts/iterate-validate.js and lib/test-loop/scoped.js.
            const scripts = getPackageJsonScripts();
            const pm = detectNodePackageManager();
            if (scripts['test:iterate']) {
                return [{ label: 'Iterate', cmd: `${pm} run test:iterate` }];
            }
            // Fallback for projects without the scoped runner: legacy multi-command.
            const cmds = [];
