...
const { isInCloseRecovery } = require('./close-gate-predicates');
...
async function runImplementationLogCloseGuard(repoPath, featureId, options = {}) {
    ...
    if (!isCloseFindingBlocking(policy, 'implementation-log')) {
        ...
        if (snapshot && isInCloseRecovery(snapshot)) {
            await wf.recordCloseRecoveryEnded(repoPath, String(featureId), {
                source: 'implementation-log-advisory',
            });
        }
