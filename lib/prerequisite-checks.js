...
const MESSAGES = {
    ...
    caddy: {
        failMessage: 'Caddy is not installed. Required for .localhost subdomain routing (aigon.localhost, cc-01.brewboard.localhost, etc.).',
        failRemediation: process.platform === 'darwin'
            ? 'Install: brew install caddy  then: aigon proxy install'
            : 'Install: see https://caddyserver.com/docs/install  then: aigon proxy install',
        failLabel: 'caddy',
        okLabel: (version) => version ? `caddy ${version}` : 'caddy',
    },
};
