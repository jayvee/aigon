...
// exercises a load-bearing surface (registries, action scope) AND runs
// in-process (no CLI subprocess spawns). Keep at <=5 entries and keep them
// fast — every entry is iterate-loop tax for a no-match diff, so a
// subprocess-heavy test here (e.g. bootstrap-engine-state, 10 CLI spawns)
// makes every quiet iterate slow. Engine bootstrap is covered by the deploy
// gate and by keyword-matched runs when entity/bootstrap files change.
const SMOKE_FILES = [
    ...
    'tests/integration/action-scope.test.js',
];

// Heavy-tier integration tests — the `--exclude` entries of the `test:integration`
// npm script, run separately by `test:integration:heavy`. Parsed here so there is
// ONE source of truth. The iterate loop must never pull these in via keyword
// matching: they spawn many `aigon` CLI subprocesses or set up real git clones
// (e.g. two-clone-git-branch-storage runs >70s). They still run in the deploy
// gate (test:core:full / test:release).
function getHeavyIntegrationTests() {
    try {
        const pkg = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8'));
        const script = (pkg.scripts && pkg.scripts['test:integration']) || '';
        return new Set([...script.matchAll(/--exclude="([^"]+)"/g)].map(m => m[1]));
    } catch (_) {
        return new Set();
    }
}

// Paths whose changes should trigger the browser smoke suite (test:browser:smoke)
// in the iterate gate. Beyond the obvious dashboard/server code, F556 adds the
// state-projection and workflow-rules modules: an architectural refactor in any
// of these can silently break which dashboard actions are available or how state
// renders, which only a browser test catches. workflow-core/** is included
// because action availability and state projection are derived from it.
const DASHBOARD_PATH_RE = /^(?:templates\/dashboard\/|lib\/(?:dashboard|server|state-render-meta|workflow-snapshot-adapter|feature-workflow-rules|research-workflow-rules|workflow-core\/))/;
...
function extractKeywords(paths) {
    ...
    // Only mine keywords from source/scripts/templates — doc and spec edits are
    // not predictive of which tests should run; including them produces a flood
    // of generic matches like "feature" or "spec".
    const SOURCE_PREFIXES = ['lib/', 'scripts/', 'templates/', 'aigon-cli.js', 'tests/'];
    for (const p of paths) {
        if (!SOURCE_PREFIXES.some(prefix => p.startsWith(prefix) || p === prefix.replace('/', ''))) continue;
...
function matchTestsForKeywords(keywords) {
    ...
    const heavy = getHeavyIntegrationTests();
    const all = [...listTestFiles(INTEGRATION_DIR), ...listTestFiles(WORKFLOW_DIR)];
    const matched = new Set();
    for (const file of all) {
        // Never pull heavy-tier tests into the fast iterate loop — they run at
        // deploy time. Keeps a 2-line lib edit from spawning dozens of CLI boots.
        if (heavy.has(path.relative(REPO_ROOT, file))) continue;
...
function classifyChanges(paths) {
    ...
    for (const p of paths) {
        ...
        // Match iterate spec: diagram check when workflow code or any templates/ change.
        if (p.startsWith('lib/workflow') || p.startsWith('templates/')) flags.workflowGen = true;
...
async function runScopedValidation({ baseRef = null, log = console.log } = {}) {
    ...
    // Step 4: Playwright smoke subset runs only if dashboard files touched.
    // Full browser suite belongs to test:deploy / the pre-push gate, not the iterate loop.
    if (flags.dashboard) {
        log(`[playwright] dashboard touched — running test:browser:smoke`);
        const ui = runChild('npm', ['run', 'test:browser:smoke']);
        ranSteps.push({ step: 'test:browser:smoke', ok: ui.ok, durationMs: ui.durationMs });
        if (!ui.ok) {
            return { ok: false, durationMs: Date.now() - totalStart, ranSteps, failedAt: 'test:browser:smoke' };
        }
    }
