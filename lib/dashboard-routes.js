    ...
    const routes = [
                    ...
                    try {
                        ...
                        if (fs.existsSync(worktreeBase)) {
                            ...
                            const entries = fs.readdirSync(worktreeBase).filter(d => {
                                ...
                                return m && m[1] === featureId && (mode === 'review' || m[2] === agentId);
                            });
                            if (entries.length > 0) {
                        ...
                        const handler = mode === 'review' ? ctx.routes.handleLaunchReview
                            : mode === 'eval' ? ctx.routes.handleLaunchEval : ctx.routes.handleLaunchImplementation;
        ...
        },
        {
            ...
            handler(req, res, ctx, match) {
                ...
                ctx.readJsonBody().then(payload => {
                    ...
                    if (agents.length === 0) {
                        ctx.sendJson(400, { error: 'At least one implementation agent is required' });
                        return;
                    }
                    if (!['implement', 'eval', 'review', 'close'].includes(stopAfter)) {
                        ctx.sendJson(400, { error: 'stopAfter must be one of: implement, eval, review, close' });
                        return;
                    }
                    const agentPattern = /^[a-z]{2,4}$/;
                    const invalidAgents = agents.filter(a => !agentPattern.test(a));
                    if (invalidAgents.length > 0) {
                        ctx.sendJson(400, { error: `Invalid agent identifier(s): ${invalidAgents.join(', ')}` });
                        return;
                    }
                    if (evalAgent && !agentPattern.test(evalAgent)) {
                        ctx.sendJson(400, { error: `Invalid eval agent identifier: ${evalAgent}` });
                        return;
                    }
                    if (reviewAgent && !agentPattern.test(reviewAgent)) {
                        ctx.sendJson(400, { error: `Invalid review agent identifier: ${reviewAgent}` });
                        return;
                    }
                    ...
                    if (modelsCsv) args.push(`--models=${modelsCsv}`);
                    if (effortsCsv) args.push(`--efforts=${effortsCsv}`);
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                ctx.readJsonBody().then(payload => {
                    ...
                    try {
                        if (ctx.state.peekActiveSessions.has(sessionName)) {
                            try { runTmux(['pipe-pane', '-t', sessionName], { stdio: 'ignore' }); } catch (_) {}
                            ctx.state.peekActiveSessions.delete(sessionName);
            ...
            handler(req, res, ctx) {
                try {
                    ...
                    for (const session of orphans) {
                        ...
                        if (!name) continue;
                        try {
                            if (ctx.state.peekActiveSessions.has(name)) {
                                try { runTmux(['pipe-pane', '-t', name], { stdio: 'ignore' }); } catch (_) {}
                                ctx.state.peekActiveSessions.delete(name);
                            }
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                ...
                if (!tmuxSessionExists(sessionName)) {
                    ctx.sendJson(409, { error: `Session "${sessionName}" is not running`, alive: false });
                    return;
                }
                ...
                try {
                    ...
                    const { uptime, lastActivity } = ctx.routes.getTmuxSessionPeekMeta(sessionName);
                    ctx.sendJson(200, { output, offset: output.length, alive: true, uptime, lastActivity });
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
                }
            }
        ...
        {
            method: 'POST',
            path: '/api/session-peek/stop',
            handler(req, res, ctx) {
                ctx.readJsonBody().then(payload => {
                    const sessionName = String(payload.sessionName || '').trim();
                    if (sessionName && ctx.state.peekActiveSessions.has(sessionName)) {
                        try { runTmux(['pipe-pane', '-t', sessionName], { stdio: 'ignore' }); } catch (_) {}
                        ctx.state.peekActiveSessions.delete(sessionName);
                        const peekFile = path.join(os.tmpdir(), `aigon-peek-${sessionName.replace(/[^a-zA-Z0-9_-]/g, '_')}.log`);
                        try { fs.unlinkSync(peekFile); } catch (_) {}
                        ctx.helpers.log(`Peek stopped for session: ${sessionName}`);
                    }
                    ctx.sendJson(200, { ok: true });
                }).catch(() => ctx.sendJson(200, { ok: true }));
            }
        },
        {
            method: 'POST',
            path: '/api/session-input',
            handler(req, res, ctx) {
                ctx.readJsonBody().then(payload => {
