...
function createDashboardRouteDispatcher(serverCtx) {
    const routes = [
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                try {
                    ...
                    ctx.sendJson(200, profileState.statusLocal());
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
                }
            }
        },
        {
            ...
            handler(req, res, ctx) {
                // F388: settings-sync now reflects unified vault status.
                try {
                    const backup = require('./backup');
                    ctx.sendJson(200, backup.status());
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
                }
            }
        ...
        {
            // Run vault push in the server process (not via open-terminal) so
            // success/failure surfaces in the dashboard; GUI terminals often
            // close as soon as the one-shot command exits, which felt like
            // "nothing happened" / session died.
            method: 'POST',
            path: '/api/backup/push',
            handler(req, res, ctx) {
                try {
                    const backup = require('./backup');
                    ...
                    const code = e && e.code;
                    const status = code === 'ENOBACKUP' || code === 'EBACKUPCONFLICT' ? 400 : 500;
                    ctx.sendJson(status, { error: e.message, code: code || undefined });
                }
            }
        },
        {
            method: 'POST',
            path: '/api/backup/pull',
            handler(req, res, ctx) {
                try {
                    const backup = require('./backup');
                    const result = backup.pull();
                    ctx.sendJson(200, { ok: true, ...result });
                } catch (e) {
                    const code = e && e.code;
                    const status = code === 'ENOBACKUP' || code === 'EBACKUPCONFLICT' ? 400 : 500;
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                try {
                    ...
                    const repoPath = String((req.query && req.query.repoPath) || process.cwd());
                    const cfgPath = path.join(repoPath, '.aigon', 'config.json');
                    const metaPath = path.join(repoPath, '.aigon', '.sync', 'sync-meta.json');
                    const cfg = core.loadJson(cfgPath);
                    const meta = fs.existsSync(metaPath) ? core.loadJson(metaPath) : {};
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                try {
                    const configPath = path.join(process.cwd(), '.aigon', 'config.json');
                    const metaPath = path.join(process.cwd(), '.aigon', '.sync', 'sync-meta.json');
                    let remote = null;
                    try {
                        const cfg = JSON.parse(fs.readFileSync(configPath, 'utf8'));
                        remote = (cfg.sync && cfg.sync.remote) ? String(cfg.sync.remote).trim() : null;
                    } catch (_) {}
                    let lastPushAt = null;
                    let lastPullAt = null;
                    try {
                    ...
                    ctx.sendJson(200, { configured: Boolean(remote), remote, lastPushAt, lastPullAt });
                } catch (e) {
                    ctx.sendJson(500, { error: e.message });
                }
            }
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                try {
                    ...
                    const url = new URL(req.url || '/api/schedule/jobs', `http://${req.headers.host || 'localhost'}`);
                    const repoHint = String(url.searchParams.get('repoPath') || url.searchParams.get('path') || '').trim();
                    ...
                    for (const repoPath of targets) {
                        const list = sk.listJobs(repoPath, { includeAll });
                        for (const j of list) {
                            jobs.push({
                                ...j,
                                repoPath,
                                displayPath: repoPath.replace(new RegExp('^' + os.homedir()), '~'),
                            });
                        }
                    }
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                ...
                ctx.readJsonBody().then((payload) => {
                    try {
                        const repoRes = sk.resolveRepoForScheduleCli(payload.repoPath != null ? String(payload.repoPath) : '');
                        if (!repoRes.ok) {
                            ctx.sendJson(400, { error: repoRes.error });
                            return;
                        }
                        const kind = String(payload.kind || '').trim();
                        const entityId = String(payload.entityId || '').trim();
                        const runAt = String(payload.runAt || '').trim();
                        const r = sk.addJob(repoRes.repoPath, { kind, entityId, runAt, payload: payload.payload || {} });
                        if (!r.ok) {
                            ctx.sendJson(400, { error: r.error });
                            return;
                        }
            ...
            handler(req, res, ctx) {
                ...
                ctx.readJsonBody().then((payload) => {
                    try {
                        ...
                        const r = sk.cancelJob(repoRes.repoPath, jobId);
                        if (!r.ok) {
                            ctx.sendJson(400, { error: r.error });
                            return;
                        }
        ...
        },
        {
            ...
            handler(req, res, ctx) {
                ...
                const resolvedRepoPath = repoPath
                    ? (ctx.helpers.resolveRequestedRepoPathOrRespond(res, repoPath) || null)
                    : process.cwd();
                if (repoPath && !resolvedRepoPath) return;
                const agentMatrix = require('./agent-matrix');
                const rows = agentMatrix.buildMatrix(resolvedRepoPath);
