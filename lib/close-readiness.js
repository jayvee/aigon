...
function buildOpenEscalationBlockers(snapshot, featureId, policy) {
    const open = getOpenEscalations(snapshot);
    if (open.length === 0) return [];
    if (!isCloseFindingBlocking(policy, 'review-escalation')) return [];
    const first = open[0];
...
function buildLastCloseFailureBlocker(lcf, featureId, policy) {
    if (!lcf) return null;
    ...
    if (mappedKind === 'criteria-attestation') return null;
    if (mappedKind === 'preauth-validation' && !isCloseFindingBlocking(policy, 'preauth-validation')) return null;
    if (mappedKind === 'post-merge-gate' && !isCloseFindingBlocking(policy, 'post-merge-gate')) return null;
    if (mappedKind === 'implementation-log' && !isCloseFindingBlocking(policy, 'implementation-log')) return null;
    ...
    const actionKind = (mappedKind === 'merge-conflict' || mappedKind === 'post-merge-gate'
        || mappedKind === 'preauth-validation' || mappedKind === 'implementation-log')
...
function buildCloseRecoveryBlocker(snapshot, featureId, policy) {
    if (!isInCloseRecovery(snapshot)) return null;
    const lcf = snapshot.lastCloseFailure;
    if (lcf) return buildLastCloseFailureBlocker(lcf, featureId, policy);
    return makeBlocker(
        'close-recovery',
        'Close recovery',
...
function buildCloseReadiness(entity, snapshot, options) {
    ...
    const recoveryBlocker = buildCloseRecoveryBlocker(snapshot, featureId, policy);
    if (recoveryBlocker) {
        blockers.push(recoveryBlocker);
    } else if (snapshot && snapshot.lastCloseFailure && !isInCloseRecovery(snapshot)) {
        const failBlocker = buildLastCloseFailureBlocker(snapshot.lastCloseFailure, featureId, policy);
    ...
    if (snapshot && snapshot.lastCloseFailure && isInCloseRecovery(snapshot)) {
        ...
        if (retryKind === 'preauth-validation' && isCloseFindingBlocking(policy, 'preauth-validation') && !blockers.some(b => b.kind === 'preauth-validation')) {
            blockers.push(buildLastCloseFailureBlocker(snapshot.lastCloseFailure, featureId, policy));
        }
        if (retryKind === 'implementation-log' && isCloseFindingBlocking(policy, 'implementation-log') && !blockers.some(b => b.kind === 'implementation-log')) {
            blockers.push(buildLastCloseFailureBlocker(snapshot.lastCloseFailure, featureId, policy));
        }
    }
    ...
    const hasEscalations = isCloseFindingBlocking(policy, 'review-escalation') && hasOpenEscalationGate(snapshot);
