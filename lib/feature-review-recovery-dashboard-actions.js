...
function isFeatureReviewRecoveryContext(snapshot, autoState, repoPath, featureId) {
    ...
    if (autoState && ['failed', 'stopped', 'quota-paused', 'died'].includes(autoState.status)) return true;
...
function isFeatureAutonomousRecoveryRelevant(autonomousController) {
    ...
    return ['failed', 'stopped', 'quota-paused', 'died'].includes(autonomousController.status);
}
...
function resolveFeatureRecommendedRecoveryKind(snapshot, autonomousController, operations) {
    ...
    if (autonomousController && autonomousController.reason === 'review-quota-paused' && hasOperation('rerun-review')) return 'rerun-review';
...
function ensureFeatureCodeReviewRecoveryAction(featureId, snapshot, autoState, actions) {
    const base = Array.isArray(actions) ? actions.slice() : [];
    const lifecycle = snapshot && (snapshot.currentSpecState || snapshot.lifecycle);
    const reviewCancelled = Boolean(snapshot && snapshot.codeReview && snapshot.codeReview.cancelledAt);
    const quotaPausedReady = autoState && autoState.reason === 'review-quota-paused' && lifecycle === 'ready';
    const needsInject = (reviewCancelled || quotaPausedReady) && !base.some((a) => a.action === 'feature-code-review');
    if (!needsInject) return base;
    base.push({
        command: formatDashboardActionCommand('feature-code-review', featureId),
        label: 'Run Review',
        action: 'feature-code-review',
        mode: 'agent',
        category: 'lifecycle',
        type: 'action',
        priority: 'high',
...
function appendFeatureAutonomousResumeAction(featureId, autoState, validActions) {
    ...
    if (!autoState || !['failed', 'stopped', 'quota-paused', 'died'].includes(String(autoState.status || ''))) return base;
...
function appendFeatureReviewRecoveryDashboardActions(repoPath, featureId, autoState, snapshot, validActions, autonomousController = null) {
    ...
    const withReview = ensureFeatureCodeReviewRecoveryAction(featureId, snapshot, autoState, withResume);

    const tagged = withReview.map((action) => {
        ...
        if (action.action === 'feature-code-review' && (
            (snapshot.currentSpecState === 'ready' && snapshot.codeReview && snapshot.codeReview.cancelledAt)
            || (autoState && autoState.reason === 'review-quota-paused' && snapshot.currentSpecState === 'code_review_in_progress')
            || (autoState && autoState.reason === 'review-quota-paused' && snapshot.currentSpecState === 'ready')
        )) {
            return tagRecoveryAction(action, {
                label: snapshot.currentSpecState === 'ready' ? 'Run Review' : 'Re-run code review',
                priority: 'high',
                metadata: {
                    recoveryOperationKind: 'rerun-review',
                    recoverySurface: snapshot.currentSpecState === 'ready',
                },
            });
        }
