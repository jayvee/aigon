...
// Lazy require config-core to avoid load-order issues
function _loadProjectConfig() {
    return require('./config-core').loadProjectConfig();
}
...
const DEFAULT_SYSTEM_CADDY_PLIST_PATH = '/Library/LaunchDaemons/com.aigon.caddy.plist';
...
// Dashboard port range constants re-exported from proxy-dns for backwards compatibility.
 ...
 * For the instance-qualified Caddy hostname (e.g. cu-600.aigon.localhost), use
 * getAigonServerCaddyHost() or resolveInstanceIdentity().caddyHost.
 * @returns {string}
 */
...
/**
 * Caddy hostname for this dashboard instance (aigon.localhost or <id>.aigon.localhost).
 * @returns {string}
 */
function getAigonServerCaddyHost() {
    const { resolveInstanceIdentity } = require('./instance-identity');
    return resolveInstanceIdentity().caddyHost;
}
 ...
 * System LaunchDaemon installs serve on 80; user-mode installs read from the
 * Caddyfile if it exists and otherwise default to 4080.
 * @returns {number}
 */
function getSystemCaddyPlistPath() {
    return process.env.AIGON_CADDY_SYSTEM_PLIST || DEFAULT_SYSTEM_CADDY_PLIST_PATH;
}

function isSystemCaddyDaemonInstalled() {
    return process.platform === 'darwin' && fs.existsSync(getSystemCaddyPlistPath());
}

function kickstartSystemCaddyDaemon() {
    if (!isSystemCaddyDaemonInstalled()) return false;
    try {
        execSync('launchctl kickstart -k system/com.aigon.caddy', { stdio: 'pipe', timeout: 10000 });
        return true;
    } catch (_) {
        return false;
    }
...
function getCaddyPort() {
    if (isSystemCaddyDaemonInstalled()) return 80;
...
/**
 * Check if Caddy is installed and running on the configured public port.
...
function isProxyAvailable() {
    ...
    try {
        // Caddy's default admin API listens on localhost:2019; /config/ returns 200
        execSync('curl -sf http://localhost:2019/config/ > /dev/null 2>&1', { stdio: 'pipe', timeout: 2000 });
        return isPortInUseSync(getCaddyPort());
    } catch (_) {
        return false;
    }
}
...
/**
 * Reload Caddy's configuration. If Caddy is not running, starts the correct
 * installed mode. A system daemon must stay on port 80; never silently fall
 * back to a user-mode 4080 proxy when the daemon is installed.
 */
function reloadCaddy() {
    ...
    try {
        ...
        // Only start if Caddy is genuinely not running — avoids spawning a second daemon
        // when reload fails transiently (the double-process SO_REUSEPORT split-brain bug).
        if (!isProxyAvailable()) {
            if (kickstartSystemCaddyDaemon()) return;
            if (isSystemCaddyDaemonInstalled()) return;
            try {
                execSync(`caddy start --config ${JSON.stringify(CADDYFILE_PATH)}`, { stdio: 'pipe', timeout: 15000 });
            } catch (_) {
                // start also failed — leave it, next explicit `aigon proxy start` will recover
            }
        }
    }
}
...
module.exports = {
    ...
    // Caddy proxy management
    isCaddyInstalled,
    isSystemCaddyDaemonInstalled,
    kickstartSystemCaddyDaemon,
    isProxyAvailable,
    proxyDiagnostics,
    getCaddyPort,
    parseCaddyRoutes,
