...
async function pollGeminiBudget({ log } = {}) {
    ...
    try {
        ...
        await sleep(6000);
        const raw = capture(SESSION_GG, { historyStart: '-500' });
        if (/waiting\s+for\s+authentication/i.test(raw)) {
            (log || console.log)('[budget-poller] gg: not authenticated — skipping cache write');
            return null;
        }
        let tiers = parseGeminiModelUsage(raw);
        if (!tiers.length) {
            tiers = parseGeminiQuotaExhausted(raw) || [];
        }
        if (!tiers.length) {
            const foot = parseGeminiFooterPlanQuota(raw);
            if (foot && foot.pct_used != null) {
                tiers = [{
                    tier: 'plan',
                    label: 'Plan quota',
                    pct_used: foot.pct_used,
                    resets_at: foot.resets_at || null,
                }];
            }
        }
        if (!tiers.length) {
