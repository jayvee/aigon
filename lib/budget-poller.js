 ...
 *   - Antigravity CLI via tmux (`agy` → `/usage` → capture "Models & Quota" groups)
 *
...
const SESSION_AG = 'aigon-budget-ag';
...
async function pollAntigravityBudget({ repoPath, log } = {}) {
    if (!binaryOnPath('agy')) {
        (log || console.log)('[budget-poller] agy binary not on PATH — skipping ag');
        return null;
    }
    killSession(SESSION_AG);
    if (!newSession(SESSION_AG, { cols: 280, rows: 120 })) {
        (log || console.log)('[budget-poller] failed to create ag tmux session');
        return null;
    }
    const cwd = repoPath || process.cwd();
    try {
        sendText(SESSION_AG, `cd ${cwd} && agy --dangerously-skip-permissions`);
        await sleep(15000);
        const earlyRaw = capture(SESSION_AG, { historyStart: '-300' });
        if (/waiting\s+for\s+authentication|sign[\s-]?in/i.test(earlyRaw)) {
            (log || console.log)('[budget-poller] ag: not authenticated — skipping cache write');
            return null;
        }
        sendText(SESSION_AG, '/usage');
        await sleep(6000);
        const raw = capture(SESSION_AG, { historyStart: '-500' });
        if (!/models\s*&\s*quota/i.test(raw)) {
            (log || console.log)('[budget-poller] ag: no "Models & Quota" screen — skipping cache write');
            return null;
        }
        const tiers = parseAntigravityUsage(raw);
        if (!tiers.length) {
            (log || console.log)('[budget-poller] ag: no usage tiers parsed — skipping cache write');
            return null;
        }
        return {
            polled_at: new Date().toISOString(),
...
function getCachePath(repoPath) {
    return require('./agent-quota-read').getLegacyBudgetPath(repoPath || process.cwd());
}
...
function readCache(repoPath) {
    ...
    return filterBudgetByAvailability(agentQuotaRead.projectBudgetApi(state, repoPath), repoPath);
}

function writeCache(repoPath, data) {
    const agentQuotaRead = require('./agent-quota-read');
    const state = agentQuotaRead.readAgentQuotaState(repoPath);
    for (const [agentId, budget] of Object.entries(data || {})) {
        if (!state.agents[agentId]) state.agents[agentId] = {};
        state.agents[agentId].budget = budget;
    }
    agentQuotaRead.writeAgentQuotaState(state, repoPath);
}
...
function filterBudgetByAvailability(data, repoPath = process.cwd()) {
    const { isAgentQuotaPanelVisible } = require('./agent-availability');
    const out = { ...(data || {}) };
    for (const id of ['cc', 'cx', 'gg', 'km', 'ag']) {
        if (!isAgentQuotaPanelVisible(id, repoPath)) out[id] = null;
    }
    return out;
}
...
async function pollOnce({ repoPath, log, force = false } = {}) {
    const logger = log || console.log;
    const root = repoPath || process.cwd();
    if (!shouldRunPoll()) {
        logger(`[budget-poller] skipped — last poll ${Math.round((Date.now() - _lastCompletedAt) / 1000)}s ago (min gap ${MIN_REFRESH_GAP_MS / 1000}s)`);
        return readCache(root);
    }
    ...
    if (shouldPollBudgetAgent('cc', root)) {
        if (tmuxAvailable()) {
            try {
    ...
    if (shouldPollBudgetAgent('cx', root)) {
        try {
    ...
    if (shouldPollBudgetAgent('km', root)) {
        if (tmuxAvailable()) {
            try {
    ...
    if (shouldPollBudgetAgent('ag', root)) {
        // Automated polls must never launch interactive agy — it opens browser login tabs.
        // Antigravity budget bars refresh on manual dashboard refresh, or when ANTIGRAVITY_TOKEN is set.
        const agInteractiveOk = Boolean(process.env.ANTIGRAVITY_TOKEN) || force;
        if (agInteractiveOk && tmuxAvailable()) {
            try {
                const ag = await pollAntigravityBudget({ repoPath: root, log: logger });
                if (ag) merged.ag = ag;
            } catch (e) {
                logger(`[budget-poller] ag error: ${e && e.message}`);
            }
        } else if (!agInteractiveOk) {
            logger('[budget-poller] ag: skipping interactive poll (set ANTIGRAVITY_TOKEN or use dashboard refresh)');
        } else {
            logger('[budget-poller] tmux not available — skipping ag');
        }
    } else {
        merged.ag = null;
    }
    if (!shouldPollBudgetAgent('gg', root)) merged.gg = null;
    writeCache(root, merged);
...
function startBudgetPoller({ repoPath, intervalMs, log } = {}) {
    ...
    async function tick(options = {}) {
        ...
        _inflight = pollOnce({ repoPath, log: logger, ...options })
    ...
    return {
        ...
        refresh: () => tick({ force: true }),
    };
}

function triggerRefresh({ repoPath, log } = {}) {
    if (_inflight) return _inflight;
    _inflight = pollOnce({ repoPath, log: log || console.log, force: true })
        .catch(() => null)
        .finally(() => { _inflight = null; });
    return _inflight;
}
