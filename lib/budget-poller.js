...
async function pollOnce({ repoPath, log, force = false } = {}) {
    ...
    // ag (Antigravity) is never polled automatically. Launching `agy` — even for a
    // budget scrape — opens a Google sign-in tab in the user's browser. It is left
    // out of every automated poll (including forced dashboard refresh); agy only
    // ever runs from an explicit, interactive feature/research assignment.
    merged.ag = null;
