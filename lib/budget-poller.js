...
function parseClaudeStatus(text) {
    ...
    // Split into sections by recognisable headers. The Usage tab has rows like:
    //   Current session        65% used   Resets 12pm (Australia/Melbourne)
    //   Current week (all)     97% used   Resets 9am  (Australia/Melbourne)
    //   Current week (Sonnet)  ...
    ...
    for (const line of lines) {
        ...
        const entry = { pct_used: pctUsed, resets_at: resetsAt, tz };

        if (/current\s+session/i.test(line)) out.session = entry;
        else if (/current\s+week\s*\(\s*all/i.test(line)) out.week_all = entry;
        else if (/current\s+week\s*\(\s*sonnet/i.test(line)) out.week_sonnet = entry;
    }
    return out;
}
