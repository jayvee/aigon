'use strict';

/**
 * Event-sourcing projector — rebuilds FeatureContext from an event stream.
 *
 * Ported from aigon-next/src/workflow/projector.ts.
 */

const { AgentStatus } = require('./types');
const {
  buildSpecReviewSummary,
  captureSpecReviewReturnLifecycle,
  resolveSpecReviewRestingLifecycle,
  shouldMutateLifecycleForSpecReviewEvent,
  shouldMutateLifecycleForCodeReviewEvent,
} = require('../spec-review-state');
const {
  emptySpecAuthor,
  emptyLastSpecRevision,
  normalizeLastSpecRevision,
  mergeSpecAuthorFromEvent,
} = require('../spec-author-provenance');
const { shouldClearIntegrityCloseFailureOnRecoveryEnd } = require('../close-gate-predicates');

// One-shot warning de-dup for the legacy `submitted` lifecycle rewrite (F501).
// Cleared per-process; the noise during boot is fine, the noise during a polling
// dashboard tick is not.
const _warnedSubmittedRewrite = new Set();

/**
 * Create an agents map from an array of agent IDs.
 * @param {string[]} agentIds
 * @returns {Record<string, { id: string, status: string, lastHeartbeatAt: string|null }>}
 */
function createAgents(agentIds, overrides) {
  const modelOverrides = (overrides && overrides.modelOverrides) || {};
  const effortOverrides = (overrides && overrides.effortOverrides) || {};
  return Object.fromEntries(
    agentIds.map((agentId) => [
      agentId,
      {
        id: agentId,
        status: AgentStatus.RUNNING,
        lastHeartbeatAt: null,
        restartCount: 0,
        currentAgentId: agentId,
        initialAgentId: agentId,
        previousAgentId: null,
        resumedFromAgentId: null,
        tokenExhausted: null,
        modelOverride: modelOverrides[agentId] != null ? modelOverrides[agentId] : null,
        effortOverride: effortOverrides[agentId] != null ? effortOverrides[agentId] : null,
      },
    ]),
  );
}

function createSpecReviewState() {
  return {
    pendingReviews: [],
    pendingCount: 0,
    pendingAgents: [],
    pendingLabel: '',
    activeReviewers: [],
    activeCheckers: [],
  };
}

function createNudgeState() {
  return [];
}

function appendRing(list, entry, limit = 20) {
  return [...(Array.isArray(list) ? list : []), entry].slice(-limit);
}

function refreshSpecReviewState(context) {
  const prev = context.specReview || {};
  const summary = buildSpecReviewSummary(prev.pendingReviews, {
    activeReviewers: prev.activeReviewers,
    activeCheckers: prev.activeCheckers,
  });
  context.specReview = summary;
}

function resolveCodeRevisionAgent(context) {
  const agents = context && context.agents ? Object.keys(context.agents) : [];
  if (context && context.mode === 'fleet') {
    return context.winnerAgentId || context.authorAgentId || agents[0] || null;
  }
  return agents[0] || context.authorAgentId || null;
}

function resolveCodeReviewAfterCancel(context, cancelledAt = null) {
  const prev = context.codeReview || {};
  const cycles = Array.isArray(context.reviewCycles) ? context.reviewCycles : [];
  const lastCode = [...cycles].reverse().find((entry) => entry.type === 'code');
  if (lastCode) {
    return {
      activeReviewerId: null,
      reviewerId: lastCode.reviewer,
      reviewStartedAt: lastCode.startedAt,
      reviewCompletedAt: lastCode.counterCompletedAt || lastCode.completedAt,
      requestRevision: true,
      revisionAgentId: prev.revisionAgentId || null,
      revisionStartedAt: prev.revisionStartedAt || null,
      revisionCompletedAt: prev.revisionCompletedAt || null,
      cancelledAt: cancelledAt || prev.cancelledAt || null,
    };
  }
  if (prev.reviewCompletedAt) {
    return { ...prev, activeReviewerId: null, cancelledAt: cancelledAt || prev.cancelledAt || null };
  }
  return {
    activeReviewerId: null,
    reviewerId: prev.reviewerId || prev.activeReviewerId || null,
    reviewStartedAt: prev.reviewStartedAt || null,
    reviewCompletedAt: prev.reviewCompletedAt || null,
    requestRevision: prev.requestRevision != null ? prev.requestRevision : null,
    revisionAgentId: prev.revisionAgentId || null,
    revisionStartedAt: prev.revisionStartedAt || null,
    revisionCompletedAt: prev.revisionCompletedAt || null,
    cancelledAt: cancelledAt || prev.cancelledAt || null,
  };
}

/**
 * Replay an event stream to produce a FeatureContext.
 * Returns null if no `feature.started` event exists.
 *
 * @param {object[]} events
 * @returns {object|null} FeatureContext or null
 */
function projectContext(events) {
  let context = null;
  let lifecycle = 'backlog';

  for (const event of events) {
    const isResearch = event.type.startsWith('research.');
    const entityType = isResearch ? 'research' : 'feature';

    switch (event.type) {
      case 'feature.bootstrapped':
      case 'research.bootstrapped': {
        // Bootstrapped from pre-engine state — initialise context from the event's lifecycle
        lifecycle = event.lifecycle || event.stage || 'backlog';
        const bootAuthor = mergeSpecAuthorFromEvent(context, event);
        context = {
          featureId: event.featureId || event.researchId,
          entityType,
          mode: event.mode || null,
          authorAgentId: bootAuthor.authorAgentId,
          specAuthor: bootAuthor.specAuthor,
          lastSpecRevision: (context && context.lastSpecRevision) || emptyLastSpecRevision(),
          agents: createAgents(event.agents || [], {
            modelOverrides: event.modelOverrides,
            effortOverrides: event.effortOverrides,
          }),
          winnerAgentId: null,
          effects: [],
          lastEffectError: null,
          specReview: createSpecReviewState(),
          nudges: createNudgeState(),
          quotaSignals: [],
          agentFailover: event.agentFailover || null,
          pauseReason: event.pauseReason || null,
          reviewCycles: [],
          pendingCodeReviewer: null,
          pendingSpecReviewer: null,
          specPath: '',
          createdAt: event.at,
          updatedAt: event.at,
        };
        break;
      }
      case 'feature.started':
      case 'research.started': {
        lifecycle = 'implementing';
        const startAuthor = mergeSpecAuthorFromEvent(context, event);
        context = {
          featureId: event.featureId || event.researchId,
          entityType,
          mode: event.mode,
          authorAgentId: startAuthor.authorAgentId,
          specAuthor: startAuthor.specAuthor,
          lastSpecRevision: (context && context.lastSpecRevision) || emptyLastSpecRevision(),
          agents: createAgents(event.agents, {
            modelOverrides: event.modelOverrides,
            effortOverrides: event.effortOverrides,
          }),
          winnerAgentId: null,
          effects: [],
          lastEffectError: null,
          specReview: createSpecReviewState(),
          nudges: createNudgeState(),
          quotaSignals: [],
          agentFailover: event.agentFailover || null,
          pauseReason: null,
          reviewCycles: [],
          pendingCodeReviewer: null,
          pendingSpecReviewer: null,
          specPath: '',
          createdAt: event.at,
          updatedAt: event.at,
        };
        for (const agent of Object.values(context.agents)) {
          agent.lastHeartbeatAt = event.at;
        }
        break;
      }
      case 'feature.reset':
      case 'research.reset': {
        // Merge-safe return to "never started". A reset re-seeds the context to
        // a fresh backlog state, discarding agents/effects/review state while
        // preserving identity and spec-author provenance. Replayed after
        // `*.started` it wins, and because it unions by id like every other
        // event it converges on every clone the log syncs to. The old reset
        // path only wiped the local `.aigon/workflows` dir, which never reached
        // the git-branch canonical log — so the next `storage sync` re-projected
        // `feature.started` and the feature snapped back to in-progress.
        lifecycle = 'backlog';
        const resetAuthor = mergeSpecAuthorFromEvent(context, event);
        context = {
          featureId: event.featureId || event.researchId || (context && context.featureId) || null,
          entityType,
          mode: null,
          authorAgentId: resetAuthor.authorAgentId,
          specAuthor: resetAuthor.specAuthor,
          lastSpecRevision: (context && context.lastSpecRevision) || emptyLastSpecRevision(),
          agents: {},
          winnerAgentId: null,
          effects: [],
          lastEffectError: null,
          specReview: createSpecReviewState(),
          nudges: createNudgeState(),
          quotaSignals: [],
          agentFailover: null,
          pauseReason: null,
          reviewCycles: [],
          pendingCodeReviewer: null,
          pendingSpecReviewer: null,
          specPath: '',
          createdAt: (context && context.createdAt) || event.at,
          updatedAt: event.at,
        };
        break;
      }
      case 'feature.paused':
      case 'research.paused':
        if (context !== null) {
          lifecycle = 'paused';
          context.pauseReason = event.reason || null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.resumed':
      case 'research.resumed':
        if (context !== null) {
          lifecycle = 'implementing';
          context.pauseReason = null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.review_requested':
      case 'research.review_requested':
        console.warn('[projector] legacy review_requested event — run `aigon doctor --fix` to migrate to *.code_review.started');
        if (context !== null) {
          lifecycle = 'code_review_in_progress';
          context.codeReview = {
            ...(context.codeReview || {}),
            activeReviewerId: event.reviewerId || event.agentId || null,
            reviewStartedAt: event.at,
            reviewCompletedAt: null,
            requestRevision: null,
          };
          context.updatedAt = event.at;
        }
        break;
      case 'feature.code_review.started':
      case 'research.code_review.started':
        if (context !== null && shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
          lifecycle = 'code_review_in_progress';
          context.codeReview = {
            ...(context.codeReview || {}),
            activeReviewerId: event.reviewerId || event.agentId || null,
            reviewStartedAt: event.at,
            reviewCompletedAt: null,
            requestRevision: null,
            cancelledAt: null,
          };
          context.pendingCodeReviewer = null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.code_review.completed':
      case 'research.code_review.completed':
        if (context !== null && shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
          const requestRevision = event.requestRevision === false ? false : true;
          lifecycle = requestRevision ? 'code_revision_in_progress' : 'ready';
          context.codeReview = {
            ...(context.codeReview || {}),
            activeReviewerId: null,
            reviewerId: event.reviewerId || event.agentId || (context.codeReview && context.codeReview.activeReviewerId) || null,
            reviewCompletedAt: event.at,
            requestRevision,
            revisionAgentId: requestRevision ? resolveCodeRevisionAgent(context) : null,
            cancelledAt: null,
          };
          context.updatedAt = event.at;
        }
        break;
      case 'feature.code_review.cancelled':
      case 'research.code_review.cancelled':
        if (context !== null && shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
          lifecycle = 'ready';
          context.codeReview = resolveCodeReviewAfterCancel(context, event.at);
          context.pendingCodeReviewer = null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.code_revision.started':
      case 'research.code_revision.started':
        if (context !== null && shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
          lifecycle = 'code_revision_in_progress';
          context.codeReview = {
            ...(context.codeReview || {}),
            revisionAgentId: event.revisionAgentId || resolveCodeRevisionAgent(context),
            revisionStartedAt: event.at,
            revisionCompletedAt: null,
          };
          context.updatedAt = event.at;
        }
        break;
      case 'feature.code_revision.completed':
      case 'research.code_revision.completed':
        if (context !== null) {
          const cr = context.codeReview || {};
          const revisionAgentId = event.revisionAgentId || cr.revisionAgentId || resolveCodeRevisionAgent(context);
          context.codeReview = {
            ...cr,
            revisionAgentId,
            revisionCompletedAt: event.at,
          };
          if (event.requestAnotherCycle === true && typeof event.nextReviewerId === 'string') {
            const cycles = Array.isArray(context.reviewCycles) ? context.reviewCycles : [];
            context.reviewCycles = [...cycles, {
              type: 'code',
              cycle: cycles.length + 1,
              reviewer: cr.reviewerId || cr.activeReviewerId || null,
              startedAt: cr.reviewStartedAt || null,
              completedAt: cr.reviewCompletedAt || null,
              counterStartedAt: cr.revisionStartedAt || null,
              counterCompletedAt: event.at,
            }];
            context.pendingCodeReviewer = event.nextReviewerId;
            if (shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
              lifecycle = 'code_review_in_progress';
            }
          } else if (shouldMutateLifecycleForCodeReviewEvent(lifecycle)) {
            lifecycle = 'ready';
          }
          context.updatedAt = event.at;
        }
        break;
      case 'feature.eval_requested':
      case 'research.eval_requested':
        if (context !== null) {
          lifecycle = 'evaluating';
          context.updatedAt = event.at;
        }
        break;
      case 'winner.selected':
        if (context !== null) {
          lifecycle = 'ready_for_review';
          context.winnerAgentId = event.agentId;
          context.updatedAt = event.at;
        }
        break;
      case 'feature_close.failed':
        if (context !== null) {
          context.lastCloseFailure = {
            kind: event.kind || 'other',
            conflictFiles: Array.isArray(event.conflictFiles) ? event.conflictFiles : [],
            stderrTail: event.stderrTail || '',
            at: event.at,
          };
          context.updatedAt = event.at;
        }
        break;
      case 'feature.close_gate_failed':
        if (context !== null) {
          const gateKind = event.gateKind || 'post-merge-gate';
          if (gateKind === 'preauth-validation') {
            context.lastCloseFailure = {
              kind: 'preauth-validation',
              unmatched: Array.isArray(event.unmatched) ? event.unmatched : [],
              noVerifyPreauth: Boolean(event.noVerifyPreauth),
              stderrTail: event.outputTail || '',
              at: event.at,
            };
          } else if (gateKind === 'implementation-log') {
            context.lastCloseFailure = {
              kind: 'implementation-log',
              expectedPattern: event.expectedPattern || null,
              stderrTail: event.outputTail || '',
              at: event.at,
            };
          } else if (gateKind === 'criteria-attestation') {
            // Removed in F659; historical gate failures are inert.
          } else {
            context.lastCloseFailure = {
              kind: 'post-merge-gate',
              gateCommand: event.gateCommand || null,
              exitCode: event.exitCode != null ? event.exitCode : 1,
              stderrTail: event.outputTail || '',
              mergedCommitSha: event.mergedCommitSha || null,
              logPath: event.logPath || null,
              at: event.at,
            };
          }
          context.updatedAt = event.at;
        }
        break;
      case 'feature.closed':
      case 'research.closed':
        if (context !== null) {
          lifecycle = 'done';
          context.lastCloseFailure = null;
          context.effects = context.effects.map((effect) => ({
            ...effect,
            status: 'succeeded',
            claimedAt: null,
            claimExpiredAt: null,
            lastError: null,
          }));
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.close_requested':
      case 'research.close_requested':
        if (context !== null) {
          lifecycle = 'closing';
          context.lastEffectError = null;
          context.closeRecovery = null;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.close_recovery.started':
        if (context !== null) {
          context.closeRecovery = {
            agentId: event.agentId || null,
            startedAt: event.at,
            returnSpecState: event.returnSpecState || lifecycle,
            sessionName: event.sessionName || null,
            source: event.source || null,
          };
          lifecycle = 'close_recovery_in_progress';
          context.updatedAt = event.at;
        }
        break;
      case 'feature.close_recovery.ended':
      case 'feature.close_recovery.cancelled':
        if (context !== null) {
          const back = context.closeRecovery && context.closeRecovery.returnSpecState;
          const allowed = new Set([
            'ready',
            'implementing',
            'code_review_in_progress',
            'code_revision_in_progress',
          ]);
          // Legacy returnSpecState 'submitted' rewrites to 'ready' (F501).
          const normalizedBack = back === 'submitted' ? 'ready' : back;
          lifecycle =
            typeof normalizedBack === 'string' && allowed.has(normalizedBack)
              ? normalizedBack
              : 'ready';
          context.closeRecovery = null;
          if (shouldClearIntegrityCloseFailureOnRecoveryEnd(event, context.lastCloseFailure)) {
            context.lastCloseFailure = null;
          }
          context.updatedAt = event.at;
        }
        break;
      case 'operator.nudge_sent':
        if (context !== null) {
          context.nudges = appendRing(context.nudges, {
            agentId: event.agentId,
            role: event.role || 'do',
            text: event.text || '',
            atISO: event.atISO || event.at,
          });
          context.updatedAt = event.atISO || event.at || context.updatedAt;
        }
        break;
      case 'feature.criteria_attested':
      case 'feature.criteria_attestation_bypassed':
        // Criteria attestation was removed in F659. Historical events remain
        // valid log entries but no longer project into close/readiness state.
        break;
      case 'review.escalation_raised':
        if (context !== null) {
          const open = Array.isArray(context.openEscalations) ? context.openEscalations.slice() : [];
          if (!open.some((entry) => entry.escalationId === event.escalationId)) {
            open.push({
              escalationId: event.escalationId,
              category: event.category,
              reason: event.reason,
              reviewerAgentId: event.reviewerAgentId || null,
              logPath: event.logPath || null,
              lineNumber: event.lineNumber != null ? event.lineNumber : null,
              raisedAt: event.at,
              source: event.source || null,
            });
          }
          context.openEscalations = open;
          context.updatedAt = event.at;
        }
        break;
      case 'review.escalation_accepted':
      case 'review.escalation_spun_off':
      case 'review.escalation_reopened':
        if (context !== null) {
          const open = Array.isArray(context.openEscalations) ? context.openEscalations : [];
          context.openEscalations = open.filter((entry) => entry.escalationId !== event.escalationId);
          context.updatedAt = event.at;
        }
        break;
      case 'feature.agent_quota_paused':
      case 'research.agent_quota_paused':
        if (context !== null) {
          context.quotaSignals = appendRing(context.quotaSignals || [], {
            kind: 'paused',
            agentId: event.agentId,
            role: event.role || 'do',
            patternId: event.patternId || null,
            resetAt: event.resetAt || null,
            detectedAt: event.detectedAt || event.at,
            paneSampleHash: event.paneSampleHash || null,
            sessionName: event.sessionName || null,
            atISO: event.at,
          }, 36);
          context.updatedAt = event.at || context.updatedAt;
        }
        break;
      case 'feature.agent_quota_resumed':
      case 'research.agent_quota_resumed':
        if (context !== null) {
          context.quotaSignals = appendRing(context.quotaSignals || [], {
            kind: 'resumed',
            agentId: event.agentId,
            role: event.role || 'do',
            atISO: event.at,
          }, 36);
          context.updatedAt = event.at || context.updatedAt;
        }
        break;
      case 'quota.signal_cleared':
        if (context !== null) {
          context.quotaSignals = (context.quotaSignals || []).filter(
            s => !(s.kind === 'paused' && s.agentId === event.agentId),
          );
          context.updatedAt = event.at || context.updatedAt;
        }
        break;
      case 'effect.requested':
        if (context !== null) {
          context.effects = [
            ...context.effects.filter((e) => e.id !== event.effect.id),
            {
              ...event.effect,
              status: 'requested',
              claimedAt: null,
              claimExpiredAt: null,
              reclaimCount: 0,
              lastError: null,
            },
          ];
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'effect.claimed':
        if (context !== null) {
          context.effects = context.effects.map((e) =>
            e.id === event.effectId
              ? { ...e, status: 'claimed', claimedAt: event.at, claimExpiredAt: null, lastError: null }
              : e,
          );
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'effect.claim_expired':
        if (context !== null) {
          context.effects = context.effects.map((e) =>
            e.id === event.effectId
              ? { ...e, status: 'reclaimable', claimedAt: null, claimExpiredAt: event.at, lastError: null }
              : e,
          );
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'effect.reclaimed':
        if (context !== null) {
          context.effects = context.effects.map((e) =>
            e.id === event.effectId
              ? { ...e, status: 'claimed', claimedAt: event.at, claimExpiredAt: null, reclaimCount: e.reclaimCount + 1, lastError: null }
              : e,
          );
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'effect.succeeded':
        if (context !== null) {
          context.effects = context.effects.map((e) =>
            e.id === event.effectId
              ? { ...e, status: 'succeeded', claimedAt: null, claimExpiredAt: null, lastError: null }
              : e,
          );
          context.lastEffectError = null;
          context.updatedAt = event.at;
        }
        break;
      case 'effect.failed':
        if (context !== null) {
          context.effects = context.effects.map((e) =>
            e.id === event.effectId
              ? { ...e, status: 'failed', claimedAt: null, claimExpiredAt: null, lastError: event.error }
              : e,
          );
          context.lastEffectError = event.error;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.agent_started':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          if (context.agents[event.agentId].status !== 'ready') {
            context.agents[event.agentId].status = 'running';
          }
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'agent.restarted':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          if (context.agents[event.agentId].status !== 'ready') {
            context.agents[event.agentId].status = 'running';
          }
          context.agents[event.agentId].restartCount = (context.agents[event.agentId].restartCount || 0) + 1;
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'agent.needs_attention':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'needs_attention';
          context.updatedAt = event.at;
        }
        break;
      case 'agent.token_exhausted':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'needs_attention';
          context.agents[event.agentId].tokenExhausted = {
            role: event.role || 'do',
            lastCommit: event.lastCommit || null,
            tokensConsumed: event.tokensConsumed != null ? Number(event.tokensConsumed) : null,
            limit: event.limit != null ? Number(event.limit) : null,
            source: event.source || null,
            currentAgentId: event.currentAgentId || context.agents[event.agentId].currentAgentId || event.agentId,
            at: event.at,
          };
          context.updatedAt = event.at;
        }
        break;
      case 'agent.failover_switched':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          const priorCurrent = context.agents[event.agentId].currentAgentId || event.agentId;
          const replacementAgentId = event.replacementAgentId || priorCurrent;
          context.agents[event.agentId].status = 'running';
          context.agents[event.agentId].previousAgentId = priorCurrent;
          context.agents[event.agentId].resumedFromAgentId = event.previousAgentId || priorCurrent;
          context.agents[event.agentId].currentAgentId = replacementAgentId;
          context.agents[event.agentId].tokenExhausted = null;
          if (replacementAgentId && replacementAgentId !== event.agentId) {
            context.agents[event.agentId].modelOverride = null;
            context.agents[event.agentId].effortOverride = null;
          }
          context.updatedAt = event.at;
        }
        break;
      case 'signal.agent_waiting':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'waiting';
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.agent_ready':
      case 'agent.marked_ready':
      case 'agent.force_ready':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'ready';
          // Force-ready is the operator's "this agent is fine, resume it" signal.
          // It must clear any exhaustion mark or the snapshot keeps rendering the
          // slot as failed and the supervisor keeps treating it as a failover
          // candidate even though the session is alive and working.
          context.agents[event.agentId].tokenExhausted = null;
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.agent_submitted':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'ready';
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.agent_failed':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].status = 'failed';
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.heartbeat':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].lastHeartbeatAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'signal.session_lost':
      case 'signal.heartbeat_expired':
        // Display-only: record the event timestamp but do NOT change agent status.
        // Liveness is a dashboard concern, not an engine state concern.
        if (context !== null && context.agents[event.agentId] !== undefined) {
          context.agents[event.agentId].lastHeartbeatExpiredAt = event.at;
          context.updatedAt = event.at;
        }
        break;
      case 'feature.spec_review.started':
      case 'research.spec_review.started':
      case 'spec_review.started':
        if (event.type === 'spec_review.started') {
          console.warn('[projector] legacy spec_review.started event — run `aigon doctor --fix` to migrate to feature.spec_review.started');
        }
        if (context !== null) {
          const prev = context.specReview || {};
          const activeReviewers = Array.isArray(prev.activeReviewers) ? prev.activeReviewers : [];
          const reviewerId = String(event.reviewerId || '').trim();
          if (reviewerId) {
            context.specReview = {
              ...prev,
              activeReviewers: [
                ...activeReviewers.filter((entry) => entry.agentId !== reviewerId),
                { agentId: reviewerId, startedAt: event.at },
              ],
            };
            refreshSpecReviewState(context);
            if (event.type !== 'spec_review.started') {
              context.specReviewReturnLifecycle = captureSpecReviewReturnLifecycle(lifecycle);
              lifecycle = 'spec_review_in_progress';
            }
            context.updatedAt = event.at;
          }
        }
        break;
      case 'feature.spec_review.completed':
      case 'research.spec_review.completed':
        if (context !== null) {
          const prev = context.specReview || {};
          const activeReviewers = Array.isArray(prev.activeReviewers) ? prev.activeReviewers : [];
          context.specReview = {
            ...prev,
            activeReviewers: activeReviewers.filter((entry) => entry.agentId !== event.reviewerId),
          };
          refreshSpecReviewState(context);
          if (shouldMutateLifecycleForSpecReviewEvent(lifecycle)) {
            lifecycle = resolveSpecReviewRestingLifecycle(context, lifecycle);
          }
          context.updatedAt = event.at;
        }
        break;
      case 'feature.spec_review.cancelled':
      case 'research.spec_review.cancelled':
        if (context !== null) {
          const prev = context.specReview || {};
          context.specReview = {
            ...prev,
            activeReviewers: [],
            activeCheckers: [],
          };
          refreshSpecReviewState(context);
          if (shouldMutateLifecycleForSpecReviewEvent(lifecycle)) {
            lifecycle = resolveSpecReviewRestingLifecycle(context, lifecycle);
          }
          context.updatedAt = event.at;
        }
        break;
      case 'feature.spec_revision.cancelled':
      case 'research.spec_revision.cancelled':
        if (context !== null) {
          const prev = context.specReview || {};
          context.specReview = {
            ...prev,
            activeReviewers: [],
            activeCheckers: [],
          };
          refreshSpecReviewState(context);
          context.activeSpecRevision = null;
          if (shouldMutateLifecycleForSpecReviewEvent(lifecycle)) {
            lifecycle = resolveSpecReviewRestingLifecycle(context, lifecycle);
          }
          context.updatedAt = event.at;
        }
        break;
      case 'feature.spec_revision.started':
      case 'research.spec_revision.started':
        if (context !== null) {
          const prev = context.specReview || {};
          const activeCheckers = Array.isArray(prev.activeCheckers) ? prev.activeCheckers : [];
          const checkerId = String(event.checkerId || event.reviewerId || '').trim();
          if (checkerId) {
            context.specReview = {
              ...prev,
              activeCheckers: [
                ...activeCheckers.filter((entry) => entry.agentId !== checkerId),
                { agentId: checkerId, startedAt: event.at },
              ],
            };
            refreshSpecReviewState(context);
            context.activeSpecRevision = {
              agentId: checkerId,
              model: event.revisionModel != null ? event.revisionModel : null,
              effort: event.revisionEffort != null ? event.revisionEffort : null,
              startedAt: event.at,
            };
            if (!context.specReviewReturnLifecycle) {
              context.specReviewReturnLifecycle = captureSpecReviewReturnLifecycle(lifecycle);
            }
            lifecycle = 'spec_revision_in_progress';
            context.updatedAt = event.at;
          }
        }
        break;
      case 'feature.spec_revision.completed':
      case 'research.spec_revision.completed':
        if (context !== null) {
          const prev = context.specReview || {};
          context.specReview = {
            ...prev,
            activeCheckers: [],
          };
          refreshSpecReviewState(context);
          if (event.lastSpecRevision) {
            context.lastSpecRevision = normalizeLastSpecRevision(event.lastSpecRevision);
          } else if (event.ackedBy || event.revisedBy) {
            context.lastSpecRevision = normalizeLastSpecRevision({
              agentId: event.revisedBy || event.ackedBy || null,
              model: event.revisionModel != null ? event.revisionModel : null,
              effort: event.revisionEffort != null ? event.revisionEffort : null,
              revisedAt: event.at || null,
              commitSha: event.commitSha || null,
            });
          } else if (context.activeSpecRevision && context.activeSpecRevision.agentId) {
            context.lastSpecRevision = normalizeLastSpecRevision({
              agentId: context.activeSpecRevision.agentId,
              model: context.activeSpecRevision.model,
              effort: context.activeSpecRevision.effort,
              revisedAt: event.at || null,
              commitSha: event.commitSha || null,
            });
          }
          context.activeSpecRevision = null;
          if (event.requestAnotherCycle === true && typeof event.nextReviewerId === 'string') {
            context.pendingSpecReviewer = event.nextReviewerId;
            if (shouldMutateLifecycleForSpecReviewEvent(lifecycle)) {
              lifecycle = 'spec_review_in_progress';
            }
          } else if (shouldMutateLifecycleForSpecReviewEvent(lifecycle)) {
            lifecycle = resolveSpecReviewRestingLifecycle(context, lifecycle);
          }
          context.updatedAt = event.at;
        }
        break;
      case 'spec_review.check_started':
        console.warn('[projector] legacy spec_review.check_started event — run `aigon doctor --fix` to migrate to feature.spec_revision.started');
        if (context !== null) {
          const prev = context.specReview || {};
          const activeCheckers = Array.isArray(prev.activeCheckers) ? prev.activeCheckers : [];
          const checkerId = String(event.checkerId || '').trim();
          if (checkerId) {
            context.specReview = {
              ...prev,
              activeCheckers: [
                ...activeCheckers.filter((entry) => entry.agentId !== checkerId),
                { agentId: checkerId, startedAt: event.at },
              ],
            };
            refreshSpecReviewState(context);
            context.updatedAt = event.at;
          }
        }
        break;
      case 'spec_review.submitted':
        if (context !== null) {
          const prev = context.specReview || {};
          const pendingReviews = prev.pendingReviews || [];
          const activeReviewers = Array.isArray(prev.activeReviewers) ? prev.activeReviewers : [];
          context.specReview = {
            ...prev,
            pendingReviews: [
              ...pendingReviews.filter((review) => review.reviewId !== event.reviewId),
              {
                reviewId: event.reviewId,
                reviewerId: event.reviewerId,
                summary: event.summary || '',
                submittedAt: event.at,
                commitSha: event.commitSha || null,
              },
            ],
            activeReviewers: activeReviewers.filter((entry) => entry.agentId !== event.reviewerId),
          };
          refreshSpecReviewState(context);
          context.updatedAt = event.at;
        }
        break;
      case 'spec_review.acked':
        if (context !== null) {
          const prev = context.specReview || {};
          const pendingReviews = prev.pendingReviews || [];
          const ackedIds = new Set(Array.isArray(event.reviewIds) ? event.reviewIds : []);
          context.specReview = {
            ...prev,
            pendingReviews: ackedIds.size === 0
              ? []
              : pendingReviews.filter((review) => !ackedIds.has(review.reviewId)),
            activeCheckers: [],
          };
          refreshSpecReviewState(context);
          context.updatedAt = event.at;
        }
        break;
      case 'agent.dropped':
        if (context !== null && context.agents[event.agentId] !== undefined) {
          delete context.agents[event.agentId];
          context.updatedAt = event.at;
        }
        break;
    }
  }

  if (context === null) {
    return null;
  }

  if (!context.specReview) {
    context.specReview = createSpecReviewState();
  } else {
    refreshSpecReviewState(context);
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'agentFailover')) {
    context.agentFailover = null;
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'pauseReason')) {
    context.pauseReason = null;
  }
  if (!Array.isArray(context.reviewCycles)) {
    context.reviewCycles = [];
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'pendingCodeReviewer')) {
    context.pendingCodeReviewer = null;
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'pendingSpecReviewer')) {
    context.pendingSpecReviewer = null;
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'closeRecovery')) {
    context.closeRecovery = null;
  }
  if (!Array.isArray(context.quotaSignals)) {
    context.quotaSignals = [];
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'specAuthor')) {
    context.specAuthor = emptySpecAuthor();
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'lastSpecRevision')) {
    context.lastSpecRevision = emptyLastSpecRevision();
  }
  if (!Array.isArray(context.openEscalations)) {
    context.openEscalations = [];
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'lastCriteriaAttestation')) {
    context.lastCriteriaAttestation = null;
  }
  if (!Object.prototype.hasOwnProperty.call(context, 'specReviewReturnLifecycle')) {
    context.specReviewReturnLifecycle = null;
  }
  // Backward compat: legacy 'submitted' lifecycle (F501) rewrites to 'ready'.
  if (lifecycle === 'submitted') {
    if (!_warnedSubmittedRewrite.has(context.entityId || '')) {
      console.warn(`[projector] legacy lifecycle "submitted" rewritten to "ready" — run aigon doctor --fix to migrate`);
      _warnedSubmittedRewrite.add(context.entityId || '');
    }
    lifecycle = 'ready';
  }
  context.currentSpecState = lifecycle;
  return context;
}

module.exports = { projectContext, resolveCodeReviewAfterCancel };
