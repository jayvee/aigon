...
function buildCandidates(context, entityType) {
  ...
  getActionCandidates(entityType).forEach((candidateDef) => {
    ...
    // Run guard for non-per-agent candidates. Bypass candidates have always
    // used this; machine-governed candidates (F341) also need pre-filtering
    // for fine-grained gates like "pendingCount > 0" that XState can't
    // express as a pure state/event transition.
    if (candidateDef.guard) {
