...
const { FEATURE_ENGINE_STATES } = require('../feature-workflow-rules');
...
function buildStateConfig(stateName, transitions) {
  ...
  transitions.forEach((transition) => {
    on[transition.event] = {
      ...(transition.to !== stateName ? { target: transition.to } : {}),
...
const featureMachine = machineSetup.createMachine({
  id: 'feature',
  initial: 'hydrating',
  context: ({ input }) => input,
  states: Object.fromEntries(
    Object.entries(FEATURE_ENGINE_STATES).map(([stateName, transitions]) => [
      stateName,
