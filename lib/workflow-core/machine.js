...
const machineSetup = setup({
  ...
  guards: {
    ...
    isReady: ({ context }) => context.currentSpecState === 'ready',
    ...
    hasCompletedCodeReview: ({ context }) => Boolean(context.codeReview && context.codeReview.reviewCompletedAt),
    ...
    specReviewReturnToInbox: ({ context }) => context.specReviewReturnLifecycle === 'inbox',
    closeRecoveryReturnToImplementing: ({ context }) =>
      context.closeRecovery && context.closeRecovery.returnSpecState === 'implementing',
    closeRecoveryReturnToCodeReviewInProgress: ({ context }) =>
      context.closeRecovery && context.closeRecovery.returnSpecState === 'code_review_in_progress',
    closeRecoveryReturnToCodeRevisionInProgress: ({ context }) =>
      context.closeRecovery && context.closeRecovery.returnSpecState === 'code_revision_in_progress',
    /** Fallback for ready, missing returnSpecState, or unknown string (matches projector whitelist). Legacy `submitted` also routes here. */
    closeRecoveryReturnToReady: ({ context }) => {
      const rs = context.closeRecovery && context.closeRecovery.returnSpecState;
      if (rs === 'implementing' || rs === 'code_review_in_progress' || rs === 'code_revision_in_progress') {
        return false;
      }
      return true;
    },
  ...
  actions: {
    ...
    captureSpecReviewReturnLifecycle: assign(({ context }) => ({
      specReviewReturnLifecycle: context.currentSpecState === 'inbox' ? 'inbox' : 'backlog',
    })),
    clearSpecReviewReturnLifecycle: assign({
      specReviewReturnLifecycle: null,
    }),
  },
});
  ...
  if (stateName === 'spec_revision_complete') {
    return {
      always: [
        ...
        { target: 'inbox', guard: 'specReviewReturnToInbox', actions: 'clearSpecReviewReturnLifecycle' },
        { target: 'backlog', actions: 'clearSpecReviewReturnLifecycle' },
      ],
    };
  }

  if (stateName === 'spec_review_complete') {
    return {
      always: [
        { target: 'spec_review_in_progress', guard: 'anotherCycleRequested', actions: 'recordNextCycle' },
        { target: 'inbox', guard: 'specReviewReturnToInbox', actions: 'clearSpecReviewReturnLifecycle' },
