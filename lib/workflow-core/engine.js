...
function createBootstrapEvent(entityType, entityId, lifecycle) {
...
function createBootstrapSnapshot(entityType, entityId, lifecycle, specPath, timestamp) {
  const idKey = getEntityIdKey(entityType);
  return {
    [idKey]: entityId,
    lifecycle,
...
function snapshotFromContext(context, eventCount) {
  const specReview = buildSpecReviewSummary(context.specReview && context.specReview.pendingReviews);
  return {
  ...
  return {
    ...
    effects: context.effects.map((effect) => {
      if (effect.id === 'close.move_spec_to_done') {
        ...
        // the feature was closed from 'implementing' or 'reviewing' state.
...
async function canCloseFeature(repoPath, featureId) {
  ...
  if (snapshot.specReview && snapshot.specReview.pendingCount > 0 && snapshot.currentSpecState !== 'closing' && snapshot.currentSpecState !== 'done') {
    ...
    return {
      ok: false,
      snapshot,
      reason: `Feature ${featureId} has ${snapshot.specReview.pendingCount} pending spec review(s) from ${pendingAgents}. Run \`aigon feature-spec-review-check ${featureId}\` before closing.`,
    };
  }
  ...
  const passThroughStates = new Set(['closing', 'done', 'evaluating', 'ready_for_review', 'reviewing']);
...
function applyResearchTransition(context, event) {
  ...
  switch (event.type) {
    ...
    case 'research.review_requested':
      sendIfAllowed({ type: 'research.review', at: event.at });
      break;
...
async function requestResearchReview(repoPath, researchId) {
  return persistResearchEvents(repoPath, researchId, [{ type: 'research.review_requested', at: now() }]);
}
...
async function ensureEntityBootstrapped(repoPath, entityType, entityId, lifecycle = 'backlog') {
  ...
  const event = entityType === 'research'
    ? { type: 'research.bootstrapped', researchId: entityId, stage: lifecycle, lifecycle, at: now() }
    : { type: 'feature.bootstrapped', featureId: entityId, stage: lifecycle, lifecycle, at: now() };
...
function ensureEntityBootstrappedSync(repoPath, entityType, entityId, lifecycle = 'backlog', specPath = null) {
  ...
  return withEntityLockSync(paths.lockPath, () => {
    ...
    const snapshot = createBootstrapSnapshot(entityType, entityId, lifecycle, resolvedSpecPath, timestamp);
