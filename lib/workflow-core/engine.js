...
const { syncBeforeWrite } = require('../spec-store/sync-guard');
const { projectContext, resolveCodeReviewAfterCancel } = require('./projector');
const { deriveAvailableActions } = require('./actions');
const { runEffects, runFeatureEffect } = require('./effects');
const { buildSpecReviewSummary, isSpecReviewCycleAllowed } = require('../spec-review-state');
const {
...
const { shouldClearIntegrityCloseFailureOnRecoveryEnd } = require('../close-gate-predicates');
const { buildSpecAuthor, buildLastSpecRevision, normalizeSpecAuthor, normalizeLastSpecRevision } = require('../spec-author-provenance');
const {
...
} = require('./portable-spec-paths');
const specLifecycle = require('../spec-lifecycle');

const DEFAULT_CLAIM_TIMEOUT_MS = 30_000;

const _specStoreCache = new Map();

function getSpecStore(repoPath) {
  const storage = resolveStorageConfig(repoPath);
  const cacheKey = `${repoPath}:${storage.backend}:${storage.git.remote || ''}:${storage.git.branch || ''}`;
  let store = _specStoreCache.get(cacheKey);
  if (!store) {
    store = createSpecStore({ repoPath, backend: storage.backend, storage });
    _specStoreCache.set(cacheKey, store);
  }
  return store;
}

async function syncSpecStoreAfterWrite(store) {
...
function entityRef(entityType, entityId) {
...
function createBootstrapEvent(entityType, entityId, lifecycle, options = {}) {
  ...
  const specAuthor = options.specAuthor
    || buildSpecAuthor({
      agentId: options.authorAgentId || null,
      model: options.model != null ? options.model : null,
      effort: options.effort != null ? options.effort : null,
      authoredAt: options.authoredAt || now(),
    });
  return {
    ...
    lifecycle,
    specAuthor,
    authorAgentId: specAuthor.agentId || options.authorAgentId || null,
    at: now(),
  };
}

/**
 * A merge-safe "return to backlog" event. Emitting this (rather than wiping the
 * local `.aigon/workflows` dir out of band) is what lets a reset converge across
 * every clone: the event unions by id into the git-branch canonical log and the
 * projector replays it as a fresh backlog seed after any prior `*.started`.
 */
function createResetEvent(entityType, entityId) {
  const idKey = getEntityIdKey(entityType);
  return {
    type: entityType === 'research' ? 'research.reset' : 'feature.reset',
    [idKey]: entityId,
    at: now(),
  };
}
...
function createBootstrapSnapshot(entityType, entityId, lifecycle, specPath, timestamp, options = {}) {
  ...
  const specAuthor = options.specAuthor
    || buildSpecAuthor({
      ...
      model: options.model != null ? options.model : null,
      effort: options.effort != null ? options.effort : null,
      authoredAt: options.authoredAt || timestamp,
    });
  return {
    entityType,
    [idKey]: entityId,
    ...
    mode: null,
    specAuthor,
    lastSpecRevision: buildLastSpecRevision(),
    authorAgentId: specAuthor.agentId || options.authorAgentId || null,
    winnerAgentId: null,
    agents: {},
...
function snapshotFromContext(context, eventCount) {
  ...
  return {
    ...
    specAuthor: normalizeSpecAuthor(context.specAuthor),
    lastSpecRevision: normalizeLastSpecRevision(context.lastSpecRevision),
    ...
    pendingCodeReviewer: context.pendingCodeReviewer || null,
    pendingSpecReviewer: context.pendingSpecReviewer || null,
    nudges: Array.isArray(context.nudges) ? context.nudges.slice() : [],
    quotaSignals: Array.isArray(context.quotaSignals) ? context.quotaSignals.slice() : [],
    effects: context.effects,
    lastEffectError: context.lastEffectError,
    lastCloseFailure: context.lastCloseFailure || null,
    closeRecovery: context.closeRecovery || null,
    openEscalations: Array.isArray(context.openEscalations) ? context.openEscalations.map(cloneOpenEscalation) : [],
    lastCriteriaAttestation: context.lastCriteriaAttestation || null,
...
function appendQuotaPauseSignalToContext(context, event, kind = 'paused') {
  const base = kind === 'resumed'
    ? { kind: 'resumed', agentId: event.agentId, role: event.role || 'do', atISO: event.at }
    : {
        kind: 'paused',
        ...
        detectedAt: event.detectedAt || event.at,
        paneSampleHash: event.paneSampleHash || null,
  ...
  return {
    ...context,
    quotaSignals: [
      ...((Array.isArray(context.quotaSignals) ? context.quotaSignals : [])),
      base,
    ].slice(-36),
    updatedAt: event.atISO || event.at || context.updatedAt,
  };
}
...
function cloneOpenEscalation(entry) {
  return {
    escalationId: entry.escalationId,
    category: entry.category,
    reason: entry.reason,
    reviewerAgentId: entry.reviewerAgentId || null,
    logPath: entry.logPath || null,
    lineNumber: entry.lineNumber != null ? entry.lineNumber : null,
    raisedAt: entry.raisedAt || entry.at || null,
    source: entry.source || null,
  };
}

function appendEscalationRaisedToContext(context, event) {
  const open = Array.isArray(context.openEscalations) ? context.openEscalations.map(cloneOpenEscalation) : [];
  if (open.some((entry) => entry.escalationId === event.escalationId)) {
    return { ...context, updatedAt: event.at || context.updatedAt };
  }
  open.push(cloneOpenEscalation({
  ...
  return { ...context, openEscalations: open, updatedAt: event.at || context.updatedAt };
}

function removeOpenEscalationFromContext(context, event) {
  const open = Array.isArray(context.openEscalations) ? context.openEscalations : [];
  const next = open.filter((entry) => entry.escalationId !== event.escalationId);
  if (next.length === open.length) {
    return { ...context, updatedAt: event.at || context.updatedAt };
  }
  return { ...context, openEscalations: next, updatedAt: event.at || context.updatedAt };
}
...
function computeStableAwareSpecPath(repoPath, entityType, entityId, lifecycle, snapshot) {
  return specLifecycle.findCanonicalSpecPath(repoPath, entityType, entityId)
    || getSpecPathForEntity(repoPath, entityType, entityId, lifecycle, { snapshot });
}
...
function buildEffects(repoPath, previous, next, event) {
  ...
  if (
    specLifecycle.shouldMoveSpecFiles(repoPath) &&
    previous !== null &&
    previousState !== next.currentSpecState &&
    event.type !== 'feature.close_requested' &&
    event.type !== 'feature.closed' &&
    event.type !== 'feature.reset' &&
    event.type !== 'research.reset'
  ) {
    effects.push({
      id: `spec.move.${previousState}.to.${next.currentSpecState}`,
      type: 'move_spec',
      payload: buildMoveSpecPayload(repoPath, {
        entityType: 'feature',
        entityId: next.featureId,
        fromLifecycle: previousState,
        toLifecycle: next.currentSpecState,
...
function materializeContext(repoPath, context) {
  ...
  return {
    ...context,
    specPath: computeStableAwareSpecPath(repoPath, 'feature', context.featureId, resolvedSpecLifecycle, context),
  };
}

function materializeMoveSpecEffect(repoPath, entityType, entityId, context, effect) {
  ...
  if (effect.id === 'close.move_spec_to_done') {
    ...
    const computeTarget = entityType === 'research' ? computeResearchSpecPath : computeSpecPath;
    return {
      ...effect,
      payload: resolveMoveSpecPayload(repoPath, entityType, entityId, {
        ...(effect.payload || {}),
        entityType,
        entityId,
        fromPath,
        toLifecycle: 'done',
        toPath: computeTarget(repoPath, entityId, 'done', context),
      }, { snapshot: context }),
    };
  }
  return {
...
function materializePendingEffects(repoPath, context) {
  return {
    ...
    effects: context.effects.map((effect) => {
      ...
      return materializeMoveSpecEffect(repoPath, 'feature', context.featureId, context, effect);
    }),
  };
}
...
function applyCodeReviewEventToContext(context, event) {
  ...
  if (event.type.endsWith('code_review.cancelled')) {
    return {
      ...context,
      codeReview: resolveCodeReviewAfterCancel(context, event.at),
...
function applyTransition(context, event) {
  ...
  switch (event.type) {
    ...
    case 'feature.code_review.cancelled':
      if (snapshot.can({ type: 'feature.code_review.cancelled', at: event.at })) {
        actor.send({ type: 'feature.code_review.cancelled', at: event.at });
      }
      return {
        ...applyCodeReviewEventToContext(actor.getSnapshot().context, event),
        pendingCodeReviewer: null,
    ...
    case 'feature.code_revision.completed':
      sendIfAllowed({
        type: 'feature.code_revision.completed',
        revisionAgentId: event.revisionAgentId,
        requestAnotherCycle: event.requestAnotherCycle === true,
        nextReviewerId: event.nextReviewerId || null,
        at: event.at,
      });
      return {
        ...applyCodeReviewEventToContext(actor.getSnapshot().context, event),
        reviewCycles: actor.getSnapshot().context.reviewCycles || [],
        pendingCodeReviewer: actor.getSnapshot().context.pendingCodeReviewer || null,
        currentSpecState: actor.getSnapshot().value,
      };
    ...
    case 'feature.close_recovery.started': {
      const priorLifecycle = context.currentSpecState;
      sendIfAllowed({ type: 'feature.close_recovery.started', at: event.at });
      return {
        ...actor.getSnapshot().context,
        closeRecovery: {
          agentId: event.agentId || null,
          startedAt: event.at,
          returnSpecState: event.returnSpecState || priorLifecycle,
          sessionName: event.sessionName || null,
    ...
    case 'feature.close_recovery.ended':
    case 'feature.close_recovery.cancelled':
      sendIfAllowed({ type: event.type, at: event.at });
      {
        const priorFailure = actor.getSnapshot().context.lastCloseFailure || null;
        const clearFailure = shouldClearIntegrityCloseFailureOnRecoveryEnd(event, priorFailure);
        return {
          ...actor.getSnapshot().context,
          closeRecovery: null,
          lastCloseFailure: clearFailure ? null : (priorFailure || null),
          updatedAt: event.at,
          currentSpecState: actor.getSnapshot().value,
        };
      }
    ...
    case 'review.escalation_raised':
      return appendEscalationRaisedToContext(context, event);
    case 'review.escalation_accepted':
    case 'review.escalation_spun_off':
    case 'review.escalation_reopened':
      return removeOpenEscalationFromContext(context, event);
    case 'feature.agent_quota_paused':
    case 'research.agent_quota_paused':
      return appendQuotaPauseSignalToContext(context, event, 'paused');
    case 'feature.agent_quota_resumed':
    case 'research.agent_quota_resumed':
      return appendQuotaPauseSignalToContext(context, event, 'resumed');
    ...
    case 'agent.failover_switched':
      if (context.agents[event.agentId] === undefined) return context;
      {
        ...
        const replacementAgentId = event.replacementAgentId || priorCurrent;
        const nextAgent = {
          ...context.agents[event.agentId],
          status: 'running',
          previousAgentId: priorCurrent,
          resumedFromAgentId: event.previousAgentId || priorCurrent,
          currentAgentId: replacementAgentId,
          tokenExhausted: null,
          lastHeartbeatAt: event.at,
        };
        if (replacementAgentId && replacementAgentId !== event.agentId) {
          nextAgent.modelOverride = null;
          nextAgent.effortOverride = null;
        }
        return {
          ...context,
          agents: {
    ...
    case 'feature.close_gate_failed': {
      const gateKind = event.gateKind || 'post-merge-gate';
      if (gateKind === 'preauth-validation') {
        return {
          ...context,
          lastCloseFailure: {
            kind: 'preauth-validation',
            unmatched: Array.isArray(event.unmatched) ? event.unmatched : [],
            noVerifyPreauth: Boolean(event.noVerifyPreauth),
            stderrTail: event.outputTail || '',
            at: event.at,
          },
          updatedAt: event.at,
        };
      }
      if (gateKind === 'implementation-log') {
        return {
          ...context,
          lastCloseFailure: {
            kind: 'implementation-log',
            expectedPattern: event.expectedPattern || null,
            stderrTail: event.outputTail || '',
            at: event.at,
          },
          updatedAt: event.at,
      ...
      if (gateKind === 'criteria-attestation') {
        return context;
      }
      return {
        ...context,
        lastCloseFailure: {
          ...
          gateCommand: event.gateCommand || null,
          exitCode: event.exitCode != null ? event.exitCode : 1,
          stderrTail: event.outputTail || '',
          mergedCommitSha: event.mergedCommitSha || null,
    ...
    case 'feature.criteria_attested':
    case 'feature.criteria_attestation_bypassed':
      return context;
    ...
    case 'feature.spec_revision.completed': {
      ...
      if (snapshot.can(specRevEvent)) {
        actor.send(specRevEvent);
      }
      return {
        ...actor.getSnapshot().context,
        specReview: {
        ...
        pendingSpecReviewer: actor.getSnapshot().context.pendingSpecReviewer || null,
        updatedAt: event.at,
        currentSpecState: actor.getSnapshot().value,
      };
    }
    case 'feature.spec_review.cancelled':
      if (snapshot.can({ type: 'feature.spec_review.cancelled', at: event.at })) {
        actor.send({ type: 'feature.spec_review.cancelled', at: event.at });
      }
      return {
        ...actor.getSnapshot().context,
        specReview: {
    ...
    case 'feature.spec_revision.cancelled':
      if (snapshot.can({ type: 'feature.spec_revision.cancelled', at: event.at })) {
        actor.send({ type: 'feature.spec_revision.cancelled', at: event.at });
      }
      return {
        ...actor.getSnapshot().context,
        specReview: {
...
async function loadCurrentUnlocked(repoPath, featureId) {
...
async function initializeCloseEffects(repoPath, featureId) {
  ...
  const store = getSpecStore(repoPath);
  const ref = entityRef('feature', featureId);
  const lockResult = await store.lock(ref, async () => {
    try {
...
async function claimNextEffect(repoPath, featureId, claimTimeoutMs) {
  ...
  const ref = entityRef('feature', featureId);
  const lockResult = await store.lock(ref, async () => {
    const { context, events } = await loadCurrentUnlocked(repoPath, featureId);
    ...
    if (candidate === undefined) {
      ...
      const completed = await applyEventsUnlocked(
        repoPath, featureId, [{ type: 'feature.closed', at: now() }],
      );
    ...
    const { context: refreshedContext, events: refreshedEvents } = await loadCurrentUnlocked(repoPath, featureId);
...
async function completeClaimedEffect(repoPath, featureId, effectId, error) {
  ...
  const lockResult = await store.lock(ref, async () => {
    ...
    const nextSnapshot =
      error === null
        ? await applyEventsUnlocked(repoPath, featureId, [{ type: 'effect.succeeded', effectId, at: now() }])
        : await applyEventsUnlocked(repoPath, featureId, [{ type: 'effect.failed', effectId, error, at: now() }]);
    ...
    if (remaining.length === 0) {
      const closed = await applyEventsUnlocked(repoPath, featureId, [{ type: 'feature.closed', at: now() }]);
...
async function emitSignal(repoPath, featureId, signal, agentId, options = {}) {
  if (options.entityType === 'research') {
    return emitResearchSignal(repoPath, featureId, signal, agentId, options);
  }
  const eventType = SIGNAL_EVENT_MAP[signal];
  const store = getSpecStore(repoPath);
  const ref = entityRef('feature', featureId);

  return store.lock(ref, async () => {
    const { context, events } = await loadCurrentUnlocked(repoPath, featureId);

    if (isSignalRedundant(context, eventType, agentId)) {
      return snapshotFromContext(context, events.length);
    }

    const event = { type: eventType, agentId, at: now() };
    if (options.source) event.source = options.source;
    return applyEventsUnlocked(repoPath, featureId, [event]);
  });
}
...
async function listKnownEscalationIds(repoPath, featureId) {
  ...
  try {
    const events = await listEvents(repoPath, featureId);
...
async function syncReviewEscalationsFromLog(repoPath, featureId, { reviewerAgentId = null, source = null } = {}) {
  ...
  if (events.length > 0) {
    await persistFeatureEscalationEvents(repoPath, featureId, events);
  }
...
async function recordCodeReviewCompleted(repoPath, entityType, entityId, payload = {}) {
  const prefix = entityType === 'research' ? 'research' : 'feature';
  const snapshot = await persistEntityEvents(repoPath, entityType, entityId, [{
  ...
  if (entityType === 'feature') {
    try {
      await syncReviewEscalationsFromLog(repoPath, entityId, {
        reviewerAgentId: payload.reviewerId || payload.agentId || null,
        source: payload.source ? `${payload.source}/escalation-sync` : 'code_review.completed/escalation-sync',
      });
    } catch (err) {
      console.warn(`⚠️  Review escalation sync failed for feature ${entityId}: ${err.message}`);
    }
  }
  return snapshot;
}

async function persistFeatureEscalationEvents(repoPath, featureId, events) {
  if (!Array.isArray(events) || events.length === 0) return showFeature(repoPath, featureId);
  return persistEvents(repoPath, featureId, events);
}

async function recordCodeReviewCancelled(repoPath, entityType, entityId, payload = {}) {
  const snapshot = await ensureEntityBootstrapped(repoPath, entityType, entityId, 'backlog');
  if (snapshot.lifecycle !== 'code_review_in_progress') {
    throw new Error(
      `Cancel code-review rejected: ${entityType} ${entityId} is in lifecycle '${snapshot.lifecycle}'. ` +
      `Only code_review_in_progress entities can be cancelled.`
    );
  }
  const prefix = entityType === 'research' ? 'research' : 'feature';
  const event = {
    type: `${prefix}.code_review.cancelled`,
    at: payload.at || now(),
    source: payload.source || null,
  };
  return persistEntityEvents(repoPath, entityType, entityId, [event]);
}
...
async function recordCodeRevisionCompleted(repoPath, entityType, entityId, payload = {}) {
  ...
  return persistEntityEvents(repoPath, entityType, entityId, [{
    ...
    revisionAgentId: payload.revisionAgentId || null,
    requestAnotherCycle: payload.requestAnotherCycle === true,
    nextReviewerId: payload.nextReviewerId || null,
    at: payload.at || now(),
    source: payload.source || null,
  }]);
}
...
async function recordCloseRecoveryStarted(repoPath, featureId, payload = {}) {
  return persistEvent(repoPath, featureId, {
    type: 'feature.close_recovery.started',
    agentId: payload.agentId || null,
    sessionName: payload.sessionName || null,
    source: payload.source || null,
    returnSpecState: payload.returnSpecState || null,
    at: payload.at || now(),
  });
}

async function recordCloseRecoveryEnded(repoPath, featureId, payload = {}) {
  return persistEvent(repoPath, featureId, {
    type: payload.cancelled ? 'feature.close_recovery.cancelled' : 'feature.close_recovery.ended',
    agentId: payload.agentId || null,
    source: payload.source || null,
    at: payload.at || now(),
  });
...
/**
 * Return a feature to the "never started" backlog state.
 * Used by `aigon feature-reset`. Records a merge-safe `feature.reset` event
 * through the specstore rather than wiping the local event log out of band:
 * under git-branch storage the canonical log lives on the `aigon-state` branch,
 * so an out-of-band local wipe was silently re-projected back to
 * `implementing` on the next `storage sync`. Emitting the event makes the reset
 * converge on every clone the log syncs to (and keeps auditable history).
 * Idempotent: silent no-op when the feature has no engine state.
 */
async function resetFeature(repoPath, featureId) {
  ...
  const ref = entityRef('feature', featureId);
  const events = await store.readEvents(ref);
  if (events.length === 0 && !fs.existsSync(root)) {
    return { removed: false, path: root };
  }
  await persistEvents(repoPath, featureId, [createResetEvent('feature', featureId)]);
  return { removed: true, path: root };
}

/**
 * Return a research topic to the "never started" backlog state.
 * Used by `aigon research-reset`. See {@link resetFeature} — records a
 * merge-safe `research.reset` event so the reset converges across git-branch
 * clones instead of being re-projected from the canonical log.
 * Idempotent: silent no-op when the research topic has no engine state.
 */
async function resetResearch(repoPath, researchId) {
  const { root } = getResearchWorkflowPaths(repoPath, researchId);
  const store = getSpecStore(repoPath);
  const ref = entityRef('research', researchId);
  const events = await store.readEvents(ref);
  if (events.length === 0 && !fs.existsSync(root)) {
    return { removed: false, path: root };
  }
  await persistResearchEvents(repoPath, researchId, [createResetEvent('research', researchId)]);
  return { removed: true, path: root };
}

/**
 * Hard-remove all local workflow-core state for an entity. Unlike
 * {@link resetFeature} (which keeps the entity in the workflow at backlog), this
 * is for `entity-delete`, where the entity ceases to exist — so it wipes the
 * `.aigon/workflows/<type>/<id>/` dir out of band rather than recording an
 * event. Idempotent: silent no-op when the dir does not exist.
 */
function removeEntityEngineState(repoPath, entityType, entityId) {
  const { root } = getEntityWorkflowPaths(repoPath, entityType, entityId);
...
async function canCloseFeature(repoPath, featureId) {
  ...
  const passThroughStates = new Set([
    'closing',
    'done',
    'evaluating',
    'ready_for_review',
...
async function listEvents(repoPath, featureId) {
  return getSpecStore(repoPath).readEvents(entityRef('feature', featureId));
}
...
function materializeResearchContext(repoPath, context) {
  ...
  return {
    ...context,
    entityType: 'research',
    specPath: computeStableAwareSpecPath(repoPath, 'research', context.featureId, resolvedSpecLifecycle, context),
  };
}
...
function materializeResearchPendingEffects(repoPath, context) {
  return {
    ...
    effects: context.effects.map((effect) => {
      ...
      return materializeMoveSpecEffect(repoPath, 'research', context.featureId, context, effect);
    }),
  };
}
...
function applyResearchTransition(context, event) {
  ...
  switch (event.type) {
    ...
    case 'research.code_review.cancelled':
      if (snapshot.can({ type: 'research.code_review.cancelled', at: event.at })) {
        actor.send({ type: 'research.code_review.cancelled', at: event.at });
      }
      return {
        ...applyCodeReviewEventToContext(actor.getSnapshot().context, event),
        entityType: 'research',
    ...
    case 'research.code_revision.completed':
      sendIfAllowed({
        type: 'research.code_revision.completed',
        revisionAgentId: event.revisionAgentId,
        requestAnotherCycle: event.requestAnotherCycle === true,
        nextReviewerId: event.nextReviewerId || null,
        at: event.at,
    ...
    case 'research.spec_revision.completed': {
      const resSpecRevEvent = {
        type: 'research.spec_revision.completed',
        requestAnotherCycle: event.requestAnotherCycle === true,
        nextReviewerId: event.nextReviewerId || null,
        at: event.at,
      };
      if (snapshot.can(resSpecRevEvent)) {
        actor.send(resSpecRevEvent);
      }
      return {
        ...actor.getSnapshot().context,
        entityType: 'research',
    ...
    case 'research.spec_review.cancelled':
      if (snapshot.can({ type: 'research.spec_review.cancelled', at: event.at })) {
        actor.send({ type: 'research.spec_review.cancelled', at: event.at });
      }
      return {
        ...actor.getSnapshot().context,
        entityType: 'research',
    ...
    case 'research.spec_revision.cancelled':
      if (snapshot.can({ type: 'research.spec_revision.cancelled', at: event.at })) {
        actor.send({ type: 'research.spec_revision.cancelled', at: event.at });
      }
      return {
        ...actor.getSnapshot().context,
        entityType: 'research',
...
async function loadCurrentResearchUnlocked(repoPath, researchId) {
  const store = getSpecStore(repoPath);
  const ref = entityRef('research', researchId);
  const events = await store.readEvents(ref);
...
async function applyResearchEventsUnlocked(repoPath, researchId, newEvents) {
  const store = getSpecStore(repoPath);
  const ref = entityRef('research', researchId);
  const previousEvents = await store.readEvents(ref);
  const previous =
  ...
  for (const event of newEvents) {
    if (event.type === 'research.started' || event.type === 'research.bootstrapped' || event.type === 'research.reset') {
...
async function persistResearchEvents(repoPath, researchId, newEvents) {
  ...
  await syncBeforeWrite(repoPath, ref);
  return store.lock(ref, async () =>
    applyResearchEventsUnlocked(repoPath, researchId, newEvents),
  { retry: false });
}
...
async function startResearch(repoPath, researchId, mode, agents = []) {
  ...
  const ref = entityRef('research', researchId);
  return store.lock(ref, async () => {
    const existing = await store.readEvents(ref);
...
async function closeResearch(repoPath, researchId) {
  ...
  return store.lock(ref, async () => {
    ...
    if (closeEvents.length > 0) {
      await applyResearchEventsUnlocked(repoPath, researchId, closeEvents);
    }
...
async function emitResearchSignal(repoPath, researchId, signal, agentId, options = {}) {
  const eventType = SIGNAL_EVENT_MAP[signal];
  const store = getSpecStore(repoPath);
  const ref = entityRef('research', researchId);

  return store.lock(ref, async () => {
    const { context, events } = await loadCurrentResearchUnlocked(repoPath, researchId);
    if (isSignalRedundant(context, eventType, agentId)) {
...
async function ensureEntityBootstrapped(repoPath, entityType, entityId, lifecycle = 'backlog') {
  ...
  if (entityType === 'research') {
    const store = getSpecStore(repoPath);
    const ref = entityRef('research', entityId);
    return store.lock(ref, async () => {
      const existingEvents = await store.readEvents(ref);
      if (existingEvents.length > 0) return showResearch(repoPath, entityId);
      return applyResearchEventsUnlocked(repoPath, entityId, [event]);
    });
  }

  const store = getSpecStore(repoPath);
  const ref = entityRef('feature', entityId);
  return store.lock(ref, async () => {
    const existingEvents = await store.readEvents(ref);
    if (existingEvents.length > 0) return showFeature(repoPath, entityId);
    return applyEventsUnlocked(repoPath, entityId, [event]);
  });
}
...
function ensureEntityBootstrappedSync(repoPath, entityType, entityId, lifecycle = 'backlog', specPath = null, options = {}) {
  ...
  return withEntityLockSync(paths.lockPath, () => {
    ...
    const eventsRaw = fs.existsSync(paths.eventsPath) ? fs.readFileSync(paths.eventsPath, 'utf8').trim() : '';
    let snapshot;
    if (eventsRaw) {
      const events = eventsRaw.split('\n').filter(Boolean).map((line) => JSON.parse(line));
      const projected = projectContext(events);
      if (projected === null) {
        throw new Error(`Cannot rebuild ${entityType} ${entityId} snapshot from its existing event history`);
      }
      snapshot = entityType === 'research'
        ? snapshotFromResearchContext(
          materializeResearchPendingEffects(repoPath, materializeResearchContext(repoPath, projected)),
          events.length,
        )
        : snapshotFromContext(
          materializePendingEffects(repoPath, materializeContext(repoPath, projected)),
          events.length,
        );
    } else {
      snapshot = createBootstrapSnapshot(entityType, entityId, lifecycle, resolvedSpecPath, timestamp, options);
...
async function listEntityEvents(repoPath, entityType, entityId) {
  return getSpecStore(repoPath).readEvents(entityRef(entityType, entityId));
}

function transitionEntityLifecycleSync(repoPath, entityType, entityId, lifecycle, specPath) {
  const paths = getEntityWorkflowPaths(repoPath, entityType, entityId);
  return withEntityLockSync(paths.lockPath, () => {
    if (!fs.existsSync(paths.snapshotPath)) {
      throw new Error(
        `Missing workflow snapshot for ${entityType} ${entityId}. Run \`aigon doctor --fix\` to migrate inbox entities, then retry.`,
      );
    }
    const timestamp = now();
    const event = createBootstrapEvent(entityType, entityId, lifecycle);
    const eventsRaw = fs.existsSync(paths.eventsPath) ? fs.readFileSync(paths.eventsPath, 'utf8') : '';
    const lines = eventsRaw.split('\n').filter(Boolean);
    lines.push(JSON.stringify(event));
    fs.writeFileSync(paths.eventsPath, `${lines.join('\n')}\n`, 'utf8');

    const snapshot = JSON.parse(fs.readFileSync(paths.snapshotPath, 'utf8'));
    rewriteWorkflowIdentityPayload(entityType, snapshot, entityId, entityId, specPath, lifecycle);
...
function migrateEntityWorkflowIdSync(repoPath, entityType, fromId, toId, specPath, lifecycle = 'backlog') {
  ...
  return withEntityLockSync(fromPaths.lockPath, () => withEntityLockSync(targetLockPath, () => {
    ...
    try {
      ...
      // Atomically rename transcript hot-tier dir (slug → numeric) within the same lock scope.
      try {
        const transcriptStore = require('../transcript-store');
        transcriptStore.renameTranscriptDirSync(repoPath, entityType, fromId, toId);
      } catch (_) { /* best-effort — transcript dir rename never blocks migration */ }
...
async function recordSpecReviewStarted(repoPath, entityType, entityId, payload = {}) {
  const snapshot = await ensureEntityBootstrapped(repoPath, entityType, entityId, 'backlog');
  if (!isSpecReviewCycleAllowed(snapshot.lifecycle)) {
    throw new Error(
      `Spec-review cycle rejected: ${entityType} ${entityId} is in lifecycle '${snapshot.lifecycle}'. ` +
      `Spec review is only allowed for inbox or backlog entities.`
    );
  }
...
async function recordSpecRevisionStarted(repoPath, entityType, entityId, payload = {}) {
  const snapshot = await ensureEntityBootstrapped(repoPath, entityType, entityId, 'backlog');
  const lcAllowed = isSpecReviewCycleAllowed(snapshot.lifecycle) || snapshot.lifecycle === 'spec_review_in_progress';
  if (!lcAllowed) {
    throw new Error(
      `Spec-revise rejected: ${entityType} ${entityId} is in lifecycle '${snapshot.lifecycle}'. ` +
      `Spec revise is only allowed for inbox, backlog, or spec_review_in_progress entities.`
    );
  }
  const checkerId = String(payload.checkerId || '').trim();
  if (!checkerId) throw new Error('recordSpecRevisionStarted requires checkerId');
  const event = {
    type: entityType === 'research' ? 'research.spec_revision.started' : 'feature.spec_revision.started',
    checkerId,
    revisionModel: payload.model != null ? payload.model : null,
    revisionEffort: payload.effort != null ? payload.effort : null,
    at: payload.at || now(),
  };
  return entityType === 'research'
    ? persistResearchEvents(repoPath, entityId, [event])
...
async function recordSpecReviewCheckStarted(repoPath, entityType, entityId, payload = {}) {
  return recordSpecRevisionStarted(repoPath, entityType, entityId, payload);
}
...
async function recordSpecReviewCancelled(repoPath, entityType, entityId, payload = {}) {
  const snapshot = await ensureEntityBootstrapped(repoPath, entityType, entityId, 'backlog');
  if (snapshot.lifecycle !== 'spec_review_in_progress') {
    throw new Error(
      `Cancel spec-review rejected: ${entityType} ${entityId} is in lifecycle '${snapshot.lifecycle}'. ` +
      `Only spec_review_in_progress entities can be cancelled.`
    );
  }
  const event = {
    type: entityType === 'research' ? 'research.spec_review.cancelled' : 'feature.spec_review.cancelled',
    at: payload.at || now(),
  };
  return entityType === 'research'
    ? persistResearchEvents(repoPath, entityId, [event])
...
async function recordSpecRevisionCancelled(repoPath, entityType, entityId, payload = {}) {
  const snapshot = await ensureEntityBootstrapped(repoPath, entityType, entityId, 'backlog');
  if (snapshot.lifecycle !== 'spec_revision_in_progress') {
    throw new Error(
      `Cancel spec-revision rejected: ${entityType} ${entityId} is in lifecycle '${snapshot.lifecycle}'. ` +
      `Only spec_revision_in_progress entities can be cancelled.`
    );
  }
  const event = {
    type: entityType === 'research' ? 'research.spec_revision.cancelled' : 'feature.spec_revision.cancelled',
    at: payload.at || now(),
  };
  return entityType === 'research'
    ? persistResearchEvents(repoPath, entityId, [event])
...
async function recordSpecReviewAcknowledged(repoPath, entityType, entityId, payload = {}) {
  ...
  const reviserId = payload.revisedBy || payload.ackedBy || null;
  let model = payload.model != null ? payload.model : null;
  let effort = payload.effort != null ? payload.effort : null;
  if (model == null && effort == null && reviserId) {
    try {
      const events = await getSpecStore(repoPath).readEvents(entityRef(entityType, entityId));
      const startedType = entityType === 'research' ? 'research.spec_revision.started' : 'feature.spec_revision.started';
      const started = [...events].reverse().find((entry) => entry.type === startedType && entry.checkerId === reviserId);
      if (started) {
        if (model == null) model = started.revisionModel != null ? started.revisionModel : null;
        if (effort == null) effort = started.revisionEffort != null ? started.revisionEffort : null;
      }
    } catch (_) { /* best-effort */ }
  }
  const lastSpecRevision = buildLastSpecRevision({
    agentId: reviserId,
    model,
    effort,
    revisedAt: ackedAt,
  ...
  const events = [
    ...
    },
    {
      ...
      ackedBy: payload.ackedBy || null,
      revisedBy: reviserId,
      revisionModel: lastSpecRevision.model,
      revisionEffort: lastSpecRevision.effort,
      lastSpecRevision,
      commitSha: payload.commitSha || null,
      at: ackedAt,
    },
...
module.exports = {
  ...
  transitionEntityLifecycleSync,
  recordSpecReviewStarted,
  recordSpecReviewCompleted,
  recordSpecReviewCancelled,
  recordSpecRevisionCancelled,
  recordSpecRevisionStarted,
  ...
  recordCodeReviewCancelled,
  persistFeatureEscalationEvents,
  ...
  requestResearchEval,
  selectWinner,
  recordCloseRecoveryStarted,
  recordCloseRecoveryEnded,
  closeFeature,
  resetFeature,
  resetResearch,
  removeEntityEngineState,
  closeResearch,
