...
function computeResearchSpecPath(repoPath, researchId, lifecycle) {
...
function materializeResearchContext(repoPath, context) {
  ...
  return {
    ...context,
    entityType: 'research',
    specPath: computeResearchSpecPath(repoPath, context.featureId, resolvedSpecLifecycle),
  };
}
    ...
    effects: context.effects.map((effect) => {
      if (effect.id === 'close.move_spec_to_done') {
        return {
          ...
          payload: {
            fromPath: context.specPath,
            toPath: computeResearchSpecPath(repoPath, context.featureId, 'done'),
          },
        };
      }
      return effect;
...
function buildResearchEffects(repoPath, previous, next, event) {
  ...
  if (event.type === 'research.started') {
    effects.push({
      ...
      type: 'move_spec',
      payload: {
        fromPath: computeResearchSpecPath(repoPath, next.featureId, 'backlog'),
        toPath: computeResearchSpecPath(repoPath, next.featureId, next.currentSpecState),
      },
    });
  }
...
async function startResearch(repoPath, researchId, mode, agents = []) {
  ...
  return withFeatureLock(lockPath, async () => {
    ...
    // If already started (has events beyond bootstrap), return current state
    ...
    // Start fresh or from bootstrapped backlog state
      ...
      closeEvents.push({
        ...
        effect: {
          ...
          payload: {
            ...
            toPath: computeResearchSpecPath(repoPath, researchId, 'done'),
          },
        },
        at: now(),
      });
