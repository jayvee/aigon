  ...
  switch (event.type) {
    ...
    case 'feature.close_recovery.cancelled':
      ...
      return {
        ...
        closeRecovery: null,
        lastCloseFailure: (
          actor.getSnapshot().context.lastCloseFailure
          && actor.getSnapshot().context.lastCloseFailure.kind === 'criteria-attestation'
        ) ? null : (actor.getSnapshot().context.lastCloseFailure || null),
        updatedAt: event.at,
        currentSpecState: actor.getSnapshot().value,
      };
    ...
    case 'feature.close_gate_failed': {
      ...
      if (gateKind === 'criteria-attestation') {
        return {
          ...context,
          lastCloseFailure: {
            kind: 'criteria-attestation',
            unattested: Array.isArray(event.unattested) ? event.unattested : [],
            invalid: Array.isArray(event.invalid) ? event.invalid : [],
            noVerifyCriteria: Boolean(event.noVerifyCriteria),
            stderrTail: event.outputTail || '',
            at: event.at,
          },
          updatedAt: event.at,
    ...
    case 'feature.criteria_attested':
      return {
        ...
        lastCriteriaAttestation: {
          counts: event.counts || {},
          criteria: Array.isArray(event.criteria) ? event.criteria : [],
          at: event.at,
        },
        lastCloseFailure: (
          context.lastCloseFailure
          && context.lastCloseFailure.kind === 'criteria-attestation'
        ) ? null : (context.lastCloseFailure || null),
        updatedAt: event.at,
      };
