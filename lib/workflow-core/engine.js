...
function createBootstrapSnapshot(repoPath, entityType, entityId, lifecycle, specPath, timestamp, options = {}) {
  const idKey = getEntityIdKey(entityType);
  const specAuthor = options.specAuthor
    || buildSpecAuthor({
      agentId: options.authorAgentId || null,
  ...
  return {
    ...
    specPath: toPortableSpecPath(repoPath, specPath),
    effects: [],
    nudges: [],
...
function rewriteWorkflowIdentityPayload(repoPath, entityType, payload, fromId, toId, specPath, lifecycle = null) {
  const idKey = getEntityIdKey(entityType);
  ...
  if (payload.specPath !== undefined) {
    payload.specPath = toPortableSpecPath(repoPath, specPath);
  }
...
function ensureEntityBootstrappedSync(repoPath, entityType, entityId, lifecycle = 'backlog', specPath = null, options = {}) {
  ...
  return withEntityLockSync(paths.lockPath, () => {
    ...
    const snapshot = createBootstrapSnapshot(repoPath, entityType, entityId, lifecycle, resolvedSpecPath, timestamp, options);
    ...
    try {
      ...
      if (fs.existsSync(tempEventsPath)) {
        const migratedEvents = fs.readFileSync(tempEventsPath, 'utf8')
          .split('\n')
          .filter(Boolean)
          .map((line) => JSON.stringify(rewriteWorkflowIdentityPayload(repoPath, entityType, JSON.parse(line), fromId, toId, specPath, lifecycle)))
          .join('\n');
      ...
      rewriteWorkflowIdentityPayload(repoPath, entityType, snapshot, fromId, toId, specPath, lifecycle);
