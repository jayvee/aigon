...
function listVisibleSpecMatches(visibleRoot, prefix) {
  // Stage directories follow `0N-<name>/` (01-inbox, 02-backlog, ...).
  // `docs/specs/features/logs/` also lives here but holds narrative log files
  // named `feature-<id>-...-log.md`, which match the same id-prefix. Filter
  // strictly to numeric-prefixed stage dirs so the resolver doesn't see log
  // files as spec candidates — otherwise every numeric feature that was ever
  // worked on trips the duplicate-match throw in getSpecPathForEntity.
