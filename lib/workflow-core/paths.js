...
// Lifecycle-independent canonical home for durable spec markdown (F668).
// `00-specs` sorts before every numbered stage folder and is deliberately NOT
// a member of CANONICAL_STAGE_DIRS: the stage resolver must never treat it as a
// stage, and it must never be double-counted alongside a stage copy.
const CANONICAL_SPEC_DIR = '00-specs';

// Feedback spec-folder layout (no workflow engine behind it, but this file is
// the single owner of stage-dir names — consumers must import, never inline;
// enforced by scripts/check-path-literals.js).
const FEEDBACK_STAGE_FOLDERS = Object.freeze({
  INBOX: '01-inbox',
  TRIAGED: '02-triaged',
  ACTIONABLE: '03-actionable',
  DONE: '04-done',
  WONT_FIX: '05-wont-fix',
  DUPLICATE: '06-duplicate',
});

const FEEDBACK_STAGE_DIRS = Object.freeze(Object.values(FEEDBACK_STAGE_FOLDERS));
...
/**
 * Canonical, lifecycle-independent directory for an entity's durable spec
 * markdown: `docs/specs/<features|research-topics>/00-specs`.
 */
function getCanonicalSpecDirForEntity(repoPath, entityType) {
  const cfg = getEntityConfig(entityType);
  return path.join(repoPath, 'docs', 'specs', cfg.docsDir, CANONICAL_SPEC_DIR);
}
