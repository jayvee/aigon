...
function withResearchLock(repoPath, researchId, work) {
  return withLockSync(getResearchPaths(repoPath, researchId).lockPath, work);
}
...
function startResearchSync(repoPath, researchId, mode, agents = []) {
  return withResearchLock(repoPath, researchId, () => {
    const { eventsPath, snapshotPath } = getResearchPaths(repoPath, researchId);
