...
function moveSpecProjectionSync(fromPath, toPath) {
  ...
  if (fsSync.existsSync(toPath) && !fsSync.existsSync(fromPath)) return;
...
function applyResearchEventsSync(repoPath, researchId, newEvents) {
  ...
  const refreshed = materializeSnapshotFromEventsSync(repoPath, researchId, readEventsSync(eventsPath));
  writeSnapshotSync(snapshotPath, refreshed);
  return refreshed;
}
