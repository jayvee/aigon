...
function getResearchPaths(repoPath, researchId) {
  return {
    eventsPath: getEventsPathForEntity(repoPath, 'research', researchId),
    snapshotPath: getSnapshotPathForEntity(repoPath, 'research', researchId),
    lockPath: getLockPathForEntity(repoPath, 'research', researchId),
  };
}
...
function applyEvents(repoPath, researchId, newEvents) {
  const { eventsPath, snapshotPath } = getResearchPaths(repoPath, researchId);
...
function startResearchSync(repoPath, researchId, mode, agents = []) {
  const { lockPath, eventsPath, snapshotPath } = getResearchPaths(repoPath, researchId);
...
function showResearchSync(repoPath, researchId) {
  const { eventsPath } = getResearchPaths(repoPath, researchId);
  const events = readEventsSync(eventsPath);
