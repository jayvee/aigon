...
const { STAGE_FOLDERS, CANONICAL_STAGE_DIRS } = require('./workflow-core/paths');

const OPEN_STAGES = [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION];
const ALL_STAGES = CANONICAL_STAGE_DIRS;
...
const INSTANCE_PERIOD_FIELDS = [
    { frontmatter: 'recurring_week', state: 'lastWeek', schedule: 'weekly' },
    { frontmatter: 'recurring_month', state: 'lastMonth', schedule: 'monthly' },
    { frontmatter: 'recurring_quarter', state: 'lastQuarter', schedule: 'quarterly' },
];

function markRecurringInstanceDeleted(repoPath, specContent, { deletedAt = new Date() } = {}) {
    const { data } = parseFrontMatter(specContent);
    const recurringSlug = typeof data.recurring_slug === 'string' ? data.recurring_slug.trim() : '';
    if (!recurringSlug) return null;

    const periods = INSTANCE_PERIOD_FIELDS.filter(({ frontmatter }) => {
        const value = data[frontmatter];
        return value !== undefined && value !== null && String(value).trim() !== '';
    });
    if (periods.length !== 1) {
        throw new Error(`recurring feature "${recurringSlug}" must define exactly one cadence period`);
    }

    const periodField = periods[0];
    const period = String(data[periodField.frontmatter]).trim();
    const state = readRecurringState(repoPath);
    state[recurringSlug] = {
        ...(state[recurringSlug] || {}),
        [periodField.state]: period,
        deletedAt: deletedAt.toISOString(),
    };
    writeRecurringState(repoPath, state);
    return { recurringSlug, schedule: periodField.schedule, period };
}
...
function createAndPrioritiseFromTemplate(repoPath, template, featureName, periodCtx) {
    ...
    const inboxDir = path.join(featurePaths.root, STAGE_FOLDERS.INBOX);
    ...
    const engine = require('./workflow-core');
    ...
    const backlogPath = path.join(featurePaths.root, STAGE_FOLDERS.BACKLOG, newName);

    engine.migrateEntityWorkflowIdSync(repoPath, 'feature', slug, paddedId, backlogPath, 'backlog');

    const found = { file: inboxFilename, folder: STAGE_FOLDERS.INBOX, fullPath: inboxPath };
    specCrud.moveFile(found, STAGE_FOLDERS.BACKLOG, newName, { actor: 'recurring/feature-prioritise' });

    const relativeBacklogPath = path.relative(repoPath, backlogPath);
    execFileSync('git', ['add', '--', relativeBacklogPath], { cwd: repoPath, stdio: 'inherit' });
    execFileSync('git', ['commit', '-m', `chore: recurring feature ${paddedId} - ${featureName}`, '--', relativeBacklogPath], { cwd: repoPath, stdio: 'inherit' });
