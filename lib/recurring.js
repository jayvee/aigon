...
function renderPattern(pattern, isoWeek) {
    return renderTemplateString(pattern, { isoWeek });
}
...
function scanTemplates(recurringDir) {
    ...
    for (const file of files) {
        ...
        if (data.schedule !== 'weekly' && data.schedule !== 'quarterly') {
        ...
        const allowed = data.schedule === 'weekly' ? ['{{YYYY-WW}}'] : ['{{YYYY-Q}}'];
        ...
        templates.push({
            file,
            filePath,
            schedule: /** @type {'weekly' | 'quarterly'} */ (data.schedule),
...
function _runCheck(repoPath) {
    ...
    const periodCtx = { isoWeek: currentWeek, isoQuarter: currentQuarter, isoDate: currentDate };
    ...
    console.log(`[recurring] Week ${currentWeek} quarter ${currentQuarter}: ${templates.length} template(s) found`);
    ...
    for (const template of templates) {
        ...
        const periodKey = template.schedule === 'weekly' ? currentWeek : currentQuarter;
        const lastRun = template.schedule === 'weekly'
            ? state[template.recurringSlug]?.lastWeek
            : state[template.recurringSlug]?.lastQuarter;
        if (lastRun === periodKey) {
            const reason = template.schedule === 'weekly' ? 'already created this week' : 'already created this quarter';
...
function listRecurringStatus(repoPath) {
    ...
    return templates.map(t => {
        ...
        const periodKey = t.schedule === 'weekly' ? currentWeek : currentQuarter;
        const lastRun = t.schedule === 'weekly'
            ? state[t.recurringSlug]?.lastWeek
            : state[t.recurringSlug]?.lastQuarter;
        ...
        return {
            ...
            reason: openExists
                ...
                : createdThisPeriod
                    ? (t.schedule === 'weekly' ? 'already created this week' : 'already created this quarter')
                    : null,
        };
    });
}
