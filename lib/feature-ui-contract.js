...
function choosePrimary(actions, feature, runtimeFacts) {
    ...
    // A pending spec review makes revision the decision. Do not simply hide
    // Start and fall through to an unrelated lifecycle action such as
    // unprioritise.
    const pendingSpecReview = (feature.specReviewSessions || []).some(review => review && review.status === 'pending');
    const revise = pendingSpecReview
        ? available.find(action => action.actionId === 'feature-spec-revise')
        : null;
    if (revise) {
        return revise.actionId;
    }
...
function buildBlockers(feature, runtimeFacts) {
    ...
    (feature.blockedBy || []).forEach(item => blockers.push({
        kind: 'dependency',
        id: item.id,
        displayKey: item.displayKey || (item.id != null ? `F${item.id}` : null),
    ...
    if (readiness && readiness.applicable && !readiness.ready && readiness.primaryBlocker
        && readiness.primaryBlocker.kind !== 'closing') {
...
function buildFeatureUiContract(feature, aggregate = {}, runtimeInput = {}) {
    ...
    // A dependency-blocked Start remains the visible next action. Do not
    // promote an unrelated enabled action (for example, unprioritise) merely
    // because Start is disabled.
    const blockedStart = dependencyIds.length > 0
        && decisions.some(action => action.actionId === 'feature-start' && action.disabled);
    const primaryActionId = blockedStart ? null : choosePrimary(decisions, feature, runtimeFacts);
    ...
    const pendingSpecRevision = lifecycle !== 'spec_revision_in_progress'
        && (feature.specReviewSessions || []).some(review => review && review.status === 'pending')
        && decisions.some(action => action.actionId === 'feature-spec-revise' && !action.disabled);
    // A submitted review is actionable before an agent has started revision.
    // Surface that fact on the card instead of presenting a deceptively plain
    // Backlog state while Start is intentionally unavailable as the next step.
    const visiblePresentation = pendingSpecRevision
        ? {
            ...presentation,
            headline: { label: 'Spec revision needed' },
            contextLine: presentation.contextLine || 'Review feedback is ready to apply.',
        }
        : presentation;
    return buildEntityUiContract({
        ...
        state: {
            ...
            label: stateMeta.label || visiblePresentation.stateLabel || machineStateMeta.label || lifecycle || '',
            severity: visiblePresentation.severity || stateMeta.severity || machineStateMeta.severity || 'normal',
        },
        presentation: {
            headline: visiblePresentation.headline || feature.cardHeadline || null,
            contextLine: visiblePresentation.contextLine || null,
            timeline: visiblePresentation.timeline || null,
            agentSummary: visiblePresentation.agentSummary || null,
