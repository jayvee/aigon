...
const adapters = [
    ...
    // config with an auto-launch script (e.g. Diviner) won't hijack aigon tabs.
    {
        ...
        launch(cmd, opts) {
            ...
            const fullCommand = `cd ${shellQuote(opts.cwd)} && ${resolved}`;
            const escCmd = fullCommand.replace(/\\/g, '\\\\').replace(/"/g, '\\"');

            // Primary: AppleScript — open as a tab in the front window, fall back to a new
            // window if no window exists. Mirrors the iTerm2 adapter's pattern.
            const tabScript = [
                'tell application "Ghostty"',
                '  activate',
                `  set cfg to make new surface configuration with properties {command:"${escCmd}"}`,
                '  if (count of windows) > 0 then',
                '    tell front window to make new tab with properties {configuration:cfg}',
                '  else',
                '    make new window with properties {configuration:cfg}',
                '  end if',
                'end tell',
            ].join('\n');
            ...
            if (r.error || r.status !== 0) {
                ...
                // permissions and on Ghostty builds where the dictionary syntax differs.
                const cli = spawnSync('ghostty', ['+new-window', `--command=${fullCommand}`], { stdio: 'pipe', encoding: 'utf8' });
