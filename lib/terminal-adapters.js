...
const adapters = [
    ...
    // --- iTerm2 (macOS, configured via tmuxApp) ---
    {
        ...
        launch(cmd, opts) {
            ...
            const titleLines = opts.title ? [`set name of current session of current window to "${opts.title.replace(/"/g, '\\"')}"`, ''] : [];
            const script = ['tell application "iTerm2"', 'activate', `create window with default profile command "${esc}"`, ...titleLines, 'end tell'].join('\n');
