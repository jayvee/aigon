...
const adapters = [
    ...
    // config with an auto-launch script (e.g. Diviner) won't hijack aigon tabs.
    {
        ...
        launch(cmd, opts) {
            ...
            // Primary: AppleScript — open a tab in the front window, fall back to a new
            // window if no window exists. Mirrors the iTerm2 adapter's pattern.
            //
            // Syntax verified against Ghostty's scripting dictionary (sdef):
            //   - `new surface configuration` (no `make`, no inline `with properties {…}`)
            //   - properties set individually after creation
            //   - `new tab in <window> with configuration <cfg>` / `new window with configuration <cfg>`
            //
            // The per-surface `command` overrides config-file `command =` and
            // `initial-command =` — so dotfiles auto-launch scripts (e.g. Diviner) won't
            // hijack aigon tabs.
            const setCfg = [
            ...
            if (r.error || r.status !== 0) {
                // Last-resort fallback: `ghostty +new-window --command=…`. Always opens a
                // new window (no tab-in-window option), but works without AppleScript
                // permissions and on Ghostty builds where the dictionary differs.
