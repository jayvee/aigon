...
const adapters = [
    ...
    {
        name: 'warp',
        detect: (env) => env.platform === 'darwin' && env.requestedTerminal === 'warp',
        launch(cmd, opts) {
    ...
    // --- iTerm2 (macOS, configured via tmuxApp) ---
    {
        name: 'iterm2',
        detect: (env) => env.platform === 'darwin' && env.tmuxApp === 'iterm2',
        launch(cmd, opts) {
            ...
            const titleLines = opts.title ? [`set name of current session of current tab of current window to "${opts.title.replace(/"/g, '\\"')}"`, ''] : [];
            // Open as a tab in the current window if one exists, otherwise create a new window
            const script = ['tell application "iTerm2"', 'activate',
                'if (count of windows) > 0 then',
                `  tell current window to create tab with default profile command "${esc}"`,
                'else',
                `  create window with default profile command "${esc}"`,
                'end if',
                ...titleLines, 'end tell'].join('\n');
            const r = spawnSync('osascript', ['-e', script], { stdio: 'pipe' });
            if (r.error || r.status !== 0) throw new Error(`Failed to open iTerm2: ${r.stderr ? r.stderr.toString().trim() : 'unknown'}. Is iTerm2 installed?`);
        },
        split: null,
        _focus(title) {
            const r = spawnSync('osascript', ['-e', iterm2FocusScript(title)], { stdio: 'pipe', encoding: 'utf8' });
    ...
    {
        name: 'terminal',
        detect: (env) => env.platform === 'darwin',
        launch(cmd, opts) {
            if (opts.title) {
...
function tileITerm2Windows() {
    ...
    const tr = spawnSync('osascript', ['-e', `tell application "iTerm2"\n${positions}\nend tell`], { encoding: 'utf8', stdio: 'pipe' });
