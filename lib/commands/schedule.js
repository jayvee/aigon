...
function usage() {
  ...
  aigon schedule add set_autonomous <slug> --run-at=<iso8601> <agents...> [options]
      Options: --review-agent= --models= --efforts= --stop-after=close --repo=<path>
  aigon schedule add research_start <id> --run-at=<iso8601> [agents...] [--background|--foreground]
      Options: --repo=<path>
  aigon schedule add agent_prompt --run-at=<iso8601> --agent=<id> --prompt=<string> [options]
      Options: --cron=<expr> --label=<slug> --repo=<path>
      --prompt may be a literal string or a slash command (e.g. /security-review).
      --cron is a 5-field expression; when set, the job re-arms after each fire.
      --label is a [a-z0-9-]+ slug (max 60 chars); auto-generated if omitted.
...
function payloadSummary(job) {
    ...
    const parts = [`repo=${JSON.stringify(path.resolve(String(job.repoPath || '')))}`];
    if (Array.isArray(payload.agents) && payload.agents.length > 0) {
        parts.push(`agents=${payload.agents.join(',')}`);
    }
    if (payload.reviewAgent) parts.push(`review=${payload.reviewAgent}`);
    if (payload.evalAgent) parts.push(`eval=${payload.evalAgent}`);
    if (payload.stopAfter) parts.push(`stopAfter=${payload.stopAfter}`);
    if (payload.models) parts.push(`models=${payload.models}`);
    if (payload.efforts) parts.push(`efforts=${payload.efforts}`);
    return parts.join(' ');
}
        ...
        schedule: async (args) => {
            ...
            if (sub === 'list') {
                ...
                for (const j of jobs) {
                    ...
                    const cron = (j.kind === sk.AGENT_PROMPT_KIND && j.payload && j.payload.cron)
                        ? ` cron=${j.payload.cron}` : '';
                    console.log(`${j.jobId}  ${j.status.padEnd(10)}  ${j.kind}  #${j.entityId}  runAt=${j.runAt} ${payloadSummary(j)}${cron}${err}`);
                }
                return;
            }
            ...
            let kind = '';
            if (kindRaw === 'feature_autonomous' || kindRaw === 'feature-autonomous') kind = 'feature_autonomous';
            else if (kindRaw === sk.SET_AUTONOMOUS_KIND || kindRaw === 'set-autonomous') kind = sk.SET_AUTONOMOUS_KIND;
            else if (kindRaw === 'research_start' || kindRaw === 'research-start') kind = 'research_start';
            else if (kindRaw === 'agent_prompt' || kindRaw === 'agent-prompt') kind = sk.AGENT_PROMPT_KIND;

            const runAtRaw = getOptionValue(opts, 'run-at');
            const runAt = runAtRaw != null ? String(runAtRaw).trim() : '';

            if (kind === sk.AGENT_PROMPT_KIND) {
                if (!runAt) { usage(); process.exitCode = 1; return; }
                const agentId = getOptionValue(opts, 'agent') != null ? String(getOptionValue(opts, 'agent')) : '';
                const promptVal = getOptionValue(opts, 'prompt') != null ? String(getOptionValue(opts, 'prompt')) : '';
                const labelVal = getOptionValue(opts, 'label') != null ? String(getOptionValue(opts, 'label')) : '';
                const cronVal = getOptionValue(opts, 'cron') != null ? String(getOptionValue(opts, 'cron')) : '';
                const payload = {
                    agentId,
                    prompt: promptVal,
                ...
                const r = sk.addJob(repoPath, { kind, runAt, payload });
                if (!r.ok) {
                    console.error(`❌ ${r.error}`);
                    process.exitCode = 1;
                    return;
                }
                console.log(`Scheduled agent_prompt #${r.job.entityId} at ${r.job.runAt}`);
                if (r.job.payload && r.job.payload.cron) console.log(`  cron:  ${r.job.payload.cron}`);
            ...
            if (kind === sk.SET_AUTONOMOUS_KIND) {
                const agents = opts._.slice(3).map(a => String(a).trim()).filter(Boolean);
                const payload = {
                    agents,
                    reviewAgent: getOptionValue(opts, 'review-agent') != null ? String(getOptionValue(opts, 'review-agent')) : undefined,
                    ...
                    efforts: getOptionValue(opts, 'efforts') != null ? String(getOptionValue(opts, 'efforts')) : undefined,
                    stopAfter: getOptionValue(opts, 'stop-after') != null ? String(getOptionValue(opts, 'stop-after')) : undefined,
                    mode: getOptionValue(opts, 'mode') != null ? String(getOptionValue(opts, 'mode')) : undefined,
                };
                const r = sk.addJob(repoPath, { kind, entityId, runAt, payload });
                if (!r.ok) {
                    console.error(`❌ ${r.error}`);
                ...
                console.log(`Scheduled ${kind} for set #${entityId} at ${r.job.runAt}`);
                console.log(`  jobId: ${r.job.jobId}`);
                console.log(`  repo:  ${r.job.repoPath}`);
                return;
            }
