 ...
 * }
 *
 * Supported targetState values: "code_review_in_progress" (implementation done, ready for review)
 */
    ...
    for (const [featureId, fixture] of Object.entries(features)) {
        ...
        try {
            if (target === 'code_review_in_progress') {
                ...
                // 1. Emit workflow events: implementation complete, awaiting review.
                //    Stop at signal.agent_submitted (cc status = ready) — this is
                //    the state that surfaces the "Run Review" action on the dashboard.
                //    Do NOT emit feature.code_review.started here; that fires when
                //    the reviewer agent actually begins, not before.
                ...
                // 2. Create git branch + worktree, then commit the implementation onto the branch.
                //    Main is kept clean (no implementation). The diff main..featureBranch shows
                //    the implementation so reviewers can see what was actually written.
                fs.mkdirSync(worktreeBase, { recursive: true });
                try {
                    // Branch from current main HEAD (clean state, no implementation yet)
                    execSync(`git -C "${repoPath}" branch "${branchName}" HEAD 2>/dev/null || true`, { stdio: 'pipe' });
                    // Remove stale worktree from a prior reset
                    ...
                    // Copy implementation files into the worktree and commit them
                    const impl = fixture.implementation;
                    if (impl && Array.isArray(impl.files) && impl.files.length > 0) {
                        const filesToAdd = [];
                        for (const { source, dest } of impl.files) {
                            const srcPath = path.join(repoPath, source);
                            const destPath = path.join(worktreePath, dest);
                            if (!fs.existsSync(srcPath)) {
                                console.warn(`   ⚠️  Seed implementation source not found: ${source}`);
                                continue;
                            }
                        ...
                        if (filesToAdd.length > 0) {
                            const gitAdd = filesToAdd.map(f => `"${f}"`).join(' ');
                            execSync(`git -C "${worktreePath}" add ${gitAdd}`, { stdio: 'pipe' });
                            const msg = (impl.commitMessage || `feat: implement feature ${featureId}`).replace(/"/g, '\\"');
                            execSync(`git -C "${worktreePath}" commit -m "${msg}" --author "Claude Code <cc@aigon.local>"`, { stdio: 'pipe' });
                        }
                    }
                } catch (wtErr) {
