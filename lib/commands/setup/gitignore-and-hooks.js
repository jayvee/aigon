...
/** Local-only git excludes for derived/runtime `.aigon/` artifacts (F589). */
const STANDARD_LOCAL_GIT_EXCLUDE_ENTRIES = [
    '.aigon/state/',
    '.aigon/locks/',
    '.aigon/worktree.json',
...
function getStandardLocalGitExcludeEntries(repoPath = process.cwd()) {
    const envGitignoreStatus = getEnvLocalGitignoreStatus(repoPath);
    return [
        ...STANDARD_LOCAL_GIT_EXCLUDE_ENTRIES,
...
function resolveGitExcludePath(repoPath) {
    const gitPath = path.join(repoPath, '.git');
    if (!fs.existsSync(gitPath)) return null;
    ...
    if (gitDir.includes(worktreesMarker)) {
...
function ensureLocalGitExclude(repoPath, entries) {
    ...
    if (!excludePath) return { addedEntries: [], path: null };
...
function untrackFiles(repoPath, files) {
    ...
    const result = spawnSync('git', ['rm', '--cached', '--', ...files], { // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
        cwd: repoPath,
    ...
    const error = result.error || new Error((result.stderr || '').toString().trim() || `git rm --cached failed with exit ${result.status}`);
