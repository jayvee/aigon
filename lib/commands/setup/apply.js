    ...
    return async (args) => {
            ...
            try {
                ...
                const aigonPaths = [
                    ...
                    // `git add` treats a non-existent pathspec as fatal (unlike
                    // `git status`), so a repo with no agents installed (agent
                    // dirs like .claude/ absent) would fail the whole auto-commit.
                    // Only stage paths that actually exist.
                    try { return fs.existsSync(path.join(process.cwd(), p.replace(/\/$/, ''))); }
