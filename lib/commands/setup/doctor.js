...
} = require('../../doctor/agent-auth-probe');
    ...
    return async (args) => {
            ...
            function runAgentAuthCheck(agentConfig) {
                ...
                if (method === 'command') {
                    ...
                    if (authCommandSpawnsAgy(authCheck.command)) {
                        return doctorAuthStatusForSkippedAgent(authCheck);
                    }
            ...
            function printAgentAuthSection() {
                ...
                for (const agentId of installedAgentIds) {
                    ...
                    const result = shouldSkipDoctorAuthProbe(agentId, agentConfig?.authCheck)
                        ? doctorAuthStatusForSkippedAgent(agentConfig?.authCheck)
                        : runAgentAuthCheck(agentConfig);
            ...
            // F693: Aigon Pro was merged into OSS. Flag leftovers from the
            // tiered era — a stale `proKey` in the global config (removable
            // under --fix) and a globally installed @senlabsai/aigon-pro that
            // is now dead weight.
            await withSection({ id: 'pro-leftovers', title: 'Aigon Pro leftovers' }, () => {
                const globalCfgPath = path.join(os.homedir(), '.aigon', 'config.json');
                let staleKey = false;
                try {
                    const cfg = JSON.parse(fs.readFileSync(globalCfgPath, 'utf8'));
                    staleKey = Object.prototype.hasOwnProperty.call(cfg, 'proKey');
                } catch (_) { /* no global config yet */ }

                let proInstalled = false;
                try {
                    const out = execSync('npm ls -g --depth=0 --parseable @senlabsai/aigon-pro', { stdio: 'pipe', encoding: 'utf8' });
                    proInstalled = /aigon-pro/.test(out);
                } catch (_) { proInstalled = false; }

                if (!staleKey && !proInstalled) return 'clean';

                console.log('\nAigon Pro leftovers\n───────────────────');
                if (staleKey) {
                    console.log('  ⚠️  ~/.aigon/config.json still carries `proKey` — Aigon has no tiers; the key is ignored.');
                    report.issue({
                        section: 'pro-leftovers',
                        sectionTitle: 'Aigon Pro leftovers',
                        check: 'stale-pro-key',
                        message: 'Stale `proKey` in ~/.aigon/config.json',
                        fix: { label: 'remove proKey', command: 'aigon doctor --fix', autoFixable: true },
                    });
                    queueFix({
                        section: 'pro-leftovers',
                        message: 'Remove the stale `proKey` from ~/.aigon/config.json',
                        label: 'remove proKey',
                        command: 'aigon doctor --fix',
                        apply: () => {
                            const cfg = JSON.parse(fs.readFileSync(globalCfgPath, 'utf8'));
                            ...
                            console.log('  ✅ stale-pro-key: removed `proKey` from ~/.aigon/config.json');
                        },
                    });
                }
                if (proInstalled) {
                    console.log('  ⚠️  @senlabsai/aigon-pro is installed globally and is no longer used.');
                    console.log('     Run: npm uninstall -g @senlabsai/aigon-pro');
                    report.issue({
                        section: 'pro-leftovers',
                        sectionTitle: 'Aigon Pro leftovers',
                        check: 'stale-pro-package',
                        message: '@senlabsai/aigon-pro installed globally but unused',
                        fix: { label: 'uninstall aigon-pro', command: 'npm uninstall -g @senlabsai/aigon-pro', autoFixable: false },
                    });
                }
                return 'leftovers found';
            });

            await withSection({ id: 'backup', title: 'Backup' }, () => {
                try {
                    console.log('\nBackup\n──────');
                    const s = require('../../backup').status();
                    if (!s.configured) {
            ...
            try {
                if (!require('../../profile-state').getProfileRemote()) {
                    console.log('');
