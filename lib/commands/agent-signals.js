...
const { detectActiveAgentSession, ROOT_DIR, getActiveProfile, loadProjectConfig } = require('../config');
const {
    checkImplementationLogEvidence,
    formatImplementationLogBlockMessage,
} = require('../implementation-log-policy');
const { detectImplementationLogMode } = require('../profile-placeholders');
...
module.exports = function agentSignalsCommands(ctx) {
    ...
    return {
            ...
            const validStatuses = [
                ...
                // Origin-continuation checkpoint signals
                'continuation-ready', 'continuation-fallback',
            ...
            const completionSignals = new Set([
                'implementation-complete', 'revision-complete', 'research-complete', 'spec-review-complete',
                'continuation-ready', 'continuation-fallback',
            ]);
            ...
            if (isFeatureSubmit) {
                ...
                if (entityType === 'feature' && status === 'implementation-complete') {
                    const projectConfig = loadProjectConfig(mainRepo);
                    const logEvidence = checkImplementationLogEvidence({
                        repoPath: mainRepo,
                        featureId: featureNum,
                        agentId,
                        loggingLevel: projectConfig.logging_level,
                        mode: detectImplementationLogMode(process.cwd()),
                        scanCwd: process.cwd(),
                    });
                    if (!logEvidence.ok && logEvidence.required) {
                        const logMessage = formatImplementationLogBlockMessage(featureNum, logEvidence);
                        if (skipFeatureBranchGates) {
                            console.warn(`\n⚠️  ${logMessage}`);
                            console.warn('   Explicit-arg recovery mode — continuing without blocking.');
                        } else {
                            console.error(`❌ agent-status ${status} blocked: ${logEvidence.reason}.`);
            ...
            if (status === 'continuation-ready' || status === 'continuation-fallback') {
                const parsedOptions = parseCliOptions(args.slice(3));
                const aigonSessionId = getOptionValue(parsedOptions, 'session');
                const reason = getOptionValue(parsedOptions, 'reason');
                const allowedReasons = new Set(['context-missing', 'context-conflict', 'task-delivery-failed']);
                if (!aigonSessionId) {
                    console.error(`❌ agent-status ${status} requires --session=<aigonSessionId>.`);
                    process.exitCode = 1;
                    return;
                }
                if (status === 'continuation-fallback' && !allowedReasons.has(String(reason || ''))) {
                    console.error('❌ continuation-fallback requires --reason=context-missing|context-conflict|task-delivery-failed.');
                    process.exitCode = 1;
                    return;
                }
                signalPayload.continuityCheckpoint = {
                    state: status === 'continuation-ready' ? 'ready' : 'fallback',
                    aigonSessionId: String(aigonSessionId),
                    reason: status === 'continuation-fallback' ? String(reason) : null,
                };
            }
            ...
            try {
                ...
                // A feature can be reset and started again with the same
                // {entity, role, agent} tuple. Use tmux's pane identifier
                // when the command runs inside an Aigon-managed session so
                // completion from the new run is not deduplicated against
                // the prior run, while repeated signals in one pane remain
                // idempotent.
                const runtimeSessionId = String(process.env.TMUX_PANE || '').trim();
                await service.recordSessionSignal({
                    entityType,
                    entityId: featureNum,
                    agentId,
                    sessionId: runtimeSessionId ? `tmux-pane:${runtimeSessionId}` : undefined,
                ...
                if (signalPayload.continuityCheckpoint) {
                    const contextStore = require('../entity-context');
                    const beforeCheckpoint = contextStore.readEntityContext(mainRepo, entityType, featureNum) || {};
                    const alreadyRecovered = (beforeCheckpoint.continuityDecisions || [])
                        .some(item => item.recoveryOfSessionId === signalPayload.continuityCheckpoint.aigonSessionId);
                    contextStore.recordContinuityCheckpoint(mainRepo, entityType, featureNum, {
                        ...signalPayload.continuityCheckpoint,
                        agentId,
                    });
                    if (signalPayload.continuityCheckpoint.state === 'fallback' && !alreadyRecovered) {
                        const recoverySessionId = signalPayload.continuityCheckpoint.aigonSessionId;
                        const { buildAgentCommand, buildTmuxSessionName, createDetachedTmuxSession } = require('../worktree');
                        const role = ['spec-revise', 'spec-check'].includes(recordedTaskType) ? 'spec-revise' : 'do';
                        const taskType = recordedTaskType || (entityType === 'research' ? 'do' : 'do');
                        const freshSessionName = buildTmuxSessionName(featureNum, agentId, {
                            repo: path.basename(mainRepo),
                            desc: 'continuation-fallback',
                            entityType: entityType === 'research' ? 'r' : 'f',
                            role,
                        });
                        const context = contextStore.readEntityContext(mainRepo, entityType, featureNum) || {};
                        const continuityInstructions = [
                            '## Fresh continuation fallback',
                            `The origin continuation failed with ${signalPayload.continuityCheckpoint.reason}.`,
                            'Use the durable handoff below; the current checkout, spec, and git state are authoritative.',
                            context.authorHandoff ? JSON.stringify(context.authorHandoff, null, 2) : 'No valid handoff is available.',
                        ].join('\n\n');
                        try {
                            if (process.env.AIGON_TEST_MODE !== '1') {
                                ...
                                createDetachedTmuxSession(freshSessionName, process.cwd(), command, {
                                    repoPath: mainRepo,
                                    entityType: entityType === 'research' ? 'r' : 'f',
                                    entityId: featureNum,
                                    agent: agentId,
                                    role,
                                    worktreePath: process.cwd(),
                                    metadata: { recoveryOfSessionId: recoverySessionId },
                                });
                            }
                            contextStore.recordFallbackLaunchOutcome(mainRepo, entityType, featureNum, recoverySessionId, {
                                state: process.env.AIGON_TEST_MODE === '1' ? 'simulated' : 'launched',
                                sessionId: freshSessionName,
                            });
                        } catch (launchError) {
                            contextStore.recordFallbackLaunchOutcome(mainRepo, entityType, featureNum, recoverySessionId, {
                                state: 'failed', error: launchError.message,
                            });
                            console.error(`❌ Continuation fallback could not launch a fresh session: ${launchError.message}`);
                            process.exitCode = 1;
                        }
                    }
                }
            ...
            if (args.includes('--shell')) {
                if (session.detected && session.agentId) {
                    console.log(`export AIGON_AGENT_ID='${session.agentId}'`);
                } else {
                    process.exitCode = 1;
                }
                return;
