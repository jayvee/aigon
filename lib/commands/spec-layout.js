...
function migrate(repoPath, options) {
  ...
  if (plan.moves.length === 0) {
    // Idempotent: nothing to move — still record (and commit) the layout version.
    ...
    if (result.committed) {
      console.log('   Committed layout version on the current branch.');
    }
...
/**
 * Rebuild the generated lifecycle symlink view (F669) after a stable-layout
 * migration. Warns with a repair command on failure but never rolls back the
 * already-published canonical layout.
 */
function refreshLifecycleView(repoPath) {
  try {
    const specView = require('../spec-view');
    const view = specView.refreshView(repoPath);
    if (view.skipped) return;
    console.log(`   Lifecycle view: ${view.created.length} link(s) created.`);
    if (view.blocked.length > 0) {
      console.log(`   ⚠️  ${view.blocked.length} collision(s) left untouched — run: aigon spec-view refresh`);
    }
  } catch (error) {
