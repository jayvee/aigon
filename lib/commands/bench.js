...
function assessModel(agentId, model) {
    ...
    // Very small models (single digit B params, not MoE)
    if (/\b[1-9]b(-|$)/.test(id) && !/\d{2,}b/.test(id) && !/-a\d/.test(id)) {
        risks.push('small parameter count — likely too weak for multi-file agentic coding');
    }
...
/**
 * Present candidates interactively, supporting:
 *   y/n per model, or 'a' to approve all remaining ✅ at once.
 * Auto-excluded models (TTS, VL, etc.) are never shown.
 * In non-interactive mode: auto-include ✅, auto-exclude ⚠️.
 */
async function promptIncludeExclude(agentId, candidates, opts = {}) {
    ...
    // Split: auto-excluded (irrelevant modality) vs. reviewable
    const assessed = candidates.map(m => ({ ...m, _a: assessModel(agentId, m) }));
    const autoExcluded = assessed.filter(m => m._a.autoExclude);
    const reviewable = assessed.filter(m => !m._a.autoExclude);

    if (autoExcluded.length > 0) {
        process.stdout.write(`   (filtered ${autoExcluded.length} irrelevant model(s): TTS/VL/robotics/etc.)\n`);
    }
    if (reviewable.length === 0) return [];

    const suitable = reviewable.filter(m => m._a.suitable);
    const risky = reviewable.filter(m => !m._a.suitable);

    process.stdout.write(`   ${reviewable.length} candidate(s) to review: ${suitable.length} ✅  ${risky.length} ⚠️\n`);
    ...
    if (nonInteractive) {
        for (const m of reviewable) {
            if (m._a.suitable) {
                approved.push({ ...m, autoNotes: m._a.notes });
                process.stdout.write(`   ✅ auto-included: ${m.value}\n`);
            } else {
                process.stdout.write(`   ⚠️  auto-excluded: ${m.value}  (${m._a.risk})\n`);
            }
        }
        return approved;
    }
...
async function modelRefresh(rawArgs) {
    ...
    const pendingPath = path.join(process.cwd(), '.aigon', 'pending-models.json');

    process.stdout.write('\n🔍  Probing providers for new models…\n');

    const results = {}; // agentId → { added: [], skipped: [] }
    ...
    if (agentFilter.length === 0 || agentFilter.includes('op')) {
        ...
        if (opData) {
            ...
            const benchConfig = loadBenchRefreshConfig(process.cwd());
            process.stdout.write('\nop (OpenRouter)\n');
            const candidates = await discoverOpModels(opOptions, benchConfig);
            process.stdout.write(`   ${candidates.length} new model(s) found\n`);

            if (candidates.length > 0 && !isDryRun) {
                const approved = await promptIncludeExclude('op', candidates, { nonInteractive });
                const skipped = candidates.filter(c => !approved.includes(c));
                results.op = { added: approved, skipped };

                for (const m of approved) {
                    opData.cli.modelOptions.push({
                        value: m.value,
                        label: m.label,
                        pricing: m.pricing || undefined,
                        notes: { implement: m.autoNotes },
            ...
            } else if (candidates.length > 0 && isDryRun) {
    ...
    if (agentFilter.length === 0 || agentFilter.includes('gg')) {
        ...
        if (ggData) {
            ...
            if (candidates.length > 0 && !isDryRun) {
                ...
                const skipped = candidates.filter(c => !approved.includes(c));
                results.gg = { added: approved, skipped };

                for (const m of approved) {
                    ggData.cli.modelOptions.push({
                        value: m.value,
                        label: m.label,
                        notes: { implement: m.autoNotes },
                        score: { implement: null },
    ...
    if (totalAdded > 0) {
        ...
        process.stdout.write('✅ All providers up to date — no new models found.\n');
    }

    // Write pending summary for non-interactive / scheduled runs
    if (nonInteractive && totalAdded > 0) {
        fs.mkdirSync(path.dirname(pendingPath), { recursive: true });
        fs.writeFileSync(pendingPath, JSON.stringify({ timestamp: new Date().toISOString(), results }, null, 2) + '\n', 'utf8');
        process.stdout.write(`\nPending summary written to: ${pendingPath}\n`);
    }
}
...
async function benchRefresh(rawArgs) {
    ...
    // Strip apiData (full OpenRouter/Gemini response) — not for storage
    function toRegistryEntry(m) {
        const entry = { value: m.value, label: m.label };
        if (m.pricing) entry.pricing = m.pricing;
        entry.score = { implement: null };
        entry.lastRefreshAt = new Date().toISOString().slice(0, 10) + 'T00:00:00.000Z';
        return entry;
    }
    if (autoAddModels) {
        if (ggNew.length > 0 && ggData) {
            for (const m of ggNew) {
                ggData.cli.modelOptions.push(toRegistryEntry(m));
        ...
        if (opNew.length > 0 && opData) {
            for (const m of opNew) {
                opData.cli.modelOptions.push(toRegistryEntry(m));
