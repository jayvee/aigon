...
const { checkRecurringFeatures, listRecurringStatus, getISOWeek } = require('../recurring');
    ...
    return {
        ...
        'recurring-list': (args) => {
            ...
            if (items.length === 0) {
                ...
                console.log(`Create a template with 'schedule: weekly' frontmatter to get started.`);
                return;
            }

            console.log(`\n📅 Recurring features (current week: ${getISOWeek()})\n`);
            const COL1 = 30;
            const COL2 = 10;
            const COL3 = 12;
            const header = `${'Slug'.padEnd(COL1)} ${'Schedule'.padEnd(COL2)} ${'Last created'.padEnd(COL3)} Status`;
            console.log(header);
            console.log('-'.repeat(header.length));

            for (const item of items) {
                const status = item.isDue ? '🔴 DUE' : '✅ done';
                const lastWeek = item.lastWeek || '—';
                const row = `${item.recurringSlug.padEnd(COL1)} ${item.schedule.padEnd(COL2)} ${lastWeek.padEnd(COL3)} ${status}`;
                console.log(row);
            }
            console.log('');
        },
