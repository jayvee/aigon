...
/**
 * Scan spec folders for entities (features or research) and return
 * those missing a workflow-core snapshot.
 *
 * @param {string} repoPath
 * @returns {{ features: Array<{id:string, stage:string, specPath:string}>, research: Array<{id:string, stage:string, specPath:string}> }}
 */
    ...
    FEATURE_STAGE_FOLDERS.forEach(({ folder, stage }) => {
        ...
        try {
            fs.readdirSync(dir)
                ...
                .forEach(file => {
                    const m = file.match(/^feature-(\d+)-/);
                    if (!m) return;
                    const id = canonicalSeedFeatureId(m[1]);
...
function bootstrapMissingWorkflowSnapshots(repoPath, entities, entityType) {
    ...
    for (const entity of entities) {
        const entityRoot = path.join(repoPath, '.aigon', 'workflows', workflowDir, entity.id);
...
module.exports = function setupCommands(ctx) {
    ...
    const commands = {
        ...
        'seed-reset': (args) => {
            ...
            // 9. Initialize Aigon and install agents
            try {
                const savedCwd = process.cwd();
                process.chdir(repoPath);

                // Run aigon init to bootstrap workflow events and .aigon/ structure
                console.log('\n📦 Initializing Aigon...');
                commands['init']([]);

                // Install all available agents so worktrees have commands ready
                // Seed repos are small demos — all agents should be available
                const agentsToInstall = getAvailableAgents();

                console.log(`📦 Installing agents: ${[...agentsToInstall].join(', ')}`);
                commands['install-agent']([...agentsToInstall]);
                ...
                console.log(`   ⚠️  Could not initialize/install agents: ${e.message}`);
            }
