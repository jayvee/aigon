...
module.exports = function setupCommands(ctx) {
    ...
    const commands = {
        ...
        'doctor': (args) => {
            ...
            const locksDir = manifest.getLocksDir();
        ...
        'seed-reset': (args) => {
            ...
            // 1. Find the latest reset-safe baseline commit for the seed repo.
            const baseline = findSeedResetBaseline(repoPath);
            if (!baseline || !baseline.commit) {
                console.error(`❌ Could not find a seed-reset baseline commit in ${repoName}`);
                return;
            }

            const baselineCommit = baseline.commit;
            const shortBaseline = baselineCommit.slice(0, 7);
            console.log(`\n🔄 Resetting ${repoName} to baseline commit ${shortBaseline}...\n`);
            ...
            // Find tmux sessions for this repo
            let tmuxSessions = [];
            try {
                ...
                if (!tmuxList.error && tmuxList.status === 0) {
                    tmuxSessions = tmuxList.stdout.split('\n').map(s => s.trim()).filter(s => {
                        const lowerSession = s.toLowerCase();
                        const lowerRepo = repoName.toLowerCase();
                        return lowerSession.startsWith(lowerRepo + '-');
                    });
                }
            } catch (e) { /* no tmux */ }
            ...
            console.log(`   git reset --hard ${shortBaseline}`);
            ...
            // 3. Kill agent processes for this repo
            try {
            ...
            // 6. Git reset to seed commit
            try {
                execSync(`git reset --hard ${baselineCommit}`, { cwd: repoPath, stdio: 'pipe' });
                console.log(`   ✓ Reset to baseline commit: ${shortBaseline}`);
            } catch (e) {
            ...
            // 8. Ensure local runtime files stay ignored without touching tracked files.
            try {
            ...
            // 9. Remove .aigon/state and .aigon/locks
            ...
            // 10. Rebuild ignored coordinator manifests from the reset spec folders.
            try {
            ...
            // 12. Remove worktree permissions/trust from Claude settings
            ...
            // 13. Re-install aigon for all detected agents so the seed repo
            //     matches the current CLI version (no manual install-agent needed).
            try {
            ...
            console.log(`\n✅ ${repoName} reset to seed state (${shortBaseline}).`);
        },
    };
