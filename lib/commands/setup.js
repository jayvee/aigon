        ...
        'doctor': (args) => {
            ...
            try {
                ...
                for (const br of allBranches) {
                    const driveMatch = br.match(/^feature-(\d+)-(?![a-z]{2}-)(.+)$/);
                    const wtMatch = br.match(/^feature-(\d+)-([a-z]{2})-(.+)$/);
                    if (driveMatch) {
                        const [, fid, slug] = driveMatch;
                        if (!byFeatureId.has(fid)) byFeatureId.set(fid, { drive: [], worktree: [] });
                        byFeatureId.get(fid).drive.push({ branch: br, slug });
                    } else if (wtMatch) {
                        const [, fid, agent, slug] = wtMatch;
                        if (!byFeatureId.has(fid)) byFeatureId.set(fid, { drive: [], worktree: [] });
                        byFeatureId.get(fid).worktree.push({ branch: br, agent, slug });
                    }
                }

                for (const [fid, groups] of byFeatureId) {
                    if (groups.drive.length === 0 || groups.worktree.length === 0) continue;
                    for (const drive of groups.drive) {
                        const siblings = groups.worktree.filter(wt => wt.slug === drive.slug);
