    ...
    const commands = {
        ...
        'install-agent': async (args) => {
            ...
            try {
                ...
                let installManifest;
                try {
                    installManifest = installManifestLib.readManifest(repoRoot);
                } catch (e) {
                            ...
                            if (isAigonRepo && fs.existsSync(contribDir)) {
                                ...
                                files.forEach(file => {
                                    ...
                                    if (isSkillMd) {
                                        ...
                                        const fileName = outputSpec.skillFileName || 'SKILL.md';
                                        ...
                                        const outputContent = formatCommandOutput(body, description, cmdName, outputConfig);
                                        const fileName = `${cmdName}${outputSpec.commandFileExtension}`;
                ...
                try {
                    const installPaths = '.aigon/docs/ .aigon/install-manifest.json .agents/ .claude/ .cursor/ .codex/ .gemini/';
        ...
        'apply': async (args = []) => {
            ...
            try {
                ...
                if (uniqueInstalledAgents.length > 0) {
                    ...
                    await commands['install-agent'](uniqueInstalledAgents);
                } else {
        ...
        'installed-notice': (args = []) => {
            // F500: print a one-shot summary of which known repos still
            // reflect an older `aigon apply` after the CLI has been upgraded.
            // Wired to `npm postinstall`; safe to run manually.
            // Silent when no known repos, no repos behind, or anything fails.
            try {
                const { readConductorReposFromGlobalConfig } = require('../config');
                const currentVersion = getAigonVersion();
        ...
        'doctor': async (args) => {
            ...
            if (doFixTemplates) {
                ...
                if (agentsToRefresh.size > 0) {
                    ...
                    await commands['install-agent']([...agentsToRefresh]);
                }
                ...
                for (const r of handEditedRows) {
                    ...
                    if (action === 'r') {
                        ...
                        await commands['install-agent']([r.agentId]);
                    }
                    // 'k' is no-op
                }
                rl.close();
            ...
            console.log('\n🔍 Install manifest...');
            {
                ...
                if (manifestReadError) {
                    ...
                    if (doFix) {
                        console.log('  💡 Run `aigon doctor --fix` after resolving the JSON error, or delete .aigon/install-manifest.json to regenerate.');
                    }
                    ...
                    if (doFix) {
                        try {
                            const { runPendingMigrations: runMig } = require('../migration');
                            await runMig(doctorRepoRoot);
                            console.log('  🔧 Install manifest initialized via migration 2.61.0');
                        } catch (e) {
        ...
        'uninstall': async (args) => {
            ...
            const unRepoRoot = process.cwd();

            let manifest;
            try {
                manifest = installManifestLib.readManifest(unRepoRoot);
            } catch (e) {
                console.error(`❌ ${e.message}`);
                return;
            }
            if (!manifest) {
                console.error('❌ No install manifest found at .aigon/install-manifest.json');
                console.error('   Run `aigon doctor --fix` to regenerate it from current install footprint.');
                return;
            }

            const files = manifest.files || [];
            if (files.length === 0) {
                console.log('ℹ️  Install manifest is empty — nothing to uninstall.');
                return;
            }

            // Check for user-modified files
            const modified = installManifestLib.getModifiedFiles(manifest, unRepoRoot);
            const modifiedPaths = new Set(modified.map(m => m.path));

            console.log('\nFiles that would be removed:');
            files.forEach(entry => {
                const exists = require('fs').existsSync(path.join(unRepoRoot, entry.path));
            ...
            if (modified.length > 0 && !force) {
                ...
                console.error('Uninstall aborted. Pass --force to override.');
                return;
            }

            // Confirm
            const unNonInteractive = !!process.env.AIGON_NONINTERACTIVE || !process.stdin.isTTY;
            if (!unNonInteractive) {
                const { createInterface } = require('readline');
                const rl = createInterface({ input: process.stdin, output: process.stdout });
                const answer = await new Promise(resolve => rl.question('\nProceed with uninstall? [y/N] ', resolve));
                rl.close();
            ...
            for (const entry of files) {
                const absPath = path.join(unRepoRoot, entry.path);
                if (!fs.existsSync(absPath)) { skipped++; continue; }
                if (modifiedPaths.has(entry.path) && !force) { skipped++; continue; }
                try {
                    fs.unlinkSync(absPath);
                    deleted++;
                    // Remove empty parent directories (best-effort, stop at repoRoot)
                    let dir = path.dirname(absPath);
                    while (dir !== unRepoRoot && dir.startsWith(unRepoRoot)) {
                        try {
            ...
            const manifestAbsPath = path.join(unRepoRoot, installManifestLib.MANIFEST_PATH);
            try {
                if (fs.existsSync(manifestAbsPath)) fs.unlinkSync(manifestAbsPath);
            } catch (_) { /* best-effort */ }

            console.log(`\n✅ Uninstalled: ${deleted} file(s) removed, ${skipped} skipped.`);
            console.log('ℹ️  Runtime state preserved: .aigon/workflows/, .aigon/state/, .aigon/sessions/, .aigon/config.json');
            console.log('Aigon uninstalled. Your `AGENTS.md`, `CLAUDE.md`, `README.md`, and project files were not touched.');
        },
        ...
        'seed-reset': async (args) => {
            ...
            // Bench-mode signal from perf-bench: skip remote pushes (worktrees are
            // local) and prefer the gold-image fast path when one exists.
            const benchMode = process.env.AIGON_BENCH_MODE === '1';
            ...
            async function provisionPhase() {
                ...
                try {
                    ...
                    // Skipped in bench mode — agent worktrees never push to the working
                    // remote during a benchmark pair, so this push is pure overhead.
                    if (workingRepoUrl && !benchMode) {
                        try {
                    ...
                    //     Skipped in bench mode — the seed doesn't change between pairs.
                    if (!benchMode) {
                        try {
            ...
            // --- Phase 2b (bench mode): FAST PATH — restore gold image ---
            // Replaces clone + provision when AIGON_BENCH_MODE=1 and a fresh
            // gold image exists. Cuts per-pair overhead from ~2-3 min to ~15-20s
            // by skipping git clone, npm install, agent install, and both pushes.

            function tryFastPathRestore() {
                if (!benchMode) return false;
                if (!goldImageExists(repoName)) return false;
                const meta = readGoldMeta(repoName);
                const currentVersion = getAigonVersionForGold();
                const versionMatch = meta && meta.aigonVersion === currentVersion;
                if (!versionMatch && !forceSnapshot) {
                    console.log(`\n⚠️  Gold image built with aigon v${meta && meta.aigonVersion} — current is v${currentVersion}. Falling back to full reset.`);
                    return false;
                }
                console.log('\n⚡ Phase 2 (fast): Restore gold image\n');
                const restore = extractGoldImage({ seedName: repoName, parentDir, repoName, repoPath });
                if (!restore.ok) {
                    console.log(`   ⚠️  Fast restore failed (${restore.error}) — falling back to full reset.`);
                    return false;
                }
                const sizeMb = (() => {
                    try { return Math.round(fs.statSync(goldImagePath(repoName)).size / 1_048_576); }
                    catch (_) { return null; }
                })();
                console.log(`   ✓ Extracted gold image (${sizeMb != null ? sizeMb + ' MB, ' : ''}${Math.round(restore.ms / 100) / 10}s)`);
                ...
                if (workingRepoUrl) {
                    try {
                        ...
                        console.log(`   ⚠️  Could not repoint origin: ${e.message}`);
                    }
                }

                // Manifests and .aigon/state are gitignored, so the tarball does not
                // contain them. Rebuild them and collapse any active stages back to
                // backlog so the board is clean for the new pair.
                try {
                    rebuildSeedFeatureManifests(repoPath, { collapseActiveToBacklog: true });
                    console.log(`   ✓ Rebuilt feature manifests`);
                } catch (e) {
                    console.log(`   ⚠️  Manifest rebuild failed: ${e.message}`);
                }

                // Ensure local runtime files stay ignored (gold image may pre-date
                // any new entries we want excluded).
                try {
                    const envGitignoreStatus = getEnvLocalGitignoreStatus(repoPath);
                    const localIgnoreEntries = [
                        '.aigon/state/',
                    ...
                    ensureLocalGitExclude(repoPath, localIgnoreEntries);
                } catch (_) { /* non-fatal */ }
            ...
            // Bench-mode fast path: extract gold image instead of clone+provision.
            if (tryFastPathRestore()) {
                console.log(`\n✅ ${repoName} reset from gold image (fast path).`);
                return;
            }
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'uninstall', 'setup', 'global-setup', 'check-prerequisites', 'check-version', 'apply', 'update', 'project-context', 'installed-notice', 'doctor', 'seed-reset', 'trust-worktree'];
