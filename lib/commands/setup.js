...
function rebuildSeedFeatureManifests(repoPath) {
    ...
    // Bootstrap minimal workflow snapshots so the dashboard can derive actions.
    // Without these, features created by writing spec files directly (e.g., seed
    // fixtures) won't show action buttons in the dashboard.
    const { getSnapshotPathForEntity, getEventsPathForEntity } = require('../workflow-core/paths');
    manifests.forEach(manifest => {
        const snapshotPath = getSnapshotPathForEntity(repoPath, 'feature', manifest.id);
        const eventsPath = getEventsPathForEntity(repoPath, 'feature', manifest.id);
        // Only bootstrap if no snapshot exists yet (don't overwrite real workflow state)
        if (fs.existsSync(snapshotPath)) return;

        const lifecycleMap = { inbox: 'inbox', backlog: 'backlog', 'in-progress': 'in_progress', done: 'done' };
        const lifecycle = lifecycleMap[manifest.stage] || manifest.stage;
        const snapshot = {
            ...
            winnerAgentId: null,
            agents: {},
            currentSpecState: manifest.stage,
        ...
        // Write a minimal event log so event-store reads don't fail
        if (!fs.existsSync(eventsPath)) {
            const event = JSON.stringify({ type: 'feature.bootstrapped', featureId: manifest.id, lifecycle, at: new Date().toISOString() });
...
function listPidsUsingPath(targetPath) {
    ...
    try {
        const output = execSync(`lsof -t +D ${quoteShellArg(targetPath)} 2>/dev/null || true`, {
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'ignore'],
        });
        return [...new Set(
...
function untrackFiles(repoPath, files) {
    ...
    try {
        execSync(`git rm --cached -- ${files.map(file => JSON.stringify(file)).join(' ')}`, {
            cwd: repoPath,
    ...
    const commands = {
        ...
        'install-agent': (args) => {
            ...
            try {
                ...
                // 2. Scaffold AGENTS.md on first install only — never overwrite user-owned file
                ...
                uniqueAgents.forEach(agentKey => {
                    ...
                    if (cliBinary) {
                        ...
                        if (!isBinAvail(cliBinary)) {
                            const installHints = {
                                claude: 'brew install claude  (or: npm i -g @anthropic-ai/claude-code)',
                                gemini: 'npm i -g @google/gemini-cli',
                                codex: 'npm i -g @openai/codex',
                                cursor: 'Download from https://cursor.com',
                                agent: 'Download Cursor from https://cursor.com (CLI is "agent")',
                                vibe: 'pip install mistral-vibe',
                            };
                            const hint = installHints[cliBinary] || `Install ${cliBinary} and add it to your PATH`;
                    ...
                    if (extras.settings && extras.settings.enabled) {
                        ...
                        if (extras.settings.policies) {
                            const policiesDir = path.join(process.cwd(), '.gemini', 'policies');
                            const policyFile = path.join(policiesDir, 'aigon.toml');
                            ...
                            if (policyContent !== existingPolicy) {
                                ...
                                console.log(`   ✅ Created .gemini/policies/aigon.toml (Policy Engine)`);
                                settingsChanged = true;
                            }
                        ...
                        if (extras.settings.hooks) {
                            ...
                            // Resolve aigon to absolute path for reliable hook execution
                            let aigonAbsPath = 'aigon';
                            try {
                                const resolved = require('child_process').execFileSync('which', ['aigon'], { encoding: 'utf8' }).trim();
                                if (resolved) aigonAbsPath = resolved;
                            } catch (_) {}
                            // Migrate existing hooks to use absolute path
                            if (aigonAbsPath !== 'aigon') {
                                Object.values(settings.hooks).forEach(hookArr => {
                                    hookArr.forEach(entry => {
                                        if (entry.hooks) {
                                            entry.hooks.forEach(h => {
                                                if (h.command && h.command.startsWith('aigon ') && !h.command.startsWith('/')) {
                                                    h.command = h.command.replace(/^aigon /, `${aigonAbsPath} `);
                                                    settingsChanged = true;
                                                }
                                            });
                                        } else if (entry.command && entry.command.startsWith('aigon ') && !entry.command.startsWith('/')) {
                                            entry.command = entry.command.replace(/^aigon /, `${aigonAbsPath} `);
                                            settingsChanged = true;
                                        }
                                    });
                                });
                            ...
                            Object.entries(extras.settings.hooks).forEach(([event, hookConfigs]) => {
                                ...
                                hookConfigs.forEach(hookConfig => {
                                    ...
                                    if (existingIdx >= 0 && hookConfig.hooks) {
                                        // Merge: add any missing hook commands to the existing entry
                                        const existing = settings.hooks[event][existingIdx];
                                        if (!existing.hooks) existing.hooks = [];
                                        hookConfig.hooks.forEach(newHook => {
                                            // Extract the command name (e.g. 'aigon project-context')
                                            const cmdName = newHook.command.replace(/^aigon /, '');
                                            const alreadyHas = existing.hooks.some(h =>
                                                h.command && h.command.includes(cmdName)
                                            );
                                            if (!alreadyHas) {
                                                const resolvedHook = { ...newHook };
                                                if (resolvedHook.command && resolvedHook.command.startsWith('aigon ')) {
                                                    resolvedHook.command = resolvedHook.command.replace(/^aigon /, `${aigonAbsPath} `);
                                                }
                                        ...
                                        if (resolved.hooks) {
                                            ...
                                            resolved.command = resolved.command.replace(/^aigon /, `${aigonAbsPath} `);
                                        }
                        ...
                        if (hookContent.hooks) {
                            ...
                            Object.entries(hookContent.hooks).forEach(([event, hookConfigs]) => {
                                ...
                                hookConfigs.forEach(hookConfig => {
                                    ...
                                    if (!alreadyExists) {
                                        // Resolve aigon to absolute path
                                        let resolvedAigon = 'aigon';
                                        try {
                                            const rp = require('child_process').execFileSync('which', ['aigon'], { encoding: 'utf8' }).trim();
                                            if (rp) resolvedAigon = rp;
                                        } catch (_) {}
                                        const resolvedHook = JSON.parse(JSON.stringify(hookConfig));
                                        if (resolvedHook.command && resolvedHook.command.startsWith('aigon ')) {
                                            resolvedHook.command = resolvedHook.command.replace(/^aigon /, `${resolvedAigon} `);
                                        }
                ...
                try {
                    const gitStatus = getStatusRaw('docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/');
                    if (gitStatus) {
                        console.log(`\n📝 To commit these changes:`);
                        console.log(`   git add docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/ 2>/dev/null; git commit -m "chore: install Aigon v${currentVersion || 'latest'}"`);
                    }
                } catch (e) {
        ...
        'update': (args = []) => {
            ...
            try {
                ...
                // 7. Restart dashboard if running (only when updating from the aigon repo itself)
                const aigonRoot = path.resolve(__dirname, '..', '..');
                const isAigonRepo = path.resolve(process.cwd()) === aigonRoot;
                if (isAigonRepo) {
                    try {
                        const { loadProxyRegistry, isProcessAlive } = require('../proxy');
                        const registry = loadProxyRegistry();
                        let restarted = false;
                        for (const [appIdKey, servers] of Object.entries(registry)) {
                        ...
                        if (restarted) {
                            // Respawn dashboard in detached process
                            const aigonBin = path.join(aigonRoot, 'aigon-cli.js');
                            const { spawn } = require('child_process');
                            const child = spawn(process.execPath, [aigonBin, 'dashboard', 'start'], {
                                detached: true,
                                stdio: 'ignore',
                                cwd: aigonRoot,
                            });
                            child.unref();
                            console.log(`\n🔄 Dashboard restarted (PID ${child.pid})`);
                        }
                    } catch (e) {
                        // Non-fatal — dashboard restart is best-effort
                    }
                }
                ...
                const aigonPaths = 'docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/ .aigon/';
                try {
        ...
        'doctor': (args) => {
            const {
                ...
                reconcileProxyRoutes: reconcileProxyRoutesFn,
            ...
            // cmux check (macOS only, shown when configured or installed)
            if (process.platform === 'darwin') {
                const { getEffectiveConfig: getConfig } = require('../config');
                const effectiveConfig = getConfig();
                if (effectiveConfig.terminal === 'cmux' || isBinAvailable('cmux')) {
                    if (isBinAvailable('cmux')) {
                        try {
                            const cmuxVersion = execSync('cmux --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
                            console.log(`  ✅ cmux ${cmuxVersion}`);
                        } catch {
                            console.log('  ✅ cmux is installed');
                        }
                    } else {
                        console.log('  ⚠️  cmux configured as terminal but not found');
                        console.log('     Install: brew tap manaflow-ai/cmux && brew install --cask cmux');
                    }
                }
            }

            // Agent CLI checks
            const agentBinMap = { cc: 'claude', gg: 'gemini', cx: 'codex', cu: 'agent', mv: 'vibe' };
            const agentInstallHints = {
                cc: 'brew install claude  (or: npm i -g @anthropic-ai/claude-code)',
                gg: 'npm i -g @google/gemini-cli',
                cx: 'npm i -g @openai/codex',
                cu: 'Download from https://cursor.com',
                mv: 'pip install mistral-vibe',
            };
            let foundAgents = 0;
            ...
            try {
                ...
                const ok = (v) => v ? '✅' : '❌';
                console.log(`  ${ok(diag.proxy.running)} aigon-proxy running`);
                const { total } = diag.routes;
                console.log(`  ℹ️  Routes: ${total} registered`);
                if (diag.fix) {
                    console.log(`\n  💡 Fix: ${diag.fix}`);
                } else if (diag.healthy) {
                    // Reconcile to clean dead entries
                    try {
                        const r = reconcileProxyRoutesFn();
                        if (r.cleaned > 0) console.log(`\n  ✅ Cleaned ${r.cleaned} stale entries, ${r.unchanged} unchanged`);
                    } catch (e) {
            ...
            function detectDefaultBranchForRepo(repoPath) {
                ...
                for (const candidate of ['main', 'master']) {
                    try {
                        execSync(`git -C ${quoted} show-ref --verify --quiet refs/heads/${candidate}`, { stdio: 'ignore' });
                        return candidate;
                    } catch (_) { /* ignore */ }
                }
                return 'main';
            ...
            function hasFeatureWorktreeCommits(id, agent) {
                const wtBase = process.cwd() + '-worktrees';
                if (!fs.existsSync(wtBase)) return false;
                ...
                try {
                    ...
                    if (hit) worktreePath = path.join(wtBase, hit);
                } catch (_) { /* ignore */ }
        ...
        'seed-reset': (args) => {
            ...
            if (!repoArg) {
                ...
                console.error('Cleans: tmux sessions, worktrees, the entire directory, then fresh clone.');
                console.error(`\nKnown seeds: ${Object.keys(SEED_REGISTRY).join(', ')}`);
                console.error('\nExamples:');
                console.error('  aigon seed-reset ~/src/brewboard');
                console.error('  aigon seed-reset brewboard            # resolves to ~/src/brewboard');
                console.error('  aigon seed-reset ~/src/trailhead --dry-run');
                console.error('  aigon seed-reset ~/src/brewboard --force   # skip confirmation');
                return;
            }
            ...
            // Inventory what needs cleaning
            let tmuxSessions = [];
            try {
                const tmuxList = spawnSync('tmux', ['list-sessions', '-F', '#S'], { encoding: 'utf8' });
                if (!tmuxList.error && tmuxList.status === 0) {
                    tmuxSessions = tmuxList.stdout.split('\n').map(s => s.trim()).filter(s => {
                        return s.toLowerCase().startsWith(repoName.toLowerCase() + '-');
                    });
                }
            } catch (e) { /* no tmux */ }
            ...
            // Summary
            if (tmuxSessions.length) console.log(`   ${tmuxSessions.length} tmux session(s): ${tmuxSessions.join(', ')}`);
            if (worktreeList.length) console.log(`   ${worktreeList.length} worktree(s)`);
            if (fs.existsSync(repoPath)) console.log(`   rm -rf ${repoPath}`);
            if (fs.existsSync(worktreeDir)) console.log(`   rm -rf ${worktreeDir}`);
            console.log(`   git clone ${seedUrl} ${repoPath}`);
            console.log(`   aigon install-agent (re-install to current CLI version)`);
            console.log('');
            ...
            tmuxSessions.forEach(sessionName => {
                try {
            ...
            try {
                execSync(`pkill -f "aigon:feature-do.*${repoName}" || true`, { stdio: 'ignore' });
                execSync(`pkill -f "aigon:research-do.*${repoName}" || true`, { stdio: 'ignore' });
                execSync(`pkill -f "aigon:feature-review.*${repoName}" || true`, { stdio: 'ignore' });
            } catch (e) { /* ignore */ }

            // 2b. Kill any straggler processes still running from the repo or its worktrees.
            // tmux shutdown is not sufficient because detached shells, dev servers, and agent
            // child processes can continue running with cwd under the old worktree paths.
            const killPatterns = [
                repoPath,
                worktreeDir,
                `${repoName}-worktrees`,
            ...
            killPatterns.forEach(pattern => {
                try {
                    execSync(`pkill -f "${String(pattern).replace(/"/g, '\\"')}" || true`, { stdio: 'ignore' });
                } catch (e) { /* ignore */ }
            });

            // 3. Remove worktree permissions/trust from Claude settings (before wiping)
            if (worktreeList.length > 0) {
                try {
                    const { removeWorktreePermissions, removeWorktreeTrust } = u;
                    if (removeWorktreePermissions) removeWorktreePermissions(worktreeList);
                    if (removeWorktreeTrust) removeWorktreeTrust(worktreeList);
                } catch (e) { /* non-fatal */ }
            }

            // 4. GC dev-proxy entries
            try {
                const { gcDevServers } = u;
                if (gcDevServers) {
                    const removed = gcDevServers();
                    if (removed > 0) console.log(`   ✓ Cleaned ${removed} stale dev-proxy entries`);
                }
            } catch (e) { /* non-fatal */ }

            // 5. Wipe the repo directory and worktree directory
            ...
            if (fs.existsSync(worktreeDir)) {
                ...
                console.log(`   ✓ Removed ${worktreeDir}`);
            }

            // 6. Clone fresh from seed repo
            try {
            ...
            // 7. Rebuild manifests from spec folders (manifests are gitignored)
            try {
            ...
            // 8. Ensure local runtime files stay ignored
            try {
                ...
                if (localExclude.addedEntries.length > 0) {
                    console.log(`   ✓ Updated local git exclude with: ${localExclude.addedEntries.join(', ')}`);
                }
            } catch (e) {
                console.log(`   ⚠️  Could not update local git exclude: ${e.message}`);
            }

            // 9. Re-install agents to match current CLI version
            try {
                ...
                getAvailableAgents().forEach(agentId => {
                    ...
                    if (!config) return;
                    const docsAgentPath = config.agentFile
                        ? path.join(repoPath, 'docs', 'agents', config.agentFile)
                        : null;
                    const localCommandDir = (config.output && !config.output.global && config.output.commandDir)
                        ? path.join(repoPath, config.output.commandDir)
                        : null;
                    const rootFilePath = config.rootFile ? path.join(repoPath, config.rootFile) : null;
                    const extras = config.extras || {};
                    const settingsPath = extras.settings?.enabled ? path.join(repoPath, extras.settings.path) : null;
                    const configPath = extras.config?.enabled ? path.join(repoPath, extras.config.path) : null;
                    const isInstalled =
                        (rootFilePath && fs.existsSync(rootFilePath)) ||
                        (docsAgentPath && fs.existsSync(docsAgentPath)) ||
                        (localCommandDir && fs.existsSync(localCommandDir)) ||
                        (settingsPath && fs.existsSync(settingsPath)) ||
                        (configPath && fs.existsSync(configPath));
                    if (isInstalled) detectedAgents.push(agentId);
                });
                if (detectedAgents.length > 0) {
                    console.log(`\n📦 Re-installing agents: ${detectedAgents.join(', ')}`);
                    commands['install-agent'](detectedAgents);
                    const cliVersion = getAigonVersion();
                    if (cliVersion) setInstalledVersion(cliVersion);

                    // Auto-commit so worktrees inherit the updated commands
                    try {
                        execSync('git add -A && git commit -m "chore: install Aigon v' + (cliVersion || 'latest') + '"', {
                            cwd: repoPath,
                            stdio: 'pipe',
                        });
                        console.log(`   ✓ Committed agent install`);
                    } catch (e) {
                        // Nothing to commit (already up to date) — that's fine
                    }
                }
                process.chdir(savedCwd);
            } catch (e) {
                console.log(`   ⚠️  Could not re-install agents: ${e.message}`);
            }
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'check-version', 'update', 'project-context', 'doctor', 'seed-reset'];
