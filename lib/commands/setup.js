    ...
    const commands = {
        ...
        'doctor': async (args) => {
            ...
            function runAgentAuthCheck(agentConfig) {
                ...
                if (method === 'none') {
                    return { status: 'unknown', message: authCheck.loginHint || 'auth managed externally' };
                }
                if (method === 'envVar') {
            ...
            function printMultiRepoVersionSweep() {
                ...
                console.log(`  ${'REPO'.padEnd(maxName + 2)}${'PATH'.padEnd(maxPath + 2)}${'VERSION'.padEnd(12)}STATUS`);
                        ...
                        : row.status === 'behind' ? '⚠️  behind'
                            : row.status === 'missing' ? '❌ missing'
                                : row.status === 'ahead' ? 'ℹ️  ahead'
                                    : '· not a dir';
                    console.log(`  ${row.name.padEnd(maxName + 2)}${doctorShortenPath(row.path).padEnd(maxPath + 2)}${String(row.version).padEnd(12)}${icon}`);
                }
