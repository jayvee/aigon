    ...
    const commands = {
        ...
        'install-agent': (args) => {
            ...
            try {
                ...
                // Git commit - auto-commit if --commit flag, otherwise suggest
                try {
                    const gitStatus = getStatusRaw('docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/');
                    if (gitStatus) {
                        const commitMsg = `chore: install Aigon v${currentVersion || 'latest'}`;
                        if (args.includes('--commit')) {
                            try {
                                execSync(`git add docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/ .aigon/version 2>/dev/null; git commit -m "${commitMsg}"`, { stdio: 'pipe' });
                                console.log(`\n📝 Auto-committed: ${commitMsg}`);
                            } catch (commitErr) {
                                console.warn(`\n⚠️  Auto-commit failed: ${commitErr.message}`);
                            }
                        } else {
                            console.log(`\n📝 To commit these changes:`);
                            console.log(`   git add docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/ .aigon/version 2>/dev/null; git commit -m "${commitMsg}"`);
                        }
                    }
                } catch (e) {
        ...
        'doctor': (args) => {
            ...
            if (doRegister) {
                ...
                if (profile.devServer.enabled) {
                    ...
                    if (!result) {
                        console.warn('⚠️  No PORT found in .env or .env.local — skipping port registration');
                        return;
                    }
                    const basePort = result.port;
            ...
            const stateDir = manifest.STATE_DIR;
            let manifestFiles = [];
            ...
            const locksDir = manifest.LOCKS_DIR;
        ...
        'seed-reset': (args) => {
            ...
            if (!repoArg) {
                ...
                console.error('\nResets a seed repo to its initial seeded state.');
                console.error('Cleans: tmux sessions, worktrees, branches, .aigon/state, git history.');
                console.error('\nExamples:');
            ...
            const repoPath = path.resolve(repoArg.replace(/^~/, process.env.HOME));
            ...
            // Detect installed agents before reset (so we can re-install after)
            const installedAgents = [];
            const agentMarkers = { cc: '.claude/commands/aigon', gg: '.gemini/commands/aigon', cu: '.cursor/commands', cx: '.codex' };
            for (const [agentId, markerDir] of Object.entries(agentMarkers)) {
                if (fs.existsSync(path.join(repoPath, markerDir))) {
                    installedAgents.push(agentId);
                }
            }

            // Find the seed commit
            let seedCommit;
            try {
                seedCommit = execSync(
                    'git log --oneline --format="%H %s" | grep "chore: seed aigon specs" | head -1',
                    { cwd: repoPath, encoding: 'utf8' }
                ).trim().split(' ')[0];
            } catch (e) { /* empty */ }

            if (!seedCommit) {
                console.error(`❌ Could not find seed commit ("chore: seed aigon specs") in ${repoName}`);
                return;
            }

            const shortSeed = seedCommit.slice(0, 7);
            console.log(`\n🔄 Resetting ${repoName} to seed commit ${shortSeed}...\n`);
            ...
            let branchList = [];
            try {
                branchList = execSync('git branch --format="%(refname:short)"', { cwd: repoPath, encoding: 'utf8' })
                    .split('\n').map(b => b.trim()).filter(b => b && b !== 'main' && b !== 'master');
            } catch (e) { /* ignore */ }
            ...
            const aigonStateDir = path.join(repoPath, '.aigon', 'state');
            const aigonLocksDir = path.join(repoPath, '.aigon', 'locks');
            let stateFiles = [];
            let lockFiles = [];
            try { stateFiles = fs.existsSync(aigonStateDir) ? fs.readdirSync(aigonStateDir) : []; } catch (e) { /* */ }
            try { lockFiles = fs.existsSync(aigonLocksDir) ? fs.readdirSync(aigonLocksDir) : []; } catch (e) { /* */ }

            // Print summary
            if (tmuxSessions.length) console.log(`   ${tmuxSessions.length} tmux session(s): ${tmuxSessions.join(', ')}`);
            if (worktreeList.length) console.log(`   ${worktreeList.length} worktree(s): ${worktreeList.map(p => path.basename(p)).join(', ')}`);
            if (branchList.length) console.log(`   ${branchList.length} branch(es): ${branchList.join(', ')}`);
            if (stateFiles.length) console.log(`   ${stateFiles.length} state file(s)`);
            if (lockFiles.length) console.log(`   ${lockFiles.length} lock file(s)`);
            console.log(`   git reset --hard ${shortSeed}`);
            ...
            if (hasRemote) console.log(`   git push --force origin main`);
            console.log('');
            ...
            // Remove worktrees
            worktreeList.forEach(wtPath => {
                try {
                    execSync(`git worktree remove --force "${wtPath}"`, { cwd: repoPath, stdio: 'pipe' });
                    console.log(`   ✓ Removed worktree: ${path.basename(wtPath)}`);
                } catch (e) {
                    try {
                        execSync(`rm -rf "${wtPath}" && git worktree prune`, { cwd: repoPath, stdio: 'pipe' });
                        console.log(`   ✓ Force-removed worktree: ${path.basename(wtPath)}`);
                    } catch (e2) {
                        console.error(`   ❌ Failed to remove worktree: ${path.basename(wtPath)}`);
                    }
                }
            });
            ...
            // Git reset to seed commit
            try {
                execSync(`git reset --hard ${seedCommit}`, { cwd: repoPath, stdio: 'pipe' });
                console.log(`   ✓ Reset to seed commit: ${shortSeed}`);
            } catch (e) {
                console.error(`   ❌ Git reset failed: ${e.message}`);
                return;
            }
            ...
            // Reset remote to match seed state
            try {
                const remoteUrl = execSync('git remote get-url origin', { cwd: repoPath, encoding: 'utf8' }).trim();
                if (remoteUrl) {
                    // Delete remote branches that no longer exist locally
                    try {
                        const remoteBranches = execSync('git branch -r --format="%(refname:short)"', { cwd: repoPath, encoding: 'utf8' })
                            .split('\n').map(b => b.trim()).filter(b => b && !b.includes('HEAD'));
                        for (const rb of remoteBranches) {
                            const branchName = rb.replace(/^origin\//, '');
                            if (branchName !== 'main' && branchName !== 'master') {
                                try {
                                    execSync(`git push origin --delete "${branchName}"`, { cwd: repoPath, stdio: 'pipe' });
                                    console.log(`   ✓ Deleted remote branch: ${branchName}`);
                                } catch (e) { /* ignore */ }
                            }
                        }
                    } catch (e) { /* no remote branches */ }

                    // Force-push main to seed commit
                    execSync('git push --force origin main', { cwd: repoPath, stdio: 'pipe' });
                    console.log('   ✓ Force-pushed main to remote');
                }
            } catch (e) {
                // No remote configured — that's fine
            }

            // Remove .aigon/state and .aigon/locks
            [aigonStateDir, aigonLocksDir].forEach(dir => {
                if (fs.existsSync(dir)) {
                    try {
                        fs.rmSync(dir, { recursive: true });
                        console.log(`   ✓ Removed ${path.relative(repoPath, dir)}/`);
                    } catch (e) { /* ignore */ }
                }
            });

            // GC dev-proxy entries
            try {
            ...
            // Remove worktree permissions/trust from Claude settings
            ...
            // Re-install agents that were present before reset
            if (installedAgents.length > 0) {
                console.log(`\n📦 Re-installing agents: ${installedAgents.join(', ')}...`);
                for (const agentId of installedAgents) {
                    try {
                        execSync(`aigon install-agent ${agentId}`, { cwd: repoPath, stdio: 'pipe' });
                        console.log(`   ✓ Installed ${agentId}`);
                    } catch (e) {
                        console.error(`   ❌ Failed to install ${agentId}: ${e.message}`);
                    }
                }
                // Auto-commit all agent files
                try {
                    const version = u.getAigonVersion ? u.getAigonVersion() : 'latest';
                    execSync(
                        `git add docs/ AGENTS.md .claude/ .cursor/ .codex/ .gemini/ .gitignore 2>/dev/null; git commit -m "chore: install Aigon v${version}"`,
                        { cwd: repoPath, stdio: 'pipe' }
                    );
                    console.log(`   ✓ Committed agent installation`);
                } catch (e) {
                    // May fail if nothing to commit
                }

                // Push final state to remote (seed + agent install)
                try {
                    execSync('git remote get-url origin', { cwd: repoPath, encoding: 'utf8' });
                    execSync('git push --force origin main', { cwd: repoPath, stdio: 'pipe' });
                    console.log('   ✓ Pushed reset state to remote');
                } catch (e) {
                    // No remote or push failed — non-fatal
                }
            }

            console.log(`\n✅ ${repoName} reset to seed state (${shortSeed}).`);
        },
    };
