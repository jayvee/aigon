    ...
    const commands = {
                    ...
                    if (extras.settings && extras.settings.enabled) {
                        ...
                        if (extras.settings.hooks) {
                            ...
                            const stableCandidates = [
                                '/opt/homebrew/bin/aigon',  // macOS Homebrew
                                '/usr/local/bin/aigon',     // macOS npm -g (pre-Homebrew node)
                                '/usr/bin/aigon',           // Linux npm -g (NodeSource / system node)
                            ];
        ...
        'global-setup': async (args = []) => {
            ...
            // Always run prerequisite checks — hard blockers print even in quiet mode
            {
                ...
                const prereqs = runPrerequisiteChecks();
            ...
            if (isInteractive) {
                ...
                if (process.platform === 'darwin') {
                    console.log('');
                    console.log('Which terminal app do you use to run agent sessions?');
                    console.log('  1) Apple Terminal  (built-in)');
                    console.log('  2) iTerm2');
                    console.log('  3) Warp');
                    const defaultKey = { 'apple-terminal': '1', 'iterm2': '2', 'warp': '3' }[terminalApp] || '1';
                    const answer = (await ask(`  Your choice [${defaultKey}]: `)).trim();
                    const termMap = {
                        '1': 'apple-terminal', 'apple-terminal': 'apple-terminal',
                        '2': 'iterm2', 'iterm2': 'iterm2',
                        '3': 'warp', 'warp': 'warp',
                    };
                    terminalApp = termMap[answer] || terminalApp;
                }
            ...
            // Persist: start from raw (preserves existing keys on --force) and
            // set the keys we own.
            const configToSave = Object.assign({}, rawConfig, {
                schemaVersion: TERMINAL_CONFIG_MIGRATION_VERSION,
                terminalApp: terminalApp || null,
                repos: rawConfig.repos || [],
            });

            cfg.saveGlobalConfig(configToSave);
        ...
        'check-prerequisites': async (args = []) => {
            ...
            const results = runPrerequisiteChecks();
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'global-setup', 'check-prerequisites', 'check-version', 'update', 'project-context', 'doctor', 'seed-reset', 'trust-worktree'];
