    ...
    const commands = {
        'init': (args) => {
            ...
            try {
                ...
                installedAgents.forEach(agentId => {
                    try { agentRegistry.ensureAgentTrust(agentId, [wtBase]); } catch (_) { /* best-effort */ }
                });
            ...
            try {
                ...
                uniqueAgents.forEach(agentKey => {
                    ...
                    if (config.commands && config.commands.length > 0 && config.output) {
                        ...
                        const migrated = isSkillMd ? [] : migrateOldFlatCommands(cmdDir, config);
                                        ...
                                        if (entry.hooks) {
                                            entry.hooks.forEach(h => {
                                                if (h.command && h.command.startsWith('aigon ') && !h.command.startsWith('/')) {
                                                    ...
                                                    settingsChanged = true;
                                                } else if (h.command && staleAigonPath.test(h.command) && !h.command.startsWith(aigonAbsPath + ' ')) {
                                                    h.command = h.command.replace(staleAigonPath, `${aigonAbsPath} `);
                                                    settingsChanged = true;
                                                }
                                            });
                                            ...
                                            settingsChanged = true;
                                        } else if (entry.command && staleAigonPath.test(entry.command) && !entry.command.startsWith(aigonAbsPath + ' ')) {
                                            entry.command = entry.command.replace(staleAigonPath, `${aigonAbsPath} `);
                                            settingsChanged = true;
                                        }
                                    });
                                });
                ...
                try {
                    ...
                    uniqueAgents.forEach(agentId => {
                        try { _agentRegistryTrust.ensureAgentTrust(agentId, [wtBase]); } catch (_) { /* best-effort */ }
                    });
                } catch (_) { /* best-effort */ }

                // Git commit suggestion - only if there are actual changes
                try {
                    const gitStatus = getStatusRaw('docs/ AGENTS.md .agents/ .claude/ .cursor/ .codex/ .gemini/');
                    if (gitStatus) {
                        console.log(`\n📝 To commit these changes:`);
                        console.log(`   git add docs/ AGENTS.md .agents/ .claude/ .cursor/ .codex/ .gemini/ 2>/dev/null; git commit -m "chore: install Aigon v${currentVersion || 'latest'}"`);
                    }
                } catch (e) {
        ...
        'check-version': async (args = []) => {
            ...
            if (!installedVersion || compareVersions(currentVersion, installedVersion) !== 0) {
                ...
                // Run pending state migrations after version update
                try {
        ...
        'update': async (args = []) => {
            ...
            // Migrate stale global config: strip --trust from cu.implementFlag.
            // cursor-agent tightened flag validation — --trust is now only valid
            // with --print (headless). Existing users who ran `aigon config create`
            // before this change have it hardcoded in ~/.aigon/config.json.
            try {
                const { loadGlobalConfig: _loadGC, saveGlobalConfig: _saveGC } = require('../config');
                const gc = _loadGC();
                const cuFlag = gc?.agents?.cu?.implementFlag;
                if (typeof cuFlag === 'string' && cuFlag.includes('--trust')) {
                    gc.agents.cu.implementFlag = cuFlag.split(/\s+/).filter(t => t !== '--trust').join(' ');
                    _saveGC(gc);
                    console.log('⚙️  Migrated: removed --trust from cu implementFlag (no longer valid in interactive mode)');
                }
            } catch (_) { /* non-fatal */ }
            ...
            try {
                ...
                const aigonPaths = 'docs/ AGENTS.md .agents/ .claude/ .cursor/ .codex/ .gemini/ .aigon/';
                try {
                ...
                // 8. Auto-run doctor --fix to bootstrap workflow state for any entities
                // that predate the workflow engine. Silent if nothing to fix.
                try {
                    const worktreeMarker = path.join(process.cwd(), '.aigon', 'worktree.json');
                    if (!fs.existsSync(worktreeMarker)) {
                        const { features: missingF, research: missingR } = findEntitiesMissingWorkflowState(process.cwd());
                        if (missingF.length + missingR.length > 0) {
                            let bootstrapped = 0;
                            bootstrapped += bootstrapMissingWorkflowSnapshots(process.cwd(), missingF, 'feature');
                            bootstrapped += bootstrapMissingWorkflowSnapshots(process.cwd(), missingR, 'research');
                            console.log(`\n🔧 Doctor: bootstrapped workflow state for ${bootstrapped} entit${bootstrapped === 1 ? 'y' : 'ies'}`);
                        }
                    }
                } catch (_) { /* non-fatal */ }
        ...
        'seed-reset': (args) => {
            ...
            function nukePhase(inv) {
                ...
                // 1f. Remove directories with retry for ENOTEMPTY / EBUSY
                ...
                // 1g. Remove Gemini tmp dirs whose .project_root points into this repo
                try {
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'check-version', 'update', 'project-context', 'doctor', 'seed-reset', 'trust-worktree'];
