        ...
        'seed-reset': (args) => {
            ...
            function provisionPhase() {
                ...
                try {
                    ...
                    try {
                        const pendingFiles = execSync('git status --porcelain', { cwd: repoPath, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] })
                            .trim()
                            .split('\n')
                            .filter(Boolean)
                        ...
                        if (pendingFiles.length === 0) throw new Error('nothing to commit');
                        execSync(`git add -- ${pendingFiles.map(file => JSON.stringify(file)).join(' ')}`, { cwd: repoPath, stdio: 'pipe' });
                        execSync(
