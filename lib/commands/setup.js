...
module.exports = function setupCommands(ctx) {
    ...
    const commands = {
        ...
        'doctor': async (args) => {
            ...
            const doAuthOnly = args.includes('--auth-only');
            ...
            function printAgentAuthSection() {
                ...
                if (doFix && unauthenticated.length > 0) {
                    console.log('\n  Auth fixes');
            ...
            function printTerminalAppSection() {
                ...
                report.issue({
                    ...
                    fix: { label: adapter.appInstallHint || `Install ${adapter.displayName || terminalConfig}`, autoFixable: false },
                });
                if (doFix) {
            ...
            function printMultiRepoVersionSweep() {
                ...
                stale.forEach(r => report.issue({
                    ...
                    fix: { label: `aigon apply in ${r.path}`, command: `cd ${r.path} && aigon apply`, autoFixable: true },
                }));
            ...
            function printDashboardHealthSection() {
                ...
                const runtimePath = path.join(os.homedir(), '.aigon', 'dashboard-runtime.json');
                let runtime = null;
                try {
            ...
            printAgentAuthSection();
            if (doAuthOnly) return;
            printModelHealthSection();
            printTerminalAppSection();
            printMultiRepoVersionSweep();
            printTmuxLivenessSection();
            await printDashboardHealthSection();
            printShellPathSection();
            printGitIdentitySection();
            ...
            // F550: Mirror legacy state-reconciliation issues into the report
            // and render the unified triage digest. Section routing keeps the
            // spec promise that install-manifest-corrupt, stale-drive-branch,
            // and similar are first-class digest entries (not buried under
            // "state-reconciliation").
