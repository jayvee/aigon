...
function bootstrapMissingWorkflowSnapshots(repoPath, entities, entityType) {
    ...
    for (const entity of entities) {
        ...
        const snapshot = {
            entityType,
            [entityType === 'research' ? 'researchId' : 'featureId']: entity.id,
            lifecycle,
            mode: null,
            winnerAgentId: null,
            agents: {},
                ...
                uniqueAgents.forEach(agentKey => {
                    ...
                    if (extras.settings && extras.settings.enabled) {
                        ...
                        if (extras.settings.hooks) {
                            ...
                            // `which aigon` under fnm returns an ephemeral multishell
                            // symlink at ~/.local/state/fnm_multishells/<shell-id>/bin/aigon
                            // that becomes stale once that shell exits — the path persists
                            // but hooks invoking it fail with "No such file or directory"
                            // in every new session. Prefer stable global paths first; only
                            // fall back to `which` when none exist, and reject fnm shell
                            // paths so we don't re-install a broken hook.
                            ...
                            if (aigonAbsPath === 'aigon') {
                                try {
                                    ...
                                    if (resolved && !resolved.includes('/fnm_multishells/')) {
                                        aigonAbsPath = resolved;
                                    }
                                } catch (_) {}
                            }
                        ...
                        if (hookContent.hooks) {
                            ...
                            Object.entries(hookContent.hooks).forEach(([event, hookConfigs]) => {
                                ...
                                hookConfigs.forEach(hookConfig => {
                                    ...
                                    if (!alreadyExists) {
                                        // Resolve aigon to absolute path. Prefer stable
                                        // globals and reject fnm_multishells paths that
                                        // go stale when the spawning shell exits. See
                                        // note upstream (settings.json hooks) for details.
                                        let resolvedAigon = 'aigon';
                                        for (const candidate of ['/opt/homebrew/bin/aigon', '/usr/local/bin/aigon']) {
                                            if (fs.existsSync(candidate)) {
                                                resolvedAigon = candidate;
                                                break;
                                            }
                                        }
                                        if (resolvedAigon === 'aigon') {
                                            try {
                                                const rp = require('child_process').execFileSync('which', ['aigon'], { encoding: 'utf8' }).trim();
                                                if (rp && !rp.includes('/fnm_multishells/')) resolvedAigon = rp;
                                            } catch (_) {}
                                        }
