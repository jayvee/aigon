...
module.exports = function setupCommands(ctx) {
    ...
    const commands = {
        ...
        'remove': async (args) => {
            ...
            if (files.length === 0) {
                console.log('ℹ️  Install manifest is empty — nothing to remove.');
                return;
            }

            const modified = installManifestLib.getModifiedFiles(manifest, removeRepoRoot);
            const modifiedPaths = new Set(modified.map(m => m.path));

            console.log('\nFiles that would be removed:');
            files.forEach(entry => {
                const exists = require('fs').existsSync(path.join(removeRepoRoot, entry.path));
            ...
            if (dryRun) {
                // Show what registry entry would be removed
                try {
                    const { readConductorReposFromGlobalConfig } = require('../dashboard-server');
                    ...
                    if (repos.map(r => path.resolve(r)).includes(abs)) {
                        console.log(`\n  Registry entry that would be removed: ${abs}`);
                    }
                } catch (_) {}
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'uninstall', 'remove', 'setup', 'global-setup', 'check-prerequisites', 'check-version', 'apply', 'update', 'project-context', 'installed-notice', 'doctor', 'seed-reset', 'trust-worktree'];
