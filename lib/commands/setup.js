    ...
    const commands = {
        ...
        'doctor': async (args) => {
            ...
            // Check: slug-only specs outside inbox (definitionally invalid)
            {
                ...
                const researchRoot = path.join(process.cwd(), 'docs', 'specs', 'research-topics');
                // 05-done slug-only specs are pre-ID historical artifacts — leave them in place.
                const NON_INBOX_FOLDERS = ['02-backlog', '03-in-progress', '04-in-evaluation', '06-paused'];
