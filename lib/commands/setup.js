    ...
    const commands = {
        ...
        'install-agent': async (args) => {
            ...
            try {
                ...
                // Load (or create) install manifest. Check for user-modified files
                // before overwriting — warn and prompt unless --force or non-interactive.
                const repoRoot = process.cwd();
                const installForce = args.includes('--force');
                const installNonInteractive = !!process.env.AIGON_NONINTERACTIVE || !process.stdin.isTTY;
                let installManifest;
                try {
                ...
                if (installManifest) {
                    const modified = installManifestLib.getModifiedFiles(installManifest, repoRoot);
                    if (modified.length > 0 && !installForce) {
                        ...
                        if (installNonInteractive) {
                            console.warn('Proceeding with overwrite (non-interactive mode).');
                        } else {
                            const { createInterface } = require('readline');
                            const rl = createInterface({ input: process.stdin, output: process.stdout });
                            const answer = await new Promise(resolve => rl.question('Proceed with overwrite? [y/N] ', resolve));
                            rl.close();
            ...
            try {
                ...
                if (fs.existsSync(docsTemplateDirUpdate)) {
                    ...
                    docFilesUpdate.forEach(file => {
                        const docContent = readTemplate(`docs/${file}`);
                        const docPath = path.join(process.cwd(), '.aigon', 'docs', file);
                        const docStatus = safeWriteWithStatus(docPath, docContent);
        ...
        'doctor': async (args) => {
            ...
            if (doReapOrphans) {
                ...
                const minAgeSecs = minAgeArg ? parseInt(minAgeArg.split('=')[1], 10) : 3600;
                ...
                // Recursively collect descendants of orphan wrapper shells
                ...
                const wrapperPids = [...orphanPids].filter(pid => reasons.get(pid).startsWith('wrapper'));
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'uninstall', 'setup', 'global-setup', 'check-prerequisites', 'check-version', 'update', 'project-context', 'doctor', 'seed-reset', 'trust-worktree'];
