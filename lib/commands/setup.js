...
module.exports = function setupCommands(ctx) {
    ...
    const commands = {
        ...
        'seed-reset': (args) => {
            ...
            // 8. Ensure seed repos always ignore local env files
            try {
                const envResult = ensureEnvLocalGitignore(repoPath);
                if (envResult.addedEntries.length > 0) {
                    console.log(`   ✓ Updated .gitignore with: ${envResult.addedEntries.join(', ')}`);
                    try {
                        execSync('git add .gitignore && git commit -m "chore: ignore local env files"', {
                            cwd: repoPath,
                            stdio: 'pipe',
                        });
                        console.log('   ✓ Committed .gitignore env ignore update');
                    } catch (e) {
                        // Keep reset usable even if commit can't be created
                        console.log('   ⚠️  Updated .gitignore but could not auto-commit; commit manually if desired');
                    }
                }
            } catch (e) {
                console.log(`   ⚠️  Could not enforce .env local gitignore: ${e.message}`);
            }
            ...
            // 11. Remove worktree permissions/trust from Claude settings
