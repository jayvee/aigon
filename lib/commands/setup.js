    ...
    function printFirstTimeNextStepHint() {
        try {
            ...
            if (onPath.length > 0) {
                ...
                console.log('\nNext: aigon install-agent <agent-id>  (e.g. cc, gg, cx, cu)');
            }
        } catch (_) {
            console.log('\nNext: aigon install-agent <agent-id>  (e.g. cc, gg, cx, cu)');
        }
    ...
    const commands = {
        ...
        'doctor': async (args) => {
            ...
            // Check: partial bootstrap (.aigon/ exists but spec folder structure missing)
            {
                ...
                const specsInboxExists = fs.existsSync(path.join(doctorCwd, 'docs', 'specs', 'features', '01-inbox'));
        ...
        'remove': async (args) => {
            ...
            // Refuse to run in a worktree — would affect the main repo
            ...
            // Bail out cleanly if there's nothing Aigon-ish in this repo.
            ...
            // Manifest is optional. A repo bootstrapped by `aigon apply` but
            // never followed by `install-agent` has no manifest — `remove`
            // must still deregister and optionally purge.
            let manifest = null;
            try {
        ...
        // First-time clone of a known seed repo. The wizard's "seed clone" step
        // calls the equivalent flow inline; this is the standalone path for
        // users who skipped that step or want to add another seed later.
        // Use `aigon seed-reset <name>` to wipe and re-clone an existing one.
        'install-seed': async (args) => {
            ...
            // Strip origin so the local sandbox doesn't accidentally push back
            // to the seed repo.
...
function createSetupCommands(overrides = {}) {
    ...
    const names = ['init', 'install-agent', 'uninstall', 'remove', 'setup', 'global-setup', 'check-prerequisites', 'check-version', 'apply', 'update', 'project-context', 'installed-notice', 'doctor', 'install-seed', 'seed-reset', 'trust-worktree'];
