...
const { runFeatureSetSpecReview } = require('../feature-set-spec-review');
const { runFeatureSetSpecRevise } = require('../feature-set-spec-revise');
...
function handleShow(args, options) {
    ...
    const isPaused = autoState && (autoState.status === 'paused-on-failure' || autoState.status === 'paused-on-quota');
    if (isPaused) {
        const pausedAt = autoState.endedAt ? ` at ${autoState.endedAt.slice(0, 19).replace('T', ' ')}` : '';
        if (autoState.status === 'paused-on-quota') {
            const pausedFeature = autoState.pausedFeature || autoState.currentFeature;
            const pauseLabel = pausedFeature ? ` (feature #${parseInt(pausedFeature, 10) || pausedFeature})` : '';
            console.log(`⏸  Set paused on quota${pauseLabel}${pausedAt}`);
            console.log(`   Re-run review or pick new agents, then: aigon set-autonomous-resume ${slug}`);
            console.log(`   Or restart with new pairing: aigon set-autonomous-start ${slug} <agents...> --review-agent=<agent>\n`);
        } else {
...
function handleSetPrioritise(rawArgs) {
    const slug = rawArgs && rawArgs[0];
    if (!slug) {
        console.error('Usage: aigon set-prioritise <slug>');
        process.exitCode = 1;
        return;
    }
    if (!featureSets.isValidSetSlug(slug)) {
    ...
    const actionCtx = buildActionContext(git);
    try {
        const result = assertActionAllowed('set-prioritise', actionCtx);
        if (result && result.delegate) {
            console.log('📡 Delegating \'set-prioritise\' to main repo...');
            runDelegatedAigonCommand(result.delegate, 'set-prioritise', [slug]);
            return;
        }
    } catch (e) {
    ...
    let order;
    try {
        order = featureSets.computeSetInboxPrioritisationOrder(slug);
    } catch (e) {
        console.error(`❌ ${e.message}`);
        process.exitCode = 1;
        return;
    ...
    console.log(`\n📋 Prioritising ${order.length} inbox feature(s) in set "${slug}" (dependency order):\n   ${order.join(' → ')}\n`);

    for (const featureSlug of order) {
        const r = spawnSync(process.execPath, [cliPath, 'feature-prioritise', featureSlug, '--from-set-prioritise'], {
            cwd: process.cwd(),
            stdio: 'inherit',
            env: process.env,
        });
        if (r.error) {
            console.error(`❌ Failed to run feature-prioritise: ${r.error.message}`);
            process.exitCode = 1;
            return;
        }
        if (r.status !== 0) {
    ...
    console.log(`\n✅ Set "${slug}" — all inbox members prioritised.`);
}
...
function printHelp() {
    ...
    console.log('  aigon set-prioritise <slug>');
    console.log('  aigon feature-set-spec-review <slug> [--agent=<agent>] [--no-launch]');
    console.log('  aigon feature-set-spec-revise <slug> [--agent=<agent>] [--no-launch]');
    console.log('  aigon set-autonomous-start <slug> [agents...] [--mode=sequential] [--review-agent=<agent>] [--models=<csv>] [--efforts=<csv>] [--stop-after=close]');
...
function createSetCommands() {
    return {
        ...
        'set-prioritise': (rawArgs) => handleSetPrioritise(rawArgs || []),
        'feature-set-spec-review': (rawArgs) => runFeatureSetSpecReview(rawArgs || []),
        'feature-set-spec-revise': (rawArgs) => runFeatureSetSpecRevise(rawArgs || []),
