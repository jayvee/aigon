...
/**
 * Refresh the generated lifecycle symlink view after a storage operation that
 * may have changed local snapshots (F669). A no-op under the legacy layout.
 * Warns with a repair command on failure but never rolls back canonical state.
 */
function refreshLifecycleView(repoPath) {
  try {
    const specView = require('../spec-view');
    const result = specView.refreshView(repoPath);
    if (!result.skipped && result.blocked.length > 0) {
      console.log(`⚠️  Lifecycle view: ${result.blocked.length} collision(s) left untouched — run: aigon spec-view refresh`);
    }
  } catch (error) {
    console.log(`⚠️  Lifecycle view refresh failed (${error.message}) — run: aigon spec-view refresh`);
  }
}
  ...
  return {
    storage: async (args) => {
      ...
      if (sub === 'doctor') {
        ...
        if (fix) refreshLifecycleView(repoPath);
      ...
      if (sub === 'lease-renew') {
        const entityType = options._[1];
        const entityId = options._[2];
        if (!entityType || !entityId) {
          console.error('Usage: aigon storage lease-renew <feature|research> <id> [--role=impl|research|close]');
          process.exitCode = 1;
          return;
        }
        let storage;
        ...
        const role = getOptionValue(options, 'role')
          || (entityType === 'research' ? 'research' : 'impl');
        const store = createSpecStore({ repoPath, storage });
        const ref = { entityType, entityId: String(entityId) };
        try {
          const result = await store.renewLease(ref, {
            role,
          ...
          console.warn(`⚠️  Lease renew failed: ${error.message}`);
        }
        return;
      }

      console.error('Usage: aigon storage <sync|status|doctor|report|convert|lease-renew> [--fix] [--json] [--backend=git-branch] [--remote=origin] [--branch=aigon-state] [--keep-refs] [--dry-run] [--role=impl]');
      process.exitCode = 1;
    },
  };
};
