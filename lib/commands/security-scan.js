...
function runAigonCommand(repoPath, args) {
    const result = spawnSync(process.execPath, [path.join(repoPath, 'aigon-cli.js'), ...args], {
        cwd: repoPath,
        encoding: 'utf8',
        stdio: 'pipe',
        env: {
...
function findExistingFeatureForFinding(repoPath, finding, slug) {
...
async function createFeatureForFinding(repoPath, finding, reportPath) {
    try {
        const slug = buildFeatureSlug(finding);
        const existing = findExistingFeatureForFinding(repoPath, finding, slug);
        if (existing) {
            return {
                skipped: true,
        ...
        runAigonCommand(repoPath, ['feature-create', displayName, '--description', summary]);
        runAigonCommand(repoPath, ['feature-prioritise', slug]);

        const created = findExistingFeatureForFinding(repoPath, finding, slug);
        if (!created) {
            throw new Error(`feature was created and prioritised, but could not be re-located for slug '${slug}'`);
        }

        return { created: true, id: created.id, filePath: created.filePath };
    } catch (err) {
        ...
        if (highFindings.length > 0) {
            ...
            for (const f of highFindings) {
                ...
                if (result.created) {
                    ...
                    console.log(`   ⏭️  skipped (already filed as feature-${result.existingId || '??'})`);
