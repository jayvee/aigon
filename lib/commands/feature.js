...
const workflowClose = require('../workflow-close');
const workflowStart = require('../workflow-start');
const workflowEval = require('../workflow-eval');
const workflowPause = require('../workflow-pause');
...
module.exports = function featureCommands(ctx) {
    ...
    function collectIncompleteFeatureEvalAgents({ featureNum, worktrees = [] }) {
        ...
        worktrees.forEach(w => {
            ...
            const agentStatus = manifest.readAgentStatus(featureNum, w.agent);
    ...
    const cmds = {
        ...
        'feature-pause': async (args) => {
            ...
            const usePauseEngine = workflowPause.isPauseEngineEnabled(loadProjectConfig());

            if (usePauseEngine) {
                // Check if engine state exists for this feature
                const repoPath = process.cwd();
                const wfState = await workflowPause.getWorkflowPauseState(repoPath, paddedId);
                const engineOpts = args.includes('--reclaim')
                    ? { claimTimeoutMs: 1 }
                    : {};

                if (wfState.state !== null) {
                    // Engine state exists — route through workflow-core
                    if (wfState.state === 'paused' && !wfState.hasPendingEffects) {
                        console.log(`✅ Feature ${paddedId} is already paused (workflow-core).`);
                        return;
                    }
                    ...
                    const result = await workflowPause.runWorkflowPause(repoPath, paddedId, {
                        specFromPath,
                        specToPath,
                    ...
                    console.log(`✅ Paused: ${found.file} -> 06-paused/ (workflow-core)`);
                    try {
                        runGit(`git add docs/specs/features/`);
                ...
                // No engine state — fall through to legacy path
            }

            // Legacy path: use state machine
            const m = require('../manifest').readManifest(paddedId);
            if (!m || !m.specPath) return console.error(`❌ No spec found for feature ${paddedId}`);
            const specFile = path.basename(m.specPath);
            const sourcePath = m.specPath;
            ctx.stateMachine.requestTransition(paddedId, 'feature-pause', { actor: 'cli/feature-pause' });
            const targetDir = path.join(PATHS.features.root, '06-paused');
            fs.mkdirSync(targetDir, { recursive: true });
            fs.renameSync(sourcePath, path.join(targetDir, specFile));
            ctx.stateMachine.completePendingOp(paddedId, 'move-spec');
            console.log(`✅ Paused: ${specFile} -> 06-paused/`);
            try {
                runGit(`git add docs/specs/features/`);
                runGit(`git commit -m "chore: pause feature ${paddedId}"`);
        ...
        'feature-resume': async (args) => {
            ...
            if (usePauseEngine) {
                ...
                if (wfState.state !== null) {
                    // Engine state exists — route through workflow-core
                    if (wfState.state === 'implementing' && !wfState.hasPendingEffects) {
                        console.log(`✅ Feature ${paddedId} is already implementing (workflow-core).`);
                        return;
                    }
                    ...
                    const result = await workflowPause.runWorkflowResume(repoPath, paddedId, {
                        specFromPath,
                        specToPath,
                        engineOptions: engineOpts,
                    });
                    ...
                    console.log(`✅ Resumed: ${found.file} -> 03-in-progress/ (workflow-core)`);
                    try {
                        runGit(`git add docs/specs/features/`);
            ...
            const specFile = path.basename(m.specPath);
            const sourcePath = m.specPath;
            ctx.stateMachine.requestTransition(paddedId, 'feature-resume', { actor: 'cli/feature-resume' });
            const targetDir = path.join(PATHS.features.root, '02-backlog');
            fs.renameSync(sourcePath, path.join(targetDir, specFile));
            ctx.stateMachine.completePendingOp(paddedId, 'move-spec');
            console.log(`✅ Resumed: ${specFile} -> 02-backlog/`);
            try {
                runGit(`git add docs/specs/features/`);
                runGit(`git commit -m "chore: resume feature ${paddedId}"`);
            } catch (e) { console.warn(`⚠️  Could not commit: ${e.message}`); }
        ...
        'feature-start': async (args) => {
            ...
            // Check if workflow-core engine is enabled for feature-start
            const useStartEngine = workflowStart.isStartEngineEnabled(loadProjectConfig());

            // Outbox replay check: if a previous feature-start was interrupted, resume
            // with the remaining pending ops instead of calling requestTransition again.
            const existingM = manifest.readManifest(featureId) || {};
            ...
            if (found.folder === '03-in-progress' && !isStartReplay) {
                // Engine resume: check for interrupted workflow-core start
                if (useStartEngine) {
                    console.log(`📋 Workflow-core feature ${featureId} is already in progress; preserving existing engine agents.`);
                } else if (agentIds.length > 0) {
                    // Always merge agents into manifest — even if some already exist.
                    // This ensures agents are never silently dropped (RC4 fix).
                    const currentAgents = Array.isArray(existingM.agents) ? existingM.agents : [];
                    const merged = [...new Set([...currentAgents, ...agentIds])];
                    const isNew = merged.length > currentAgents.length;
                    manifest.writeManifest(featureId, { agents: merged }, { type: 'agent-backfill', actor: 'cli/feature-start' });
                    if (isNew) {
                        console.log(`📋 Registered agents [${merged.join(', ')}] for feature ${featureId}`);
                    } else {
                        console.log(`📋 Agents [${merged.join(', ')}] already registered for feature ${featureId}`);
                    }
                } else {
            ...
            if (useStartEngine) {
                ...
                // Engine handles: state creation, spec move effect, log init effect
                const repoPath = process.cwd();
                ...
                try {
                    const result = await workflowStart.runWorkflowStart(repoPath, featureId, {
                        agents: agentIds,
                        specFromPath,
                        specToPath,
                        num,
                    ...
                    // Write legacy manifest for backward compatibility (agent status files)
                    const engineAgents = Object.keys(result.snapshot?.agents || {});
                    workflowStart.writeLegacyManifest(manifest, featureId, engineAgents);
                } catch (e) {
                    process.exitCode = 1;
                ...
                if (!isStartReplay) {
                    try {
                        ctx.stateMachine.requestTransition(featureId, 'feature-start', {
                            agents: agentIds,
                            actor: 'cli/feature-start',
                        });
                    } catch (e) {
                        process.exitCode = 1;
                    ...
                    console.log(`📋 Resuming interrupted feature-start (${existingM.pending.length} ops remaining)...`);
                }
                ...
                if (found) {
                    moveFile(found, '03-in-progress', null, { actor: 'cli/feature-start' });
                ...
                ctx.stateMachine.completePendingOp(featureId, 'move-spec');
            ...
            if (mode === 'drive') {
                ...
                if (!useStartEngine) {
                    // Legacy path: create log file manually + complete pending op
                    ...
                    ctx.stateMachine.completePendingOp(featureId, 'init-log');
                }
                // Engine path: log already created by init_log effect
                ...
                agentIds.forEach(agentId => {
                    ...
                    // Mark worktree + log ops complete (legacy path only — engine tracks its own effects)
                    if (!useStartEngine) {
                        ctx.stateMachine.completePendingOp(featureId, `create-worktree-${agentId}`);
                        ctx.stateMachine.completePendingOp(featureId, `init-log-${agentId}`);
                    }
                });
        ...
        'feature-do': (args) => {
            ...
            if (!found) return printError('feature', id, `Run 'aigon feature-start ${id}' first.`);
        ...
        'feature-eval': async (args) => {
            ...
            // --- State transition: workflow-core engine or legacy path ---
            const evalFeatureId = name;
            const useEvalEngine = workflowEval.isEvalEngineEnabled(loadProjectConfig());
            const repoPath = process.cwd();
            let engineHandledTransition = false;

            // Check if this feature has engine state (started via workflow-core)
            if (useEvalEngine && await workflowEval.hasEngineState(repoPath, evalFeatureId)) {
                // Workflow-core path: synthesize agent-ready signals from legacy
                // manifest status files, then request eval through the engine.
                // The XState allAgentsReady guard enforces that all agents submitted.
                await workflowEval.synthesizeAgentReadySignals(repoPath, evalFeatureId, manifest);
                ...
                const evalResult = await workflowEval.runWorkflowEval(repoPath, evalFeatureId, {
                    specFromPath: specFrom ? specFrom.fullPath : null,
                    specToPath: specTo,
                });

                if (evalResult.kind === 'guard_failed') {
                    console.error(`❌ ${evalResult.message}`);
                    return;
                }
                ...
                if (evalResult.kind === 'busy') {
                    console.error(`⏳ ${evalResult.message}`);
                    return;
                }

                console.log(`🔧 Eval transition completed via workflow-core engine`);
                engineHandledTransition = true;
            }

            if (!engineHandledTransition) {
                // Legacy path: outbox replay + requestTransition
                const existingEvalM = manifest.readManifest(evalFeatureId) || {};
                const isEvalReplay = existingEvalM.stage === 'in-evaluation' &&
                    Array.isArray(existingEvalM.pending) && existingEvalM.pending.length > 0;

                if (!isEvalReplay) {
                    try {
                        ctx.stateMachine.requestTransition(evalFeatureId, 'feature-eval', { actor: 'cli/feature-eval' });
                    } catch (e) {
                        console.error(`❌ ${e.message}`);
                        return;
                    }
                } else {
                    console.log(`📋 Resuming interrupted feature-eval (${existingEvalM.pending.length} ops remaining)...`);
                }
            }

            // Find the feature and move spec to in-evaluation (idempotent: no-op if already there)
            // Engine path may have already moved the spec via effects, but this is idempotent.
            const evalSessionForMove = detectActiveAgentSession();
            let found = findFile(PATHS.features, name, ['03-in-progress']);
            if (found) {
                moveFile(found, '04-in-evaluation', null, { actor: 'cli/feature-eval', agentId: evalSessionForMove.detected ? evalSessionForMove.agentId : undefined });
            ...
            if (!engineHandledTransition) {
                ctx.stateMachine.completePendingOp(evalFeatureId, 'move-spec');
            }
...
`;
        ...
        'feature-close': async (args) => {
            const actionCtx = buildActionContext(ctx.git);
            try {
                const result = assertActionAllowed('feature-close', actionCtx);
                if (result && result.delegate) {
            ...
            // Resume check: detect interrupted close from either system
            const closeFeatureId = name;
            const useCloseEngine = workflowClose.isCloseEngineEnabled(loadProjectConfig());

            if (useCloseEngine) {
                // Workflow-core resume: check if a close is already in progress
                const wfState = await workflowClose.getWorkflowCloseState(process.cwd(), closeFeatureId);
                if (wfState.state === 'done') {
                    console.log(`✅ Feature ${closeFeatureId} is already closed (workflow-core).`);
                    return;
                }
                if (wfState.inProgress) {
                    console.log(`📋 Resuming interrupted feature-close (workflow-core)...`);
                    const result = await workflowClose.resumeClose(process.cwd(), closeFeatureId);
                    if (result.kind === 'busy') {
                        return console.error(`⏳ ${result.message}`);
                    }
                ...
                // Legacy outbox replay: if a previous feature-close was interrupted after the merge
                // but before the spec move, replay the remaining pending ops and return.
                const existingCloseM = manifest.readManifest(closeFeatureId) || {};
                const isCloseReplay = existingCloseM.stage === 'done' &&
                    Array.isArray(existingCloseM.pending) && existingCloseM.pending.length > 0;

                if (isCloseReplay) {
                    console.log(`📋 Resuming interrupted feature-close (${existingCloseM.pending.length} ops remaining)...`);
                    if (existingCloseM.pending.includes('move-spec')) {
                        const specAlreadyDone = findFile(PATHS.features, name, ['05-done']);
                        if (!specAlreadyDone) {
                            const specToMove = findFile(PATHS.features, name, ['04-in-evaluation', '03-in-progress']);
                            if (specToMove) {
                                moveFile(specToMove, '05-done', null, { actor: 'cli/feature-close' });
                                console.log(`📋 Moved spec to done`);
                            }
                        }
                        ctx.stateMachine.completePendingOp(closeFeatureId, 'move-spec');
                    }
                    return;
                }
            }
            ...
            // Determine all participating agents from manifest
            const closeManifest = manifest.readManifest(num) || {};
            const allAgents = Array.isArray(closeManifest.agents) && closeManifest.agents.length > 0
                ? closeManifest.agents
                : (agentId ? [agentId] : ['solo']);
            ...
            // --- State transition + spec move ---
            let workflowBootstrapped = false;
            if (useCloseEngine) {
                // Workflow-core path: bootstrap state, then close with durable effects
                const repoPath = process.cwd();
                const existingManifest = manifest.readManifest(closeFeatureId) || {};
                const isWorktree = !!(worktreePath);
                // manifest.agents is an array of IDs (e.g. ["cc"]), not an object
                const rawAgents = existingManifest.agents;
                const agents = Array.isArray(rawAgents) && rawAgents.length > 0
                    ? rawAgents
                    : [agentId || 'cc'];
                // In Drive mode (no agentId arg), winner is the sole agent
                const winnerId = agentId || (agents.length === 1 ? agents[0] : 'solo');
                const agentsObj = Object.fromEntries(agents.map(a => [a, {}]));
                const mode = workflowClose.resolveMode(
                    { agents: agentsObj },
                    isWorktree,
                );

                try {
                    await workflowClose.bootstrapWorkflowState(repoPath, closeFeatureId, {
                        mode,
                        agents,
                        winnerId,
                    });
                    console.log(`🔧 Workflow-core state bootstrapped for feature ${closeFeatureId}`);
                    workflowBootstrapped = true;
                } catch (e) {
                    console.error(`❌ Workflow-core bootstrap failed: ${e.message}`);
                    console.error(`   Falling back to legacy close path.`);
                }

                if (workflowBootstrapped) {
                    const specFromPath = findFile(PATHS.features, name, ['04-in-evaluation', '03-in-progress']);
                    const featuresRoot = typeof PATHS.features === 'string' ? PATHS.features : PATHS.features.root;
                    const specToDir = path.join(featuresRoot, '05-done');
                    const specToPath = specFromPath
                        ? path.join(specToDir, specFromPath.file)
                        : null;

                    if (specFromPath && specToPath) {
                        const engineOpts = args.includes('--reclaim')
                            ? { claimTimeoutMs: 1 } // Force immediate reclaim
                            : {};
                        const closeResult = await workflowClose.runWorkflowClose(repoPath, closeFeatureId, {
                            specFromPath: specFromPath.fullPath,
                            specToPath,
                            winnerId,
                            engineOptions: engineOpts,
                        });
                        ...
                        if (closeResult.kind === 'busy') {
                            return console.error(`⏳ ${closeResult.message}`);
                        }
                        if (closeResult.kind === 'error') {
                            return console.error(`❌ ${closeResult.message}`);
                        }
                        console.log(`📋 Moved spec to done (workflow-core)`);
                    }

                    // Also update legacy manifest for backward compat
                    try {
                        ctx.stateMachine.requestTransition(num, 'feature-close', { actor: 'cli/feature-close' });
                    } catch (e) {
                        if (!e.message.includes("Invalid transition")) throw e;
                    }
                    ctx.stateMachine.completePendingOp(num, 'move-spec');
                    try {
                        manifest.writeManifest(num, { winner: winnerId }, { type: 'winner-recorded', actor: 'cli/feature-close' });
                        console.log(`🏆 Winner recorded: ${winnerId}`);
                    } catch (e) { /* non-fatal */ }
                }
            }

            if (!useCloseEngine || !workflowBootstrapped) {
                // Legacy path: requestTransition + moveFile + completePendingOp
                try {
                    ctx.stateMachine.requestTransition(num, 'feature-close', { actor: 'cli/feature-close' });
                } catch (e) {
                    if (!e.message.includes("Invalid transition")) throw e;
                ...
                if (postMergeFound) {
                    const closeSession = detectActiveAgentSession();
                    moveFile(postMergeFound, '05-done', null, { actor: 'cli/feature-close', agentId: closeSession.detected ? closeSession.agentId : undefined });
                    console.log(`📋 Moved spec to done`);
                }
                ...
                try {
                    const winnerId = agentId || 'solo';
                    manifest.writeManifest(num, { winner: winnerId }, { type: 'winner-recorded', actor: 'cli/feature-close' });
                    console.log(`🏆 Winner recorded in manifest: ${winnerId}`);
                } catch (e) { /* non-fatal */ }
            }
        ...
        'feature-reset': (args) => {
            ...
            let stateCount = 0;
            const stateDir = manifest.getStateDir();
        ...
        'feature-autopilot': (args) => {
            ...
            if (subcommand === 'status') {
                ...
                featureIds.forEach(fid => {
                    ...
                    fWorktrees.forEach(wt => {
                        ...
                        let updatedStr = '';
                        try {
                            const agentState = manifest.readAgentStatus(wt.featureId, wt.agent);
                            if (agentState) {
            ...
            existingWorktrees.forEach(wt => {
                ...
                try {
                    const agentState = manifest.readAgentStatus(featureNum, wt.agent);
            ...
            try {
                ...
                if (!tmuxList.error && tmuxList.status === 0) {
                    ...
                    sessions.forEach(sessionName => {
                        if (!u.matchTmuxSessionByEntityId(sessionName, id)) return;
...
function createFeatureCommands(overrides = {}) {
    ...
    const names = [
        ...
        'feature-do', 'feature-validate', 'feature-eval', 'feature-review',
