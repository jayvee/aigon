    ...
    const cmds = {
        ...
        'feature-start': async (args) => {
            ...
            if (found.folder === '03-in-progress' && !isStartReplay) {
                ...
                if (useStartEngine) {
                    const wfState = workflowStart.getWorkflowStartState(process.cwd(), featureId);
                    // getWorkflowStartState is async but we need sync context here;
                    // use the sync check: if engine state exists and is implementing with pending effects, resume
                    // For now, delegate to the engine's own resume detection in runWorkflowStart
                }
            ...
            if (useStartEngine) {
                ...
                try {
                    ...
                    workflowStart.writeLegacyManifest(manifest, featureId, agentIds);
                } catch (e) {
                    process.exitCode = 1;
