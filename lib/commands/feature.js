...
async function persistAndRunEffects(repoPath, featureId, effects, options = {}) {
    effects = specLifecycle.filterMoveSpecEffects(repoPath, effects);
    if (effects.length > 0) {
...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        ...
        'feature-create': (args) => {
            ...
            // First positional is always the name. Flags (--agent, --description, --quick)
            ...
            const flags = {};
            const positional = [];
            const knownFlags = new Set(['--agent', '--description', '--set', '--quick']);
            ...
            if (agentId && built && built.filePath) {
                ...
                require('../entity-context').bindOriginNativeSession(process.cwd(), 'feature', built.entityId, foundSession);
            }
        },
        ...
        'feature-now': (args) => withActionDelegate('feature-now', args, ctx, async () => {
            const featureNow = require('../feature-now');
            await featureNow.run(args, {
                ctx, PATHS, findFile, getNextId, runPreHook, runPostHook, readTemplate, runGit, loadProjectConfig, u, workflow: wf,
            });
        }),
        ...
        'feature-status': (args) => {
            ...
            if (deepStatus.implementedBy) {
                const { formatLeaseHolderLabel } = require('../feature-status');
                const impl = deepStatus.implementedBy;
                const source = impl.source === 'stats'
                    ? ' from close stats'
                    : (impl.source === 'event-history' ? ' from lease history' : '');
                line('Implemented by', `${formatLeaseHolderLabel(impl)}${source}`);
            }
