...
`;
        ...
        'feature-close': async (args) => {
            ...
            // Phase 3.7: Remote branch gate (optional, GitHub-only v1)
            if (!isResume) {
                const projectConfig = loadProjectConfig(target.repoPath);
                if (projectConfig.honourRemoteBranchGate) {
                    const { checkGitHubGate } = require('../remote-gate-github');
                    const defaultBranch = getDefaultBranch();
                    console.log(`\n🔒 Remote branch gate enabled — checking GitHub PR status for ${target.branchName}...`);
                    const gateResult = checkGitHubGate(target.branchName, defaultBranch);
                    if (!gateResult.ok) {
                        process.exitCode = 1;
                        return console.error(`❌ Remote gate blocked: ${gateResult.message}`);
                    }
                    console.log(`✅ Remote gate passed — PR #${gateResult.prNumber} is open and mergeable (${gateResult.url})`);
                }
            }
        ...
        'feature-push': (args) => {
            ...
            if (!id) {
                process.exitCode = 1;
                return console.error("Usage: aigon feature-push <ID> [agent]\n\nPushes the resolved feature branch to origin with upstream tracking.\nDoes not alter workflow state, move specs, or merge anything.\n\nExample: aigon feature-push 55\n         aigon feature-push 55 gg");
            }
            ...
            if (!target.ok) {
                if (target.delegate) {
                    ...
                    const argsStr = args.map(a => JSON.stringify(a)).join(' ');
                    execSync(`node "${path.join(target.delegate, 'aigon-cli.js')}" feature-push ${argsStr}`, { stdio: 'inherit', cwd: target.delegate });
                    return;
                }
                process.exitCode = 1;
        ...
        'feature-autonomous-start': async (args) => {
            ...
            if (subcommand === '__run-loop') {
                ...
                while (true) {
                    ...
                    if (effectiveStopAfter === 'implement' && allReady) {
                        ...
                        const mySession = findAutoSessionNameByFeatureId(featureNum);
                        ...
                        if (evalStarted && effectiveStopAfter === 'close') {
                            ...
                            if (closeTriggered) {
                                if (next && (next.currentSpecState === 'done' || next.lifecycle === 'done')) {
                                    ...
                                    // Kill this AutoConductor session so the dashboard stops showing "Running autonomously"
                                    const mySession = findAutoSessionNameByFeatureId(featureNum);
                                    if (mySession) {
                                        spawnSync('tmux', ['kill-session', '-t', mySession], { stdio: 'ignore' });
                                    }
                                    return;
                                }
            ...
            if (!featureId || featureId.startsWith('-')) {
                console.error('Usage: aigon feature-autonomous-start <feature-id> <agents...> [--eval-agent=<agent>] [--review-agent=<agent>] [--stop-after=implement|eval|review|close]');
            ...
            const reviewAgentOption = String(getOptionValue(options, 'review-agent') || '').trim() || null;
