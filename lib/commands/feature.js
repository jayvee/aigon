...
`;
        ...
        'feature-close': async (args) => {
            ...
            // Phase 3.6: Auto-ack any outstanding spec-reviews so Done never carries pending state.
            // Policy is auto-ack-with-warning per feature 283 — reviewer suggestions were already
            // folded into the spec if they mattered; leaving them pending blocks no one.
            if (!isResume) {
                const specReviewState = require('../spec-review-state');
                const state = specReviewState.readState(target.repoPath, 'feature', target.name);
                const { pendingCount, pendingAgents } = specReviewState.derivePending(state);
                if (pendingCount > 0) {
                    console.warn(`⚠️  Auto-acknowledging ${pendingCount} outstanding spec-review(s) from ${pendingAgents.join(', ')} before close.`);
                    specReviewState.recordAck(target.repoPath, 'feature', target.name, {
                        ackedBy: 'feature-close-auto',
                        notes: 'Auto-ack on feature-close — outstanding reviews resolved by closing the feature.',
                    });
                }
            }
