...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        ...
        'feature-pause': (args) => {
            ...
            // ID-based: use state machine (transition BEFORE moving file to avoid auto-reconcile race)
        ...
        'feature-start': (args) => {
            ...
            if (found.folder === '03-in-progress' && !isStartReplay) {
                // If agents are being provided but manifest has none, update the manifest
                // (happens when auto-reconcile moved to in-progress before start ran)
