        ...
        'feature-autopilot': async (args) => {
            ...
            if (subcommand === 'status') {
                ...
                featureIds.forEach(fid => {
                    ...
                    const engineSnapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', fid);
                    fWorktrees.forEach(wt => {
                        let status = 'unknown';
                        let updatedStr = '';

                        if (engineSnapshot && engineSnapshot.agents && engineSnapshot.agents[wt.agent]) {
                            const agentState = engineSnapshot.agents[wt.agent];
                            status = agentState.status || 'unknown';
                            if (agentState.lastHeartbeatAt) {
                                const d = new Date(agentState.lastHeartbeatAt);
                                const diffMs = Date.now() - d.getTime();
                                const diffMin = Math.floor(diffMs / 60000);
                                updatedStr = diffMin < 1 ? 'just now' : `${diffMin}m ago`;
                            }
                        } else if (!engineSnapshot) {
                            // Legacy fallback when no workflow snapshot exists yet.
                            try {
                                const agentState = readAgentStatus(wt.featureId, wt.agent, 'feature', { mainRepoPath: mainRepo });
                                if (agentState) {
                                    status = agentState.status || 'unknown';
            ...
            const spawnedAgents = [];
            const spawnSnapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
            existingWorktrees.forEach(wt => {
                const sessionName = buildTmuxSessionName(wt.featureId, wt.agent, { desc: wt.desc, role: 'do' });

                // Check if agent is already ready in engine snapshot (don't re-spawn).
                if (spawnSnapshot && spawnSnapshot.agents && spawnSnapshot.agents[wt.agent]?.status === 'ready') {
                    console.log(`   ✓ ${wt.agent} — already submitted, skipping`);
                    spawnedAgents.push({ ...wt, alreadySubmitted: true });
                    return;
                }

                // Legacy fallback for pre-engine entities.
                if (!spawnSnapshot) {
                    try {
                        const agentState = readAgentStatus(featureNum, wt.agent, 'feature', { mainRepoPath: mainRepo });
                ...
                const autonomousDoCmd = `aigon feature-do ${featureNum} --autonomous --auto-submit --agent=${wt.agent} --max-iterations=${maxIterations}`;
                const cmd = buildAgentCommand({
                    ...wt,
                    featureId: featureNum,
                    repoPath: mainRepo,
                    rawCommand: autonomousDoCmd,
                });
                try {
            ...
            if (allAlreadySubmitted) {
                ...
                try {
                    while (!interrupted) {
                        ...
                        const statusRows = [];
                        const monitorSnapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);

                        spawnedAgents.forEach(wt => {
                            let status = 'unknown';
                            let updatedStr = '';

                            if (monitorSnapshot && monitorSnapshot.agents && monitorSnapshot.agents[wt.agent]) {
                                const agentState = monitorSnapshot.agents[wt.agent];
                                status = agentState.status || 'unknown';
                                if (agentState.lastHeartbeatAt) {
                                    const d = new Date(agentState.lastHeartbeatAt);
                                    const diffMs = Date.now() - d.getTime();
                            ...
                            } else if (!monitorSnapshot) {
                                // Legacy fallback while snapshot is not initialized yet.
                                try {
                                    const legacyState = readAgentStatus(featureNum, wt.agent, 'feature', { mainRepoPath: mainRepo });
                                    if (legacyState) {
                                        status = legacyState.status || 'unknown';
                                        if (legacyState.updatedAt) {
                                            const d = new Date(legacyState.updatedAt);
                                            const diffMs = Date.now() - d.getTime();
                                            const diffMin = Math.floor(diffMs / 60000);
                                            updatedStr = diffMin < 1 ? 'just now' : `${diffMin}m ago`;
                                        }
                            ...
                            if (monitorSnapshot && (!monitorSnapshot.agents || Object.keys(monitorSnapshot.agents).length === 0)) {
                                // Snapshot exists but agents are not initialized yet.
                                allSubmitted = false;
                            } else if (status !== 'ready' && status !== 'submitted') {
                                allSubmitted = false;
                            }
            ...
            if (autoEval) {
                ...
                const readySnapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
                ...
                while (Date.now() - startedAt < evalConfirmTimeoutMs) {
                    const evalSnapshot = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
