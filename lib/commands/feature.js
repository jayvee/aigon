        ...
        'feature-autonomous-start': async (args) => {
            ...
            if (subcommand === '__run-loop') {
                ...
                while (true) {
                    ...
                    console.log(`[${new Date().toLocaleTimeString()}] state=${stage} agents=[${agentStatuses.join(', ')}] allReady=${allReady} evalTriggered=${evalTriggered} closeTriggered=${closeTriggered} isFleet=${isFleet} effectiveStopAfter=${effectiveStopAfter}`);
                    ...
                    if (effectiveStopAfter === 'implement' && allReady) {
                        ...
                        console.log('   (session closing in 10s)');
                    ...
                    if (!isFleet) {
                        ...
                        // Phase 2 (close mode only): wait for eval agent to exit, parse winner, close
                        if (evalStarted && effectiveStopAfter === 'close') {
                            evalClosePolls++;
                            const next = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
                            const evalSessionRunning = expectedEvalSessionName ? tmuxSessionExists(expectedEvalSessionName) : false;
                            console.log(`  [eval-close ${evalClosePolls}/${MAX_EVAL_CLOSE_POLLS}] state=${next?.currentSpecState} evalSession running=${evalSessionRunning} closeTriggered=${closeTriggered}`);

                            // Already closed — just wait for state to confirm
                            if (closeTriggered) {
                                if (next && (next.currentSpecState === 'done' || next.lifecycle === 'done')) {
                                ...
                                // Eval agent has exited — parse the winner from the eval file
                                ...
                                closeTriggered = true;
                                evalClosePolls = 0; // reset poll counter for post-close confirmation
                            }
            ...
            console.log(`   feature: ${featureNum} | agents: ${agentIds.join(', ')} | stop-after: ${effectiveStopAfter}${isFleet && evalAgent ? ` | eval-agent: ${evalAgent}` : ''}`);
