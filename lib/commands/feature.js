                            ...
                            if (reviewStarted && effectiveStopAfter === 'close' && !closeTriggered && !feedbackInjected) {
                                ...
                                if (reviewCompleted) {
                                    ...
                                    if (tmuxSessionExists(implSessionName)) {
                                        ...
                                        // Slash-command agents (cc/gg/cu) have a runnable prefix
                                        // ending in `:` or `-`. Codex uses a `$aigon-` naming
                                        // convention for skill files that is NOT a runnable
                                        // command — cx must be told to follow the skill, not
                                        // "run" a phantom CLI. Otherwise cx literally tries
                                        // `aigon feature-review-check 273` which errors out.
                                        const hasRunnablePrefix = /[:/\-]$/.test(cmdPrefix) && !cmdPrefix.startsWith('$');
                                        const invocation = hasRunnablePrefix
                                            ? `run ${cmdPrefix}feature-review-check ${featureNum}`
                                            : `follow your feature-review-check skill/prompt for feature ${featureNum}`;
                                        const feedbackPrompt = `The review is complete. Please ${invocation} to check and address the review feedback, then signal completion with: aigon agent-status feedback-addressed`;
