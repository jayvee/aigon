        ...
        'feature-close': (args) => {
            ...
            try {
                if (logPathToUpdate) {
                    ...
                    // Ensure we always emit at least one normalized session record.
                    // For non-CC agents there may be no transcript adapter yet.
                    if (!telemetryData && winAgent) {
                        telemetry.writeAgentFallbackSession(num, winAgent, {
                            repoPath,
                            source: 'feature-close-fallback',
                            model: `${winAgent}-cli`,
                            endAt: new Date().toISOString(),
                            sessionId: `${branchName}-${Date.now()}`,
                        });
                    }
                }
            } catch (e) {
