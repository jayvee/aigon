                    ...
                    if (!isFleet) {
                        ...
                        if (shouldTriggerEval && allReady && !evalTriggered) {
                            ...
                            const evalCommand = buildAgentCommand({
                                ...
                                desc: featureDesc,
                                repoPath: mainRepo,
                                autonomous: effectiveStopAfter === 'close'
                            }, 'evaluate');
                        ...
                        // Phase 2 (close mode only): wait for eval to finish and state to reach 'done'
                        if (evalStarted && effectiveStopAfter === 'close') {
                            evalClosePolls++;
                            const next = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
                            const evalSessionRunning = expectedEvalSessionName ? tmuxSessionExists(expectedEvalSessionName) : false;
                            console.log(`  [eval-close ${evalClosePolls}/${MAX_EVAL_CLOSE_POLLS}] state=${next?.currentSpecState} evalSession running=${evalSessionRunning}`);
                            if (next && (next.currentSpecState === 'done' || next.lifecycle === 'done')) {
                            ...
                            if (!evalSessionRunning) {
                                console.error(`❌ Eval agent session ended but feature state is '${next?.currentSpecState}', not 'done'.`);
