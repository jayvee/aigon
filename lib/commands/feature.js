...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        ...
        'feature-start': async (args) => {
            ...
            const agentIds = explicitAgentIds.length > 0
                ? explicitAgentIds
                : (requestedWorkflow ? requestedWorkflow.agents.slice() : []);
...
`;
        ...
        'feature-autonomous-start': async (args) => {
            ...
            const stopAfter = String(
                optionWasProvided(options, 'stop-after')
                    ? getOptionValue(options, 'stop-after')
                    : ((requestedWorkflow && requestedWorkflow.stopAfter) || 'close')
            ).trim();
            if (!['implement', 'eval', 'review', 'close'].includes(stopAfter)) {
            ...
            const evalAgentOption = String(
                optionWasProvided(options, 'eval-agent')
                    ? (getOptionValue(options, 'eval-agent') || '')
                    : ((requestedWorkflow && requestedWorkflow.evalAgent) || '')
            ).trim() || null;
            const reviewAgentOption = String(
                optionWasProvided(options, 'review-agent')
                    ? (getOptionValue(options, 'review-agent') || '')
                    : ((requestedWorkflow && requestedWorkflow.reviewAgent) || '')
            ).trim() || null;
