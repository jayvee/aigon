...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        'feature-create': (args) => {
            ...
            // First positional is always the name. Everything after is the
            // description, either as free words (joined with spaces) or via
            // an explicit --description flag. The old flag form stays
            // supported for backward compatibility.
            const name = args[0];
            let description = '';
            const descIdx = args.indexOf('--description');
            if (descIdx >= 0) {
                description = args.slice(descIdx + 1).join(' ').trim();
                // Warn if there are stranded args between the name and --description
                if (descIdx > 1) {
                    const stranded = args.slice(1, descIdx).join(' ');
                ...
                // No flag — everything after the name becomes the description
                description = args.slice(1).join(' ').trim();
            }
            entity.entityCreate(entity.FEATURE_DEF, name, ctx, { description });
        },
        ...
        'feature-autonomous-start': async (args) => {
            ...
            if (subcommand === '__run-loop') {
                ...
                while (true) {
                    ...
                    console.log(`[${new Date().toLocaleTimeString()}] state=${stage} agents=[${agentStatuses.join(', ')}] allReady=${allReady} evalTriggered=${evalTriggered} reviewTriggered=${reviewTriggered} closeTriggered=${closeTriggered} isFleet=${isFleet} effectiveStopAfter=${effectiveStopAfter}`);
                    ...
                    if (!isFleet) {
                        ...
                        if (reviewStarted && effectiveStopAfter === 'close' && !closeTriggered) {
                            reviewClosePolls++;
                            const reviewSessionRunning = expectedReviewSessionName ? tmuxSessionExists(expectedReviewSessionName) : false;
                            ...
                            if (reviewCompleted) {
                                console.log('✅ Review completion signaled. Proceeding to close.');
                        ...
                        // Step 4: Close (either no review, or review completed)
