                ...
                while (true) {
                    ...
                    if (!isFleet) {
                        ...
                        if (evalTriggered) {
                            ...
                            console.log(`  [post-trigger ${postTriggerPolls}/${MAX_POST_TRIGGER_POLLS}] state=${next?.currentSpecState} evalSession=${expectedEvalSessionName} running=${evalSessionRunning}`);
                            ...
                            if (postTriggerPolls >= MAX_POST_TRIGGER_POLLS) {
                                if (next && next.currentSpecState === 'evaluating' && !evalSessionRunning) {
                                    console.error(`❌ State reached 'evaluating' but eval session is no longer running. Eval may have completed or crashed.`);
                                } else {
                                    console.error(`❌ State did not reach 'evaluating' after ${MAX_POST_TRIGGER_POLLS} polls. state=${next?.currentSpecState} evalSessionRunning=${evalSessionRunning}`);
                                }
                                process.exitCode = 1;
                                return;
                            }
