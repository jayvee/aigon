...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        ...
        'feature-pause': (args) => {
            ...
            // ID-based: use state machine (transition BEFORE moving file — manifest is source of truth)
            const paddedId = String(id).padStart(2, '0');
            const manifest = require('../manifest').readManifest(paddedId);
            if (!manifest || !manifest.specPath) return console.error(`❌ No spec found for feature ${paddedId}`);
            const specFile = path.basename(manifest.specPath);
            const sourcePath = manifest.specPath;
        ...
        'feature-resume': (args) => {
            ...
            // ID-based: use state machine (transition BEFORE moving file)
            const paddedId = String(id).padStart(2, '0');
            const manifest = require('../manifest').readManifest(paddedId);
            if (!manifest || !manifest.specPath) return console.error(`❌ No spec found for feature ${paddedId}`);
            const specFile = path.basename(manifest.specPath);
