...
function optionWasProvided(options, key) {
    return Object.prototype.hasOwnProperty.call(options, key);
}

function resolveRequestedWorkflow(repoPath, slug) {
    if (!slug) return null;
...
module.exports = function featureCommands(ctx) {
    ...
    function encodeWorkflowStages(stages) {
        return Buffer.from(JSON.stringify(stages || []), 'utf8').toString('base64');
    }

    function decodeWorkflowStages(encoded) {
        const raw = Buffer.from(String(encoded || ''), 'base64').toString('utf8');
        const parsed = JSON.parse(raw);
        if (!Array.isArray(parsed)) {
            throw new Error('workflow stages payload must decode to an array');
        }
        return parsed.map(stage => workflowDefinitions.normalizeWorkflowStage(stage));
    }

    function replaceImplementStageAgents(stages, agentIds) {
    ...
    function countReviewCompletions(reviewState, agentId) {
        return ((reviewState && reviewState.history) || []).filter(entry => entry.agent === agentId).length;
    }

    function findWorkflowEvalWinner(mainRepoPath, featureNum, candidateAgents) {
        const numStr = String(featureNum).padStart(2, '0');
        const evalFilePath = path.join(mainRepoPath, 'docs', 'specs', 'features', 'evaluations', `feature-${numStr}-eval.md`);
        if (!fs.existsSync(evalFilePath)) return null;
        try {
            const evalContent = fs.readFileSync(evalFilePath, 'utf8');
            const agentPattern = candidateAgents.length > 0 ? candidateAgents.join('|') : '[a-z]{2,4}';
            const match = evalContent.match(new RegExp(`\\*\\*Winner:\\*\\*\\s+(${agentPattern})\\b`, 'i'));
    ...
    async function runStageBasedAutoConductor({
        featureNum,
        mainRepo,
        stages,
        loopSessionName,
        ...
        const implementStage = stages.find(stage => stage.type === 'implement');
        const implementAgents = implementStage ? implementStage.agents.slice() : [];
        const lastStage = stages[stages.length - 1] || null;
        const isFleet = implementAgents.length > 1;
        const MAX_REVIEW_POLLS = 120;
        const MAX_COUNTER_REVIEW_POLLS = 120;
        const MAX_EVAL_POLLS = 120;
        const MAX_CLOSE_POLLS = 10;
        ...
        const updateAutoState = (patch) => persistFeatureAutoState(mainRepo, featureNum, {
            ...
            agents: implementAgents,
            stopAfter: lastStage ? lastStage.type : 'close',
            stages,
        ...
        console.log(`   stages: ${stages.map(stage => `${stage.type}${stage.agents.length ? `(${stage.agents.join(',')})` : ''}`).join(' -> ')}`);
        console.log(`   poll interval: ${pollSeconds}s`);
        console.log('');
        updateAutoState({ status: 'running', running: true, currentStageIndex: 0, currentStageType: stages[0] ? stages[0].type : null });

        let stageIndex = 0;
        let featureDesc = featureNum;
        let evalWinner = null;
        ...
        try {
            while (true) {
                ...
                if (snapshot.specPath) {
                    const match = path.basename(snapshot.specPath).match(/^feature-\d+-(.+)\.md$/);
                    if (match) featureDesc = match[1];
                }

                const workflowState = snapshot.currentSpecState || snapshot.lifecycle || 'unknown';
                const agentStatuses = implementAgents.map(agent => {
                    const status = snapshot.agents && snapshot.agents[agent] ? snapshot.agents[agent].status : null;
                    return `${agent}=${status}`;
                });
                const allReady = implementAgents.every(agent => {
                    const status = snapshot.agents && snapshot.agents[agent] ? snapshot.agents[agent].status : null;
                ...
                updateAutoState({
                    workflowState,
                    allReady,
                    currentStageIndex: currentStage ? stageIndex : null,
                    currentStageType: currentStage ? currentStage.type : null,
                    winner: evalWinner,
                });

                console.log(`[${new Date().toLocaleTimeString()}] state=${workflowState} stage=${currentStage ? `${stageIndex + 1}/${stages.length}:${currentStage.type}` : 'done'} agents=[${agentStatuses.join(', ')}] allReady=${allReady} winner=${evalWinner || 'none'}`);

                if (!currentStage) {
                    finishAuto('completed', { reason: 'workflow-finished', workflowState });
                    stopAutoSession();
                    return;
                }

                const stateForStage = stageState.get(stageIndex) || {};

                if (currentStage.type === 'implement') {
                    if (allReady) {
                        console.log('✅ Implement stage complete.');
                        stageIndex += 1;
                        if (stageIndex >= stages.length) {
                            finishAuto('stopped', { reason: 'stop-after-implement', workflowState });
                            stopAutoSession();
                            return;
                        }
                        continue;
                    ...
                    const reviewAgent = currentStage.agents[0];
                    if (!stateForStage.started) {
                        ...
                        if (!reviewWorktree || !reviewWorktree.path) {
                            ...
                            finishAuto('failed', { reason: 'review-worktree-missing', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                        ...
                        if (tmuxSessionExists(reviewSessionName)) {
                            ...
                            if (!tmuxSessionExists(reviewSessionName)) {
                                ...
                                finishAuto('failed', { reason: 'review-session-start-failed', workflowState });
                                process.exitCode = 1;
                                return;
                            }
                        ...
                        stageState.set(stageIndex, {
                            started: true,
                            sessionName: reviewSessionName,
                            baselineCount: countReviewCompletions(reviewState, reviewAgent),
                            polls: 0,
                        });
                    } else {
                        stateForStage.polls = (stateForStage.polls || 0) + 1;
                        const reviewState = featureReviewState.readReviewState(mainRepo, featureNum);
                        const reviewCount = countReviewCompletions(reviewState, reviewAgent);
                        const reviewSessionRunning = stateForStage.sessionName ? tmuxSessionExists(stateForStage.sessionName) : false;
                        console.log(`  [review ${stateForStage.polls}/${MAX_REVIEW_POLLS}] reviewSession running=${reviewSessionRunning} completions=${reviewCount - (stateForStage.baselineCount || 0)}`);

                        if (reviewCount > (stateForStage.baselineCount || 0)) {
                            console.log('✅ Review stage complete.');
                            stageIndex += 1;
                            if (stageIndex >= stages.length) {
                                finishAuto('stopped', { reason: 'stop-after-review', workflowState });
                                stopAutoSession();
                                return;
                            }
                            continue;
                        ...
                        if (!reviewSessionRunning) {
                            ...
                            finishAuto('failed', { reason: 'review-exited-without-signal', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                        if (stateForStage.polls >= MAX_REVIEW_POLLS) {
                            console.error(`❌ Review session timed out after ${MAX_REVIEW_POLLS} polls.`);
                            finishAuto('failed', { reason: 'review-timeout', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                    }
                } else if (currentStage.type === 'counter-review') {
                    const implAgent = implementAgents[0];
                    if (!stateForStage.injected) {
                        ...
                        const implAgentConfig = loadAgentConfig(implAgent);
                        const cmdPrefix = implAgentConfig?.placeholders?.CMD_PREFIX || '/aigon:';
                        const reviewCheckCommand = `${cmdPrefix}feature-review-check ${featureNum}`;
                        const feedbackPrompt = `The review is complete. Please run ${reviewCheckCommand} to check and address the review feedback, then signal completion with: aigon agent-status feedback-addressed`;
                        ...
                        if (tmuxSessionExists(implSessionName)) {
                            ...
                            spawnSync('tmux', ['send-keys', '-t', implSessionName, 'Enter'], { stdio: 'ignore' });
                            console.log(`📝 Feedback prompt injected into session: ${implSessionName}`);
                        } else {
                            console.log(`⚠️  Implementation session not found (${implSessionName}). Treating counter-review as complete.`);
                        }

                        stageState.set(stageIndex, {
                            injected: true,
                            implSessionName,
                            baselineUpdatedAt: implStatus && implStatus.updatedAt ? implStatus.updatedAt : null,
                            polls: 0,
                        });
                    } else {
                        stateForStage.polls = (stateForStage.polls || 0) + 1;
                        const implSessionRunning = stateForStage.implSessionName ? tmuxSessionExists(stateForStage.implSessionName) : false;
                        const implStatus = readAgentStatus(featureNum, implAgent, 'feature', { mainRepoPath: mainRepo });
                        const signaled = !!(implStatus
                            && implStatus.status === 'feedback-addressed'
                            && implStatus.updatedAt
                            && implStatus.updatedAt !== stateForStage.baselineUpdatedAt);
                        console.log(`  [counter-review ${stateForStage.polls}/${MAX_COUNTER_REVIEW_POLLS}] implSession running=${implSessionRunning} status=${implStatus && implStatus.status} signaled=${signaled}`);

                        if (signaled || !implSessionRunning) {
                            console.log(signaled ? '✅ Counter-review stage complete.' : '⚠️  Implementation session exited. Treating counter-review as complete.');
                            stageIndex += 1;
                            if (stageIndex >= stages.length) {
                                finishAuto('stopped', { reason: 'stop-after-counter-review', workflowState });
                                stopAutoSession();
                                return;
                            }
                            continue;
                        }
                        if (stateForStage.polls >= MAX_COUNTER_REVIEW_POLLS) {
                            console.error(`❌ Counter-review timed out after ${MAX_COUNTER_REVIEW_POLLS} polls.`);
                            finishAuto('failed', { reason: 'counter-review-timeout', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                    }
                } else if (currentStage.type === 'eval') {
                    const evalAgent = currentStage.agents[0];
                    if (!stateForStage.triggered) {
                        const evalSessionName = buildTmuxSessionName(featureNum, evalAgent, {
                            repo: path.basename(mainRepo),
                            desc: featureDesc,
                            entityType: 'f',
                            role: 'eval'
                        ...
                        stageState.set(stageIndex, {
                            triggered: true,
                            sessionName: evalSessionName,
                            polls: 0,
                        });
                    } else {
                        stateForStage.polls = (stateForStage.polls || 0) + 1;
                        const evalSessionRunning = stateForStage.sessionName ? tmuxSessionExists(stateForStage.sessionName) : false;
                        evalWinner = findWorkflowEvalWinner(mainRepo, featureNum, implementAgents);
                        console.log(`  [eval ${stateForStage.polls}/${MAX_EVAL_POLLS}] evalSession running=${evalSessionRunning} winner=${evalWinner || 'none'}`);

                        if (evalWinner) {
                            console.log(`✅ Eval stage complete. Winner: ${evalWinner}`);
                            stageIndex += 1;
                            if (stageIndex >= stages.length) {
                                finishAuto('stopped', { reason: 'stop-after-eval', workflowState, winner: evalWinner });
                                stopAutoSession();
                                return;
                            }
                            continue;
                        }
                        if (!evalSessionRunning) {
                            console.error(`❌ Eval agent exited but no winner was written for feature ${featureNum}.`);
                            finishAuto('failed', { reason: 'eval-exited-without-winner', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                        if (stateForStage.polls >= MAX_EVAL_POLLS) {
                            console.error(`❌ Eval stage timed out after ${MAX_EVAL_POLLS} polls.`);
                            finishAuto('failed', { reason: 'eval-timeout', workflowState });
                            process.exitCode = 1;
                            return;
                        }
                    }
                } else if (currentStage.type === 'close') {
                    if (!stateForStage.triggered) {
                        const closeArgs = ['feature-close', featureNum];
                        if (isFleet) {
                            if (!evalWinner) {
                                console.error('❌ Close stage requires an eval winner in fleet mode.');
                                finishAuto('failed', { reason: 'close-missing-winner', workflowState });
                                process.exitCode = 1;
                                return;
                            }
                            closeArgs.push(evalWinner);
                        }
                        console.log(`🚀 Triggering: aigon ${closeArgs.join(' ')}`);
                        const closeResult = runAigonCliCommand(mainRepo, closeArgs);
                        ...
                        if (closeResult.error || closeResult.status !== 0) {
                            ...
                            finishAuto('failed', { reason: 'feature-close-failed', workflowState, winner: evalWinner });
                            process.exitCode = 1;
                            return;
                        }
                        stageState.set(stageIndex, { triggered: true, polls: 0 });
                    } else {
                        stateForStage.polls = (stateForStage.polls || 0) + 1;
                        ...
                        if (next && (next.currentSpecState === 'done' || next.lifecycle === 'done')) {
                            ...
                            finishAuto('completed', { reason: 'feature-closed', workflowState: next.currentSpecState || next.lifecycle || 'done', winner: evalWinner });
                            stopAutoSession();
                            return;
                        }
                        if (stateForStage.polls >= MAX_CLOSE_POLLS) {
                            console.error(`❌ feature-close ran but state never reached 'done' after ${MAX_CLOSE_POLLS} polls.`);
                            finishAuto('failed', { reason: 'feature-close-state-timeout', workflowState: next?.currentSpecState || next?.lifecycle || workflowState, winner: evalWinner });
                            process.exitCode = 1;
                            return;
                        }
                    }
    ...
    const cmds = {
        ...
        'feature-start': async (args) => {
            ...
            const explicitAgentIds = options._.slice(1);
            const workflowSlug = String(getOptionValue(options, 'workflow') || '').trim();
            let requestedWorkflow = null;
            try {
                requestedWorkflow = resolveRequestedWorkflow(process.cwd(), workflowSlug);
            } catch (e) {
                process.exitCode = 1;
                return console.error(`❌ ${e.message}`);
            }
            const workflowArgs = workflowDefinitions.applyWorkflowDefinition(requestedWorkflow, {
                agents: explicitAgentIds,
            });
            const agentIds = workflowArgs.agents;
            ...
            if (requestedWorkflow && explicitAgentIds.length === 0) {
                console.log(`ℹ️  Using workflow "${requestedWorkflow.slug}" for agents: ${workflowArgs.agents.join(', ')}`);
            }
...
`;
        ...
        'feature-close': async (args) => {
            ...
            try {
                const { loadProxyRegistry, isProcessAlive: proxyIsProcessAlive, getAigonServerAppId } = require('../proxy');
                ...
                close.restartServerIfLibChanged(
                    { preMergeBaseRef: mergeResult.preMergeBaseRef },
                    {
                        ...
                        getServerRegistryEntry: () => {
                            const reg = loadProxyRegistry();
                            const apps = reg[serverAppId] || {};
                            return apps[''] || null;
                        },
        ...
        'feature-reset': async (args) => {
            ...
            // 0. Kill any running sessions FIRST (agent procs, tmux, preview, Warp).
            //    Without this, autonomous runs and tmux sessions linger after reset.
            try { closeSessionsForFeature(id); } catch (e) {
                console.warn(`   ⚠️  sessions-close step failed: ${e.message}`);
            }
            ...
            // 4. Remove workflow-core engine state (.aigon/workflows/features/<id>/).
            //    Run LAST so a failure here does not block the legacy cleanup above.
            //    Engine state is keyed on the padded id; see lib/workflow-core/paths.js.
            let engineRemoved = false;
            try {
                const result = await wf.resetFeature(process.cwd(), paddedId);
                if (result.removed) {
            ...
            // 5. Clean stale dev-proxy entries
        ...
        'feature-autonomous-start': async (args) => {
            ...
            if (subcommand === '__run-loop') {
                ...
                const stagesEncoded = String(getOptionValue(options, 'stages-b64') || '').trim() || null;
                ...
                if (stagesEncoded) {
                    let stages = null;
                    try {
                        stages = decodeWorkflowStages(stagesEncoded);
                    } catch (error) {
                        console.error(`❌ Invalid --stages-b64 payload: ${error.message}`);
                        process.exitCode = 1;
                        return;
                    }
                    await runStageBasedAutoConductor({
                        featureNum,
                        mainRepo,
                        stages,
                        loopSessionName,
            ...
            if (!featureId || featureId.startsWith('-')) {
                console.error('Usage: aigon feature-autonomous-start <feature-id> [agents...] [--workflow <slug>] [--eval-agent=<agent>] [--review-agent=<agent>] [--stop-after=implement|eval|review|close]');
            ...
            const workflowArgs = workflowDefinitions.applyWorkflowDefinition(requestedWorkflow, {
                ...
                evalAgent: optionWasProvided(options, 'eval-agent')
                    ? String(getOptionValue(options, 'eval-agent') || '').trim() || null
                    : undefined,
                reviewAgent: optionWasProvided(options, 'review-agent')
                    ? String(getOptionValue(options, 'review-agent') || '').trim() || null
                    : undefined,
                stopAfter: optionWasProvided(options, 'stop-after')
                    ? String(getOptionValue(options, 'stop-after') || '').trim()
                    : undefined,
            });
            let agentIds = workflowArgs.agents;
            let workflowStages = workflowArgs.version === 2 && Array.isArray(workflowArgs.stages)
                ? workflowArgs.stages.map(stage => ({
                    type: stage.type,
                    agents: Array.isArray(stage.agents) ? stage.agents.slice() : [],
                }))
                : null;
            ...
            const stopAfter = String(workflowArgs.stopAfter || 'close').trim();
            if (!['implement', 'eval', 'review', 'close'].includes(stopAfter)) {
                console.error('❌ --stop-after must be one of: implement, eval, review, close');
                process.exitCode = 1;
                return;
            }
            const evalAgentOption = String(workflowArgs.evalAgent || '').trim() || null;
            const reviewAgentOption = String(workflowArgs.reviewAgent || '').trim() || null;
            ...
            if (workflowStages) {
                const stageAgentSet = [...new Set(workflowStages.flatMap(stage => stage.agents || []))];
                const invalidStageAgents = stageAgentSet.filter(agent => !availableAgents.includes(agent));
                if (invalidStageAgents.length > 0) {
                    console.error(`❌ Unknown workflow stage agent(s): ${invalidStageAgents.join(', ')}. Available: ${availableAgents.join(', ')}`);
                    process.exitCode = 1;
                    return;
                }
            }
            ...
            if (existingWorktrees.length > 0) {
                if (requestedWorkflow && positionalAgents.length === 0) {
                    console.log(`ℹ️  Feature ${featureNum} already has worktrees; using existing agents instead of workflow "${requestedWorkflow.slug}".`);
                }
                agentIds = existingWorktrees.map(wt => wt.agent);
                if (workflowStages) {
                    workflowStages = replaceImplementStageAgents(workflowStages, agentIds);
                }
            ...
            if (workflowStages) {
                ...
                const loopCmdParts = [
                    ...
                    `--stages-b64=${encodeWorkflowStages(workflowStages)}`,
                ...
                console.log(`   feature: ${featureNum} | agents: ${agentIds.join(', ')} | stages: ${workflowStages.map(stage => `${stage.type}${stage.agents.length ? `(${stage.agents.join(',')})` : ''}`).join(' -> ')}`);
                ...
                persistFeatureAutoState(mainRepo, featureNum, {
                    ...
                    agents: agentIds,
                    stopAfter: workflowStages[workflowStages.length - 1] ? workflowStages[workflowStages.length - 1].type : 'close',
                    stages: workflowStages,
                    mode: agentIds.length > 1 ? 'fleet' : 'solo_worktree',
                });
...
function createFeatureCommands(overrides = {}) {
    ...
    const names = [
        ...
        'feature-close', 'feature-push', 'feature-cleanup', 'feature-reset', 'feature-backfill-timestamps',
