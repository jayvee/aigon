    ...
    const cmds = {
        ...
        'feature-start': (args) => {
            ...
            if (found.folder === '03-in-progress' && !isStartReplay) {
                // If agents are being provided but manifest has none, backfill them
                // (e.g. feature was moved to in-progress manually or by doctor before start ran)
                if (agentIds.length > 0 && (!existingM.agents || existingM.agents.length === 0)) {
                    manifest.writeManifest(featureId, { agents: agentIds }, { type: 'agent-backfill', actor: 'cli/feature-start' });
                    console.log(`📋 Registered agents [${agentIds.join(', ')}] for feature ${featureId}`);
                } else {
