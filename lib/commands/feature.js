...
module.exports = function featureCommands(ctx) {
    ...
    const cmds = {
        'feature-create': (args) => {
            // Guard: must be on default branch
            ...
            createSpecFile({
                ...
                build: (value) => {
                    ...
                    // Inject description into the Summary section if provided
                    if (description) {
        ...
        'feature-prioritise': (args) => {
            ...
            const moved = moveFile(found, '02-backlog', newName, { actor: 'cli/feature-prioritise' });
            ...
            // Rebuild manifest deterministically at prioritisation time.
            // This clears stale coordinator state for reused IDs (e.g. old stage=done files)
            // and guarantees feature-start sees a fresh backlog manifest.
            manifest.writeManifest(paddedId, {
                ...
                winner: null,
                pending: [],
            }, { type: 'transition:feature-prioritise', actor: 'cli/feature-prioritise' });
...
`;
        ...
        'feature-close': (args) => {
            ...
            // Outbox replay check: if a previous feature-close was interrupted after the merge
            ...
            // Compute and persist git signals + token telemetry in the feature log frontmatter.
            let logPathToUpdate = null;
            let gitSignals = null;
            try {
                ...
                const winningAgent = agentId || (mode === 'multi-agent'
                    ? ((branchName.match(/^feature-\d+-([a-z]{2})-/) || [])[1] || null)
                    : null);
                const expectedLogName = winningAgent
                    ? `feature-${num}-${winningAgent}-${desc}-log.md`
                    : `feature-${num}-${desc}-log.md`;
                const logsDir = path.join(PATHS.features.root, 'logs');
                const expectedLogPath = path.join(logsDir, expectedLogName);

                if (fs.existsSync(expectedLogPath)) {
                    logPathToUpdate = expectedLogPath;
                } else if (fs.existsSync(logsDir)) {
                    const fallbackPrefix = winningAgent
                        ? `feature-${num}-${winningAgent}-`
                        : `feature-${num}-`;
                    const fallback = fs.readdirSync(logsDir)
                        .filter(file => file.startsWith(fallbackPrefix) && file.endsWith('-log.md'))
                        .sort()[0];
                    if (fallback) logPathToUpdate = path.join(logsDir, fallback);
                }

                if (logPathToUpdate) {
                    upsertLogFrontmatterScalars(logPathToUpdate, gitSignals);
                    console.log(`📊 Recorded git signals in log frontmatter: ${path.basename(logPathToUpdate)}`);
                } else {
                    console.warn(`⚠️  Could not locate implementation log for feature ${num}; skipped git signal write.`);
                }
            } catch (e) {
                console.warn(`⚠️  Could not compute/write git signals: ${e.message}`);
            }

            // Capture token/cost telemetry from Claude Code transcripts
            try {
                if (logPathToUpdate) {
                    const repoPath = process.cwd();
                    const winAgent = agentId || (mode === 'multi-agent'
                        ? ((branchName.match(/^feature-\d+-([a-z]{2})-/) || [])[1] || null)
                        : null);
                    const telemetryData = telemetry.captureFeatureTelemetry(num, desc, {
                        agentId: winAgent,
                        repoPath,
                    ...
                    if (telemetryData) {
                        upsertLogFrontmatterScalars(logPathToUpdate, telemetryData);
                        console.log(`📊 Recorded token telemetry in log frontmatter ($${telemetryData.cost_usd} across ${telemetryData.sessions} session${telemetryData.sessions !== 1 ? 's' : ''})`);
                    }
                }
            } catch (e) {
                console.warn(`⚠️  Could not capture telemetry: ${e.message}`);
            }

            // Record the transition atomically now that the merge succeeded.
            // If the process crashes after this point, the next run will replay the spec move.
            try {
                ctx.stateMachine.requestTransition(num, 'feature-close', { actor: 'cli/feature-close' });
            } catch (e) {
                // If stage is already 'done' (re-run after partial close), continue gracefully.
                if (!e.message.includes("Invalid transition")) throw e;
            }

            // Move spec to done (idempotent: no-op if already there)
            ...
            // Record winner in manifest (logs stay flat in logs/)
            try {
                const manifest = require('../manifest');
                ...
                // Non-fatal: winner can be inferred from agent ID
            }
