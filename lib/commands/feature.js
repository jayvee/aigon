...
`;
        ...
        'feature-close': async (args) => {
            ...
            {
                const snapshot = await wf.showFeatureOrNull(closeRepoPath, closeFeatureId);
                if (snapshot) {
            ...
            // Reset agent settings files before merge — these are modified by feature-start
            // (worktree permissions) and always conflict on merge. They'll be regenerated
            // by install-agent on the next session start. Don't let them block the merge.
            ...
            // Stash remaining dirty files so the merge doesn't fail
            let didStash = false;
            try {
            ...
            // --- State transition + spec move (engine-only) ---
            {
                ...
                // Explicit migration path for features that pre-date workflow-core.
                const existingSnapshot = await wf.showFeatureOrNull(repoPath, closeFeatureId);
                if (!existingSnapshot) {
                    const lifecycle = found.folder === '04-in-evaluation' ? 'evaluating' : 'implementing';
                    const migration = await wf.migrateEntityLifecycleIfNeeded({
                        repoPath,
                        entityType: 'feature',
                        entityId: closeFeatureId,
                        mode: resolveFeatureMode(allAgents),
                        agents: allAgents,
                        readyAgents: allAgents,
                        lifecycle,
                    });
            ...
            {
                // Resolve any unmerged files first (e.g. from stash pop conflicts)
                try {
                ...
                const stagedPaths = [];
                // Stage the entire specs directory so both the new location (05-done)
                // and the deletion from the old location (03-in-progress) are captured.
                ...
                try {
                    ...
                    // Commit failed — verify the spec actually landed in 05-done on disk
                    const verifyDone = findFile(PATHS.features, num, ['05-done']);
                    if (!verifyDone) {
                        // Spec didn't make it — force move it now
                        ...
                        if (stuckSpec) {
                            ...
                            try {
                                ...
                                execSync(`git add docs/specs/features/`, { stdio: 'pipe' });
                                execSync(`git commit -m "chore: complete feature ${num} - move spec to done (recovery)"`, { stdio: 'pipe' });
                                console.log(`📋 Recovered: moved spec to done`);
                            } catch (e2) {
                                console.error(`❌ Could not move spec to done: ${e2.message}`);
                            }
                        }
                    }
                }
        ...
        'feature-open': (args) => {
            ...
            const terminal = process.platform === 'linux' && ['warp', 'terminal', 'cmux'].includes(requestedTerminal)
                ? 'tmux'
                : requestedTerminal;
            ...
            if (featureIds.length > 1) {
                ...
                if (terminal === 'warp') {
                    ...
                    try {
                        openInCmuxSplitPanes(worktreeConfigs, configName, title);

                        console.log(`\n🚀 Opening ${worktreeConfigs.length} features side-by-side in cmux:`);
                        console.log(`   Features: ${idsLabel}\n`);
                        worktreeConfigs.forEach(wt => {
                            console.log(`   ${wt.featureId.padEnd(4)} ${wt.agent.padEnd(8)} → ${wt.path}`);
                        });
                    ...
                    console.log(`   (Side-by-side launch requires Warp or cmux terminal. Use --terminal=warp or --terminal=cmux)\n`);
                    worktreeConfigs.forEach(wt => {
                ...
                if (terminal === 'warp') {
                    ...
                    try {
                        ...
                        console.log(`\n🚀 Opening ${worktreeConfigs.length} worktrees side-by-side in cmux:`);
                        console.log(`   Feature: ${paddedId} - ${desc}\n`);
                        worktreeConfigs.forEach(wt => {
                            console.log(`   ${wt.agent.padEnd(8)} → ${wt.path}`);
                        });
