...
module.exports = function featureCommands(ctx) {
    ...
    function runAigonCliCommand(mainRepoPath, args) {
        return spawnSync(process.execPath, [path.join(mainRepoPath, 'aigon-cli.js'), ...args], {
            cwd: mainRepoPath,
            encoding: 'utf8',
            stdio: 'pipe'
        });
...
`;
        ...
        'feature-autonomous-start': async (args) => {
                ...
                while (true) {
                    ...
                    if (!isFleet) {
                        ...
                        if (effectiveStopAfter === 'eval' && allReady && !evalTriggered) {
                            const evalArgs = ['feature-eval', featureNum, '--no-launch', `--agent=${effectiveEvalAgent}`];
                            console.log(`🚀 Triggering: aigon ${evalArgs.join(' ')}`);
                            const evalResult = runAigonCliCommand(mainRepo, evalArgs);
                            ...
                            if (evalResult.error || evalResult.status !== 0) {
                                console.error(`❌ feature-eval failed for feature ${featureNum}.`);
                                process.exitCode = 1;
                                return;
                            }
                            evalTriggered = true;
                        ...
                        if (evalTriggered) {
                            const next = workflowSnapshotAdapter.readWorkflowSnapshotSync(mainRepo, 'feature', featureNum);
                            if (next && next.currentSpecState === 'evaluating') {
            ...
            if (isFleet && !agentIds.includes(evalAgent)) {
                console.error(`❌ --eval-agent must be one of the implementation agents: ${agentIds.join(', ')}`);
                return;
            }
