                    ...
                    if (!isFleet) {
                        ...
                        if (soloReadyToAdvance && reviewAgent && !reviewTriggered) {
                            ...
                            if (tmuxSessionExists(reviewSessionName)) {
                                ...
                                createDetachedTmuxSession(reviewSessionName, mainRepo, reviewCommand);
                                if (!tmuxSessionExists(reviewSessionName)) {
                        ...
                        if (reviewStarted && effectiveStopAfter === 'close' && !closeTriggered) {
                            reviewClosePolls++;
                            const reviewSessionRunning = expectedReviewSessionName ? tmuxSessionExists(expectedReviewSessionName) : false;
                            console.log(`  [review-close ${reviewClosePolls}/${MAX_REVIEW_CLOSE_POLLS}] reviewSession running=${reviewSessionRunning}`);

                            if (!reviewSessionRunning) {
                                console.log('✅ Review session completed. Proceeding to close.');
                        ...
                        const readyToClose = soloReadyToAdvance && effectiveStopAfter === 'close'
                            && (!reviewAgent || (reviewStarted && !(expectedReviewSessionName && tmuxSessionExists(expectedReviewSessionName))));
