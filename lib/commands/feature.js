    ...
    function collectIncompleteFeatureEvalAgents({ featureNum, worktrees = [] }) {
        ...
        worktrees.forEach(w => {
            ...
            try {
                ...
                const m = logContent.match(/^---\n([\s\S]*?)\n---\n/);
                const status = m ? (m[1].match(/status:\s*(\S+)/)?.[1] || 'unknown') : 'unknown';
                if (status !== 'submitted') {
...
`;
        ...
        'feature-close': (args) => {
            ...
            // Stamp completedAt on the winner's log (flat logs/)
            const logsDir = path.join(PATHS.features.root, 'logs');
            if (fs.existsSync(logsDir)) {
                try {
                    const winnerLogs = fs.readdirSync(logsDir)
                        .filter(f => f.startsWith(`feature-${num}-`) && f.endsWith('-log.md') && !fs.lstatSync(path.join(logsDir, f)).isDirectory());
                    // In multi-agent mode, find the winner's log; in drive mode, any log is the winner
                    const winnerLog = agentId
                        ? winnerLogs.find(f => f.includes(`-${agentId}-`))
                        : winnerLogs[0];
                    if (winnerLog) {
                        updateLogFrontmatterInPlace(path.join(logsDir, winnerLog), {
                            setCompletedAt: true
                        });
                    }
                } catch (e) {
...
function createFeatureCommands(overrides = {}) {
    ...
    const names = [
        ...
        'feature-autopilot', 'feature-open', 'sessions-close'
    ];
