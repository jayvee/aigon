    ...
    const cmds = {
        ...
        'feature-pause': async (args) => {
            ...
            if (usePauseEngine) {
                ...
                if (wfState.state !== null) {
                    ...
                    if (wfState.state === 'paused') {
                        console.log(`✅ Feature ${paddedId} is already paused (workflow-core).`);
                        return;
                    }

                    const found = findFile(PATHS.features, paddedId, ['03-in-progress']);
                    if (!found) return console.error(`❌ Could not find feature "${paddedId}" in in-progress.`);
        ...
        'feature-resume': async (args) => {
            ...
            if (usePauseEngine) {
                ...
                if (wfState.state !== null) {
                    // Engine state exists — route through workflow-core
                    if (wfState.state === 'implementing') {
                        console.log(`✅ Feature ${paddedId} is already implementing (workflow-core).`);
                        return;
                    }

                    const found = findFile(PATHS.features, paddedId, ['06-paused']);
                    if (!found) return console.error(`❌ Could not find feature "${paddedId}" in paused.`);
                    const specFromPath = found.fullPath;
        ...
        'feature-eval': (args) => {
            ...
            // Outbox replay check: if a previous feature-eval was interrupted, resume
            // with the remaining pending ops instead of calling requestTransition again.
