...
`;
        ...
        'feature-close': (args) => {
            ...
            // Stash .env.local if it has local changes (it must not block merges)
            let envLocalStashed = false;
            try {
                const envStatus = execSync('git status --porcelain .env.local', { encoding: 'utf8' }).trim();
                if (envStatus) {
                    execSync('git stash push -m "aigon-feature-close-env-local" -- .env.local', { encoding: 'utf8', stdio: 'pipe' });
            ...
            try {
                ...
                if (envLocalStashed) try { execSync('git stash pop', { stdio: 'pipe' }); } catch (_) {}
        ...
        'feature-reset': (args) => {
            ...
            if (found) {
                const targetDir = path.join(PATHS.features.root, '02-backlog');
                const specBasename = path.basename(found.path);
