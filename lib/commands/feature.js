...
module.exports = function featureCommands(ctx) {
    ...
    function resolveReviewAgentFromOptions(options, fallbackAgent = 'cc') {
        const availableAgents = getAvailableAgents();
        const agentAliasMap = buildAgentAliasMap();
        const agentArgRaw = getOptionValue(options, 'agent');
        const resolvedAgent = agentArgRaw
            ? (agentAliasMap[String(agentArgRaw).toLowerCase()] || String(agentArgRaw).toLowerCase())
            : fallbackAgent;
        if (!availableAgents.includes(resolvedAgent)) {
            console.error(`❌ Unknown agent '${agentArgRaw || resolvedAgent}'. Supported agents: ${availableAgents.join(', ')}`);
            return null;
        }
        return resolvedAgent;
    }

    function launchPromptCommand({ commandName, entityId, agentId, argsString = '', modelTask = 'review' }) {
    ...
    function handleFeatureSpecPromptCommand(args, commandName) {
        ...
        const resolvedSpec = resolveFeatureSpecInfo(process.cwd(), id, gitLib);
        if (!resolvedSpec.path) {
            console.error(`❌ Could not resolve feature spec "${id}".`);
            return;
        }

        const sessionInfo = detectActiveAgentSession();
        if (!sessionInfo.detected && !noLaunch) {
            const agentId = resolveReviewAgentFromOptions(options);
            if (!agentId) return;
            launchPromptCommand({ commandName, entityId: id, agentId });
            return;
        }

        printAgentContextWarning(commandName, id);
        console.log(`📋 Spec: ./${path.relative(process.cwd(), resolvedSpec.path)}`);
    }

    async function recordFeatureSpecReviewFromHead(id, kind) {
        if (!id) {
            console.error(`Usage: aigon feature-spec-review${kind === 'ack' ? '-check' : ''}-record <ID|slug>`);
            process.exitCode = 1;
            return;
        }
        const resolvedSpec = resolveFeatureSpecInfo(process.cwd(), id, gitLib);
        ...
        const parsedFile = parseFeatureSpecFileName(path.basename(resolvedSpec.path));
        const entityId = parsedFile && (parsedFile.id || parsedFile.name);
        try {
            ...
            if (kind === 'review') {
                await wf.recordSpecReviewSubmitted(process.cwd(), 'feature', entityId || id, {
                    reviewId: commit.sha,
                ...
                await wf.recordSpecReviewAcknowledged(process.cwd(), 'feature', entityId || id, {
                    ackedBy: process.env.AIGON_AGENT_ID || null,
                    commitSha: commit.sha,
                });
            }
            console.log(`✅ Recorded feature spec review state for ${entityId || id}`);
        } catch (error) {
    ...
    const cmds = {
        ...
        'feature-start': async (args) => {
            ...
            const pickerModelOverrides = u.parseAgentOverrideMap(options.models);
            const pickerEffortOverrides = u.parseAgentOverrideMap(options.efforts);
...

            {
                ...
                const repoPath = process.cwd();
                const projCfgStart = loadProjectConfig(repoPath);
...
`;
        ...
        'feature-spec-review': (args) => {
            handleFeatureSpecPromptCommand(args, 'feature-spec-review');
        },

        'feature-spec-review-check': (args) => {
            handleFeatureSpecPromptCommand(args, 'feature-spec-review-check');
        },

        'feature-spec-review-record': async (args) => {
            await recordFeatureSpecReviewFromHead(args[0], 'review');
        },

        'feature-spec-review-check-record': async (args) => {
            await recordFeatureSpecReviewFromHead(args[0], 'ack');
        },
...
function createFeatureCommands(overrides = {}) {
    ...
    const names = [
        ...
        'feature-spec-review-record', 'feature-spec-review-check-record',
