...
`;
    ...
    'radar': async (args) => {
        console.warn("⚠️  'aigon radar' is deprecated — use 'aigon dashboard' instead");
        return commands['dashboard'](args);
    },
    ...
    'dashboard': async (args) => {
        ...
        // No subcommand: start foreground server + open browser
        if (!sub) {
            let port;
            ...
            const running = listDashboardInstances().find(i =>
                i.worktreePath === process.cwd() && isProcessAlive(i.pid)
            );
            if (running) {
                console.log(`⚠️  Dashboard already running (PID ${running.pid})`);
            ...
            runDashboardServer(port, instanceName);
            return;
        }

        if (sub === 'list') {
            const instances = listDashboardInstances();
            if (instances.length === 0) { console.log('No dashboard instances running.'); return; }
            console.log(`Running instances (${instances.length}):`);
            instances.forEach(i => {
                const alive = isProcessAlive(i.pid);
                const name = i.name || 'main';
                console.log(`  ${name.padEnd(40)} http://localhost:${i.port}  PID ${i.pid} ${alive ? '✅' : '⛔ dead'}`);
            });
            return;
        }

        if (sub === 'open') {
            const name = options._[1] || instanceName;
            const instances = listDashboardInstances();
            const target = instances.find(i => i.name === name) || instances.find(i => !i.name || i.name === 'main');
            if (!target) { console.error(`❌ No instance found: ${name}`); process.exitCode = 1; return; }
            try { openInBrowser(`http://localhost:${target.port}`); } catch (e) { /* ignore */ }
            console.log(`🌐 Dashboard: http://localhost:${target.port}`);
            return;
        }
        ...
        if (sub === 'status') {
            ...
            instances.forEach(i => {
                const alive = isProcessAlive(i.pid);
                console.log(`Dashboard (${i.name || 'main'}): ${alive ? `✅ running (PID ${i.pid})` : '⛔ stopped'}`);
                if (alive) console.log(`   http://localhost:${i.port}`);
            });
            if (instances.length === 0) console.log('Dashboard: ⛔ not running');
            if (repos.length === 0) {
        ...
        console.log('  (no subcommand)    Start dashboard server (foreground)');
