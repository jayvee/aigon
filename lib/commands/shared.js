...
const commands = {
    ...
    'feature-eval': (args) => {
        ...
        // Count worktrees for this feature to determine mode
        {
            const paddedNum = name.toString().padStart(2, '0');
            let featureWorktreeCount = 0;
            try {
                ...
                stdout.split('\n').forEach(line => {
                    ...
                    if (!wtMatch) return;
                    if (wtMatch[1].match(new RegExp(`feature-(?:${paddedNum}|${parseInt(paddedNum, 10)})-(\\w+)-`))) {
                        featureWorktreeCount++;
                    }
                });
            } catch (e) { /* ignore */ }

            if (featureWorktreeCount <= 1) {
                // Solo feature: either Drive (branch) or Drive worktree (1 worktree)
...
`;
    ...
    'feature-close': (args) => {
        ...
        if (worktreePath && fs.existsSync(worktreePath)) {
            ...
            // Deregister any radar/dev-server instance running from this worktree
            if (agentId) {
                const serverId = `${agentId}-${num}`;
                try {
                    deregisterRadarServer(serverId);
                } catch (e) { /* entry may not exist — that's fine */ }
            }
        ...
        // Clean up any stale dev-proxy entries (dead radar/dev-server instances)
        try {
        ...
        try {
            ...
            lines.forEach(line => {
                ...
                if (wtPath.includes(`feature-${paddedId}-`) || wtPath.includes(`feature-${unpaddedId}-`)) {
                    const wtStatus = utils.getWorktreeStatus(wtPath);
                    if (wtStatus) {
                        console.warn(`   ⚠️  Worktree has uncommitted changes — moving to Trash: ${wtPath}`);
                    }
                    // Deregister any radar/dev-server instance for this worktree
                    const wtAgentMatch = wtPath.match(new RegExp(`feature-(?:${paddedId}|${unpaddedId})-(\\w+)-`));
                    if (wtAgentMatch) {
                        try { deregisterRadarServer(`${wtAgentMatch[1]}-${unpaddedId}`); } catch (e) { /* may not exist */ }
                    }

                    console.log(`   Removing worktree: ${wtPath}`);
                    removedWorktreePaths.push(wtPath);
                    if (utils.safeRemoveWorktree(wtPath)) { worktreeCount++; }
                    else { console.error(`   ❌ Failed to remove ${wtPath}`); }
                }
            });
    ...
    'feature-backfill-timestamps': (args) => {
        ...
        if (!repos || repos.length === 0) {
            return console.error('❌ No repos registered in global config. Run aigon radar register first.');
        }
    ...
    'radar': async (args) => {
        ...
        if (sub === 'start') {
            const radarCtx = detectRadarContext();
            const proxyAvailable = isProxyAvailable();

            // Radar is a single process serving API + dashboard HTML on one port.
            // Main gets a fixed port; worktrees get dynamic ports.

            // Resolve the CLI entrypoint (aigon-cli.js, not this module)
            const cliEntrypoint = path.resolve(__dirname, '..', '..', 'aigon-cli.js');

            if (radarCtx.isWorktree) {
                // --- Worktree: isolated instance on a dynamic port ---
                const registry = loadProxyRegistry();
                const aigonEntries = registry[RADAR_APP_ID] || {};
                const existing = aigonEntries[radarCtx.serverId];
                if (existing && existing.dashboard && existing.dashboard.pid > 0 && isProcessAlive(existing.dashboard.pid)) {
                    console.log(`⚠️  Radar already running for ${radarCtx.serverId} (PID ${existing.dashboard.pid})`);
                    console.log(`   Dashboard: ${resolveRadarUrl(radarCtx.serverId, existing.dashboard.port)}`);
                    return;
                }

                let radarPort;
                try {
                    radarPort = await allocatePort(RADAR_DYNAMIC_PORT_START);
                } catch (e) {
                    console.error(`❌ ${e.message}`);
                    process.exitCode = 1;
                    return;
                ...
                const child = spawn(process.execPath, [cliEntrypoint, 'radar', '--daemon', `--port=${radarPort}`], {
                    detached: true,
                    stdio: 'ignore',
                    env: process.env
                });
                ...
                // Register — service and dashboard are the same process/port
                const entry = {
                    service: { port: radarPort, pid: child.pid },
                    dashboard: { port: radarPort, pid: child.pid },
                    worktree: radarCtx.worktreePath,
                    started: new Date().toISOString()
                };

                if (proxyAvailable) {
                    registerRadarServer(radarCtx.serverId, entry);
                } else {
                    const reg = loadProxyRegistry();
                    if (!reg[RADAR_APP_ID]) reg[RADAR_APP_ID] = {};
                    reg[RADAR_APP_ID][radarCtx.serverId] = entry;
                    saveProxyRegistry(reg);
                }

                const url = resolveRadarUrl(radarCtx.serverId, radarPort);
                console.log(`✅ Radar started (worktree: ${radarCtx.serverId})`);
                console.log(`   API:       http://127.0.0.1:${radarPort}/api/status`);
                console.log(`   Dashboard: ${url} (PID ${child.pid})`);
                if (!proxyAvailable) {
                    console.log(`\n   💡 Run \`aigon proxy-setup\` for subdomain routing (e.g., ${getDevProxyUrl(RADAR_APP_ID, radarCtx.serverId)})`);
                }
            } else {
                // --- Main: fixed port ---
                ...
                const { spawn } = require('child_process');
                const child = spawn(process.execPath, [cliEntrypoint, 'radar', '--daemon', `--port=${port}`], {
                    detached: true,
                    stdio: 'ignore',
                    env: process.env
                });
                ...
                // Register — service and dashboard are the same process/port
                const entry = {
                    service: { port, pid: child.pid },
                    dashboard: { port, pid: child.pid },
                    started: new Date().toISOString()
                };
                ...
                const url = resolveRadarUrl('', port);
                console.log(`✅ Radar started (PID ${child.pid})`);
                console.log(`   API:       http://127.0.0.1:${port}/api/status`);
                console.log(`   Dashboard: ${url}`);
                if (!proxyAvailable) {
                    console.log(`\n   💡 Run \`aigon proxy-setup\` for subdomain routing (e.g., http://aigon.test)`);
                }
        ...
        if (sub === 'stop') {
            ...
            if (radarCtx.isWorktree) {
                // Stop worktree Radar instance
                const registry = loadProxyRegistry();
                const aigonEntries = registry[RADAR_APP_ID] || {};
                const entry = aigonEntries[radarCtx.serverId];
                if (!entry) {
                    console.log(`⛔ No Radar running for ${radarCtx.serverId}`);
                    return;
                }
                // Kill service and dashboard processes
                [entry.service, entry.dashboard].forEach(proc => {
                    if (proc && proc.pid > 0) {
                        try { process.kill(proc.pid, 'SIGTERM'); } catch (e) { /* already dead */ }
                    }
                });
                deregisterRadarServer(radarCtx.serverId);
                console.log(`✅ Radar stopped (${radarCtx.serverId})`);
            } else {
                // Stop main Radar instance
                const pid = isRadarAlive();
                if (!pid) {
                ...
                // Kill service process (stored in meta)
                const stopMeta = readRadarMeta();
                const servicePidToKill = stopMeta && stopMeta.servicePid;
                if (servicePidToKill && servicePidToKill !== pid) {
                    try { process.kill(servicePidToKill, 'SIGTERM'); } catch (e) { /* already dead */ }
                }
                // Kill dashboard process (pid from RADAR_PID_FILE)
                try {
                ...
                const stoppedPids = [servicePidToKill && servicePidToKill !== pid ? `service:${servicePidToKill}` : null, `dashboard:${pid}`].filter(Boolean).join(', ');
                console.log(`✅ Radar stopped (${stoppedPids})`);
            }
            return;
        }
        ...
        if (sub === 'status') {
            const pid = isRadarAlive();
            // Support both new format (dashboardPort/servicePid) and legacy format (port)
            const dashPort = meta && meta.dashboardPort ? meta.dashboardPort : (meta && Number.isInteger(meta.port) ? meta.port : RADAR_DASHBOARD_PORT);
            const svcPort = meta && meta.servicePort ? meta.servicePort : RADAR_DEFAULT_PORT;
            const mainUrl = resolveRadarUrl('', dashPort);
            console.log(`Radar:     ${pid ? `✅ running (PID ${pid})` : '⛔ stopped'}`);
            if (pid) {
                if (meta && meta.servicePid) {
                    console.log(`Service:   port ${svcPort} (PID ${meta.servicePid})`);
                }
                console.log(`Dashboard: ${mainUrl}`);
            }

            // Show worktree instances from the proxy registry
            const registry = loadProxyRegistry();
            const aigonEntries = registry[RADAR_APP_ID] || {};
            const worktreeIds = Object.keys(aigonEntries).filter(id => id !== '');
            if (worktreeIds.length > 0) {
                console.log(`\nWorktree instances (${worktreeIds.length}):`);
                for (const id of worktreeIds) {
                    const entry = aigonEntries[id];
                    const dashPort = entry.dashboard ? entry.dashboard.port : '?';
                    const svcPort = entry.service ? entry.service.port : '?';
                    const wtUrl = resolveRadarUrl(id, dashPort);
                    const alive = entry.dashboard && entry.dashboard.pid > 0 && isProcessAlive(entry.dashboard.pid);
                    console.log(`  ${id.padEnd(12)} ${wtUrl.padEnd(35)} service:${svcPort} dashboard:${dashPort} ${alive ? '✅' : '⛔ dead'}`);
                }
            }
            ...
            if (pid) {
                try {
                    const payload = await requestRadarJson('/api/status', svcPort);
        ...
        if (sub === 'open') {
            const radarCtx = detectRadarContext();
            // For worktree context, look up the dashboard port from the registry
            ...
            if (radarCtx.isWorktree) {
                ...
                if (entry && entry.dashboard) {
                    dashboardPort = entry.dashboard.port;
                }
            }
            const url = resolveRadarUrl(radarCtx.serverId, dashboardPort);
            const localhostUrl = `http://127.0.0.1:${dashboardPort}`;
            ...
            if (screenshot) {
                const healthy = await waitForHealthy(localhostUrl, 15000, 400);
                if (!healthy) {
                ...
                try {
                    const result = await captureDashboardScreenshot(localhostUrl, output, width, height);
...
`;
    ...
    'feature-open': (args) => {
        // Canonical command for "start/resume an agent on a feature".
        // Context-aware: works for worktree-based (fleet) and branch-based (drive) modes.
        // Delegates to the same logic as worktree-open.
        return commands['worktree-open'](args);
    },
        ...
        if (subcommand === 'start') {
            ...
            for (const [appId, servers] of Object.entries(registry)) {
                for (const [serverId, info] of Object.entries(servers)) {
                    ...
                    // Radar entries have nested service/dashboard format
