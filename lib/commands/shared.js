...
`;
    ...
    'doctor': (args) => {
        ...
        if (!isProxyAvailable()) {
            ...
            console.log(`  ⚠️  Proxy unavailable: ${reason}`);
        } else {
            try {
                const r = reconcileProxyRoutes();
                const parts = [];
                ...
                const summary = parts.length > 0
                    ? `${parts.join(', ')}, ${r.unchanged} unchanged`
                    : `${r.unchanged} route${r.unchanged === 1 ? '' : 's'} unchanged`;
                console.log(`  ✅ Proxy reconciled: ${summary}`);
            } catch (e) {
                console.log(`  ⚠️  Proxy reconciliation failed: ${e.message}`);
            }
        }
