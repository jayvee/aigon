...
`;
    ...
    'feature-backfill-timestamps': (args) => {
        ...
        filteredPaths.forEach(repoPath => {
            ...
            logFiles.forEach(logFile => {
                ...
                let content;
                try { content = fs.readFileSync(logPath, 'utf8'); } catch (e) { totalErrors++; return; } // optional
    ...
    'feature-autopilot': (args) => {
        ...
        existingWorktrees.forEach(wt => {
            ...
            } catch (e) { /* proceed with spawn */ } // optional
    ...
    'research-autopilot': (args) => {
        ...
        if (allAlreadySubmitted) {
            ...
            try {
                while (!interrupted) {
                    try { spawnSync('sleep', [String(pollInterval / 1000)], { stdio: 'ignore' }); } catch (e) { break; } // optional
                    if (interrupted) break;
    ...
    'sessions-close': (args) => {
        ...
        if (foundAny) {
            try { execSync('sleep 1', { stdio: 'ignore' }); } catch (e) { /* ignore */ } // optional
            killPatterns.forEach(pattern => {
                try {
