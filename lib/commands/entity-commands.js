...
function resolveEntitySpec(def, id, folders, ctx) {
    const u = ctx.utils;
    const found = u.findFile(def.paths, id, folders);
...
async function entityResetBase(def, id, ctx, extraSteps = {}) {
    ...
    const found = u.findFile(def.paths, id, foldersToSearch);
    if (found) {
    ...
    // the workflow as a first-class backlog item.
    //
    // Without this, the dashboard read model (workflow-read-model.js)
    // classifies "numeric ID + spec on disk + no engine snapshot" as
    // LEGACY_MISSING_WORKFLOW — read-only, amber "legacy" badge, no Start
    // action. We kept manually bootstrapping individual features (F285, F293)
    // after reset. Doing it here makes reset produce the state the read model
    // actually expects.
    //
