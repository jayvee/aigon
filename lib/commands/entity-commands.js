...
const { matchTmuxSessionByEntityId } = require('../agent-sessions/names');
    ...
    try {
        ...
        if (!tmuxList.error && tmuxList.status === 0) {
            (tmuxList.stdout || '').split('\n').map((line) => line.trim()).filter(Boolean).forEach((sessionName) => {
                const matched = matchTmuxSessionByEntityId(sessionName, entityId);
                if (!matched || matched.role !== 'review') return;
