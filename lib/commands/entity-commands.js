...
async function entitySpecReviewPrompt(def, args, commandName, ctx) {
    ...
    const isCheck = /-check$/.test(commandName);
    const sessionInfo = u.detectActiveAgentSession();
    const launchedInline = !sessionInfo.detected && !noLaunch;
    const agentIdForEvent = launchedInline
        ? resolveReviewAgentFromOptions(options, ctx)
        : String(process.env.AIGON_AGENT_ID || '').trim() || null;
...
function createEntityCommands(def, ctx) {
    const cmds = {
        ...
        [`${def.prefix}-spec-review-check`]: (args) => {
            return entitySpecReviewPrompt(def, args, `${def.prefix}-spec-review-check`, ctx);
        },
