...
const { parseTmuxSessionName, resolveTmuxRepoName } = require('../agent-sessions/names');
...
const { resolveResumeArgs } = require('../session-sidecar');

function readContextInput(options) {
    const file = getOptionValue(options, 'file');
    const inline = getOptionValue(options, 'json');
    if (file) return JSON.parse(fs.readFileSync(path.resolve(String(file)), 'utf8'));
    if (inline) return JSON.parse(String(inline));
    if (!process.stdin.isTTY) return JSON.parse(fs.readFileSync(0, 'utf8'));
    throw new Error('provide handoff JSON with --file=<path>, --json=<object>, or stdin');
}

function entityContextCommand(def, args) {
    const options = parseCliOptions(args);
    const action = options._[0];
    const id = options._[1];
    if (!['record', 'show'].includes(action) || !id) {
        process.exitCode = 1;
        return console.error(`Usage: aigon ${def.prefix}-context <record|show> <ID> [--file=<json>]`);
    }
    try {
        if (action === 'show') {
            const value = entityContext.readPublicEntityContext(process.cwd(), def.type, id);
            console.log(JSON.stringify(value, null, 2));
            return value;
        }
        const recordedBy = String(process.env.AIGON_AGENT_ID || '').trim() || null;
        const handoff = entityContext.recordAuthorHandoff(
            process.cwd(), def.type, id, readContextInput(options), { recordedBy }
        );
        console.log(`✅ Recorded ${def.type} ${String(id).padStart(2, '0')} author handoff v${handoff.artifactVersion}.`);
        return handoff;
    } catch (error) {
        process.exitCode = 1;
        console.error(`❌ Could not ${action} author handoff: ${error.message}`);
        if (error.issues) console.error(`   Required fields: ${entityContext.HANDOFF_FIELDS.join(', ')}`);
        return null;
    }
}

function closeCodeReviewSessions(entityType, entityId, snapshot) {
    const paddedId = String(entityId).padStart(2, '0');
    const repoPrefix = resolveTmuxRepoName({ cwd: process.cwd() });
    const expectedType = entityType === 'research' ? 'r' : 'f';
    const codeReview = snapshot && snapshot.codeReview ? snapshot.codeReview : {};
    const reviewerId = codeReview.activeReviewerId || snapshot.pendingCodeReviewer || null;
    const pkillPattern = entityType === 'research'
        ? `aigon:research-review ${paddedId}`
        : `aigon:feature-code-review ${paddedId}`;
    try {
        spawnSync('pkill', ['-f', pkillPattern], { stdio: 'ignore' });
    } catch (_) { /* no matching process */ }

    try {
        const tmuxList = spawnSync('tmux', ['list-sessions', '-F', '#S'], { encoding: 'utf8' });
        if (!tmuxList.error && tmuxList.status === 0) {
            (tmuxList.stdout || '').split('\n').map((line) => line.trim()).filter(Boolean).forEach((sessionName) => {
                const matched = parseTmuxSessionName(sessionName);
                if (!matched) return;
                if (matched.repoPrefix !== repoPrefix) return;
                if (matched.type !== expectedType) return;
                if (matched.role !== 'review') return;
                if (String(parseInt(matched.id, 10)) !== String(parseInt(entityId, 10))) return;
                if (reviewerId && matched.agent !== reviewerId) return;
...
const SPEC_FOLDERS_ALL = [
    STAGE_FOLDERS.INBOX, STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS,
    STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.DONE, STAGE_FOLDERS.PAUSED,
];
...
function resolveReviewAgentFromOptions(options, ctx, fallbackAgent) {
    const u = ctx.utils;
    const repoPath = (ctx && ctx.repoPath) || process.cwd();
    const availableAgents = u.getAvailableAgents(repoPath);
    const aliasMap = u.buildAgentAliasMap();
    const raw = getOptionValue(options, 'agent');
    const defaultAgent = fallbackAgent || getDefaultAgent(repoPath);
    ...
    if (!availableAgents.includes(resolvedAgent)) {
        ...
        const check = agentAvailability.tryAssertAgentUsable(resolvedAgent, repoPath, { explicit: true });
        if (!check.ok) {
...
function buildLaunchPromptArgs({ agentId, model, flagTokens, promptFlag, prompt, resumeProviderSessionId }) {
    const modelTokens = buildModelArgTokens(agentId, model);
    const standardArgs = [...flagTokens, ...modelTokens, ...(promptFlag ? [promptFlag] : []), prompt];
    if (!resumeProviderSessionId) return standardArgs;
    const resumeSpec = resolveResumeArgs(agentId, resumeProviderSessionId);
    if (!resumeSpec) return standardArgs;
    if (resumeSpec.isSubcommand) return [...resumeSpec.prependArgs, prompt];
    return [...flagTokens, ...modelTokens, ...resumeSpec.appendArgs, ...(promptFlag ? [promptFlag] : []), prompt];
}

function launchPromptCommand({ commandName, entityId, agentId, argsString = '', modelTask = 'review', ctx,
    resumeProviderSessionId = null, continuityInstructions = '' }) {
    const u = ctx.utils;
    const cliConfig = u.getAgentCliConfig(agentId);
    let prompt = resolveAgentCommandPrompt({
        agentId,
        commandName,
        argsString: [String(entityId), String(argsString || '').trim()].filter(Boolean).join(' '),
        cliConfig,
    });
    if (continuityInstructions) prompt += `\n\n${continuityInstructions}`;
    ...
    const spawnArgs = buildLaunchPromptArgs({ agentId, model, flagTokens, promptFlag, prompt, resumeProviderSessionId });
    const env = { ...process.env };
    env.AIGON_ENTITY_TYPE = commandName.startsWith('research-') ? 'research' : 'feature';
    env.AIGON_ENTITY_ID = String(entityId);
...
async function entitySpecReviewPrompt(def, args, commandName, ctx) {
    ...
    if (!agentIdForEvent && sessionInfo.detected && sessionInfo.agentId) {
        agentIdForEvent = sessionInfo.agentId;
    }
    ...
    if (agentIdForEvent) {
        try {
            if (isCheck) {
                await wf.recordSpecRevisionStarted(process.cwd(), def.type, spec.entityId, {
            ...
            console.warn(`⚠️  Could not record ${isCheck ? 'spec-revise' : 'spec-review'} start: ${error.message}`);
        }
    }
    ...
    if (isCheck && agentIdForEvent) {
        const snapshot = def.type === 'research'
            ? await wf.showResearchOrNull(process.cwd(), spec.entityId)
            : await wf.showFeatureOrNull(process.cwd(), spec.entityId);
        continuity = entityContext.readEntityContext(process.cwd(), def.type, spec.entityId) || {};
        continuityDecision = resolveContinuityPolicy({
            phase: 'spec-revise',
            selectedAgent: agentIdForEvent,
        ...
        entityContext.recordContinuityDecision(process.cwd(), def.type, spec.entityId, continuityDecision);
        console.log(`🧭 Continuity: ${continuityDecision.strategy} — ${continuityDecision.reasons.join(', ')}`);
        ...
        if (continuityDecision.strategy === 'resume-origin') {
            const sessionId = `spec-revise-${def.type}-${String(spec.entityId).padStart(2, '0')}-${Date.now()}`;
            continuityInstructions += `\n\n## Continuation checkpoint\nAfter reconciling the current checkout, run exactly one:\n`;
            continuityInstructions += `aigon agent-status continuation-ready ${spec.entityId} ${agentIdForEvent} --session=${sessionId}\n`;
            continuityInstructions += `aigon agent-status continuation-fallback ${spec.entityId} ${agentIdForEvent} --session=${sessionId} --reason=<context-missing|context-conflict|task-delivery-failed>`;
        }
    }
    ...
    if (launchedInline) {
        ...
        launchPromptCommand({
            commandName, entityId: id, agentId: agentIdForEvent, ctx,
            continuityInstructions,
...
async function entityResetBase(def, id, ctx, extraSteps = {}) {
    ...
    const entityType = def.type === 'research' ? 'research' : 'feature';
    const ref = { entityType, entityId: paddedId };
    let destructiveLeaseCoord = null;
    ...
    try {
        await assertNoForeignActiveLeases(process.cwd(), ref);
        destructiveLeaseCoord = await coordinateMutatingCommand(process.cwd(), {
            entityType,
            entityId: paddedId,
            role: 'close',
        ...
        process.exitCode = 1;
        console.error(`❌ ${formatLeaseConflict(error)}`);
        if (destructiveLeaseCoord) {
            const { releaseEntityLeases } = require('../spec-store/lease-coordination');
            await releaseEntityLeases(destructiveLeaseCoord.store, destructiveLeaseCoord.ref, ['close'], {
                agentId: process.env.AIGON_AGENT_ID || null,
            });
        }
    ...
    const foldersToSearch = def.type === 'research'
        ? [STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.PAUSED]
        : [STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.DONE, STAGE_FOLDERS.PAUSED];
    const stableLayout = specLifecycle.isStableSpecLayout(process.cwd());
    const found = stableLayout ? null : ctx.specCrud.findFile(def.paths, id, foldersToSearch);
    if (found) {
        const targetDir = path.join(def.paths.root, STAGE_FOLDERS.BACKLOG);
        ...
        fs.renameSync(sourceSpecPath, targetPath);
        ...
        try {
            ...
            stageAndCommitSpecMove(runGit, process.cwd(), {
                ...
                toPath: targetPath,
                message: `chore: reset ${def.type} ${paddedId} - move spec to backlog`,
            });
    ...
    try {
        ...
        if (stableLayout) {
            specLifecycle.refreshLifecycleView(process.cwd(), { warn: console.warn });
        }
    } catch (e) {
        console.warn(`   ⚠️  Could not remove workflow engine state for ${paddedId}: ${e.message}`);
    }

    // Step 4.5: release work-role lease (git-branch backend; advisory no-op on local)
    try {
        const { createSpecStore, resolveStorageConfig } = require('../spec-store');
        const { releaseEntityLeases, workLeaseRole } = require('../spec-store/lease-coordination');
        const storage = resolveStorageConfig(process.cwd());
        const store = destructiveLeaseCoord && destructiveLeaseCoord.store
            ? destructiveLeaseCoord.store
            : createSpecStore({ repoPath: process.cwd(), storage });
        await releaseEntityLeases(store, ref, [workLeaseRole(entityType), 'close'], {
            agentId: process.env.AIGON_AGENT_ID || null,
        });
    } catch (e) {
        console.warn(`   ⚠️  Could not release lease for ${paddedId}: ${e.message}`);
    }
...
function collectResetLeaks(def, paddedId, unpaddedId) {
    ...
    // 2. Workflow engine state that did not return to backlog. Reset records a
    // merge-safe `*.reset` event and keeps the dir (event log + a backlog
    // snapshot) so the return-to-backlog converges across git-branch clones —
    // so the dir's mere presence is NOT a leak. Flag only when the snapshot is
    // still parked at a started/active lifecycle, which is the real failure the
    // old dir-absence check was standing in for.
    try {
        const wfSubdir = def.type === 'research' ? 'research' : 'features';
        const snapshotPath = path.join(repo, '.aigon', 'workflows', wfSubdir, paddedId, 'snapshot.json');
        if (fs.existsSync(snapshotPath)) {
            let lifecycle = null;
            try {
                const snap = JSON.parse(fs.readFileSync(snapshotPath, 'utf8'));
                lifecycle = snap.currentSpecState || snap.lifecycle || null;
            } catch (e) { /* unreadable snapshot — treat as leak below */ }
            if (lifecycle !== 'backlog') {
                leaks.push(`workflow snapshot not reset to backlog (lifecycle=${lifecycle || 'unknown'}): .aigon/workflows/${wfSubdir}/${paddedId}/`);
            }
        }
    } catch (e) { /* observation only */ }
...
function createEntityCommands(def, ctx) {
    const cmds = {
        ...
        [`${def.prefix}-context`]: (args) => entityContextCommand(def, args),
        ...
        [`${def.prefix}-cancel-code-review`]: async (args) => {
            const id = args[0];
            if (!id) return console.error(`Usage: aigon ${def.prefix}-cancel-code-review <id>`);
            const paddedId = String(id).padStart(2, '0');
            const repoPath = process.cwd();
            const snapshot = def.type === 'research'
                ? await wf.showResearchOrNull(repoPath, paddedId)
            ...
            if (snapshot.lifecycle !== 'code_review_in_progress') {
                process.exitCode = 1;
                return console.error(`❌ ${def.prefix} ${paddedId} is not in code_review_in_progress (current: ${snapshot.lifecycle}).`);
            }
            closeCodeReviewSessions(def.type, paddedId, snapshot);
            await wf.recordCodeReviewCancelled(repoPath, def.type, paddedId, {
                source: `${def.prefix}-cancel-code-review`,
            });
            if (def.type === 'feature') {
                const { readFeatureAutoState, writeFeatureAutoState } = require('../auto-session-state');
                const auto = readFeatureAutoState(repoPath, paddedId);
                if (auto && String(auto.reason || '') === 'review-quota-paused') {
                    writeFeatureAutoState(repoPath, paddedId, {
                        reason: null,
                        workflowState: 'ready',
            ...
            console.log(`✅ Code review cancelled for ${def.prefix} ${paddedId}. Returned to ready.`);
        },
        ...
        [`${def.prefix}-transcript`]: (args) => {
            ...
            if (agentArgRaw) {
                const aliasMap = u.buildAgentAliasMap();
                const available = u.getAllKnownAgents();
            ...
            const repoPath = (ctx.git && typeof ctx.git.getMainRepoPath === 'function')
                ? ctx.git.getMainRepoPath(process.cwd())
                : process.cwd();
