...
async function entitySpecReviewPrompt(def, args, commandName, ctx) {
    ...
    if (isCheck && agentIdForEvent) {
        const snapshot = def.type === 'research'
            ? await wf.showResearchOrNull(process.cwd(), spec.entityId)
            : await wf.showFeatureOrNull(process.cwd(), spec.entityId);
        const continuity = entityContext.readEntityContext(process.cwd(), def.type, spec.entityId) || {};
        ...
        entityContext.recordContinuityDecision(process.cwd(), def.type, spec.entityId, decision);
        console.log(`🧭 Continuity: ${decision.strategy} — ${decision.reasons.join(', ')}`);
    }
