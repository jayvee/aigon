...
async function entityResetBase(def, id, ctx, extraSteps = {}) {
    ...
    if (found) {
        ...
        specMoved = true;
        try {
            // Stage both endpoints of the move:
            //   sourceSpecPath (e.g. 03-in-progress/...) is now empty on disk → records the deletion
            //   targetPath (02-backlog/...) → records the addition
            // Replaces an earlier broad `git add docs/specs/features/` that was both wrong for research
            // and swept unrelated changes into the reset commit.
            spawnSync('git', ['add', '--', sourceSpecPath, targetPath], { stdio: 'pipe' });
            const commitResult = spawnSync(
                'git',
                ['commit', '-m', `chore: reset ${def.type} ${paddedId} - move spec to backlog`, '--', sourceSpecPath, targetPath],
                { stdio: 'pipe' },
            );
