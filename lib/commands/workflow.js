...
function buildDefinitionFromOptions(slug, options) {
    ...
    const revisionAgent = getOptionValue(options, 'revision');
    ...
    if (typeof reviewAgent === 'string' && reviewAgent) {
        ...
        if (typeof revisionAgent === 'string' && revisionAgent) {
            stages.push({ type: 'revision', agents: [revisionAgent] });
        } else {
            stages.push({ type: 'revision', agents: [implementAgents[0]] });
        }
    }
...
async function handleCreate(args, options) {
    const slug = args._[1];
    if (!slug) {
        console.error('Usage: aigon workflow create <slug> [--file=<path> | --json=<inline> | --from=<slug> | --implement=<agents> [--review=<agent>] [--revision=<agent>] [--eval=<agent>] [--no-close]] [--label=<label>] [--description=<desc>] [--global]');
        process.exitCode = 1;
        return;
    }
        ...
        workflow: async (rawArgs) => {
            ...
            switch (subcommand) {
                ...
                case '--help':
                    ...
                    console.log('  --revision=<agent>                Agent to address review feedback (default: first implementer)');
