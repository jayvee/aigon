...
module.exports = function feedbackCommands(ctx) {
    ...
    return {
        'feedback-create': (args) => {
            ...
            console.error('❌ feedback-create no longer writes feedback items. Capture the input as research with `origin: customer-feedback` instead.');
            return;
        },
        ...
        'feedback-migrate': (args) => {
            const options = parseCliOptions(args);
            const dryRun = options['dry-run'] !== undefined;
            const repoPath = process.cwd();

            if (!fs.existsSync(path.join(repoPath, 'docs', 'specs', 'feedback'))) {
                console.log('ℹ️  No docs/specs/feedback/ directory — nothing to migrate.');
                return;
            }

            const total = countFeedbackFiles(repoPath);
            if (total === 0) {
                console.log('ℹ️  No feedback files found — nothing to migrate.');
                return;
            }

            console.log(`\n📦 Feedback → research migration${dryRun ? ' (dry-run)' : ''}`);
            console.log(`   Found ${total} feedback file(s). Idempotency key: feedback_refs in research frontmatter.\n`);

            const result = migrateFeedbackToResearch(repoPath, {
                dryRun,
                log: (msg) => console.log(`   ${msg}`),
            });

            console.log(`\n✅ Migration complete: ${result.migrated} created, ${result.skipped} skipped (already migrated).`);
            ...
            if (!dryRun && result.migrated > 0) {
                console.log('\n💡 Feedback commands are deprecated. New input: `aigon research-create` with origin: customer-feedback.');
            }
        },
    };
};
...
function createFeedbackCommands(overrides = {}) {
    ...
    const names = ['feedback-create', 'feedback-list', 'feedback-triage', 'feedback-migrate'];
