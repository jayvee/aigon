...
const { stagePaths, stageAndCommitPaths, stageAndCommitSpecMove } = require('../git-staging');
const { STAGE_FOLDERS } = require('../workflow-core/paths');
const { coordinateMutatingCommand, formatLeaseConflict } = require('../spec-store/lease-coordination');
...
module.exports = function researchCommands(ctx) {
    ...
    const { runEffects, runFeatureEffect } = require('../workflow-core/effects');
    ...
    return {
        ...
        'research-unprioritise': async (args) => {
            const id = args[0];
            if (!id) return console.error('Usage: aigon research-unprioritise <ID>');

            const paddedId = String(id).padStart(2, '0');
            const repoPath = process.cwd();

            if (!(await wf.showResearchOrNull(repoPath, paddedId))) {
                process.exitCode = 1;
                return console.error(`❌ Research ${paddedId} has no workflow-core snapshot.\n   Run \`aigon doctor --fix\` to migrate legacy research items, then retry.`);
            }

            const snapshot = await wf.showResearch(repoPath, paddedId);
            const workflowId = String(snapshot.researchId || paddedId);

            if (snapshot.currentSpecState !== 'backlog') {
                process.exitCode = 1;
                return console.error(`❌ Cannot unprioritise research ${paddedId} from state "${snapshot.currentSpecState}". Research must be in backlog.`);
            }

            const found = findFile(PATHS.research, paddedId, [STAGE_FOLDERS.BACKLOG]);
            if (!found) return console.error(`❌ Could not find research "${paddedId}" in backlog.`);

            const specFromPath = path.join(PATHS.research.root, STAGE_FOLDERS.BACKLOG, found.file);
            const specContent = fs.readFileSync(specFromPath, 'utf8');
            const { data: specFm } = parseFrontMatter(specContent);
            const numericNamed = /^research-\d+-(.+)\.md$/.exec(found.file);
            const legacySlugStrip = numericNamed && !specFm.aigon_id;
            let inboxFilename = found.file;
            if (legacySlugStrip) {
                inboxFilename = `research-${numericNamed[1]}.md`;
            }
            const stableLayout = specLifecycle.isStableSpecLayout(repoPath);
            const canonicalSpecPath = stableLayout
                ? (specLifecycle.findCanonicalSpecPath(repoPath, 'research', workflowId) || specFromPath)
                : null;
            const specToPath = stableLayout
                ? canonicalSpecPath
                : path.join(PATHS.research.root, STAGE_FOLDERS.INBOX, inboxFilename);

            if (!stableLayout && legacySlugStrip && fs.existsSync(specToPath)) {
                process.exitCode = 1;
                return console.error(`❌ Inbox already contains ${inboxFilename}; refusing to overwrite.`);
            }

            let effectResearchId = workflowId;
            if (legacySlugStrip) {
                const slugId = numericNamed[1];
                try {
                    wf.migrateEntityWorkflowIdSync(repoPath, 'research', workflowId, slugId, specToPath, 'inbox');
                } catch (err) {
                    process.exitCode = 1;
                    return console.error(`❌ ${err.message}`);
                }
                effectResearchId = slugId;
            } else if (numericNamed) {
                try {
                    wf.transitionEntityLifecycleSync(repoPath, 'research', workflowId, 'inbox', specToPath);
                } catch (err) {
                    process.exitCode = 1;
                    return console.error(`❌ ${err.message}`);
                }
            } else {
                await wf.persistEntityEvents(repoPath, 'research', workflowId, [{
                    type: 'research.bootstrapped',
                    researchId: workflowId,
                    stage: 'inbox',
                    lifecycle: 'inbox',
            ...
            if (stableLayout) {
                ...
                const moveEffectDef = {
                    id: 'unprioritise.move_spec',
                    type: 'move_spec',
                    payload: { fromPath: specFromPath, toPath: specToPath },
                };
                await wf.persistEntityEvents(repoPath, 'research', effectResearchId, [
                    { type: 'effect.requested', effect: moveEffectDef, at },
                ]);

                const post = await wf.showResearch(repoPath, effectResearchId);
                const moveEff = (post.effects || []).find((e) => e.id === moveEffectDef.id && e.status !== 'succeeded');
                if (!moveEff) {
                    process.exitCode = 1;
                    return console.error('❌ No pending move_spec effect after unprioritise request');
                }
                await runEffects(repoPath, effectResearchId, [moveEff], runFeatureEffect);
                await wf.persistEntityEvents(repoPath, 'research', effectResearchId, [
                    { type: 'effect.succeeded', effectId: moveEffectDef.id, at: new Date().toISOString() },
                ]);
            }
            ...
            try {
                ...
                if (stableLayout) {
                    ...
                    stageAndCommitSpecMove(runGit, process.cwd(), {
                        ...
                        toPath: specToPath,
                        message: `chore: unprioritise research ${workflowId} - move to inbox`,
                    });
                    console.log('📝 Committed research unprioritisation');
                }
            } catch (e) {
        ...
        'research-create': (args) => {
            ...
            const flags = {};
            const positional = [];
            const knownFlags = new Set(['--agent', '--description', '--quick']);
            for (let i = 0; i < rest.length; i++) {
                ...
                if (knownFlags.has(token)) {
                    ...
                    if (key === 'quick') {
                        ...
                        flags.description = rest.slice(i + 1).filter(value => value !== '--quick').join(' ').trim();
                        i = rest.length;
                    } else {
                        flags[key] = rest[i + 1];
                        i += 1;
            ...
            const description = (flags.description || positional.join(' ')).trim();
            ...
            const built = entity.entityCreate(def, name, ctx, { description, agent: agentId || null });
            if (agentId && built && built.filePath) {
                const { draftSpecWithAgent } = require('../research-draft');
                draftSpecWithAgent(built.filePath, agentId, description);
                const foundSession = require('../session-sidecar').findNewAgentSession(
                    agentId,
                    process.cwd(),
                    Date.parse(built.originSession.captureStartedAt)
                );
                require('../entity-context').bindOriginNativeSession(process.cwd(), 'research', built.entityId, foundSession);
            }
        },
        ...
        'research-start': async (args) => {
            ...
            let spec = resolveResearchSpec(id, [STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS]);
            if (!spec) {
                console.error(`❌ Could not find research "${id}" in backlog or in-progress.`);
                return;
            }
            if (spec.found.folder === STAGE_FOLDERS.IN_PROGRESS && agentIds.length === 0) {
            ...
            if (!existingSnapshot && spec.found.folder === STAGE_FOLDERS.IN_PROGRESS) {
            ...
            if (needsStart) {
                try {
                    await coordinateMutatingCommand(repoPath, {
                        entityType: 'research',
                        entityId: spec.num,
                        role: 'research',
                        agentId: agentIds[0] || process.env.AIGON_AGENT_ID || null,
                        takeover: getOptionValue(options, 'takeover') !== undefined,
                        offline: getOptionValue(options, 'offline') !== undefined,
                    });
                } catch (error) {
                    process.exitCode = 1;
                    return console.error(`❌ ${formatLeaseConflict(error)}`);
                }
            ...
            spec = resolveResearchSpec(spec.num, [STAGE_FOLDERS.IN_PROGRESS]) || spec;
            const num = spec.num;
            ...
            if (mode === 'fleet') {
                ...
                console.log(`\n📋 Research topic: ./docs/specs/research-topics/${STAGE_FOLDERS.IN_PROGRESS}/${found.file}`);
                ...
                console.log(`📋 Topic: ./docs/specs/research-topics/${STAGE_FOLDERS.IN_PROGRESS}/${found.file}`);
                console.log(`\n💡 Next: Run agent with /aigon-research-do ${num}`);
                console.log(`   When done: aigon research-close ${num}`);
            }
        },

        'research-pause': async (args) => {
            const id = args[0];
            if (!id) return console.error(operatorPauseUsageLine('research-pause'));
            const result = await entity.pausePrestartEntity(def, id, ctx);
            if (result && result.handled) return;
            process.exitCode = 1;
            console.error(`❌ Cannot pause research "${id}" from its current state. Pause is available for inbox and backlog research items.\n   ${operatorPauseScopeHint('research')}`);
        },

        'research-resume': async (args) => {
            const id = args[0];
            if (!id) return console.error(operatorPauseUsageLine('research-resume'));
            const result = await entity.resumePrestartEntity(def, id, ctx);
            if (result && result.handled) return;
            process.exitCode = 1;
            console.error(`❌ Cannot resume research "${id}" from its current state. Resume is available for pre-start paused research items.\n   ${operatorPauseScopeHint('research')}`);
        },

        'research-do': async (args) => {
            const options = parseCliOptions(args);
            const id = options._[0] || args[0];
            ...
            let found = findFile(PATHS.research, id, [STAGE_FOLDERS.IN_PROGRESS]);
            ...
            try {
                await coordinateMutatingCommand(process.cwd(), {
                    entityType: 'research',
                    entityId: num,
                    role: 'research',
                    agentId: getOptionValue(options, 'agent') || process.env.AIGON_AGENT_ID || null,
                    takeover: getOptionValue(options, 'takeover') !== undefined,
                    offline: getOptionValue(options, 'offline') !== undefined,
                });
            } catch (error) {
            ...
            console.log(`\n📄 Topic: ./docs/specs/research-topics/${STAGE_FOLDERS.IN_PROGRESS}/${found.file}`);
            ...
            if (isFleetMode) {
                ...
                console.log(`\n✅ When done — MANDATORY completion signal:`);
                console.log(`   git add docs/specs/research-topics/logs/research-${num}-<agent>-findings.md`);
                console.log(`   git commit -m "docs: research findings for <agent>"`);
                console.log(`   aigon agent-status research-complete ${num} <agent>`);
                console.log(`\n   Hard rules — your findings are NOT recorded as complete until the status command exits 0:`);
                console.log(`   - Do NOT say "done", "complete", "findings written", or "ready for evaluation" before exit 0`);
                console.log(`   - The status command IS the completion signal; prose is not a substitute`);
                console.log(`   - Recording completion has nothing to do with research-eval — that is a separate user step`);
                console.log(`\n⚠️  IMPORTANT:`);
                console.log(`   - Do NOT run 'aigon research-close' from an agent session`);
                console.log(`   - The user will run 'aigon research-eval ${num}' after all findings are complete`);
            } else {
                console.log(`\n📝 Next Steps:`);
                ...
                console.log(`\n✅ When done — MANDATORY completion signal:`);
                console.log(`   aigon agent-status research-complete ${num} solo`);
                console.log(`\n   Then (and only then): aigon research-close ${num}`);
            }
        },
        ...
        'research-eval': async (args) => {
            ...
            if (!id) {
                return console.error(
                    ...
                    "Examples:\n" +
                    "  aigon research-eval 05          # Continue when all findings are complete\n" +
                    "  aigon research-eval 05 --force  # Evaluate even if some agents are unfinished"
                );
            }

            const spec = resolveResearchSpec(id, [STAGE_FOLDERS.IN_PROGRESS, STAGE_FOLDERS.IN_EVALUATION]);
            if (!spec) {
                console.error(`❌ Could not find research "${id}" in progress or in-evaluation.`);
                return;
            }

            if (spec.found.folder === STAGE_FOLDERS.IN_PROGRESS && !forceEval) {
                const incomplete = collectIncompleteResearchAgents(spec.num);
                if (incomplete.length > 0) {
                    console.log('');
                    console.log(`⚠️  ${incomplete.length} agent(s) not yet complete:`);
        ...
        'research-review': async (args) => {
            ...
            const spec = resolveResearchSpec(id, [STAGE_FOLDERS.IN_PROGRESS]);
            if (!spec) {
        ...
        'research-close': async (args) => {
            ...
            const closeSpec = resolveResearchSpec(id, [STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.IN_PROGRESS]);
            if (!closeSpec) {
                console.error(`❌ Could not find research "${id}" in in-evaluation or in-progress.`);
                return;
            }
            const { num } = closeSpec;
            if (closeSpec.found.folder === STAGE_FOLDERS.IN_PROGRESS) {
            ...
            let leaseCoord = null;
            try {
                const { coordinateMutatingCommand, assertNoForeignActiveLeases, formatLeaseConflict } = require('../spec-store/lease-coordination');
                const closeOptions = parseCliOptions(args);
                const ref = {
                    entityType: 'research',
                    entityId: String(num),
                };
                await assertNoForeignActiveLeases(repoPath, ref, {
                    offline: getOptionValue(closeOptions, 'offline') !== undefined,
                });
                leaseCoord = await coordinateMutatingCommand(repoPath, {
                    entityType: 'research',
                    entityId: num,
                    role: 'close',
            ...
            try {
                ...
                if (fs.existsSync(telemetryDir)) {
                    const files = fs.readdirSync(telemetryDir)
                        .filter(f => f.startsWith(`research-${num}-`) && (f.endsWith('.json') || f.endsWith('.json.gz')));
                    if (files.length > 0) {
                        let inputTokens = 0, outputTokens = 0, costUsd = 0, model = null, sessions = 0;
                        for (const file of files) {
                            try {
                                const data = JSON.parse(readTelemetryFile(path.join(telemetryDir, file)));
                ...
                const implementedBy = deriveImplementationProvenance(repoPath, 'research', num, {
                    stats: existing,
                    preferActive: true,
                });
                const merged = writeStats(repoPath, 'research', num, {
                ...
                try {
                    await recordCanonicalStats(repoPath, 'research', num, merged);
                } catch (err) {
            ...
            if (leaseCoord) {
                ...
                if (typeof leaseCoord.store.sync === 'function') {
                    const syncResult = await leaseCoord.store.sync();
                    if (syncResult && syncResult.ok === false) {
                        console.warn(`⚠️  Closed research ${num}, but could not publish storage state: ${syncResult.error || 'unknown error'}`);
        ...
        'research-delete': async (args) => {
            await entity.entityDelete(def, args[0], ctx);
        },

        'research-submit': () => {
            console.error(
                "❌ `aigon research-submit` was removed in v3.\n" +
                "   Use: aigon agent-status research-complete <ID> <agent>\n" +
                "   (Or run `aigon agent-status research-complete` with no args from inside the tmux session.)"
            );
            process.exitCode = 1;
        },
        ...
        'research-open': (args) => {
            ...
            if (terminalApp === 'warp') {
                ...
                const sessionResults = ensureAgentSessions(researchNum, agentConfigs.map(c => c.agent), {
                    ...
                    sessionMetaBuilder: (sessionName, rid, agent, sessionCwd) => ({
                        repoPath: path.resolve(process.cwd()),
                        entityType: 'r',
                        entityId: rid,
                        agent,
                        role: 'do',
                        worktreePath: path.resolve(sessionCwd),
                    }),
                });
                ...
                agentConfigs.forEach(config => {
                    ...
                    try { openTerminalAppWithCommand(cwd, `tmux attach -t ${shellQuote(sessionName)}`, sessionName, terminalApp); }
        ...
        'research-autopilot': async (args) => {
            ...
            if (subcommand === 'status') {
                ...
                const found = findFile(PATHS.research, idArg, [STAGE_FOLDERS.IN_PROGRESS]);
            ...
            let agentIds = positionalAgents.length > 0
                ? positionalAgents
                : (conductorConfig.defaultAgents || agentRegistry.getDefaultFleetAgents(process.cwd()));
            ...
            let found = findFile(PATHS.research, researchId, [STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.IN_PROGRESS]);
            ...
            agentIds.forEach(agentId => {
                ...
                if (spawnSnapshot && spawnSnapshot.agents && spawnSnapshot.agents[agentId]) {
                    ...
                    if (engineStatus === 'ready') {
                        console.log(`   ✓ ${agentId} — already complete, skipping`);
                ...
                try {
                    createDetachedTmuxSession(sessionName, process.cwd(), cmd, {
                        repoPath: path.resolve(process.cwd()),
                        entityType: 'r',
                        entityId: researchNum,
                        agent: agentId,
                        role: 'do',
                        worktreePath: path.resolve(process.cwd()),
                    });
                } catch (e) {
            ...
            if (allAlreadySubmitted) {
                ...
                try {
                    while (!interrupted) {
                        ...
                        const ast = require('../agent-status');
                        spawnedAgents.forEach(({ agent }) => {
                            let status = 'unknown'; let updatedStr = '';
                            ...
                            const rowFile = ast.readAgentStatus(researchNum, agent, 'research');
                            const fileQuota = rowFile && String(rowFile.status) === 'quota-paused';
                            const displayStatus = fileQuota ? 'quota-paused' : status;
                            previousStatuses[agent] = status;
                            if (!(status === 'ready' || fileQuota)) allSubmitted = false;
                            statusRows.push({ agent, status: displayStatus, updatedStr });
                        });
