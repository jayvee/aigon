...
module.exports = function researchCommands(ctx) {
    ...
    async function ensureResearchEngineState(repoPath, num, { mode = 'solo', lifecycle = 'implementing' } = {}) {
        if (!await wf.showResearchOrNull(repoPath, num)) {
            await wf.migrateEntityLifecycleIfNeeded({ repoPath, entityType: 'research', entityId: num, mode, agents: [], lifecycle });
        }
    }
    ...
    return {
        ...
        'research-eval': async (args) => {
            ...
            if (!setupOnly) {
                const repoPath = process.cwd();
                const lifecycle = spec.found.folder === '04-in-evaluation' ? 'evaluating' : 'implementing';
                await ensureResearchEngineState(repoPath, spec.num, { lifecycle });
        ...
        'research-close': async (args) => {
            ...
            const repoPath = process.cwd();
            const lifecycle = closeSpec.found.folder === '04-in-evaluation' ? 'evaluating' : 'implementing';
            await ensureResearchEngineState(repoPath, num, { mode: isFleetMode ? 'fleet' : 'solo', lifecycle });
...
function createResearchCommands(overrides = {}) {
    ...
    const names = [
        ...
        'research-do', 'research-submit', 'research-review', 'research-eval', 'research-close', 'research-reset',
        'research-autopilot'
    ];
