...
module.exports = function researchCommands(ctx) {
    ...
    function collectIncompleteResearchAgents(researchNum) {
        ...
        findingsFiles.forEach(file => {
            ...
            if (!m) return;
            const agent = m[1];
            const statusState = readAgentStatus(researchNum, agent, 'research');
            const status = statusState ? (statusState.status || 'unknown') : 'unknown';
            if (status !== 'submitted') {
    ...
    return {
        ...
        'research-start': (args) => {
            ...
            wf.startResearchSync(process.cwd(), spec.num, workflowMode, agentIds);

            const movedFromBacklog = spec.found.folder === '02-backlog';
            if (movedFromBacklog) {
                try {
                    ctx.git.runGit('git add docs/specs/research-topics/');
                    ctx.git.runGit(`git commit -m "chore: start research ${spec.num} - move spec to in-progress"`);
        ...
        'research-eval': (args) => {
            ...
            if (!setupOnly) {
                wf.requestResearchEvalSync(process.cwd(), spec.num);
        ...
        'research-close': (args) => {
            ...
            wf.closeResearchSync(process.cwd(), num);
