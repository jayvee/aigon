...
module.exports = function researchCommands(ctx) {
    ...
    return {
        ...
        'research-start': (args) => {
            const result = entity.entityStart(def, args, ctx);
            if (!result) return;

            const { mode, num, desc, agentIds, backgroundMode } = result;
            ...
            if (mode === 'fleet') {
                ...
                agentIds.forEach(agentId => {
                    ...
                    // Mark worktree op complete (if any)
                    ctx.stateMachine.completePendingOp(num, `create-worktree-${agentId}`, def.prefix);
                });

                console.log(`\n🚛 Fleet mode started with ${agentIds.length} agents!`);
                console.log(`\n📋 Research topic: ./docs/specs/research-topics/03-in-progress/${result.found.file}`);
                ...
                console.log(`📋 Topic: ./docs/specs/research-topics/03-in-progress/${result.found.file}`);
        ...
        'research-eval': (args) => {
            ...
            const result = entity.entityEval(def, id, ctx, { force: forceEval, setupOnly });
            if (!result && !setupOnly) return;
            if (setupOnly) return;
        ...
        'research-close': (args) => {
            ...
            const closeResult = entity.entityClose(def, id, ctx);
            ...
            // Finalize: move spec to done, close sessions, commit
            entity.entityCloseFinalize(def, closeResult, ctx);
        ...
        'research-autopilot': (args) => {
            ...
            if (subcommand === 'status') {
                ...
                findingsFiles.forEach(file => {
                    ...
                    try {
                        const agentState = readAgentStatus(researchNum, agentId, 'research');
                        if (agentState) {
                ...
                try {
                    while (!interrupted) {
                        ...
                        spawnedAgents.forEach(({ agent }) => {
                            let status = 'unknown', updatedStr = '';
                            try {
                                const agentState = readAgentStatus(researchNum, agent, 'research');
                                if (agentState) {
                                    status = agentState.status || 'unknown';
                                    if (agentState.updatedAt) {
                                        const diffMin = Math.floor((Date.now() - new Date(agentState.updatedAt).getTime()) / 60000);
                                        updatedStr = diffMin < 1 ? 'just now' : `${diffMin}m ago`;
                                    }
                                }
                            } catch (e) { /* skip */ }
