...
module.exports = function researchCommands(ctx) {
    ...
    // Helper functions for eval agent checking
    function collectIncompleteResearchAgents({ researchNum, logsDir, loadAgentConfigFn }) {
    ...
    return {
        ...
        'research-start': (args) => {
            ...
            if (!alreadyInProgress) {
                ...
                // Commit the spec move (important: agent sessions need to see the spec in the right directory)
                try {
                    runGit('git add docs/specs/research-topics/');
                    runGit(`git commit -m "chore: start research ${researchNum} - move spec to in-progress"`);
            ...
            if (mode === 'fleet') {
                ...
                console.log(`\n   When all agents finish: aigon research-eval ${researchNum}`);
            } else {
        ...
        'research-eval': (args) => {
            ...
            const found = findFile(PATHS.research, id, ['03-in-progress', '04-in-evaluation']);
            if (!found) return console.error(`❌ Could not find research "${id}" in progress or in-evaluation.\n\nRun 'aigon research-start ${id} [agents...]' first.`);
            ...
            if (found.folder === '03-in-progress') {
                if (!forceEval) {
                    const logsDir = path.join(PATHS.research.root, 'logs');
                    const incompleteAgents = collectIncompleteResearchAgents({ researchNum, logsDir, loadAgentConfigFn: loadAgentConfig });
                    if (incompleteAgents.length > 0) {
                        ...
                        console.log('');
                        console.log(`   To proceed anyway: aigon research-eval ${researchNum} --force`);
                        console.log('');
                        return;
                    }
                }
                moveFile(found, '04-in-evaluation', null, { actor: 'cli/research-eval' });
                try {
                    runGit('git add docs/specs/research-topics/');
                    runGit(`git commit -m "chore: move research ${researchNum} to evaluation"`);
                } catch (e) { /* already committed or nothing to commit */ }
                console.log(`📋 Research ${researchNum} moved to in-evaluation.`);
            }

            // --setup-only: just do the transition, don't print agent warning
            // (used by dashboard which opens its own agent session)
            if (setupOnly) return;
        ...
        'research-close': (args) => {
            ...
            if (!id) return console.error("Usage: aigon research-close <ID>\n\nCloses research by moving it to done.\nPrefers in-evaluation → done; also allows in-progress → done (with warning).");

            // Look in in-evaluation first, then in-progress
            let found = findFile(PATHS.research, id, ['04-in-evaluation']);
            let skippingEval = false;
            if (!found) {
                found = findFile(PATHS.research, id, ['03-in-progress']);
                if (found) skippingEval = true;
            }
            if (!found) return console.error(`❌ Could not find research "${id}" in in-evaluation or in-progress.`);
            ...
            if (skippingEval) {
                console.log(`⚠️  Research ${researchNum} is still in-progress (eval hasn't run). Closing anyway.`);
            }
            ...
            // Move to done
            moveFile(found, '05-done', null, { actor: 'cli/research-close' });
        ...
        'research-autopilot': (args) => {
            ...
            const autoEval = getOptionValue(options, 'auto-eval') !== undefined
                || getOptionValue(options, 'auto-synthesize') !== undefined; // backward compat
            ...
            if (autoEval) {
                ...
                console.log(`   aigon research-eval ${researchNum}`);
            }
        },
    };
};
...
function createResearchCommands(overrides = {}) {
    ...
    const names = [
        ...
        'research-do', 'research-submit', 'research-eval', 'research-close',
        'research-autopilot'
    ];
