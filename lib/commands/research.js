...
module.exports = function researchCommands(ctx) {
    ...
    return {
        ...
        'research-close': (args) => {
            ...
            // Auto-detect if synthesis already ran (check for Output/Selected Features in the doc)
            let synthesisComplete = false;
            try {
                const specContent = fs.readFileSync(path.join(found.dir, found.file), 'utf8');
                synthesisComplete = /^## (?:Output|Recommendation)\b/m.test(specContent) && /Selected Features/i.test(specContent);
            } catch (_) {}

            if (isFleetMode && !forceComplete && !synthesisComplete) {
                // Fleet mode without synthesis: Show summary and suggest synthesize
