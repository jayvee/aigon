...
module.exports = function researchCommands(ctx) {
    ...
    return {
        ...
        'research-start': (args) => {
            ...
            // Guard: already running — tell user to re-attach instead
            if (found.folder === '03-in-progress') {
                const runningId = String(parseInt(researchNum, 10) || researchNum);
                console.log(`ℹ️  Research ${runningId} is already running. Use \`research-open ${runningId}\` to re-attach.`);
                return;
            }
            ...
            if (mode === 'fleet') {
                ...
                if (backgroundMode) {
                    try {
                        ...
                        console.error(`\n❌ ${e.message}`);
                        console.error('   Background mode requires tmux. Install: brew install tmux');
                        return;
                    }
                    console.log(`\n🟡 Background mode enabled — spawning detached tmux sessions...`);
                    agentIds.forEach(agentId => {
                        const sessionName = buildResearchTmuxSessionName(researchNum, agentId);
                        const command = buildResearchAgentCommand(agentId, researchNum);
                        const existing = safeTmuxSessionExists(researchNum, agentId, { isResearch: true });
                        if (existing && existing.running) {
                            console.log(`   ✓ ${agentId}: ${existing.sessionName} (already running)`);
                            return;
                        }
                        try {
                            createDetachedTmuxSession(sessionName, process.cwd(), command);
                            console.log(`   ✓ ${agentId}: ${sessionName}`);
                        } catch (e) {
                            console.warn(`   ⚠️  ${agentId}: failed to start session (${e.message})`);
                        }
                    });
                    console.log(`   View all sessions anytime: aigon research-open ${researchNum}`);
                }
            } else {
