...
module.exports = function researchCommands(ctx) {
    ...
    function resolveReviewAgentFromOptions(options, fallbackAgent = 'cc') {
        ...
        const resolvedAgent = agentArgRaw
            ? (aliasMap[String(agentArgRaw).toLowerCase()] || String(agentArgRaw).toLowerCase())
            : fallbackAgent;
    ...
    function launchPromptCommand({ commandName, entityId, agentId, modelTask = 'review' }) {
        const cliConfig = getAgentCliConfig(agentId);
        const prompt = resolveAgentCommandPrompt({
            agentId,
            commandName,
            argsString: String(entityId),
            cliConfig,
        });
        ...
        const result = require('child_process').spawnSync(cliConfig.command, spawnArgs, { stdio: 'inherit', env });
        if (result.error) {
    ...
    function handleResearchSpecPromptCommand(args, commandName) {
        ...
        const spec = resolveResearchSpec(id, ['01-inbox', '02-backlog', '03-in-progress', '04-in-evaluation', '05-done', '06-paused']);
        if (!spec) {
            console.error(`❌ Could not resolve research spec "${id}".`);
            return;
        }

        const sessionInfo = detectActiveAgentSession();
        if (!sessionInfo.detected && !args.includes('--no-launch')) {
        ...
        console.log(`📋 Spec: ./${path.relative(process.cwd(), spec.found.fullPath)}`);
    }

    async function recordResearchSpecReviewFromHead(id, kind) {
        if (!id) {
            console.error(`Usage: aigon research-spec-review${kind === 'ack' ? '-check' : ''}-record <ID|slug>`);
            process.exitCode = 1;
            return;
        }
        const spec = resolveResearchSpec(id, ['01-inbox', '02-backlog', '03-in-progress', '04-in-evaluation', '05-done', '06-paused']);
        ...
        try {
            ...
            if (kind === 'review') {
                await wf.recordSpecReviewSubmitted(process.cwd(), 'research', spec.num, {
                    reviewId: commit.sha,
                ...
                await wf.recordSpecReviewAcknowledged(process.cwd(), 'research', spec.num, {
                    ackedBy: process.env.AIGON_AGENT_ID || null,
                    commitSha: commit.sha,
                });
            }
            console.log(`✅ Recorded research spec review state for ${spec.num}`);
        } catch (error) {
    ...
    return {
        ...
        'research-spec-review-record': async (args) => {
            await recordResearchSpecReviewFromHead(args[0], 'review');
        },

        'research-spec-review-check-record': async (args) => {
            await recordResearchSpecReviewFromHead(args[0], 'ack');
        },

        'research-spec-review': (args) => {
            handleResearchSpecPromptCommand(args, 'research-spec-review');
        },

        'research-spec-review-check': (args) => {
            handleResearchSpecPromptCommand(args, 'research-spec-review-check');
        },
        ...
        'research-reset': async (args) => {
            ...
            let specMoved = false;
            const found = findFile(PATHS.research, id, ['03-in-progress', '04-in-evaluation', '06-paused']);
            if (found) {
                const targetDir = path.join(PATHS.research.root, '02-backlog');
            ...
            try {
                const result = await wf.resetResearch(process.cwd(), paddedId);
                if (result.removed) {
                    engineRemoved = true;
                    console.log(`   🗑️  Removed workflow engine state: .aigon/workflows/research/${paddedId}/`);
                }
            } catch (e) {
            ...
            console.log(
                `\n✅ Research reset complete: sessions ${sessionsClosed ? 'closed' : 'attempted'}, ` +
...
function createResearchCommands(overrides = {}) {
    ...
    const names = [
        ...
        'research-do', 'research-submit', 'research-review', 'research-spec-review', 'research-spec-review-check',
        'research-spec-review-record', 'research-spec-review-check-record',
        'research-eval', 'research-close', 'research-reset',
        'research-autopilot'
    ];
