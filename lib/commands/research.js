...
const { stagePaths, stageAndCommitPaths } = require('../git-staging');
...
module.exports = function researchCommands(ctx) {
    ...
    return {
        ...
        'research-unprioritise': async (args) => {
            ...
            const found = findFile(PATHS.research, paddedId, ['02-backlog']);
            if (!found) return console.error(`❌ Could not find research "${paddedId}" in backlog.`);

            const specFromPath = path.join(PATHS.research.root, '02-backlog', found.file);
            const numericNamed = /^research-\d+-(.+)\.md$/.exec(found.file);
            let inboxFilename = found.file;
            if (numericNamed) {
                inboxFilename = `research-${numericNamed[1]}.md`;
            }
            const specToPath = path.join(PATHS.research.root, '01-inbox', inboxFilename);
            ...
            try {
                ...
                stageAndCommitPaths(runGit, process.cwd(), stagedPaths, `chore: unprioritise research ${workflowId} - move to inbox`);
