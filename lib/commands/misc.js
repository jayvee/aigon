    ...
    return {
        ...
        'agent-status': async (args) => {
            ...
            const validStatuses = [
                ...
                'implementing', 'reviewing', 'revising', 'spec-reviewing',
            ...
            // `submitted` is remapped based on the recorded taskType when we have one
            // (so a revision session's trap-fired `submitted` advances correctly).
            const SUBMITTED_ALIAS = 'submitted';
            const FEEDBACK_ADDRESSED_ALIAS = 'feedback-addressed';
            const isDeprecatedSubmitted = status === SUBMITTED_ALIAS;
            const isDeprecatedFeedbackAddressed = status === FEEDBACK_ADDRESSED_ALIAS;
            if (isDeprecatedSubmitted) {
                console.warn(`⚠️  'aigon agent-status submitted' is deprecated. Use 'implementation-complete' (initial implementation) or 'revision-complete' (after addressing review feedback).`);
            }
            ...
            const completionSignals = new Set(['submitted', 'implementation-complete', 'revision-complete', 'research-complete', 'spec-review-complete']);
            ...
            // Deprecated `submitted` → canonical completion signal. The branch-context block
            // below is skipped for explicit `agent-status submitted <id> <agent>`; without this
            // remap, status stays `submitted` and never matches isReadyCompletion (F339 regression).
            if (isDeprecatedSubmitted) {
                const _readPrefix = entityType === 'research' ? 'research' : 'feature';
                const existingState = readAgentStatus(featureNum, agentId, _readPrefix, { mainRepoPath: mainRepo });
                const recordedTaskType = existingState && typeof existingState.taskType === 'string' ? existingState.taskType : null;
                if (recordedTaskType === 'revise') {
            ...
            const isFeatureSubmit = (status === 'submitted' || status === 'implementation-complete' || status === 'revision-complete') && entityType === 'feature';
            if (isFeatureSubmit) {
            ...
            if (status === 'reviewing' || status === 'review-complete') {
                try {
                    if (status === 'reviewing') {
                        await wf.recordCodeReviewStarted(mainRepo, entityType, featureNum, {
                            reviewerId: agentId,
                            source: 'agent-status/reviewing',
                        });
                    } else {
                        await wf.recordCodeReviewCompleted(mainRepo, entityType, featureNum, {
                            reviewerId: agentId,
                            requestRevision: true,
                            source: 'agent-status/review-complete',
                        });
                    }
            ...
            // Completion signals that drive the workflow engine to "agent ready" state.
            const isReadyCompletion = status === 'implementation-complete' || status === 'research-complete';
            const isRevisionCompletion = status === 'revision-complete';
            ...
            const startTaskType = process.env.AIGON_TASK_TYPE
                || (status === 'implementing' ? 'do'
                    ...
                    : status === 'revising' ? 'revise'
                    : status === 'spec-reviewing' ? 'spec-review'
                    : null);
            const isStartSignal = status === 'implementing' || status === 'reviewing' || status === 'revising' || status === 'spec-reviewing';

            if (isReadyCompletion || isRevisionCompletion) {
                if (!hasWorkflowState) {
                    console.error(`❌ Cannot signal '${status}' for ${entityType} ${featureNum}: workflow state is not initialized.`);
                    console.error(`   Run the appropriate start command again or repair the workflow state before retrying.`);
                    process.exitCode = 1;
                    return;
                }
                try {
                    if (isRevisionCompletion) {
                        await wf.recordCodeRevisionCompleted(mainRepo, entityType, featureNum, {
                            revisionAgentId: agentId,
                            source: `agent-status/${status}`,
                        });
                    }
                    await wf.emitSignal(mainRepo, featureNum, 'agent-ready', agentId, { entityType });
                } catch (err) {
            ...
            const isCompletion = isReadyCompletion || isRevisionCompletion || status === 'spec-review-complete';
            const cachePayload = {
                status,
            ...
            if (isCompletion) {
                // Clear taskType after a successful completion so a subsequent shell-trap
                // exit doesn't trip the mismatch check against a stale value.
                cachePayload.taskType = null;
                cachePayload.flags = {};
                // Clear any stale quota-pause fields so the dashboard doesn't show
                // a paused badge after the agent self-signals completion.
                cachePayload.quotaPausedAt = undefined;
                cachePayload.quotaPauseMeta = undefined;
                cachePayload.priorQuotaStatus = undefined;
            } else if (isStartSignal && startTaskType) {
                cachePayload.taskType = startTaskType;
            }
            writeAgentStatusAt(mainRepo, featureNum, agentId, cachePayload, manifestPrefix);

            // Emit non-completion engine signals alongside legacy writes.
            if (hasWorkflowState) {
        ...
        'check-agent-signal': (args = []) => {
            ...
            const SIGNALED = new Set(['submitted', 'implementing', 'implementation-complete', 'revising', 'revision-complete']);
        ...
        'capture-gemini-telemetry': async () => {
            // Gemini CLI has no SessionEnd event, so we wire this command into
            // the AfterAgent hook (fires after every model response). Each
            // firing rewrites the same normalized record (keyed by Gemini
            // sessionId) — the final firing wins with the complete session.
            // No log frontmatter update — bench sessions aren't worktree
            // logs and the cc captureSessionTelemetry log path is cc-specific.
            try {
                const repoPath = process.env.AIGON_PROJECT_PATH || process.cwd();
                ...
                if (process.env.AIGON_ENTITY_TYPE && process.env.AIGON_ENTITY_ID && process.env.AIGON_AGENT_ID) {
                    ...
                    if (!branch) return;
                    const m = branch.match(/^feature-(\d+)-gg-(.+)$/);
                    if (!m) return;
                    featureId = m[1];
                    agentId = 'gg';
                }
                if (agentId !== 'gg') return;

                if (process.env.AIGON_ACTIVITY) activity = process.env.AIGON_ACTIVITY;

                telemetry.parseGeminiTranscripts(repoPath, {
                    featureId,
                    entityType,
                    repoPath,
                    activity,
        ...
        'token-window': async (args) => {
            ...
            try {
                const { getEnrichedSessions } = require('../worktree');
                sessions = getEnrichedSessions().sessions || [];
            } catch (e) {
                sessions = [];
            }
        ...
        'perf-bench': async (args = []) => {
            const perfBench = require('../perf-bench');

            if (args.includes('--help') || args.includes('-h')) {
                console.log(`Usage: aigon perf-bench [seed] [agent] [options]

Measure end-to-end agent run time on a deterministic seed repo.
...
Options:
  --all                  Sweep every non-quarantined agent/model pair
  --agents <ids>         Comma-separated agent filter for --all
  --model <id>           Model override for a single run
  --effort <level>       Effort override for a single run
  --judge                Add LLM-judged quality scoring
  --check [file]         Compare a run or result file to baseline.json
  --save-baseline        Save a single run as baseline.json
  --skip-baseline        Skip bare-provider baseline capture
  --skip-quota-check     Include depleted pairs in --all sweeps
  --dry-run              Print planned run data without executing agents
  --timeout <seconds>    Override run timeout

Committed JSON runs in .aigon/benchmarks/ are the shipped reference data for
each release. Before tagging, run:
  aigon perf-bench brewboard --all --judge`);
                return;
            }

            // Parse flags: --flag and --flag value (next token) forms
            const flags = new Set();
            const kv = {};
            for (let i = 0; i < args.length; i++) {
                ...
                if (a.includes('=')) {
                    ...
                    kv[a.slice(2)] = args[i + 1];
                    i++;
                } else {
                    flags.add(a);
                }
            }
            const positionals = args.filter((a, i) => !a.startsWith('--') && (i === 0 || !args[i - 1].startsWith('--') || args[i - 1].includes('=')));

            const seedName = positionals[0] || kv.seed || 'brewboard';
            const dryRun = flags.has('--dry-run');
            const timeoutMs = kv.timeout ? parseInt(kv.timeout, 10) * 1000 : undefined;

            // --check: compare a result file (or fresh run) to baseline
            if (flags.has('--check')) {
                const fileArg = positionals[0];
                let current;
                if (fileArg && fs.existsSync(fileArg)) {
                    current = JSON.parse(fs.readFileSync(fileArg, 'utf8'));
                } else {
                    current = await perfBench.runBenchmark({
                        seedName: kv.seed || 'brewboard',
                        agent: kv.agent || positionals[0] || 'cc',
                        model: kv.model || null,
                        effort: kv.effort || null,
                        repoPath: process.cwd(),
                        skipBaseline: flags.has('--skip-baseline'),
                        timeoutMs,
                    });
                }
                const cmp = perfBench.compareToBaseline(process.cwd(), current);
                if (!cmp.baseline) {
                    console.log(`ℹ️  No baseline.json yet. Run 'aigon perf-bench --save-baseline' to record one.`);
                    return;
                }
                const arrow = cmp.deltaPct >= 0 ? '↑' : '↓';
                console.log(`Current: ${current.totalMs}ms  Baseline: ${cmp.baseline.totalMs}ms  Δ ${arrow}${Math.abs(cmp.deltaPct).toFixed(1)}%`);
                if (cmp.regressed) {
                    console.error(`❌ Regression: ${cmp.deltaPct.toFixed(1)}% > ${perfBench.REGRESSION_THRESHOLD_PCT}% threshold`);
                    process.exitCode = 1;
                } else {
                    console.log(`✅ Within ${perfBench.REGRESSION_THRESHOLD_PCT}% threshold`);
                }
                return;
            }

            const judgeOpts = flags.has('--judge') || kv.judge != null
                ? {
                    judge: true,
                    judgeBinary: kv['judge-binary'] || undefined,
                    judgeAgentId: kv['judge-agent'] || undefined,
                    judgeModel: kv['judge-model'] || undefined,
                    judgeTimeoutMs: kv['judge-timeout'] ? parseInt(kv['judge-timeout'], 10) * 1000 : undefined,
                }
                : {};

            // --all: sweep every configured (agent, model) pair at default effort
            if (flags.has('--all')) {
                const agentFilter = kv.agents ? kv.agents.split(',').map(s => s.trim()) : [];
                try {
                    await perfBench.runAllBenchmarks({
                        seedName,
                        agents: agentFilter,
                        repoPath: process.cwd(),
                        skipBaseline: true, // skipped in sweeps; saves ~2 min per run
                        timeoutMs,
                        dryRun,
                        skipQuotaCheck: flags.has('--skip-quota-check'),
                        ...judgeOpts,
                    });
                } catch (err) {
            ...
            // Single run: aigon perf-bench <seed> <agent> [--model x] [--effort y]
            const agent = positionals[1] || kv.agent || 'cc';
            try {
                const result = await perfBench.runBenchmark({
                    seedName,
                    agent,
                    model: kv.model || null,
                    effort: kv.effort || null,
                ...
                const phaseLine = result.phases.map(p => `${p.name}=${Math.round(p.ms / 1000)}s`).join('  ');
                console.log(`\n📊 ${result.seed} #${result.featureId}  ${result.agent}${result.model ? `  ${result.model}` : ''}${result.effort ? `  effort=${result.effort}` : ''}`);
                console.log(`   total: ${Math.round(result.totalMs / 1000)}s  baseline: ${result.baselineMs != null ? Math.round(result.baselineMs / 1000) + 's' : '—'}  overhead: ${result.overheadMs != null ? Math.round(result.overheadMs / 1000) + 's' : '—'}`);
                console.log(`   ${phaseLine}`);
                if (flags.has('--save-baseline')) {
                    perfBench.saveBaselineFile(process.cwd(), result);
                    console.log(`✅ Saved as baseline.json`);
                }
            } catch (err) {
                console.error(`❌ ${err.message}`);
                process.exitCode = 1;
        ...
        'agent-probe': async (args = []) => {
            if (args.includes('--quota')) {
                ...
                const benchIndex = includeBench ? benchHydrate.hydrateBenchVerdicts(process.cwd()) : {};
                ...
                for (const agentId of agentIds) {
                    ...
                    for (const targetInfo of targets) {
                        ...
                        let benchCol = '';
                        if (includeBench) {
                            const found = benchIndex[benchHydrate.benchKey(agentId, targetInfo.value)];
                            if (!found || found.benchVerdict === 'unknown') benchCol = '  bench=not tested';
                            else if (found.benchVerdict === 'passed') benchCol = '  bench=passed';
                            else benchCol = `  bench=failed ${found.lastBenchAt ? found.lastBenchAt.slice(0, 10) : ''}`.trimEnd();
                        }
                        console.log(`${entry.verdict.padEnd(9)}  reset=${reset}  probed=${entry.lastProbedAt}${benchCol}`);
                        if (debug) {
        ...
        'agent-quarantine': async (args = []) => {
            const transcriptStore = require('../transcript-store');
            const matrixApply = require('../matrix-apply');
            const repoPath = process.cwd();
            ...
            if (!agentId || !modelId) {
                console.error('Usage: aigon agent-quarantine <agentId> <modelId> [--dry-run]');
                console.error('Example: aigon agent-quarantine cc claude-opus-4-5-20251101');
                process.exitCode = 1;
                return;
            }

            // Snapshot active transcripts before altering the matrix
            const snapshotResult = transcriptStore.snapshotQuarantineTranscripts(repoPath, agentId, modelId);
            if (snapshotResult.copied > 0 || snapshotResult.skipped > 0) {
                console.log(`📄 Quarantine snapshot: ${snapshotResult.copied} transcript(s) archived to ${snapshotResult.dir}`);
                if (snapshotResult.skipped > 0) console.log(`   (${snapshotResult.skipped} session(s) had no captured body)`);
            } else {
                console.log(`📄 No active sessions found for agent ${agentId} — snapshot dir: ${snapshotResult.dir}`);
            }

            if (dryRun) {
                console.log(`[dry-run] Would mark ${agentId} / ${modelId} as quarantined in agent matrix`);
                return;
            }

            // Mark the model as quarantined in the agent matrix JSON (best-effort)
            try {
                const agentJsonPath = require('path').join(__dirname, '..', '..', 'templates', 'agents', `${agentId}.json`);
                const fs = require('fs');
                if (fs.existsSync(agentJsonPath)) {
                    const config = JSON.parse(fs.readFileSync(agentJsonPath, 'utf8'));
                    const opts = config.cli && config.cli.modelOptions;
                    if (Array.isArray(opts)) {
                        const opt = opts.find(o => o.value === modelId);
                        if (opt) {
                            opt.quarantined = { at: new Date().toISOString(), reason: 'manual-quarantine' };
                            fs.writeFileSync(agentJsonPath, JSON.stringify(config, null, 2) + '\n');
                            console.log(`✅ Marked ${agentId} / ${modelId} as quarantined in ${agentJsonPath}`);
                            console.log(`   Run 'aigon server restart' to pick up the updated agent config.`);
                        } else {
                            console.warn(`⚠️  Model ${modelId} not found in ${agentId} agent config — snapshot taken but matrix not updated.`);
                        }
                    }
                } else {
                    console.warn(`⚠️  Agent config not found: ${agentJsonPath}`);
                }
            } catch (err) {
                console.error(`❌ Failed to update agent matrix: ${err.message}`);
                process.exitCode = 1;
            }
        },

        'matrix-apply': async (args = []) => {
            const matrixApply = require('../matrix-apply');
            const repoPath = process.cwd();
            const dryRun = args.includes('--dry-run');
            const feedbackId = args.find(a => !a.startsWith('--'));

            if (!feedbackId) {
                console.error('Usage: aigon matrix-apply <feedback-id> [--dry-run]');
                console.error('Example: aigon matrix-apply 42');
                process.exitCode = 1;
                return;
            }

            try {
                const results = matrixApply.applyFeedback(repoPath, feedbackId, { dryRun });
                const prefix = dryRun ? '[dry-run] Would apply' : 'Applied';
                for (const result of results) {
                    console.log(`✅ ${prefix}: ${result.changeKind} to ${result.agentId} / ${result.modelValue || '(new)'}`);
                }
                const patchFile = results[0].patchFile;
                const filePaths = [...new Set(results.map(r => r.filePath))];
                for (const f of filePaths) console.log(`   Agent file: ${f}`);
                console.log(`   Patch from: ${patchFile}`);
                if (!dryRun) {
                    console.log(`\n   Run 'aigon server restart' to pick up the updated agent config.`);
                }
            } catch (err) {
                console.error(`❌ ${err.message}`);
                process.exitCode = 1;
