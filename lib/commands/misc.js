...
module.exports = function miscCommands(ctx) {
    ...
    return {
        ...
        'agent-status': async (args) => {
            ...
            if (!hasExplicitArgs && (status === 'implementation-complete' || status === 'revision-complete' || status === 'spec-review-complete' || status === 'research-complete' || isDeprecatedSubmitted)) {
                ...
                if (isDeprecatedSubmitted) {
                    // Resolve the deprecated alias to the new canonical signal based on taskType.
                ...
                } else if (recordedTaskType && expectedByStatus[status] && !expectedByStatus[status].has(recordedTaskType)) {
        ...
        'perf-bench': async (args = []) => {
            ...
            // --check: compare last/current run to stored baseline. With a path
            // argument, compares that file. With no positional, runs a fresh bench.
            if (flags.has('--check')) {
                const fileArg = positionals[0];
                let current;
                if (fileArg) {
                    if (!fs.existsSync(fileArg)) {
                        console.error(`❌ Result file not found: ${fileArg}`);
                        process.exitCode = 1;
                        return;
                    }
                    ...
                    current = await perfBench.runBenchmark({
                        seedName: kvFlags.seed || 'brewboard',
                        agent: kvFlags.agent || 'cc',
                        repoPath: process.cwd(),
                        skipBaseline: flags.has('--skip-baseline'),
                        timeoutMs: kvFlags.timeout ? parseInt(kvFlags.timeout, 10) * 1000 : undefined,
                    });
                }
            ...
            const seedName = positionals[0] || kvFlags.seed || 'brewboard';
            ...
            try {
                ...
                const phaseLine = result.phases.map(p => `${p.name}=${p.ms}ms`).join(' ');
                console.log(`\n📊 ${result.seed} #${result.featureId} via ${result.agent}`);
                console.log(`   total: ${result.totalMs}ms  baseline: ${result.baselineMs ?? '—'}  overhead: ${result.overheadMs ?? '—'}ms`);
                console.log(`   phases: ${phaseLine}`);
...
function createMiscCommands(overrides = {}) {
    ...
    const names = ['agent-status', 'repair', 'status', 'deploy', 'commits', 'insights', 'capture-session-telemetry', 'session-list', 'security-scan-commit', 'check-agent-signal', 'check-agent-submitted', 'next', 'workflow-rules', 'help', 'rollout', 'stats', 'token-window', 'perf-bench', 'matrix-apply'];
