...
module.exports = function miscCommands(ctx) {
    ...
    return {
        ...
        'repair': async (args = []) => {
            ...
            const currentRepoStatus = getStatus ? getStatus(repoPath) : '';
            const dirtyWorktrees = [];
            for (const wt of worktrees) {
                if (getStatus && getStatus(wt.path)) {
            ...
            if (stateFiles.length > 0) repairActions.push(`remove ${stateFiles.length} stale state file(s)`);
            if (heartbeatFiles.length > 0) repairActions.push(`remove ${heartbeatFiles.length} stale heartbeat file(s)`);
...
function createMiscCommands(overrides = {}) {
    ...
    const names = ['agent-status', 'status', 'deploy', 'commits', 'insights', 'capture-session-telemetry', 'security-scan-commit', 'check-agent-signal', 'check-agent-submitted', 'next', 'workflow-rules', 'help', 'rollout', 'stats'];
