...
module.exports = function miscCommands(ctx) {
    ...
    return {
        ...
        'matrix-apply': async (args = []) => {
            ...
            try {
                const result = matrixApply.applyFeedback(repoPath, feedbackId, { dryRun });
                const prefix = result.dryRun ? '[dry-run] Would apply' : 'Applied';
                console.log(`✅ ${prefix}: ${result.changeKind} to ${result.agentId} / ${result.modelValue || '(new)'}`);
                console.log(`   Agent file: ${result.filePath}`);
                console.log(`   Patch from: ${result.patchFile}`);
                if (!result.dryRun) {
