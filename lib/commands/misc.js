        ...
        'agent-status': (args) => {
            ...
            if (status === 'reviewing' || status === 'review-complete') {
                try {
                    ...
                    if (parsedSession && parsedSession.type === 'f' && /^review-[a-z]{2}$/.test(parsedSession.agent)) {
                        reviewSessionInfo = {
            ...
            if (entityType === 'feature' && (status === 'reviewing' || status === 'review-complete')) {
                try {
                    if (status === 'reviewing') {
                        featureReviewState.markReviewingSync(mainRepo, featureNum, agentId, new Date().toISOString());
                    } else {
                        featureReviewState.completeReviewSync(mainRepo, featureNum, agentId, new Date().toISOString());
                    }
                    console.log(`✅ Review status updated: ${status} (feature ${featureNum}, ${agentId})`);
                } catch (err) {
            ...
            // Opportunistic cmux status update (no-op if not running inside cmux)
            if (process.env.CMUX_WORKSPACE_ID) {
                const { cmuxSetStatus, cmuxNotify } = require('../worktree');
                const statusIcons = {
                    implementing: { icon: 'hammer', color: 'yellow' },
                    submitted: { icon: 'check', color: 'green' },
                    error: { icon: 'x', color: 'red' },
                    waiting: { icon: 'clock', color: 'blue' },
                    reviewing: { icon: 'eye', color: 'magenta' },
                };
                const meta = statusIcons[status] || {};
                cmuxSetStatus('stage', status, meta);
                cmuxSetStatus('feature', featureNum, { icon: 'tag' });

                if (status === 'submitted') {
                    cmuxNotify(`Agent ${agentId} submitted`, `Feature ${featureNum} ready for review`);
                } else if (status === 'error') {
                    cmuxNotify(`Agent ${agentId} error`, `Feature ${featureNum} encountered an error`);
                }
            }
