...
module.exports = function miscCommands(ctx) {
    ...
    return {
        ...
        'spec-review': (args) => {
            ...
            if (!['submit', 'ack'].includes(subcmd) || !['feature', 'research'].includes(entityType) || !entityId) {
                process.exitCode = 1;
                return console.error(
                    `Usage:\n` +
                    `  aigon spec-review submit <feature|research> <ID> --reviewer=<agentId> [--summary=<text>] [--commit-sha=<sha>]\n` +
                    `  aigon spec-review ack <feature|research> <ID> --acked-by=<agentId> [--notes=<text>] [--commit-sha=<sha>]`
                );
            }
            const options = u.parseCliOptions(args.slice(3));
            const getOpt = (name) => u.getOptionValue(options, name);
            const repoPath = process.cwd();
            const commitSha = getOpt('commit-sha') || null;

            try {
                if (subcmd === 'submit') {
                    const reviewerId = getOpt('reviewer') || process.env.AIGON_AGENT_ID;
                    const summary = getOpt('summary') || '';
                    specReviewState.recordSubmission(repoPath, entityType, entityId, { reviewerId, summary, commitSha });
                    console.log(`✅ spec-review submitted: ${entityType} ${entityId} — reviewer=${reviewerId}`);
                } else {
                    const ackedBy = getOpt('acked-by') || process.env.AIGON_AGENT_ID || 'unknown';
                    const notes = getOpt('notes') || null;
                    const { ackedCount } = specReviewState.recordAck(repoPath, entityType, entityId, { ackedBy, notes, commitSha });
                    console.log(`✅ spec-review acked: ${entityType} ${entityId} — ${ackedCount} pending review(s) resolved by ${ackedBy}`);
                }
            } catch (err) {
                process.exitCode = 1;
...
function createMiscCommands(overrides = {}) {
    ...
    const names = ['agent-status', 'repair', 'status', 'deploy', 'commits', 'insights', 'capture-session-telemetry', 'security-scan-commit', 'check-agent-signal', 'check-agent-submitted', 'next', 'workflow-rules', 'help', 'rollout', 'stats', 'spec-review'];
