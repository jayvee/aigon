            ...
            if (entityType === 'feature') {
                ...
                if (fs.existsSync(snapshotPath)) {
                    const signalMap = {
                        ...
                        'error': 'agent-failed',
                        'implementing': 'agent-started',
                        'waiting': 'agent-waiting',
                    };
                    const signal = signalMap[status];
                    if (signal) {
                        const signalPromise = wf.emitSignal(mainRepo, featureNum, signal, agentId);
                        // Also emit heartbeat for implementing status (refreshes liveness)
                        const heartbeatPromise = status === 'implementing'
                            ? emitHeartbeat(mainRepo, featureNum, agentId)
                            : Promise.resolve();
                        Promise.all([signalPromise, heartbeatPromise])
                            .catch((err) => {
