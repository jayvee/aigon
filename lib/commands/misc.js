...
module.exports = function miscCommands(ctx) {
    ...
    return {
        ...
        'security-scan-commit': () => {
            // PostCommit hook: scan the last commit for secrets using gitleaks.
            // Runs outside git's hook mechanism — cannot be bypassed by --no-verify.
            ...
            if (!isBinaryAvailable('gitleaks')) return;
...
function createMiscCommands(overrides = {}) {
    ...
    const names = ['agent-status', 'status', 'deploy', 'insights', 'capture-session-telemetry', 'security-scan-commit', 'next', 'help'];
