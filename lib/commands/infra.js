...
<dict>
    ...
    <string>${logPath}</string>
        ...
        'backup': async (args) => {
            // F693: the backup/vault engine is in-repo again (was @aigon/pro).
            const backup = require('../backup');
            try { return await backup.handleBackupCommand(args); }
            catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
        },
        ...
        'sync': async (args) => {
            ...
            if (args[0] === 'legacy') {
                ...
                try { return await sync.handleSyncCommand(args.slice(1)); }
                catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
            }
        ...
        'vault': async (args) => {
            // Vault is the user-facing alias for the unified backup engine (F706).
            const backup = require('../backup');
            try { return await backup.handleBackupCommand(args); }
            catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
        },
