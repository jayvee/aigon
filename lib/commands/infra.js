...
// parseFrontMatterStatus is needed by conductor (menubar-render, status subcommands)
    ...
    const commands = {
        ...
        'proxy': async (args) => {
            ...
            if (subcommand === 'start') {
                ...
                const listenPort = await isPortAvailableFn(80) ? 80 : 4100;
                const { spawn } = require('child_process');
                ...
                // Create launchd plist for auto-start on boot (macOS)
                const plistPath = path.join(os.homedir(), 'Library', 'LaunchAgents', 'com.aigon.proxy.plist');
...
<dict>
    ...
    <string>${path.join(os.homedir(), '.aigon', 'dev-proxy', 'proxy.log')}</string>
</dict>
</plist>`;
                ...
                console.log(`   Or restart your Mac for auto-start.`);
                ...
                console.log('Usage: aigon proxy <start|stop|install|status>');
                console.log('  start    — start the proxy daemon');
                console.log('  stop     — stop the proxy daemon');
                console.log('  install  — install launchd plist for auto-start on boot');
