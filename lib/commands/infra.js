...
module.exports = function infraCommands(ctx) {
    ...
    const commands = {
        ...
        'dashboard': async (args) => {
            ...
            // Keep dashboard as a compatibility alias. Server lifecycle lives on `aigon server`.
            if (!isPreview && (!sub || ['start', 'stop', 'restart', 'status'].includes(sub))) {
            ...
            const appId = isPreview ? `${SERVER_APP_ID}-preview` : SERVER_APP_ID;
            const proxyAvailable = isProxyAvailable();
    ...
    <string>${logPath}</string>
        ...
        'server': async (args) => {
            ...
            if (sub === 'stop') {
                ...
                // Always clear the canonical entry if present.
            ...
            console.error('Usage: aigon server [start|stop|restart|status]');
            console.error('');
