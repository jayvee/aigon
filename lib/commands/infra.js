...
const {
    ...
    getDashboardRuntimeEntry,
} = require('../server-runtime');
const { parseCliOptions } = require('../cli-parse');
const { runDashboardServer } = require('../dashboard-server');

function resolveCurrentRepoRoot() {
    try {
        // In a git worktree --show-toplevel returns the worktree path, not the main
        // repo. --git-common-dir always points to the shared .git dir whose parent
        // is the main repo root — use that instead.
        ...
        // Fallback for non-standard git layouts (bare repos, etc.)
...
module.exports = function infraCommands(ctx) {
    ...
    function resolveServerIdentity(cliOptions = {}) {
        const { resolveInstanceIdentity } = require('../instance-identity');
        return resolveInstanceIdentity({ forcePrimary: cliOptions.primary === true });
    }

    /**
     * Running dashboard for the effective AIGON_HOME + instance id — reads dashboard-runtime*.json only.
     * Returns { pid, port, startedAt?, version? } or null.
     */
    function getServerRegistryEntry(instanceId) {
        const identity = resolveServerIdentity();
        const slotId = instanceId || (
            (identity.isPrimary || identity.isEphemeral) ? 'main' : identity.instanceId
        );
        return getDashboardRuntimeEntry({ isProcessAlive, instanceId: slotId });
    }

    function getServerUrl(entry, identity) {
        const resolved = identity || resolveServerIdentity();
        if (isProxyAvailable()) {
            const hostname = resolved.caddyHost;
            const route = parseCaddyRoutes().find(r => r.hostname === hostname);
            if (route && (!entry || !entry.port || route.port === entry.port)) {
                return getDevProxyUrl(SERVER_APP_ID, resolved.caddyServerId);
            }
        }
        const { getConfiguredServerPort } = require('../config');
        const port = entry && entry.port ? entry.port : (resolved.port || getConfiguredServerPort());
    ...
    const commands = {
        ...
        'board': async (args) => {
            ...
            if (flags.has('--storage')) {
                ...
                const report = await runStorageReport();
                console.log(`\n📦 Cross-repo storage report (${report.specs.length} specs)\n`);
                for (const spec of report.specs) {
                    const leases = Object.entries(spec.leases || {})
                        ...
                        .join(', ');
                    console.log(`  ${spec.key.padEnd(8)} ${path.basename(spec.repo).padEnd(16)} ${leases || '(no active leases)'}`);
                }
                return;
            }
        ...
        'proxy': async (args) => {
            ...
            if (subcommand === 'start') {
                ...
                try {
                    if (u.isSystemCaddyDaemonInstalled && u.isSystemCaddyDaemonInstalled()) {
                        if (!u.kickstartSystemCaddyDaemon || !u.kickstartSystemCaddyDaemon()) {
                            console.error('❌ Caddy system daemon is installed but not serving.');
                            console.error('   Run: sudo launchctl kickstart -k system/com.aigon.caddy');
                            return;
                        }
                    } else {
                        execSync(`caddy start --config ${JSON.stringify(CADDYFILE_PATH)}`, { stdio: 'pipe', timeout: 15000 });
                    }
...
<dict>
    ...
    <string>${logPath}</string>
            ...
            if (subcommand === 'start') {
                ...
                if (!basePort) {
                    ...
                    if (registryEntry) {
                        ...
                        console.error('❌ No basePort configured. Run `aigon apply` or set PORT in .env or devProxy.basePort in .aigon/config.json');
                        return;
                    }
                }
        ...
        'config': (args) => {
            ...
            if (subcommand === 'init') {
                ...
                if (scope === 'global') {
                    ...
                    console.log(`       "ag": { "cli": "agy", "implementFlag": "--dangerously-skip-permissions" },`);
                    ...
                    console.log(`\n💡 Run 'aigon apply' to regenerate templates with the new profile.`);
                }
                ...
                if (remainingArgs.length < 2) {
                    ...
                    console.error(`    aigon config set iterate.validationCommand "npm run test:iterate"`);
                    return;
                }
                ...
                // F521: enforce scope rules — user-scope keys cannot be set per-project,
                // repo-scope keys cannot be set globally.
                const settingScope = u.getSettingScope(key);
                if (settingScope === 'user' && scope !== 'global') {
                    console.error(`❌ scope_violation: "${key}" is user-scope and can only be set globally.`);
                    console.error(`   Re-run with --global: aigon config set --global ${key} ${value}`);
                    process.exitCode = 1;
                    return;
                }
                if (settingScope === 'repo' && scope !== 'project') {
                    console.error(`❌ scope_violation: "${key}" is repo-scope and can only be set per-project.`);
                    console.error(`   Re-run with --project: aigon config set --project ${key} ${value}`);
                    process.exitCode = 1;
                    return;
                }
                ...
                // F523: if this key is baked into installed agent command files
                // (profile, devServer.enabled, instructions.*), re-run
                // install-agent --all so the new value reaches future agents.
                // Skips silently when the repo has no install manifest.
                const { keyAffectsInstalledCommands, regenerateAgentInstructions } =
                    require('../agent-instructions-regen');
                const { DASHBOARD_SETTINGS_SCHEMA } = require('../dashboard-server');
                if (keyAffectsInstalledCommands(key, DASHBOARD_SETTINGS_SCHEMA)) {
                    const regen = regenerateAgentInstructions(process.cwd());
                    if (regen.regenerated && regen.committed) {
                        console.log(`🔁 Regenerated agent instructions (install-agent --all + commit).`);
                    } else if (regen.regenerated && regen.committed === false && !regen.error) {
                        console.log(`🔁 Regenerated agent instructions (no changes to commit).`);
                    } else if (regen.skipped && regen.reason === 'no-install-manifest') {
                        // Silent — user hasn't installed agents in this repo.
                    } else if (regen.error) {
                        console.warn(`⚠️  Could not regenerate agent instructions: ${regen.error}`);
                    }
                }
        ...
        'profile': async (args) => {
            ...
            // Profile sync subcommands (configure/push/pull/status) moved to
            // @aigon/pro with feature 236. Profile (show/set/detect) stays in
            // OSS — it operates on the project-profile preset, not on sync state.
            if (subcommand === 'configure' || subcommand === 'push' || subcommand === 'pull' || subcommand === 'status') {
                const { isProAvailable, getPro } = require('../pro');
                if (isProAvailable()) {
                    const pro = getPro();
                    if (pro && pro.profile && typeof pro.profile.handleProfileSyncCommand === 'function') {
                        try { return await pro.profile.handleProfileSyncCommand(args); }
                        catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
                    }
                }
                console.error('🔒 Profile sync is a Pro feature — coming later.');
            ...
            if (subcommand === 'help' || subcommand === '--help' || subcommand === '-h') {
                ...
                if (isProAvailable()) {
                    const pro = getPro();
                    if (pro && pro.profile && typeof pro.profile.printUsage === 'function') {
                        pro.profile.printUsage();
                        return;
                    }
                }
                console.log('Usage: aigon profile <show|set|detect>');
                return;
            }
            ...
            if (subcommand === 'show') {
                ...
                console.log('Usage: aigon profile <show|set|detect>');
                console.log('  show    — Show effective project profile');
                console.log('  set <type>  — Set profile in .aigon/config.json');
                console.log('  detect  — Auto-detect profile from project files');
            }
        },

        'backup': async (args) => {
            // Backup engine moved to @aigon/pro (feature 236). Stub mirrors
            // the insights delegation pattern.
            const { isProAvailable, getPro } = require('../pro');
            if (isProAvailable()) {
                const pro = getPro();
                if (pro && pro.backup && typeof pro.backup.handleBackupCommand === 'function') {
                    try { return await pro.backup.handleBackupCommand(args); }
                    catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
                }
            }
            console.error('🔒 Backup is a Pro feature — coming later.');
            console.error('   Pro is in development and not yet available for purchase.');
            process.exitCode = 1;
        },

        'sync': async (args) => {
            // Sync engine moved to @aigon/pro (feature 236).
            const { isProAvailable, getPro } = require('../pro');
            if (isProAvailable()) {
                const pro = getPro();
                if (args[0] === 'legacy' && pro && pro.sync && typeof pro.sync.handleSyncCommand === 'function') {
                    try { return await pro.sync.handleSyncCommand(args.slice(1)); }
                    catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
                }
                if (pro && pro.backup && typeof pro.backup.handleBackupCommand === 'function') {
                    console.error('⚠️  `aigon sync` is deprecated — use `aigon backup` instead.');
                    try { return await pro.backup.handleBackupCommand(args); }
                    catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
                }
            }
            console.error('🔒 Sync is a Pro feature — coming later.');
            console.error('   Pro is in development and not yet available for purchase.');
            process.exitCode = 1;
        },

        'vault': async (args) => {
            // Vault is the user-facing alias for the unified backup engine
            // (F388). Moved to @aigon/pro with feature 236.
            const { isProAvailable, getPro } = require('../pro');
            if (isProAvailable()) {
                const pro = getPro();
                const handler = (pro && pro.vault && pro.vault.handleBackupCommand)
                    || (pro && pro.backup && pro.backup.handleBackupCommand);
                if (typeof handler === 'function') {
                    try { return await handler(args); }
                    catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
                }
            }
            console.error('🔒 Vault is a Pro feature — coming later.');
            console.error('   Pro is in development and not yet available for purchase.');
            process.exitCode = 1;
        },
        // ── Server command ──────────────────────────────────────────────────
        ...
        'server': async (args) => {
            ...
            const { formatMixedInvocationHint } = require('../instance-identity');
            ...
            const buildLaunchParams = (extra = {}) => {
                const slotId = (identity.isPrimary || identity.isEphemeral) ? 'main' : identity.instanceId;
                const previewMode = options.preview === true;
                const sandboxPreview = !!process.env.AIGON_PREVIEW_SANDBOX_REPO;
                const mainRepo = resolveCurrentRepoRoot();
                return {
                dashCtx: {
                    isWorktree: !identity.isPrimary,
                    instanceName: identity.isPrimary ? 'main' : identity.instanceId,
                    worktreePath: identity.isWorktreeCode ? identity.codeRoot : null,
                    serverId: identity.caddyServerId,
                },
                instanceName: identity.isPrimary ? 'main' : identity.instanceId,
                serverId: identity.caddyServerId,
                instanceId: slotId,
                isPreview: previewMode,
                repoRoot: previewMode ? identity.codeRoot : mainRepo,
                defaultRepoPath: previewMode
                    ? (process.env.AIGON_PREVIEW_SANDBOX_REPO || mainRepo)
                    : undefined,
                appId: SERVER_APP_ID,
                proxyAvailable: (!identity.isEphemeral || sandboxPreview) && isProxyAvailable(),
                ...extra,
            };
            };
            ...
            if (!sub || sub === 'start') {
                ...
                if (identity.isMixedInvocation) {
                    console.error(formatMixedInvocationHint('start'));
                    process.exitCode = 1;
                    return;
                }

                if (previewMode) {
                    if (!identity.isWorktreeCode) {
                        console.error('❌ --preview requires a worktree checkout. Use: aigon preview <feature-id>');
                        process.exitCode = 1;
                        return;
                    }
                    const dashDir = path.join(identity.codeRoot, 'templates', 'dashboard');
                    if (!fs.existsSync(dashDir)) {
                        console.error(`❌ No templates/dashboard/ in ${identity.codeRoot}`);
                        process.exitCode = 1;
                        return;
                    }
                }
                ...
                if (!identity.isEphemeral && existing && existing.pid && existing.pid !== process.pid && isProcessAlive(existing.pid)) {
                    if (!process.stdout.isTTY) {
                        ...
                        console.log(`   ${getServerUrl(existing, identity)}`);
                        return;
                    }
                }

                await launchDashboardServer(buildLaunchParams(), launchDeps);
                return;
            }

            if (sub === 'stop') {
                if (identity.isMixedInvocation) {
                    console.error(formatMixedInvocationHint('stop'));
                    process.exitCode = 1;
                    return;
                }
                const { isServiceInstalled, stopService } = require('../supervisor-service');
                if (isServiceInstalled() && identity.isPrimary) {
                    stopService();
            ...
            if (sub === 'restart') {
                if (identity.isMixedInvocation) {
                    console.error(formatMixedInvocationHint('restart'));
                    process.exitCode = 1;
                    return;
                }
                autoRegisterRepoIfNeeded(resolveCurrentRepoRoot());
                ...
                if (isServiceInstalled() && identity.isPrimary) {
                    ...
                    await launchDashboardServer(buildLaunchParams({ _isRestart: true }), launchDeps);
                }
                return;
            }

            if (sub === 'status') {
                const entry = getServerRegistryEntry(identity.instanceId);
            ...
            console.error('  start --preview    Serve dashboard templates from this worktree (isolated instance)');
        ...
        // ── Preview command ─────────────────────────────────────────────────
        // Launch an isolated dashboard for a feature worktree without disturbing
        // the primary aigon.localhost server.
        'preview': async (args) => {
            const options = parseCliOptions(args);
            const featureId = options._[0];
            const {
                ...
                gcPreviewSandboxes,
            } = require('../preview-launcher');

            if (featureId === 'gc') {
                const removed = await gcPreviewSandboxes({ isProcessAlive });
                if (removed > 0) {
                    console.log(`✅ Removed ${removed} orphaned sandbox preview${removed === 1 ? '' : 'es'}.`);
                } else {
                    console.log('ℹ️  No orphaned sandbox previews found.');
                }
                return;
            }

            if (!featureId) {
                console.error('Usage: aigon preview <feature-id> [--stop] [--sandbox[=fixture]] [--agent=<id>] [--open]');
                console.error('       aigon preview gc');
                console.error('');
                console.error('  Starts (or stops) an isolated dashboard for a feature worktree.');
                console.error('  Default preview uses your real project data (read-only-safe UI/backend checks).');
                console.error('  --sandbox uses a throwaway AIGON_HOME + seeded fixture repo for write-heavy backend changes.');
                console.error('  Preview URL: <agent>-<id>.aigon.localhost (never aigon.localhost).');
                process.exitCode = 1;
                return;
            }

            const repoPath = resolveCurrentRepoRoot();
            const agentId = options.agent || null;
            const sandboxMode = options.sandbox !== undefined && options.sandbox !== false;
            const matches = findFeatureWorktrees(repoPath, featureId, agentId);

            if (matches.length === 0) {
                const hint = agentId
                    ? `feature #${featureId} agent ${agentId}`
                    : `feature #${featureId}`;
                console.error(`❌ No worktree found for ${hint}.`);
                console.error('   Start the feature first: aigon feature-start <id>');
                process.exitCode = 1;
                return;
            }

            if (!agentId && matches.length > 1) {
                console.log(`ℹ️  Multiple worktrees for feature #${featureId} — using newest (${matches[0].agent}). Pass --agent=<id> to pick one.`);
            }

            const worktree = matches[0];

            if (options.stop === true) {
                const instanceId = require('../preview-launcher').getPreviewInstanceId(worktree.path);
                const wasSandbox = instanceId && require('../preview-sandbox').readSandboxState(instanceId);
                const stopped = await stopPreviewForWorktree(worktree.path, { isProcessAlive });
                if (stopped) {
                    console.log(`✅ Preview stopped for feature #${featureId} (${worktree.agent})${wasSandbox ? ' [sandbox]' : ''}.`);
                } else {
                    console.log(`ℹ️  No running preview for feature #${featureId} (${worktree.agent}).`);
                }
                return;
            }

            try {
                const result = sandboxMode
                    ? await startSandboxPreviewForWorktree(worktree.path, { isProcessAlive, sandbox: options.sandbox })
                    : await startPreviewForWorktree(worktree.path, { isProcessAlive });
                if (result.alreadyRunning) {
                    console.log(`ℹ️  Preview already running (PID ${result.pid})`);
                } else if (!result.healthy) {
                    console.log('⚠️  Preview started but health check timed out — it may still be warming up.');
                } else {
                    console.log(`✅ Preview started (PID ${result.pid || 'unknown'})`);
                }
                if (result.sandbox) {
                    console.log('🧪 Sandbox: isolated AIGON_HOME + seeded fixture repo (writes stay in the sandbox).');
                    if (result.fixture) console.log(`   Fixture: ${result.fixture}`);
                    if (result.repoPath) console.log(`   Data repo: ${result.repoPath}`);
                } else {
                    console.log('📂 Data: real project + ~/.aigon (read-only-safe; use --sandbox for write-heavy backend changes).');
                }
                console.log(`🔀 Preview: ${result.url}`);
                console.log(`   Worktree: ${worktree.path}`);
                console.log(`   Templates: ${worktree.path}/templates/dashboard/`);
                if (result.logPath) {
                    console.log(`   Log: ${result.logPath}`);
                }
                if (options.open === true) {
                    try { openInBrowser(result.url); } catch (_) { /* ignore */ }
                }
            } catch (e) {
                console.error(`❌ ${e.message}`);
                process.exitCode = 1;
    ...
    return { ...commands, ...require('./storage')(ctx), ...require('./spec-layout')(ctx), ...require('./spec-view')(ctx) };
};
...
function createInfraCommands(overrides = {}) {
    ...
    const names = ['dashboard', 'server', 'preview', 'terminal-focus', 'board', 'proxy-setup', 'proxy', 'dev-server', 'config', 'hooks', 'profile', 'sync', 'backup', 'storage', 'spec-layout', 'spec-view'];
...
module.exports.autoRegisterRepoIfNeeded = autoRegisterRepoIfNeeded;
module.exports.resolveCurrentRepoRoot = resolveCurrentRepoRoot;
module.exports.isAigonRepo = isAigonRepo;
