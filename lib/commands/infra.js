...
<dict>
    ...
    <string>${logPath}</string>
        ...
        'backup': async (args) => {
            // F388: aigon-vault — unified backup of project state + global
            // settings into one private git repo. Replaces F359 (`aigon sync`)
            // and F380 (`aigon settings/profile sync`) with a single command.
            try {
                const backup = require('../backup');
                await backup.handleBackupCommand(args);
            } catch (error) {
                console.error(`❌ ${error.message}`);
                process.exitCode = 1;
            }
        ...
        'sync': async (args) => {
            // F388: `aigon sync` is now a deprecated alias for `aigon backup`.
            // The legacy multi-repo bundle flow in ../sync stays reachable via
            ...
            console.error('    `aigon backup` covers project state AND global settings in one private vault repo.');
            try {
                const backup = require('../backup');
                await backup.handleBackupCommand(args);
            } catch (error) {
