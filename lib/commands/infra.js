...
module.exports = function infraCommands(ctx) {
    ...
    function buildDashboardServerOptions(isPreview, repoRoot) {
    ...
    async function resolveDashboardPort(dashCtx, instanceName) {
    ...
    function reconcileProxyRoutesSafely(proxyAvailable, shouldLog) {
    ...
    async function stopDashboardProcess(existing, label) {
    ...
    async function launchDashboardServer({ dashCtx, instanceName, serverId, isPreview, repoRoot, proxyAvailable, shouldLogProxyReconcile }) {
        const port = await resolveDashboardPort(dashCtx, instanceName);
        reconcileProxyRoutesSafely(proxyAvailable, shouldLogProxyReconcile);
        runDashboardServer(port, instanceName, serverId, buildDashboardServerOptions(isPreview, repoRoot));
    }
    ...
    const commands = {
        ...
        'dashboard': async (args) => {
            ...
            // --preview: validate we're in a worktree with templates/dashboard/
            if (isPreview) {
                if (!dashCtx.isWorktree) {
                    console.error('❌ --preview can only be used from an aigon worktree.');
                    console.error('   Run this from a worktree directory (e.g., aigon-worktrees/feature-NNN-agent-name/)');
                    process.exitCode = 1;
                    return;
                }
                const templateDir = path.join(repoRoot, 'templates', 'dashboard');
                if (!fs.existsSync(templateDir)) {
                    console.error('❌ No templates/dashboard/ directory found in this worktree.');
            ...
            // Info: if running from a worktree without --preview, note that this is the management UI
            const worktreeMarker = path.join(repoRoot, '.aigon', 'worktree.json');
            if (!isPreview && fs.existsSync(worktreeMarker) && (!sub || sub === 'start')) {
                console.log('ℹ️  Running dashboard from a worktree. This is the Aigon management UI.');
                console.log('   To preview worktree template changes, run:  aigon dashboard --preview');
                console.log('   To start the project dev server instead, run:  aigon dev-server start');
                console.log('');
            }
            ...
            if (sub === 'restart') {
                ...
                const appServers = registry[appId] || {};
                const existing = appServers[registryServerId];
                const stopped = await stopDashboardProcess(existing, 'dashboard');
                if (!stopped) {
            ...
            console.log('');
            console.log('Flags:');
            console.log('  --preview          Start a preview dashboard from a worktree');
            console.log('                     Serves templates from the worktree, shares API data');
        },
    ...
    <string>${logPath}</string>
        ...
        'server': async (args) => {
            ...
            if (!sub || sub === 'start') {
                ...
                // Regular foreground start — delegate to `aigon dashboard start`
                // (dashboard start already integrates the supervisor loop)
                return commands.dashboard(args);
            }

            if (sub === 'stop') {
                return commands.dashboard(['stop']);
            }

            if (sub === 'restart') {
                return commands.dashboard(['restart']);
            }

            if (sub === 'status') {
                const { loadProxyRegistry: loadReg } = require('../proxy');
                const { getSupervisorStatus } = require('../supervisor');
                const appId = getAppId();
                const registry = loadReg();
                const entry = registry[appId] && registry[appId][''];
            ...
            console.error('Usage: aigon server [start|stop|status]');
            console.error('');
