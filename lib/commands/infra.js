    ...
    const commands = {
        ...
        'dashboard': async (args) => {
            ...
            if (!sub || sub === 'start') {
                ...
                runDashboardServer(port, instanceName, serverId, serverOptions);
                return;
            }
            ...
            if (sub === 'restart') {
                ...
                const { startSupervisorLoop: startSupervisor, getSupervisorStatus: getSupervisorStat } = require('../supervisor');
                const restartOptions = {
                    ...(isPreview ? { templateRoot: repoRoot } : {}),
                    startSupervisorLoop: startSupervisor,
                    getSupervisorStatus: getSupervisorStat,
                };
                runDashboardServer(port, instanceName, serverId, restartOptions);
                return;
            }
...
<dict>
    ...
    <string>${logPath}</string>
        ...
        'server': async (args) => {
            ...
            if (sub === 'stop') {
                // Delegate to `aigon dashboard stop` — same process
                return commands.dashboard(['stop', ...args.slice(1)]);
            }
...
function createInfraCommands(overrides = {}) {
    ...
    const names = ['dashboard', 'terminal-focus', 'board', 'proxy-setup', 'proxy', 'dev-server', 'config', 'hooks', 'profile'];
