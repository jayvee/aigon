...
<dict>
    ...
    <string>${logPath}</string>
        ...
        'profile': async (args) => {
            ...
            // Profile sync subcommands (configure/push/pull/status) operate on
            // ~/.aigon sync state; show/set/detect operate on the project-profile
            // preset. Both live in-repo since F693.
            if (subcommand === 'configure' || subcommand === 'push' || subcommand === 'pull' || subcommand === 'status') {
                const profileState = require('../profile-state');
                try { return await profileState.handleProfileSyncCommand(args); }
                catch (error) { console.error(`❌ ${error.message}`); process.exitCode = 1; return; }
            }
            if (subcommand === 'help' || subcommand === '--help' || subcommand === '-h') {
                const profileState = require('../profile-state');
                if (typeof profileState.printUsage === 'function') { profileState.printUsage(); return; }
