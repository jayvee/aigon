...
module.exports = function infraCommands(ctx) {
    ...
    /**
     * Running dashboard for the effective AIGON_HOME — reads dashboard-runtime.json only.
     * Returns { pid, port, startedAt?, version? } or null.
     */
    function getServerRegistryEntry() {
        return getDashboardRuntimeEntry({ isProcessAlive });
    }
...
<dict>
    ...
    <string>${logPath}</string>
        ...
        'server': async (args) => {
            ...
            const isE2eServer = process.env.AIGON_E2E_SERVER === '1';
            ...
            if (!sub || sub === 'start') {
                ...
                if (!isE2eServer && existing && existing.pid && existing.pid !== process.pid && isProcessAlive(existing.pid)) {
                ...
                await launchDashboardServer({
                    ...
                    appId: SERVER_APP_ID,
                    // E2E servers must never touch the real Caddyfile — getAigonServerAppId()
                    // is a fixed 'aigon' identity regardless of cwd, so an unguarded route
                    // write here hijacks the user's real aigon.localhost route to point at
                    // the ephemeral test port (and leaves it dangling after teardown kills it).
                    proxyAvailable: !isE2eServer && isProxyAvailable(),
                }, {
                    DASHBOARD_DYNAMIC_PORT_START,
                    hashBranchToPort,
                    isPortAvailable,
