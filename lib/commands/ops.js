...
function hasTerminalBootstrapRegression(entityType, events, currentLifecycle) {
    if (currentLifecycle === 'done' || !Array.isArray(events)) return false;
    const terminalType = `${entityType}.closed`;
    const bootstrapType = `${entityType}.bootstrapped`;
    const terminalIndex = events.map(event => event.type).lastIndexOf(terminalType);
    if (terminalIndex < 0) return false;
    const tail = events.slice(terminalIndex + 1);
    return tail.some(event => event.type === bootstrapType)
        && tail.every(event => event.type === bootstrapType || String(event.type || '').startsWith('lease.'));
}
...
module.exports = function opsCommands(ctx) {
    ...
    return {
            ...
            const events = snapshot ? await wf.listEntityEvents(repoPath, entityType, padded) : [];
            const terminalBootstrapRegression = hasTerminalBootstrapRegression(entityType, events, currentLifecycle);
            const reconciliationSnapshot = terminalBootstrapRegression
                ? { ...snapshot, lifecycle: 'done', currentSpecState: 'done' }
                : snapshot;
            const specReconciliation = reconcileEntitySpec(repoPath, entityType, padded, { snapshot: reconciliationSnapshot, dryRun: true });
            const workflowDone = currentLifecycle === 'done' || terminalBootstrapRegression;
            ...
            const repairActions = [];
            if (terminalBootstrapRegression) repairActions.push('restore terminal workflow lifecycle to done');
            ...
            let repairedSnapshot = snapshot;
            if (terminalBootstrapRegression) {
                const idKey = entityType === 'research' ? 'researchId' : 'featureId';
                repairedSnapshot = await wf.persistEntityEvents(repoPath, entityType, padded, [{
                    type: `${entityType}.bootstrapped`,
                    [idKey]: padded,
                    stage: 'done',
                    lifecycle: 'done',
                    at: new Date().toISOString(),
                }]);
                console.log('   🩹 Restored terminal workflow lifecycle to done');
            }

            if (specReconciliation.driftDetected) {
                reconcileEntitySpec(repoPath, entityType, padded, { snapshot: repairedSnapshot });
            }
