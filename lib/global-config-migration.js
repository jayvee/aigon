...
/**
 * Dashboard runtime registry path — primary and ephemeral-isolated homes use the
 * legacy single slot; non-primary instances sharing real ~/.aigon use per-instance files.
 * @param {string} [instanceId] - When omitted, derived from current instance identity.
 * @returns {string}
 */
function getDashboardRuntimePath(instanceId) {
    const base = path.join(getAigonHome(), '.aigon');
    let resolvedId = instanceId;
    if (!resolvedId) {
        try {
            const { resolveInstanceIdentity } = require('./instance-identity');
            const identity = resolveInstanceIdentity();
            resolvedId = (identity.isPrimary || identity.isEphemeral) ? 'main' : identity.instanceId;
        } catch (_) {
            resolvedId = 'main';
        }
    }
    if (!resolvedId || resolvedId === 'main') {
        return path.join(base, 'dashboard-runtime.json');
    }
    const safe = String(resolvedId).replace(/[^a-zA-Z0-9._-]+/g, '-');
    return path.join(base, `dashboard-runtime-${safe}.json`);
}
...
function canonicalizeTerminalApp(value) {
    return require('./terminal-adapters').canonicalize(value);
}
...
function migrateLegacyTerminalSettings(config) {
    ...
    // Legacy `terminal` was a string (e.g. "warp", "tmux"). The current schema
    // uses it as an object namespace (e.g. `terminal.focusOnLaunch`); only
    // touch it when it's still the legacy string shape.
