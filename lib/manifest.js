 ...
 * Auto-reconciles if the cached manifest's stage disagrees with the
 * actual spec folder location (folder is always source of truth).
 *
...
function readManifest(id) {
    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            // Validate cached stage against folder reality
            const folderInfo = deriveFromFolder(id);
            if (folderInfo && cached.stage !== folderInfo.stage) {
                // Folder wins — auto-reconcile stale manifest
                const staleStage = cached.stage;
                cached.stage = folderInfo.stage;
                cached.specPath = folderInfo.specPath;
                if (folderInfo.name) cached.name = folderInfo.name;
                if (!cached.events) cached.events = [];
                cached.events.push({ type: 'auto-reconcile-stage', at: new Date().toISOString(), actor: 'readManifest', from: staleStage, to: folderInfo.stage });
                atomicWriteJSON(filePath, cached);
            }
            return cached;
        } catch (e) {
