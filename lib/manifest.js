...
/**
 * Get the state directory. In a worktree, reads from the main repo's .aigon/state/
 * since agents write their status files there (not inside worktrees).
 * @param {Object} [options] - { mainRepoPath } to override auto-detection
 ...
 * @param {Object} [options] - { mainRepoPath } to override auto-detection
 * @returns {string} Absolute path to the locks directory
 */
...
function coordinatorPath(id, prefix = 'feature', options) {
    return path.join(getStateDir(options), `${prefix}-${canonicalId(id)}.json`);
}
...
function lockPath(id, prefix = 'feature', options) {
    return path.join(getLocksDir(options), `${prefix}-${canonicalId(id)}.lock`);
}
 ...
 *
 * In worktrees, pass { mainRepoPath } to read from the main repo's state dir.
 *
 * @param {string|number} id - Entity ID
 * @param {string} [prefix='feature'] - Entity prefix ('feature' or 'research')
 * @param {Object} [options] - { mainRepoPath } to read from a specific repo
 * @returns {Object|null} Coordinator manifest, or null when missing/corrupt
 */
function readManifest(id, prefix = 'feature', options) {
    const stateDir = getStateDir(options);
    for (const cid of candidateIds(id)) {
        const filePath = path.join(stateDir, `${prefix}-${cid}.json`);
 ...
 * @param {string} [prefix='feature'] - Entity prefix ('feature' or 'research')
 * @param {Object} [options] - { mainRepoPath } to write to a specific repo
 */
function writeManifest(id, data, event, prefix = 'feature', options) {
    const coordPath = coordinatorPath(id, prefix, options);
 ...
 *
 * In worktrees, pass { mainRepoPath } to read from the main repo's state dir
 * (where agents write their status files).
 *
 * @param {string|number} id - Entity ID
 ...
 * @returns {boolean}
 */
function acquireLock(id, prefix = 'feature', options) {
    const lp = lockPath(id, prefix, options);
 ...
 * @param {Object} [options] - { mainRepoPath } to write to a specific repo
 */
function releaseLock(id, prefix = 'feature', options) {
    const lp = lockPath(id, prefix, options);
    if (!fs.existsSync(lp)) {
