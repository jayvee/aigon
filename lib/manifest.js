...
function getSpecsRoot() { return path.join(getRoot(), 'docs', 'specs'); }
...
function coordinatorPath(id) {
    return path.join(getStateDir(), `feature-${id}.json`);
}

function agentStatusPath(id, agent) {
    return path.join(getStateDir(), `feature-${id}-${agent}.json`);
}

function lockPath(id) {
    return path.join(getLocksDir(), `feature-${id}.lock`);
}
...
function deriveFromFolder(id) {
    ...
    const featureRoot = path.join(getSpecsRoot(), 'features');
...
function findLogFiles(id) {
    const logsDir = path.join(getSpecsRoot(), 'features', 'logs');
 ...
 * Returns the stored manifest, or a transient derived object if none exists.
 *
 * IMPORTANT: Does NOT persist on read. Only explicit commands (feature-create,
 * feature-prioritise, feature-start, requestTransition) should write manifests.
 * The dashboard can call this for display purposes without creating side effects.
 *
 * @param {string|number} id - Feature ID
 * @returns {Object} Coordinator manifest (may be transient if no file exists)
 */
function readManifest(id) {
    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            // Corrupted file — fall through to derive
        }
    }

    // Derive a transient manifest from folder state — NOT persisted.
    // Commands that need a real manifest should call ensureManifest().
    return bootstrapManifest(id);
}

/**
 * Ensure a persisted manifest exists for a feature.
 * Creates one from derived state if it doesn't exist yet.
 * Call this from commands that create/modify features (not from reads).
 *
 * @param {string|number} id - Feature ID
 * @returns {Object} Coordinator manifest (guaranteed persisted)
 */
