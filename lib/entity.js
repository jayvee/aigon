...
function buildFeatureDependencySvg(featureId, featureIndex, graph) {
    ...
    for (const dependent of included) {
        ...
        for (const dep of deps) {
            ...
            edgePaths.push(`<path d="M ${x1} ${y1} C ${c1} ${y1}, ${c2} ${y2}, ${x2} ${y2}" fill="none" stroke="#94a3b8" stroke-width="2" marker-end="url(#dep-arrow)"/>`);
        }
    }
    ...
    return [
        `<svg xmlns="http://www.w3.org/2000/svg" width="${svgWidth}" height="${svgHeight}" viewBox="0 0 ${svgWidth} ${svgHeight}" role="img" aria-label="Feature dependency graph for feature ${featureId}">`,
        '<defs><marker id="dep-arrow" markerWidth="10" markerHeight="8" refX="9" refY="4" orient="auto"><path d="M0,0 L10,4 L0,8 Z" fill="#94a3b8"/></marker></defs>',
        edgePaths.join(''),
        nodeBlocks.join(''),
        '</svg>',
    ].join('');
...
function refreshFeatureDependencyGraphs(paths, utils) {
    ...
    const graph = buildDependencyGraph(paths, utils);
