...
function entityPrioritise(def, name, ctx) {
    ...
    if (def.type === 'feature') {
        try {
            const content = fs.readFileSync(moved.fullPath, 'utf8');
            const { data } = u.parseFrontMatter(content);
            if (Array.isArray(data.depends_on) && data.depends_on.length > 0) {
                const featureIndex = buildFeatureIndex(def.paths);
                // Also register the newly-prioritised feature itself
                featureIndex.byId[paddedId] = { paddedId, slug: baseName, file: newName, folder: '02-backlog', fullPath: moved.fullPath };
                ...
                // Rewrite spec frontmatter with canonical padded IDs
