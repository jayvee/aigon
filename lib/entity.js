...
async function pausePrestartEntity(def, id, ctx) {
    ...
    const resolved = featureSpecResolver.resolveEntitySpec(repoPath, def.type, id);
    ...
    if (!found) {
        found = def.paths && specCrudLib.findFile(def.paths, id, ['01-inbox', '02-backlog']);
...
async function resumePrestartEntity(def, id, ctx) {
    ...
    if (!found) {
        found = def.paths && specCrudLib.findFile(def.paths, id, ['06-paused']);
        if (!found) return { handled: false };
        if (!workflowId) workflowId = parseWorkflowEntityId(def, found.file);
    }
