...
function migrateWorkflowEntityId(repoPath, entityType, fromId, toId, specPath) {
    ...
    if (fs.existsSync(oldSnapshotPath)) {
        ...
        snapshot.researchId = entityType === 'research' ? toId : snapshot.researchId;
...
function entityPrioritise(def, name, ctx) {
    ...
    const migratedWorkflowId = migrateWorkflowEntityId(process.cwd(), def.type, baseName, paddedId, moved.fullPath);
