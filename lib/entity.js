...
function entityStart(def, args, ctx) {
    ...
    if (found.folder === '03-in-progress') {
        if (agentIds.length > 0) {
            console.log(`📋 Registered agents [${agentIds.join(', ')}] for ${def.type} ${entityId}`);
        } else {
...
function collectIncompleteAgents(def, entityId, ctx) {
    ...
    if (def.type === 'research') {
        ...
        findingsFiles.forEach(file => {
            ...
            try {
                const agent = match[1];
                const agentState = readAgentStatus(entityId, agent, def.prefix);
        ...
        worktrees.forEach(w => {
            const agentState = readAgentStatus(entityId, w.agent, def.prefix);
            const status = agentState ? (agentState.status || 'unknown') : 'unknown';
            if (status !== 'submitted') {
