...
const { isValidSetSlug } = featureSets;
...
function _recurring() { return require('./recurring'); }
    ...
    try {
        return specCrudLib.createSpecFile({
            ...
            afterWrite: (built) => {
                ...
                const agentId = entityContext.resolveCreationAuthor(
                    options,
                    process.env,
                    u.detectActiveAgentSession
                );
                const directNativeSession = entityContext.resolveDirectNativeSession(agentId, process.env);
...
function entityPrioritise(def, name, ctx, args = []) {
    ...
    // A set is a single planning unit: members must receive their backlog IDs
    // together, in dependency order. Letting one member through this generic
    // command creates a mixed inbox/backlog set that can later be started
    // while its peers are still unprioritised.
    if (def.type === 'feature' && !args.includes('--from-set-prioritise')) {
        const content = fs.readFileSync(found.fullPath, 'utf8');
        const setSlug = featureSets.readSetTag(content);
        if (setSlug) {
            const inboxMembers = (featureSets.scanFeatureSets(def.paths).get(setSlug) || [])
                .filter((member) => member.stage === 'inbox');
            if (inboxMembers.length > 1) {
                process.exitCode = 1;
                return console.error(
                    `❌ Feature "${baseName}" belongs to set "${setSlug}", which still has ${inboxMembers.length} inbox members.\n`
    ...
    const canonicalSpecPath = stableLayout
        ? (specLifecycle.findCanonicalSpecPath(repoPath, def.type, paddedId) || found.fullPath)
        : null;
    const futureSpecPath = stableLayout ? canonicalSpecPath : path.join(def.paths.root, STAGE_FOLDERS.BACKLOG, newName);
    try {
    ...
    const moved = stableLayout
        ? { ...found, folder: STAGE_FOLDERS.BACKLOG, file: newName, fullPath: canonicalSpecPath || found.fullPath }
        : specCrudLib.moveFile(found, STAGE_FOLDERS.BACKLOG, newName, { actor: `cli/${def.prefix}-prioritise` });
    if (stableLayout) {
    ...
    const extraPaths = [];
    if (def.type === 'feature' && !stableLayout) {
    ...
    try {
        if (stableLayout && extraPaths.length === 0) {
            console.log(`📝 No spec move commit needed under stable layout`);
        } else if (stableLayout) {
            stageAndCommitPaths(
                runGit,
                process.cwd(),
                extraPaths,
                `chore: update ${def.type} ${paddedId} generated metadata`,
            );
            console.log(`📝 Committed ${def.type} metadata update`);
        } else {
...
function entityCloseFinalize(def, closeResult, ctx) {
    ...
    const stableLayout = specLifecycle.isStableSpecLayout(repoPath);
    const postFound = stableLayout ? null : specCrudLib.findFile(def.paths, num, [STAGE_FOLDERS.IN_EVALUATION, STAGE_FOLDERS.IN_PROGRESS]);
    if (postFound) {
    ...
    } else if (def.type === 'research' && !stableLayout) {
...
async function pausePrestartEntity(def, id, ctx) {
    ...
    try {
        if (specLifecycle.isStableSpecLayout(repoPath)) {
            specLifecycle.refreshLifecycleView(repoPath, { warn: console.warn });
        } else {
...
async function entityDelete(def, id, ctx) {
    ...
    if (resolved) {
        ...
        if (resolved.path && fs.existsSync(resolved.path)) {
            found = {
                ...
                folder: path.basename(path.dirname(resolved.path)),
            };
        }
    }

    if (!found) {
        found = specCrudLib.findFile(def.paths, idNormalized, [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.BACKLOG, STAGE_FOLDERS.PAUSED]);
        if (!workflowId && found) workflowId = parseWorkflowEntityId(def, found.file);
    }

    if (!workflowId) {
        if (found) workflowId = parseWorkflowEntityId(def, found.file);
        if (!workflowId) {
    ...
    let recurringDisposition = null;
    if (def.type === 'feature' && found) {
        try {
            const specContent = fs.readFileSync(found.fullPath, 'utf8');
            recurringDisposition = _recurring().markRecurringInstanceDeleted(repoPath, specContent);
        } catch (error) {
            process.exitCode = 1;
            return console.error(`❌ Cannot delete recurring feature ${workflowId}: ${error.message}`);
        }
    }

    let tracked = false;
    if (found) {
        tracked = isGitTracked(repoPath, found.fullPath);
        if (tracked) {
        ...
        console.warn(`⚠️  No spec file on disk for ${def.type} ${workflowId}; removing workflow state only.`);
    }
    ...
    console.log(`✅ Deleted ${def.type}: ${found ? found.file : workflowId}`);
    if (recurringDisposition) {
        console.log(`⏭️  Skipped ${recurringDisposition.recurringSlug} for ${recurringDisposition.period}; the next ${recurringDisposition.schedule} period remains eligible.`);
    }
}
