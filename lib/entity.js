...
const { isGitTracked, stageAndCommitPaths, stageAndCommitSpecMove } = require('./git-staging');
...
function entityCreate(def, name, ctx, options = {}) {
    ...
    try {
        return specCrudLib.createSpecFile({
            ...
            inboxDir: path.join(def.paths.root, STAGE_FOLDERS.INBOX),
            ...
            build: (value) => {
                ...
                const filePath = path.join(def.paths.root, STAGE_FOLDERS.INBOX, filename);
...
async function entityDelete(def, id, ctx) {
    ...
    if (pathsToCommit.length > 0) {
        try {
            stageAndCommitPaths(ctx.git.runGit, repoPath, pathsToCommit, `chore: delete ${def.type} ${label} - remove spec`);
        } catch (error) {
