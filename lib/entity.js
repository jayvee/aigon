...
async function pausePrestartEntity(def, id, ctx) {
    ...
    const repoPath = process.cwd();
    const found = def.paths && specCrudLib.findFile(def.paths, id, ['01-inbox', '02-backlog']);
    ...
    const snapshot = await _wf().showEntityOrNull(repoPath, def.type, workflowId);
    if (!snapshot) {
    ...
    const pausedDir = path.join(def.paths.root, '06-paused');
    const fromPath = found.fullPath || path.join(def.paths.root, found.folder, found.file);
    const toPath = path.join(pausedDir, found.file);
    // workflow-core may already have run move_spec (backlog → paused); only rename if the file is still on disk at the old path.
...
async function resumePrestartEntity(def, id, ctx) {
    ...
    const repoPath = process.cwd();
    const found = def.paths && specCrudLib.findFile(def.paths, id, ['06-paused']);
    if (!found) return { handled: false };
