...
function installLaunchd() {
  ...
  const servicePath = [opencodeBin, homeBin, '/opt/homebrew/bin', '/usr/local/bin', '/usr/bin', '/bin', process.env.PATH || '']
    .filter(Boolean)
    .join(':');
...
function installSystemd() {
  ...
  const homeBin = path.join(os.homedir(), '.local', 'bin');
  const opencodeBin = path.join(os.homedir(), '.opencode', 'bin');
  const servicePath = [opencodeBin, homeBin, '/usr/local/bin', '/usr/bin', '/bin', process.env.PATH || '']
    .filter(Boolean)
    .join(':');
  ...
  if (process.platform === 'darwin') {
    ...
    if (uid != null) {
      ...
      try {
        ...
        return;
      } catch (e) { kickstartErr = e; }
      // Log the failure so operators know the fallback path ran and why.
      // Incident 2026-04-29: silent swallow here let the racey unload+load run,
      // causing a SIGTERM cascade that left the service dead for 4 hours.
      const detail = (kickstartErr && (kickstartErr.stderr || kickstartErr.message)) || String(kickstartErr);
      console.warn(`[supervisor-service] kickstart -k failed — falling back to unload+load (${String(detail).trim() || 'no detail'})`);
    }
    // Fallback: service not loaded or kickstart failed.
    // Capture the existing PID before unloading so we can wait for it to exit,
    // avoiding the race where unload sends SIGTERM but the old process is still
    // alive when load fires the new instance.
    const plistPath = path.join(os.homedir(), 'Library', 'LaunchAgents', `${SERVICE_LABEL}.plist`);
    let existingPid = null;
    if (uid != null) {
      try {
        const info = execSync(`launchctl print gui/${uid}/${SERVICE_LABEL} 2>/dev/null`, { encoding: 'utf8', stdio: 'pipe' });
        const m = info.match(/\bpid\s*=\s*(\d+)/);
        if (m) existingPid = parseInt(m[1], 10);
      } catch (_) {}
    }
    try { execSync(`launchctl unload "${plistPath}" 2>/dev/null`, { stdio: 'ignore' }); } catch (_) {}
    // Wait up to 5s for the old process to exit before loading the replacement.
    // Polling via execSync('sleep') is intentional — this is a CLI command, not the server loop.
    if (existingPid) {
      const deadline = Date.now() + 5000;
      while (Date.now() < deadline) {
        try { process.kill(existingPid, 0); } catch (_) { break; } // ESRCH = process gone
