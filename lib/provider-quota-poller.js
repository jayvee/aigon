...
function buildProviderEntry({
    ...
    // Spec: verdict is 'unknown' when no key found OR both endpoints
    // unreachable. lastError still records the failure for diagnostics.
    const verdict = computeVerdict(balanceUsd, lowThresholdUsd);
