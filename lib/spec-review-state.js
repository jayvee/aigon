...
function parseSpecReviewSubject(subject) {
    ...
    const match = raw.match(/^spec-review(?:-check)?:\s*(feature|research)\s+(.+?)(?:\s+[:—-]\s+.+)?$/);
    if (!match) return null;
    const isAck = /^spec-review-check:/.test(raw);
    return {
        entityType: match[1],
        entityId: match[2],
        isAck,
        isReview: !isAck,
        summary: raw.replace(/^spec-review(?:-check)?:\s*(feature|research)\s+(.+?)(?:\s+[:—-]\s+)?/, '').trim(),
        subject: raw,
    };
}
