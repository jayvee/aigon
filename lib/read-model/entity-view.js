...
function readSpecFacets(specPath) {
    const facets = { complexity: null, set: null, name: null, criteria: { total: 0, done: 0 } };
    ...
    facets.name = base.replace(/^(feature|research)-\d+-/, '').replace(/-/g, ' ') || base;
    return facets;
}
...
function buildEntityView(repoPath, entityType, id, options = {}) {
    ...
    const stage = snapshot
        ? workflowSnapshotAdapter.snapshotToStage(snapshot)
        : stageFromFolder(options.folderFallback);

    // Engine-first done check (F397). Folder is consulted only as fallback.
    const closed = workflowCore.isEntityDone(absRepo, type, entityId, options.folderFallback || null);
    ...
    const sessions = buildSessionFacet(sessionService, type, entityId);
