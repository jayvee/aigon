...
const { isAgentActive } = require('./agent-registry');
...
function classifyManifestEntries(manifest, repoRoot) {
    ...
    for (const entry of manifest.files) {
        ...
        if (!agentId) continue;
        if (!installedAgents.includes(agentId) || !isAgentActive(agentId)) continue;
        ...
        // Resolve upstream template. Prefer the manifest's recorded
        // templatePath/templateSha (F502); fall back to deriving the path
        // from the installed file name for older manifests.
        let templatePath = entry.templatePath || null;
        let recordedTemplateSha = entry.templateSha || null;
        if (!templatePath) {
        ...
        if (templatePath) currentTemplateSha = sha256OfFile(path.join(repoRoot, templatePath));

        // STALE_TEMPLATE = current template sha differs from the sha
        // recorded at install time (content-based — robust against `git
        // checkout` mtime ticks). For legacy manifests without templateSha,
        // fall back to the installedAt vs mtime heuristic.
        let status = 'OK';
        if (templatePath && currentTemplateSha) {
            if (recordedTemplateSha) {
                if (recordedTemplateSha !== currentTemplateSha) status = 'STALE_TEMPLATE';
            } else {
                try {
        ...
        byAgent[agentId].push({
            path: entry.path,
            templatePath,
            templateSha: currentTemplateSha,
            recordedTemplateSha,
...
function detectStaleTemplates(repoRoot) {
    ...
    for (const [agentId, entries] of Object.entries(classified)) {
        if (!isAgentActive(agentId)) continue;
