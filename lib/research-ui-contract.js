...
function choosePrimary(actions, research) {
    ...
    const revise = pendingReview
        ? available.find(action => action.actionId === 'research-spec-revise')
        : null;
    if (revise) {
...
function buildResearchUiContract(research, aggregate = {}, runtimeInput = {}) {
    ...
    const pendingSpecRevision = lifecycle !== 'spec_revision_in_progress'
        && (research.specReviewSessions || []).some(review => review && review.status === 'pending')
        && decisions.some(action => action.actionId === 'research-spec-revise' && !action.disabled);
    ...
    return buildEntityUiContract({
        entity: {
            ...
            numericId: research.numericId !== undefined ? research.numericId : research.id,
            displayKey: research.displayKey || `R${research.id}`,
            name: research.name || '',
            title: research.name || '',
            slug: research.slug || slugFromSpecPath(research.specPath),
            setSlug: research.set || null,
        },
        state: {
            lifecycle,
        ...
        presentation: {
            headline: visiblePresentation.headline || research.cardHeadline || null,
