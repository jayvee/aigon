...
registerMigration('2.61.0', async ({ repoPath, log }) => {
    ...
    const manifest = installManifestLib.createEmptyManifest(aigonVersion);
    const now = new Date().toISOString();

    // Standard install path roots (mirrors the installPaths string in install-agent).
    const SCAN_DIRS = [
        path.join(repoPath, '.aigon', 'docs'),
        path.join(repoPath, '.agents'),
        path.join(repoPath, '.claude', 'commands', 'aigon'),
        path.join(repoPath, '.claude', 'skills'),
        path.join(repoPath, '.cursor', 'commands'),
        path.join(repoPath, '.cursor', 'rules'),
        path.join(repoPath, '.codex'),
        path.join(repoPath, '.gemini'),
    ];

    // Also pick up alias files directly under .claude/ (parent of commands/aigon/)
    const clauDir = path.join(repoPath, '.claude');
        ...
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isFile()) {
                try {
                    ...
                    const relPath = path.relative(repoPath, abs).replace(/\\/g, '/');
                    manifest.files.push({
                        path: relPath,
                        sha256: checksum,
    ...
    // Alias files in .claude/ root (non-recursive, skip subdirs)
    if (fs.existsSync(clauDir)) {
        scanDir(clauDir, false);
    }
