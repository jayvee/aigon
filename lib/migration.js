...
function restoreBackup(repoPath, version) {
    ...
    // Empty backup means there was nothing to restore
...
function createLogger(repoPath, version) {
    const logPath = path.join(getVersionDir(repoPath, version), 'migration.log');
    const entries = [];
 ...
 */
async function runMigration(repoPath, version, migrateFn) {
    // Idempotency check
    ...
    try {
        ...
        const manifest = {
            fromVersion: null, // Caller can set this
            toVersion: version,
            migratedAt: new Date().toISOString(),
            status: 'success',
            entities: {
...
async function runPendingMigrations(repoPath, fromVersion) {
    ...
    for (const { version, migrate } of sorted) {
        ...
        const result = await runMigration(repoPath, version, migrate);
