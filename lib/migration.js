...
// Feature 422 migration: synthesize install manifest for repos that ran install-agent
// before manifests were tracked. Scans the standard aigon install paths and creates
// .aigon/install-manifest.json with sha256+version+timestamp for every found file.
registerMigration('2.61.0', async ({ repoPath, log }) => {
    const installManifestLib = require('./install-manifest');
    const manifestPath = path.join(repoPath, installManifestLib.MANIFEST_PATH);
    if (fs.existsSync(manifestPath)) {
        log('migrate_initialize_install_manifest: manifest already exists, skipping.');
        return;
    }

    const { getAigonVersion } = require('./version');
    const aigonVersion = getAigonVersion() || 'unknown';
    const manifest = installManifestLib.synthesizeManifestFromDisk(repoPath, aigonVersion);

    if (manifest.files.length === 0) {
        log('migrate_initialize_install_manifest: no aigon-owned files found, skipping manifest creation.');
        return;
    }

    installManifestLib.writeManifest(repoPath, manifest);
    console.log(`✅ Initialized install manifest at .aigon/install-manifest.json (${manifest.files.length} files tracked)`);
    log(`migrate_initialize_install_manifest: wrote manifest with ${manifest.files.length} entries`);
});

// F589: stop tracking derived install manifest in git — local exclude + untrack.
registerMigration('2.69.0', async ({ repoPath, log }) => {
    const installManifestLib = require('./install-manifest');
    const { ensureLocalGitExclude, untrackFiles } = require('./commands/setup/gitignore-and-hooks');
    const manifestRel = installManifestLib.MANIFEST_PATH;
    const gitDir = path.join(repoPath, '.git');

    if (!fs.existsSync(gitDir)) {
        log('migrate_untrack_install_manifest: not a git repo, skipping.');
        return;
    }

    let tracked = false;
    try {
        execSync(`git ls-files --error-unmatch -- ${manifestRel}`, {
            cwd: repoPath,
    ...
    if (tracked) {
        const untracked = untrackFiles(repoPath, [manifestRel]);
        if (untracked.ok) {
            console.log(`✅ Untracked ${manifestRel} (kept working copy)`);
            log('migrate_untrack_install_manifest: git rm --cached succeeded.');
        } else {
            throw new Error(`migrate_untrack_install_manifest: failed to untrack ${manifestRel}: ${untracked.error}`);
        }
    } else {
        log('migrate_untrack_install_manifest: manifest not tracked, skipping git rm.');
    }

    const exclude = ensureLocalGitExclude(repoPath, ['.aigon/install-manifest.json']);
    if (exclude.addedEntries.length > 0) {
        log(`migrate_untrack_install_manifest: added local exclude entries: ${exclude.addedEntries.join(', ')}`);
    }
});

// F574: migrate legacy feedback specs into research with origin metadata.
registerMigration('2.68.0', async ({ repoPath, log }) => {
    const { hasUnmigratedFeedback, migrateFeedbackToResearch } = require('./feedback-migrate');
    if (!hasUnmigratedFeedback(repoPath)) {
        log('migrate_feedback_to_research: no unmigrated feedback files, skipping.');
        return;
    }
    const result = migrateFeedbackToResearch(repoPath, { log });
    if (result.migrated > 0) {
        console.log(`✅ Migrated ${result.migrated} feedback file(s) to research (origin: customer-feedback).`);
    }
    if (result.skipped > 0) {
        log(`migrate_feedback_to_research: skipped ${result.skipped} already-migrated file(s).`);
    }
    if (result.errors.length > 0) {
        throw new Error(`feedback migration errors: ${result.errors.join('; ')}`);
    }
});

// F616: merge legacy budget-cache.json + quota.json into agent-quota.json.
registerMigration('2.70.0', async ({ repoPath, log }) => {
    const { mergeLegacyFiles } = require('./agent-quota-read');
    if (mergeLegacyFiles(repoPath)) {
        log('migrate_agent_quota_state: merged budget-cache.json + quota.json → agent-quota.json');
    } else {
        log('migrate_agent_quota_state: no migration needed');
    }
});

// REGRESSION: inbox spec-review submit+ack incorrectly promoted lifecycle to backlog (spec-folder drift).
// Superseded: the original full reproject corrupted done features (post-close review events).
// Restore + targeted reproject live in 2.72.0 / 2.73.0.
registerMigration('2.71.0', async ({ repoPath, log }) => {
    log('reproject_spec_review_resting_lifecycle: superseded — no-op (see 2.72.0 / 2.73.0)');
});

registerMigration('2.72.0', async ({ repoPath, log }) => {
    const backupPath = path.join(getVersionDir(repoPath, '2.71.0'), 'backup.tar.gz');
    const manifestPath = path.join(getVersionDir(repoPath, '2.71.0'), 'manifest.json');
    if (!fs.existsSync(backupPath) || !fs.existsSync(manifestPath)) {
        log('restore_pre_271_workflows: no 2.71.0 backup — skip');
        return;
    }
    let manifest;
    try {
        ...
        log('restore_pre_271_workflows: unreadable 2.71.0 manifest — skip');
        return;
    }
    if (manifest.status !== 'success') {
        log('restore_pre_271_workflows: 2.71.0 did not complete — skip');
        return;
    }
    restoreBackup(repoPath, '2.71.0');
    log('restore_pre_271_workflows: restored .aigon/workflows from pre-2.71.0 backup');
});

registerMigration('2.73.0', async ({ repoPath, log }) => {
    const { projectContext } = require('./workflow-core');
    const workflowsDir = path.join(repoPath, '.aigon', 'workflows');
    let repaired = 0;

    for (const [entityType, wfSubdir] of [['feature', 'features'], ['research', 'research']]) {
        const root = path.join(workflowsDir, wfSubdir);
        if (!fs.existsSync(root)) continue;
        for (const entityId of fs.readdirSync(root)) {
            const snapPath = path.join(root, entityId, 'snapshot.json');
            const eventsPath = path.join(root, entityId, 'events.jsonl');
            if (!fs.existsSync(snapPath) || !fs.existsSync(eventsPath)) continue;
            ...
            const nextLifecycle = projected.currentSpecState;
            if (snapshot.lifecycle === nextLifecycle && snapshot.currentSpecState === nextLifecycle) {
                continue;
            }

            snapshot.lifecycle = nextLifecycle;
            snapshot.currentSpecState = nextLifecycle;
            snapshot.updatedAt = new Date().toISOString();
            try {
                const visible = featureSpecResolver.resolveEntitySpec(repoPath, entityType, entityId, { snapshot });
                if (visible && visible.path) snapshot.specPath = visible.path;
            } catch (_) { /* keep existing specPath */ }
    ...
    if (repaired === 0) {
        log('reproject_spec_review_safe: no repairs needed');
    } else {
        log(`reproject_spec_review_safe: repaired ${repaired} snapshot(s)`);
    }
});

registerMigration('2.74.0', async ({ repoPath, log }) => {
    ...
    try {
        ...
        if (fs.existsSync(cfgPath)) projectConfig = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
    } catch (_) {
        projectConfig = {};
    }
    const policy = resolveCloseIntegrityPolicy(projectConfig);
    const workflowsDir = path.join(repoPath, '.aigon', 'workflows', 'features');
    let repaired = 0;
    if (!fs.existsSync(workflowsDir)) {
        log('repair_advisory_close_recovery: no feature workflows');
        return;
    }

    for (const featureId of fs.readdirSync(workflowsDir)) {
        const snapPath = path.join(workflowsDir, featureId, 'snapshot.json');
        ...
        if (!snapshot || snapshot.currentSpecState !== 'close_recovery_in_progress') continue;
        const kind = snapshot.lastCloseFailure && snapshot.lastCloseFailure.kind;
        const isRemoved = !kind || kind === 'criteria-attestation';
        const isAdvisory =
            (kind === 'preauth-validation' && !isCloseFindingBlocking(policy, 'preauth-validation')) ||
            (kind === 'post-merge-gate' && !isCloseFindingBlocking(policy, 'post-merge-gate'));
        if (!isRemoved && !isAdvisory) continue;
        await workflowEngine.recordCloseRecoveryEnded(repoPath, featureId, {
            source: 'migration/advisory-close-recovery',
        });
        repaired += 1;
    }

    if (repaired === 0) {
        log('repair_advisory_close_recovery: no repairs needed');
    } else {
        log(`repair_advisory_close_recovery: repaired ${repaired} feature(s)`);
    }
});

// F693: Aigon Pro was merged into OSS — every engine ships to every user with
// no key. Drop the now-meaningless `proKey` from the global config so `aigon
// doctor` stops flagging it and no stale credential lingers on disk.
registerMigration('2.76.0', async ({ log }) => {
    const globalCfgPath = path.join(os.homedir(), '.aigon', 'config.json');
    let cfg;
    try {
        cfg = JSON.parse(fs.readFileSync(globalCfgPath, 'utf8'));
    } catch (_) {
        log('drop_pro_key: no global config — skip');
        return;
    }
    if (!Object.prototype.hasOwnProperty.call(cfg, 'proKey')) {
        log('drop_pro_key: no proKey — skip');
        return;
    }
    delete cfg.proKey;
    fs.writeFileSync(globalCfgPath, JSON.stringify(cfg, null, 2) + '\n');
    log('drop_pro_key: removed proKey from ~/.aigon/config.json');
});
