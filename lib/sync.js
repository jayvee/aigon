...
function normalizeOriginUrl(originUrl) {
    ...
    // Strip protocol prefixes first so ssh://git@host/path and git@host:path converge
    ...
    // Handle remaining user@host/path (e.g. from ssh:// URLs after prefix strip)
    if (/^[^@/]+@/.test(value)) {
...
function copyPortableStateFromRepo(repoPath, targetAigonDir) {
    ...
    copyTree(path.join(sourceAigon, 'workflows'), path.join(targetAigonDir, 'workflows'), (rel) => {
        ...
        // Exclude lock files and derived files that are rebuilt from event logs
        return base !== 'lock' && base !== 'snapshot.json' && base !== 'stats.json';
    });
