...
const DRAFT_PROMPT_TEMPLATE = path.join(__dirname, '..', 'templates', 'prompts', 'research-draft.md');
...
function getPopulatedDraftSections(specPath) {
    ...
    const sections = ['Context', 'Questions to Answer', 'Scope', 'Success Criteria'];
...
function draftSpecWithAgent(specPath, agentId, description) {
    ...
    if (!agent) {
        const valid = agentRegistry.getLaunchableAgentIds().join(', ');
        console.error(`❌ Unknown agent '${agentId}'. Valid agents: ${valid}`);
        return null;
    }
    if (!agentRegistry.isAgentLaunchable(agentId)) {
        const deactivatedMsg = agentRegistry.formatDeactivatedAgentMessage(agentId);
        console.error(`❌ Cannot draft with ${deactivatedMsg || `agent \`${agentId}\``}`);
        return null;
    }
    ...
    console.log(`\n🖊️  Launching ${agent.displayName || agent.name} to draft the research spec...`);
