...
testAsync('resumes interrupted pause with pending effects', async () => {
  const tmp = makeTmpDir();
  try {
    await wf.startFeature(tmp, '63', 'solo_worktree', ['cc']);

    const inProgressDir = path.join(tmp, 'docs', 'specs', 'features', '03-in-progress');
    const pausedDir = path.join(tmp, 'docs', 'specs', 'features', '06-paused');
    fs.mkdirSync(inProgressDir, { recursive: true });
    const specFromPath = path.join(inProgressDir, 'feature-63-demo.md');
    const specToPath = path.join(pausedDir, 'feature-63-demo.md');
    fs.writeFileSync(specFromPath, '# Feature 63\n');

    const now = new Date().toISOString();
    await wf.persistEvents(tmp, '63', [
      { type: 'feature.paused', at: now },
      {
    ...
    const result = await runWorkflowPause(tmp, '63', { specFromPath, specToPath });
...
testAsync('resumes interrupted resume with pending effects', async () => {
  const tmp = makeTmpDir();
  try {
    await wf.startFeature(tmp, '73', 'solo_worktree', ['cc']);

    const pausedDir = path.join(tmp, 'docs', 'specs', 'features', '06-paused');
    const inProgressDir = path.join(tmp, 'docs', 'specs', 'features', '03-in-progress');
    fs.mkdirSync(pausedDir, { recursive: true });
    const specFromPath = path.join(pausedDir, 'feature-73-demo.md');
    const specToPath = path.join(inProgressDir, 'feature-73-demo.md');
    fs.writeFileSync(specFromPath, '# Feature 73\n');

    const now = new Date().toISOString();
    await wf.persistEvents(tmp, '73', [
      { type: 'feature.paused', at: now },
      { type: 'feature.resumed', at: now },
      {
        type: 'effect.requested',
        effect: {
    ...
    const result = await runWorkflowResume(tmp, '73', { specFromPath, specToPath });
