 ...
 *
 * Returns shape:
 *   { tone, glyph, verb, subject, owner, age, ageLabel, detail }
...
const { resolveAutonomousHeadline, resolveOperatorPauseLabel } = require('./pause-semantics');
...
const FINALISING_SIGNAL_LABEL = Object.freeze({
    'implementation-complete': 'Finalising implementation',
    'revision-complete':       'Finalising revision',
    'review-complete':         'Finalising review',
    'research-complete':       'Finalising research',
    'spec-review-complete':    'Finalising spec review',
});

// A session exit and its workflow completion signal are observed by separate
// collectors. Keep that normal handoff visually stable before treating it as
// a missing-confirmation recovery state.
const COMPLETION_HANDOFF_GRACE_SECONDS = 30;
...
const DRIVE_STATUS_MAP = Object.freeze({
    ...
    'addressing-code-review':  { tone: 'running',   verb: 'Addressing review',      subject: null },
    ...
    'ready':                   { tone: 'ready', verb: 'Implemented',     subject: null, detail: null },
    ...
    'revision-complete':       { tone: 'ready', verb: 'Revision complete', subject: null, detail: null },
    ...
    'quota-paused':            { tone: 'warn',      verb: 'Quota waiting',    subject: null,        detail: null },
});
...
function computeCardHeadline(entity, snapshot, agents, autonomousPlan, lane, options) {
    ...
    const closeReadiness = opts.closeReadiness || (entity && entity.closeReadiness) || null;
    const currentSpecState = snapshot ? (snapshot.currentSpecState || snapshot.lifecycle) : null;

    // ── 1. Warn-class — supersedes everything ──────────────────────────────
    if (opts.closingInProgress || (closeReadiness && closeReadiness.phase === 'closing')) {
        return makeHeadline('running', 'Closing…', { detail: closeReadiness && closeReadiness.closeLogHint });
    }
    if (entity && entity.lastCloseFailure) {
        const lcfKind = entity.lastCloseFailure.kind;
        const staleIntegrityFailure = (lcfKind === 'preauth-validation' || lcfKind === 'post-merge-gate' || lcfKind === 'criteria-attestation')
            && currentSpecState !== 'close_recovery_in_progress';
        if (!staleIntegrityFailure) {
    ...
    // ── 2b. Close readiness blocker (F658) — before autonomous handoff ───
    if (closeReadiness && closeReadiness.applicable && closeReadiness.primaryBlocker) {
        const pb = closeReadiness.primaryBlocker;
        const tone = (pb.kind === 'open-escalation' || pb.kind === 'autonomous-stopped')
            ? 'blocked'
            : 'attention';
        const verb = pb.kind === 'autonomous-stopped' || pb.kind === 'open-escalation'
            ? `Blocked: ${pb.label.toLowerCase()}`
            : `Blocked: ${pb.label}`;
        return makeHeadline(tone, verb, { detail: pb.detail });
    }
    ...
    const autonomousHeadline = resolveAutonomousHeadline(autonomousController);
    if (autonomousHeadline) {
        return makeHeadline(autonomousHeadline.tone, autonomousHeadline.verb, {
            detail: autonomousHeadline.detail,
            age: ageSecondsFrom(autonomousController.updatedAt || autonomousController.endedAt, now),
        });
    }
    ...
    // The agent's status file is complete while the engine still awaits the
    // corresponding event. Present this as the same finalisation handoff as a
    // newly-ended session so the normal path never flashes two different labels.
    ...
    if (pendingAgent) {
        ...
        const fallbackStage = typeof signal === 'string'
            ? signal.replace(/-complete$/, '').replace(/-/g, ' ')
            : 'work';
        return makeHeadline('running', FINALISING_SIGNAL_LABEL[signal] || `Finalising ${fallbackStage}`, {
    ...
    if (stages.length > 0 && (lane === 'in-progress' || lane === 'in-evaluation')) {
        ...
        if (running) {
            ...
            return makeHeadline('running', verb, {
                ...
                ageLabel: running.startedAt ? `${verb} for` : null,
            });
        }
    ...
    if (lane === 'in-progress' && agents.length > 0) {
        ...
        const implementationStartedAt = effectiveStatus === 'implementing'
            && entity && entity.startupReadiness && entity.startupReadiness.startedAt;
        let head = mapped
            ? makeHeadline(mapped.tone, mapped.verb, {
                  subject: mapped.subject,
                  owner: primary.id,
                  detail: mapped.detail || null,
                  age: implementationStartedAt ? ageSecondsFrom(implementationStartedAt, now) : null,
                  ageLabel: implementationStartedAt ? 'Implementation running for' : null,
              })
            : null;

        // The session exit is observed just before its workflow completion
        // event. Keep the normal handoff stable, then expose a recovery label
        // only if confirmation remains absent beyond the grace period.
        const flags = (primary && primary.flags) || {};
        if (flags.sessionEnded && status === 'implementing') {
            const endedAge = ageSecondsFrom(flags.sessionEndedAt, now);
            head = endedAge !== null && endedAge <= COMPLETION_HANDOFF_GRACE_SECONDS
                ? makeHeadline('running', 'Finalising implementation', { owner: primary.id })
                : makeHeadline('attention', 'Implementation needs confirmation', {
                      owner: primary.id,
    ...
    // ── 10b. Operator-parked lane / lifecycle ───────────────────────────────
    if (lane === 'paused' || currentSpecState === 'paused') {
        const verb = resolveOperatorPauseLabel(snapshot && snapshot.pauseReason);
        return makeHeadline('idle', verb, {});
    }
