...
function computeCardHeadline(entity, snapshot, agents, autonomousPlan, lane, options) {
    ...
    if (entity && entity.evalStatus === 'pick winner') {
        ...
        // (which opens a picker modal where they can override the winner
        // and optionally cherry-pick changes from the loser). The eval
        // section's 'Recommended: <agent>' line already names the winner;
        // duplicating it in the headline detail makes the card noisy.
        return makeHeadline('attention', 'Eval complete', {
