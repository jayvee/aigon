...
function computeCardHeadline(entity, snapshot, agents, autonomousPlan, lane, options) {
    ...
    if (pendingAgent) {
        ...
        return makeHeadline('attention', `Confirm ${signalLabel}`, {
    ...
    if (stages.length > 0 && (lane === 'in-progress' || lane === 'in-evaluation')) {
        ...
        if (running) {
            const verbLabel = STAGE_VERBS[running.type] || (running.label || running.type || 'run');
        ...
        if (failed) {
            const verbLabel = STAGE_VERBS[failed.type] || (failed.label || failed.type || 'Stage');
        ...
        if (waitingIdx > 0 && stages[waitingIdx - 1].status === 'complete') {
            ...
            const verbLabel = STAGE_VERBS[w.type] || (w.label || w.type || 'Stage');
    ...
    if (currentSpecState && STATE_RENDER_META[currentSpecState]) {
        ...
        return makeHeadline(t, meta.label || currentSpecState, {});
    }
