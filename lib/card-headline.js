...
function computeCardHeadline(entity, snapshot, agents, autonomousPlan, lane, options) {
    ...
    if (pendingAgent) {
        ...
        const SIGNAL_LABELS = {
            ...
            'spec-review-complete':    'Spec review done',
        };
        const friendlyLabel = (typeof signal === 'string' && SIGNAL_LABELS[signal])
            || (typeof signal === 'object' && signal && signal.message)
            || 'Agent finished';
        return makeHeadline('ready', friendlyLabel, {
    ...
    if (lane === 'in-progress' && agents.length > 0) {
        ...
        const effectiveStatus = entityType === 'research' && (status === 'submitted' || status === 'ready' || status === 'implementation-complete')
            ? 'research-complete'
