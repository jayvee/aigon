...
const DRIVE_STATUS_MAP = Object.freeze({
    ...
    'submitted':               { tone: 'attention', verb: 'COMPLETE',         subject: null,        detail: 'awaiting review' },
...
function computeCardHeadline(entity, snapshot, agents, autonomousPlan, lane, options) {
    ...
    if (lane === 'in-progress' && agents.length > 0) {
        ...
        if (head && head.detail === 'awaiting review' && agents.filter(a => a).length > 1) {
            head = { ...head, detail: 'awaiting evaluation' };
        }
