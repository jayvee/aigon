...
const DRIVE_STATUS_MAP = Object.freeze({
    ...
    'revising':                { tone: 'running',   verb: 'Revising',         subject: null },
    // 'addressing-review' is what the dashboard's feature-code-revise CLI
    // writes when injecting the revise prompt into the existing implementer
    // tmux session (lib/commands/feature.js:1041). Same user-facing state as
    // 'revising' (which is what worktree.js sets when starting a NEW session
    // for revise). Both map to the same Revising label.
    'addressing-review':       { tone: 'running',   verb: 'Revising',         subject: null },
