...
const { STAGE_FOLDERS, FEEDBACK_STAGE_DIRS } = require('./workflow-core/paths');
...
// Feedback stage dirs come from the workflow-core constants leaf (not utils —
// utils → dashboard-server → dashboard-fs-watch would be a circular require).
...
function createDashboardFsWatch(options = {}) {
    ...
    function attachWatcher(repoPath, watchPath, onEvent, watchOptions = {}) {
        const handle = fs.watch(watchPath, { persistent: false, ...watchOptions }, onEvent);
        handle.on('error', (err) => {
    ...
    function watchRepo(repoPath) {
        ...
        try {
            if (supportsRecursiveWatch()) {
                ...
                const initialWorkflowEntityDirs = new Set(resolveWorkflowEntityDirs(absRepoPath));
                for (const watchPath of resolveRepoWatchPaths(absRepoPath)) {
                    ...
                    entry.handles.push(handle);
                    if (initialWorkflowEntityDirs.has(watchPath)) {
                        entry.workflowEntityWatchPaths.add(watchPath);
                    }
                }
                for (const parent of [
