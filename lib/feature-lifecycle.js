...
async function runResume(args, deps) {
    const { ctx, def, persistAndRunEffects, findFile, PATHS } = deps;
            ...
            if (!isNumeric) {
                if (specLifecycle.isStableSpecLayout(process.cwd())) {
                    process.exitCode = 1;
                    return console.error(`❌ Stable spec layout requires a numeric workflow id for feature-resume.\n   Use \`aigon feature-resume <ID>\`.`);
                }
...
async function runUnprioritise(args, deps) {
    const { ctx, persistAndRunEffects, findFile, PATHS } = deps;
            ...
            const canonicalSpecPath = stableLayout
                ? (specLifecycle.findCanonicalSpecPath(repoPath, 'feature', workflowId) || specFromPath)
                : null;
            const specToPath = stableLayout
                ? canonicalSpecPath
                : path.join(PATHS.features.root, STAGE_FOLDERS.INBOX, inboxFilename);
            ...
            const unprioritiseEffects = stableLayout
                ? []
