...
function mapSnapshotActionToDashboard(entityType, entityId, action) {
  const mapping = {
    ...
    [ManualActionKind.RESEARCH_START]: {
      ...
      reason: 'Start this research topic',
    },
    [ManualActionKind.PAUSE_RESEARCH]: {
      action: 'research-pause',
      reason: 'Pause research',
    },
    [ManualActionKind.RESUME_RESEARCH]: {
      action: 'research-resume',
      reason: 'Resume paused research',
    },
    [ManualActionKind.RESEARCH_EVAL]: {
      action: 'research-eval',
      reason: 'Evaluate findings',
    },
    [ManualActionKind.RESEARCH_CLOSE]: {
      action: 'research-close',
      reason: 'Close research and move to done',
    },
  ...
  const isTransition = [
    'feature-pause', 'feature-resume', 'feature-eval', 'feature-close',
    'research-start', 'research-pause', 'research-resume', 'research-eval', 'research-close',
  ].includes(entry.action);
  // High priority for eval (all submitted) and close (winner picked)
  const isHighPriority =
    action.kind === ManualActionKind.FEATURE_EVAL ||
    action.kind === ManualActionKind.FEATURE_CLOSE ||
    action.kind === ManualActionKind.SELECT_WINNER ||
    action.kind === ManualActionKind.RESEARCH_EVAL ||
    action.kind === ManualActionKind.RESEARCH_CLOSE;
...
function mapSnapshotActionToBoard(entityType, entityId, action) {
  const mapping = {
    ...
    [ManualActionKind.RESEARCH_START]: 'research-start',
    [ManualActionKind.PAUSE_RESEARCH]: 'research-pause',
    [ManualActionKind.RESUME_RESEARCH]: 'research-resume',
    [ManualActionKind.RESEARCH_EVAL]: 'research-eval',
    [ManualActionKind.RESEARCH_CLOSE]: 'research-close',
    [ManualActionKind.RESTART_AGENT]: entityType === 'research' ? 'research-open' : 'feature-open',
...
async function readFeatureSnapshot(repoPath, featureId) {
  const snapshotPath = getSnapshotPathForEntity(repoPath, 'feature', featureId);
  try {
