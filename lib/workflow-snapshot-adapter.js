...
function snapshotToDashboardActions(entityType, entityId, snapshot, stage) {
  ...
  // When lastCloseFailure.kind === 'merge-conflict', swap FEATURE_CLOSE for FEATURE_RESOLVE_AND_CLOSE
  ...
  if (lcf && lcf.kind === 'merge-conflict' && closeFailureSwapStates.has(context.currentSpecState)) {
    nextActions = nextActions.map(a =>
      a.action === 'feature-close'
        ? {
            ...
            label: 'Resolve & close',
            reason: 'Resolve merge conflicts and retry close',
