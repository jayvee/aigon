...
const { getSnapshotPathForEntity } = require('./workflow-core/paths');
...
const LIFECYCLE_TO_STAGE = Object.freeze({
  [LifecycleState.INBOX]: 'inbox',
  [LifecycleState.BACKLOG]: 'backlog',
  [LifecycleState.SPEC_REVIEW_IN_PROGRESS]: 'backlog',
  [LifecycleState.SPEC_REVIEW_COMPLETE]: 'backlog',
  [LifecycleState.SPEC_REVISION_IN_PROGRESS]: 'backlog',
  [LifecycleState.SPEC_REVISION_COMPLETE]: 'backlog',
  [LifecycleState.IMPLEMENTING]: 'in-progress',
  [LifecycleState.READY]: 'in-progress',
  [LifecycleState.CODE_REVIEW_IN_PROGRESS]: 'in-progress',
  [LifecycleState.CODE_REVIEW_COMPLETE]: 'in-progress',
  [LifecycleState.CODE_REVISION_IN_PROGRESS]: 'in-progress',
  [LifecycleState.CODE_REVISION_COMPLETE]: 'in-progress',
  [LifecycleState.CLOSE_RECOVERY_IN_PROGRESS]: 'in-progress',
...
function createSyntheticContext(entityType, entityId, stage) {
  ...
  if (entityType === 'research') {
    base.researchId = entityId;
  } else {
    base.featureId = entityId;
  }
  // Pre-engine rows (no workflow snapshot yet): scheduling requires a real snapshot + addJob validation.
  base.__aigonSyntheticPreEngine = true;
  return base;
}
...
function getFreshSnapshotActions(context, entityTypeFromCaller) {
  if (!context) return [];
  const { deriveAvailableActions } = require('./workflow-core/actions');
  return deriveAvailableActions(context, entityTypeFromCaller);
}
...
const SNAPSHOT_ACTION_DESCRIPTORS = Object.freeze({
  ...
  [ManualActionKind.FEATURE_UNPRIORITISE]: { action: 'feature-unprioritise', reason: 'Return prioritised item to inbox', board: true },
  ...
  [ManualActionKind.FEATURE_SCHEDULE]: { action: 'feature-schedule', reason: 'Schedule a deferred autonomous kickoff', board: true },
  [ManualActionKind.FEATURE_STOP]: { action: 'feature-stop', reason: null, board: false },
  [ManualActionKind.PAUSE_FEATURE]: { action: 'feature-pause', reason: 'Park before start or interrupt implementation (operator only)', board: true },
  [ManualActionKind.RESUME_FEATURE]: { action: 'feature-resume', reason: 'Resume operator-parked feature', board: true },
  [ManualActionKind.FEATURE_CODE_REVIEW]: { action: 'feature-code-review', reason: 'Code review of implementation with a different agent', board: false },
  [ManualActionKind.FEATURE_CODE_REVISE]: { action: 'feature-code-revise', reason: 'Address code review feedback as the implementer', board: true },
  [ManualActionKind.FEATURE_SPEC_REVIEW]: { action: 'feature-spec-review', reason: 'Review the spec with a different agent', board: true },
  [ManualActionKind.FEATURE_SPEC_REVISE]: { action: 'feature-spec-revise', reason: 'Revise spec based on pending spec-review feedback', board: true },
  [ManualActionKind.FEATURE_CANCEL_SPEC_REVIEW]: { action: 'feature-cancel-spec-review', reason: 'Cancel stuck spec review and return to backlog', board: true },
  [ManualActionKind.FEATURE_CANCEL_SPEC_REVISION]: { action: 'feature-cancel-spec-revision', reason: 'Cancel stuck spec revision and return to backlog', board: true },
  [ManualActionKind.FEATURE_CANCEL_CODE_REVIEW]: { action: 'feature-cancel-code-review', reason: 'Cancel in-progress code review and return to ready', board: true },
  [ManualActionKind.FEATURE_EVAL]: { action: 'feature-eval', reason: 'All agents ready; compare implementations', board: true },
  ...
  [ManualActionKind.FEATURE_RESOLVE_AND_CLOSE]: { action: 'feature-resolve-and-close', reason: 'Resolve merge conflicts and retry close', board: true },
  [ManualActionKind.FEATURE_RESET]: { action: 'feature-reset', reason: 'Reset feature back to backlog (destructive)', board: true },
  [ManualActionKind.FEATURE_DELETE]: { action: 'feature-delete', reason: 'Delete this feature spec and workflow state', board: true },
  [ManualActionKind.FEATURE_NUDGE]: { action: 'feature-nudge', reason: 'Send a message into a running agent session', board: false },
  [ManualActionKind.RESEARCH_NUDGE]: { action: 'research-nudge', reason: 'Send a message into a running research agent session', board: false },
  [ManualActionKind.RESEARCH_START]: { action: 'research-start', reason: 'Start this research topic', board: true },
  [ManualActionKind.RESEARCH_SCHEDULE]: { action: 'research-schedule', reason: 'Schedule a deferred research start', board: true },
  [ManualActionKind.RESEARCH_PRIORITISE]: { action: 'research-prioritise', reason: 'Move to backlog', board: true },
  [ManualActionKind.RESEARCH_UNPRIORITISE]: { action: 'research-unprioritise', reason: 'Return prioritised topic to inbox', board: true },
  [ManualActionKind.RESEARCH_STOP]: { action: 'research-stop', reason: null, board: false },
  [ManualActionKind.PAUSE_RESEARCH]: { action: 'research-pause', reason: 'Park before start (operator only)', board: true },
  [ManualActionKind.RESUME_RESEARCH]: { action: 'research-resume', reason: 'Resume operator-parked research', board: true },
  ...
  [ManualActionKind.RESEARCH_SPEC_REVISE]: { action: 'research-spec-revise', reason: 'Revise research spec based on pending spec-review feedback', board: true },
  [ManualActionKind.RESEARCH_CANCEL_SPEC_REVIEW]: { action: 'research-cancel-spec-review', reason: 'Cancel stuck spec review and return to backlog', board: true },
  [ManualActionKind.RESEARCH_CANCEL_SPEC_REVISION]: { action: 'research-cancel-spec-revision', reason: 'Cancel stuck spec revision and return to backlog', board: true },
  [ManualActionKind.RESEARCH_CANCEL_CODE_REVIEW]: { action: 'research-cancel-code-review', reason: 'Cancel in-progress code review and return to ready', board: true },
  ...
  [ManualActionKind.RESEARCH_DELETE]: { action: 'research-delete', reason: 'Delete this research spec and workflow state', board: true },
  ...
  [ManualActionKind.SWITCH_AGENT]: { action: 'switch-agent', reason: null, board: false },
  ...
  [ManualActionKind.MARK_SUBMITTED]: { action: 'mark-submitted', reason: 'Mark agent as complete', board: false },
...
const HIGH_PRIORITY_ACTION_KINDS = new Set([
  ...
  ManualActionKind.FEATURE_RESOLVE_AND_CLOSE,
  ManualActionKind.SELECT_WINNER,
  ManualActionKind.RESEARCH_START,
...
function getSnapshotActionDescriptor(entityType, action) {
  ...
  if (action.kind === ManualActionKind.SWITCH_AGENT) {
    return { ...descriptor, reason: `Switch agent ${action.agentId || ''} to failover chain successor` };
  }
...
async function readWorkflowSnapshot(repoPath, entityType, entityId) {
  return getSpecStore(repoPath).readSnapshot(entityRef(entityType, entityId));
}
...
function getSpecStore(repoPath) {
  ...
  if (!store) {
    store = createSpecStore({ repoPath, storage });
...
function readWorkflowSnapshotSync(repoPath, entityType, entityId) {
  ...
  try {
    ...
    const data = getSpecStore(repoPath).readSnapshotSync(entityRef(entityType, entityId));
...
/**
 * Fresh read — bypasses the mtime cache. Used by SetConductor polling so a
 * just-closed feature is observed on the next poll (cache can otherwise lag).
 */
function readWorkflowSnapshotSyncUncached(repoPath, entityType, entityId) {
  return getSpecStore(repoPath).readSnapshotSync(entityRef(entityType, entityId));
}
...
function snapshotToDashboardActions(entityType, entityId, snapshot, stage) {
  const raw = snapshot || (stage ? createSyntheticContext(entityType, entityId, stage) : null);
  if (!raw) {
    return { nextAction: null, nextActions: [], validActions: [] };
  }
  // REGRESSION: deriveAvailableActions defaults to feature when context.entityType is missing;
  // the dashboard read path already knows the row kind — never trust a stale/missing snapshot.entityType alone.
  const context = Object.assign({}, raw, { entityType });

  let nextActions = getFreshSnapshotActions(context, entityType)
    .map(a => mapSnapshotActionToDashboard(entityType, entityId, a))
    .filter(Boolean);

  if (context.currentSpecState === LifecycleState.DONE || context.lifecycle === LifecycleState.DONE) {
    nextActions = nextActions.filter(a => a.category !== ActionCategory.SESSION);
  }

  // When lastCloseFailure is set, swap FEATURE_CLOSE for FEATURE_RESOLVE_AND_CLOSE
  // on states where a close recovery is plausible: in-flight implementation or active recovery.
  const lcf = raw && raw.lastCloseFailure;
  const closeFailureSwapStates = new Set([
    'implementing',
    'ready',
  ...
  const closeFailureSwapKinds = new Set([
    'merge-conflict',
    'post-merge-gate',
    'preauth-validation',
  ]);
  const swapIntegrityGateInRecovery = lcf && lcf.kind
    && (lcf.kind === 'post-merge-gate' || lcf.kind === 'preauth-validation')
    && context.currentSpecState === 'close_recovery_in_progress';
  const swapMergeConflict = lcf && lcf.kind === 'merge-conflict'
    && closeFailureSwapStates.has(context.currentSpecState);
  if (lcf && lcf.kind && closeFailureSwapKinds.has(lcf.kind)
    && (swapMergeConflict || swapIntegrityGateInRecovery)) {
    const resolveLabel = lcf.kind === 'merge-conflict' ? 'Resolve & close' : 'Close with agent';
    const resolveReason = lcf.kind === 'merge-conflict'
      ? 'Resolve merge conflicts and retry close'
      : 'Fix what blocked close on merged main, then retry';
    nextActions = nextActions.map(a =>
      a.action === 'feature-close'
        ? {
            ...a,
            action: 'feature-resolve-and-close',
            kind: ManualActionKind.FEATURE_RESOLVE_AND_CLOSE,
            label: resolveLabel,
            reason: resolveReason,
            command: formatDashboardActionCommand('feature-resolve-and-close', entityId),
            priority: 'high',
          }
        : a
    );
...
function snapshotToBoardCommand(entityType, entityId, snapshot) {
  ...
  const context = Object.assign({}, snapshot, { entityType });
  const actions = getFreshSnapshotActions(context, entityType);
...
function readWorkflowEventsSync(repoPath, entityType, entityId) {
  return getSpecStore(repoPath).readEventsSync(entityRef(entityType, entityId));
}
...
function parseEventTime(event) {
  const ms = event && event.at ? new Date(event.at).getTime() : NaN;
  return Number.isFinite(ms) ? ms : null;
}

function durationMs(fromIso, toIso) {
  const from = fromIso ? new Date(fromIso).getTime() : NaN;
  const to = toIso ? new Date(toIso).getTime() : NaN;
  if (!Number.isFinite(from) || !Number.isFinite(to)) return null;
  return Math.max(0, to - from);
}

function firstEventAt(events, predicate) {
  let best = null;
  for (const event of Array.isArray(events) ? events : []) {
    if (!predicate(event)) continue;
    const ms = parseEventTime(event);
    if (ms == null) continue;
    if (best == null || ms < best.ms) best = { ms, at: event.at };
  }
  return best ? best.at : null;
}

function lastEventAt(events, predicate) {
  let best = null;
  for (const event of Array.isArray(events) ? events : []) {
    if (!predicate(event)) continue;
    const ms = parseEventTime(event);
...
function computeStartupReadiness(events, snapshot) {
  const allEvents = Array.isArray(events) ? events : [];
  const agentIds = snapshot && snapshot.agents
    ? Object.keys(snapshot.agents).sort((a, b) => a.localeCompare(b))
    : [];
  const startedAt = firstEventAt(allEvents, event =>
    event.type === 'feature.started' || event.type === 'research.started'
  );
  const readyTypes = new Set(['signal.agent_ready', 'signal.agent_submitted', 'agent.marked_ready', 'agent.force_ready']);
  const agentTimings = agentIds.map(agentId => {
    const firstHeartbeatAt = firstEventAt(allEvents, event =>
      event.agentId === agentId && event.type === 'signal.heartbeat'
    );
    const agentReadyAt = firstEventAt(allEvents, event =>
      event.agentId === agentId && readyTypes.has(event.type)
    );
    return {
      agentId,
      firstHeartbeatAt,
      agentReadyAt,
      featureStartedToFirstHeartbeatMs: durationMs(startedAt, firstHeartbeatAt),
      firstHeartbeatToAgentReadyMs: durationMs(firstHeartbeatAt, agentReadyAt),
      featureStartedToAgentReadyMs: durationMs(startedAt, agentReadyAt),
    };
  });
  const firstHeartbeatAt = firstEventAt(allEvents, event =>
    event.type === 'signal.heartbeat' && (!agentIds.length || agentIds.includes(event.agentId))
  );
  const allHeartbeatsAt = agentIds.length > 0 && agentTimings.every(t => t.firstHeartbeatAt)
    ? lastEventAt(agentTimings.map(t => ({ at: t.firstHeartbeatAt })), () => true)
    : null;
  const allReadyAt = agentIds.length > 0 && agentTimings.every(t => t.agentReadyAt)
    ? lastEventAt(agentTimings.map(t => ({ at: t.agentReadyAt })), () => true)
    : null;
  const heartbeatCount = agentTimings.filter(t => t.firstHeartbeatAt).length;
  const readyCount = agentTimings.filter(t => t.agentReadyAt).length;
  let phase = null;
  let phaseLabel = null;
  if (startedAt && agentIds.length > 0 && !allReadyAt) {
    if (heartbeatCount < agentIds.length) {
      phase = heartbeatCount === 0 ? 'agents_booting' : 'agents_partially_booted';
      phaseLabel = heartbeatCount === 0
        ? 'Agents booting'
        : `Agents booting (${heartbeatCount}/${agentIds.length})`;
    } else {
      phase = 'agents_active';
      phaseLabel = 'Agents active';
    }
  } else if (allReadyAt) {
    phase = 'all_ready';
    phaseLabel = 'All agents ready';
  }
  return {
    startedAt,
    firstHeartbeatAt,
    ...
    allReadyAt,
    featureStartedToFirstHeartbeatMs: durationMs(startedAt, firstHeartbeatAt),
    featureStartedToAllHeartbeatsMs: durationMs(startedAt, allHeartbeatsAt),
    featureStartedToAllReadyMs: durationMs(startedAt, allReadyAt),
    heartbeatCount,
    readyCount,
...
module.exports = {
  ...
  readWorkflowSnapshotSyncUncached,
