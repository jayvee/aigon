...
function snapshotToDashboardActions(entityType, entityId, snapshot) {
  ...
  // Derive actions at read time from the snapshot context — never use cached availableActions.
  // This ensures action logic changes (e.g., solo vs fleet eval rules) take effect immediately
  // without requiring every snapshot on disk to be regenerated.
  const { deriveAvailableActions } = require('./workflow-core/actions');
  const freshActions = deriveAvailableActions(snapshot);

  const nextActions = freshActions
