...
const ACTION_SCOPES = {
    ...
    'set-prioritise':       { scope: 'main-only' },
    'feature-set-spec-review': { scope: 'main-only' },
    'feature-set-spec-revise': { scope: 'main-only' },
    'feature-unprioritise': { scope: 'main-only' },
    'research-unprioritise': { scope: 'main-only' },
...
function assertActionAllowed(action, context, options = {}) {
    ...
    if (def.scope === 'main-only') {
        ...
        try {
            ...
            execSync(`git checkout ${context.defaultBranch}`, { stdio: 'pipe' }); // nosemgrep: javascript.lang.security.detect-child-process.detect-child-process
...
// Collapses the repeated four-step delegation guard (build → assert → delegate →
// catch) into a single call. Handlers wrap their body in `withActionDelegate`
// and only the inner `fn` runs when the action is allowed locally; when the
// gatekeeper returns a delegate target, the command is re-run in the main repo
// and `fn` is skipped.
