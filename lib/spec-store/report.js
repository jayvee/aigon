...
const { fetchRemoteRefs, listRefSpecKeys, readRefPayload, remoteTrackingPrefix, fetchStateBranch, stateTrackingRef, runGit, refExists, readFileFromCommit, listTreeFiles } = require('./git-plumbing');
...
function loadRepoStorageConfig(repoPath) {
  try {
    ...
    if (raw.storage && (raw.storage.backend === 'git-ref' || raw.storage.backend === 'git-branch')) {
...
function readSpecsFromGitBranchCheckout(repoPath) {
  ...
  return keys.map((key) => {
    ...
    const leases = storage.backend === 'git-branch'
      ? leasesFromGitBranchTip(repoPath, tip, key)
      : deriveAllLeases(events, key);
...
async function runStorageReport(options = {}) {
  ...
  for (const repoPath of repoPaths) {
    ...
    } else if (storage && storage.backend === 'git-branch') {
