...
async function runStorageDoctor(repoPath, options = {}) {
  ...
  if (typeof store.readIdentityPending === 'function') {
    for (const row of store.readIdentityPending()) {
      issues.push({
        severity: 'warn',
        ...
        message: `Reserved-but-unmaterialised ${row.kind} id ${row.number} (since ${row.reservedAt})`,
      });
    }
  }
