...
const { withStorageFetchLock } = require('./fetch-lock');
const { attachIdentityAlloc } = require('./identity-alloc');
const { SEQUENCES_BRANCH_PATH } = require('./identity-sequences');
...
function createGitBranchBackend(repoPath, gitConfig) {
  ...
  async function mergeRemote(options = {}) {
    ...
    // Identity sequences are CAS-authoritative (monotonic reservation file).
    // Carry the remote blob forward verbatim when it differs — never union-merge.
    if (remoteTip && remoteTip !== localTip) {
      const remoteBlob = treeBlobSha(repoPath, remoteTip, SEQUENCES_BRANCH_PATH);
      const localBlob = treeBlobSha(repoPath, localTip, SEQUENCES_BRANCH_PATH);
      if (remoteBlob && remoteBlob !== localBlob) {
        updates[SEQUENCES_BRANCH_PATH] = readFileFromCommit(repoPath, remoteTip, SEQUENCES_BRANCH_PATH);
      }
    }
  ...
  async function fetchRemoteProjection(options = {}) {
    if (offline) return { ok: true, skipped: true, reason: 'offline' };
    const priorTip = commitSha(trackingRef);
    return withStorageFetchLock(async () => {
      fetchStateBranch(repoPath, remote, branch, trackingRef);
      const mergedKeys = await mergeRemote(options);
      const newTip = commitSha(trackingRef);
      return { ok: true, backend: 'git-branch', tipMoved: priorTip !== newTip, mergedKeys };
    });
  }
  ...
  async function runSyncBeforeWrite(ref) {
    ...
    await fetchRemoteProjection(key ? { key } : {});
  ...
  const coreStore = {
    ...
    async sync() {
      ...
      const merged = await withStorageFetchLock(async () => {
        fetchStateBranch(repoPath, remote, branch, trackingRef);
        return mergeRemote();
      });
      // Stable layout: the dashboard reads a feature's stage from the generated
      // lifecycle view, not the snapshot. mergeRemote() rebuilt local snapshots
      // from the merged remote events but left the view untouched — so a lifecycle
      // change made on another machine (e.g. a close) updates snapshot.json here
      // yet never moves the stage symlink, leaving the change invisible on this
      // machine's board. Regenerate the view once per sync so remote changes surface.
      try {
        const specLifecycle = require('../spec-lifecycle');
        if (specLifecycle.isStableSpecLayout(repoPath)) {
          specLifecycle.refreshLifecycleView(repoPath, { warn: () => {} });
        }
      } catch (_) { /* best-effort — a view refresh must never fail the sync */ }
      try {
        pushStateBranch(repoPath, remote, branch);
        advanceTrackingRefToTip();
  ...
  const storeWithLeases = Object.assign(coreStore, leaseApi);
  return attachIdentityAlloc(storeWithLeases, {
    repoPath,
    backend: 'git-branch',
    gitBranchCtx,
  });
