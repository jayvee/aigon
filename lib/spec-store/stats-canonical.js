...
async function recordCanonicalStats(repoPath, entityType, entityId, stats) {
  ...
  const remoteGitBackend = storage.backend === 'git-branch';
  if (!remoteGitBackend || !stats || typeof stats !== 'object') {
  ...
  if (syncResult && syncResult.ok === false) {
    throw new Error(syncResult.error || 'SpecStore sync failed');
  }
  return { recorded: true, eventId: getEventId(event), sync: syncResult };
}
