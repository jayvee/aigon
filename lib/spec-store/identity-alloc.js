...
/**
 * Reserve the next identity without a full SpecStore (CLI convenience).
 *
 * @param {string} repoPath
 * @returns {{ kind: string, number: number, numericId: string, key: string, paddedId: string }}
 */
function reserveIdentityForRepo(repoPath) {
  const { createSpecStore } = require('./index');
  const store = createSpecStore({ repoPath });
  if (typeof store.reserveIdentitySync !== 'function') {
    throw new Error('SpecStore missing reserveIdentitySync');
  }
  return store;
}
