...
/** Permanent internal test seam for deterministic two-clone interleavings (F612). */
let casTestHooks = null;

function setCasTestHooks(hooks) {
  casTestHooks = hooks || null;
}

function clearCasTestHooks() {
  casTestHooks = null;
}

async function invokeCasTestHook(phase, payload) {
  if (!casTestHooks || typeof casTestHooks[phase] !== 'function') return;
  await casTestHooks[phase](payload);
}
...
/** A rejected FF-only push (someone else advanced the branch) vs a network/hook fault. */
...
function createGitBranchLeaseApi(ctx) {
  ...
  async function casTransition({ key, role, decide }) {
    for (let attempt = 1; attempt <= MAX_LEASE_CAS_RETRIES; attempt += 1) {
      ...
      await invokeCasTestHook('afterFetch', { repoPath, key, role, attempt });
      ...
      await invokeCasTestHook('beforePush', { repoPath, key, role, attempt, baseTip });

      try {
        pushStateBranch();
        advanceTrackingRefToTip();
        await invokeCasTestHook('afterPushBeforeProjection', { repoPath, key, role, attempt });
  ...
  async function clearExpiredLeasesForKey(key) {
    ...
    const leasesPath = leasesPathForKey(key);
    const map = parseLeaseFile(readFileFromCommit(baseTip, leasesPath));
    const expiredRoles = Object.entries(map)
      .filter(([, record]) => isLeaseRecordExpired(record))
      .map(([role]) => role);
    if (expiredRoles.length === 0) return { ok: true, cleared: 0 };

    const nextMap = { ...map };
    let events = readEventsFromCommit(baseTip, key);
    for (const role of expiredRoles) {
      const existing = nextMap[role];
      delete nextMap[role];
      const event = buildLeaseEvent('lease.released', {
        key,
        role,
      ...
      if (!events.some((entry) => getEventId(entry) === eventId)) {
        events = [...events, event];
      }
    }
    const updates = {
      [leasesPath]: serializeLeaseFile(nextMap),
      [eventsPathForKey(key)]: serializeEventsJsonl(events),
    };
    commitUpdates(updates, [], `aigon-specstore: doctor clear expired leases ${key}`);
    try {
      pushStateBranch();
      advanceTrackingRefToTip();
      await rebuildProjectionForKey(key, events);
      writeSyncState({ lastSyncAt: new Date().toISOString(), lastFetchAt: new Date().toISOString(), lastError: null, lastFetchError: null });
    } catch (error) {
      return { ok: false, error: error.message, cleared: expiredRoles.length };
    }
    return { ok: true, cleared: expiredRoles.length };
  }
  ...
  return {
    ...
    assertLeaseAllowed,
    readLeaseEvents,
    clearExpiredLeasesForKey,
  };
}

module.exports = {
  createGitBranchLeaseApi,
  MAX_LEASE_CAS_RETRIES,
  setCasTestHooks,
  clearCasTestHooks,
  isNonFastForward,
};
