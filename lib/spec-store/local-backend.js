...
function createLocalBackend(repoPath) {
  ...
  async syncBeforeWrite() {
    return { ok: true, backend: 'local', skipped: true };
  },
  ...
  const store = Object.assign(backend, createLeaseApi(backend));
  return attachIdentityAlloc(store, { repoPath, backend: 'local' });
}
