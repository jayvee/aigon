...
const PROBE_PACE_MS = process.env.AIGON_QUOTA_TEST === '1' ? 0 : 1000;
...
async function phaseBudget({ repoPath, log, force }) {
    ...
    const budgetAgents = ['cc', 'cx', 'km', 'ag'];
    let anyOk = false;
    let anyError = false;
            ...
            if (agentId === 'cc') {
                ...
                if (!agInteractiveOk) {
                    outcomes.budgetByAgent[agentId] = {
                        probeMethod: 'skipped-interactive-auth',
                        lastError: 'ANTIGRAVITY_TOKEN not set',
                        erroredAt: new Date().toISOString(),
                    };
                    continue;
                }
                result = await budgetPoller.pollAntigravityBudget({ repoPath, log: logger });
            }
            if (result) {
    ...
    for (const agent of agentRegistry.getAllAgents()) {
        ...
        for (const target of targets) {
            ...
            try {
                ...
                if (result.result && result.result.exitCode != null && result.result.exitCode !== 0) {
                    anyError = true;
                    agentQuotaRead.patchAgentQuotaState(repoPath, (state) => {
                        ...
                        agentState.models[modelKey] = {
                            ...
                            verdict: 'error',
                            probeMethod: 'error',
                            lastError: `probe exited ${result.result.exitCode}`,
                            erroredAt: new Date().toISOString(),
