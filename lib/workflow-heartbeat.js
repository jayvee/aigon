 ...
 */
async function sweepEntityExpiredHeartbeats(repoPath, entityType, entityId, options = {}) {
  const snapshotPath = getSnapshotPathForEntity(repoPath, entityType, entityId);
  if (!fs.existsSync(snapshotPath)) return [];

  const snapshot = await wf.showEntity(repoPath, entityType, entityId);
  ...
  for (const [agentId, agent] of Object.entries(snapshot.agents)) {
    ...
    if (now - lastBeat > timeoutMs) {
      try {
        await wf.emitSignal(repoPath, entityId, 'heartbeat-expired', agentId, { entityType });
...
function listEntityIds(repoPath, entityType) {
  ...
  const root = path.join(repoPath, '.aigon', 'workflows', dirName);
  if (!fs.existsSync(root)) return [];
  return fs.readdirSync(root).filter((entry) => /^\d+$/.test(entry));
}

async function sweepExpiredHeartbeats(repoPath, featureId, options = {}) {
  // Backward-compatible single-entity call:
  // sweepExpiredHeartbeats(repoPath, featureId, options)
  if (typeof featureId === 'string') {
    const entityType = options.entityType || 'feature';
    return sweepEntityExpiredHeartbeats(repoPath, entityType, featureId, options);
  }

  // New all-entities call:
  // sweepExpiredHeartbeats(repoPath, { ...options })
  const resolvedOptions = (featureId && typeof featureId === 'object') ? featureId : options;
  const results = [];
  for (const entityType of ['feature', 'research']) {
    const entityIds = listEntityIds(repoPath, entityType);
    for (const entityId of entityIds) {
      const expiredAgents = await sweepEntityExpiredHeartbeats(repoPath, entityType, entityId, resolvedOptions);
      expiredAgents.forEach((agentId) => {
        results.push({ entityType, entityId, agentId });
      });
    }
  }
  return results;
