...
function sessionsForScenario(spec) {
    ...
    if (spec.state === 'implementing') {
        ...
        sessions.push(session(
            `${spec.entityType}-${spec.key}-review`,
            'cx',
            spec.entityType === 'research' ? 'findings-review' : 'code-review',
            spec.entityType === 'research' ? 'Reviewing findings' : 'Reviewing code',
        ));
...
function featureVariants() {
    ...
    return [
        stateScenario('feature', 'backlog', {
            key: 'feature-continuity-origin-unavailable',
            scenario: 'Fresh handoff selected',
            detail: 'The historic spec has no attributable origin session, so Aigon starts fresh with the durable handoff.',
            context: {
                agents: {}, authorAgentId: 'cx',
                specAuthor: { agentId: 'cx', model: null, effort: null, authoredAt: FIXED_TIME },
            },
            gallery: {
                continuity: {
                    originSession: { source: 'direct-agent-session', captureState: 'unavailable', hasNativeSession: false },
                    authorHandoff: { schemaVersion: 1, artifactVersion: 1, status: 'valid' },
                    latestContinuityDecision: {
                        strategy: 'fresh-with-handoff', confidence: 'high', reasons: ['origin-unavailable'], selectedAgent: 'cc',
                    },
                },
            },
        }),
        stateScenario('feature', 'spec_revision_in_progress', {
            key: 'feature-continuity-checkpoint-fallback',
            scenario: 'Continuation fell back safely',
            detail: 'The resumed author reported a context conflict; one fresh session now has the validated handoff.',
            context: { authorAgentId: 'cc', agents: { cc: baseAgent('running') } },
            gallery: {
                continuity: {
                    originSession: { source: 'aigon-launched', captureState: 'captured', hasNativeSession: true },
                    authorHandoff: { schemaVersion: 1, artifactVersion: 2, status: 'valid' },
                    latestContinuityDecision: {
                        strategy: 'fresh-with-handoff', confidence: 'high', reasons: ['checkpoint-fallback', 'context-conflict'], selectedAgent: 'cc',
                        parentOriginSessionId: 'spec-draft-feature-675',
                    },
                },
            },
        }),
        ...
        stateScenario('feature', 'ready', {
            ...
            detail: 'The completed review approved the implementation. Close or address the review; another review remains optional.',
            context: {
                agents: readySolo,
                codeReview: { reviewCompletedAt: FIXED_TIME },
            },
            gallery: {
                sessions: [{
                    ...session('feature-implementing-ready-solo-review', 'op', 'code-review', 'Code review', 'complete'),
                    sessionRunning: true,
        ...
        stateScenario('feature', 'ready', {
            key: 'feature-review-addressing',
            scenario: 'Addressing review',
            context: {
                agents: {
                    cc: {
                        ...baseAgent('ready'),
                        runtimeStatus: 'addressing-code-review',
                    },
                },
                codeReview: { reviewCompletedAt: FIXED_TIME },
            },
            gallery: {
                sessions: [
                    session('feature-review-addressing-implementation', 'cc', 'implementation', 'Implementation'),
                    {
                        ...session('feature-review-addressing-review', 'op', 'code-review', 'Code review', 'complete'),
                        sessionRunning: true,
                        requestRevision: false,
                        completedAt: FIXED_TIME,
                    },
        ...
        stateScenario('feature', 'ready', {
            key: 'feature-review-addressed',
            scenario: 'Revision complete',
            context: {
                agents: {
                    cc: {
                        ...baseAgent('ready'),
                        runtimeStatus: 'revision-complete',
                    },
                },
                codeReview: { reviewCompletedAt: FIXED_TIME },
            },
            gallery: {
                sessions: [
                    session('feature-review-addressed-implementation', 'cc', 'implementation', 'Implementation'),
                    {
                        ...session('feature-review-addressed-review', 'op', 'code-review', 'Code review', 'complete'),
                        sessionRunning: true,
                        requestRevision: false,
                        completedAt: FIXED_TIME,
                    },
        ...
        stateScenario('feature', 'implementing', {
            key: 'feature-implementation-ready-to-push',
            scenario: 'Finalising implementation',
            detail: 'The implementation result is recorded while the workflow completion event catches up.',
            context: {
                agents: {
                    cc: baseAgent('ready', {
                        isWorking: false,
        ...
        stateScenario('feature', 'implementing', {
            ...
            scenario: 'Implementation needs confirmation',
            detail: 'The session ended but its workflow completion event is still missing.',
            severity: 'warning',
            context: {
                agents: {
                    cc: baseAgent('running', {
...
function buildSetCurrentFeatureContract(options) {
    ...
    const closeFailure = Boolean(options.currentCloseFailure);
    const lifecycle = closeFailure ? 'ready'
        : (failed ? 'implementing' : (memberStatus === 'stopped' ? 'ready' : 'implementing'));
    ...
    const manual = Boolean(options.manualCurrentFeature);
    const plan = manual ? null : autonomousPlan(
        closeFailure ? 'close' : 'implement',
        closeFailure || failed ? 'failed' : (running ? 'running' : 'stopped'),
    );
    const lastCloseFailure = closeFailure
        ? { kind: 'merge-conflict', conflictFiles: ['site/package-lock.json'] }
        : null;
    const actionContext = makeContext('feature', lifecycle, 'solo_worktree', {
        featureId: String(options.currentFeature),
        agents: {},
        lastCloseFailure,
    });
    const validActions = snapshotToDashboardActions(
        'feature',
        String(options.currentFeature),
        actionContext,
        'in-progress',
    ).validActions;
    return buildFeatureUiContract({
        ...
        cardHeadline: {
            verb: closeFailure ? 'Close failed'
                : (failed ? 'Implementation failed' : (running ? 'Implementing' : 'Implementation complete')),
            owner: manual ? 'cc' : null,
            tone: manual && running ? 'running' : null,
        },
        cardPresentation: {
            severity: closeFailure || failed ? 'error' : 'normal',
            contextLine: closeFailure ? 'Merge conflict in site/package-lock.json' : null,
        },
        lastCloseFailure,
        autonomousPlan: plan,
        autonomousController: manual ? null : {
            status: closeFailure || failed ? 'failed' : (running ? 'running' : 'stopped'),
            running,
        },
    }, actionContext);
}

function buildSetMemberContracts(members, dependencyEdges) {
    const memberById = new Map(members.map(member => [String(member.id), member]));
    return members.map((member) => {
        const lifecycle = member.status === 'complete' ? 'done'
            : (member.status === 'running' ? 'implementing'
                : (member.status === 'waiting' ? 'backlog' : member.status));
        const context = makeContext('feature', lifecycle, 'solo_worktree', {
            featureId: String(member.id),
            agents: {},
        });
        const validActions = snapshotToDashboardActions(
            'feature',
            String(member.id),
            context,
            stageForState(lifecycle),
        ).validActions;
        const blockedBy = dependencyEdges
            .filter(edge => String(edge.from) === String(member.id))
            .map((edge) => {
                const dependency = memberById.get(String(edge.to));
                return {
                    id: edge.to,
                    displayKey: `F${edge.to}`,
                    name: dependency ? dependency.name : `Feature ${edge.to}`,
                };
            });
        return buildFeatureUiContract({
            id: member.id,
            displayKey: member.id === 'new' ? null : `F${member.id}`,
            name: member.name,
            stage: member.status === 'complete' ? 'done'
                : (member.status === 'running' ? 'in-progress'
                    : (member.status === 'waiting' ? 'backlog' : member.status)),
...
function setScenario(key, scenario, status, options = {}) {
    const autonomous = Object.prototype.hasOwnProperty.call(options, 'autonomous')
        ? options.autonomous
        : (status === 'idle' ? null : {
        status,
        members: ['681', '682', '683'],
    ...
    const setState = {
        ...
        activeMemberCount: options.activeMemberCount || 0,
    };
    const validActions = buildSetValidActions(setState);
    ...
    const dependencyEdges = [{ from: '682', to: '681' }, { from: '683', to: '682' }];
    const setCard = {
        slug: 'autonomous-recovery',
        ...
        lastEvent: autonomous ? { label: scenario, at: FIXED_TIME } : null,
        depGraph: {
    ...
    const memberContracts = buildSetMemberContracts(members, dependencyEdges);
    return {
        key,
        entityType: 'set',
        mode: 'set',
        ...
        scenario,
        detail: options.detail || null,
        state: contract.state.lifecycle,
        stateLabel: options.stateLabel || contract.state.label,
        lane: options.lane || contract.state.lane || (options.isComplete ? 'done' : (status === 'idle' ? 'backlog' : 'in-progress')),
        contract,
        ...
        memberContracts,
        showExpandedMembers: Boolean(options.showExpandedMembers),
    };
}
...
function setScenarios() {
    return [
        setScenario('set-inbox-members', 'Set contains inbox features', 'idle', {
            detail: 'Prioritise the inbox members before spec review or autonomous start.',
            inboxMemberCount: 2,
        ...
        setScenario('set-manual-running', 'Manual member in progress', 'running', {
            autonomous: null,
            activeMemberCount: 1,
            currentFeature: '682',
            currentMemberStatus: 'running',
            manualCurrentFeature: true,
            completed: ['681'],
        }),
        setScenario('set-paused-failure', 'Set paused after a feature failed', 'paused-on-failure', {
            detail: 'F682 failed to close. Resolve the merge conflict, then resume the set.',
            completed: ['681'],
            currentFeature: '682',
            failedFeature: '682',
            currentCloseFailure: true,
        ...
        setScenario('set-conductor-interrupted', 'Set conductor interrupted after completed implementation', 'interrupted', {
            detail: 'F682 implementation completed, but the detached conductor disappeared before review. Resume with the same agents, choose another pairing, or take over manually.',
            completed: ['681'],
            currentFeature: '682',
            currentMemberStatus: 'complete',
            stateLabel: 'Conductor interrupted',
            severity: 'error',
        }),
...
function buildScenario(spec, index) {
    ...
    const agentRows = Object.entries(spec.context.agents || {}).map(([id, agent]) => ({
        id,
        ...agent,
        status: agent.runtimeStatus || agent.status,
    }));
    const sessions = sessionsForScenario(spec);
    const agentOwnedStates = new Set(['implementing', 'ready', 'code_revision_in_progress']);
    const headlineOwner = Object.prototype.hasOwnProperty.call(spec.gallery, 'headlineOwner')
        ? spec.gallery.headlineOwner
        : (agentRows.length === 1 && agentOwnedStates.has(spec.state) ? agentRows[0].id : null);
    const headlineAgent = headlineOwner
        ? agentRows.find(agent => String(agent.id) === String(headlineOwner))
        : null;
    const runningHeadlineStatuses = new Set([
        'running', 'implementing', 'reviewing', 'researching', 'addressing-code-review',
    ]);
    const headlineTone = headlineAgent && runningHeadlineStatuses.has(headlineAgent.status)
        ? 'running'
        : null;
    const galleryElapsed = spec.entityType === 'feature'
        && spec.state === 'implementing'
        && spec.mode === 'solo_worktree'
        ? { age: 9 * 60, ageLabel: 'Implementation running for' }
        : {};
    let contract;
    if (spec.entityType === 'feature') {
        contract = buildFeatureUiContract({
            ...
            cardHeadline: { verb: spec.scenario, detail: spec.detail, owner: headlineOwner, tone: headlineTone, ...galleryElapsed },
            cardPresentation: {
        ...
        contract = buildResearchUiContract({
            ...
            validActions: contractActions,
            stateRenderMeta: stateMeta,
            cardHeadline: { verb: spec.scenario, detail: spec.detail, owner: headlineOwner, tone: headlineTone },
            cardPresentation: {
                severity: spec.severity || (spec.state === 'paused' ? 'warning' : 'normal'),
                contextLine: spec.detail,
            },
    ...
    return {
        ...
        specAuthor: spec.context.specAuthor || null,
        contract,
        dashboardActions,
        ...
        autonomousController: spec.gallery.autonomousController || null,
        continuity: spec.gallery.continuity || null,
    };
}
    ...
    return {
        ...
        coverage: {
            ...
            set: {
                entityType: 'set',
                definitionVersion: 2,
                restingStates: ['inbox', 'idle', 'running', 'interrupted', 'paused-on-failure', 'paused-on-quota', 'stopped', 'done'],
