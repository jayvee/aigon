...
function findFile(typeConfig, nameOrId, searchFolders = typeConfig.folders) {
    const kind = typeConfig.prefix === 'research' ? 'research' : 'feature';
    const resolved = tryResolveSpecIdentity(nameOrId, { kind });
    const lookupId = resolved ? resolved.numericId : nameOrId;
    const isId = /^\d+$/.test(lookupId);
    for (const folder of searchFolders) {
        ...
        for (const file of files) {
            ...
            if (isId) {
                ...
                const paddedId = String(lookupId).padStart(2, '0');
                const unpadded = String(parseInt(lookupId, 10));
...
function findUnprioritizedFile(typeConfig, name) {
    const foldersToSearch = [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.PAUSED];
...
/**
 * Find an inbox/paused spec by slug, numeric id, or display key (numbered or legacy slug-only).
 */
function findInboxFile(typeConfig, name) {
    const byLookup = findFile(typeConfig, name, [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.PAUSED]);
    if (byLookup) return byLookup;
    return findUnprioritizedFile(typeConfig, name);
}

/**
 * @returns {{ paddedId: string|null, slug: string, legacy: boolean }|null}
 */
function parseNumberedSpecFilename(typeConfig, filename) {
    const idMatch = filename.match(new RegExp(`^${typeConfig.prefix}-(\\d+)-(.+)\\.md$`));
    if (idMatch) {
        return { paddedId: idMatch[1], slug: idMatch[2], legacy: false };
    }
    const slugMatch = filename.match(new RegExp(`^${typeConfig.prefix}-(.+)\\.md$`));
    if (slugMatch && !/^\d+$/.test(slugMatch[1])) {
        return { paddedId: null, slug: slugMatch[1], legacy: true };
    }
    return null;
}
