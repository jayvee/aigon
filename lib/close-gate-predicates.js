...
function isImplementationLogRetry(snapshot) {
    return Boolean(
        snapshot
        && snapshot.currentSpecState === 'close_recovery_in_progress'
        && snapshot.lastCloseFailure
        && snapshot.lastCloseFailure.kind === 'implementation-log',
    );
}
...
const ADVISORY_CLOSE_RECOVERY_SOURCES = new Set([
    'preauth-validation-advisory',
    'post-merge-gate-advisory',
    'implementation-log-advisory',
    'migration/advisory-close-recovery',
]);

function shouldClearIntegrityCloseFailureOnRecoveryEnd(event, lastCloseFailure) {
    if (!event || !ADVISORY_CLOSE_RECOVERY_SOURCES.has(event.source)) return false;
    const kind = lastCloseFailure && lastCloseFailure.kind;
    return !kind || kind === 'criteria-attestation' || kind === 'preauth-validation' || kind === 'post-merge-gate' || kind === 'implementation-log';
}
