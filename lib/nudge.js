...
async function sendNudge(repoPath, rawId, message, options = {}) {
    ...
    if (transport.useSendKeys) {
        // Use send-keys -l (literal key input) then Enter — same mechanism as the injection
        ...
        tmuxFn(['send-keys', '-t', session.sessionName, '-l', text], {
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'pipe'],
        });
        sleepMs(100);
        tmuxFn(['send-keys', '-t', session.sessionName, transport.submitKey || 'Enter'], {
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'pipe'],
        });
    } else {
