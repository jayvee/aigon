...
function extractLastPromptLine(paneTail) {
    ...
    for (let i = lines.length - 1; i >= 0; i -= 1) {
        ...
        // `›` U+203A (Codex). Without this, Codex prompts are invisible to the
        // submit-success detector and submitNudge falsely reports ok on first
        // iteration even when Enter never landed.
...
async function sendNudge(repoPath, rawId, message, options = {}) {
    ...
    if (transport.useSendKeys) {
        // Use send-keys -l (literal key input) then submitKey — same mechanism as the injection
        // subshell. Avoids paste-buffer -p which can leave some TUIs (e.g. kimi) in a state
        // where the subsequent C-m is ignored. tmuxInjectLiteral defensively cancels copy-mode
        // first so a scrolled-up pane doesn't hang send-keys -l indefinitely.
        tmuxInjectLiteral(session.sessionName, text, {
            submitKey: transport.submitKey || 'Enter',
            deps: typeof deps.runTmux === 'function' ? { runTmux: deps.runTmux } : undefined,
        });
    } else {
        ...
        submitPaneTail = submit.paneTail;
    }
