...
const { readPublicEntityContext } = require('./entity-context');
...
function decorateDetailEvent(event) {
    ...
    if ((type === 'feature.bootstrapped' || type === 'research.bootstrapped') && event) {
        const prioritised = event.stage === 'backlog' || event.lifecycle === 'backlog';
        return {
            ...event,
            displayLabel: prioritised
                ? (type.startsWith('research.') ? 'Research prioritised' : 'Feature prioritised')
                : (type.startsWith('research.') ? 'Research created' : 'Feature created'),
            displayActor: event.specAuthor?.agentId || event.authorAgentId || 'system',
        };
    }
    const displayMap = {
...
function buildDetailPayload(repoPath, type, id, specPathHint, options = {}) {
    ...
    const continuityContext = (type === 'feature' || type === 'research')
        ? readPublicEntityContext(absRepo, type, id)
        : { originSession: null, authorHandoff: null, latestContinuityDecision: null };
    Object.assign(manifest, continuityContext);
    ...
    return {
        ...
        originSession: manifest.originSession || null,
        authorHandoff: manifest.authorHandoff || null,
        latestContinuityDecision: manifest.latestContinuityDecision || null,
