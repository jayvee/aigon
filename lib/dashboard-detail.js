...
const { buildCriteriaAttestationView } = require('./criteria-attestation');
const {
...
function decorateDetailEvent(event) {
    ...
    const displayMap = {
        ...
        'feature.close_gate_failed': {
            label: event && event.gateKind === 'preauth-validation'
                ...
                : (event && event.gateKind === 'criteria-attestation'
                    ? 'Criteria attestation failed'
                    : 'Close gate failed'),
            actor: 'system',
        },
        ...
        'feature.criteria_attested': {
            label: 'Criteria attested',
            actor: 'system',
        },
        'feature.criteria_attestation_bypassed': {
            label: 'Criteria attestation bypassed',
            actor: 'system',
        },
    ...
    if (type === 'feature.close_gate_failed' && event.gateKind === 'criteria-attestation') {
        const parts = [];
        if (Array.isArray(event.unattested) && event.unattested.length > 0) {
            parts.push(`missing: ${event.unattested.join(', ')}`);
        }
        if (Array.isArray(event.invalid) && event.invalid.length > 0) {
            parts.push(`invalid: ${event.invalid.join(', ')}`);
        }
        if (parts.length > 0) decorated.message = parts.join('; ');
    }
    if (type === 'feature.criteria_attested' && event.counts) {
        const c = event.counts;
        decorated.message = `${c.met || 0} met, ${c.deferred || 0} deferred, ${c.dropped || 0} dropped / ${c.total || 0}`;
    }
...
function buildDetailPayload(repoPath, type, id, specPathHint, options = {}) {
    ...
    let criteriaAttestation = [];
    if (type === 'feature' && resolvedSpecPath) {
        try {
            criteriaAttestation = buildCriteriaAttestationView(resolvedSpecPath, snapshot);
        } catch (_) {
            criteriaAttestation = [];
        }
    }
