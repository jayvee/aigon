...
function buildManagedProjectIndex(aigonDir) {
    return buildFileIndex(aigonDir, (ent) => !isProjectEntryExcluded(ent.name, ent.isDirectory()));
}
...
function restoreProjectExactly(sourceDir, repoPath, archiveDir, token) {
...
function buildPullPlan(helper) {
    ...
    for (const ent of fs.readdirSync(projectsRoot, { withFileTypes: true })) {
        ...
        const localIndex = buildManagedProjectIndex(path.join(local, '.aigon'));
        const incomingIndex = buildManagedProjectIndex(sourceDir);
        projects.push({
            name: ent.name,
            path: local,
            sourceDir,
...
function pull(options = {}) {
    ...
    if (fs.existsSync(settingsConfig)) {
        ...
        core.saveJson(path.join(archiveRoot, 'settings', 'config.json'), local);
