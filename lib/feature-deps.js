 ...
 * Returns { slug, status } where status is the folder name.
 ...
 * (backlog or later, not inbox and not missing).
...
function checkDepsPrioritised(parentRefs, paths) {
    ...
    for (const ref of parentRefs) {
        ...
        if (status === STAGE_FOLDERS.INBOX || status === 'missing') {
...
function scanInboxBySet(setSlug, specRoot) {
    const inboxDir = path.join(specRoot, STAGE_FOLDERS.INBOX);
...
function getAllKnownSets(specRoot, folders) {
    const stageFolders = folders || [STAGE_FOLDERS.INBOX, STAGE_FOLDERS.BACKLOG];
    const sets = new Set();
