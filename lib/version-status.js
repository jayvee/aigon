...
function getRepoVersionStatus(repoPath) {
    ...
    if (!isWorktree) {
        appliedVersion = getInstalledVersion(repoPath);
        try {
    ...
    try {
        ...
        const dashboardRuntimePath = getDashboardRuntimePath(slotId);
        if (fs.existsSync(dashboardRuntimePath)) {
            const rt = JSON.parse(fs.readFileSync(dashboardRuntimePath, 'utf8'));
            if (rt.pid) {
                try {
...
module.exports = { getRepoVersionStatus, formatDriftNotice, isAigonDevMode, getDashboardRuntimePath };
