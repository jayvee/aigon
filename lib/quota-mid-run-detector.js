...
/**
 * F446 Mid-run quota detection via tmux pane scan (runs on dashboard-status-collector polls).
 */
 ...
 * @param {object} [deps]
 */
function scanActiveSessions(repoPath, deps = {}) {
    ...
    for (const f of entries) {
        ...
        let alive = tmuxSessionExists(sessionName);
        if (!alive && sidecar.tmuxId) {
            const { target, isId } = resolveTmuxTarget(sidecar.tmuxId, sessionName);
            alive = tmuxSessionExists(target);
            if (!isId) alive = alive || tmuxSessionExists(sessionName);
        }
        if (!alive) continue;
        ...
        let paneRaw;
        try {
            paneRaw = capturePaneText(sessionName, sidecar.tmuxId);
        } catch (_) {
            continue;
        }
        if (!paneRaw) continue;
