...
const { DEFAULT_STATE_BRANCH } = require('./spec-store/storage-config');
...
function buildRepoStorageStatus(repoPath) {
  ...
  if (storage.backend === 'git-branch') {
    branch = storage.git.branch || DEFAULT_STATE_BRANCH;
    ...
    ({ ahead, behind } = computeAheadBehind(repoPath, localTip, remoteTip));
  } else {
    refPrefix = storage.git.refPrefix;
  ...
  if (branch) dto.branch = branch;
  if (refPrefix) dto.refPrefix = refPrefix;
  return dto;
}
...
function buildRepoStorageActions(storageStatus) {
  ...
  if (storageStatus && (storageStatus.backend === 'git-ref' || storageStatus.backend === 'git-branch')) {
