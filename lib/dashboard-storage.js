...
const FOREIGN_LEASE_BLOCKED_ACTIONS = Object.freeze({
  feature: new Set(['feature-close', 'feature-resolve-and-close', 'feature-reset']),
  research: new Set(['research-close', 'research-reset']),
});
...
function applyForeignLeaseActionBlocks(item, entityType, leases) {
  if (!item || !Array.isArray(item.validActions)) return;
  const foreign = firstForeignLease(leases);
  if (!foreign) return;
...
function attachActiveLeasesToEntities(repoPath, entities, entityType) {
  ...
  entities.forEach((item) => {
    ...
    if (leases.length > 0) {
      ...
      applyForeignLeaseActionBlocks(item, entityType, leases);
    }
  });
}
