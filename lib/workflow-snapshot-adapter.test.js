...
test('validActions are action kinds', () => {
    const snap = buildSnapshot({
        availableActions: [
            { kind: ManualActionKind.PAUSE_FEATURE, label: 'Pause', eventType: 'feature.pause', recommendedOrder: 40 },
            { kind: ManualActionKind.FEATURE_EVAL, label: 'Eval', eventType: 'feature.eval', recommendedOrder: 50 },
        ],
    });
    const result = adapter.snapshotToDashboardActions('feature', '42', snap);
    assert.deepStrictEqual(result.validActions, ['pause-feature', 'feature-eval']);
});
...
test('pads feature ID in commands', () => {
    ...
    assert.strictEqual(result.nextAction.command, '/afe 05');
});
...
test('includes agentId in action output', () => {
    ...
    const result = adapter.snapshotToDashboardActions('feature', '42', snap);
    assert.strictEqual(result.nextActions[0].agentId, 'cc');
});
