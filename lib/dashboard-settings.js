...
const { buildRepoStorageStatus, buildRepoStorageActions } = require('./dashboard-storage');
...
const DASHBOARD_SETTINGS_SCHEMA = [
    ...
    },
    {
        ...
        type: 'select',
        scope: 'shared',
        options: agentRegistry.getLaunchableAgentIds(),
    ...
    },
    {
        ...
        // Default off; removed by F682 after production validation. Toggling it
        // only changes how cards render — never workflow state or data.
        key: 'dashboard.contractCards',
        label: 'Contract card renderer (preview)',
        type: 'boolean',
        scope: 'shared',
        description: 'Render pipeline feature, research, and feature-set cards with the contract-driven preview renderer. Off uses the current card builder. Both renderers dispatch the same actions and session tools; switching never changes workflow state.'
    },
    {
...
agentRegistry.getLaunchableAgentIds().forEach(agentId => {
...
function buildDashboardSettingsPayload(repoPath, options = {}) {
    ...
    const settings = DASHBOARD_SETTINGS_SCHEMA.map(def => {
        const defCopy = { ...def };
        if (def.key === 'defaultAgent') {
            const agentAvailability = require('./agent-availability');
            defCopy.options = agentAvailability.getUsableAgentIds(cwd, { forPicker: true });
        }
    ...
    const agentAvailability = require('./agent-availability');
    const defaultAgent = effectiveConfig.defaultAgent || globalDisplayConfig.defaultAgent;
    let defaultAgentWarning = null;
    if (defaultAgent) {
        const avail = agentAvailability.getAgentAvailability(defaultAgent, cwd);
        if (avail.state === 'disabled') {
            defaultAgentWarning = `defaultAgent '${defaultAgent}' is disabled. Run: aigon agent enable ${defaultAgent}`;
        }
    }
    const storage = globalOnly ? null : buildRepoStorageStatus(cwd);
    return {
        repoPath: cwd,
        ...
        settings,
        storage,
        storageActions: storage ? buildRepoStorageActions(storage) : [],
        agentAvailability: agentAvailability.groupAvailabilityReport(cwd, { includeAll: true }),
        defaultAgentWarning,
    };
}
