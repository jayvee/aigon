...
function buildMissingSnapshotState(currentStage, entityType, entityId) {
    ...
    //
    // Actions derivation: even without a snapshot, the stage tells the
    // action registry which lifecycle actions are valid (e.g. inbox →
    // feature-prioritise). Pass the stage to snapshotToDashboardActions so
    // inbox entities (created via `aigon feature-create` — slug-keyed, no
    // snapshot yet) still expose the Prioritise button. Without this, F294's
    // deletion of COMPAT_INBOX silently removed Prioritise for every
    // slug-keyed inbox entry — classic write-path-contract gap where a
    // producer (feature-create) wasn't audited before deleting its read path.
    const actionStage = currentStage === 'inbox' ? currentStage : null;
...
function normalizeAutonomousWorkflow(repoPath, featureId, autoState) {
    ...
    if (isFleet) {
        ...
        if (stopAfter === 'close' && reviewAgent) {
            stages.push({ type: 'counter-review', agents: [implementAgent] });
        }
        if (stopAfter === 'close') stages.push({ type: 'close' });
    }
...
function findAutonomousStageFailure(stageType, autoState) {
    ...
    if (stageType === 'counter-review') return /^feedback-/.test(reason);
