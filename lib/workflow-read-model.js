...
function createCompatibilityState(entityType, entityId, currentStage) {
    // No-ID inbox items still need the Prioritise action surfaced on the card
    // so the user can move them to backlog. The action registry's prioritise
    // guard fires on `currentSpecState === 'inbox'` for both entity types; we
    // synthesise a context with that stage and let snapshotToDashboardActions
    // derive the action (it will produce `feature-prioritise <slug>` or
    // `research-prioritise <slug>` — the CLI accepts a name slug just as well
    // as a numeric ID). Without this, cards with no snapshot render with zero
    // actions and the dashboard offers no way to promote them out of inbox.
    const stage = currentStage || 'inbox';
    const actions = entityType && entityId
        ? workflowSnapshotAdapter.snapshotToDashboardActions(entityType, entityId, null, stage)
...
function getBaseDashboardState(entityType, repoPath, entityId, currentStage, snapshotOverride) {
    ...
    if (isNumericEntityId(entityId)) {
        return {
            ...createLegacyMissingWorkflowState(entityId, currentStage),
            specDrift: null,
        };
    }

    return {
        ...createCompatibilityState(entityType, entityId, currentStage),
        specDrift: null,
    };
}
