...
function buildAutonomousStagePlan(options) {
    ...
    const stages = resolved.stages.map((rawStage, index) => {
        ...
        if (type === 'implement') {
            ...
            if (findAutonomousStageFailure(type, autoState)) status = 'failed';
            else if (reviewApprovedNoRevision || feedbackAddressed || closeRunning || closeComplete) status = 'complete';
...
function getFeatureDashboardState(repoPath, featureId, currentStage, agents, options) {
    ...
    if (snapshot && failoverActionAppenders.length > 0) {
        for (const appender of failoverActionAppenders) {
            try { quotaMerged = appender(repoPath, 'feature', featureId, enrichedSnapshot, agents, quotaMerged); } catch (_) { /* ignore appender errors */ }
        }
    }
    const autonomousMerged = snapshot
        ? appendFeatureAutonomousDashboardActions(repoPath, featureId, autoState, quotaMerged)
        : quotaMerged;
    const quotaNext = snapshot && autonomousMerged.length > 0
        ? { command: autonomousMerged[0].command, reason: autonomousMerged[0].reason }
        : null;
    return appendReadModelFlags({
        ...baseState,
        nextAction: snapshot ? (quotaNext || snapshotActions.nextAction) : baseState.nextAction,
        nextActions: snapshot ? autonomousMerged : baseState.nextActions,
        validActions: snapshot ? autonomousMerged : baseState.validActions,
...
function getResearchDashboardState(repoPath, researchId, currentStage, agents, options) {
    ...
    const quotaMergedResearch = snapshot
        ...
        : baseState.validActions;
    const quotaNextResearch = snapshot && quotaMergedResearch.length > 0
        ? { command: quotaMergedResearch[0].command, reason: quotaMergedResearch[0].reason }
        : null;
    return appendReadModelFlags({
        ...baseState,
        nextAction: snapshot ? (quotaNextResearch || snapshotActions.nextAction) : baseState.nextAction,
        nextActions: snapshot ? quotaMergedResearch : baseState.nextActions,
        validActions: snapshot ? quotaMergedResearch : baseState.validActions,
        evalStatus: evaluation.evalStatus,
        evalSession: evaluation.evalSession,
        reviewStatus: review.reviewStatus,
        reviewSessions: review.reviewSessions,
