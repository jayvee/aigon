...
// Failover action appender (F421). Until F693 this was a runtime registry that
// Aigon Pro filled at startup; the engine is now in-repo, so it is called
// directly alongside its sibling appenders — action policy has exactly one
// source (CLAUDE.md hot rule 5c). Required lazily: lib/agent-failover.js pulls
// in the worktree/status helpers that also reach back here.
const {
...
function deriveReviewStateFromSnapshot(repoPath, entityId, entityPrefix, currentStage, snapshot) {
    ...
    if (snapshot.currentSpecState === LifecycleState.CODE_REVIEW_IN_PROGRESS && codeReview) {
        ...
        const sessionRunning = Boolean(sessionInfo && sessionInfo.running);
        reviewStatus = sessionRunning ? 'running' : 'session-lost';
        reviewSessions = [{
            session: sessionInfo ? sessionInfo.session : null,
            agent,
            running: sessionRunning,
            sessionRunning,
            status: sessionRunning ? 'in-progress' : 'session-lost',
            startedAt: (awaitingNextSession ? null : (codeReview.reviewStartedAt || null)),
            completedAt: null,
            cycle: reviewCycles.length + 1,
            statusCls: sessionRunning
                ? getStateRenderMeta(LifecycleState.CODE_REVIEW_IN_PROGRESS).cls
                : 'status-review-stale',
        }].filter(s => s.agent);
        return { reviewStatus, reviewSessions };
    }
...
function findAutonomousStageFailure(stageType, autoState, snapshot = null) {
    ...
    // The controller can disappear after the implementer has already completed.
    // Session loss is an orchestration interruption, not proof that the current
    // implementation stage failed; stage progress still comes from the snapshot.
    if (reason === 'auto-session-lost') return false;
...
function getFeatureDashboardState(repoPath, featureId, currentStage, agents, options) {
    ...
    if (snapshot) {
        try {
            escalationMerged = appendFailoverDashboardActions(repoPath, 'feature', featureId, enrichedSnapshot, agents, escalationMerged);
        } catch (_) { /* failover actions are additive — never block the read model */ }
    }
...
function appendLiveSpecCycleSessions(rows, repoPath, entityType, entityId, role, defaults) {
    const seen = new Set(rows
        .map(row => row && (row.session || row.sessionName))
        .filter(Boolean));
    const seenAgents = new Set(rows
        .map(row => row && row.agent)
        .filter(Boolean)
        .map(String));
    const live = listSpecReviewTmuxSessions(repoPath, entityType, entityId, role);
    live.forEach((session) => {
        if (!session || !session.session || !session.agent) return;
        const existingAgentRow = rows.find(row => row && String(row.agent || '') === String(session.agent));
        if (existingAgentRow && !(existingAgentRow.session || existingAgentRow.sessionName)) {
            existingAgentRow.session = session.session;
            existingAgentRow.sessionRunning = true;
            return;
        }
        if (seen.has(session.session) || seenAgents.has(String(session.agent))) return;
        rows.push({
        ...
        seenAgents.add(String(session.agent));
    });
    return rows;
}
...
function readSpecReviewSessions(repoPath, entityType, entityId, currentStage, snapshot) {
    ...
    if (!isActiveStage || !entityId) return [];
    if (!snapshot || !snapshot.specReview) {
        return appendLiveSpecCycleSessions([], repoPath, entityType, entityId, 'spec-review', {
            status: 'reviewing',
            source: 'live-session',
        });
    }
    ...
    return appendLiveSpecCycleSessions([...active, ...pending], repoPath, entityType, entityId, 'spec-review', {
        status: 'reviewing',
        source: 'live-session',
    }).sort((left, right) => {
...
function readSpecCheckSessions(repoPath, entityType, entityId, currentStage, snapshot) {
    ...
    if (!isActiveStage || !entityId) return [];
    if (!snapshot || !snapshot.specReview) {
        return appendLiveSpecCycleSessions([], repoPath, entityType, entityId, 'spec-revise', {
    ...
    appendLiveSpecCycleSessions(rows, repoPath, entityType, entityId, 'spec-revise', {
        status: 'addressing-spec-review',
        source: 'live-session',
    });
    appendLiveSpecCycleSessions(rows, repoPath, entityType, entityId, 'spec-check', {
        status: 'addressing-spec-review',
        source: 'live-session',
    });
    return rows.sort((left, right) => String(left.agent || '').localeCompare(String(right.agent || '')));
}
