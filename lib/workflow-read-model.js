...
function getResearchDashboardState(repoPath, researchId, currentStage, agents) {
    const model = getWorkflowReadModel('research', researchId, currentStage, agents);
    ...
    if (snapshot) {
        const snapshotActions = workflowSnapshotAdapter.snapshotToDashboardActions('research', researchId, snapshot);
        return {
            workflowEngine: 'workflow-core',
            workflowSnapshot: snapshot,
            snapshotStatuses,
    ...
    return {
        ...
        workflowSnapshot: null,
        snapshotStatuses,
        nextAction: getDashboardNextCommand('research', researchId, currentStage, agents),
        nextActions: getDashboardNextActions('research', researchId, currentStage, agents),
        validActions: model.validActions,
        workflowEvents: [],
        context: model.context,
        readModel: model,
