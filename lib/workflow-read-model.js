...
function getFeatureDashboardState(repoPath, featureId, currentStage, agents, options) {
    ...
    const quotaNext = snapshot && quotaMerged.length > 0
        ? { command: quotaMerged[0].command, reason: quotaMerged[0].reason }
        : null;
    return appendReadModelFlags({
        ...baseState,
        nextAction: snapshot ? (quotaNext || snapshotActions.nextAction) : baseState.nextAction,
        nextActions: snapshot ? quotaMerged : baseState.nextActions,
        validActions: snapshot ? quotaMerged : baseState.validActions,
