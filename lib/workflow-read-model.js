...
function getResearchDashboardState(repoPath, researchId, currentStage, agents) {
    ...
    // Enrich snapshot with infra data (findings, flags) and review data for unified action derivation
    const enrichedSnapshot = enrichSnapshotWithInfraData(snapshot, agents, {
        reviewSessions: review.reviewSessions,
    });
...
function readFeatureEvalState(repoPath, featureId, currentStage, snapshot) {
    ...
    if (currentStage === 'in-evaluation' && featureId) {
        ...
        evalSession = findFirstTmuxSessionByPrefix(evalPrefix, session => {
            // Feature eval sessions are named {repo}-f{id}-eval-{desc}, not {repo}-f{id}-eval-{agent}.
            // The suffix after eval- is the feature description, not an agent ID.
            // Look up the actual eval agent from the session's tmux command or log files.
...
function readResearchReviewState(repoPath, researchId, currentStage) {
    let reviewStatus = null;
    let reviewSessions = [];
    const isActiveStage = currentStage === 'in-progress' || currentStage === 'in-review' || currentStage === 'in-evaluation';
