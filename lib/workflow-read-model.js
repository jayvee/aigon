...
const AUTONOMOUS_REASON_DEFINITIONS = Object.freeze({
    ...
    'review-quota-paused': { category: 'quota', label: 'Reviewer quota paused', recovery: 'wait-quota' },
...
function buildAutonomousController(autoState, sessionLivenessLookup = null) {
    if (!autoState) return null;
    const reason = autoState.reason || null;
    const reasonMeta = resolveAutonomousReason(reason);
...
function getFeatureDashboardState(repoPath, featureId, currentStage, agents, options) {
    ...
    const autoState = readFeatureAutoState(repoPath, featureId);
    const autonomousController = buildAutonomousController(autoState, (sessionName) => tmuxSessionExists(sessionName));
    ...
    const recoveryMerged = snapshot
        ? appendFeatureReviewRecoveryDashboardActions(repoPath, featureId, autoState, snapshot, autonomousMerged)
        : autonomousMerged;
