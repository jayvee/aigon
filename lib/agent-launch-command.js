...
function buildRawAgentCommand(wt, taskType = 'do') {
    ...
    if (isResearch) {
        ...
        const verbMap = { do: 'do', eval: 'eval', review: 'review', revise: 'revise' };
        ...
        // `revise` replaces the implementer's own session (same session name,
        // same role: 'do') to make another implementation pass over review
        // feedback — so it takes the implement tier, not the reviewer's.
        modelKey = (normalizedTaskType === 'do' || normalizedTaskType === 'revise') ? 'implement'
            : normalizedTaskType === 'eval' ? 'evaluate'
            : 'review';
    }

    if (wt.continuityInstructions) {
        const instructions = String(wt.continuityInstructions);
        const fileMatch = String(prompt).match(/^\$\(<\s+(.+?)\s*\)$/);
        if (fileMatch) {
            const promptPath = fileMatch[1].replace(/^'(.+)'$/, '$1').replace(/^"(.+)"$/, '$1');
    ...
    const launchEnvExports = launchFragments.envExports.length > 0
        ...
        : '';
    const resumeConfig = wt.resumeProviderSessionId ? agentRegistry.getResumeConfig(wt.agent) : null;
    const resumeArgs = resumeConfig && resumeConfig.kind === 'append'
        ? [resumeConfig.flag, wt.resumeProviderSessionId]
        : [];
    const flags = [...flagTokens, ...launchFragments.args, nameFlag, ...resumeArgs].filter(Boolean).join(' ');
    ...
    const stdinRedirect = '';
    if (resumeConfig && resumeConfig.kind === 'subcommand') {
        const subcommandArgs = [
            ...(resumeConfig.args || []),
            ...(resumeConfig.workingDirectoryFlag && absPath ? [resumeConfig.workingDirectoryFlag, absPath] : []),
            wt.resumeProviderSessionId,
        ]
            .map(shellQuote)
            .join(' ');
        return `${cdPrefix}${launchEnvExports}${prefix}${cliConfig.command} ${subcommandArgs} ${quotedPrompt}${stdinRedirect}`;
    }
...
function buildAgentWrapperEnvironmentLines() {
    ...
    if (process.env.PATH) {
        lines.push(`export PATH=${shellQuote(process.env.PATH)}`);
    }
    return lines;
}
