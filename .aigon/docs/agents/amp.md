...
## Agent Identity
- **Agent ID**: `am`
- **Worktree Pattern**: `../feature-NN-am-description`
...
- Amp is launched as an interactive TUI. Aigon intentionally does not use `amp -x` / `--execute` for normal workflow sessions because execute mode exits after one turn.
- Aigon's model picker maps to Amp modes and launches with `--mode <rush|smart|deep>`, not `--model`.
- Amp transcript and usage telemetry are not captured by Aigon v1.
<!-- AIGON_END -->
