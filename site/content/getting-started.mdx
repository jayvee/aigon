...
| Mistral Vibe | `vibe` | `pip install mistral-vibe` (requires `MISTRAL_API_KEY`) |
...
**Warp** (recommended — supports side-by-side split panes):
```bash
aigon config set --global terminal warp
```

**iTerm2** (agents run in tmux sessions opened in iTerm2):
```bash
...
**VS Code / Cursor** (opens the worktree folder — run the agent command manually):
```bash
aigon config set --global terminal code       # or: cursor
```
...
While the agent works, start the dashboard and register the repo:
...
4. `aigon dashboard` — web UI at localhost:4100 showing pipeline, agent status, sessions. The **Reports tab** tracks token usage, cost, and throughput across agents — check it after your first feature to see telemetry in action
