...
After Fleet agents signal `implementation-complete`, `feature-eval` generates a structured comparison so you can score and select the best one.
...
**Tip:** Use a different model for evaluation than those that implemented. For example, if Claude and Antigravity implemented, evaluate with Codex.
...
aigon feature-close 108 cx --adopt ag   # Review diffs from specific agents
```
