...
- Slash command: `/afs 07 cc ag cx`
- CLI: `aigon feature-start 07 cc ag cx`
...
- CLI (with review): `aigon feature-autonomous-start 07 cc --review-agent=ag`

```
implement → [review] → close
```

{/* SCREENSHOT: AutoConductor session for solo mode — poll log showing implement → review → close */}
...
- Workspace: one worktree per agent
- CLI: `aigon feature-autonomous-start 07 cc ag`
- CLI (explicit evaluator): `aigon feature-autonomous-start 07 cc ag --eval-agent=cc`

```
implement (parallel) → evaluate → close
```

{/* SCREENSHOT: Dashboard showing Swarm run — multiple complete agents, evaluator running, auto badge visible */}
...
| `implementation-complete` | Agent committed code and its implementation log |
