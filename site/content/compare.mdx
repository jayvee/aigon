...
**OSS vs Pro split:** The quota poller, exhaustion event vocabulary, and dashboard quota display are OSS. Automatic failover (`policy: switch`) and the manual "Failover now" dashboard button are [Aigon Pro](/docs/getting-started#aigon-pro-beta-access) features. Without Pro, exhaustion events are still recorded in `events.jsonl` and the supervisor logs a warning, but no agent switch is triggered. See the [Agent Failover guide](/docs/guides/agent-failover) for configuration details.
