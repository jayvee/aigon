...
> **Most coding tools give you one model's opinion and stop when its quota runs out. Aigon coordinates Claude, Codex, Antigravity (and more...) on the same spec — multiple coding agents implement, a different coding agent reviews, and work continues on whichever subscription still has quota.**
...
> **A note on routing agents.** Some agents Aigon supports are *routers* rather than vendors — GitHub Copilot CLI (`cp`) is one GitHub Copilot entitlement that can route work to GPT, Claude, Gemini, or MAI models. That gives you model choice inside one subscription; it does **not** give you several independent quota pools, and two `cp` sessions are not provider-independent from each other. For genuine quota arbitrage, pair `cp` with an agent on a different subscription.
...
**Economics.** If you already pay for Claude, Antigravity, and Codex access, a single favourite tool only spends one quota. Aigon can help you choose another available agent when a running session reports exhaustion; automatic failover requires the relevant configured capability.
...
**Pick Aigon** if you want to watch the work happen (`tmux attach`, dashboard); your existing Claude / Antigravity / Codex subscriptions should be the budget; you want the option to escalate to autonomous mode without giving up the supervised default.
...
The last column isn't just a posture claim — it's a feature claim. Aigon surfaces quota only where a supported source exists. Antigravity is intentionally excluded from automatic polling because its CLI uses interactive OAuth; exhaustion is detected from a running session. Failover behavior depends on the configured capability rather than a universal provider guarantee.

The quota poller, the exhaustion event vocabulary, the dashboard quota display, automatic failover (`policy: switch`), and the manual "Failover now" dashboard button all ship in Aigon. Exhaustion events are recorded in `events.jsonl` regardless of the policy you configure. See the [Agent Failover guide](/docs/guides/agent-failover) for configuration details.
