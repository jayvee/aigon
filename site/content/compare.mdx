...
The last column isn't just a posture claim — it's a feature claim. Aigon ships a quota poller that checks Claude Code, Codex, Gemini CLI, Kimi, and Amp every 30 minutes, surfaces headroom on the dashboard and at feature-start, and auto-fails over to a vendor with quota when one runs out.
