...
Feature specs can declare dependencies using `depends_on` in the `## Dependencies` section:
...
When you run `aigon feature-prioritise`, dependencies are resolved (names/slugs canonicalized to IDs) and checked for circular references via DFS. Dependent features cannot be started until their dependencies are in `05-done`.
