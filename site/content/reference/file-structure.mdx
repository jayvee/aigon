...
Aigon stores workflow state through SpecStore. Human-readable specs live in your repository under `docs/specs/`; the authoritative feature/research lifecycle is the workflow-core event log. By default that event log is local under `.aigon/workflows/`; with git-ref storage enabled, canonical events live under `refs/aigon/specs/<key>/events` and `.aigon/workflows/` becomes a local projection cache.
...
- **Traceable History**: workflow events, snapshots, telemetry, specs, and logs stay inspectable from the repo checkout; git-ref storage can also sync canonical workflow/lease/stats events through custom Git refs.
...
| What lifecycle state is a feature or research topic in? | SpecStore events: local `.aigon/workflows/<type>/<id>/events.jsonl` or git-ref `refs/aigon/specs/<key>/events` | Workflow-core is authoritative; folders mirror this state for users. |
| Which clone has an active git-ref lease? | `lease.*` events in the SpecStore event stream | Dashboard cards/details, `aigon storage report`, and `aigon board --storage` surface active non-expired leases. |
...
Local runtime state lives in `.aigon/` (gitignored). Spec folders in `docs/specs/` are the committed, human-readable view; `.aigon/` is the local engine projection underneath. When git-ref storage is enabled, canonical workflow, lease, and stats events sync through Git refs while spec markdown and code continue to use normal Git.
...
| `.aigon/workflows/features/{id}/stats.json` | Local stats projection rebuilt from canonical stats events where available |
| `.aigon/telemetry/` | Normalised per-session telemetry records (agent, model, tokens, cost, duration) |
| `.aigon/cache/` | Cached data (e.g. `commits.json` for commit analytics and `stats-aggregate.json` for aggregate stats) |
