...
```json
{
  ...
  "agents": {
    ...
    "ag": { "cli": "agy", "implementFlag": "--dangerously-skip-permissions" },
...
| `agents.{id}.cli` | Global | CLI command for each agent | `claude`, `agy`, `codex`, `vibe` |
...
aigon config set agents.ag.models.evaluate gemini-3.5-flash-medium
...
| `ag` (Antigravity) | `--dangerously-skip-permissions` | Auto-approves all |
| `cx` (Codex) | _(empty)_ | Interactive by default |
| `cp` (GitHub Copilot) | `--allow-all --interactive` | Auto-approves tool use in the interactive TUI |

The full per-agent table, including `op`, `km`, and `am`, is in the [Agents reference](/docs/reference/agents#permission-modes).
