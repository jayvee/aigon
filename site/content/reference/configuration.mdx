...
| `storage.backend` | Project | SpecStore backend for workflow events | `local` (default), `git-ref` |
| `storage.git.remote` | Project | Remote used by git-ref storage | Remote name such as `origin` |
| `storage.git.refPrefix` | Project | Git ref namespace for canonical SpecStore events | Default: `refs/aigon/specs` |
| `storage.git.offline` | Project | Skip pre-write fetch/merge for git-ref storage | `false` (default), `true` |
...
Git-ref storage is opt-in. Prefer the conversion command because it validates remote push access and imports existing local workflow events:
...
Git-ref storage syncs canonical workflow, lease, and stats events through `refs/aigon/specs/<key>/events`. Spec markdown and code changes still use normal Git. See [Storage Backends](/docs/reference/storage-backends) for two-machine behavior, leases, offline mode, and diagnostics.
