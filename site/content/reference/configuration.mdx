...
**Spec complexity:** Feature and research specs can declare a **`complexity`** level (and optional per-agent `recommended_models`) in YAML frontmatter. When you start work from the dashboard, those values **pre-select** model and effort before your config defaults are used. Global keys above are still the baseline when the spec leaves a slot empty or you pick **Default** in the picker. See [Agents → Spec complexity and model defaults](/docs/reference/agents#spec-complexity-and-model-defaults).
