...
Scheduled jobs live in `.aigon/state/scheduled-kickoffs.json` (lock-protected, store version `1`). The store is part of the portable state covered by [`aigon sync`](/docs/reference/commands/infra/sync), so a job scheduled on one laptop fires from whichever machine has its server running at `runAt`.
