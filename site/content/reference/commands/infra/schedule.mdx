...
<Callout type="info">
  **Aigon Pro command.** The schedule engine lives in `@aigon/pro`. In open-source Aigon, `aigon schedule` is a thin stub that prints a Pro notice and exits non-zero unless Pro is installed.
</Callout>
