---
title: "schedule"
description: "Schedule a feature autonomous run, feature-set autonomous run, or research start for a specific time. Polled by the aigon server."
---
...
<Callout type="info">
  **Aigon Pro command.** The schedule engine lives in `@senlabsai/aigon-pro`. In open-source Aigon, `aigon schedule` is a thin stub that prints a Pro notice and exits non-zero unless Pro is installed.
</Callout>
...
Queue a [`set-autonomous-start`](/docs/guides/feature-sets-autonomous) for a future time. At run time the scheduler dispatches the existing set conductor — ordering, failure pauses, resume, and reset semantics stay in SetConductor.

```bash
aigon schedule add set_autonomous nightly \
  --run-at=2026-06-19T09:00:00Z cx gg \
  --review-agent=cc --stop-after=close
```
...
- [`set-autonomous-start`](/docs/guides/feature-sets-autonomous) — what a `set_autonomous` job actually runs.
