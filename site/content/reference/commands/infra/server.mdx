...
- **Default binding**: `127.0.0.1` (loopback), so only the local machine can reach the dashboard.
- **Non-loopback access**: configure an explicit host allow-list and shared secret/token before exposing the server beyond loopback.
