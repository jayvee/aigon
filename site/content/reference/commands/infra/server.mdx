...
| `aigon update` didn't restart server | The persistent service auto-recovers — launchd detects the process died and relaunches it. `aigon update` now also polls the server after restart and prints `✅ Server restarted and responding on port <N>` or `⚠️` if it didn't respond within 5s |
