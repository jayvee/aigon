...
```bash
aigon security-scan [--since <sha>] [--no-feature] [--no-feedback]
```
...
4. For each **HIGH**-severity finding that hasn't been seen before, creates and prioritises an Aigon feature automatically
5. Prints a summary of findings, skipped duplicates, and feature specs created
...
aigon security-scan --since abc1234 --no-feature
```
