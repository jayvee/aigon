...
| `revision` | Follows `review`. The implementer addresses feedback. | Exactly 1 |
...
```json
{
  ...
  "description": "CC implements, CX reviews, CC revises, auto-close",
  "stages": [
...
- At most one `review`, `revision`, and `eval` stage each
...
| `--revision=<agent>` | Agent to address review feedback (defaults to first implementer) |
...
aigon workflow create my-arena --implement=cc,ag --eval=cu --label="CU judges"
...
- [Autonomous Mode guide](/docs/guides/autonomous-mode) — when to use autonomous mode and how it orchestrates
