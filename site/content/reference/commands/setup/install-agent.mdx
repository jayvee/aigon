...
aigon install-agent cc ag cx cu op km am cp --force
```

Agent IDs come from `templates/agents/*.json` in the Aigon package. Current bundled IDs are `cc`, `ag`, `cx`, `cu`, `op`, `km`, `am`, and `cp`.
