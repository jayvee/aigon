...
`feature-eval` is the Fleet mode judging step — it compares all agent implementations against the spec, picks a winner, and records that decision so `feature-close` knows which branch to merge. Run it from the main repo after all agents have signalled `implementation-complete`.
...
1. Reads all completed implementations from the feature's workspaces
...
aigon feature-eval 42 --agent=ag
...
aigon feature-close 42 cc --adopt ag
```
