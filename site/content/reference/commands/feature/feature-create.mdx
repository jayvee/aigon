...
- **YAML frontmatter** — includes **`complexity`** (`low` / `medium` / `high` / `very-high`) only. Model/effort defaults at start come from each agent’s complexity ladder + `aigon config`, not from the spec; see [Agents: Spec complexity and model defaults](/docs/reference/agents#spec-complexity-and-model-defaults).
...
```bash
aigon feature-prioritise dark-mode    # Moves the allocated F<ID> feature to backlog
