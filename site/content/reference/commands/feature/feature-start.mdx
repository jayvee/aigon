...
If you omit `--models` / `--efforts`, the CLI uses the same **spec `complexity` → per-agent complexity ladder → `aigon config models`** resolution as the dashboard. See [Agents: Spec complexity and model defaults](/docs/reference/agents#spec-complexity-and-model-defaults).
