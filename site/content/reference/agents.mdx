...
| `mv` | Mistral Vibe | _(none — headless only)_ | `vibe` |
| `cu` | Cursor _(retired)_ | `/aigon-` | `agent` |

> **Mistral Vibe** runs headless via the `-p` flag — no slash commands or interactive UI. It uses the Devstral 2 model and requires `MISTRAL_API_KEY` to be set. Install with `pip install mistral-vibe`.

## Installing agents

```bash
aigon install-agent cc gg          # Install Claude + Gemini
aigon install-agent cx mv          # Add Codex + Mistral Vibe later
```
...
| **mv** | _(none — headless)_ | _(none)_ | `docs/agents/mistral-vibe.md` |
