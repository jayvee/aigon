...
Slash-invocable agents (`cc`, `cu`) receive Aigon prompts through their native slash-command discovery. File-prompt agents (`ag`, `cx`, `op`, `km`, `am`) launch with the canonical prompt body inlined directly — no slash-command registration is required on those CLIs.
