...
Slash-invocable agents (`cc`, `gg`, `cu`) receive aigon prompts through their native slash-command discovery. Non-invocable agents (`cx`, `op`, `km`, `am`) launch with the canonical prompt body inlined directly — no slash-command registration is required on those CLIs.
...
**When new templates are available**: SessionStart hooks (cc, gg, cu, codex) print a short notice telling you to run `aigon apply`. You decide when. See [Applying Aigon updates to your repos](/docs/guides/applying-aigon-updates).
