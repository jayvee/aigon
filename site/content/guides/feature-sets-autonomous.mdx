...
Two live session types can appear on a set card:

- **Spec review** — launched by `feature-set-spec-review` (one reviewer session anchored on a set member, tagged in the session sidecar as `metadata.setSpecReview.setSlug`)
- **Conductor** — launched by `set-autonomous-start` (detached tmux session `<repo>-s<slug>-auto`)

The dashboard view is read-only. The actual set behavior comes from the workflow engine, session sidecars, and the set conductor — not from frontend naming guesses.
...
- Historical design note in the repo: `docs/specs/research-topics/05-done/research-34-feature-set.md`
