...
**Automated recurring templates** — including a `security-scan-weekly` style job that runs `aigon security-scan` on a cadence — are driven by the [recurring-features](/docs/guides/recurring-features) engine. You can still run `aigon security-scan` on demand whenever you want a full-repo pass.

`aigon recurring-list` shows whether a weekly scan is due; see the [recurring-features guide](/docs/guides/recurring-features) for how cadence batches work.
