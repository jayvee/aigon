...
{/* PLACEHOLDER: Terminal output of aigon security-scan — showing scanner results summary, deduped findings skipped, and follow-up features created for HIGH severity issues */}
...
This runs all configured scanners plus an LLM analysis via the `/security-review` skill, deduplicates findings using fingerprints from previous scans, and auto-creates follow-up features for any new HIGH-severity findings. See the [`security-scan` reference](/reference/commands/infra/security-scan) for full options.
