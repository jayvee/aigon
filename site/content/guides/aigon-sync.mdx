---
title: Aigon Sync
description: Push portable Aigon state to a private Git vault — multi-machine sync via @senlabsai/aigon-pro (dashboard Settings → Aigon Sync).
---

import { Callout } from "nextra/components";

<Callout type="info">
  **Aigon Pro feature.** The vault engine (`aigon backup`, `aigon vault`), dashboard **Settings → Aigon Sync**, and deprecated `aigon sync` aliases ship in **`@senlabsai/aigon-pro`**. Open-source Aigon prints a gate notice for those commands when Pro is not installed. Ordinary backups — `git`, archives, clones, Time Machine — **do not** require Pro. See [aigon.build/pro](/pro).
</Callout>
...
- **`settings/`** — sanitized global config plus workflow definitions paths as implemented by `@senlabsai/aigon-pro`

**Pull** restores compatible settings and project `.aigon/` slices onto machines where those repos exist.

## CLI (requires `@senlabsai/aigon-pro`)
...
If the panel shows an install message, ensure `@senlabsai/aigon-pro` is linked or installed and **`aigon server restart`** so the dashboard loads `dashboard/backup-sync.js` from Pro.
...
- **[Getting started](/docs/getting-started)** — linking `@senlabsai/aigon-pro` for beta builds
