...
Files under `.aigon/` that are tracked by the project repository remain under
normal Git authority: push omits them, restore planning ignores them, and pull
preserves the currently checked-out versions.

Pushing from a machine that has only some Vault projects refreshes those local
project snapshots and preserves snapshots for projects that are not cloned on
that machine. Removing a repo locally therefore cannot silently erase its only
Vault recovery copy.
