...
## Disabling dashboard GitHub status

If you don't use GitHub PRs and don't want the PR status section appearing on dashboard cards, set `github.prCheck` to `false` in your project config:

```bash
aigon config set github.prCheck false
```

Or add it directly to `.aigon/config.json`:
...
This disables GitHub PR checking entirely: the PR status section is hidden from dashboard feature cards, and `feature-close` skips the open-PR gate.
