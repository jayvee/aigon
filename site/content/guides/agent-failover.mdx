...
Manual / Drive / Fleet runs **never** auto-swap — the operator is at the wheel and clicks the "Failover now →" button when they're ready. With those conditions met, the exhaustion handler:
...
|-------|------|-------------|
| `policy` | `"notify"` \| `"switch"` \| `"pause"` | `notify` logs and notifies but does not switch. `switch` triggers auto-failover. `pause` marks the agent slot paused for manual action. |
...
1. Feature is started in autonomous mode (e.g. via `aigon feature-autonomous-start`) on `cc` with chain `[cc, cx, ag]` and `policy: switch`.
...
5. Because the feature is autonomous and `policy: switch`, the exhaustion handler fires. It selects `cx` (next in chain after `cc`).
...
If `cx` later exhausts, the process repeats with `ag`. When `ag` exhausts with no next agent, `agent.token_exhausted` is recorded but no switch fires.
