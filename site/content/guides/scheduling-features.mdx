...
<Callout type="info">
  **Aigon Pro feature.** One-shot scheduled kickoffs via [`aigon schedule`](/docs/reference/commands/infra/schedule) — including `feature_autonomous` and `research_start` jobs polled by the dashboard server — ship in **Aigon Pro**. In open-source Aigon the CLI prints a gate message and exits non-zero; install `@senlabsai/aigon-pro` when available for your organization. See [aigon.build/pro](/pro) for launch updates.
</Callout>
...
- **[Autonomous mode](/docs/guides/autonomous-mode)** — what fires when a `feature_autonomous` job runs.
