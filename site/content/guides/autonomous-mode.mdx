...
As the AutoConductor moves a feature through implement → review → revision → close, the dashboard card reflects the current **lifecycle state** with a coloured chip in the card header. The state is canonical — it lives on the workflow snapshot, not in frontend heuristics — and the badge maps one-to-one from the [`STATE_RENDER_META`](https://github.com/jpv-aigon/aigon/blob/main/lib/state-render-meta.js) table the server attaches to every dashboard row.
