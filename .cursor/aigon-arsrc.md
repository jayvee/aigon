...
```bash
SPEC_PATH=$(find docs/specs/research-topics -maxdepth 2 \( -name "research-<name>-*.md" -o -name "research-<name>.md" \) | head -1)
...
```bash
git add "$SPEC_PATH"
git commit --allow-empty -m "spec-review-check: research <name> — <decision summary>" -m "reviewed: <comma-separated reviewer ids>
...
Notes:
- <important rationale>"
aigon research-spec-review-check-record <name>
```
