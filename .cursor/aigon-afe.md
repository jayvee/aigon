...
**Do NOT run `feature-close` automatically.** Once the user approves, tell them to run `/aigon-feature-close <name>`.
...
Then **ask**: "Which implementation would you like to merge?" and **WAIT**. Do NOT run `feature-close` automatically. Once chosen, tell them to run (from the main repo, not a worktree): `/aigon-feature-close <name> <winning-agent>`.
