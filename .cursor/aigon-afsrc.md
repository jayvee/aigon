...
```bash
SPEC_PATH=$(aigon feature-spec <name> 2>/dev/null || true)
if [ -z "$SPEC_PATH" ]; then
  SPEC_PATH=$(find docs/specs/features -maxdepth 2 \( -name "feature-<name>-*.md" -o -name "feature-<name>.md" \) | head -1)
fi
...
```bash
git add "$SPEC_PATH"
git commit --allow-empty -m "spec-review-check: feature <name> — <decision summary>" -m "reviewed: <comma-separated reviewer ids>
...
Notes:
- <important rationale>"
aigon feature-spec-review-check-record <name>
```
