...
**After setup completes:** Ask the user if they want you to switch to the worktree and start implementing. If yes, `cd` to the worktree directory and run `/aigon-feature-implement` from there. Remember that `/aigon-feature-done` must be run from the main repo later.
