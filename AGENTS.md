...
- **Agent launch env vars**: `launchPromptCommand()` in `lib/commands/entity-commands.js` injects `AIGON_ENTITY_TYPE` (`feature`|`research`), `AIGON_ENTITY_ID`, `AIGON_AGENT_ID`, and `AIGON_PROJECT_PATH` into every agent spawn. These allow agent-side commands (e.g. `aigon agent-context`) to identify the current session without parsing branch names.
