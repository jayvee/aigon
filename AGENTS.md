...
| `lib/dashboard-status-collector.js` | ~2300 | Read-side collector: repo/feature/research/feedback/summary status, log/detail reads, plus derived set-card payloads (progress/current feature/dep graph/validActions) |
...
| `lib/budget-poller.js` | ~450 | F322 budget scrape primitives (tmux/app-server parsers + per-agent poll helpers). **F616:** dashboard/server no longer starts `startBudgetPoller`; writes route through `.aigon/state/agent-quota.json` via `lib/agent-quota-read.js`. Kept for scrape logic reused by `lib/agent-quota-poller.js`. |
| `lib/agent-quota-read.js` / `lib/agent-quota-poller.js` | ~250/~350 | **F616 unified agent-quota state:** single cache `.aigon/state/agent-quota.json` (`schemaVersion: 1`), one background poller, `GET/POST /api/agent-quota`. Merges F322 budget windows, F444 probe verdicts, and F615 provider wallets. `aigon doctor --fix` migrates legacy `budget-cache.json` + `quota.json`. Deprecated shims: `/api/budget`, `/api/quota`. |
| `lib/quota-probe.js` / `lib/quota-poller.js` | ~340/~15 | F444 probe classification + `scripts/probe-agent.js` integration. **F616:** `quota-poller.js` is a thin re-export of `agent-quota-poller.js`; reads/writes delegate to `agent-quota-read.js`. |
...
6. **Update docs when you change architecture** — new modules/patterns/repo structure → update `AGENTS.md` (and `docs/architecture.md`) in the same PR. Import-edge changes must keep `scripts/module-graph-baseline.json` accurate — `scripts/check-module-graph.js` runs in `test:core` and ratchets cycles/boundary violations.
