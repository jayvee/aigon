...
- **Criteria attestation (F647)**: before close, every `## Acceptance Criteria` line must have a matching indexed line in the implementation log `## Criteria Attestation` (`met` / `deferred` / `dropped`). `deferred` raises `spec-shortfall` escalations via `source: 'criteria-attestation'`. Parser/guard: `lib/criteria-attestation.js`; escape hatch `--no-verify-criteria`.
