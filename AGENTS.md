...
| `lib/entity-ui-contract.js` / `lib/feature-ui-contract.js` / `lib/research-ui-contract.js` / `lib/feature-set-ui-contract.js` / `lib/workflow-core/runtime-facts.js` | ~600 | **Versioned entity interaction read contracts (F675/F677):** shared validated envelope plus entity-specific projectors for feature, research, and feature sets. Projects canonical definitions and immutable runtime facts into state, presentation, decisions, tools, blockers, drag targets, agents, inspectable sessions/Peek, autonomous plans, and nested current-set-member contracts. **F678 made these the production read contract**: the collector emits a validated `uiContract` on every interactive feature, research, and set card (done feature rows stay lean per F459/F469/F590). Identity (kind/numericId/slug/set), session inspection targets (`live-pane` vs `console-snapshot`), stage-owned sessions, internal-signal quarantine, set `specCycle`, and nested current-member contracts are all server-owned — browser code must not re-derive them. Contract changes that must repaint a card belong in `entityUiContractFingerprint`. Full contract: `docs/feature-interaction-contract.md` |
