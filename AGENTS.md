...
- **Criteria attestation (F647, advisory)**: optional `## Criteria Attestation` lines in the implementation log (`met` / `deferred` / `dropped`). Close no longer blocks on missing attestation; when complete, `feature-close` records `feature.criteria_attested`. `deferred` still raises `spec-shortfall` escalations when attestation is submitted. Parser: `lib/criteria-attestation.js`.
