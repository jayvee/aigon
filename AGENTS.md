...
| `lib/workflow-definitions.js` | ~260 | Saved workflow definition storage + validation: legacy flat v1 plus `version: 2` stage-based pipelines, built-ins, project/global precedence, CRUD helpers, and launch-time override merging for `workflow`, `feature-start --workflow`, and `feature-autonomous-start --workflow`. |
