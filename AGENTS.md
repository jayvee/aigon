...
| `lib/agent-sessions/` | ~700 | AgentSession domain model, sidecar-compatible store, event helpers, and injectable service contract for long-lived interactive agent runtime records. Normalizes `.aigon/sessions/{sessionName}.json` without importing worktree, workflow-core, dashboard, or command modules |
