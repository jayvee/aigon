...
- **Cross-machine sync (F388 aigon-vault)**: `aigon backup configure|push|pull|status|schedule` — one private git repo (`projects/<name>/` + `settings/`) backs up every registered project's `.aigon/` plus global settings. Daily auto-push runs from the dashboard server. Engine: `lib/backup.js`. Legacy `aigon sync` / `aigon settings` stay as deprecated aliases that delegate here.
...
| `lib/sync.js` | ~900 | Legacy cross-machine state backup/restore (`aigon sync legacy …`); superseded by F388 vault |
| `lib/sync-state.js` | ~430 | F359 per-project git-backed `.aigon/` sync. Reachable via `aigon sync legacy push/pull/configure`; superseded by `lib/backup.js` for the primary `aigon backup` flow |
| `lib/backup.js` | ~470 | F388 unified vault: one private repo with `projects/<name>/` (filtered `.aigon/`) and `settings/` (stripped `~/.aigon/config.json` + workflow-definitions). Helper repo at `~/.aigon/.vault/repo`, branch `main`. CLI `aigon backup configure|push|pull|status|schedule`. Daily auto-push registered in `lib/dashboard-server.js`; failure emits dashboard `backup-failed` notification |
| `lib/scheduled-kickoff.js` | ~470 | F367/F379 server-side poller for deferred kickoffs. Persists jobs in `.aigon/state/scheduled-kickoffs.json` (lock-protected, store version 1), polls every 45s, fires `feature_autonomous` / `research_start` / `agent_prompt` at `runAt` (catch-up if server was down). `agent_prompt` jobs may carry a 5-field `cron` expression; on successful fire the poller re-enqueues the job at the next cron-derived runAt. Job CRUD lives in `lib/commands/schedule.js` |
| `lib/commands/schedule.js` | ~175 | CLI dispatcher for `aigon schedule add|list|cancel`. Validates payloads through `feature-autonomous-payload` and `validateAgentPromptPayload`, persists via `scheduled-kickoff` |
| `lib/commands/agent-launch.js` | ~120 | F379 internal CLI primitive: `aigon agent-launch --agent <id> --prompt <string> [--repo <path>] [--label <slug>]` spawns a fresh agent session in a detached tmux tab carrying `--prompt` as the first user input. Shelled-into by `scheduled-kickoff` when an `agent_prompt` job fires; not user-facing |
| `lib/cron-parse.js` | ~110 | F379 minimal Vixie-style 5-field cron parser used by the scheduler for `agent_prompt` re-arming. Supports `*`, `N`, `N-M`, `*/S`, `N-M/S`, comma lists; honours the DOM/DOW OR-rule. No external dependency |
