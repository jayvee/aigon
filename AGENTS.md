...
| `lib/commands/setup.js` | ~4515 | init, install-agent, check-version, update, doctor + state reconciliation. Composed from extracted helpers in `lib/commands/setup/`: `seed-reset.js` (seed-repo full reset), `worktree-cleanup.js` (orphan worktree GC), `gitignore-and-hooks.js` (.gitignore + git hooks scaffolding), `pid-utils.js` (server PID file helpers), `agent-trust.js` (per-agent trust install scope routing) |
| `lib/dashboard-server.js` | ~2590 | HTTP/UI module: dashboard, API, WebSocket relay, HTTP action dispatch. Never mutates engine state directly and never reads engine-state/spec/log files directly |
...
| `lib/quota-probe.js` / `lib/quota-poller.js` | ~340/~95 | F444 agent quota availability: wraps `scripts/probe-agent.js`, classifies CLI stdout/stderr with `templates/agents/<id>.json:quota` regex packs, caches per-(agent, model) verdicts in `.aigon/state/quota.json`, gates `feature-start`, filters `perf-bench --all`, and lets the dashboard/server refresh quota state without mutating workflow-core snapshots |
...
12. **`cli.modelOptions` is a closed registry — human approval is the only door in.** Adding a model to `templates/agents/<id>.json` requires running `aigon model-refresh` interactively. Non-interactive discovery (perf-bench, scheduled jobs, CI) routes candidates to `.aigon/pending-models.json`; a human runs `aigon model-refresh --approve-pending` to drain the queue. The modality / domain filter (`isIrrelevantForCoding`) is enforced at discovery in `lib/commands/bench.js` — TTS, robotics, computer-use, image-gen, and `-latest` aliases never reach the prompt. Full contract: **`docs/model-inclusion-policy.md`**. Background: 2026-05-22 incident where `aigon perf-bench` silently dumped 19 non-coding Gemini variants into `gg.json` via an autoAddModels path that has since been removed.
