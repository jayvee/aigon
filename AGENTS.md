...
| `lib/spec-store/` | ~400 | **Durable spec storage boundary (F573).** `createSpecStore({ repoPath })` exposes spec-shaped I/O plus lease coordination (`acquireLease`, `renewLease`, `releaseLease`, `readLeases`), pre-write sync, `aigon storage doctor|report`. Local backend thin-wraps workflow-core helpers; git-ref backend (F577) stores canonical events in Git refs with local projection cache; git-branch backend (F609) stores them as a file tree on an orphan branch (`aigon-state`: `meta.json` + `specs/<KEY>/events.jsonl`) via `git-plumbing.js` tree helpers, never checking the branch out. Design: `docs/specstore-architecture.md` |
