...
| `lib/remote-gate-github.js` | ~170 | **Remote branch gate (GitHub v1)**: `checkGitHubGate()` queries `gh pr list` for the feature branch, applies the v1 gate policy (open, non-draft, mergeable), returns a provider-neutral result shape. Used by `feature-close` when `honourRemoteBranchGate` is enabled. Fail-closed on missing/unauthed `gh`. |
