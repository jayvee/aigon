...
11. **Never hand-edit `cli.modelOptions` in `templates/agents/*.json`.** All additions go through `aigon model-refresh` (interactive) or `aigon model-refresh --approve-pending` (drains `.aigon/pending-models.json`). Scheduled / non-interactive discovery writes to the pending queue, never to the agent JSON. Full contract — modality filters, economic gates, approval flow — lives in **`docs/model-inclusion-policy.md`**. The 2026-05-22 incident (TTS/robotics/computer-use/`-latest` aliases polluting the gg dropdown) was an autoAddModels bypass; that code path is gone.
