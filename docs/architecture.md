...
| Feature close failure details | `lib/workflow-core/` event log + snapshot `lastCloseFailure` projection | `feature_close.failed` on merge/push failures; `feature.close_gate_failed` when the post-merge gate fails (F644). Projector sets `lastCloseFailure` (`kind`, `conflictFiles` or `gateCommand`/`logPath`, `stderrTail`, `at`) on snapshot; cleared on `feature.closed`. Dashboard swaps "Close" for "Resolve & close" when `kind === 'merge-conflict'`. Post-merge gate failures use `kind === 'post-merge-gate'` and enter `close_recovery_in_progress`. |
