...
| agent transcript binding | `.aigon/sessions/{name}.json` fields `agentSessionId` + `agentSessionPath` | F357: populated post-launch by background capture process (`lib/session-sidecar.js`). `agentSessionId` is the Claude UUID / Codex stem / Antigravity sessionId; `agentSessionPath` is the full transcript path. Read as optional — absent on pre-F357 sidecars. Used by `aigon feature-do --resume` to resume a dead tmux session deterministically. |
