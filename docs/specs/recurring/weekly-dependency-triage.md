...
# dependency-triage-{{YYYY-WW}}

## Summary

Run the weekly dependency and package-supply-chain triage for the Aigon repo. Capture the current dependency-security signals, write a concise report to `.aigon/reports/dependency-triage-{{YYYY-WW}}.md`, and create + prioritise follow-up features only when the findings require code or dependency changes.
...
- [ ] Write `.aigon/reports/dependency-triage-{{YYYY-WW}}.md` with a concise summary of actionable findings and raw command outcomes
...
4. Write `.aigon/reports/dependency-triage-{{YYYY-WW}}.md` with:
   - headline summary
   - actionable findings
