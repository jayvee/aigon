---
schedule: weekly
name_pattern: agent-matrix-benchmark-{{YYYY-WW}}
recurring_slug: weekly-agent-matrix-benchmark
complexity: low
---

# agent-matrix-benchmark-{{YYYY-WW}}
...
- [ ] Write a benchmark report to `.aigon/reports/agent-matrix-benchmark-{{YYYY-WW}}.md` with sections: Agent/Model, Task, Outcome (pass/fail), TypeScript clean (yes/no), Approach summary (2–3 sentences), Notable issues
...
4. Write the benchmark report to `.aigon/reports/agent-matrix-benchmark-{{YYYY-WW}}.md` (in the aigon repo, not brewboard).
