---
schedule: weekly
name_pattern: stale-entity-sweep-{{YYYY-WW}}
recurring_slug: weekly-stale-entity-sweep
complexity: low
---

# stale-entity-sweep-{{YYYY-WW}}

## Summary

Scan inbox and backlog specs for features and research topics that have been sitting untouched for more than 3 weeks. Scan in-progress features for those with no commits in the last 7 days. Write a triage report to `.aigon/reports/stale-entities-{{YYYY-WW}}.md`. Close without evaluation when complete.
...
- [ ] Write findings to `.aigon/reports/stale-entities-{{YYYY-WW}}.md` with:
...
4. Write `.aigon/reports/stale-entities-{{YYYY-WW}}.md`
