---
schedule: monthly
name_pattern: Competitive Scan {{YYYY-MM}}
...
## Summary
Perform a monthly scan of the competitive landscape in agent-orchestration, spec-driven, and multi-agent harness spaces. This involves scanning GitHub releases, vendor blogs, Hacker News, and Reddit for key movements. The output will be a draft patch under `docs/competitive/scans/{{YYYY-MM}}.md` summarizing findings and proposing cell flips on `docs/competitive/matrix.md`.
...
### Step 4: Generate Output File `docs/competitive/scans/{{YYYY-MM}}.md`

1.  Create a new markdown file at `docs/competitive/scans/{{YYYY-MM}}.md`.
...
1.  After generating `docs/competitive/scans/{{YYYY-MM}}.md`, compare its content (especially the "Proposed Matrix Patch") with the previous month's scan (if available).
2.  If no material changes are detected in the landscape (e.g., no significant "New Tools", "Changed Tools", "Benchmark Updates", and an empty "Proposed Matrix Patch"), then:
    *   Write "no material changes; matrix unchanged" into the summary section of `docs/competitive/scans/{{YYYY-MM}}.md`.
    *   Immediately close this feature.
3.  If material changes are detected, keep the feature open for human review.

### Step 6: Commit Changes

1.  Commit the newly created `docs/competitive/scans/{{YYYY-MM}}.md` file.
2.  If the feature was immediately closed due to no material changes, ensure the commit message reflects this (e.g., "chore: Competitive Scan {{YYYY-MM}} - no material changes").
