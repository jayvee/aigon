---
schedule: weekly
name_pattern: workflow-state-integrity-{{YYYY-WW}}
recurring_slug: weekly-workflow-state-integrity
complexity: low
---

# workflow-state-integrity-{{YYYY-WW}}

## Summary

Run `aigon doctor` across all registered repos and write a state-health report to `.aigon/reports/workflow-state-{{YYYY-WW}}.md`. Surface snapshotless inbox entities, slug/numeric mismatches, and port conflicts without applying fixes. Close without evaluation when complete.
...
- [ ] Write findings to `.aigon/reports/workflow-state-{{YYYY-WW}}.md` with:
...
4. Write `.aigon/reports/workflow-state-{{YYYY-WW}}.md`
