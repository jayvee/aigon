...
complexity: high
agent: cc
recurring_week: 2026-W29
recurring_template: weekly-model-catalog-intelligence.md
transitions:
  - { from: "inbox", to: "backlog", at: "2026-07-13T11:34:51.540Z", actor: "recurring/feature-prioritise" }
---

# model-catalog-intelligence-2026-W29
...
- [ ] Commit the applied changes as `chore(models): refresh op catalog 2026-W29`.
...
- [ ] Write `.aigon/reports/model-catalog-intelligence-2026-W29.md` using the template below.
...
Write to `.aigon/reports/model-catalog-intelligence-2026-W29.md`:
...
- [ ] `chore(models): refresh op catalog 2026-W29` — only if patches approved
...
3. **Diff** — set algebra on values; apply known alias map (maintain in report until codified in Pro). For retirement classification, call `lib/model-catalog-diff.js` (`buildOpenRouterCatalogIndex` + `classifyModelOptions` + `formatRetireCandidateReport`) so each existing `op` `modelOptions` row is tagged `active`, `retire-candidate`, `archive-candidate`, or `unchanged` with proposed `quarantined` / `archived` blocks — never silent deletes.
...
- Write `.aigon/reports/model-catalog-intelligence-2026-W29.md`.
