...
- [ ] Server-injected data is still emitted by `buildDashboardHtml`, but the inline bootstrap uses a `window.__AIGON_BOOTSTRAP__ = { initialData, instanceName, agents, defaultAgent }`-style object (or equivalent `globalThis` property) that modules can reliably read. `js/injected.js` is the only module that reads that global and re-exports typed values; no other module touches `window.__AIGON_*` directly.
- [ ] The Pro stub-override mechanism keeps working: `templates/dashboard/stubs/*.js` (backup-sync, benchmark-matrix, insights-dashboard, pro-reports) — trace how the server chooses stub vs Pro implementation when serving these paths, and preserve that swap under ESM. The served OSS stubs and Pro-provided scripts must expose the same module contract (`export` names or default adapter) so dynamic script injection such as `logs.js` loading `/js/pro-reports.js` is removed or converted to `import()`. Verify both with and without aigon-pro installed, or with `?forcePro=0`.
...
- [ ] Circular dependencies: `state ↔ api ↔ init` style cycles must be broken by design (e.g. `render()` currently called from `api.js` — pass callbacks, use a tiny event emitter, or import from a dispatcher module), not papered over with `window.` escape hatches. Zero `window.<fn>` cross-module calls remain except documented Alpine/DOM-attribute boundaries and intentional third-party library globals (`Alpine`, `Terminal`, Chart/marked until dash-arch-8/compat work changes them).
...
- [ ] ESLint config for `templates/dashboard/js/**` switches to `sourceType: "module"` with `no-undef` enforced; the large classic-script cross-file global allowlist in `eslint.config.js` is deleted or reduced to documented third-party/injected globals only. `npm run test:core` lint stage passes.
