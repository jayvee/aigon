...
transitions:
  - { from: "inbox", to: "backlog", at: "2026-06-24T13:04:52.734Z", actor: "cli/feature-prioritise" }
---
...
- [ ] The dashboard set card exposes a server-owned valid action labeled "Revise Set Specs" when the set contains at least one eligible member with pending spec reviews. Add this to `buildSetValidActions` in `lib/feature-set-workflow-rules.js` by threading a new count field into the `setState` object — mirror F583's existing `reviewableMemberCount` / `launchableSpecReviewMemberCount` pattern with a parallel field (e.g. `pendingSpecReviseMemberCount`) populated by a dedicated helper at the producer (`lib/dashboard-status-collector.js:331`, where `setState` is built for `buildSetValidActions`). Do not reuse F583's review counts and do not branch on member state in frontend code.
- [ ] Because the valid-action count is computed server-side before any agent is selected, it is agent-agnostic, while the active revision set (AC: same-agent skip) is only knowable after the author picks the revision agent. The action may therefore be exposed even though the chosen agent authored every pending review. In that case the launched command must degrade gracefully — report "no eligible members for the selected revision agent" and make no commits — rather than erroring or revising the agent's own reviews. This mirrors F583's reviewable-vs-launchable split.
...
- [ ] Tests cover command validation, member ordering, pending-review detection, the same-agent skip and the "selected agent authored all pending reviews → no-op, no commits" degradation path, prompt payload shape, per-member acknowledgement/record behavior, and dashboard valid-action exposure.
...
- For dashboard support, extend the server-owned set action registry (`buildSetValidActions`) and API/action dispatch path. Thread the pending-revision count into the `setState` object at its producer (`lib/dashboard-status-collector.js:331`) alongside F583's existing counts, computed by a dedicated helper next to `countReviewableSetMembers`/`countLaunchableSetSpecReviewMembers`. The frontend should render the action from `validActions` and pass the selected agent triplet; eligibility must be computed server-side.
...
<svg xmlns="http://www.w3.org/2000/svg" width="568" height="132" viewBox="0 0 568 132" role="img" aria-label="Feature dependency graph for feature 585" style="font-family: system-ui, -apple-system, sans-serif"><defs><marker id="dep-arrow-585" markerWidth="10" markerHeight="8" refX="9" refY="4" orient="auto"><path d="M0,0 L10,4 L0,8 Z" fill="#94a3b8"/></marker></defs><path d="M 244 66 C 284 66, 284 66, 324 66" fill="none" stroke="#94a3b8" stroke-width="2" marker-end="url(#dep-arrow-585)"/><g><rect x="24" y="24" width="220" height="84" rx="12" ry="12" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/><text x="36" y="48" font-size="14" font-weight="700" fill="#0f172a">#583</text><text x="36" y="70" font-size="13" font-weight="500" fill="#1f2937">feature set wide spec rev…</text><text x="36" y="90" font-size="12" fill="#475569">done</text></g><g><rect x="324" y="24" width="220" height="84" rx="12" ry="12" fill="#dbeafe" stroke="#f59e0b" stroke-width="3"/><text x="336" y="48" font-size="14" font-weight="700" fill="#0f172a">#585</text><text x="336" y="70" font-size="13" font-weight="500" fill="#1f2937">feature set wide spec rev…</text><text x="336" y="90" font-size="12" fill="#475569">in-progress</text></g></svg>
<!-- AIGON_DEP_GRAPH_END -->
