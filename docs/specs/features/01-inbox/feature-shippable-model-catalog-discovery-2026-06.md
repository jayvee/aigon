...
recurring_template: monthly-shippable-model-catalog-discovery.md
one_off: true
---

# shippable-model-catalog-discovery-2026-06

## Summary

**One-off test run** of the monthly recurring template
`monthly-shippable-model-catalog-discovery.md`. Same workflow as the scheduled
monthly job: discover models from provider catalogues, diff against the OSS
shippable list, probe candidates, and produce **recommendations only** (no direct
`templates/agents/*.json` edits).

Use this inbox copy to validate the workflow end-to-end before the recurring
poller creates future instances automatically on the 1st of each month.
...
- [ ] Write `catalog-proposed.json` and `catalog-report.md` under `~/src/aigon/.aigon/matrix-refresh/2026-06/`.
...
- [ ] Commit from `~/src/aigon`: `git add .aigon/matrix-refresh/2026-06/ && git commit -m "chore: shippable model catalog discovery 2026-06 (one-off test)"` — **do not** commit `templates/agents/*.json` changes in this pass.
...
- `pricing-drift` — API pricing differs from catalog by >10%
...
6. Create triage items; commit **only** `.aigon/matrix-refresh/2026-06/`.
...
```bash
test -f ~/src/aigon/.aigon/matrix-refresh/2026-06/catalog-proposed.json
test -f ~/src/aigon/.aigon/matrix-refresh/2026-06/catalog-report.md
node -e "const j=require(process.env.HOME+'/src/aigon/.aigon/matrix-refresh/2026-06/catalog-proposed.json'); if(!Array.isArray(j.changes)) process.exit(1)"
```
...
- May commit only `.aigon/matrix-refresh/2026-06/` under `~/src/aigon`

## Open Questions

- Should `pricing-drift` rows be dropped from this monthly pass and left to `weekly-agent-matrix-pricing-refresh`? (Default: include but mark `deferTo: weekly-agent-matrix-pricing-refresh` in rationale.)
- When restoring Pro `bench-refresh`, should this recurring job call `--dry-run` only and let the CLI own JSON shape? (Default: keep agent-driven until CLI ships.)

## Related

- Recurring template: `docs/specs/recurring/monthly-shippable-model-catalog-discovery.md`
- OSS policy: `~/src/aigon/docs/model-inclusion-policy.md`
- F503 / F537: discovery lived in `bench-refresh`, removed from OSS user surface
- Matrix recurring family: pricing (weekly), benchmark (weekly), qualitative (quarterly)
