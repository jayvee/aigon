...
Emit OpenCode slash commands like Claude does. Install flat aigon-*.md files under .opencode/commands (same templates as cc) while retaining the existing skill tree at .agents/skills as secondaryOutput. Refactor install-agent to iterate every output spec on op.json so agents can ship multiple targets without one-off logic. Rebases the intent of f4727b8e from the abandoned feat/dashboard-drive-tool-label branch onto the new lib/commands/setup/ submodule layout (304eff8f). Note: branch's op.json modelOptions changes are stale and should NOT be carried over — main has revised modelOptions with pricing/scores/lastRefreshAt.
