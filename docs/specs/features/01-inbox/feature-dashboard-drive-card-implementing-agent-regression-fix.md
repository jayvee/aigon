...
Restore driveToolAgentId on Drive cards by inferring the implementing CLI from do-* tmux sessions and per-agent status files when the engine row is synthetic solo. Surface driveToolAgentId in the dashboard API and render it next to Drive on kanban cards. REGRESSION: solo_branch Drive cards hide which agent is implementing. Cherry-picks the intent of f8dcd0e7 from the abandoned feat/dashboard-drive-tool-label branch (now stale due to lib/commands/setup.js submodule split).
