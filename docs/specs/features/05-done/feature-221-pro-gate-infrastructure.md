...
Feature 226 (`pro-availability-is-global-not-project-scoped`) corrected an architectural mistake in this spec. AC6, AC7, and AC11 above describe a `forcePro` key in **project** config (`.aigon/config.json`) as the override mechanism. That was wrong: Pro availability is a property of the aigon install, not of any individual repo. A per-repo override produces incoherent UX — during the 2026-04-06 manual test of this gate, flipping `forcePro: false` in brewboard caused the gate to fire when clicking "Start Autonomously" on a brewboard feature (subprocess `cwd: ~/src/brewboard`) but the dashboard top-nav "Insights PRO" tab still showed as Pro-enabled (server `cwd: ~/src/aigon`, `forcePro: true`).

The corrected mechanism is the **`AIGON_FORCE_PRO` environment variable** (per feature 226):
- `AIGON_FORCE_PRO=false` (or `0`) — simulate the free tier even when `@aigon/pro` is installed
- `AIGON_FORCE_PRO=true` (or `1`) — no effect (Pro still requires the package)
- Unset / anything else — fall back to package availability

`lib/pro.js` no longer reads `loadProjectConfig()`. The `forcePro` key has been removed from project configs the team controls; `lib/config.js` emits a soft warning when it sees the key in any project config.

The original ACs above are preserved unchanged as a historical record of what 221 shipped, including this mistake. See `docs/specs/features/03-in-progress/feature-226-pro-availability-is-global-not-project-scoped.md` for the corrected design.
