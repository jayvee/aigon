...
# Feature: aigon-vault — unified backup & sync (F712)

> Replaces `aigon sync` (F708) and supersedes the earlier F711 "aigon-profile-sync" framing.
## Summary
...
- F708 (aigon-state-sync) — superseded by this feature; `aigon sync` becomes a deprecated alias.
