...
transitions:
  - { from: "inbox", to: "backlog", at: "2026-04-28T13:06:57.963Z", actor: "cli/feature-prioritise" }
---

# Feature: benchmark-token-axis-and-judge
...
Add a token-usage and quality axis to perf-bench. Two coupled but separable pieces: (A) Token-usage axis for perf-bench: readBenchmarkTelemetryUsage() in lib/perf-bench.js aggregates input/cached-input/output/thinking tokens, billable, costUsd, sessions, and model from .aigon/telemetry/feature-<id>-*.json per pair runner; lib/telemetry.js parseCodexTranscripts/findCodexSessionFiles accept an afterMs cutoff so a benchmark run only counts Codex sessions started after the bench began (avoids double-counting older sessions); .bench-token-line in templates/dashboard/styles.css renders tokens as a second line beneath each bench-cell time. (B) LLM judge rubric: lib/benchmark-judge.js defines IMPLEMENTATION_RUBRIC_V1 with 5 weighted criteria (requirements 0.35, correctness 0.25, minimality 0.15, code_quality 0.15, risk 0.10) on a 0-10 scale for grading what each agent produced. Currently uncommitted on main with no spec; this spec retroactively captures the work-in-progress so it has a closeout trail. (A) is partially live (8 brewboard-07 artifacts emitted today); (B) is defined with tests but not yet wired into perf-bench.
