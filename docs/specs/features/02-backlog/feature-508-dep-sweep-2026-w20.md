...
transitions:
  - { from: "inbox", to: "backlog", at: "2026-05-10T20:59:14.070Z", actor: "recurring/feature-prioritise" }
---
...
Run `npm audit` and `npm outdated` and write the findings to `docs/reports/dep-sweep-2026-W20.md`. Close without evaluation when complete.

## Acceptance Criteria

- [ ] Run `npm audit --json` and capture the output
- [ ] Run `npm outdated` and capture the output
- [ ] Write findings to `docs/reports/dep-sweep-2026-W20.md` with sections for audit results and outdated packages
...
3. Write `docs/reports/dep-sweep-2026-W20.md` with:
   - Summary: total vulnerabilities by severity
   - Full `npm audit` output
   - Full `npm outdated` output
4. `git add docs/reports/ && git commit -m "chore: dep-sweep report 2026-W20"`
