...
- [ ] `alpinejs` (3.x), `marked`, `chart.js` (4.x UMD), and `chartjs-adapter-date-fns` (bundle) live under `templates/dashboard/js/vendor/<lib>/` with exact versions recorded (a small `VERSIONS.md` or version-suffixed filenames — follow whatever the xterm vendoring did).
...
- [ ] Licenses: each vendored lib's license file/header ships alongside it (MIT for all four — include the license text as xterm vendoring presumably does; if it doesn't, fix that here for xterm as well).
...
- Copy minified dist files from the pinned npm packages (fetch once at development time; do NOT add them as runtime `dependencies` — they are static assets, matching the xterm precedent).
