...
transitions:
  - { from: "inbox", to: "backlog", at: "2026-04-26T03:32:09.096Z", actor: "recurring/feature-prioritise" }
---

# agent-matrix-pricing-refresh-2026-W17

## Summary

Scan vendor pricing pages and release notes to detect changes since the last matrix refresh.
Produce a reviewable patch file at `.aigon/matrix-refresh/{{YYYY-MM-DD}}/proposed.json` and
...
- [ ] Write `.aigon/matrix-refresh/{{YYYY-MM-DD}}/proposed.json` with the structured patch (see format below)
...
- [ ] Commit: `git add .aigon/matrix-refresh/ && git commit -m "chore: agent-matrix pricing refresh {{YYYY-MM-DD}}"`
...
- The patch file path: `.aigon/matrix-refresh/{{YYYY-MM-DD}}/proposed.json`
