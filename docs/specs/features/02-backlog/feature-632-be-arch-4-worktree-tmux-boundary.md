...
- [ ] As an implementing agent, "the only module doing tmux mechanics is `TmuxSessionHost`" (the F554 contract) is actually true, and the be-arch-1 guard keeps it true.
...
- [ ] Tmux mechanics: every direct tmux invocation in `worktree.js` moves behind `TmuxSessionHost` (`lib/agent-sessions/hosts/tmux.js`). `worktree.js` retains, at most, thin deprecated re-exports for external callers (37 importers — keep the facade, mark each re-export `@deprecated` pointing at the new home). Grep proves no `tmux` binary invocation outside `hosts/tmux.js` (encode as a be-arch-1 boundary rule or a targeted test).
...
- [ ] Baseline shrink: worktree-related cycles/violations removed from the be-arch-1 baseline. AGENTS.md module map rows for `worktree.js`, `agent-sessions/`, `terminal-adapters.js`, `agent-launch` updated.
