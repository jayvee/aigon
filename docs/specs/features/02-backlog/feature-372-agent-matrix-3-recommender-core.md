...
set: agent-matrix
transitions:
  - { from: "inbox", to: "backlog", at: "2026-04-26T01:30:55.459Z", actor: "cli/feature-prioritise" }
---

# Feature: agent-matrix-3-recommender-core
...
## Summary
Phase B core of the agent-matrix research. Extend `lib/spec-recommendation.js` with `rankAgentsForOperation(op, complexity, { excludeOverBudget = false, excludeQuarantined = true })` returning a ranked list of `{ agentId, model, effort, score, rationale, confidence }`. Score = qualitative score (1–5 from `cli.modelOptions[].score.<op>`) − cost penalty (normalised public-API $/op from `stats-aggregate.perTriplet`). Sparse cells (zero benchmark sessions) fall back to qualitative score with `confidence: 'low'` and an honest `rationale: 'no benchmark data — qualitative only'`. Never invent numbers to fill empty cells. The `excludeOverBudget` filter is a no-op until `feature-agent-cost-awareness` ships.
