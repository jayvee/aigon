...
depends_on: [591]
transitions:
  - { from: "inbox", to: "backlog", at: "2026-06-25T08:12:06.185Z", actor: "cli/feature-prioritise" }
---
...
- [ ] All ~22 `getAllAgentIds()` call sites **and** every enumerator layered on top of it are audited and routed to the correct list: **all-known** (analytics, telemetry display, session-parse, `getAttributionAgentIds()`, and `entity-commands.js`'s `*-transcript` agent-arg validation — viewing a historic `gg` transcript must keep working) vs **launchable-only** (install, Fleet & start-modal pickers, workflow rosters, `agent-probe` defaults, `feature-start`/`feature-do`/`feature-eval` validation, `entity-commands.js`'s `resolveReviewAgentFromOptions`). **Layered enumerators that are easy to miss because they don't call `getAllAgentIds()` directly**: `getSortedAgentIds()` (exposed as `lib/templates.js`'s `getAvailableAgents()`, ~20 call sites — this is the actual agent-validation gate in `feature-do.js:253/453`, `feature-eval.js:88`, `feature-autonomous.js:1187`, `validation.js:349`, and `trust-worktree.js:16`, not `getAllAgentIds()`), `getAgentAliasMap()`, and `getDefaultFleetAgents()` — all built on `getAllAgents()`/`getAllAgentIds()` in `lib/agent-registry.js`.
...
- **Call-site audit** — the bulk of the work. Triage every `getAllAgentIds()` caller (~22) and each hardcoded roster/list into all-known vs launchable-only. Known launchable-only sites: `lib/workflow-definitions.js:47` (implement roster), `lib/config.js:189` (auto-detection), `lib/commands/misc.js:1590` + `scripts/probe-agent.js:248` (probe defaults), install + Fleet/start-modal pickers, `feature-start`/`feature-do`/`feature-eval` validation.
- **Layered enumerators — do not stop at `getAllAgentIds()` callers.** `lib/templates.js:196` (`getAvailableAgents()`) wraps `agentRegistry.getSortedAgentIds()` (`agent-registry.js:677`, itself built on `getAllAgents()`), not `getAllAgentIds()` directly — it is the validator actually invoked by `feature-do.js`, `feature-eval.js`, `feature-autonomous.js`, `validation.js`, and `trust-worktree.js`, so it must become launchable-only or `feature-start … gg` will keep succeeding through this path even after `getAllAgentIds()` callers are fixed. Conversely, `entity-commands.js`'s `*-transcript` agent-arg check also calls `getAvailableAgents()` to validate a *historic* lookup — if `getAvailableAgents()` is made launchable-only wholesale, that site breaks the "historic telemetry stays readable" criterion and needs its own all-known list instead. Also audit `getAgentAliasMap()` and `getDefaultFleetAgents()` (same `getAllAgents()` base).
...
<svg xmlns="http://www.w3.org/2000/svg" width="568" height="132" viewBox="0 0 568 132" role="img" aria-label="Feature dependency graph for feature 592" style="font-family: system-ui, -apple-system, sans-serif"><defs><marker id="dep-arrow-592" markerWidth="10" markerHeight="8" refX="9" refY="4" orient="auto"><path d="M0,0 L10,4 L0,8 Z" fill="#94a3b8"/></marker></defs><path d="M 244 66 C 284 66, 284 66, 324 66" fill="none" stroke="#94a3b8" stroke-width="2" marker-end="url(#dep-arrow-592)"/><g><rect x="24" y="24" width="220" height="84" rx="12" ry="12" fill="#dbeafe" stroke="#2563eb" stroke-width="2"/><text x="36" y="48" font-size="14" font-weight="700" fill="#0f172a">#591</text><text x="36" y="70" font-size="13" font-weight="500" fill="#1f2937">add antigravity agent</text><text x="36" y="90" font-size="12" fill="#475569">in-progress</text></g><g><rect x="324" y="24" width="220" height="84" rx="12" ry="12" fill="#e5e7eb" stroke="#f59e0b" stroke-width="3"/><text x="336" y="48" font-size="14" font-weight="700" fill="#0f172a">#592</text><text x="336" y="70" font-size="13" font-weight="500" fill="#1f2937">retire gg deactivated age…</text><text x="336" y="90" font-size="12" fill="#475569">backlog</text></g></svg>
<!-- AIGON_DEP_GRAPH_END -->
