...
Decide and implement a policy for the .aigon/benchmarks/*.json artifacts produced by perf-bench runs. Open questions: (a) commit-vs-gitignore — currently 70+ historical files are tracked but new runs accumulate fast and bloat the repo; (b) retention — keep all-time, last-N, or rotate by date? (c) dashboard read path — Pro reads aggregate all-<seed>-*.json plus per-pair files; whatever policy ships must keep that read path working; (d) cleanup CLI — should aigon perf-bench have a --prune or --keep-last=N flag or a separate aigon perf-bench-prune subcommand? (e) where do quality-judged JSONs live (same dir, same shape with quality:: field appended). Picks one approach and implements it. Triggered by 8 untracked artifacts left in working tree after 2026-04-28 release prep.
