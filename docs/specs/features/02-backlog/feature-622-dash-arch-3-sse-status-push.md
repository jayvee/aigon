...
- [ ] `GET /api/events` streams `text/event-stream` with: `retry:` hint, an initial `status` event carrying the current `statusVersion`, a comment heartbeat every ~25s (keeps proxies from idling the connection), and correct connection cleanup on client disconnect (no leaked response objects; verify with repeated open/close).
- [ ] Every `statusVersion` bump (interval poll, watcher-triggered `pollRepoStatus`, `/api/refresh`, action-triggered refresh) broadcasts `event: status`, `data: {"statusVersion": N}` to all connected clients. No full payload over SSE — clients fetch `/api/status` with `If-None-Match`, reusing the dash-arch-1 path and its gzip/serialization cache.
