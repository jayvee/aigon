...
- Current completion wording uses `implementation-complete` and `research-complete`; `submitted` remains only in the explicit compatibility mapping in the agent-status reference.
...
- Final pass: source regression guards, `npm run build --prefix site`, `node tests/unit/dashboard-card-gallery.test.js`, and `npm run test:gallery` passed (25 unit assertions and 16 gallery checks, including mobile layout).
