---
status: submitted
updated: 2026-03-16T04:49:46.325Z
events:
...
- **Radar restart issue**: Initial test showed missing data because `aigon radar stop/start` only restarts the process tracked in the current PID file. The main radar daemon (port 4100) was a separate process that needed to be killed explicitly. Verified with `ps aux | grep "node.*aigon"` before/after.
