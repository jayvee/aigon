...
- Fixed frontmatter carry-over bug in `lib/recurring.js:createAndPrioritiseFromTemplate` — the instance spec was stripped of all template frontmatter (losing `complexity`, etc.) instead of carrying over non-template-specific fields; now preserves `complexity`, `recurring_slug`, etc. while excluding only `schedule` and `name_pattern` (legacy `recommended_models` keys were later removed from the product; specs are complexity-only).
