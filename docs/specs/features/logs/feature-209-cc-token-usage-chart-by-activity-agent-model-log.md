...
- Added stacked area chart to `insights-dashboard.js` (aigon-pro) with:
...
- All syntax checks pass (`node -c` on utils.js, dashboard-server.js, insights-dashboard.js)

## Decisions

- Chart is in aigon-pro (Insights tab) per spec's suggestion — not in free tier
...
- Cross-repo: data pipeline in aigon `lib/utils.js`, chart rendering in aigon-pro `dashboard/insights-dashboard.js`
