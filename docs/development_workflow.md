...
Recurring features are template specs in `docs/specs/recurring/` that the system automatically instantiates and prioritises on a schedule.

### Template format

Each template is a standard feature spec with extra frontmatter:

```markdown
---
schedule: weekly          # only 'weekly' in v1
name_pattern: dep-sweep-{{YYYY-WW}}  # rendered with ISO year+week
recurring_slug: weekly-dep-sweep     # stable dedupe key, unique across templates
---
...
- `schedule: weekly` — the template runs once per ISO week (Mon–Sun, ISO 8601)
- `name_pattern` — supports `{{YYYY-WW}}` (e.g. `2026-W17`); the rendered value becomes the feature name
- `recurring_slug` — must be unique across all templates; each created instance carries this in its frontmatter so the dedupe check doesn't rely on filenames

### How it works

On server startup (and every 24 hours), Aigon checks each template in `docs/specs/recurring/`:

1. If an open instance (in inbox, backlog, in-progress, or in-evaluation) with the same `recurring_slug` exists, skip.
2. If the current ISO week matches the `lastWeek` in `.aigon/recurring-state.json`, skip.
3. Otherwise, clone the template into `docs/specs/features/01-inbox/`, render `name_pattern`, and immediately prioritise it (assigns an ID, moves to backlog).

Done (`05-done/`) and paused (`06-paused/`) instances do not block creation — a closed or abandoned instance lets the new week's instance be created.

### CLI commands

```bash
aigon recurring-run   # run the due-check now (same logic as the background scheduler)
aigon recurring-list  # show each template's slug, schedule, last created week, and due/not-due status
```

### Adding a custom template

1. Create `docs/specs/recurring/my-task.md` with `schedule: weekly`, `name_pattern`, and `recurring_slug` frontmatter.
2. Write the standard feature spec body (Summary, Acceptance Criteria, etc.).
3. Run `aigon recurring-list` to confirm it appears and shows as due.
4. Run `aigon recurring-run` to create it immediately, or wait for the next server check.

Templates in `docs/specs/recurring/` are excluded from the Kanban board and dashboard discovery — they are input templates only, not features.
