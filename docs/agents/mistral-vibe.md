...
| `aigon agent-status submitted` | Signal implementation complete after committing code and updating the log |
...
4. **Complete properly**: Use `/aigon-feature-close <ID>` for Drive, `/aigon-feature-close <ID> mv` for Fleet
...
### Prerequisites
- **`vibe` CLI**: Install with `pip install mistral-vibe`
- **`MISTRAL_API_KEY`**: Required environment variable. Get your API key from [Mistral La Plateforme](https://console.mistral.ai) and set it in your shell profile:
  ```bash
  export MISTRAL_API_KEY="your-api-key-here"
  ```
  Or run `vibe --setup` to configure interactively.

### Model
Mistral Vibe uses **Devstral 2** as its default model. No model selection flags are needed.
